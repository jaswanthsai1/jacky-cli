...
Backend is selected during ``jacky tools`` setup (web.backend in config.yaml).
When available, Jacky can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Jacky config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Jacky' config/.env layer (``jacky config set``, ``jacky tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.jacky/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``jacky tools``).
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `jacky tools` repaint.
        try:
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_jacky_dir("cache/web", "web_cache")
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `jacky plugins enable {disabled_key}` "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`jacky plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from jacky_cli.config import get_env_value as _gev

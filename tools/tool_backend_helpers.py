...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `jacky model` to refresh your "
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.jacky/.env`` (via
    ``jacky_cli.config.get_env_value`` when available) so tool-side

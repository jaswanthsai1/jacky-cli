...
# Cache validation (mirrors jacky_cli/profiles.py::_count_skills, d5eee133e):
...
# All skills live in ~/.jacky/skills/ (seeded from bundled skills/ on install).
...
def _skills_dir() -> Path:
    ...
    set JACKY_HOME. Keep the legacy SKILLS_DIR module attribute for tests and
    external patchers, but when it has not been patched, resolve from the live
    profile-scoped JACKY_HOME on every call.
    """
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from JACKY_HOME/.env."""
...
def _capture_required_environment_variables(
    ...
    # — the desktop app / TUI — set JACKY_INTERACTIVE and register a
    # secret-capture callback that routes to a secure secret.request overlay, so
    # they fall through and actually prompt. (JACKY_INTERACTIVE is the same flag
    # tools/approval.py uses to tell an interactive surface from a messaging one.)
    if _is_gateway_surface() and not env_var_enabled("JACKY_INTERACTIVE"):
        return {
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("JACKY_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_jacky_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.jacky/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``JACKY_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``JACKY_PLATFORM`` environment variable
    3. ``JACKY_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from jacky_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("JACKY_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.jacky/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``jacky skills`` config UI). Default False
    ...
    # (_skills_dir() resolves the LIVE profile JACKY_HOME; the module-level
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_jacky_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from jacky_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: jacky plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.jacky/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `jacky skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.jacky.* first (agentskills.io convention), fall back to top-level
        jacky_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            jacky_meta = metadata.get("jacky", {}) or {}

        tags = _parse_tags(jacky_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            jacky_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )

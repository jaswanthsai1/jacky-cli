...
~/.jacky/skills/. Existing skills (bundled, hub-installed, or user-created)
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `jacky config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.jacky/skills/ (single source of truth)
...
def _skills_dir() -> Path:
    ...
    kanban workers) import this module once under the launch JACKY_HOME and
    ...
    live profile-scoped JACKY_HOME on every call.
    """
    ...
    try:
        ...
        if rec.get("pinned"):
            return (
                ...
                f"`jacky curator unpin {name}` if they want to delete it. "
        ...
        if skill_usage.get_record(name).get("pinned"):
            return {
                ...
                "error": (
                    ...
                    f"`jacky curator unpin {name}` if they want it changed."
                ),
            }
    except Exception:
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.jacky/skills/) first, then any
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    """Look for ``name`` under SKILL.md across OTHER Jacky profiles.
    ...
    # Default profile (~/.jacky/skills) — only consider when active is non-default.
    ...
    # All named profiles (~/.jacky/profiles/*/skills)
    ...
    if others:
        if len(others) == 1:
            ...
            base += (
                ...
                f"another profile, switch profiles (`jacky -p "
            ...
            base += (
                ...
                f"Switch profiles (`jacky -p <name>`) to edit there, or "
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    # RECOVERABLE: archival into ~/.jacky/skills/.archive/ is documented as
    # the maximum destructive action the curator may take, and
    # `jacky curator restore` promises the skill can be brought back. Route
    ...
    if result.get("success"):
        ...
        try:
            ...
            if action == "create":
                ...
                # keeps its usage record as STATE_ARCHIVED so `jacky curator
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_jacky_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "will refuse with a message pointing the user to `jacky curator unpin <name>`. "

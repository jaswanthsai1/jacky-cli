...
One ``CDPSupervisor`` runs per Jacky ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Jacky logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "jacky-dialog-bridge.invalid"
...
(() => {
  if (window.__jackyDialogBridgeInstalled) return;
  window.__jackyDialogBridgeInstalled = true;
  const ENDPOINT = "http://jacky-dialog-bridge.invalid/";

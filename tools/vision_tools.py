...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("JACKY_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: JACKY_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("JACKY_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _normalize_to_supported_image(
    ...
    out_dir = get_jacky_dir("cache/vision", "temp_vision_images")
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_jacky_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $JACKY_HOME/cache/vision/ and cleaned up
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_jacky_dir("cache/video", "temp_video_files")

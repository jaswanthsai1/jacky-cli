...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.jacky/config.yaml``. Jacky
...
Configuration is loaded from ~/.jacky/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``jacky_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from jacky_cli.config import get_env_value as _get_env_value
    except ImportError:
...
from tools.xai_http import jacky_xai_user_agent
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.jacky/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.jacky/config.yaml.
    ...
    try:
        ...
        logger.debug("jacky_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
# custom voice-cloning scripts, etc.) into Jacky without any Python code
...
#           output_format: wav
#
# Jacky writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__JACKY_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `jacky model` or set XAI_API_KEY.")
    ...
    # output_format when Jacky actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": jacky_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Jacky caches Piper voice models.
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.jacky/config.yaml (tts: section).
    ...
    platform = get_session_env("JACKY_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'jacky-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run jacky setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'jacky setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'jacky tools' and select Piper under TTS, or install manually: "
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_jacky_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }

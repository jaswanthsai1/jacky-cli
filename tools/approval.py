...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("JACKY_YOLO_MODE", ""))
...
# os.environ["JACKY_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export JACKY_INTERACTIVE.
_jacky_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "jacky_interactive",
    default=None,
)


def set_jacky_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["JACKY_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``JACKY_INTERACTIVE`` env var for legacy callers.
    """
    return _jacky_interactive_ctx.set("1" if interactive else "")


def reset_jacky_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_jacky_interactive_context`."""
    _jacky_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``JACKY_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _jacky_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("JACKY_INTERACTIVE")
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("JACKY_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return get_session_env("JACKY_SESSION_PLATFORM", "") or ""
    except Exception:
        return os.getenv("JACKY_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set JACKY_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind JACKY_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds JACKY_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("JACKY_CRON_SESSION"):
        return False
    if env_var_enabled("JACKY_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $JACKY_HOME, or by the resolved absolute
# active profile home path such as /home/jacky/.jacky/config.yaml. The
# resolved-absolute form is folded into the ~/.jacky/ patterns at detection
...
# go stale when JACKY_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_JACKY_ENV_PATH = (
    r'(?:~\/\.jacky/|'
    r'(?:\$home|\$\{home\})/\.jacky/|'
    r'(?:\$jacky_home|\$\{jacky_home\})/)'
    r'\.env\b'
)
# ~/.jacky/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _JACKY_ENV_PATH; matches the JACKY_HOME override form as
# well as ~/.jacky/.
_JACKY_CONFIG_PATH = (
    r'(?:~\/\.jacky/|'
    r'(?:\$home|\$\{home\})/\.jacky/|'
...
DANGEROUS_PATTERNS = [
    ...
    # `jacky` and `gateway` (e.g. `jacky -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bjacky\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart jacky gateway (kills running agents)"),
    (r'\bjacky\s+update\b', "jacky update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `jacky gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart jacky-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart jacky-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(jacky|gateway|cli\.py)\b', "kill jacky/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill jacky` but not
    # `kill -9 $(pgrep -f jacky)` because the substitution is opaque
    ...
    # the `jacky gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.jacky.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(jacky|ai\.jacky)\b', "stop/restart jacky launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Jacky file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.jacky/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Jacky-managed security file (~/.jacky/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_JACKY_CONFIG_PATH}|{_JACKY_ENV_PATH})', "in-place edit of Jacky config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_JACKY_CONFIG_PATH}|{_JACKY_ENV_PATH})', "in-place edit of Jacky config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_JACKY_CONFIG_PATH}|{_JACKY_ENV_PATH})', "in-place edit of Jacky config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.jacky/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / JACKY_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Jacky home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\jacky), so folding the user home
    # first would eat the prefix the Jacky-home fold needs.
    command = _rewrite_resolved_jacky_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / JACKY_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.jacky``); the matched
...
def _rewrite_resolved_jacky_home(command: str) -> str:
    """Rewrite the resolved absolute Jacky home prefix to ``~/.jacky/``.

    Resolves the active ``JACKY_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.jacky/`` so the static ``_JACKY_CONFIG_PATH`` / ``_JACKY_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/jacky/.jacky/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$JACKY_HOME``. Matches both POSIX and Windows separators. No-op when the
    ...
    try:
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.jacky")
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    os.environ["JACKY_SPINNER_PAUSE"] = "1"
    try:
        ...
        if "JACKY_SPINNER_PAUSE" in os.environ:
            del os.environ["JACKY_SPINNER_PAUSE"]
        print()
        sys.stdout.flush()
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Jacky approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``JACKY_YOLO_MODE`` (frozen at import time
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            JACKY_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("JACKY_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set JACKY_INTERACTIVE or "
                "JACKY_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set JACKY_INTERACTIVE or "
            "JACKY_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("JACKY_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``JACKY_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("JACKY_EXEC_ASK")
        ...
        if env_var_enabled("JACKY_CRON_SESSION"):
            if _get_cron_approval_mode() == "deny":
                ...
                try:
                    ...
                    try:
                        from jacky_cli.config import load_config as _load_cfg

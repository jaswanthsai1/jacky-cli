...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``jacky tools`` repaint, tool-registration scans,
    ...
    2. ``~/.jacky/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.jacky/.env`` first, then ``os.environ``.

    Wraps :func:`jacky_cli.config.get_env_value` so tests can patch
    ``tools.xai_http.get_env_value`` to inject dotenv-only secrets into the
    xAI credential resolver.
    """
    try:
        from jacky_cli.config import get_env_value as _jacky_get_env_value

        value = _jacky_get_env_value(name)
...
def jacky_xai_user_agent() -> str:
    """Return a stable Jacky-specific User-Agent for xAI HTTP calls."""
    try:
        from jacky_cli import __version__
    except Exception:
        __version__ = "unknown"
    return f"Jacky-Agent/{__version__}"


def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Jacky config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Jacky home, then mark it seen."""
...
def resolve_xai_http_credentials(*, force_refresh: bool = False) -> Dict[str, str]:
    ...
    Prefers Jacky-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``jacky_cli.config.get_env_value`` so keys
    stored in ``~/.jacky/.env`` (the standard Jacky location) are honored —

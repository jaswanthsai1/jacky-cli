...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for Jacky-owned URL tools.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``JACKY_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("JACKY_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``JACKY_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.

...
-----------------------------------------------------------------------------
    ...
    ~/.jacky/checkpoints/
        store/                          — single bare-ish git repo
            ...
            refs/jacky/<hash16>        — per-project branch tip
...
CHECKPOINT_BASE = get_jacky_home() / "checkpoints"

# Single shared store directory under CHECKPOINT_BASE.
_STORE_DIRNAME = "store"
_REFS_PREFIX = "refs/jacky"
...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Jacky convention — don't recursively snapshot siblings)
    ".worktrees/",
...
_GIT_TIMEOUT: int = max(10, min(60, env_int("JACKY_CHECKPOINT_TIMEOUT", 30)))
...
def _git_env(
    ...
    The shared store is internal Jacky infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``jacky checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `jacky checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
def _init_store(store: Path, working_dir: str) -> Optional[str]:
    ...
    cfg_wd = str(base)
    _run_git(["config", "user.email", "jacky@local"], store, cfg_wd)
    _run_git(["config", "user.name", "Jacky Checkpoint"], store, cfg_wd)
...
# look for ``HEAD``/``info/exclude``/``JACKY_WORKDIR``.  In v2 we also write
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    # Compat marker for tests that look at JACKY_WORKDIR
    # (write in addition to the JSON metadata).
    try:
        (shadow_repo / "JACKY_WORKDIR").write_text(
...
# ---------------------------------------------------------------------------
# Public helpers for `jacky checkpoints` CLI
# ---------------------------------------------------------------------------

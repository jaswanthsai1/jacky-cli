...
(see ``jacky_cli`` slash handlers): memory shows full content, skills show
...
Pending records live under ``<JACKY_HOME>/pending/{memory,skills}/<id>.json``
...
def _pending_dir(subsystem: str) -> Path:
    return get_jacky_home() / "pending" / subsystem

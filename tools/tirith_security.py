...
it is automatically downloaded from GitHub releases to $JACKY_HOME/bin/tirith.
...
def _get_jacky_home() -> str:
    """Return the Jacky home directory, respecting JACKY_HOME env var."""
    return str(get_jacky_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_jacky_home(), ".tirith-install-failed")
...
def _jacky_bin_dir() -> str:
    """Return $JACKY_HOME/bin, creating it if needed."""
    d = os.path.join(_get_jacky_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $JACKY_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_jacky_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $JACKY_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $JACKY_HOME/bin/tirith
    ...
    jacky_bin = os.path.join(_jacky_bin_dir(), "tirith")
    if os.path.isfile(jacky_bin) and os.access(jacky_bin, os.X_OK):
        _resolved_path = jacky_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return jacky_bin

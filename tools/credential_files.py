...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.jacky",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``JACKY_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside JACKY_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    jacky_home = _resolve_jacky_home()

    # Reject absolute paths — they bypass the JACKY_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to JACKY_HOME)",
            relative_path,
        )
        return False

    host_path = jacky_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape JACKY_HOME.
    from tools.path_security import validate_within_dir

    containment_error = validate_within_dir(host_path, jacky_home)
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if resolved_path.is_file():
                        container_path = f"/root/.jacky/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="jacky-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching jacky_constants.get_jacky_dir().
...
def get_cache_directory_mounts(
    ...
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_jacky_dir(new_subpath, old_name)
        if host_dir.is_dir():
...
def map_cache_path_to_container(
    ...
    caller decides which ``container_base`` applies (Docker ``/root/.jacky``,
    SSH ``<remote_home>/.jacky``, etc.) and whether translation is wanted.

...
<url>``) is spawned as a direct child of the Jacky process. Jacky's own
...
Jacky process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Jacky startup's opt-in

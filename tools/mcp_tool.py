...
transport, discovers their tools, and registers them into the jacky-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.jacky/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.jacky/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Jacky' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            jacky_home = os.path.expanduser(
                os.getenv(
                    "JACKY_HOME", os.path.join(os.path.expanduser("~"), ".jacky")
                )
            )
            candidates = [
                os.path.join(jacky_home, "node", "bin", resolved_command),
                ...
                # image (which the Jacky Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Jacky MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Jacky gateways know how to render.
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Jacky' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (JACKY_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit JACKY_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Jacky' logging (agent.log via jacky_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'jacky-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'jacky-agent[all]'"
            )
        ...
        # ungraceful exit of this Jacky process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Jacky' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Jacky is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `jacky mcp login {server_name}` (or delete the tokens "
            f"file under ~/.jacky/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local JACKY_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_jacky_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_jacky_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local JACKY_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_jacky_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Jacky config file.
    ...
    ``os.environ`` (which includes ``~/.jacky/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Jacky' MEDIA: tag convention and out to messaging
            ...
            # Jacky' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Jacky's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Jacky uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Jacky registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``jacky tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Jacky registry.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple jacky

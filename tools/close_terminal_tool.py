#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Jacky desktop GUI.
...
``read_terminal`` it is gated on ``JACKY_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — JACKY_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("JACKY_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Jacky desktop GUI (the tabs mirroring terminal(background=true) runs). "

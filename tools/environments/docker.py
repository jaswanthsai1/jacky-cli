...
def _load_jacky_env_vars() -> dict[str, str]:
    """Load ~/.jacky/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Jacky profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale jacky-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=jacky-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Jacky process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=jacky-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a jacky process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Jacky
    process leaves its container behind permanently because there's no
    subsequent Jacky process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=jacky-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=jacky-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``JACKY_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("JACKY_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using JACKY_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'jacky'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``jacky-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.jacky/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. jacky-agent:latest) already use /init as PID 1
        ...
        container_name = f"jacky-{uuid.uuid4().hex[:8]}"
        # Labels make jacky-created containers identifiable to:
        #   * the orphan reaper (`jacky-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`jacky-task-id`, `jacky-profile`)
        #   * operators running `docker ps --filter label=jacky-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Jacky profile is captured at
        ...
        label_args = [
            "--label", "jacky-agent=1",
            "--label", f"jacky-task-id={task_label}",
            "--label", f"jacky-profile={profile_name}",
        ]
        ...
        self._labels = {
            "jacky-agent": "1",
            "jacky-task-id": task_label,
            "jacky-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Jacky process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Jacky secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Jacky-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_jacky_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_jacky_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _JACKY_PROVIDER_ENV_BLOCKLIST)
        jacky_env = _load_jacky_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = jacky_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("jacky-task-id", "")
        profile_label = self._labels.get("jacky-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"jacky-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``jacky-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=jacky-agent=1",
                    "--filter", f"label=jacky-task-id={task_label}",
                    "--filter", f"label=jacky-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True, encoding="utf-8", errors="replace",
        ...
        # container) but can happen if a previous Jacky process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Jacky exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Jacky process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Jacky startup. That covers the
        ...
        # outlives Jacky processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"jacky-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``jacky /quit``

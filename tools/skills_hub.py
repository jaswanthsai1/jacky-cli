#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Jacky Skills Hub.
...
Used by jacky_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _skills_dir() -> Path:
    ...
    return Path(forced) if forced is not None else _jacky_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            jacky_meta = metadata.get("jacky", {})
            if isinstance(jacky_meta, dict):
                tags = jacky_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    ~/.jacky/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                jacky_meta = meta_block.get("jacky", {})
                if isinstance(jacky_meta, dict):
                    tags = jacky_meta.get("tags", [])
...
def _jacky_index_cache_file() -> Path:
    return _index_cache_dir() / "jacky-index.json"


def _load_jacky_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for JACKY_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    jacky_index_cache_file = _jacky_index_cache_file()
    if jacky_index_cache_file.exists():
        try:
            age = time.time() - jacky_index_cache_file.stat().st_mtime
            if age < JACKY_INDEX_TTL:
                return json.loads(jacky_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Jacky index fetch returned %d", resp.status_code)
            ...
            logger.debug(
                "Jacky index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Jacky index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        jacky_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        jacky_index_cache_file.write_text(json.dumps(data))
    except OSError:
        pass
...
class JackyIndexSource(SkillSource):
    """Skill source backed by the centralized Jacky Skills Index.
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "jacky-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "jacky-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(auth: Optional[GitHubAuth] = None) -> List[SkillSource]:
    ...
    sources: List[SkillSource] = [
        ...
        JackyIndexSource(auth=auth), # Centralized index (search + resolved install paths)

...
`jacky skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.jacky.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.jacky.blueprint`` is a mapping containing
    ...
    jacky = meta.get("jacky") if isinstance(meta, dict) else None
    blueprint = jacky.get("blueprint") if isinstance(jacky, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.jacky.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.jacky.blueprint`` block) they can hand
    to ``jacky skills publish`` to share. ``body`` is the plain-language

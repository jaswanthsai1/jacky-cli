...
This module provides the glue:
    - ``JackyTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Jacky Agent"
"""
...
_USER_SKIPPED_SENTINEL = "__jacky_user_skipped__"
...
def _get_token_dir() -> Path:
    ...
    Uses JACKY_HOME so each profile gets its own OAuth tokens.
...
def _raise_if_non_interactive(lead: str) -> None:
    ...
    shared, actionable ``jacky mcp login`` next-step so the guidance wording
    lives in one place across every non-interactive OAuth boundary (#57836).
    """
    if not _is_interactive():
        raise OAuthNonInteractiveError(
            f"{lead} "
            "Run `jacky mcp login <server>` interactively to (re)authorize, "
...
# ---------------------------------------------------------------------------
# JackyTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class JackyTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        JACKY_HOME/mcp-tokens/<server_name>.json         -- tokens
        JACKY_HOME/mcp-tokens/<server_name>.client.json   -- client info
        JACKY_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # Jacky records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Jacky.</p></body></html>"
            ) if code else (
...
def _paste_callback_reader(result: dict) -> None:
    ...
    if line.lower() in _SKIP_TOKENS:
        ...
        print(
            "  OAuth skipped. Run `jacky mcp login <server>` later to "
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "Jacky Agent")
    scope = cfg.get("scope")
...
def build_oauth_auth(
    ...
    if not _is_interactive() and not storage.has_cached_tokens():
        raise OAuthNonInteractiveError(
            ...
            f"`jacky mcp login {server_name}` interactively first to complete "

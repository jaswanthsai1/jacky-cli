...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Jacky exit
...
# display_jacky_home imported lazily at call site (stale-module safety during jacky update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of jacky directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "jacky-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("JACKY_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("JACKY_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("JACKY_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (JACKY_INTERACTIVE=1).
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (JACKY_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Jacky password prompt or the sudo -S password-pipe path.
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Jacky can track lifecycle and output.
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``jacky-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Jacky processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Jacky processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Jacky processes a generous grace
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.jacky/.env or environment variables."
        )
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    expand itself, so the Jacky host/container must NOT ``expanduser`` it.

    SSH ``cwd`` is interpreted by the *remote* shell (``cd ~`` / ``cd ~/x``
    over ``ssh ... bash -c``). Expanding ``~`` locally would rewrite it to the
    Jacky host HOME (often ``/opt/data`` under Docker) and inject a
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # Startup orphan reaper for jacky-tagged containers left behind by
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        # prior Jacky processes that hit SIGKILL / OOM / a closed terminal
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Jacky can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/jacky
        # restart|stop targeting jacky-gateway) must never run inside the
        ...
        # never restart. This mirrors the `jacky gateway restart` guard in
        # jacky_cli/gateway.py and the cron-path guard in jacky_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_JACKY_GATEWAY") == "1":
            from jacky_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `jacky gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                # jacky-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in jacky-agent dev work (PRs #31329, "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("JACKY_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("JACKY_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("JACKY_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("JACKY_SESSION_USER_ID", "")
                            _gw_user_name = _gse("JACKY_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("JACKY_SESSION_MESSAGE_ID", "")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("JACKY_INTERACTIVE"):
                    output += (

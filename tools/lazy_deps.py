"""
Lazy dependency installer for opt-in Jacky Agent backends.

Many Jacky features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``jacky-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``jacky tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``JACKY_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/jacky`` read-only), setting
  ``JACKY_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Jacky, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (JACKY_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    "platform.discord": (
        ...
        "aiohttp==3.14.3",  # patched floor for CVE-2026-34513/34518/34519/34520/34525 + 34993(RCE)/47265
    ),
    "platform.slack": (
        "slack-bolt==1.27.0",
        "slack-sdk==3.40.1",
    ...
    "platform.teams": ("microsoft-teams-apps==2.0.13.4", "aiohttp==3.14.3"),  # aiohttp 3.14.3: patched floor for CVE-2026-34993(RCE)/47265 + 34513/34518/34519/34520/34525
    ...
    # HF Agent Trace Viewer upload (jacky trace upload / /upload-trace).
...
_LAZY_TARGET_ENV = "JACKY_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``JACKY_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("JACKY_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``jacky update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Jacky under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``jacky update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="jacky-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``jacky_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = jacky_subprocess_env(inherit_credentials=False)
...
def active_features() -> list[str]:
    ...
    Used by ``jacky update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``jacky update``. Never raises; lazy-install failures

...
# Must match jacky_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``jacky-cli`` (which bundles
...
def _build_child_agent(
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. jacky-cli) so that individual
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.jacky/logs/subagent-<sid>-<ts>.log``
...
def delegate_task(
    ...
    if background:
        ...
        try:
            ...
            _source = get_session_env("JACKY_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("JACKY_UI_SESSION_ID", "")
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'jacky auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Jacky config.

    Prefer the shared persistent loader because it follows the active
    JACKY_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``JACKY_IGNORE_USER_CONFIG=1`` (``jacky chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("JACKY_IGNORE_USER_CONFIG") == "1"
    if not prefer_legacy:
        try:
            from jacky_cli.config import load_config_readonly

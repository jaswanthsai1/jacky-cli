...
etc.) surface as specific blocked checks via `jacky computer-use doctor`
...
# fetches the latest release, so a `JACKY_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `JACKY_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("JACKY_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when Jacky should disable cua-driver telemetry for this user.
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of jaswanthsai1/jacky-cli#47072: instead of hardcoding
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or JACKY_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `jacky computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        f"{installer}\n"
        "Or run `jacky tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of jaswanthsai1/jacky-cli#47072: read the canonical
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of jaswanthsai1/jacky-cli#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Jacky-managed secrets out of the child env.
                env=_sanitize_subprocess_env(cua_driver_child_env()),
            )
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `jacky computer-use doctor` and check "
                f"{display_jacky_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    jaswanthsai1/jacky-cli#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of jaswanthsai1/jacky-cli#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Jacky run, with
        ...
        #     concurrent Jacky runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Jacky run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"jacky-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Jacky' minimal core.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps jacky `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of jaswanthsai1/jacky-cli#47072: read the canonical
        ...
        if mode == "vision":
            ...
            # Surface 2 of jaswanthsai1/jacky-cli#47072: prefer the
    ...
    def click(
        ...
        *,
        ...
        # jaswanthsai1/jacky-cli#47072). cua-driver-rs gained an explicit

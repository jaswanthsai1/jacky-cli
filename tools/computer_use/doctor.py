"""
`jacky computer-use doctor` — thin client for cua-driver's `health_report` MCP tool.
...
flow through here without code changes on the Jacky side — the only
...
def _sanitized_cua_env() -> Dict[str, str]:
    """Telemetry-policy env with Jacky provider secrets stripped.
...
def run_doctor(
    ...
    *,
    ...
    Honors `JACKY_CUA_DRIVER_CMD` via the same `_cua_driver_cmd()` resolver
    ...
    if driver_cmd is None:
        try:
            ...
            driver_cmd = os.environ.get("JACKY_CUA_DRIVER_CMD") or "cua-driver"

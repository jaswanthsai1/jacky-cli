"""
Jacky Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.jacky/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove Jacky PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing jacky-agent or jacky PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# Jacky Agent" comment and following line
                if '# Jacky Agent' in line or '# jacky-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('jacky' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing jacky
                if 'jacky' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the jacky wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "jacky",
        Path("/usr/local/bin/jacky"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains jacky_cli reference)
                content = wrapper.read_text()
                if 'jacky_cli' in content or 'jacky-agent' in content:
                    wrapper.unlink()
...
def remove_node_symlinks(jacky_home: Path) -> list:
    ...
    symlinks node/npm/npx into the same directory as the ``jacky`` command:
    ...
    the FHS fix).  Only symlinks that resolve into this Jacky home's ``node``
    directory are removed — links the user has repointed elsewhere (nvm, fnm,
    etc.) are left untouched.
    """
    node_dir = (jacky_home / "node").resolve()
    removed = []
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``jacky gateway run`` processes
    ...
    try:
        from jacky_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from jacky_cli.gateway import get_launchd_plist_path
...
#
#   1. Sets User-scope env vars ``JACKY_HOME`` and ``JACKY_GIT_BASH_PATH``
...
#      like ``%LOCALAPPDATA%\jacky\git\cmd``, ``%LOCALAPPDATA%\jacky\git\bin``,
#      ``%LOCALAPPDATA%\jacky\git\usr\bin``, ``%LOCALAPPDATA%\jacky\node``.
#      Again not in any rc file — only accessible via the registry or the
#      .NET [Environment] API.
#   3. Downloads PortableGit to ``%LOCALAPPDATA%\jacky\git\`` and Node to
#      ``%LOCALAPPDATA%\jacky\node\`` as user-scoped, isolated copies.
#      These are ~200MB combined and serve no purpose after uninstall.
#   4. On the ``jacky dashboard`` + gateway paths, drops files into
#      ``%LOCALAPPDATA%\jacky\gateway-service\`` and sometimes
...
def _jacky_path_markers(jacky_home: Path) -> list[str]:
    """Path-entry substrings that identify Jacky-owned User-PATH entries."""
    root = str(jacky_home).rstrip("\\/")
    # Match on prefix so sub-entries (git\cmd, git\bin, git\usr\bin, node, etc.)
    # all get swept.  Also match the bare jacky-agent install dir.
    markers = [root + "\\jacky-agent", root + "\\git", root + "\\node", root + "\\venv"]
    # Also match if JACKY_HOME was customised to somewhere else — find-and-nuke
    # any entry whose path component contains "jacky".  We don't want to catch
    # unrelated entries like "cjacky-foo" or "unjackyed", so we look for
    # backslash-jacky as a word-ish boundary.
    return markers


def remove_path_from_windows_registry(jacky_home: Path) -> list[str]:
    """Strip Jacky-owned entries from User-scope PATH in the registry.
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, key_path, 0,
            ...
            markers = _jacky_path_markers(jacky_home)
            kept: list[str] = []
...
def remove_jacky_env_vars_windows() -> list[str]:
    """Delete JACKY_HOME and JACKY_GIT_BASH_PATH from User-scope env vars."""
    try:
        import winreg
    except ImportError:
        return []
    ...
    try:
        with winreg.OpenKey(winreg.HKEY_CURRENT_USER, "Environment", 0,
                            winreg.KEY_READ | winreg.KEY_WRITE) as key:
            for name in ("JACKY_HOME", "JACKY_GIT_BASH_PATH"):
                try:
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its JACKY_HOME directory.

    We shell out to ``jacky -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current JACKY_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m jacky_cli.main` so we don't depend on a `jacky`
    #    wrapper that may be half-removed mid-uninstall.
    jacky_invocation = [_sys.executable, "-m", "jacky_cli.main", "--profile", name]
    ...
    # 3. Wipe the profile's JACKY_HOME directory.
    try:
...
def run_gui_uninstall(args):
    ...
    Mirrors ``jacky uninstall --gui``. Removes the desktop app's built
    artifacts, the packaged app bundle (best-effort), and the Electron
    userData dir — nothing under ``$JACKY_HOME`` config/sessions/.env, and
    never the Python agent or its venv.
    """
    from jacky_cli.gui_uninstall import (
        agent_is_installed,
        gui_install_summary,
        uninstall_gui,
    )
    ...
    summary = gui_install_summary(jacky_home)
    ...
    if not summary["gui_installed"]:
        print("No Jacky Chat GUI installation was found.")
        print(f"  Checked: {jacky_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the Chat GUI only. The Jacky agent stays installed.", Colors.CYAN))
    print()
    ...
    print()
    if agent_is_installed(jacky_home):
        print(color("Kept intact:", Colors.GREEN, Colors.BOLD))
        print(f"  • The Jacky agent at {jacky_home / 'jacky-agent'}")
        print(f"  • Your config, sessions, and secrets under {jacky_home}")
        print()
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The Jacky agent is still installed. Run 'jacky' to use the CLI,")
    print("or 'jacky uninstall' to remove the agent too.")
    print()
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.jacky/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.jacky/ for future reinstall
    """
    ...
    # offer to clean them up too instead of leaving zombie JACKY_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_jacky_home(jacky_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    # Non-interactive fast path (``--yes``): no prompts. ``--full`` selects a
    # full wipe (code + ~/.jacky data); otherwise keep-data. Named profiles
    ...
    print(f"  Config:  {jacky_home / 'config.yaml'}")
    print(f"  Secrets: {jacky_home / '.env'}")
    print(f"  Data:    {jacky_home / 'cron/'}, {jacky_home / 'sessions/'}, {jacky_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their JACKY_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL Jacky data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the Jacky code but keep your configuration and data.")
...
def _print_uninstall_dry_run(*, project_root: Path, jacky_home: Path, full_uninstall: bool) -> None:
    ...
    print("  • Jacky PATH entries from shell configs / Windows User PATH")
    print("  • Jacky wrapper scripts and Jacky-managed node/npm/npx symlinks")
    print("  • Desktop Chat GUI artifacts")
    print(f"  • Code checkout: {project_root}")
    if full_uninstall:
        print(f"  • Jacky config/data: {jacky_home}")
        if _is_default_jacky_home(jacky_home):
        ...
        print(f"  • Keep Jacky config/data: {jacky_home}")
    print()
...
def _perform_uninstall(
    *,
    ...
    ``jacky`` wrapper + node symlinks → remove the desktop Chat GUI artifacts →
    delete the code checkout → (Windows) remove PortableGit/Node → optionally
    wipe ``$JACKY_HOME`` data and named profiles on full uninstall.
    """
    print()
    ...
    if _is_windows():
        ...
        # Expand %LOCALAPPDATA% etc. in jacky_home so the marker matching is
        # against fully resolved paths — installer writes literal strings
        # like C:\Users\<u>\AppData\Local\jacky\git\cmd, not %LOCALAPPDATA%.
        removed_path_entries = remove_path_from_windows_registry(Path(os.path.expandvars(str(jacky_home))))
        if removed_path_entries:
            for entry in removed_path_entries:
                log_success(f"Removed from User PATH: {entry}")
        else:
            log_info("No Jacky-owned PATH entries in User environment")

        log_info("Removing JACKY_HOME / JACKY_GIT_BASH_PATH User env vars...")
        removed_env = remove_jacky_env_vars_windows()
        if removed_env:
            for name in removed_env:
                log_success(f"Removed User env var: {name}")
        else:
            log_info("No Jacky-set User env vars to remove")
    
    # 3. Remove wrapper script
    log_info("Removing jacky command...")
    ...
    #     (only when they still point into this Jacky home's node dir, so we
    #     never clobber an existing nvm / user-managed Node).
    log_info("Removing Jacky-managed node/npm/npx symlinks...")
    removed_node_links = remove_node_symlinks(jacky_home)
    if removed_node_links:
        for link in removed_node_links:
            log_success(f"Removed {link}")
    else:
        log_info("No Jacky-managed node/npm/npx symlinks found")
    ...
    #     step-5 rmtree(jacky_home) would sweep the in-tree artifacts anyway,
    #     but the packaged app + Electron userData live OUTSIDE JACKY_HOME and
    #     must be cleaned explicitly here.
    log_info("Removing desktop Chat GUI artifacts...")
    try:
        from jacky_cli.gui_uninstall import uninstall_gui
        gui_removed = uninstall_gui(jacky_home)
        if not gui_removed:
    ...
    #     under JACKY_HOME but they're install tooling, not config — safe to
    #     remove even in "keep data" mode.  If we're doing a full uninstall
    #     the step-5 rmtree(jacky_home) would sweep them anyway; calling
    ...
    if _is_windows():
        ...
        removed_artifacts = remove_portable_tooling_windows(jacky_home)
    ...
    # 5. Optionally remove ~/.jacky/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile JACKY_HOME dirs live under
        ...
        try:
            if jacky_home.exists():
                shutil.rmtree(jacky_home)
                log_success(f"Removed {jacky_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {jacky_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {jacky_home}")
...
def main(argv=None) -> int:
    """Module entrypoint: ``python -m jacky_cli.uninstall --mode <gui|lite|full>``.
    ...
    so ``import jacky_cli`` resolves from source while the venv is torn down.

    This module imports only stdlib + ``jacky_constants`` + ``jacky_cli.colors``
    (and lazily ``jacky_cli.gui_uninstall``), so it runs fine under a bare
    system Python with no site-packages from the venv.
    """
    import argparse

    parser = argparse.ArgumentParser(prog="python -m jacky_cli.uninstall")
    parser.add_argument(
        "--mode",

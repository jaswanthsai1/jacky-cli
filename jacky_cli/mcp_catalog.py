...
entries via ``jacky mcp catalog`` or the interactive ``jacky mcp picker``,
and install them with ``jacky mcp install <name>`` (or by toggling in the
picker, which flows them through any required env/OAuth setup).

Catalog policy:
- Entries are added only by merging a PR into jacky-agent. Presence in the
...
- Manifests pin transport details (commands, args, refs). MCPs are never
  auto-updated; users explicitly re-run ``jacky mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.jacky/.env`` (the
...
from jacky_cli.cli_output import prompt as _prompt_input
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Jacky install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Jacky understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Jacky is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Jacky
        understands. Update Jacky to install this entry.
...
def _install_root() -> Path:
    ...
    root = get_jacky_home() / "mcp-installs"
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.jacky/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.jacky/.env via save_env_value()."""
...
def _probe_tools(name: str) -> Optional[List[tuple]]:
    ...
    try:
        ...
        from jacky_cli.mcp_config import _probe_single_server
...
def _apply_tool_selection(
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``jacky mcp configure <name>``.
    """
    print()
    ...
    if probed is None:
        ...
        if manifest_default:
            ...
            print(color(
                ...
                f"Run `jacky mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `jacky mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `jacky mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `jacky mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `jacky auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`jacky auth {entry.auth.provider}` if you have not "
    ...
    from jacky_cli.mcp_config import _save_mcp_server
    ...
    print(color(
        ...
        f"Start a new Jacky session to load its tools.",
        Colors.GREEN,
    ))

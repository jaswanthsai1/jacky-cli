"""
Doctor command for jacky CLI.

Diagnoses issues with Jacky Agent setup.
"""
...
_DHH = display_jacky_home()  # user-facing display path (e.g. ~/.jacky or ~/.jacky/profiles/coder)

# Load environment variables from ~/.jacky/.env so API key checks work
_env_path = get_env_path()
load_jacky_dotenv(jacky_home=_env_path.parent, project_env=PROJECT_ROOT / ".env")
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.jacky/.env contains provider auth/base URL settings."""
...
def _doctor_tool_availability_detail(toolset: str) -> str:
    ...
    if toolset == "kanban" and not os.environ.get("JACKY_KANBAN_TASK"):
...
def _has_healthy_oauth_fallback_for_apikey_provider(provider_label: str) -> bool:
    ...
    if normalized == "minimax":
        try:
            from jacky_cli.auth import get_minimax_oauth_auth_status
...
def _check_version_consistency(issues: list[str]) -> None:
    """Verify pyproject.toml version matches jacky_cli.__version__.

    A git conflict resolution (reset/merge) can revert one file without the
    other, leaving ``jacky --version`` reporting a stale version while
    ...
    """
    try:
        from jacky_cli import __version__ as init_version
    except Exception:
        return
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            f"(pyproject.toml {pyproject_version} != jacky_cli/__init__.py {init_version})",
            "Re-sync version files (e.g. run 'jacky update', or set "
            "jacky_cli/__init__.py __version__ to match pyproject.toml)",
            issues,
        )
...
def _check_s6_supervision(issues: list[str]) -> None:
    ...
    Reports:
      - Whether the main-jacky and dashboard static services are up
    ...
    for static in ("main-jacky", "dashboard"):
    ...
    if not profiles:
        check_info("No per-profile gateways registered yet — create one with `jacky profile create <name>`")
        return
...
def _build_apikey_providers_list() -> list:
    ...
    try:
        ...
        try:
            from jacky_cli.providers import normalize_provider as _normalize_provider
...
def managed_scope_check() -> None:
    ...
    resolved from the JACKY_MANAGED_DIR override (rather than the system
    ...
    if os.environ.get("JACKY_MANAGED_DIR", "").strip():
        check_info(f"managed dir set via JACKY_MANAGED_DIR={managed_dir}")
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `jacky`.
    os.environ.setdefault("JACKY_INTERACTIVE", "1")

    # Handle `jacky doctor --ack <id>` as a fast path. Persist the ack and
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.jacky/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 🩺 Jacky Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
        ...
        if fresh_hits:
            for hit in fresh_hits:
                ...
                manual_issues.append(
                    ...
                    f"`jacky doctor --ack {hit.advisory.id}`."
                )
    ...
    # Detect drift between pyproject.toml and jacky_cli/__init__.py versions
    ...
    managed_scope_check()
    # Check ~/.jacky/.env (primary location for user config)
    env_path = JACKY_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'jacky setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'jacky setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'jacky setup' to create one")
                issues.append("Run 'jacky setup' to create .env")
    
    # Check ~/.jacky/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = JACKY_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from jacky_cli.config import get_compatible_custom_providers as _compatible_custom_providers
                from jacky_cli.providers import (
                ...
                if catalog_provider is None or (
                    ...
                    _fail_and_issue(
                        ...
                        (
                            ...
                            f"Fix: run 'jacky config set model.provider <valid_provider>'"
                        ),
                        issues,
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom"):
                try:
                    if runtime_provider == "openrouter":
                        ...
                        from jacky_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.jacky/.env or run 'jacky setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'jacky setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'jacky config set model.provider <name>'"
                            ),
                            issues,
                        )
                except Exception:
        ...
        if fallback_config.exists():
            ...
            if should_fix:
                ...
                if example_config.exists():
                    ...
                    from jacky_cli.config import DEFAULT_CONFIG, save_config
    ...
    if config_path.exists():
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'jacky setup' to migrate config")
                else:
                    issues.append("Run 'jacky doctor --fix' or 'jacky setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'jacky doctor --fix'")
        except Exception:
            pass

        # Detect stale JACKY_MAX_ITERATIONS ghost in .env shadowing
        ...
        # bridge normally derives JACKY_MAX_ITERATIONS from agent.max_turns
        ...
        try:
            import yaml
            from jacky_cli.config import load_env, remove_env_value
            with open(config_path, encoding="utf-8") as f:
                raw_config = yaml.safe_load(f) or {}
            ...
            env_ghost = load_env().get("JACKY_MAX_ITERATIONS")
            drift = (
            ...
            if drift:
                check_warn(
                    f"JACKY_MAX_ITERATIONS={env_ghost} in .env shadows "
                    f"agent.max_turns={cfg_max_turns} in config.yaml",
                    "(stale ghost from an earlier `jacky setup` run)",
                )
                if should_fix:
                    if remove_env_value("JACKY_MAX_ITERATIONS"):
                        check_ok(
                            "Removed stale JACKY_MAX_ITERATIONS from .env "
                            f"(config.yaml agent.max_turns={cfg_max_turns} is now authoritative)"
                        )
                        fixed_count += 1
                    else:
                        check_warn("Could not remove JACKY_MAX_ITERATIONS from .env")
                        manual_issues.append(
                            "Manually delete the JACKY_MAX_ITERATIONS line from "
                            f"{_DHH}/.env — config.yaml agent.max_turns is authoritative."
                        )
                else:
                    issues.append(
                        "Stale JACKY_MAX_ITERATIONS in .env shadows config.yaml — "
                        "run 'jacky doctor --fix'"
                    )
        except Exception:
            pass
    ...
    try:
        ...
        if codex_status.get("logged_in"):
            ...
            # Native OAuth uses Jacky' own device-code flow — the Codex CLI is
    ...
    for subdir_name in expected_subdirs:
        subdir_path = jacky_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Jacky a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Jacky Agent Persona\n\n"
                "<!-- Edit this file to customize how Jacky communicates. -->\n\n"
                "You are Jacky, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    memories_dir = jacky_home / "memories"
    ...
    state_db_path = jacky_home / "state.db"
        ...
        try:
            ...
            if _write_reason is not None:
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db FTS write corruption — run 'jacky doctor --fix' "
            ...
            if is_malformed_db_error(e):
                ...
                if should_fix:
                    ...
                    issues.append(
                        "state.db schema malformed — run 'jacky doctor --fix' "
                        "(or 'jacky sessions repair') to recover hidden sessions"
                    )
            else:
    ...
    wal_path = jacky_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'jacky doctor --fix' to checkpoint")
    ...
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "jacky"
        ...
        _cmd_link = _cmd_link_dir / "jacky"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(jacky not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            manual_issues.append(
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/jacky → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/jacky points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/jacky → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/jacky — run 'jacky doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/jacky exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/jacky not found",
                    "(jacky command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/jacky → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/jacky symlink — run 'jacky doctor --fix'")
    ...
    if _safe_which("node"):
        ...
        if agent_browser_path.exists():
            ...
            # `jacky update` wiped node_modules (issue #48521).
            check_warn(
        ...
        if agent_browser_ok and not _is_termux():
            try:
                ...
                # to eagerly load in every `jacky doctor` invocation.
    ...
    if _npm_bin:
        ...
        # The WhatsApp bridge may live under a writable JACKY_HOME mirror
            ...
            try:
                ...
                if total == 0:
                    ...
                    if audit_extra and audit_extra[0] == "--workspace":
                        ...
                        # tree — in that case it is an npm bug, not a Jacky one.
                        check_info(
            ...
            if r.status_code == 402:
                return _ConnectivityResult(
                    ...
                    ["OpenRouter account has insufficient credits. "
                     "Fix: run 'jacky config set model.provider <provider>' "
    ...
    def _probe_apikey_provider(pname, env_vars, default_url, base_env,
        ...
        try:
            ...
            # GOOGLE_API_KEY/GEMINI_API_KEY always shows red in ``jacky doctor``.
    ...
    # never the legitimate source for `jacky doctor`.
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'jacky setup' to configure missing API keys for full tool access")
    except Exception as e:
        check_warn("Could not check tool availability", f"({e})")
    
    _section("Skills Hub")
    hub_dir = JACKY_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: jacky skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = JACKY_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                ...
                if hcfg.api_key or hcfg.base_url:
                    ...
                    check_warn("Honcho config not found", "run: jacky memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                _fail_and_issue(
                    "Honcho API key or base URL not set",
                    "run: jacky memory setup",
                    "No Honcho API key — run 'jacky memory setup'",
                    issues,
                )
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                _fail_and_issue(
                    ...
                    "(set MEM0_API_KEY in .env or run jacky memory setup)",
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: jacky memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: jacky memory setup")
        except Exception as _e:
            check_warn(f"{_active_memory_provider} check failed", str(_e))

    try:
        from jacky_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
                ...
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "jacky -p" in content:
                            _m = _re.search(r"jacky -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'jacky doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))

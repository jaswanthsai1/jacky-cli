...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_jacky_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:jacky_pkce",
    ...
    if provider == "nous":
        ...
        # <jacky-root>/shared/nous_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `jacky --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``jacky auth add openai-codex`` overwrote
    ...
    raise SystemExit(f"`jacky auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Jacky reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `jacky auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `jacky auth` is called bare."""

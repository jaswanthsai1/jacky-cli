...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Jacky Agent. Use 'jacky debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    jacky debug share              Upload debug report (asks for confirmation)
    jacky debug share --yes        Skip confirmation (for scripts/CI)
    jacky debug share --lines 500  Include more log lines
    jacky debug share --expire 30  Keep paste for 30 days
    jacky debug share --local      Print report locally (no upload)
    jacky debug share --no-redact  Disable upload-time secret redaction
    jacky debug share --nous       Upload to Nous-internal storage (private)
    jacky debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'jacky debug share'",
    )

"""``jacky skills`` subcommand parser.
...
def build_skills_parser(subparsers, *, cmd_skills: Callable) -> None:
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.jacky/skills/.bundled_manifest) "
            "so future 'jacky update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    skills_list_modified = skills_subparsers.add_parser(
        "list-modified",
        help="List bundled skills you've edited (which `jacky update` keeps)",
        description=(
            "Show the bundled skills whose local copy differs from the version last "
            "synced, i.e. the ones `jacky update` reports as user-modified and skips. "
            "Use `jacky skills diff <name>` to see changes and `jacky skills reset "
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`jacky update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `jacky update`. Pass --sync to re-seed now."
        ),
    )

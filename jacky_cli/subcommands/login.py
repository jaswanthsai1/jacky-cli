...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``jacky login`` was removed in favor of ``jacky auth`` / ``jacky model``
    (the runtime handler in ``jacky_cli/auth.py::login_command`` just prints a
    deprecation message and exits).  The subparser is kept registered so that
    old scripts/aliases invoking ``jacky login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``jacky --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``jacky login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``jacky model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `jacky auth` to manage credentials, "
            "`jacky model` to select a provider, or `jacky setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `jacky model`",
    )
    login_parser.add_argument(
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: jacky-cli)"
    )

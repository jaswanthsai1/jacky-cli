"""``jacky dashboard`` / ``jacky serve`` subcommand parsers.
...
``jacky_cli/main.py:main()`` (god-file Phase 2); handler injected to avoid
importing ``main``.
"""
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `jacky dashboard` / `jacky serve` cmdlines and SIGTERM them directly —
    # the same path `jacky update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Jacky web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Jacky web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Jacky Agent web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Jacky desktop app shells (<= 0.15.x) spawn the
    # backend as `jacky dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Jacky couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Jacky Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Jacky backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Jacky backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # (cmd_dashboard exports JACKY_SERVE_HEADLESS=1). `dashboard` leaves it
    # unset and serves the browser UI as before.
    serve_parser.set_defaults(func=cmd_dashboard, no_open=True, headless_backend=True)

    # `jacky dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.jacky/.env.
    # Nested subparser so bare `jacky dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "JACKY_DASHBOARD_OAUTH_CLIENT_ID into ~/.jacky/.env, and prints "
            "how to engage the login gate. Requires being logged in (jacky setup)."
        ),
    )
    ...
    dashboard_register_parser.add_argument(
        ...
        help=(
            ...
            "https://jacky.example.com/auth/callback. Omit for localhost-only use."
        ),
    )
    dashboard_register_parser.add_argument(
        "--portal-url",
        ...
        help=(
            ...
            "portal. Also settable via JACKY_DASHBOARD_PORTAL_URL. Mainly for "

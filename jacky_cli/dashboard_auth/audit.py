...
Profile-aware location: ``$JACKY_HOME/logs/dashboard-auth.log``.
...
from ``jacky_constants`` or other jacky_cli modules — so it can be
...
def _resolve_log_path() -> Path:
    """``$JACKY_HOME/logs/dashboard-auth.log`` with the standard fallback.

    Mirrors ``jacky_constants.get_jacky_home`` semantics: env var wins,
    else ``~/.jacky``. A local copy avoids an import cycle with the
    middleware which lives below ``jacky_cli``.
    """
    home = os.environ.get("JACKY_HOME") or str(Path.home() / ".jacky")

...
Three cookies in play:
  ...
  - jacky_session_pkce: short-lived PKCE state + CSRF nonce + provider
...
https://datatracker.ietf.org/doc/html/draft-west-cookie-prefixes):
  ...
  * Gated HTTPS, direct deploy (Path=/) — ``__Host-`` prefix. Binds the
    ...
    guarantee.
  * Gated HTTPS, behind a reverse-proxy prefix (Path=/jacky) —
    ``__Secure-`` prefix. ``__Host-`` is disallowed when Path != "/";
    ``__Secure-`` keeps the Secure-required hardening without the
    Path constraint, and the explicit ``Path=/jacky`` covers
...
setter wrote ``__Secure-jacky_session_at`` would never find the value.
...
SESSION_AT_COOKIE = "jacky_session_at"
SESSION_RT_COOKIE = "jacky_session_rt"
PKCE_COOKIE = "jacky_session_pkce"
...
SSO_ATTEMPT_COOKIE = "jacky_sso_attempt"
...
def _cookie_path(prefix: str) -> str:
    ...
    Under ``X-Forwarded-Prefix: /jacky`` we want ``Path=/jacky`` so:
...
def set_session_cookies(
    ...
    *,
    ...
    issues a 24h rotating refresh token (jacky #37247); a provider that
    ...
    ``prefix`` is the normalised X-Forwarded-Prefix value (e.g. ``/jacky``)

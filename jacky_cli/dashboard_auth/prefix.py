...
prefix (e.g. ``mission-control.tilos.com/jacky/*`` -> dashboard on
:9119), injecting ``X-Forwarded-Prefix: /jacky`` so the backend can
reconstruct prefixed URLs (Location: headers, OAuth redirect_uri,
cookie Path attributes, SPA asset URLs).

This module is also the home of the ``JACKY_DASHBOARD_PUBLIC_URL`` /
...
def _warn_if_malformed(source: str, raw: str) -> None:
    ...
    missing scheme (``jacky.example.com`` instead of
    ``https://jacky.example.com``) — the single most common cause of
    "I set JACKY_DASHBOARD_PUBLIC_URL but the OAuth callback is still
    ...
    _log.warning(
        ...
        source,
        cleaned,
        cleaned.split("://")[-1] or "jacky.example.com",
    )
...
def normalise_prefix(raw: Optional[str]) -> str:
    ...
    Returns a string like ``"/jacky"`` (no trailing slash) or ``""``
...
# ---------------------------------------------------------------------------
# JACKY_DASHBOARD_PUBLIC_URL / dashboard.public_url
# ---------------------------------------------------------------------------
...
def resolve_public_url() -> str:
    ...
    Precedence (mirrors ``dashboard.oauth.client_id``):
      ...
      1. ``JACKY_DASHBOARD_PUBLIC_URL`` env var (when non-empty after
    ...
    """
    env_raw = os.environ.get("JACKY_DASHBOARD_PUBLIC_URL", "")
    env_clean = _normalise_public_url(env_raw)
    if env_clean:
        return env_clean
    _warn_if_malformed("JACKY_DASHBOARD_PUBLIC_URL env var", env_raw)

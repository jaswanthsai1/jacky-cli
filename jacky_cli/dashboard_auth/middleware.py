...
from jacky_cli.dashboard_auth import list_session_providers
from jacky_cli.dashboard_auth.audit import AuditEvent, audit_log
from jacky_cli.dashboard_auth.base import ProviderError, RefreshExpiredError
...
from jacky_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /jacky``, the
    ``login_url`` is prefixed (``/jacky/login?next=...``) so the
    ...
    """
    from jacky_cli.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from jacky_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["jacky.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``jacky_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if session is None:
        ...
        from jacky_cli.dashboard_auth.cookies import clear_session_cookies
        from jacky_cli.dashboard_auth.prefix import prefix_from_request

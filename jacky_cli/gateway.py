"""
Gateway subcommand for jacky CLI.

Handles: jacky gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_jacky_home is imported lazily at call sites to avoid ImportError
# when jacky_constants is cached from a pre-update version during `jacky update`.
...
def _get_parent_pid(pid: int) -> int | None:
    ...
    "jacky CLI that invoked this scan" from "real gateway process".
    """
    if pid <= 1:
        return None
    try:
...
def _get_ancestor_pids() -> set[int]:
    ...
    ``jacky gateway status`` from falsely counting the ``jacky`` CLI that
...
def _scan_gateway_pids(
    ...
    # scan (e.g. ``jacky gateway status``) is never mistaken for a running
    # gateway.  See #13242.
    ...
    current_home = str(get_jacky_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        ...
        if current_profile_name:
            return (
                ...
                or f"jacky_home={current_home_lc}" in command_lc
            )

        # Default-profile case: no profile flag in argv. Accept as long as
        # the command doesn't advertise *some other* profile. JACKY_HOME
        ...
        # JACKY_HOME= in argv is.
        if "--profile " in command_lc or " -p " in command_lc:
            return False
        if (
            "jacky_home=" in command_lc
            and f"jacky_home={current_home_lc}" not in command_lc
        ):
            return False
        return True
...
def find_gateway_pids(
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``jacky update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            Jacky profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to Jacky profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from jacky_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "jacky_cli.main"]
    if profile != "default":
...
def _capture_gateway_argv(pid: int) -> list[str] | None:
    ...
    Windows Scheduled Task running ``pythonw.exe -m jacky_cli.main gateway
...
def launch_detached_gateway_restart_by_cmdline(
    ...
    ``gateway run`` whose JACKY_HOME or argv doesn't match a known profile).
...
def _spawn_gateway_restart_watcher(old_pid: int, run_argv: list[str]) -> bool:
    ...
    # when the user closes the terminal, leaving ``jacky update`` users
    # with no running gateway until they re-invoke ``jacky gateway``
    ...
    from jacky_cli._subprocess_compat import (
    ...
    if sys.platform == "win32":
        try:
            from jacky_cli.gateway_windows import (
    ...
    watcher = textwrap.dedent(
        ...
        # the env (VIRTUAL_ENV / PYTHONPATH / JACKY_HOME) the windowless
        # base interpreter needs to import jacky_cli.  Empty on POSIX, where
...
def _jacky_home_from_systemd_unit_file(system: bool = False) -> str | None:
    """Read ``JACKY_HOME`` from the on-disk unit file (not ``systemctl show``).
    ...
    for line in text.splitlines():
        ...
        if body.startswith("JACKY_HOME="):
            value = body.split("=", 1)[1].strip().strip('"')
            return value or None
    return None


def _sync_jacky_home_from_systemd_unit(system: bool) -> None:
    """When acting on a system-scope unit, adopt its ``JACKY_HOME``.

    Under ``sudo``, ``JACKY_HOME`` is stripped and ``HOME=/root``, so
    :func:`get_jacky_home` falls back to ``/root/.jacky`` — the wrong
    profile. The unit file pins ``JACKY_HOME`` for the actual gateway
    ...
    unit_home = (_jacky_home_from_systemd_unit_file(system=True) or "").strip()
    if not unit_home:
        unit_home = _read_systemd_unit_environment(system=True).get("JACKY_HOME", "").strip()
    if not unit_home:
        return
    current = os.environ.get("JACKY_HOME", "").strip()
    if current == unit_home:
        return
    os.environ["JACKY_HOME"] = unit_home
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}jacky gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}jacky gateway restart{scope_flag}"
    )
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    if is_linux() and is_container():
        ...
        try:
            from jacky_cli.service_manager import detect_service_manager, get_service_manager
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    if _launchd_unsupported_marker_exists():
        ...
        print("  Stop it with: jacky gateway stop")
    else:
        print(
            "⚠ Gateway process is running for this profile, but the service is not active"
        )
        print(f"  PID(s): {_format_gateway_pids(snapshot.gateway_pids, limit=None)}")
        print("  This is usually a manual foreground/tmux/nohup run, so `jacky gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``jacky gateway status`` output so users with
...
def _gateway_list() -> None:
    ...
    """
    try:
        from jacky_cli.profiles import list_profiles, get_active_profile_name
    except Exception:
...
def _reap_unsupervised_gateway_orphans() -> bool:
    ...
    in-process under a ``gateway restart`` argv (jacky_cli/gateway.py restart
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (JACKY_HOME-scoped).
...
def _container_systemd_operational() -> bool:
    ...
    This is NOT our Jacky Docker image — that one runs s6-overlay as
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``jacky gateway run`` must remain interruptible from
    PowerShell/CMD. Detached service-style launches opt in via
    ``JACKY_GATEWAY_DETACHED=1``; older wrappers without the env marker are
    treated as detached when no interactive stdin is attached.
    """
    if not is_windows():
        return False

    detached = os.getenv("JACKY_GATEWAY_DETACHED", "").strip().lower()
...
_SERVICE_BASE = "jacky-gateway"
SERVICE_DESCRIPTION = "Jacky Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current JACKY_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (JACKY_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary JACKY_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(jacky_home: str | None = None, default_root: str | Path | None = None) -> str:
    """Return ``--profile <name>`` only when JACKY_HOME is a named profile.

    For ``~/.jacky/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        jacky_home: Optional explicit JACKY_HOME path. Defaults to the current
            ``get_jacky_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
        default_root: Optional Jacky root to compare against. Used when
            generating a system service for another user from a sudo/root
            process, where ``Path.home()`` and ``get_default_jacky_root()``
...
def _profile_arg_for_target_user(jacky_home: str, target_home_dir: str) -> str:
    """Return the profile arg for a system service running as another user."""
    target_root = Path(target_home_dir) / ".jacky"
    try:
        Path(jacky_home).resolve().relative_to(target_root.resolve())
        return _profile_arg(jacky_home, default_root=target_root)
    except ValueError:
        return _profile_arg(jacky_home)


def get_service_name() -> str:
    """Derive a systemd service name scoped to this JACKY_HOME.

    Default ``~/.jacky`` returns ``jacky-gateway`` (backward compatible).
    Profile ``~/.jacky/profiles/coder`` returns ``jacky-gateway-coder``.
    Any other JACKY_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older Jacky installs that predate the
# jacky-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (jacky-gateway-*.service) and unrelated third-party
# "jacky" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("jacky.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "jacky_cli.main gateway",
    "jacky_cli/main.py gateway",
    "gateway/run.py",
    " jacky gateway ",
    "/jacky gateway ",
)
...
def _find_legacy_jacky_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy Jacky gateway units.

    Detects unit files installed by older Jacky versions that used a
    different service name (e.g. ``jacky.service`` before the rename to
    ``jacky-gateway.service``). When both a legacy unit and the current
    ``jacky-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``jacky-gateway-coder.service`` and unrelated third-party
      ``jacky-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``jacky.service`` running an unrelated
...
def has_legacy_jacky_units() -> bool:
    """Return True when any legacy Jacky gateway unit files exist."""
    return bool(_find_legacy_jacky_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy Jacky gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_jacky_units()
    if not legacy:
        return
    print_warning("Legacy Jacky gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current jacky-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    jacky gateway migrate-legacy")


def remove_legacy_jacky_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy Jacky gateway unit files.

    Iterates over whatever ``_find_legacy_jacky_units()`` returns — which is
    ...
    legacy = _find_legacy_jacky_units()
    if not legacy:
        print("No legacy Jacky gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy Jacky gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: jacky gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd removal path, guarded by `if system == "Linux"` / systemd-only branch
            ...
            print_info("  Re-run with: sudo jacky gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    jacky gateway uninstall")
    print_info("    sudo jacky gateway uninstall --system")
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            print_warning(
                ...
                "root shell, or install a user service instead: jacky gateway install"
            )
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode Jacky often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.jacky`` → ``ai.jacky.gateway.plist`` (backward compatible).
    Profile ``~/.jacky/profiles/coder`` → ``ai.jacky.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.jacky.gateway-{suffix}" if suffix else "ai.jacky.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.jacky/jacky-agent  -> /home/alice/.jacky/jacky-agent
      /opt/jacky                 -> /opt/jacky  (kept as-is)
...
def _jacky_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current JACKY_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_jacky_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.jacky                    → /home/alice/.jacky
      /root/.jacky/profiles/coder     → /home/alice/.jacky/profiles/coder
      /opt/custom-jacky               → /opt/custom-jacky  (kept as-is)
    """
    current_jacky = get_jacky_home().resolve()
    current_default = (Path.home() / ".jacky").resolve()
    target_default = Path(target_home_dir) / ".jacky"

    # Default ~/.jacky → remap to target user's default
    if current_jacky == current_default:
        return str(target_default)

    # Profile or subdir of ~/.jacky → preserve the relative structure
    try:
        relative = current_jacky.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.jacky) — keep as-is
        return str(current_jacky)
...
def _build_service_path_dirs(project_root: Path | None = None) -> list[str]:
    ...
    jacky_node = jacky_home / "node" / "bin"
    if _is_dir(jacky_node):
        candidates.append(str(jacky_node))
    jacky_nm = jacky_home / "node_modules" / ".bin"
    if _is_dir(jacky_nm):
        candidates.append(str(jacky_nm))
...
def _stable_service_working_dir() -> str:
    ...
    uses an absolute python interpreter and ``-m jacky_cli.main``, so module
    ...
    dir, or a clone that ``jacky update`` later relocates/removes — the path
    ...
    ``JACKY_HOME`` is the stable anchor: it is where config/state/logs live,
    it never moves, and it is guaranteed to exist whenever the gateway is
    meaningfully installed. Fall back to ``PROJECT_ROOT`` only if JACKY_HOME
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        jacky_home = _jacky_home_for_target_user(home_dir)
        profile_arg = _profile_arg_for_target_user(jacky_home, home_dir)
        # Remap all paths that may resolve under the calling user's home
        # (e.g. /root/) to the target user's home so the service can
        # actually access them.
        python_path = _remap_path_for_user(python_path, home_dir)
        # Anchor cwd to the target user's JACKY_HOME (stable, always exists)
        # rather than a remapped source-checkout path that can rot. See
        # _stable_service_working_dir() for the full rationale.
        working_dir = str(jacky_home) if jacky_home else _remap_path_for_user(working_dir, home_dir)
...
User={username}
Group={group_name}
ExecStart={python_path} -m jacky_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run
...
Environment="JACKY_HOME={jacky_home}"
Restart=always
RestartSec=5
...
"""
    ...
    jacky_home = str(get_jacky_home().resolve())
    profile_arg = _profile_arg(jacky_home)
...
def systemd_unit_is_current(system: bool = False) -> bool:
    ...
    # pinned JACKY_HOME is adopted before any compare/regenerate" at a single
    # site, so a future callsite cannot regress it by forgetting to pre-sync.
    #
    # Under ``sudo jacky gateway … --system``, JACKY_HOME is often stripped
    # and falls back to ``/root/.jacky``. Adopting the unit's pinned home
    # first makes TimeoutStopSec / WorkingDirectory / JACKY_HOME comparisons
    ...
    _sync_jacky_home_from_systemd_unit(system=system)
...
def _temp_home_in_service_definition(definition: str) -> str | None:
    """Return the temp-dir JACKY_HOME baked into a service definition, or None.

    A generated systemd unit / launchd plist carries the resolved JACKY_HOME
    ...
    exported a throwaway ``JACKY_HOME=/tmp/...`` — writing it to the real
    ...
    Seen live 2026-06-11: an E2E guard probe ran ``jacky gateway restart``
    with ``JACKY_HOME=/tmp/jacky-e2e-<pr>`` exported; the restart path's
    ...
    Matches both systemd ``Environment="JACKY_HOME=..."`` lines and launchd
    ``<key>JACKY_HOME</key><string>...</string>`` pairs.
    """
    import re
    import tempfile

    candidates = re.findall(r'JACKY_HOME=([^"\n]+)', definition)
    candidates += re.findall(
        r"<key>JACKY_HOME</key>\s*<string>(.*?)</string>", definition, flags=re.S
    )
    temp_roots = {
...
def _refuse_temp_home_service_write(definition: str, kind: str) -> bool:
    """Refuse (with guidance) when a service definition carries a temp JACKY_HOME."""
    temp_home = _temp_home_in_service_definition(definition)
    if temp_home is None:
        return False
    print(
        f"✗ Refusing to write the gateway {kind}: JACKY_HOME resolves to a "
        f"temporary directory ({temp_home})."
    )
    print(
        "  This usually means a test/E2E environment exported JACKY_HOME. "
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    # single JACKY_HOME-sync chokepoint (adopts the unit's pinned home before
    ...
    # sandboxed by the test conftest (only JACKY_HOME is). If a test
    # exercises ``run_gateway()`` with a pytest-tmp JACKY_HOME, the freshly
    # generated unit bakes that ``/tmp/pytest-of-.../jacky_test`` path into
    # ``Environment="JACKY_HOME=..."``. Writing that to the developer's
    ...
    if not system and (
        ...
        or '/jacky_test"' in new_unit
        or "/jacky_test/" in new_unit
    ):
        return False

    # Structural variant of the same belt: refuse to bake ANY temp-dir
    # JACKY_HOME into the unit (manual E2E homes like /tmp/jacky-e2e-NNN
    ...
    print(
        f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current Jacky install"
    )
    return True
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo jacky gateway uninstall --system")
    print_info("         jacky gateway install")
    print_info("         jacky gateway start")


def _get_restart_drain_timeout() -> float:
    """Return the configured gateway restart drain timeout in seconds."""
    raw = os.getenv("JACKY_RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(
    ...
    # Offer to remove legacy units (jacky.service from pre-rename installs)
    # before installing the new jacky-gateway.service. If both remain, they
    ...
    if has_legacy_jacky_units():
        print()
        print_legacy_unit_warning()
        print()
        if non_interactive or prompt_yes_no("Remove the legacy unit(s) before installing?", True):
            remove_legacy_jacky_units(interactive=False)
            print()

    unit_path = get_systemd_unit_path(system=system)
    scope_flag = " --system" if system else ""

    # Existing system units already pin JACKY_HOME; adopt it before any
    ...
    # ``sudo jacky gateway install --system --force`` would bake /root/.jacky
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}jacky gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}jacky gateway status{scope_flag}             # Check status"
    )
    print(
...
def _require_service_installed(action: str, system: bool = False) -> None:
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}jacky gateway install{scope_flag}")
        sys.exit(1)
...
def systemd_start(system: bool = False):
    ...
    # JACKY_HOME sync happens inside refresh_systemd_unit_if_needed's
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `jacky gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `jacky gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}jacky gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}jacky gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}jacky gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}jacky gateway start{scope_flag}"
        )
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.jacky.gateway-{suffix}" if suffix else "ai.jacky.gateway"


# Cached launchd domain result — probing is cheap but should only run once per
# process invocation (each ``jacky gateway start/stop/status`` call).
...
#      background process (the `nohup jacky gateway run` workaround). See #23387.
...
def _launchd_reload_log_path() -> Path:
    ...
    return get_jacky_home() / "logs" / "launchd-reload.log"
...
def _launchd_unsupported_marker_path() -> Path:
    return get_jacky_home() / ".gateway-launchd-unsupported"
...
def _gateway_run_command() -> list[str]:
    """Build the `python -m jacky_cli.main [--profile X] gateway run --replace` argv.

    Profile-aware: honors the active JACKY_HOME via `_profile_arg()` so the
    detached fallback launches into the same profile as the CLI invocation.
    """
    cmd = [get_python_path(), "-m", "jacky_cli.main"]
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup jacky gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    print(
        f"  Try manually: nohup jacky gateway run --replace "
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    if (
        ...
        # to ~/.jacky/logs/launchd-reload.log, which the health watchdog
        # can tail to detect a persistent orphan. See jacky-restart
        # rootcause handoff (2026-06-26 incident).
        reload_log_path = get_jacky_home() / "logs" / "launchd-reload.log"
        try:
    ...
    print(
        "↻ Updated gateway launchd service definition to match the current Jacky install"
    )
    return True
...
def launchd_stop():
    ...
    # `jacky gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    # Jacky PID tracking — may be a detached fallback process spawned when
    ...
    # launchd PID and the Jacky PID file).
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current Jacky install")
    else:
        print("⚠ Service definition is stale relative to the current Jacky install")
        print("  Run: jacky gateway start")
    ...
    if service_listed:
        if launchd_pid is not None:
            ...
            if fallback_pid:
                ...
                print("  Cron jobs will fire. Stop with: jacky gateway stop")
            else:
    ...
    if deep:
        log_file = get_jacky_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def _running_under_gateway_supervisor() -> bool:
    ...
    a reliable marker into the child's environment:
      ...
      - the s6-overlay container longrun exports ``JACKY_S6_SUPERVISED_CHILD``.
    """
    if os.environ.get("INVOCATION_ID"):
        return True
    if os.environ.get("JACKY_S6_SUPERVISED_CHILD"):
        return True
...
def _guard_named_profile_under_multiplexer(force: bool = False) -> None:
    ...
    named-profile ``jacky gateway run`` is always a misconfiguration, so we
...
def _guard_supervised_gateway_conflict(force: bool = False) -> None:
    ...
    Running ``jacky gateway run [--replace]`` (or the manual-restart fallback)
...
def _guard_existing_gateway_process_conflict(replace: bool = False) -> None:
    ...
    ``jacky gateway run`` can burn memory/CPU just to fail with "already
    ...
    unrelated gateway processes from other JACKY_HOME roots.
    """
    ...
    print("  Use 'jacky gateway restart' to replace it,")
    print("  or 'jacky gateway stop' first.")
    print("  Or use 'jacky gateway run --replace' to auto-replace.")
    sys.exit(1)
...
def _guard_official_docker_root_gateway() -> None:
    ...
    if not hasattr(os, "geteuid") or os.geteuid() != 0:
        return
    if _truthy_env(os.getenv("JACKY_ALLOW_ROOT_GATEWAY")):
        return
    if not _is_official_docker_checkout():
        return

    print_error(
        "Refusing to run the Jacky gateway as root inside the official Docker image."
    )
    print(
        "  The image entrypoint normally drops privileges to the 'jacky' user. "
        "If you override entrypoint in Docker Compose, include "
        "/opt/jacky/docker/entrypoint.sh before the Jacky command."
    )
    print(
        "  Running the gateway as root can leave root-owned files in "
        "$JACKY_HOME and break later non-root dashboard/gateway runs."
    )
    print(
        "  Set JACKY_ALLOW_ROOT_GATEWAY=1 only if you intentionally accept this risk."
    )
    sys.exit(1)
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False, force: bool = False):
    ...
    # from sibling CLI processes, but foreground `jacky gateway run` still
    # needs to obey the banner's "Press Ctrl+C to stop" contract.
    # Service-style launchers set JACKY_GATEWAY_DETACHED=1; older wrappers
    ...
    # /restart) rather than through `jacky gateway restart` which already
    ...
    # `jacky gateway start/restart` — leaving the gateway vulnerable to
    ...
    # JACKY_GATEWAY_EXIT_DIAG env var (default: on while we're still
...
_PLATFORMS = [
    ...
    # plugins/platforms/matrix/adapter.py::register(). #41112.
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. jacky) and copy the bot token",
        ...
        "vars": [
            ...
            },
            {
                ...
                "password": False,
                "help": "Channel ID where Jacky delivers cron results and notifications.",
            },
            {
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Jacky connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: jacky pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and Jacky will connect automatically over WebSocket",
        ],
        "vars": [
            {
                "name": "YUANBAO_APP_ID",
...
def _all_platforms() -> list[dict]:
    ...
    ``jacky setup gateway`` without needing the gateway to be running.
    ...
    every host. Currently:
      - Matrix is hidden on Windows. The [matrix] extra pulls
        ...
        picker. Users who want Matrix on Windows can run jacky under
        WSL.
    """
    ...
    # User-installed platform plugins under ~/.jacky/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_jacky_home() / "whatsapp" / "session" / "creds.json"
...
def _setup_standard_platform(platform: dict):
    ...
    if platform.get("key") == "telegram":
        ...
        if choice.strip() == "1":
            try:
                from jacky_cli.telegram_managed_bot import (
            ...
            if value:
                ...
                if is_email:
                    ...
                    access_choices = [
                        ...
                        "Use DM pairing (unknown users request access, you approve with 'jacky pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Approve with: jacky pairing approve <platform> <code>"
                    )
                elif is_email:
                    print_success("  Unknown email senders will be ignored.")
                else:
                    print_info(
                        "  Skipped — configure later with 'jacky gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print()
    print_info("  1. Jacky will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info(
        "  3. Jacky will store the returned account_id/token in ~/.jacky/.env."
    )
    print_info(
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `jacky gateway setup`.")
        return
    ...
    try:
        credentials = asyncio.run(qr_login(str(get_jacky_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info(
            "  Unknown DM users can request access and you approve them with `jacky pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_qqbot():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info(
            "  Unknown users can request access; approve with `jacky pairing approve`."
        )
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info('    signal-cli link -n "JackyAgent"')
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``jacky_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from jacky_cli import setup as _s
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.jacky/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.jacky/.env: {', '.join(required)}")
    else:
        print_info(
...
def gateway_setup():
    ...
    if any_configured:
            ...
            if supports_systemd_services() and _system_scope_wizard_would_need_root():
                ...
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: jacky gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    try:
                        ...
                        print_info("  You can try manually: jacky gateway install")
                else:
                    print_info("  Skipped start and auto-start setup.")
                    print_info("  You can install later: jacky gateway install")
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo jacky gateway install --system"
                        )
                    print_info("  Or run in foreground:  jacky gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: jacky gateway run")
                print_info(
                    "  For persistence:   tmux new -s jacky 'jacky gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: jacky gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup jacky gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
        ...
        print()
        print_info("No platforms configured. Run 'jacky gateway setup' when ready.")
...
def _dispatch_via_service_manager_if_s6(
    ...
    :mod:`jacky_cli.service_manager` are caught and surfaced as
    actionable CLI messages (no raw ``CalledProcessError`` traceback).
    """
    from jacky_cli.service_manager import (
    ...
    if profile is None:
        ...
        # JACKY_HOME=<root>/profiles/<name>, "" for the default root,
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``jacky gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``jacky gateway install|uninstall|start|stop|restart``
...
def _maybe_redirect_run_to_s6_supervision(args) -> bool:
    ...
    Three gates make this a no-op outside the intended scope:
      ...
      1. ``_dispatch_via_service_manager_if_s6`` returns False unless
         ...
         ``jacky gateway run`` are unaffected.
      2. ``JACKY_S6_SUPERVISED_CHILD`` is exported by
         ``S6ServiceManager._render_run_script`` for the supervised
         process itself — i.e. when s6-supervise execs ``jacky gateway
         ...
         → ...).
      3. ``--no-supervise`` (or ``JACKY_GATEWAY_NO_SUPERVISE=1``) opts
    ...
    no_supervise = getattr(args, "no_supervise", False) or \
        os.environ.get("JACKY_GATEWAY_NO_SUPERVISE", "").lower() in ("1", "true", "yes")
    if no_supervise:
        return False
    if os.environ.get("JACKY_S6_SUPERVISED_CHILD"):
    ...
    # `docker logs` and ${JACKY_HOME}/logs/gateways/<profile>/current,
    ...
    print(
        ...
        "  dashboard supervised alongside if JACKY_DASHBOARD is set).\n"
        "  This is the recommended setup for the s6 container image — the\n"
        "  gateway will keep running even if it crashes.\n"
        "  Use `--no-supervise` (or JACKY_GATEWAY_NO_SUPERVISE=1) to opt out\n"
    ...
    #
    # Prefer `sleep infinity` (matches the static main-jacky service's
    # pattern in docker/s6-rc.d/main-jacky/run, and frees the Python
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: jacky gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning(
                ...
                print_info(
                    "  Consider running in foreground instead: jacky gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s jacky 'jacky gateway run'"
                )
                print()
            ...
            print()
            print(
                "  jacky gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s jacky 'jacky gateway run'         # persistent via tmux"
            )
            print(
                "  nohup jacky gateway run > ~/.jacky/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            from jacky_cli.service_manager import detect_service_manager
            if detect_service_manager() == "s6":
                print("Per-profile gateways are auto-registered when you create a profile.")
                print()
                print("  jacky profile create <name>     # creates the s6 service slot")
                print("  jacky -p <name> gateway start   # bring it up via s6")
                print("  jacky status                    # see currently-supervised gateways")
                return
            ...
            print()
            print("To run the gateway: jacky gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: jacky gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: jacky gateway stop")
            sys.exit(1)
        if supports_systemd_services():
            ...
            if detect_service_manager() == "s6":
                ...
                print()
                print("  jacky profile delete <name>     # tears down the s6 service slot")
                print("  jacky -p <name> gateway stop    # stop without deleting the profile")
                return
        ...
        # slot — start them individually via `jacky -p <name> gateway
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: jacky gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_JACKY_GATEWAY") == "1":
            print_error(
                "Refusing to stop the gateway from inside the gateway process.\n"
                "This command was blocked to prevent restart loops.\n"
                "Use `jacky gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_JACKY_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `jacky gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: jacky gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  jacky gateway install")
                else:
                    print("To install as a service:")
                    print("  jacky gateway install")
                    print("  sudo jacky gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  jacky gateway run      # Run in foreground")
                if is_termux():
                    print(
                        "  nohup jacky gateway run > ~/.jacky/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s jacky 'jacky gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  jacky gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  jacky gateway install  # Install as user service")
                    print(
                        "  sudo jacky gateway install --system  # Install as boot-time system service"
                    )
        ...
        # Stop, disable, and remove legacy Jacky gateway unit files from
        # pre-rename installs (e.g. jacky.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_jacky_units(interactive=not yes, dry_run=dry_run)

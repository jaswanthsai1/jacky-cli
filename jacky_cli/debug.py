"""``jacky debug`` debug tools for Jacky Agent.

Currently supports:
    jacky debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.jacky/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[jacky debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.jacky/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``jacky debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``jacky debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `jacky debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "jacky-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``jacky debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.jacky/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``jacky debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "jacky-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----JackyDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from jacky_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_jacky_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``jacky dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``jacky dump`` is run
        internally.
...
_NOUS_BUNDLE_FORMAT = "jacky-debug-share/1"
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``jacky debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "jacky debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `jacky debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  jacky debug delete <url>")

    print("\nShare these links with the Jacky team for support.")
...
⚠️  --nous: This uploads your debug bundle to Nous-INTERNAL storage (AWS S3),
  ...
  • System info (OS, Python/Jacky version, provider, which API keys are
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``jacky debug share --nous``: upload the bundle to Nous-S3.
    ...
    suggests ``--local``.
    """
    from jacky_cli.diagnostics_upload import share_to_nous
    ...
    if redact:
        logger.info(
            "jacky debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `jacky debug share --local` to print the report instead, "
            "or `jacky debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: jacky debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'jacky debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``jacky debug`` call.
    ...
    # best-effort — any failure is swallowed so ``jacky debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: jacky debug <command>")
        print()
        print("Commands:")

...
the existing module-level functions in jacky_cli.gateway and
jacky_cli.gateway_windows directly. This protocol is a thin facade
...
``jacky gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``jacky gateway start/stop/restart``).
    """
    ...
    # `jacky gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged jacky
    ...
    detection always fails. Since every Jacky runtime call inside the
    container drops to jacky via ``s6-setuidgid``, that silent failure
...
# in ``jacky_cli.gateway`` (systemd/launchd) and ``jacky_cli.gateway_windows``
...
# active (set via the ``jacky -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in jacky_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from jacky_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from jacky_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from jacky_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from jacky_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in jacky_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from jacky_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from jacky_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from jacky_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from jacky_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``jacky_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `jacky profile create`
# runs inside the container (Phase 4). Static services (main-jacky, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's JACKY_HOME blindly;
    derive the shared profile root from the current JACKY_HOME and map the
    ...
    jacky_home = Path(os.environ.get("JACKY_HOME", "/opt/data"))
    if jacky_home.parent.name == "profiles":
        root = jacky_home.parent.parent
    else:
        root = jacky_home
...
# UID/GID of the in-image ``jacky`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the jacky user.
    ...
    (the ``jacky`` user — UID 10000 — running every Jacky runtime
    ...
    is dispatched as the jacky user.
    ...
    with jacky ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           jacky:jacky, 0755 (parent must already exist)
    ``svc_dir/event/``                     jacky:jacky, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 jacky:jacky, 0755
    ``svc_dir/supervise/event/``           jacky:jacky, 03730
    ``svc_dir/supervise/control``          jacky:jacky, 0660    (FIFO)
    ...
    slot's supervise/ was jacky-owned.
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        path.chmod(mode)
        try:
            os.chown(path, _JACKY_UID, _JACKY_GID)
        except PermissionError:
            # Running as the jacky user already — directory is jacky-
    ...
    if not control.exists():
        ...
        control.chmod(0o660)
        try:
            os.chown(control, _JACKY_UID, _JACKY_GID)
        except PermissionError:
            pass
    ...
    # when the parent slot's supervise/ is jacky-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            try:
                os.chown(log_control, _JACKY_UID, _JACKY_GID)
            except PermissionError:
                pass
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``jacky -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`jacky profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-jacky, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources JACKY_HOME (and any extra env) via with-contenv —
             so e.g. ``-e JACKY_HOME=/data/jacky`` is honored at run
          ...
          4. Drops to the jacky user and exec's
             ``jacky -p <profile> gateway run`` (or just ``jacky
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``jacky gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root JACKY_HOME profile" (the implicit profile that exists at
        the top of $JACKY_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``jacky_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$JACKY_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``JACKY_HOME``,
        ...
        allocator in ``jacky_cli.profiles._allocate_gateway_port``).
        ...
        # guard.
        lines.append("export JACKY_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's JACKY_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``jacky gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-JACKY_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The JACKY_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own JACKY_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "jacky gateway run --replace"
        else:
            gateway_cmd = f"jacky -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid jacky {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${JACKY_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the JACKY_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e JACKY_HOME=/data/jacky``
        gets its logs under /data/jacky/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``jacky logs`` reads and what
        ...
        return (
            ...
            f': "${{JACKY_HOME:=/opt/data}}"\n'
            f'log_dir="$JACKY_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as jacky and crash-loops on
            ...
            f'chown jacky:jacky "$JACKY_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R jacky:jacky "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid jacky s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``jacky gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``jacky -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with jacky ownership
            ...
            # the jacky user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is jacky-owned (see ``_seed_supervise_skeleton``), and on

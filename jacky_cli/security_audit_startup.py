...
3. Running inside a container with no persistent volume mount over the
   JACKY_HOME data dir (state is ephemeral — lost on container restart).
...
logger = logging.getLogger("jacky.security_audit")
...
def _running_as_root() -> Optional[str]:
    ...
    return (
        ...
        "full host compromise. Run Jacky as an unprivileged user (or in a "
...
def _in_container() -> bool:
    ...
    if os.path.exists("/.dockerenv"):
        return True
    if os.environ.get("JACKY_DESKTOP_CHILD_PID"):
...
def _container_no_volume_mount(jacky_home: Optional[Path]) -> Optional[str]:
    if not _in_container():
        return None
    home = jacky_home or Path(
        os.environ.get("JACKY_HOME", os.path.expanduser("~/.jacky"))
    )
    try:
...
def run_security_audit(
    *, jacky_home: Optional[Path] = None, config: Optional[dict] = None
) -> list[str]:
    ...
    for check in (
        ...
        try:
            ...
            continue
    try:
        r = _container_no_volume_mount(jacky_home)
        if r:
            findings.append(r)
    except Exception:
        pass
...
def log_startup_security_warnings(
    *,
    ...
    _AUDIT_RAN = True
    try:
        findings = run_security_audit(jacky_home=jacky_home, config=config)
    except Exception:
        return []
    if findings:
        logger.warning(

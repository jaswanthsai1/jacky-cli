...
manual BotFather token copy-paste. Jacky talks only to the Nous onboarding
...
DEFAULT_API_URL = "https://setup.jacky-agent.nousresearch.com"
...
DEFAULT_MANAGER_BOT = "JackySetupBot"

DEFAULT_BOT_NAME = "Jacky Agent"
...
def generate_username_slug(length: int = 16) -> str:
    ...
    keeping ``jacky_<slug>_bot`` under Telegram's 32-character username limit.
    """
...
def generate_bot_username(profile_name: Optional[str] = None) -> str:
    """Generate a secure suggested bot username like ``jacky_<slug>_bot``.
    ...
    """
    _ = profile_name
    return f"jacky_{generate_username_slug()}_bot"
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting Jacky Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the Jacky Telegram onboarding service.")

"""
Jacky MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Jacky-specific extra)

Usage:
    jacky mcp serve
    jacky mcp serve --verbose
...
logger = logging.getLogger("jacky.mcp_serve")
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("JACKY_HOME", Path.home() / ".jacky")
...
class EventBridge:
    ...
    This is the Jacky equivalent of OpenClaw's WebSocket gateway bridge.
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Jacky MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "jacky",
        instructions=(
            "Jacky Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Jacky MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(

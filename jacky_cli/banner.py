...
Pure display functions with no JackyCLI state dependency.
"""
import json
import logging
import os
...
# (e.g. nix-built jacky — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_UPSTREAM_REPO_URL = "https://github.com/jaswanthsai1/jacky-cli.git"
_OFFICIAL_REPO_CANONICAL = "github.com/jaswanthsai1/jacky-cli"
...
def check_for_updates() -> Optional[int]:
    """Check whether a Jacky update is available.

    Two paths: if ``JACKY_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = jacky_home / ".update_check"
    embedded_rev = os.environ.get("JACKY_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # JACKY_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `jacky update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/jacky/update/check`
    ...
    try:
        from jacky_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $JACKY_HOME/jacky-agent/ may be a stale copy from --clone-all;
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Jacky git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$JACKY_HOME/jacky-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``jacky_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from jacky_cli.build_info import get_build_sha
...
_RELEASE_URL_BASE = "https://github.com/jaswanthsai1/jacky-cli/releases/tag"
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Jacky checkout. Cached per-process. Release URL always points at the
    canonical jaswanthsai1/jacky-cli repo (forks don't get a link).
    """
...
_JACKY_MODEL_ALIASES = {"minimax-m3": "Jacky Ultra"}
def _jacky_model_alias(m):
    return _JACKY_MODEL_ALIASES.get((str(m).split("/")[-1] or m).lower(), m)


def _render_jacky_console(console, skin, model: str, cwd: str, session_id: str = None):
    """JACKY's bespoke operator HUD — a tactical console, not the Jacky panel.

    Compact + unique: wordmark, a skull/reticle emblem beside a status block,
    the hunt-loop pipeline, the engine row, and a READY line. No tool/skill
    walls (tools surface on demand via tool_search), so it renders instantly.
    """
    ...
    red = skin.get_color("banner_title", "#FF3030")
    org = skin.get_color("banner_accent", "#FF6B1A")
    dimc = skin.get_color("banner_dim", "#7A1010")
    txt = skin.get_color("banner_text", "#FFD7B0")
    border = skin.get_color("banner_border", "#8B0000")
    lbl = skin.get_color("ui_label", "#FF8C42")
    ok = skin.get_color("ui_ok", "#46d160")

    term_w = _sh.get_terminal_size().columns
    console.print()
    if term_w >= 95:
        console.print(skin.banner_logo or JACKY_AGENT_LOGO)
        console.print()
    ...
    top.add_row(emblem, info)

    loop = f" [{dimc}]▸[/] ".join(f"[{txt}]{s}[/]" for s in
        ["scope", "recon", "rank", "enumerate", "test", "validate", "chain", "report"])
    engines = "   ".join([f"[{org}]🌳[/] [{txt}]ptt[/]", f"[{org}]🔗[/] [{txt}]chain[/]",
        f"[{org}]📝[/] [{txt}]report[/]", f"[{org}]🧬[/] [{txt}]evolve[/]", f"[{org}]🧠[/] [{txt}]ledger[/]"])
    lines = (
        f"\n[{lbl}]HUNT LOOP[/]  {loop}\n"
        f"[{lbl}]ENGINES[/]    {engines}\n"
        f"[{lbl}]ARSENAL[/]    [{txt}]113 repos · 240 skills · 21 tools[/] [{dimc}]· surfaced on demand[/]\n"
        f"\n[{ok}]READY ▸[/] [{txt}]hand me a target, or[/] [{org}]resume <t>[/]  [{dimc}]·  /help for commands[/]"
    )

    panel = Panel(
        Group(top, console.render_str(lines)),
        title=f"[bold {red}]⌖ JACKY ▸ OFFENSIVE OPS CONSOLE[/]",
        border_style=border,
        padding=(0, 2),
    )
    console.print(panel)
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
    ...
    # ── JACKY console — bespoke tactical HUD that REPLACES the default panel ──
    try:
        from jacky_cli.skin_engine import get_active_skin as _gas
        if getattr(_gas(), "name", "") == "jacky":
            _render_jacky_console(console, _gas(), model, cwd, session_id)
            return
    except Exception:
        pass
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else JACKY_CADUCEUS
    except Exception:
        _bskin = None
        _hero = JACKY_CADUCEUS
    left_lines = ["", _hero, ""]
    ...
    try:
        from jacky_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from jacky_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built jacky; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else JACKY_AGENT_LOGO
        console.print(_logo)
        console.print()

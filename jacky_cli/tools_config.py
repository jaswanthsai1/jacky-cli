"""
Unified tool configuration for Jacky Agent.

`jacky tools` and `jacky setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.jacky/config.yaml under
...
from jacky_cli.nous_subscription import (
...
from jacky_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _JACKY_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `jacky tools` → Video Generation, which walks
...
# `jacky tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from jacky_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `jacky tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``jacky tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from jacky_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``jacky tools`` checklist actually offers
    for ``platform``.
    ...
    so ``jacky tools`` never claims to add or remove a toolset the user was
...
from jacky_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    ...
    "x_search": {
        ...
        "setup_note": (
            "Jacky routes X searches through xAI's built-in x_search "
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set JACKY_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "JACKY_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "JACKY_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "JACKY_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "JACKY_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "JACKY_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `jacky model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("JACKY_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Jacky telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``jacky update`` and
      by ``jacky computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `jacky update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `jacky computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `jacky update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `jacky update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.jacky/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Jacky process.")
            return True
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            _print_warning("    Node.js not found - browser tools require: npm install (in jacky-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/jaswanthsai1/jacky-cli:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.jacky/config.yaml")
        ...
        # Run the full `jacky auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.jacky/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from jacky_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: jacky auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `jacky auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: jacky auth spotify")
        except Exception as exc:
        ...
        try:
            from jacky_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: jacky plugins enable observability/langfuse")
        _print_info("    Restart Jacky for tracing to take effect.")
        _print_info("    Verify: jacky plugins list")
        ...
        try:
            ...
            _print_info("    Run later: jacky auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `jacky auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: jacky auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: jacky auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``jacky tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``jacky tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: jacky tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``jacky-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``jacky-discord``) is an opt-in to the platform's native default-off
    ...
    # "jacky-cli" (which include all _JACKY_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[jacky-cli, spotify]`` after enabling Spotify via ``jacky
        ...
        # (e.g. "jacky-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``jacky tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `jacky tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"jacky-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("jacky-")}
    ...
    # shipped as a bundled plugin but user must opt in via `jacky tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `jacky tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by jacky tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `jacky` instead of
    # `jacky-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `jacky tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (jacky-cli, jacky-telegram, etc.)
    ...
    # Opening `jacky tools` is the user's opt-in to reconfigure tools, so treat
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `jacky tools` gets a silent no-op
...
def _configure_provider(
    ...
    *,
    ...
    # "enable all tools" prompt (that lives in `jacky model`).
    if managed_feature:
        from jacky_cli.nous_subscription import (
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``jacky model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `jacky model`, then re-run this."
        )
        return
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `jacky model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `jacky tools` and `jacky setup tools`.
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, jacky mcp

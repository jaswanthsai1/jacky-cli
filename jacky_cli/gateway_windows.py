...
_TASK_NAME_DEFAULT = "Jacky_Gateway"
_TASK_DESCRIPTION = "Jacky Agent Gateway - Messaging Platform Integration"
...
def _preserve_jacky_home_path(path: str | Path) -> str:
    """Render Jacky-owned paths under the configured JACKY_HOME spelling.

    Windows installs may keep ``%LOCALAPPDATA%\\jacky`` as a symlink/junction to
    ...
    path lives under JACKY_HOME.
    """
    candidate = Path(path)
    try:
        from jacky_cli.config import get_jacky_home

        home = Path(get_jacky_home())
...
def _current_profile_cli_args() -> list[str]:
    """Return CLI args that preserve the current Jacky profile."""
    from jacky_cli.gateway import _profile_arg
...
def _launch_elevated_gateway_command(command: str, extra_args: list[str] | None = None) -> bool:
    ...
    """
    _assert_windows()
    args = ["-m", "jacky_cli.main", *_current_profile_cli_args(), "gateway", command]
    if extra_args:
...
def _launch_elevated_install(
    ...
    *,
    ...
    old_start_now = os.environ.get("JACKY_GATEWAY_INSTALL_START_NOW")
    old_start_on_login = os.environ.get("JACKY_GATEWAY_INSTALL_START_ON_LOGIN")
    old_handoff = os.environ.get("JACKY_GATEWAY_ELEVATED_HANDOFF")
    try:
        if start_now is not None:
            os.environ["JACKY_GATEWAY_INSTALL_START_NOW"] = "1" if start_now else "0"
        if start_on_login is not None:
            os.environ["JACKY_GATEWAY_INSTALL_START_ON_LOGIN"] = "1" if start_on_login else "0"
        os.environ["JACKY_GATEWAY_ELEVATED_HANDOFF"] = "1"
        ...
        for key, old in (
            ("JACKY_GATEWAY_INSTALL_START_NOW", old_start_now),
            ("JACKY_GATEWAY_INSTALL_START_ON_LOGIN", old_start_on_login),
            ("JACKY_GATEWAY_ELEVATED_HANDOFF", old_handoff),
        ):
            if old is None:
...
def get_task_name() -> str:
    ...
    Default profile: ``Jacky_Gateway``
    Named profile X: ``Jacky_Gateway_<X>``
    """
    _assert_windows()
    # Local import to avoid circular module initialization during jacky_cli boot.
    from jacky_cli.gateway import _profile_suffix
...
def get_task_script_path() -> Path:
    ...
    Lives under ``%LOCALAPPDATA%\\jacky\\gateway-service\\<task_name>.cmd``
    (or ``<JACKY_HOME>/gateway-service/<task_name>.cmd`` so per-profile
    Jacky installs stay self-contained).
    """
    _assert_windows()
    from jacky_cli.config import get_jacky_home

    script_dir = Path(get_jacky_home()) / "gateway-service"
...
def _stable_gateway_working_dir(project_root: Path) -> str:
    ...
    Mirror the POSIX service invariant: anchor at ``JACKY_HOME`` whenever it
    ...
    source checkout only if ``JACKY_HOME`` cannot be used yet. Preserve the
...
def _build_gateway_cmd_script(
    ...
    The script:
      ...
      - exports JACKY_HOME, PYTHONIOENCODING, VIRTUAL_ENV
      - invokes ``pythonw -m jacky_cli.main [--profile X] gateway run``
    ...
    lines.append(f'set "JACKY_HOME={jacky_home}"')
    lines.append('set "PYTHONIOENCODING=utf-8"')
    lines.append('set "JACKY_GATEWAY_DETACHED=1"')
    pythonw_path, venv_dir, extra_pythonpath = _resolve_detached_python(python_path)
    # VIRTUAL_ENV lets the gateway's own python detection find the venv
    # if someone imports jacky_constants-based logic during startup.
    lines.append(f'set "VIRTUAL_ENV={_preserve_jacky_home_path(venv_dir)}"')
    pythonpath_entries = [
        _preserve_jacky_home_path(Path(__file__).resolve().parent.parent),
        *[_preserve_jacky_home_path(entry) for entry in extra_pythonpath],
    ]
    lines.append(f'set "PYTHONPATH={";".join([*pythonpath_entries, "%PYTHONPATH%"])}"')

    prog_args = [pythonw_path, "-m", "jacky_cli.main"]
    if profile_arg:
...
def _build_gateway_vbs_script(
    ...
    repo_root = _preserve_jacky_home_path(Path(__file__).resolve().parent.parent)
    static_pythonpath = os.pathsep.join(
        [repo_root, *[_preserve_jacky_home_path(entry) for entry in extra_pythonpath]]
    )
    ...
    lines = [
        ...
        f"env.Item({_quote_vbs_string('JACKY_HOME')}) = {_quote_vbs_string(jacky_home)}",
        f"env.Item({_quote_vbs_string('PYTHONIOENCODING')}) = {_quote_vbs_string('utf-8')}",
        f"env.Item({_quote_vbs_string('JACKY_GATEWAY_DETACHED')}) = {_quote_vbs_string('1')}",
        f"env.Item({_quote_vbs_string('VIRTUAL_ENV')}) = {_quote_vbs_string(_preserve_jacky_home_path(venv_dir))}",
...
def _write_task_script() -> Path:
    ...
    python_path = _preserve_jacky_home_path(get_python_path())
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    jacky_home = str(Path(get_jacky_home()))
    profile_arg = _profile_arg(jacky_home)

    content = _build_gateway_cmd_script(python_path, working_dir, jacky_home, profile_arg)
    ...
    vbs_content = _build_gateway_vbs_script(python_path, working_dir, jacky_home, profile_arg)
...
def _install_scheduled_task(task_name: str, script_path: Path) -> tuple[bool, str]:
    ...
    Always recreate instead of ``/Change``. Older Jacky builds and failed
...
def _build_gateway_argv() -> tuple[list[str], str, dict[str, str]]:
    ...
    python_exe, venv_dir, extra_pythonpath = _resolve_detached_python(
        _preserve_jacky_home_path(get_python_path())
    )
    project_root = _preserve_jacky_home_path(PROJECT_ROOT)
    working_dir = _stable_gateway_working_dir(PROJECT_ROOT)
    jacky_home = str(Path(get_jacky_home()))
    profile_arg = _profile_arg(jacky_home)

    argv = [python_exe, "-m", "jacky_cli.main"]
    if profile_arg:
    ...
    env_overlay = {
        ...
        "JACKY_GATEWAY_DETACHED": "1",
        "VIRTUAL_ENV": _preserve_jacky_home_path(venv_dir),
    }
    _prepend_pythonpath(
        env_overlay,
        [project_root, *[_preserve_jacky_home_path(entry) for entry in extra_pythonpath]]
        if extra_pythonpath
        else [project_root],
    )
...
def windowless_gateway_restart_spec(
    ...
    ``jacky_cli`` package without the venv launcher's site config.

    Returns ``(new_argv, working_dir, env_overlay)``.  ``new_argv``
    preserves every argument after the interpreter (``-m jacky_cli.main
    ...
    from jacky_cli.config import get_jacky_home
    from jacky_cli.gateway import PROJECT_ROOT
    ...
    try:
        jacky_home = str(Path(get_jacky_home()).resolve())
    except Exception:
        jacky_home = ""
    ...
    if jacky_home:
        env_overlay["JACKY_HOME"] = jacky_home
    _prepend_pythonpath(
        env_overlay,
...
def _spawn_detached(script_path: Path | None = None) -> int:
    ...
    We spawn ``pythonw.exe -m jacky_cli.main gateway run``
    ...
    log_dir = Path(get_jacky_home()) / "logs"
...
def _prompt_install_choices(
    ...
    env_start_now = _install_choice_from_env("JACKY_GATEWAY_INSTALL_START_NOW")
    env_start_on_login = _install_choice_from_env("JACKY_GATEWAY_INSTALL_START_ON_LOGIN")
    ...
    from jacky_cli.setup import prompt_yes_no
...
def _install_startup_fallback(script_path: Path, start_now: bool, detail: str) -> None:
    ...
    # Re-running `jacky -p <profile> gateway install` must be safe.
    ...
    from jacky_cli.gateway import find_gateway_pids, _profile_arg
    ...
    if running_pids:
        ...
        profile_arg = _profile_arg()
        start_cmd = f"jacky {profile_arg} gateway start" if profile_arg else "jacky gateway start"
    ...
    *,
    ...
    if not start_on_login:
        ...
        if start_now:
            ...
            print("  Run later with: jacky gateway start")
        return
    ...
    if not _is_running_as_admin() and not elevated_handoff:
        ...
        if prompt_yes_no("  Open the UAC prompt now?", False):
            if _launch_elevated_install(force=force, start_now=start_now, start_on_login=start_on_login):
                print("✓ Launched elevated Jacky gateway install prompt.")
                if start_now:
                    print("  Approve the Windows UAC prompt; the elevated install will start the gateway afterwards.")
                else:
                    print("  Approve the Windows UAC prompt, then run: jacky gateway status")
                return
    ...
    if ok:
        ...
        if start_now:
            ...
            print("  Start manually with: jacky gateway start")
        _print_next_steps()
        return
...
def _report_gateway_start(via: str) -> None:
    ...
    if pids:
        ...
        from jacky_cli.config import get_jacky_home
        print(f"    type {Path(get_jacky_home())}\\logs\\gateway.log")
        print(f"    type {Path(get_jacky_home())}\\logs\\gateway-stdio.log")


def _print_next_steps() -> None:
    from jacky_cli.config import get_jacky_home

    jacky_home = Path(get_jacky_home())
    print()
    print("Next steps:")
    print("  jacky gateway status                      # Check status")
    print(f"  type {jacky_home}\\logs\\gateway.log       # View logs")
        ...
        if code == 0:
            ...
            if prompt_yes_no("  Open the UAC prompt now?", False):
                if _launch_elevated_uninstall():
                    print("✓ Launched elevated Jacky gateway uninstall prompt.")
                    print("  Approve the Windows UAC prompt, then run: jacky gateway status")
                    return
...
def start() -> None:
    ...
    if not task_installed and not startup_installed:
        ...
        if not prompt_yes_no("  Install it now so the gateway starts on login?", True):
            print("  Run: jacky gateway install")
            return
        install(force=False)
        task_installed = is_task_registered()
        startup_installed = is_startup_entry_installed()
        if not task_installed and not startup_installed:
            ...
            print("  If a UAC prompt opened, approve it, then run: jacky gateway start")
            return
...
def restart() -> None:
    ...
    if not _wait_for_gateway_ready(timeout_s=15.0):
        raise RuntimeError(
            ...
            "Check logs/gateway.log and run `jacky gateway status`."
        )

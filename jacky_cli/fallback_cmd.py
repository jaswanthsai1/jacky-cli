"""
jacky fallback — manage the fallback provider chain.
...
Subcommands:
  jacky fallback [list]   Show the current fallback chain (default when no subcommand)
  jacky fallback add      Pick provider + model via the same picker as `jacky model`,
                           then append the selection to the chain
  jacky fallback remove   Pick an entry to delete from the chain
  jacky fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.jacky/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from jacky_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `jacky model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  jacky fallback add")
        print()
        return
    ...
    print("  Docs: https://jacky-agent.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `jacky model`, then append the selection to the chain."""
    from jacky_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `jacky model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `jacky fallback list` to view, or `jacky fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``jacky fallback [subcommand]``."""

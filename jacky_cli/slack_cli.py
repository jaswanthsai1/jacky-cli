"""``jacky slack ...`` CLI subcommands.

Today only ``jacky slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ jacky slack manifest > slack-manifest.json
    # or:
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Jacky. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Jacky deployment — users can tweak them in the Slack UI after
    pasting.
    ...
    ``/new``, etc. work inline.
    """
    from jacky_cli.commands import slack_app_manifest
    ...
    if include_assistant:
        features["assistant_view"] = {
            "assistant_description": "Chat with Jacky in threads and DMs.",
        }
    ...
    return {
        ...
        "display_information": {
            "name": bot_name[:35],
            "description": (bot_description or "Your Jacky agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``jacky_cli/main.py``):
      --write [PATH]  Write to file instead of stdout (default path:
                      ``$JACKY_HOME/slack-manifest.json``)
      --name NAME     Override the bot display name (default: "Jacky")
    ...
    """
    name = getattr(args, "name", None) or "Jacky"
    description = getattr(args, "description", None) or "Your Jacky agent on Slack"
    ...
    if write_target is not None:
        if isinstance(write_target, bool) and write_target:
            ...
            try:
                ...
                target = Path(os.environ.get("JACKY_HOME") or str(Path.home() / ".jacky")) / "slack-manifest.json"
        else:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Jacky app\n"

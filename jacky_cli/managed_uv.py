...
Jacky owns its own uv binary at ``$JACKY_HOME/bin/uv`` (or ``uv.exe`` on
Windows).  Every code path that needs uv resolves it from that single location.
If the binary is missing, ``ensure_uv()`` bootstraps it via the official
standalone installer with ``UV_UNMANAGED_INSTALL`` / ``UV_INSTALL_DIR`` pointed
at ``$JACKY_HOME/bin`` so the installer writes directly there — no PATH
...
def managed_uv_path() -> Path:
    """Return the path where Jacky keeps *its* uv binary.

    ``$JACKY_HOME/bin/uv`` on POSIX, ``$JACKY_HOME\\bin\\uv.exe`` on
...
class _UvResult(str):
    ...
    ``(path, fresh_bootstrap)`` tuple across releases. ``jacky update`` runs
    the call site from the *old*, already-imported ``jacky_cli.main`` against
...
def update_managed_uv() -> Optional[str]:
    ...
    Call this during ``jacky update`` so the managed copy stays current.
...
def _install_uv(target: Path) -> None:
    ...
    ``$JACKY_HOME/bin/`` instead of ``~/.local/bin/``.
    """

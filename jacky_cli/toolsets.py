...
Usage:
    from jacky_cli.toolsets import get_toolset, resolve_toolset, get_all_toolsets
...
_JACKY_CORE_TOOLS = [
    ...
    # read-only terminal tab (both gated on JACKY_DESKTOP via check_fn —
    ...
    # spawned as a kanban worker (JACKY_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `jacky tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (JACKY_KANBAN_TASK env "
    ...
    # Coding posture (base Jacky — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "coding": {
        ...
        # non-configurable-toolset recovery loop in jacky_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Jacky toolsets (CLI + messaging platforms)
    #
    ...
    # the `jacky send` CLI), not by the model deciding to send on its own.
    # ==========================================================================
    ...
    "jacky-cli": {
        ...
        "tools": _JACKY_CORE_TOOLS,
        "includes": []
    },

    "jacky-cron": {
        # Mirrors jacky-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `jacky tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as jacky-cli; gated by `jacky tools`",
        "tools": _JACKY_CORE_TOOLS,
        "includes": []
    },
    ...
    "jacky-email": {
        "description": "Email bot toolset - interact with Jacky via email (IMAP/SMTP)",
        "tools": _JACKY_CORE_TOOLS,
        "includes": []
    },
    ...
    "jacky-sms": {
        "description": "SMS bot toolset - interact with Jacky via SMS (Twilio)",
        "tools": _JACKY_CORE_TOOLS,
        "includes": []
    },
    ...
    "jacky-webhook": {
        ...
        "tools": _JACKY_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "jacky-gateway": {
        ...
        "tools": [],
        "includes": ["jacky-telegram", "jacky-discord", "jacky-whatsapp", "jacky-slack", "jacky-signal", "jacky-bluebubbles", "jacky-homeassistant", "jacky-email", "jacky-sms", "jacky-mattermost", "jacky-matrix", "jacky-dingtalk", "jacky-feishu", "jacky-wecom", "jacky-wecom-callback", "jacky-weixin", "jacky-qqbot", "jacky-webhook", "jacky-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``jacky-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_JACKY_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``jacky-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (jacky-<name>).
        # Gives them _JACKY_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("jacky-"):
            platform_name = name[len("jacky-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_JACKY_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(

#!/usr/bin/env python3
"""
Jacky Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Jacky Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills jacky-agent-dev,github-auth
...
os.environ["JACKY_QUIET"] = "1"  # Our own modules

import yaml

from jacky_cli.fallback_config import get_fallback_chain
from jacky_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from jacky_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from jacky_cli.pt_input_extras import (
...
from jacky_cli.banner import _format_context_length, format_banner_version_label
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.jacky/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("JACKY_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.jacky/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If JACKY_IGNORE_USER_CONFIG=1 is set (via ``jacky chat --ignore-user-config``),
    the user config at ``~/.jacky/config.yaml`` is skipped entirely and only the
    ...
    ignore_user_config = os.environ.get("JACKY_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Jacky, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Jacky - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see jacky_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
            ...
            # jacky_cli/config.py DEFAULT_CONFIG (display.show_reasoning).
            "show_reasoning": True,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from jacky_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and JackyCLI.__init__ checks "default" first.
    ...
    from jacky_cli.config import _expand_env_vars
    ...
    # jacky_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `jacky config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (jacky_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && jacky` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _JACKY_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_JACKY_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["JACKY_REDACT_SECRETS"] = str(redact).lower()
...
try:
    from jacky_cli.config import print_config_warnings
...
try:
    from jacky_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``jacky --help`` paths inside the chat command flow).  See
...
from jacky_cli.banner import build_welcome_banner
from jacky_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from jacky_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    _accept_hooks = os.environ.get("JACKY_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _arm_exit_watchdog(timeout_s: float | None = None) -> None:
    ...
    Tune with ``JACKY_EXIT_WATCHDOG_S`` (seconds); ``0`` disables.
    """
    if timeout_s is None:
        try:
            timeout_s = float(os.getenv("JACKY_EXIT_WATCHDOG_S", "30"))
...
def _notify_session_finalize(
    *,
    ...
    try:
        from jacky_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.jacky/jacky-agent`` clone is updated only by ``jacky update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run jacky -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"jacky-{short_id}"
    branch_name = f"jacky/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"jacky pid={os.getpid()}", str(wt_path)],
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    ...
    ``jacky -w`` locks each worktree with reason ``jacky pid=<pid>`` so a
    concurrent jacky process' startup prune leaves an in-use worktree alone.
    ...
    - ``"dead"``  — locked but the owning pid is gone, or the reason isn't a
                    parseable jacky lock (safe to unlock + reap).
    ...
    for line in result.stdout.splitlines():
        if line.startswith("worktree "):
            ...
            m = re.search(r"jacky pid=(\d+)", reason)
            if not m:
                # Locked by something we don't recognize as a jacky session
                # (or lock reason unavailable). Treat as dead — a foreign lock
                # on a jacky -w worktree is almost certainly a leftover, and
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`jacky_cli.config.load_config` (the authoritative loader that
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_jacky_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`jacky_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Lock handling (orthogonal to age): ``jacky -w`` locks each worktree with
    reason ``jacky pid=<pid>`` so a concurrent jacky process leaves an in-use
    ...
    *dead*-locked one (owning pid gone — a crashed session) is unlocked first
    ...
    Also prunes orphaned ``jacky/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("jacky-"):
            continue
        ...
        # jacky process means the worktree is actively in use — never touch
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``jacky/jacky-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``jacky -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("jacky/jacky-") or b.startswith("pr-"))
    ]
...
#
# Detection priority:
#   1. JACKY_LIGHT / JACKY_TUI_LIGHT env (true/false) — explicit override
#   2. JACKY_TUI_THEME=light|dark — explicit theme
#   3. JACKY_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Jacky assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("JACKY_LIGHT", "JACKY_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("JACKY_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("JACKY_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from jacky_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_jacky_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._jacky_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Jacky output it's common to display cron schedules verbatim.
    #
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_jacky_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_jacky_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._jacky_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_jacky_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._jacky_bp_start = now
        ...
        _vt100_mod._jacky_bp_timeout_patched = True
...
def _build_compact_banner() -> str:
    ...
    if os.environ.get("JACKY_FAST_STARTUP_BANNER") == "1":
        from jacky_cli import __release_date__ as _release_date
        from jacky_cli import __version__ as _version

        version_line = f"Jacky Agent v{_version} ({_release_date})"
    else:
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from jacky_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.jacky/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.jacky/config.yaml on first use)
...
class JackyCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Jacky Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: JACKY_MAX_TOKENS)
        _env_mt = os.environ.get("JACKY_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from jacky_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("JACKY_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("JACKY_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("JACKY_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `jacky chat --ignore-rules` in jacky_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("JACKY_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("JACKY_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `jacky update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Jacky processes for this JACKY_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.jacky/checkpoints/.  Opt-in via
        ...
        self._history_file = _jacky_home / ".jacky_history"
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from jacky_cli.active_sessions import try_acquire_active_session
    ...
    def _recover_terminal_after_interrupt(self) -> None:
        ...
        """
        try:
            from jacky_cli.curses_ui import flush_stdin
            flush_stdin()
        except Exception:
            pass
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``jacky pets`` without a restart, mirroring
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from jacky_cli.voice import format_voice_record_key_for_status
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from jacky_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from jacky_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from jacky_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if self._model_is_default:
            ...
            try:
                from jacky_cli.codex_models import get_codex_model_ids
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``jacky doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``jacky doctor`` so the banner stays
        small.
        """
        try:
            from jacky_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Jacky needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        from jacky_cli.model_switch import is_nous_hermes_non_agentic
        ...
        if is_nous_hermes_non_agentic(model_name):
            ...
            self._console_print(
                ...
                "designed for use with Jacky Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.jacky/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from jacky_cli.clipboard import save_clipboard_image

        img_dir = get_jacky_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'jacky setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from jacky_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from jacky_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from jacky_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Jacky!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from jacky_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from jacky_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [Jacky #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``jacky sessions list``. Delegates the
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `jacky sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("JACKY_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.jacky/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``jacky --resume <id>``
        ...
        saved_dir = get_jacky_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"jacky_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: jacky --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from jacky_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Jacky session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from jacky_cli.context_switch_guard import merge_preflight_compression_warning
        ...
        try:
            from jacky_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `jacky model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from jacky_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from jacky_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from jacky_cli.model_switch import (
            switch_model,
            parse_model_flags,
            resolve_persist_behavior,
        )
        from jacky_cli.providers import get_label
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from jacky_cli.commands import resolve_command
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_jacky_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_jacky_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in jacky_cli/commands.py instead of touching every dispatch site.
        from jacky_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from jacky_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from jacky_cli.skin_engine import get_active_skin
            ...
            from jacky_cli.main import _print_version_info
            ...
            from jacky_cli.config import reload_env
            count = reload_env()
            ...
            try:
                # Discover from disk (bundled + user), matching `jacky plugins
                ...
                # catalog lives behind `jacky plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: jacky plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_jacky_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: jacky plugins list)")
                else:
                    # Loaded-plugin details (tools/hooks/commands counts, errors)
                    # keyed by name, when available.
                    loaded: dict = {}
                    try:
                        from jacky_cli.plugins import get_plugin_manager
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: jacky plugins list)")
                    print("  Enable/disable: jacky plugins enable/disable <name>")
            except Exception as e:
            ...
            from jacky_cli.moa_config import (
                moa_usage,
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from jacky_cli.commands import COMMANDS
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            from jacky_cli.goals import gather_background_processes as _gather_bg
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``JACKY_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``JACKY_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from jacky_cli.partial_compress import (
    ...
    def _show_usage(self):
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by jacky_logging not
    ...
    def _print_nous_credits_block(self) -> bool:
        ...
        wall-clock-bounded inside the helper; also honors JACKY_DEV_CREDITS_FIXTURE
    ...
    def _show_credits(self):
        ...
        if not view.logged_in:
            ...
            print("  Run `jacky portal` to log in, then /credits.")
            return
    ...
    def _show_billing(self, command: str = "/billing"):
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Nous Portal.')}")
                print("  Run `jacky portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        from jacky_cli.nous_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from jacky_cli.nous_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `jacky portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from jacky_cli.auth import step_up_nous_billing_scope
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from jacky_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.jacky/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_jacky_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _voice_speak_response(self, text: str):
        ...
        try:
            ...
            os.makedirs(os.path.join(tempfile.gettempdir(), "jacky_voice"), exist_ok=True)
            mp3_path = os.path.join(
                tempfile.gettempdir(), "jacky_voice",
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _jacky_home / "interrupt_debug.log"
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard JACKY_HOME, so no -p needed.
            try:
            ...
            print(f"  jacky --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  jacky -c \"{session_title}\"{profile_flag}")
            print()
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from jacky_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from jacky_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Jacky Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Jacky Agent! Type your message or /help for commands."
        ...
        try:
            from jacky_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("JACKY_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(JACKY_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→Jacky migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from jacky_cli.config import get_config_path as _get_cfg_path_resid
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in jacky_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from jacky_cli.model_switch import resolve_persist_behavior
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_jacky_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Jacky binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        # Dynamic prompt: shows Jacky symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Jacky needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Jacky needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Jacky needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = JackyCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_jacky_osd_patched", False):
                ...
                _pt_renderer._jacky_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``JACKY_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("JACKY_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the jacky process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: jacky setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `jacky sessions list`
...
def _run_kanban_goal_loop_q(cli: "JackyCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``JACKY_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("JACKY_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from jacky_cli import kanban_db as _kb
    from jacky_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Jacky Agent CLI - Interactive AI Assistant
    ...
    os.environ["JACKY_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Jacky Gateway (messaging platforms)...")
    ...
    # Default to jacky-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Jacky): with no explicit --toolsets, collapse
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `jacky skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    # mode registers its own inside JackyCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  JACKY_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("JACKY_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``jacky chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("JACKY_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from jacky_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Jacky" box, no tool-gen
                        ...
                        if os.environ.get("JACKY_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("JACKY_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from jacky_cli.kanban_db import (
                ...
                # Single-query mode (`jacky chat -q "…"`): skip the welcome

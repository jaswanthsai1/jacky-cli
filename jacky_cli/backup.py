"""
Backup and import commands for jacky CLI.

`jacky backup` creates a zip archive of the entire ~/.jacky/ directory
(excluding the jacky-agent repo and transient files).

`jacky import` restores from a backup zip, overlaying onto the current
JACKY_HOME root.
"""
...
# ``jacky-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/jacky-agent/``
...
# JACKY_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "jacky-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under JACKY_HOME) —
...
# File names that ``jacky import`` must never overwrite, matched by basename so
...
# Reserved archive subtree for provider state that lives OUTSIDE JACKY_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside JACKY_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to jacky root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``jacky-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/jacky-agent/`` — must be preserved.
        if part == "jacky-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside JACKY_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Jacky home directory."""
    jacky_root = get_default_jacky_root()

    if not jacky_root.is_dir():
        print(f"Error: Jacky home directory not found at {jacky_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"jacky-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"jacky-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_jacky_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(jacky_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(jacky_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``jacky-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "jacky-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(jacky_root)
    ...
    # outside JACKY_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_jacky_home()}."
        )
    ...
    print(f"\nRestore with: jacky import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Jacky backup.
    ...
    # Look for telltale files that a jacky home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Jacky backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.jacky/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a jacky dir name
        if prefix in {".jacky", "jacky"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_jacky_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (jacky_root / "config.yaml").exists()
        has_env = (jacky_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Jacky configuration.")
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under JACKY_HOME.
            ...
            target = jacky_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(jacky_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_jacky_home()}."
            )
        ...
        profiles_dir = jacky_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # jacky_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: jacky profile list  (after installing jacky)")

        # Guidance
        print()
        if not (jacky_root / "jacky-agent").is_dir():
            print("Note: The jacky-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: jacky update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  jacky -p {pname} gateway install")

        print("Done. Your Jacky configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and jacky backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to JACKY_HOME).
...
# — `jacky update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $JACKY_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside JACKY_HOME and the entry is silently
...
def _quick_snapshot_root(jacky_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(jacky_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside JACKY_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``jacky update``.
    ...
    Args:
        ...
        jacky_home: Override for the Jacky home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(jacky_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for jacky backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_jacky_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, jacky_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``jacky_root`` to ``out_path``.
...
def _pre_update_backup_dir(jacky_home: Optional[Path] = None) -> Path:
    home = jacky_home or get_jacky_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of JACKY_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<JACKY_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``jacky update``) should continue even if the backup fails.
    """
    jacky_root = jacky_home or get_default_jacky_root()
    if not jacky_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(jacky_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, jacky_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `jacky claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of JACKY_HOME under ``backups/`` before a
    ``jacky claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``jacky import <archive>``.  Writes to
    ``<JACKY_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `jacky import` and the

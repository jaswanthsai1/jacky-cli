"""Implementation of the ``jacky bundles`` CLI subcommand.

Mirrors the structure of ``jacky_cli/skills_hub.py`` but for skill
...
def _console() -> Console:
    # Bind to stderr so piping `jacky bundles list | grep …` doesn't
...
def _cmd_list(args) -> None:
    ...
    if not bundles:
        c.print(
            ...
            f"  jacky bundles create <name> --skill skill1 --skill skill2[/]\n"
...
def register_cli(subparser) -> None:
    """Build the ``jacky bundles`` argparse tree.

    Called from ``jacky_cli/main.py`` where it owns the top-level
    ...
    p_create.add_argument(
        ...
        help="Human-readable description shown in /help and `jacky bundles list`",
    )
    p_create.add_argument(
...
def bundles_command(args) -> None:
    """Dispatch ``jacky bundles <subcommand>`` to the right handler."""

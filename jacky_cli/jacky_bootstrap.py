"""Windows UTF-8 bootstrap for Jacky entry points.
...
should be imported at the very top of every Jacky entry point
(``jacky``, ``jacky-agent``, ``jacky-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Jacky modules.

    Jacky ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Jacky source root
...
def activate_durable_lazy_target() -> None:
    ...
    (``JACKY_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("JACKY_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import jacky_bootstrap``
# (or ``from jacky_bootstrap import apply_windows_utf8_bootstrap``) at

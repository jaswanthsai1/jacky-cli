"""
Profile management for multiple isolated Jacky instances.

Each profile is a fully independent JACKY_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.jacky/profiles/<name>/`` by default.

The "default" profile is ``~/.jacky`` itself — backward compatible,
zero migration needed.

Usage::

    jacky profile create coder          # fresh profile + bundled skills
    jacky profile create coder --clone  # also copy config, .env, SOUL.md, skills
    jacky profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    jacky -p coder chat                 # or via flag
    jacky profile use coder             # set as sticky default
    jacky profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See jacky_constants.get_subprocess_home().
    "home",
]
...
# Marker file written by `jacky profile create --no-skills`.  When present in
# a profile's root, callers of seed_profile_skills() (fresh-create, `jacky
...
# `jacky skills install` or drop SKILL.md files into the profile's skills/.
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Three categories:
      ...
      2. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         Jacky infrastructure directories that only the default profile
         (``~/.jacky``) ever contains.  Gated on ``source_dir`` actually
    ...
    is_default_source = source_resolved == _get_default_jacky_home().resolve()
...
# Directories/files to exclude when exporting the default (~/.jacky) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "jacky-agent",         # repo checkout (multi-GB)
    ".worktrees",           # git worktrees
...
# Allow-list for ``export_profile("default")``: when JACKY_HOME equals the
...
# into the export. The set below identifies the *known Jacky profile
# artifacts* at the root of JACKY_HOME; everything else is excluded.
...
# (#58394). Add new artifacts here when introduced in ``jacky_constants``.
...
_RESERVED_NAMES = frozenset({
    "jacky", "default", "test", "tmp", "root", "sudo",
})

# Jacky subcommands that cannot be used as profile names/aliases
_JACKY_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the jacky root, NOT to the current JACKY_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where JACKY_HOME points outside
    ``~/.jacky``, profiles live under ``JACKY_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_jacky_home() / "profiles"


def _get_default_jacky_home() -> Path:
    """Return the default (pre-profile) JACKY_HOME path.

    In standard deployments this is ``~/.jacky``.
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_jacky_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name in _RESERVED_NAMES:
        raise ValueError(
            ...
            f"the Jacky installation itself or a common system binary.  "
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its JACKY_HOME directory."""
    canon = normalize_profile_name(name)
    if canon == "default":
        return _get_default_jacky_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: alias-name validity, reserved names, jacky subcommands, existing
    binaries in PATH.
    """
    canon = normalize_profile_name(name)
    try:
    ...
    if canon in _JACKY_SUBCOMMANDS:
        return f"'{canon}' conflicts with a jacky subcommand"
...
def create_wrapper_script(name: str, target: Optional[str] = None) -> Optional[Path]:
    ...
    if is_windows:
        ...
        try:
            jacky_exe = shutil.which("jacky") or "jacky"
            wrapper_path.write_text(f'#!/bin/sh\nexec {shlex.quote(jacky_exe)} -p {profile} "$@"\n')
...
def _migrate_profile_config_if_outdated(profile_dir: Path) -> None:
    ...
    ``_config_version``) or that is simply older than the running Jacky. If we
    ...
    try:
        ...
        # not be migrated. The next `jacky doctor --fix` can still surface the
...
def find_alias_for_profile(profile_name: str) -> Optional[str]:
    ...
    alias whose body invokes ``jacky -p <profile>``. When the alias name equals
    the profile name this is trivial, but a custom alias (``jacky profile alias
...
# Real wrappers are a few hundred bytes of shell; the needle (``jacky -p X``)
...
def _read_distribution_meta(profile_dir: Path) -> tuple:
    ...
    never break ``jacky profile list`` for an unrelated profile.
    """
...
def _check_gateway_running(profile_dir: Path) -> bool:
    ...
    *this* reader — which is exactly the case for a dashboard process that is
    ...
    agree.  Parameterized by ``profile_dir`` so it never mutates ``JACKY_HOME``.
    """
    try:
...
# ``config.yaml``. ``config.yaml`` is the user-facing Jacky config
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_jacky_home()
...
def profiles_to_serve(multiplex: bool) -> List[Tuple[str, Path]]:
    """Return the ``(profile_name, jacky_home)`` pairs a gateway should serve.
    ...
    - ``multiplex=True``: returns the default profile plus every valid named
      profile under ``profiles/``, each paired with its own JACKY_HOME.
    ...
    The returned ``jacky_home`` is the path to pass to
    ``set_jacky_home_override`` when scoping a turn to that profile.
    """
    active = get_active_profile_name() or "default"
    if not multiplex:
        return [(active, get_profile_dir(active))]

    serve: List[Tuple[str, Path]] = [("default", _get_default_jacky_home())]
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``jacky update`` skips re-seeding this profile's
    ...
    if canon == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.jacky)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.jacky/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
            symlinks=True,
    ...
    # Keys pages, `jacky -p <name> auth add`) had no file until first
    ...
    if not env_path.exists():
        try:
            env_path.write_text(
                "# Per-profile secrets for this Jacky profile.\n"
    ...
    if not soul_path.exists():
        try:
            from jacky_cli.default_soul import DEFAULT_SOUL_MD
            soul_path.write_text(DEFAULT_SOUL_MD, encoding="utf-8")
        except Exception:
            pass  # best-effort — don't fail profile creation over this

    # Write the opt-out marker so seed_profile_skills() and `jacky update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "(`jacky profile create --no-skills`).\n"
    ...
    # Cloned configs can be older than the running Jacky (or predate schema
    ...
    if description and description.strip():
        try:
            ...
            pass  # non-fatal — user can describe later with `jacky profile describe`

    # Phase 4: when running inside a container under s6, register the
    # new profile's gateway as a runtime s6 service so
    # `jacky -p <profile> gateway start` can supervise it via
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches JACKY_HOME at module level.
    Returns the sync result dict, or None on failure.

    Profiles that opted out of bundled skills (via ``jacky profile create
...
def backfill_profile_envs(quiet: bool = False) -> List[str]:
    ...
    default_env = _get_default_jacky_home() / ".env"
...
def _profile_bound_backend_pids(canon: str, profile_dir: Path) -> list[int]:
    """PIDs of running Jacky *backends* bound to this profile.
    ...
    names it, so find it by inspection: a Jacky backend subcommand
    (``serve``/``dashboard``/``gateway``) that is bound to *this* profile either
    by a ``--profile <canon>`` / ``-p <canon>`` selector or by a ``JACKY_HOME``
    ...
    # Never terminate ourselves or a parent (e.g. `jacky -p <canon> profile
    ...
    jacky_markers = ("jacky_cli.main", "jacky-gateway", "tui_gateway")
    pids: list[int] = []
    ...
    for proc in psutil.process_iter(["pid", "name", "username", "cmdline"]):
        try:
            ...
            # Must be a Jacky process: either an entrypoint marker in argv, or
            # a resolved executable named `jacky`.
            joined = " ".join(argv)
            exe_name = os.path.basename(argv[0]).lower()
            is_jacky = (
                any(marker in joined for marker in jacky_markers)
                or exe_name == "jacky"
                or exe_name.startswith("jacky")
            )
            if not is_jacky:
                continue
            ...
            # ...or by JACKY_HOME env pointing at this profile dir.
            if not bound:
                try:
                    env_home = (proc.environ() or {}).get("JACKY_HOME", "")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if canon == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.jacky).\n"
            "To remove everything, use: jacky uninstall"
        )
    ...
    # the rmtree below fail with ENOTEMPTY and — before the ensure_jacky_home
...
def _maybe_register_gateway_service(profile_name: str) -> None:
    ...
    ``JACKY_HOME`` and binds the port resolved by ``gateway/config.py``
    ...
    try:
        ...
        from jacky_cli.service_manager import get_service_manager
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set JACKY_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("JACKY_HOME")
    try:
        os.environ["JACKY_HOME"] = str(profile_dir)
        from jacky_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["JACKY_HOME"] = old_home
        elif "JACKY_HOME" in os.environ:
            del os.environ["JACKY_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.jacky/active_profile``. Use ``"default"`` to clear.
    """
    canon = normalize_profile_name(name)
    validate_profile_name(canon)
    if canon != "default" and not profile_exists(canon):
        raise FileNotFoundError(
            ...
            f"Create it with: jacky profile create {canon}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from JACKY_HOME.

    Returns ``"default"`` if JACKY_HOME is not set or points to ``~/.jacky``.
    ...
    default_resolved = _get_default_jacky_home().resolve()
...
def _default_export_ignore(root_dir: Path):
    ...
    * **Root-level allow-list** — only entries whose name appears in
      ...
      JACKY_HOME equals the cwd) is excluded. Blacklisting was tried
      first and proved unable to anticipate every non-Jacky file the
      user may have lying alongside JACKY_HOME (#58394).
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if canon == "default":
        # The default profile IS ~/.jacky itself — its parent is ~/ and its
        # directory name is ".jacky", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: jacky profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.jacky itself — disallow
    # that and guide the user toward a named profile.
    canon = normalize_profile_name(inferred_name)
    validate_profile_name(canon)
    if canon == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.jacky). "
            "Specify a different name: jacky profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="jacky_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"jacky_{old_name}"
    legacy_old_host = f"jacky.{old_name}"
    new_host = f"jacky_{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_jacky_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if isinstance(block, dict) and "aiPeer" not in block:
            if source_host.startswith("jacky_"):
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a JACKY_HOME path string.

    Called early in the CLI entry point, before any jacky modules
    are imported, to set the JACKY_HOME environment variable.
    """

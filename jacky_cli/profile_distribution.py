"""Profile distributions — shareable, packaged Jacky profiles via git.

A distribution is a Jacky profile published as a git repository (or
...
* ``jacky profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``jacky skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``jacky profile``, not a parallel tree):

    jacky profile install <source> [--name N] [--alias] [--force] [--yes]
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "backups", "cache",
    # Infrastructure
    "jacky-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            jacky_requires=str(data.get("jacky_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.jacky_requires:
            out["jacky_requires"] = self.jacky_requires
        if self.author:
...
def check_jacky_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Jacky {op}{target}, "
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``jacky profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Jacky profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Jacky distribution."
        )

    # Version check up-front so we fail fast
    check_jacky_requires(manifest.jacky_requires, jacky_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.jacky).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="jacky_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `jacky profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`jacky profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="jacky_dist_update_") as tmp:
        plan = plan_install(

"""Safe Jacky Console command engine.

This module backs ``jacky console`` and is intentionally narrower than the
full Jacky CLI. It exposes a curated set of native adapters that can later be
...
class ConsoleCommand:
    ...
    usage: str
    summary: str
    handler: Callable[["JackyConsoleEngine", list[str]], str]
    mutating: bool = False
    confirmation: str = ""
...
def _strip_console_status_footer(text: str) -> str:
    ...
    if not (
        prev.startswith("Run 'jacky doctor'")
        and last.startswith("Run 'jacky setup'")
    ):
        return text.rstrip()
...
def _parser_root() -> tuple[_ArgumentParser, argparse._SubParsersAction]:
    parser = _ArgumentParser(prog="jacky", add_help=False)
...
def _clean_summary(text: str | None) -> str:
    ...
    if not summary:
        return ""
    if summary.startswith("Run `jacky "):
        return ""
    return summary
...
# dashboard opens a fresh JackyConsoleEngine per /api/console connection, so
...
def _dispatch_extracted_subcommand(
    *,
    ...
    parser, subparsers = _parser_root()
    module = importlib.import_module(module_name)
    main_module = importlib.import_module("jacky_cli.main")
    builder = getattr(module, builder_name)
...
def _extracted_handler(
    ...
    main_handler_name: str,
    namespace_update: Callable[[argparse.Namespace, ConsoleContext], None] | None = None,
) -> Callable[["JackyConsoleEngine", list[str]], str]:
    def handler(_engine: JackyConsoleEngine, args: list[str]) -> str:
...
def _register_command_family(
    ...
    *,
    ...
    handler_factory: Callable[[Sequence[str]], Callable[["JackyConsoleEngine", list[str]], str]],
    ...
    for child_path in paths:
        ...
        command_summary = summary or (summaries or {}).get(full_path) or f"Run `jacky {usage}`."
        engine.register(
            full_path,
            usage,
            command_summary,
            ...
            confirmation=confirmation or f"Run `jacky {usage}`?",
            contexts=ALL_CONTEXTS if child_key in hosted_paths else LOCAL_CONTEXTS,
        )


class JackyConsoleEngine:
    """Curated line-command executor for Jacky Console."""
        ...
        try:
            ...
            if _contains_shell_syntax(raw_line, tokens):
                raise ConsoleCommandError(
                    "Jacky Console does not run shell syntax. Use one supported "
                    "Jacky command at a time."
                )
    ...
    def _register_defaults(self) -> None:
        self.register(("status",), "status", "Show Jacky component status.", _status, contexts=ALL_CONTEXTS)
        self.register(("doctor",), "doctor", "Run diagnostics without auto-fix.", _doctor, contexts=ALL_CONTEXTS)
        self.register(("logs",), "logs [name] [-n N]", "Show recent Jacky logs.", _logs, contexts=ALL_CONTEXTS)
        ...
        self.register(
            ...
            _config_set,
            mutating=True,
            confirmation="Update Jacky configuration?",
    ...
    def _register_broad_cli_surface(self) -> None:
        """Register non-admin CLI commands that are safe for Jacky Console."""

        extracted = {
            "version": (
                "jacky_cli.subcommands.version",
            ...
            "prompt-size": (
                "jacky_cli.subcommands.prompt_size",
            ...
            "insights": (
                "jacky_cli.subcommands.insights",
            ...
            "security": (
                "jacky_cli.subcommands.security",
            ...
            "import": (
                "jacky_cli.subcommands.import_cmd",
        ...
        self.register(
            ...
            _config_migrate,
            mutating=True,
            confirmation="Update Jacky configuration with missing defaults?",
        )
        self.register(
            ("sessions", "export"),
        ...
        self.register(
            ...
            _adder_handler("send", (), "jacky_cli.send_cmd", "register_send_subparser"),
            mutating=True,
        ...
        _register_command_family(
            ...
            root="portal",
            paths=portal_paths,
            summaries=_adder_summaries("jacky_cli.portal_cli", "add_parser"),
            handler_factory=lambda fixed: _adder_handler(
                "portal",
                fixed,
                "jacky_cli.portal_cli",
                "add_parser",
            ),
        )
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("jacky_cli.projects_cmd", "build_parser"),
            mutating=[
                ("create",),
                ("add-folder",),
                ("remove-folder",),
        ...
        _register_command_family(
            ...
            summaries=_builder_summaries("jacky_cli.kanban", "build_parser"),
            mutating=[
                ("init",),
                ("boards", "create"),
                ("boards", "rm"),
    ...
    def _resolve_command(self, tokens: Sequence[str]) -> tuple[ConsoleCommand, list[str]]:
        ...
        for size in range(min(len(tokens), 3), 0, -1):
            ...
            if command:
                if self.context not in command.contexts:
                    raise ConsoleCommandError(
                        f"`jacky {command.usage}` is not available in "
                        f"{self.context} Jacky Console."
                    )
        ...
        raise ConsoleCommandError(f"Unsupported Jacky Console command: {probe}.{suffix}")
    ...
    def _rejection_for(self, tokens: Sequence[str]) -> str:
        first = tokens[0]
        if first.startswith("-"):
            return f"{first} is not available in Jacky Console."
        blocked_top = {
            "acp",
            "chat",
            "claw",
        ...
        if first in blocked_top:
            return f"`jacky {first}` is not available in Jacky Console."
        blocked_pairs = {
            ("config", "edit"): "`config edit` opens an editor and is not available in Jacky Console.",
            ("mcp", "serve"): "`mcp serve` starts a server and is not available in Jacky Console.",
            ("profile", "alias"): "`profile alias` creates shell wrappers and is not available in Jacky Console.",
            ("skills", "config"): "`skills config` is interactive and is not available in Jacky Console.",
            ("skills", "publish"): "`skills publish` is not available in Jacky Console.",
            ("portal", "login"): "`portal login` is interactive and is not available in Jacky Console.",
            ("portal", "open"): "`portal open` opens a browser and is not available in Jacky Console.",
            ("kanban", "tail"): "`kanban tail` streams output and is not available in Jacky Console.",
            ("kanban", "watch"): "`kanban watch` streams output and is not available in Jacky Console.",
            ("kanban", "daemon"): "`kanban daemon` starts a service and is not available in Jacky Console.",
            ("kanban", "dispatcher"): "`kanban dispatcher` starts a worker and is not available in Jacky Console.",
            ("kanban", "swarm"): "`kanban swarm` starts agent work and is not available in Jacky Console.",
            ("kanban", "decompose"): "`kanban decompose` starts agent work and is not available in Jacky Console.",
            ("kanban", "specify"): "`kanban specify` starts agent work and is not available in Jacky Console.",
            ("kanban", "gc"): "`kanban gc` is not available in Jacky Console.",
        }
        if len(tokens) >= 2:
        ...
        if tuple(tokens[:2]) in {("sessions", "delete"), ("sessions", "prune")}:
            return "`sessions delete` and `sessions prune` are not available in Jacky Console."
        return ""
...
def _enforce_hosted_line_policy(path: tuple[str, ...], args: Sequence[str]) -> None:
    if path == ("config", "set"):
        ...
        if key and not _hosted_config_key_allowed(key):
            raise ConsoleCommandError(
                f"`config set {key}` is not available in hosted Jacky Console. "
    ...
    if path == ("mcp", "add"):
        if _flag_present(args, "--command") or _flag_present(args, "--args"):
            raise ConsoleCommandError(
                "Hosted Jacky Console does not add stdio MCP servers. "
                "Use catalog install or an HTTP/SSE URL."
            )
        if _flag_present(args, "--preset"):
            raise ConsoleCommandError(
                "Hosted Jacky Console does not add MCP presets directly. "
                "Use `mcp install <catalog-name>`."
            )
        url = _flag_value(args, "--url")
        if not url:
            raise ConsoleCommandError(
                "Hosted Jacky Console requires `mcp add` to use --url with "
                "an HTTP/SSE endpoint."
            )
        scheme = urlparse(url).scheme.lower()
        if scheme not in {"http", "https"}:
            raise ConsoleCommandError(
                "Hosted Jacky Console only accepts http:// or https:// MCP URLs."
            )
        return
...
def _apply_confirmed_defaults(args: argparse.Namespace, context: ConsoleContext) -> None:
    ...
    if getattr(args, "auth_action", None) == "add":
        ...
        if auth_type in {"api-key", "api_key"} and not getattr(args, "api_key", None):
            raise ConsoleCommandError("auth add --type api-key requires --api-key in Jacky Console.")
...
def _status(_engine: JackyConsoleEngine, args: list[str]) -> str:
...
def _doctor(_engine: JackyConsoleEngine, args: list[str]) -> str:
...
def _logs(_engine: JackyConsoleEngine, args: list[str]) -> str:
    if "-f" in args or "--follow" in args:
        raise ConsoleCommandError("`logs -f` is not available in Jacky Console.")
    ...
    from jacky_cli.logs import list_logs, tail_log
...
def _sessions_list(_engine: JackyConsoleEngine, args: list[str]) -> str:
...
def _config_show(_engine: JackyConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config show")
    from jacky_cli.config import show_config

    return _capture_output(show_config)


def _config_path(_engine: JackyConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config path")
    from jacky_cli.config import get_config_path

    return str(get_config_path())


def _config_set(_engine: JackyConsoleEngine, args: list[str]) -> str:
    if len(args) < 2:
        raise ConsoleCommandError("Usage: config set <key> <value>")
    key = args[0]
    value = " ".join(args[1:])
    from jacky_cli.config import set_config_value

    return _capture_output(lambda: set_config_value(key, value))


def _config_migrate(_engine: JackyConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "config migrate")

    def _run() -> None:
        from jacky_cli.config import migrate_config
...
def _sessions_export(_engine: JackyConsoleEngine, args: list[str]) -> str:
...
def _sessions_rename(_engine: JackyConsoleEngine, args: list[str]) -> str:
...
def _sessions_optimize(_engine: JackyConsoleEngine, args: list[str]) -> str:
...
def _sessions_repair(_engine: JackyConsoleEngine, args: list[str]) -> str:
...
def _profile_status(_engine: JackyConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "profile")
    return _dispatch_extracted_subcommand(
        root="profile",
        fixed=(),
        args=(),
        module_name="jacky_cli.subcommands.profile",
...
def _cron_list(_engine: JackyConsoleEngine, args: list[str]) -> str:
    parser = _ArgumentParser(prog="cron list", add_help=False)
    parser.add_argument("--all", action="store_true")
    ns = parser.parse_args(args)
    from jacky_cli.cron import cron_list

    return _capture_output(lambda: cron_list(show_all=ns.all))


def _cron_status(_engine: JackyConsoleEngine, args: list[str]) -> str:
    _expect_no_args(args, "cron status")
    from jacky_cli.cron import cron_status

    return _capture_output(cron_status)


def _cron_pause(_engine: JackyConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
        raise ConsoleCommandError("Usage: cron pause <job>")
    from cron.jobs import AmbiguousJobReference, pause_job

    try:
        job = pause_job(args[0], reason="paused from jacky console")
...
def _cron_resume(_engine: JackyConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def _cron_run(_engine: JackyConsoleEngine, args: list[str]) -> str:
    if len(args) != 1:
...
def run_console_repl(
    *,
    ...
    """Run the local ``jacky console`` REPL."""
    ...
    engine = JackyConsoleEngine()
    if interactive:
        print("Jacky Console. Type `help` for commands, `exit` to quit.", file=stdout)

    while True:
        if interactive:
            print("jacky> ", end="", file=stdout, flush=True)
        line = stdin.readline()
        if line == "":
            if interactive:
                print(file=stdout)

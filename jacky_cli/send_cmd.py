"""CLI subcommand: ``jacky send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"jacky send: {file_path} is not a text file. --file reads the "
                ...
                f'  jacky send --to telegram "MEDIA:{file_path}"\n'
                f'  jacky send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  jacky send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"jacky send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"jacky send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"jacky send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"jacky send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"jacky send: no targets found for platform '{platform_filter}'. "
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `jacky gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.jacky/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_jacky_env() -> None:
    """Populate ``os.environ`` from ``~/.jacky/.env`` AND bridge top-level
    ...
    startup that ``jacky send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.jacky/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.jacky/config.yaml`` into
       ...
       via ``jacky config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``jacky send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.jacky/.env and ~/.jacky/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "jacky send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  jacky send --to telegram \"hello\"\n"
            "  jacky send --to discord:#ops --file report.md\n"
            "  jacky send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "jacky send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `jacky send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Jacky "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.jacky/.env + ~/.jacky/config.yaml) — no LLM, "
        ...
        epilog=(
            ...
            "  jacky send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `jacky send --list telegram`.",
    )

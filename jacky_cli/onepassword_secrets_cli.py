"""CLI handlers for ``jacky secrets onepassword ...``.
...
the CLI through OS package managers and signed installers, so Jacky expects
...
# ---------------------------------------------------------------------------
# Argparse wiring — called from jacky_cli.main
# ---------------------------------------------------------------------------
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    console.print(
        Panel.fit(
            ...
            "Jacky resolves [cyan]op://vault/item/field[/cyan] references through your\n"
    ...
    console.print(
        "  Map credentials:  [cyan]jacky secrets onepassword set OPENAI_API_KEY "
        "\"op://Private/OpenAI/api key\"[/cyan]\n"
        "  Preview:          [cyan]jacky secrets onepassword sync[/cyan]\n"
        "  Status:           [cyan]jacky secrets onepassword status[/cyan]"
    )
    return 0
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if not enabled:
        console.print("\n  Run [cyan]jacky secrets onepassword setup[/cyan] to enable.")
        return 0
    if binary and not token_set:
        ...
        if who:
            ...
            console.print(
                ...
                "Jacky will warn and skip 1Password on next startup.[/yellow]"
            )
    if not references:
        console.print(
            "\n  [yellow]No references mapped yet.[/yellow]  Add one: "
            "[cyan]jacky secrets onepassword set ENV_VAR \"op://…\"[/cyan]"
        )
    return 0
...
def cmd_set(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "[cyan]jacky secrets onepassword setup[/cyan] to turn it on.[/yellow]"
        )
    return 0
...
def cmd_sync(args: argparse.Namespace) -> int:
    ...
    if not op_cfg.get("enabled"):
        console.print(
            ...
            "`jacky secrets onepassword setup` first.[/yellow]"
        )
        return 1
    ...
    if not references:
        console.print(
            ...
            "`jacky secrets onepassword set ENV_VAR \"op://…\"`.[/yellow]"
        )
        return 0
    ...
    console.print(
        ...
        "[cyan]jacky[/cyan] invocation.  Re-run with [cyan]--apply[/cyan] to export "
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    console.print(
        ...
        "next Jacky invocation.\n"
        "  Your reference mappings are left in config.yaml — remove them with "
        "[cyan]jacky secrets onepassword remove ENV_VAR[/cyan] if you no longer "
        "need them."
    )
    return 0

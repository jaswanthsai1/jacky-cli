"""CLI subcommand: ``jacky pets <subcommand>``.
...
def _cmd_list(args) -> int:
    ...
    if getattr(args, "installed", False):
        ...
        if not pets:
            _print("No pets installed. Try: jacky pets install boba")
            return 0
    ...
    _print("\nInstall one with: jacky pets install <slug>")
    return 0
...
def _cmd_install(args) -> int:
    ...
    if getattr(args, "select", False) or not _has_active_pet():
        ...
        _print(f"  Make it active with: jacky pets select {slug}")
    return 0
...
def _cmd_select(args) -> int:
    ...
    if not slug:
        pets = store.installed_pets()
        if not pets:
            _err("✗ no pets installed — run: jacky pets install boba")
            return 1
    ...
    if pet is None or not pet.exists:
        _err(f"✗ '{slug}' is not installed — run: jacky pets install {slug}")
        return 1
...
def _cmd_show(args) -> int:
    ...
    if pet is None:
        _err("✗ no pet to show — run: jacky pets install boba")
        return 1
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        _print("  → no pets installed. Run: jacky pets install boba")
        ok = False
    elif active is None:
        _print("  → active pet unresolved. Run: jacky pets select <slug>")
        ok = False
    elif not enabled:
        _print("  → pet display is disabled. Run: jacky pets select " + active.slug)

"""Centralized logging setup for Jacky Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.jacky/logs/`` (profile-aware via ``get_jacky_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``jacky_cli.web_server``,
    ``jacky_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Jacky (TUI, gateway, ``hy_memory`` server, MCP
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_jacky_session_injector", False):
    ...
    _session_record_factory._jacky_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``jacky logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``jacky logs --component gateway``.
    "gateway": ("gateway", "jacky_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("jacky_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "jacky_cli.web_server",
        "jacky_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    ...
    """Configure the Jacky logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    jacky_home
        Override for the Jacky home directory.  Falls back to
        ``get_jacky_home()`` (profile-aware).
    log_level
    ...
    home = jacky_home or get_jacky_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_jacky_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._jacky_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the jacky group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from jacky_cli.config import is_managed
...
# the module header): when several Jacky processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._jacky_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="jacky-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_jacky_queue", False):

...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Jacky'
...
The actual runtime resolution happens in jacky_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "jacky"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, jacky-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Jacky' MCP servers + Codex's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Jacky for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from jacky_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the jacky-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "jacky-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Permissions + Jacky tool callback are always-on production
            ...
            if "jacky-tools" in mig_report.migrated:
                msg_lines.append(
                    "Jacky tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Jacky runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            ...
            "Jacky tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("OpenAI/Codex turns will use the default Jacky runtime.")

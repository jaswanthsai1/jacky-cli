"""jacky webhook — manage dynamic webhook subscriptions from the CLI.
...
Subscriptions persist to ~/.jacky/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _jacky_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: jacky gateway run
"""
...
def webhook_command(args):
    """Entry point for 'jacky webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: jacky webhook {subscribe|list|remove|test}")
        print("Run 'jacky webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (jacky gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: jacky webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from jacky webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (jacky gateway run)")

"""Slash command definitions and autocomplete for the Jacky CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Jacky config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Jacky works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while Jacky is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.jacky/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update Jacky Agent to the latest version", "Info"),
    CommandDef("version", "Show Jacky Agent version", "Info", aliases=("v",)),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`jacky_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/jacky`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Jacky ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Jacky
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``jacky skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``jacky skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``jacky skills list`` and
...
# unrelated command landed. These claim their slots right after /jacky,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /jacky <command>).
...
# native slot, the alias spelling stays reachable via /jacky reset).
...
# ``/jacky <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/jacky" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /jacky credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /jacky billing.
#   - moa: high-cost slash mode, available through /jacky moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /jacky debug on Slack.
_SLACK_VIA_JACKY_ONLY = frozenset({"credits", "billing", "moa", "debug"})
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/jacky <verb>`` subcommand.
    ...
    can still reach them via ``/jacky <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/jacky`` is always reserved as the first entry so the
    legacy ``/jacky <subcommand>`` form keeps working for anything that
    ...
    # Reserve /jacky as the catch-all top-level command.
    entries.append(("jacky", "Talk to Jacky or run a subcommand", "[subcommand] [args]"))
    seen.add("jacky")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_JACKY_ONLY:
            # Intentionally Slack-via-/jacky only (see _SLACK_VIA_JACKY_ONLY).
            return
    ...
    # everything except /jacky, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://jacky-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /jacky handler.

    Maps both canonical names and aliases so /jacky bg do stuff works
    the same as /jacky background do stuff.

    Plugin-registered slash commands are included so ``/jacky <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from jacky_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]

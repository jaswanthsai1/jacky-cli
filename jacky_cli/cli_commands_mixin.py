...
``cli.py``'s ``JackyCLI`` class. ``JackyCLI`` inherits ``CLICommandsMixin`` so
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: jacky --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Jacky config/state.
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_jacky_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from jacky_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run jacky chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in jacky_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `jacky gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `jacky sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from jacky_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `jacky sessions list` to see available sessions.")
            return
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from jacky_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from jacky_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from jacky_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to jacky_cli.curator so the CLI and the `jacky curator`
        ...
        try:
            from jacky_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            if out is not None:
                print(out)
                return
        from jacky_cli.skills_hub import handle_skills_slash
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``jacky bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: jacky bundles create "
        ...
        _cprint(
            ...
            f"Manage with `jacky bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Jacky-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"Jacky keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_jacky_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="jacky_prompt_")
        try:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Jacky is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Jacky is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Jacky is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from jacky_cli.models import _is_anthropic_fast_model
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update Jacky Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``jacky update`` so the user sees update output directly and gets
        ...
        if is_managed():
            print(f"  ✗ {format_managed_message('update Jacky Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Jacky Agent"),

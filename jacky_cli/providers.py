"""
Single source of truth for provider identity in Jacky Agent.
...
2. **Jacky overlays** — transport type, auth patterns, aggregator flags,
...
# -- Jacky overlay ----------------------------------------------------------
# Jacky-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class JackyOverlay:
    """Jacky-specific provider metadata layered on top of models.dev."""
...
JACKY_OVERLAYS: Dict[str, JackyOverlay] = {
    "moa": JackyOverlay(
        transport="openai_chat",
        auth_type="virtual",
        base_url_override="moa://local",
    ),
    "openrouter": JackyOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "openai-codex": JackyOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "openai-api": JackyOverlay(
        transport="codex_responses",
        base_url_override="https://api.openai.com/v1",
        base_url_env_var="OPENAI_BASE_URL",
    ),
    "xai-oauth": JackyOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://api.x.ai/v1",
        base_url_env_var="XAI_BASE_URL",
    ),
    "qwen-oauth": JackyOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="JACKY_QWEN_BASE_URL",
    ),
    "lmstudio": JackyOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": JackyOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": JackyOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": JackyOverlay(
    ...
    "kimi-for-coding": JackyOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": JackyOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": JackyOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": JackyOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": JackyOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": JackyOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": JackyOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": JackyOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "opencode": JackyOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": JackyOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "azure-foundry": JackyOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": JackyOverlay(
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "jacky", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Jacky overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Jacky overlay
    ...
    overlay = JACKY_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + jacky extra
    ...
    if overlay is not None:
        # Jacky-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in JACKY_OVERLAYS
    if provider == "bedrock":

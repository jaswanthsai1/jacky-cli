...
# xAI's public API is OpenAI-compatible for the endpoints Jacky commonly
# uses. The Responses endpoint is included because Jacky' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Jacky-managed OAuth credentials."""

    auth_hint = "jacky auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`jacky auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`jacky auth reset xai-oauth` or re-authenticate with "
                    "`jacky auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `jacky auth add xai-oauth --type oauth`."
            )

"""
Configuration management for Jacky Agent.

Config files are stored in ~/.jacky/ for easy access:
- ~/.jacky/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.jacky/.env         - API keys and secrets

This module provides:
- jacky config          - Show current configuration
- jacky config edit     - Open config in editor
- jacky config set      - Set a specific value
- jacky config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``jacky config set`` (which rewrites
    ...
    ``config.yaml`` in place: jacky never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.jacky/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``jacky logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  jacky config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Jacky makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Jacky itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``jacky update``, the TUI build.
...
#   on every plugin install / ``jacky update``.
...
# * ``JACKY_HOME`` / ``JACKY_PROFILE`` / ``JACKY_CONFIG`` /
#   ``JACKY_ENV`` — Jacky runtime location flags. Writing these into
...
#
# IMPORTANT: ``JACKY_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (JACKY_LANGFUSE_PUBLIC_KEY,
# JACKY_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Jacky runtime location — never via dashboard env writer.
    # NOT a JACKY_* blanket: integration credentials (JACKY_GEMINI_*,
    # JACKY_LANGFUSE_*, JACKY_SPOTIFY_*, ...) ARE allowed.
    "JACKY_HOME", "JACKY_PROFILE", "JACKY_CONFIG", "JACKY_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Jacky runtime location "
            "(JACKY_HOME, JACKY_PROFILE, ...) cannot be persisted via "
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "JACKY_TOOL_PROGRESS", "JACKY_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `jacky plugins enable
    # observability/langfuse` or `jacky tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "JACKY_LANGFUSE_ENV",
    "JACKY_LANGFUSE_RELEASE",
    "JACKY_LANGFUSE_SAMPLE_RATE",
    "JACKY_LANGFUSE_MAX_CHARS",
    "JACKY_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from jacky_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("JACKY_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_jacky_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Jacky is running in package-manager-managed mode.

    Two signals: the JACKY_MANAGED env var (set by the systemd service),
    or a .managed marker file in JACKY_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade jacky-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``jacky_cli/`` — i.e. the git checkout for source
    installs, ``/opt/jacky`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$JACKY_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Jacky was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$JACKY_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. JACKY_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.jacky``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$JACKY_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.jacky:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$JACKY_HOME``, a
    ...
    ``docker`` and ``jacky update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``jaswanthsai1/jacky-cli`` image bakes a ``docker``
        stamp into ``/opt/jacky`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $JACKY_HOME.
    try:
    ...
    #    host install whose shared $JACKY_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``jacky update``.
    try:
        method = (
            (get_jacky_home() / ".install_method")
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$JACKY_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/jacky`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Jacky lives in a ``uv tool`` layout.

    ``uv tool install jacky-agent`` places the install at
    ``.../uv/tools/jacky-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``jacky-agent`` happens to be uv-tool-installed on the machine while
    the *active* Jacky is a regular pip/venv install, causing
    ``jacky update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/jacky-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade jacky-agent"
    if method == "docker":
        return "docker pull jaswanthsai1/jacky-cli:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade jacky-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade jacky-agent"
        return "pip install --upgrade jacky-agent"
    return "jacky update"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``jacky update`` / ``jacky update --check`` so the wording — and the
...
# Long-form text for ``jacky update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# jacky_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Jacky, it doesn't update
...
✗ ``jacky update`` doesn't apply inside the Docker container.

Jacky Agent runs as a published image (jaswanthsai1/jacky-cli), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull jaswanthsai1/jacky-cli:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate jacky-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm jaswanthsai1/jacky-cli:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/jaswanthsai1/jacky-cli/tags
  • Your config and session history live under ``$JACKY_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``jacky update`` inside Docker.
...
def format_managed_message(action: str = "modify this Jacky installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("JACKY_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Jacky installation is managed by NixOS "
            f"(JACKY_MANAGED={env_hint}).\n"
            "Edit services.jacky-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Jacky installation is managed by Homebrew "
            f"(JACKY_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade jacky-agent"
        )

    return (
        f"Cannot {action}: this Jacky installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Jacky."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from JACKY_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, jacky_bin
    or None if container mode is not active, we're already inside the
    container, or JACKY_DEV=1 is set.
    ...
    container_mode_file = get_jacky_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "jacky-agent")
    exec_user = info.get("exec_user", "jacky")
    jacky_bin = info.get("jacky_bin", "/data/current-package/bin/jacky")
...
def get_config_path() -> Path:
    ...
    return get_jacky_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_jacky_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_jacky_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the JACKY_UID / JACKY_GID env vars set by Docker deployments.

    Docker containers running Jacky commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level JACKY_HOME once, but
    subdirectories created at runtime by ``ensure_jacky_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("JACKY_UID", "").strip()
    gid_str = os.environ.get("JACKY_GID", "").strip()
    try:
...
def _chown_to_jacky_uid(path) -> None:
    """Chown ``path`` to ``JACKY_UID:JACKY_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_jacky_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_jacky_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the jacky group can
    share state with the gateway service.

    The mode can be overridden via the JACKY_HOME_MODE environment variable
    (e.g. JACKY_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse JACKY_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``JACKY_UID``/``JACKY_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("JACKY_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_jacky_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Jacky runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("JACKY_CONTAINER") or os.environ.get("JACKY_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set JACKY_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into JACKY_HOME, upgrading legacy empty templates.
...
def ensure_jacky_home():
    """Ensure ~/.jacky directory structure exists with secure permissions.
    ...
    """
    home = get_jacky_home()
    # Named profiles must be created explicitly (e.g. ``jacky profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old JACKY_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_jacky_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_jacky_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"JACKY_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Jacky-level retry loop that wraps the whole call.  Lower
        ...
        # prompt's environment-hints block. Lets a host that wraps Jacky
        ...
        # identity slot (SOUL.md). Empty by default. The JACKY_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Jacky adds a coding operating brief
    ...
    "terminal": {
        ...
        #             JACKY_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force JACKY_HOME/home when it exists (old strict
        ...
        # skipped. When empty, Jacky auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Jacky sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Jacky
        ...
        # bundled Jacky image, which drops to the `jacky` user via
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Jacky sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Jacky should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``jacky chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.jacky/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: jacky sweeps the checkpoint base at startup
    ...
    # AGENTS.md, CLAUDE.md, .jacky.md, or .cursorrules before Jacky applies
    ...
    # single read_file page exceeds these limits, Jacky truncates the
    ...
    "compression": {
        ...
        "codex_app_server_auto": "native",  # Codex app-server (codex CLI runtime) thread
                                      ...
                                      # thread context, so Jacky' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); jacky =
                                      # Jacky' compression threshold triggers
    ...
    # See tools/kanban_tools.py and jacky_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``jacky dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``jacky kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``jacky kanban decompose`` and the kanban
        ...
        # ``jacky profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `jacky model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `jacky` (and `jacky chat`) launches by default:
        ...
        # REPL and `--tui` (or JACKY_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `jacky --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `jacky -c` muscle memory.  Default off so existing
        # users aren't surprised.  JACKY_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `jacky --tui` drops a one-time hint
        ...
        # can set `path`/`paths` to JACKY_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/jacky`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `jacky pets`. Disabled until
        ...
        "pet": {
            ...
            # get_jacky_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``JACKY_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``JACKY_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``JACKY_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # ``JACKY_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``JACKY_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/jacky``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Jacky appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.jacky/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.jacky/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.jacky/cache/delegation/
    ...
    # response. If not, Jacky feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Jacky auto-pauses the goal and
    ...
    "moa": {
        ...
        # <jacky_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.jacky/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${JACKY_SKILL_DIR} and ${JACKY_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `jacky curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `jacky curator run --consolidate` overrides
        ...
        # normally restored on every `jacky update`, so pruning them only
        ...
        # skipped), snapshot ~/.jacky/skills/ into
        # ~/.jacky/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `jacky curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for jacky-specific overrides; everything else
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (JACKY_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Jacky' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.jacky/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or JACKY_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `jacky doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``jacky_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Jacky to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $JACKY_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # Also overridable via JACKY_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `jacky -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `jacky` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `jacky kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     jacky-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Jacky tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.jacky/logs/.
    ...
    # update model picker lists without shipping a jacky-agent release.
    ...
    "gateway": {
        ...
        # internal JACKY_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.jacky/sessions/sessions.json entirely.
        ...
        # the JACKY_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.jacky.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.jacky/.env,
        ...
        # behavior: files must live under the Jacky cache, under
        ...
        # user. Bridged to JACKY_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Jacky
        # cache (~/.jacky/cache/{documents,images,audio,video,screenshots})
        ...
        # to JACKY_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # JACKY_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to JACKY_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.jacky/state.db.
    ...
    "sessions": {
        ...
        # default of ``jacky sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.jacky/sessions/session_{sid}.json`` on every turn
    ...
    # ``jacky update`` behaviour.
    "updates": {
        # Run a full ``jacky backup``-style zip of JACKY_HOME before every
        # ``jacky update``.  Backups land in ``<JACKY_HOME>/backups/`` and
        # can be restored with ``jacky import <path>``.  Off by default:
        # zipping a large JACKY_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``jacky update
        ...
        "backup_keep": 5,
        # What `jacky update` does with uncommitted local changes to the
    ...
    "lsp": {
        ...
        #              ``<JACKY_HOME>/lsp/bin/`` on first use.
    ...
    # `jacky tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.jacky/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.jacky/.env (or your shell) and never in config.yaml.
            ...
            # ~/.jacky/bin/ on first use.  When False you must install
        ...
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.jacky/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Jacky disables it for our users unless they
        # explicitly opt in here. When false (default), Jacky sets
    ...
    # Jacky Desktop (Electron app) launch options. These only affect
    # `jacky desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the JACKY_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
OPTIONAL_ENV_VARS = {
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Jacky mints short-lived tokens from. Falls back to "
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'jacky model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Jacky auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Jacky, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @jacky:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. JACKY_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #jacky)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: jacky-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'jacky-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Jacky API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Jacky API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Jacky API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # JACKY_TOOL_PROGRESS and JACKY_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.jacky.config`` entries, then checks
    ...
    try:
        ...
        # should never break `jacky update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Jacky' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Jacky versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``jacky_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'jacky setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Jacky won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'jacky doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("JACKY_HOME", "~/.jacky")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "jacky update / jacky -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("JACKY_TOOL_PROGRESS")
            old_mode = get_env_value("JACKY_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from JACKY_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from JACKY_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("JACKY_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from JACKY_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$JACKY_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_jacky_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `jacky plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `jacky curator status` has no
    ...
    #   3. Creates `~/.jacky/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_jacky_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_jacky_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `jacky config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `jacky config set`"
                    )
    ...
    # behavior set it to true explicitly via `jacky config set`.
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from jacky_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: jacky config set <key> <value>")
    ...
    # `jacky update`, but only the version bump is persisted.
    ...
    # metadata.jacky.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_jacky_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``jacky config set``
    ...
    backends) — while display paths (``jacky status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _jacky_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.jacky/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.jacky/config.yaml.
    ...
    (which change ``JACKY_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/jacky/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_jacky_dotenv()
...
#   openai-codex (OAuth — jacky auth) — OpenAI Codex
#   nous         (OAuth — jacky auth) — Nous Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.jacky/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Jacky writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.jacky/.env.
    ...
    interactive menu render (`jacky tools`, `jacky setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `jacky tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Jacky env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.jacky/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.jacky/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.jacky/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.jacky/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Jacky — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Jacky vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.jacky/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.jacky/.env`` over ``os.environ``.

    Used for Jacky-managed credentials where a deliberate edit to ``.env``
...
def show_config():
    ...
    from jacky_cli.auth import get_anthropic_key
    ...
    # Warn on stale JACKY_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("JACKY_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale JACKY_MAX_ITERATIONS={_env_ghost} "
                f"(run 'jacky doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  jacky config edit     # Edit config file", Colors.DIM))
    print(color("  jacky config set <key> <value>", Colors.DIM))
    print(color("  jacky setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `jacky config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `jacky config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: jacky config set <key> <value>")
            print()
            print("Examples:")
            print("  jacky config set model anthropic/claude-sonnet-4")
            print("  jacky config set terminal.backend docker")
            print("  jacky config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'jacky config migrate' to add them")
        ...
        print("  jacky config           Show current configuration")
        print("  jacky config edit      Open config in editor")
        print("  jacky config set <key> <value>   Set a config value")
        print("  jacky config check     Check for missing/outdated config")
        print("  jacky config migrate   Update config with new options")
        print("  jacky config path      Show config file path")
        print("  jacky config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``jacky config`` UI so users

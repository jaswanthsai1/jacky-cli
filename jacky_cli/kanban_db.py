...
``<root>`` is the **shared Jacky root** (the parent of any active
...
``jacky -p <profile>`` joins the same board as the dispatcher that
...
the profile's Jacky home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``JACKY_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``JACKY_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``jacky kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.jacky``. In Docker / custom
deployments where ``JACKY_HOME`` points outside ``~/.jacky`` (e.g.
``/opt/jacky``), ``<root>`` is ``JACKY_HOME``. Legacy env-var
overrides still work:

* ``JACKY_KANBAN_DB`` — pin the database file path directly.
* ``JACKY_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``JACKY_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``JACKY_KANBAN_DB``,
``JACKY_KANBAN_WORKSPACES_ROOT``, and ``JACKY_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/jacky-kanban-v1-spec.pdf`` for the full
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active JACKY_HOME so dispatcher- and
...
# ``JACKY_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``JACKY_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("JACKY_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``JACKY_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("JACKY_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``JACKY_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "JACKY_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "jacky_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``jacky-agent``
...
def kanban_home() -> Path:
    """Return the shared Jacky root that anchors the kanban board.

    Resolution order:

    1. ``JACKY_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_jacky_root()``, which already returns ``<root>``
       when ``JACKY_HOME`` is ``<root>/profiles/<name>``, and returns
       ``JACKY_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``JACKY_HOME`` would silently fork the board per profile,
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``jacky kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``JACKY_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``jacky kanban boards
    ...
    env = os.environ.get("JACKY_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``jacky kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``JACKY_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("JACKY_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``JACKY_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("JACKY_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``JACKY_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("JACKY_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``jacky kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``JACKY_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (jacky_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``JACKY_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("JACKY_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``jacky gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Jacky treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``JACKY_KANBAN_DB`` env → ``JACKY_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `jacky kanban list` mid-integrity-probe) block
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``jacky kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``JACKY_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from jacky_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``jacky --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from jacky_cli import projects_db as _pdb
    ...
    # `jacky --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``jacky kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `jacky kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``jacky kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``jacky kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``jacky kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``jacky kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``JACKY_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Jacky' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Jacky-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``jacky kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Jacky checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Jacky treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Jacky creates/reuses that linked worktree. With
      no ``workspace_path``, Jacky anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Jacky uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``JACKY_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Jacky
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``jacky kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``jacky kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Jacky profile.
    ...
    if not rows:
        return False
    try:
        from jacky_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from jacky_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Jacky profile.
        # `_default_spawn` invokes ``jacky -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Jacky
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `jacky kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from jacky_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_jacky_argv() -> list[str]:
    """Return the interpreter-bound Jacky CLI invocation."""
    # ``jacky_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``jacky`` package — there is no
    # ``jacky`` package to import.
    return [sys.executable, "-m", "jacky_cli.main"]


def _absolute_jacky_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Jacky shim."""
...
def _jacky_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Jacky executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_jacky_argv()
    return [_absolute_jacky_path(path)]


def _resolve_jacky_argv() -> list[str]:
    """Resolve the ``jacky`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$JACKY_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("jacky")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\jacky.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m jacky_cli.main`` — fallback for setups where
       Jacky is launched from a venv and the ``jacky`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_jacky_bin`` for the same reason. Kept
    local (not imported from gateway) because ``jacky_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("JACKY_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _jacky_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _jacky_path_argv(resolved_env_bin)
        return _module_jacky_argv()

    jacky_bin = _safe_which_no_cwd("jacky") if _IS_WINDOWS else shutil.which("jacky")
    if jacky_bin:
        return _jacky_path_argv(jacky_bin)
    return _module_jacky_argv()
...
def _resolve_worker_cli_toolsets(jacky_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``JACKY_KANBAN_TASK`` is set.
    """
    if not jacky_home:
        return None
    try:
        ...
        token = set_jacky_home_override(jacky_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for JACKY_HOME=%r (%s)",
            jacky_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``jacky -p <profile> chat -q ...`` subprocess.
    ...
    ``JACKY_KANBAN_DB`` / ``JACKY_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject JACKY_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `jacky -p <name>` the
    # _apply_profile_override() runs *before* jacky_constants is imported.
    # If JACKY_HOME is absent from the child's env, get_jacky_home() falls
    # back to Path.home() / ".jacky" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from jacky_cli.profiles import resolve_profile_env
    try:
        env["JACKY_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via JACKY_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # JACKY_HOME never had profiles created.
        pass
    if task.tenant:
        env["JACKY_TENANT"] = task.tenant
    env["JACKY_KANBAN_TASK"] = task.id
    env["JACKY_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["JACKY_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["JACKY_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["JACKY_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["JACKY_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["JACKY_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`jacky -p <name>`
    # rewrites JACKY_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_jacky_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["JACKY_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["JACKY_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["JACKY_KANBAN_BOARD"] = resolved_board
    # JACKY_PROFILE is the author the kanban_comment tool defaults to.
    # `jacky -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["JACKY_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited JACKY_TUI=1
    ...
    # older jacky builds on PATH that predate the flag's precedence.
    env.pop("JACKY_TUI", None)

    cmd = [
        *_resolve_jacky_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped JACKY_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("JACKY_HOME"))
    if worker_toolsets:
    ...
    # `jacky kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`jacky` executable not found on PATH. "
            "Install Jacky Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``jacky kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled JACKY_PROFILE values like "jacky-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Jacky root exists

    Reads profile paths directly so this module has no import dependency on
    ``jacky_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from jacky_cli.jacky_constants import get_default_jacky_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``jacky kanban assignees`` for the terminal.

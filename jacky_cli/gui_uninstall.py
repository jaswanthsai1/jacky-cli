"""
Jacky Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``jacky desktop`` / ``jacky gui``)
     Built inside the agent checkout under ``$JACKY_HOME/jacky-agent/``:
       ...
       - ``$JACKY_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Jacky.app`` or ``~/Applications/Jacky.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Jacky`` (NSIS per-user)
...
the app name ("Jacky"), separate from ``$JACKY_HOME``:
  - macOS:   ``~/Library/Application Support/Jacky``
  - Windows: ``%APPDATA%\\Jacky``
  - Linux:   ``$XDG_CONFIG_HOME/Jacky`` (default ``~/.config/Jacky``)
...
``jacky uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(jacky_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return jacky_home / "jacky-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Jacky"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Jacky"
...
def source_built_gui_artifacts(jacky_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``jacky desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``jacky-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(jacky_home)
    ...
    return [
        ...
        jacky_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Jacky" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Jacky.app"),
            home / "Applications" / "Jacky.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Jacky).
            local_base / "Programs" / "Jacky",
            # Older / alternate layout some builds used.
            local_base / "jacky-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Jacky")
    else:
        ...
        paths += [
            data_base / "applications" / "jacky.desktop",
            data_base / "applications" / "Jacky.desktop",
        ]
    return paths


def agent_is_installed(jacky_home: Path) -> bool:
    """Return True when a usable Python agent install exists under JACKY_HOME.
    ...
    if (agent_root / "jacky_cli").is_dir():
        return True
...
def gui_is_installed(jacky_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(jacky_home):
        if p.exists():
            return True
...
def gui_install_summary(jacky_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = jacky_home if jacky_home is not None else get_jacky_home()
...
def uninstall_gui(jacky_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``jacky-agent/jacky_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$JACKY_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove jacky' or "
            "'sudo dnf remove jacky'). AppImage builds are a single file you "

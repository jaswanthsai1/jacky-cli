...
1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with jacky-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.jacky/plugins/<name>/``
3. **Project plugins** – ``./.jacky/plugins/<name>/`` (opt-in via
   ``JACKY_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``jacky_agent.plugins``
   entry-point group.
...
# ---------------------------------------------------------------------------
#
# Set ``JACKY_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.jacky/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("JACKY_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When JACKY_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Jacky loggers.
    """
    ...
    logger.debug(
        "JACKY_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # finish. Jacky' shipped guidance lives in the evidence-based
    ...
    # Kanban task lifecycle hooks. Fired by jacky_cli.kanban_db when a task
    ...
    # separate `jacky -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `jacky kanban dispatch`),
...
ENTRY_POINTS_GROUP = "jacky_agent.plugins"

_NS_PARENT = "jacky_plugins"
...
class PluginManifest:
    ...
    #              in ~/.jacky/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``jacky plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Jacky profile name (e.g. ``"default"``).

        Derived from ``JACKY_HOME`` via
        :func:`jacky_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.jacky/profiles/<name>``, or ``"custom"`` when
        ``JACKY_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Jacky core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``jacky honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``jacky <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`jacky_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from jacky_cli.dashboard_auth import (
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_jacky_dotenv()`` startup — after
        ``~/.jacky/.env`` loads, before Jacky reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Jacky' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``jacky model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: jacky_cli.main imports plugins indirectly
        from jacky_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_skill(
        ...
        ``~/.jacky/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if env_var_enabled("JACKY_SAFE_MODE"):
            logger.info("JACKY_SAFE_MODE=1 — plugin discovery skipped")
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to jacky_cli/. Two layouts are
        ...
        # 2. User plugins (~/.jacky/plugins/)
        user_dir = get_jacky_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.jacky/plugins/)
        if _env_enabled("JACKY_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".jacky" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set JACKY_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with jacky and must
            ...
            # all ~20 of them added several seconds to every `jacky`
            # invocation — including plain `jacky chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Jacky ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `jacky plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``jacky plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`jacky plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `jacky plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``jacky_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``jacky_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="jacky-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``jacky tools`` TUI so plugin-provided toolsets appear

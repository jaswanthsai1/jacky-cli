"""
Dump command for jacky CLI.

Outputs a compact, plain-text summary of the user's Jacky setup
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.jacky/.env.
    ...
    exports. ``jacky debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.jacky_build_sha`` by the Dockerfile's
    ``JACKY_GIT_SHA`` build-arg (see ``jacky_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``jacky dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from jacky_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(jacky_home: Path) -> int:
...
def _cron_summary(jacky_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = jacky_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"jacky_home:      {display_jacky_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.jacky/.env: a managed
        ...
        # A credential added via `jacky auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `jacky auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["jacky-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(jacky_home)}")
    lines.append(f"  skills:             {_count_skills(jacky_home)}")

#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the Jacky Skills Hub.

Powers both:
  - `jacky skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    if len(exact) > 1:
        ...
        # so users can copy it for `jacky skills install`.
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.jacky/skills/`` that look
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.jacky/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    # into `jacky skills install` fail.
    ...
    c.print(table)
    c.print("[dim]Use: jacky skills inspect <identifier> to preview, "
            "jacky skills install <identifier> to install "
            "(--json for scripting)[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    # skips the external API sources and serves everything from "jacky-index".
    ...
    # The identifier is what you pass to `jacky skills install`. Browse used
    ...
    c.print("[dim]Tip: 'jacky skills inspect <identifier>' to preview, "
            "'jacky skills install <identifier>' to install, "
            "'jacky skills search <query>' to search deeper[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]jacky skills install {url} --name <your-name>[/]\n\n"
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                ...
                "It ships with jacky-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_jacky_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    # metadata.jacky.blueprint block, it is a runnable automation. Register it as
        ...
        if spec is not None:
            ...
            if registered is not None:
                ...
                c.print(
                    ...
                    "or via[/] [bold]jacky cron add[/][dim].[/]\n"
                )
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="jacky skills install <id> to install"))
...
def browse_skills(page: int = 1, page_size: int = 20, source: str = "all") -> dict:
    ...
    page_size = max(1, min(page_size, 100))
    _TRUST_RANK = {"builtin": 3, "trusted": 2, "community": 1}
    # "jacky-index" must carry a high limit: when the index is available the
    ...
    _PER_SOURCE_LIMIT = {"jacky-index": 5000, "official": 100, "skills-sh": 100,
    ...
    # source-skip logic — querying jacky-index AND the external APIs at once
...
def do_list(source_filter: str = "all",
    ...
    config — ``jacky -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``JACKY_HOME`` at process
...
def do_list_modified(console: Optional[Console] = None,
    ...
    """List bundled skills the user has edited (which `jacky update` keeps)."""
    ...
    c.print(f"\n[bold]{len(modified)} user-modified bundled skill(s)[/] "
            "[dim](kept as-is by `jacky update`):[/]")
    for entry in modified:
        c.print(f"  [yellow]~[/] {entry['name']}")
    c.print()
    c.print("[dim]See changes:   jacky skills diff <name>[/]")
    c.print("[dim]Resume updates: jacky skills reset <name>          (keep your copy, re-baseline)[/]")
    c.print("[dim]Revert to stock: jacky skills reset <name> --restore[/]\n")
...
def do_diff(name: str, console: Optional[Console] = None) -> None:
    ...
    c.print()
    c.print(f"[dim]Revert with: jacky skills reset {name} --restore[/]\n")
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: jacky skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: jacky skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: jacky skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_jacky_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via Jacky Skills Hub.\n\n"
                        f"This skill was scanned by the Jacky Skills Guard before submission.",
...
def skills_command(args) -> None:
    """Router for `jacky skills <subcommand>` — called from jacky_cli/main.py."""
    ...
    if action == "browse":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: jacky skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: jacky skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: jacky skills [browse|search|install|inspect|list|list-modified|diff|check|update|audit|uninstall|reset|opt-out|opt-in|publish|snapshot|tap]\n")
        _console.print("Run 'jacky skills <command> --help' for details.\n")

...
the tool registry or launching Jacky (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``jacky-cli`` to the non-existent ``jacky``. ``resolve_toolset('jacky')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``jacky-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``jacky`` instead of ``jacky-cli``),
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `jacky tools` to reconfigure."
        )
    return warnings

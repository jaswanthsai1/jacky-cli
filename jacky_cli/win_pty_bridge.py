"""Windows ConPTY bridge for the `jacky dashboard` chat tab.

Drop-in counterpart to ``jacky_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``jacky_cli.web_server`` consumes: ``spawn``, ``read``,

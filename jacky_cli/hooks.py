"""jacky hooks — inspect and manage shell-script hooks.

Usage::

    jacky hooks list
    jacky hooks test <event> [--for-tool X] [--payload-file F]
    jacky hooks revoke <command>
    jacky hooks doctor

Consent records live under ``~/.jacky/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.jacky/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``jacky hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: jacky hooks {list|test|revoke|doctor}")
        print("Run 'jacky hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.jacky/config.yaml.")
        print("See `jacky hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `jacky hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `jacky hooks test` and `jacky hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Jacky wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `jacky hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `jacky hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `jacky hooks doctor`.")

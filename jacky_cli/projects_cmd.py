"""``jacky project`` CLI — manage first-class, multi-folder Projects.
...
convention. State lives in the per-profile ``$JACKY_HOME/projects.db`` store
(see :mod:`jacky_cli.projects_db`).
...
def build_parser(
    ...
    p_create.add_argument("name", help="Human name, e.g. 'Jacky Agent'")
...
def projects_command(args: argparse.Namespace) -> int:
    """Entry point from ``jacky project …`` argparse dispatch."""
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: jacky project <action> [options]\n"
                "Run 'jacky project --help' for the full list.",
                file=sys.stderr,
            )
        return 0
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if not projs:
        print("No projects yet. Create one with `jacky project create <name>`.")
        return 0
    for p in projs:
...
def _cmd_set_primary(args, conn, proj) -> int:
    if not pdb.set_primary(conn, proj.id, args.path):
        print(
            ...
            f"add it first with `jacky project add-folder`.",
            file=sys.stderr,
        )
        return 1

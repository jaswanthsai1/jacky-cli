"""
Jacky Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m jacky_cli.main web          # Start on http://127.0.0.1:9119
    python -m jacky_cli.main web --port 8080
"""
...
from jacky_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
try:
    ...
    # running `jacky dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["JACKY_WEB_DIST"]) if "JACKY_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``jacky gateway run`` — but the
    desktop app spawns a ``jacky dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same JACKY_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import jacky_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from jacky_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire jacky_cli.gateway import into a background thread so the event
    # loop is not blocked and JACKY_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (JACKY_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `jacky
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("JACKY_DESKTOP") == "1":
...
app = FastAPI(title="Jacky Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from jacky_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# JACKY_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("JACKY_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Jacky-Session-Token"
...
#
# This list is defined in ``jacky_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from jacky_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Jacky-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``jacky-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from jacky_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from jacky_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from jacky_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Jacky (no terminal prompt), "
    ...
    "updates.refresh_cua_driver": {
        "type": "bool",
        "description": (
            "Refresh an already-installed cua-driver during jacky update. "
...
class ModelAssignment(BaseModel):
    ...
    # ``jacky model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Jacky provider slug — fine.
    ...
    1. Vendor-name → Jacky-provider mapping: when the provider string is
       not a known Jacky provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from jacky_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from jacky_cli.model_normalize import normalize_model_for_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from jacky_cli.models import _AGGREGATOR_PROVIDERS
...
_MANAGED_FILES_ROOT_ENV = "JACKY_DASHBOARD_FILES_ROOT"
...
# JACKY_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to JACKY_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Jacky credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no JACKY_HOME resolution, so it blocks these trees wherever
...
def _default_jacky_root_is_opt_data() -> bool:
    raw = os.environ.get("JACKY_HOME", "").strip()
    if not raw:
        return False
    try:
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``jacky update``.
    ...
    a container — e.g. the jacky-webui image sharing the Jacky source tree),
    the dashboard's ``jacky update`` button is the correct update path and
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Jacky root is actually /opt/data
    # (the container/hosted layout) or when JACKY_DASHBOARD_FILES_ROOT is set.
    if _default_jacky_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Jacky inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``jacky:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.jacky-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``jacky_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from jacky_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from jacky_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_jacky_home() at call time
    ...
    try:
        ...
        # Windows install the first import of jacky_cli.gateway blocks the
        ...
        # and which providers are registered so ``jacky status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Nous Research" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from jacky_cli.dashboard_auth import list_providers as _list_providers
        ...
        try:
            from jacky_cli.auth import get_nous_session_validity
        ...
        status = {
            ...
            "can_update_jacky": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Jacky Cloud
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the jacky home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_jacky_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_jacky_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
async def get_portal_status():
    ...
    auth: Dict[str, Any] = {}
    try:
        from jacky_cli.auth import get_nous_auth_status
...
async def run_prompt_size():
    try:
        proc = _spawn_jacky_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_jacky_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_jacky_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from jacky_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.jacky/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_jacky_home() / "logs"
...
def _spawn_jacky_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``jacky <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``jacky_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "jacky_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "JACKY_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["jacky", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``jacky gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``jacky gateway restart`` children race each other on the
    ...
    return _spawn_jacky_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``jacky gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``JACKY_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/jacky/update")
async def update_jacky():
    """Kick off ``jacky update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Jacky updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("jacky-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "jacky-update",
    ...
    try:
        proc = _spawn_jacky_action(["update"], "jacky-update")
    except Exception as exc:
        _log.exception("Failed to spawn jacky update")
...
@app.get("/api/jacky/update/check")
async def check_jacky_update(force: bool = False):
    """Report whether a Jacky update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/jacky/update`` actually runs ``jacky update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Jacky version string
    ...
    try:
        ...
        if force:
            try:
                (get_jacky_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.jacky/config.yaml`` under ``tts.``.
    """
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Jacky surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Jacky supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        jacky_home / f"{name}.json",
        jacky_home / name / "config.json",
    ):
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_jacky_home()))
            return
...
# in jacky_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(
    ...
    """
    try:
        from jacky_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `jacky model` does so GUI onboarding lands on a
    ...
    if slug == "nous":
        try:
            ...
            from jacky_cli.auth import get_provider_auth_state
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from jacky_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.jacky/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from jacky_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # post-model-selection behaviour (jacky_cli/main.py
        ...
        if provider.strip().lower() == "nous":
            try:
                from jacky_cli.nous_subscription import apply_nous_managed_defaults
        ...
        # endpoint, mirroring the ``jacky model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from jacky_cli.main import _auto_provider_name, _save_custom_provider
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``jacky model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from jacky_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from jacky_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `jacky model` provider — would be
        ...
        # Vertex — a `jacky model` provider — would be invisible in the desktop
...
async def get_env_vars(profile: Optional[str] = None):
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `jacky model` picker uses (not desktop-only prefix guesses).
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Jacky from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Jacky to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Jacky from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Jacky to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Jacky in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Jacky through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Jacky via Home Assistant.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Jacky to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Jacky inside Feishu / Lark.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Jacky through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Jacky to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Jacky to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in jacky gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in jacky gateway setup",
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.jacky-agent.nousresearch.com"
_TELEGRAM_ONBOARDING_USER_AGENT = f"JackyDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Jacky Agent").strip() or "Jacky Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Jacky appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's JACKY_HOME.
...
# the canonical ``jacky auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.jacky/.anthropic_oauth.json`` — Jacky-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_jacky_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_jacky_oauth_credentials,
            _get_jacky_oauth_file,
        )
    except ImportError:
        read_jacky_oauth_credentials = None  # type: ignore
        _get_jacky_oauth_file = None  # type: ignore

    jacky_creds = None
    if read_jacky_oauth_credentials:
        try:
            jacky_creds = read_jacky_oauth_credentials()
        except Exception:
            jacky_creds = None
    if jacky_creds and jacky_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "jacky_pkce",
            "source_label": f"Jacky PKCE ({_get_jacky_oauth_file() if _get_jacky_oauth_file else None})",
            "token_preview": _truncate_token(jacky_creds.get("accessToken")),
            "expires_at": jacky_creds.get("expiresAt"),
            "has_refresh_token": bool(jacky_creds.get("refreshToken")),
        }
    ...
    try:
        ...
        pass
    try:
        from jacky_cli.config import get_env_value
    except ImportError:
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Jacky even
    when they also have a separate Jacky-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Jacky never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Nous Portal",
        "flow": "device_code",
        "cli_command": "jacky auth add nous",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "jacky auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "jacky auth add qwen-oauth",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "jacky auth add minimax-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "jacky auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from jacky_cli import auth as hauth
        if provider_id == "nous":
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Jacky, so the disconnect
    ...
    user sees exactly what executes, and Jacky then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Jacky — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``jacky model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `jacky model` universe (zero-edit for new plugins).
    try:
        from jacky_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"jacky auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("jacky_pkce", "claude_code", ...)
    ...
    sync with the `jacky model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Jacky-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_jacky_oauth_file
                oauth_file = _get_jacky_oauth_file()
            ...
            try:
                from jacky_cli.auth import clear_provider_auth
        ...
        try:
            from jacky_cli.auth import clear_provider_auth, invalidate_nous_auth_status_cache
...
#          → persists to ~/.jacky/.anthropic_oauth.json AND credential pool
...
# Guarded so jacky web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Jacky file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``jacky auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_jacky_oauth_file
    oauth_file = _get_jacky_oauth_file()
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "jacky-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    if provider_id == "nous":
        ...
        portal_base_url = (
            os.getenv("JACKY_PORTAL_BASE_URL")
    ...
    if provider_id == "xai-oauth":
        from jacky_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _nous_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from jacky_cli.auth import persist_nous_credentials
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``jacky auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``jacky model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Jacky "
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        from jacky_cli.auth import _save_codex_tokens
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `jacky sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_jacky_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from jacky_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_jacky_home() / "logs" / log_name
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, JACKY_HOME)."""
    from jacky_cli import profiles as profiles_mod
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    token = set_jacky_home_override(str(home))
    try:
        with cron_jobs.use_cron_store(home):
            result = getattr(cron_jobs, func_name)(*args, **kwargs)
    finally:
        reset_jacky_home_override(token)
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Jacky home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (jacky_cli.mcp_config), so
# servers managed here show up under `jacky mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from jacky_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from jacky_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from jacky_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from jacky_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from jacky_cli.mcp_config import (
        _get_mcp_servers,
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the JACKY_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``jacky mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        from tools.mcp_oauth import JackyTokenStorage, force_interactive_oauth
        ...
        # resolve JACKY_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = JackyTokenStorage(name)
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds JACKY_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (jacky_cli.webhook); the webhook
...
async def list_webhooks():
    import jacky_cli.webhook as wh
...
# spawn the real `jacky gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_jacky_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_jacky_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from jacky_cli.auth import read_credential_pool
...
async def run_doctor():
    try:
        proc = _spawn_jacky_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_jacky_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_jacky_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"jacky-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_jacky_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `jacky import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_jacky_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from jacky_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from jacky_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <jacky_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_jacky_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_jacky_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``jacky`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_jacky_action`` tracks one process/log per name, so a shared
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from jacky_cli.skills_hub import _resolve_source_meta_and_bundle
        from tools.skills_hub import create_source_router
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`jacky -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the JACKY_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_jacky_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "jacky setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local JACKY_HOME override so the write lands in the target
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local JACKY_HOME override (same mechanism as
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's JACKY_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``jacky profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from JACKY_HOME).
    """
    from jacky_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``jacky profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the JACKY_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``jacky profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from jacky_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_jacky_home()`` at call
       time — the context-local override from ``set_jacky_home_override``
    ...
    retargeted to the *current* ``get_jacky_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a JACKY_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_jacky_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_jacky_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from jacky_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    from jacky_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``jacky tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``jacky tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `jacky tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``jacky tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.jacky/.env`` via ``save_env_value`` —
    the same store ``jacky tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``jacky tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``jacky -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same JACKY_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Jacky,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``jacky computer-use permissions grant`` as a background action.
...
#
# The endpoint spawns the same ``jacky --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from jacky_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from jacky_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from jacky_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from jacky_cli.dashboard_auth.audit import AuditEvent, audit_log
        from jacky_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``jacky --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``JACKY_TUI_RESUME`` env var —
    matching what ``jacky_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``JACKY_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``JACKY_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``JACKY_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``JACKY_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_jacky_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``jacky -p <name>`` performs. The in-process
    ``JACKY_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from jacky_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("JACKY_TUI_DISABLE_MOUSE", "1")
    env.setdefault("JACKY_TUI_INLINE", "1")
    ...
    env["JACKY_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["JACKY_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["JACKY_TUI_RESUME"] = resume

    if sidecar_url:
        env["JACKY_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile JACKY_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["JACKY_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``JACKY_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("JACKY_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from jacky_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="jacky-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Jacky Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Jacky CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="jacky-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_jacky_root_is_opt_data() else "local"
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from jacky_cli.console_engine import JackyConsoleEngine

        engine = JackyConsoleEngine(
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Jacky Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Jacky inside WSL2 to use the dashboard's /chat "
...
# JACKY_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`jacky_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from jacky_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/jacky/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /jacky`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__JACKY_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `jacky serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("JACKY_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (jacky serve): web UI disabled — use "
            "`jacky dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/jacky``)
        ...
        ``__JACKY_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__JACKY_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__JACKY_BASE_PATH__="{prefix}";'
                f"window.__JACKY_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__JACKY_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__JACKY_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__JACKY_BASE_PATH__="{prefix}";'
                f"window.__JACKY_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/jacky`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Jacky backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Jacky Teal",         "description": "Classic dark teal — the canonical Jacky look"},
    {"name": "default-large", "label": "Jacky Teal (Large)", "description": "Jacky Teal with bigger fonts and roomier spacing"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.jacky/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.jacky/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_jacky_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.jacky/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as jacky_cli.plugins):
    1. User plugins:    ~/.jacky/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.jacky/plugins/  (only if JACKY_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from jacky_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_jacky_home() / "plugins", "user"),
    ...
    # ``jacky_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("JACKY_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".jacky" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from jacky_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_jacky_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"jacky auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from jacky_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from jacky_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from jacky_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from jacky_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.jacky/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"jacky_dashboard_plugin_{plugin['name']}"
...
from jacky_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``JACKY_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("JACKY_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``JACKY_SERVE_HEADLESS``), so
    the banner announces the bind rather than a browser URL.
    """
    import uvicorn

    try:
        from jacky_cli.nous_auth_keepalive import start_nous_auth_keepalive
    ...
    # the jacky-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing JACKY_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Jacky Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `jacky dashboard register` (Nous Portal) or "
    ...
    # JACKY_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "JACKY_BACKEND_READY" if headless else "JACKY_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Jacky backend listening on {host}:{actual_port}")
            else:
                print(f"  Jacky Web UI → http://{host}:{actual_port}")

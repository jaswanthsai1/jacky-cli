"""Agent-construction and session-resume display methods for ``JackyCLI``.

Extracted from ``cli.py`` as part of the god-file decomposition campaign
(``~/.jacky/plans/god-file-decomposition.md``, Phase 4 step 2). This mixin holds
...
Behavior-neutral: every method is lifted verbatim from ``JackyCLI``. ``self.*``
...
class CLIAgentSetupMixin:
    """Agent construction + session-resume display methods for ``JackyCLI``."""
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not _is_callable_provider and (not isinstance(api_key, str) or not api_key):
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: jacky setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: jacky setup")
            return False
        ...
        # `jacky chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `jacky auth add openai-codex`
        # without `jacky model`), fall back to the provider's first catalog
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (jacky sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (jacky sessions list).{_RST}")
                return False
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ Jacky: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ Jacky: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")

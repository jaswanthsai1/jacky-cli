...
Used by ``jacky kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``jacky_cli/goals.py`` — same aux
...
JACKY_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("JACKY_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Jacky Agent board.
...
def _profile_author() -> str:
    """Mirror of ``jacky_cli.kanban._profile_author``. Kept local to
    avoid a circular import when kanban.py imports this module."""
    return (
        os.environ.get("JACKY_PROFILE")
    ...
    *,
    ...
    try:
        resp = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=JACKY_KANBAN_SPECIFY_MAX_TOKENS,

#!/usr/bin/env python3
"""
Jacky CLI - Main entry point.

Usage:
    jacky                     # Interactive chat (default)
    jacky chat                # Interactive chat
    jacky gateway             # Run gateway in foreground
    jacky gateway start       # Start gateway as service
    jacky gateway stop        # Stop gateway service
    jacky gateway status      # Show gateway status
    jacky gateway install     # Install gateway service
    jacky gateway uninstall   # Uninstall gateway service
    jacky setup               # Interactive setup wizard
    jacky logout              # Clear stored authentication
    jacky status              # Show status of all components
    jacky cron                # Manage cron jobs
    jacky cron list           # List cron jobs
    jacky cron status         # Check if cron scheduler is running
    jacky doctor              # Check configuration and dependencies
    jacky honcho setup                    # Configure Honcho AI memory integration
    jacky honcho status                   # Show Honcho config and connection status
    jacky honcho sessions                 # List directory → session name mappings
    jacky honcho map <name>               # Map current directory to a session name
    jacky honcho peer                     # Show peer names and dialectic settings
    jacky honcho peer --user NAME         # Set user peer name
    jacky honcho peer --ai NAME           # Set AI peer name
    jacky honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    jacky honcho mode                     # Show current memory mode
    jacky honcho mode [hybrid|honcho|local]  # Set memory mode
    jacky honcho tokens                   # Show token budget settings
    jacky honcho tokens --context N       # Set session.context() token cap
    jacky honcho tokens --dialectic N     # Set dialectic result char cap
    jacky honcho identity                 # Show AI peer identity representation
    jacky honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    jacky honcho migrate                  # Step-by-step migration guide: OpenClaw native → Jacky + Honcho
    jacky version             Show version
    jacky update              Update to latest version
    jacky uninstall           Uninstall Jacky Agent
    jacky acp                 Run as an ACP server for editor integration
    jacky sessions browse     Interactive session picker with search

    jacky claw migrate --dry-run  # Preview migration without changes
"""

# IMPORTANT: jacky_bootstrap must be the very first import — it sets up
# UTF-8 stdio on Windows so print()/subprocess children don't hit
# UnicodeEncodeError with non-ASCII characters.  No-op on POSIX.
#
# Guarded against ModuleNotFoundError because ``jacky_bootstrap`` is a
# top-level module registered via pyproject.toml's ``py-modules`` list.
# When the user upgrades code via ``git pull`` (or ``jacky update``
# crashes between ``git reset --hard`` and ``uv pip install -e .``), the
# new code references ``jacky_bootstrap`` but the editable install's
# ``.pth`` file still points at the old set of top-level modules.  Without
# this guard, jacky crashes on import and the user can't run
...
def _set_process_title() -> None:
    """Set the process title to 'jacky' so tools like 'ps', 'top', and
    'htop' show the app name instead of 'python3.xx'.

    Purely cosmetic — non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep — installed via ``jacky tools`` or
      ...
      4. No-op on Windows (the .exe name is already ``jacky.exe``).
    """
    # Strategy 1: setproctitle (best — works on macOS, Linux, BSD)
    try:
        import setproctitle  # type: ignore[import-untyped]

        setproctitle.setproctitle("jacky")
        return
    except ImportError:
        pass
    ...
    try:
        ...
        if system == "Linux":
            ...
            libc.prctl(15, b"jacky", 0, 0, 0)  # PR_SET_NAME = 15
        elif system == "Darwin":
            libc = ctypes.CDLL("libc.dylib", use_errno=True)
            libc.pthread_setname_np(b"jacky")
        # Windows: the .exe name is already ``jacky.exe`` — nothing to do.
    except Exception:
        pass
...
# that run *before* jacky_cli.config is importable. Mirrors the explicit
...
def _config_default_interface_early() -> str:
    ...
    value = "cli"
    try:
        home = os.environ.get("JACKY_HOME")
        if home:
            cfg_path = os.path.join(home, "config.yaml")
        else:
            cfg_path = os.path.join(os.path.expanduser("~"), ".jacky", "config.yaml")
...
def _wants_tui_early(argv: "list[str] | None" = None) -> bool:
    ...
    explicit ``--tui``/``JACKY_TUI=1``, then a real-TTY gate (a
    ...
    cron jobs, pipes run ``jacky … chat -q`` with stdio on a pipe. This
    ...
    if "--cli" in argv:
        return False
    if os.environ.get("JACKY_TUI") == "1" or "--tui" in argv:
        return True
    try:
...
# `entry.tsx`; this is just the earlier cousin. ``JACKY_TUI_NO_EARLY_DISABLE``
# escapes the behaviour for diagnostics.
def _suppress_mouse_residue_early() -> None:
    if os.environ.get("JACKY_TUI_NO_EARLY_DISABLE") == "1":
        return
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`jacky --tui … >log`, CI capture):
...
def _print_fast_version_info() -> None:
    from jacky_cli import __release_date__, __version__

    print(f"Jacky Agent v{__version__} ({__release_date__})")
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``jacky --version`` before config/logging imports on Termux."""
    if os.environ.get("JACKY_TERMUX_DISABLE_FAST_CLI") == "1":
        return False
...
from jacky_cli.subcommands._shared import add_accept_hooks_flag as _add_accept_hooks_flag
from jacky_cli.subcommands.cron import build_cron_parser
from jacky_cli.subcommands.gateway import build_gateway_parser
from jacky_cli.subcommands.profile import build_profile_parser
from jacky_cli.subcommands.model import build_model_parser
from jacky_cli.subcommands.setup import build_setup_parser
from jacky_cli.subcommands.postinstall import build_postinstall_parser
from jacky_cli.subcommands.whatsapp import build_whatsapp_parser
from jacky_cli.subcommands.slack import build_slack_parser
from jacky_cli.subcommands.login import build_login_parser
from jacky_cli.subcommands.logout import build_logout_parser
from jacky_cli.subcommands.auth import build_auth_parser
from jacky_cli.subcommands.status import build_status_parser
from jacky_cli.subcommands.webhook import build_webhook_parser
from jacky_cli.subcommands.hooks import build_hooks_parser
from jacky_cli.subcommands.doctor import build_doctor_parser
from jacky_cli.subcommands.security import build_security_parser
from jacky_cli.subcommands.dump import build_dump_parser
from jacky_cli.subcommands.debug import build_debug_parser
from jacky_cli.subcommands.backup import build_backup_parser
from jacky_cli.subcommands.import_cmd import build_import_cmd_parser
from jacky_cli.subcommands.config import build_config_parser
from jacky_cli.subcommands.console import build_console_parser
from jacky_cli.subcommands.version import build_version_parser
from jacky_cli.subcommands.update import build_update_parser
from jacky_cli.subcommands.uninstall import build_uninstall_parser
from jacky_cli.subcommands.dashboard import build_dashboard_parser
from jacky_cli.subcommands.gui import build_gui_parser
from jacky_cli.subcommands.logs import build_logs_parser
from jacky_cli.subcommands.prompt_size import build_prompt_size_parser
from jacky_cli.subcommands.memory import build_memory_parser
from jacky_cli.subcommands.acp import build_acp_parser
from jacky_cli.subcommands.tools import build_tools_parser
from jacky_cli.subcommands.insights import build_insights_parser
from jacky_cli.subcommands.skills import build_skills_parser
from jacky_cli.subcommands.pairing import build_pairing_parser
from jacky_cli.subcommands.plugins import build_plugins_parser
from jacky_cli.subcommands.mcp import build_mcp_parser
from jacky_cli.subcommands.claw import build_claw_parser


def _require_tty(command_name: str) -> None:
    """Exit with a clear error if stdin is not a terminal.

    Interactive TUI commands (jacky tools, jacky setup, jacky model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'jacky {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any jacky module import.
#
# Many modules cache JACKY_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("JACKY_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.jacky/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set JACKY_HOME before imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    profile_index = None
    ...
    def _inside_mcp_add_args(index: int) -> bool:
        """True once argv reaches `jacky mcp add ... --args <command argv>`.
        ...
        ``--profile``), not to Jacky' own profile selector.
        """
        try:
    ...
    def _resolve_sudo_user_profile_env(name: str) -> str | None:
        """Resolve `sudo jacky -p <name>` against the invoking user's home.
        ...
        candidate = home / ".jacky" / "profiles" / name
        try:
            if candidate.is_dir():
    ...
    # after the subcommand (`jacky chat -p coder`), so keep scanning broadly.
    ...
    # Mirrors jacky_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If JACKY_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.jacky/profiles/coder or
    # /opt/data/profiles/coder).  If JACKY_HOME points to the jacky root
    # instead (e.g. systemd hardcodes JACKY_HOME=/root/.jacky), we must
    # still read active_profile — the user may have switched profiles via
    # `jacky profile use` and the gateway should honour that choice.
    # See issue #22502.
    jacky_home_env = os.environ.get("JACKY_HOME", "")
    if profile_name is None and jacky_home_env:
        if Path(jacky_home_env).parent.name == "profiles":
            return

    # 2. If no flag, check active_profile in the jacky root.
    #
    # EXCEPTION: a supervised s6 gateway child (exported by the container
    # run-script as JACKY_S6_SUPERVISED_CHILD=1) must NOT follow the sticky
    ...
    # reserved ``gateway-default`` slot runs bare ``jacky gateway run`` to mean
    # "the root JACKY_HOME profile". If the reserved default child read
    ...
    # 3. If we found a profile, resolve and set JACKY_HOME
    if profile_name is not None:
        try:
            from jacky_cli.profiles import resolve_profile_env

            jacky_home = resolve_profile_env(profile_name)
        except FileNotFoundError as exc:
            jacky_home = _resolve_sudo_user_profile_env(profile_name)
            if not jacky_home:
            ...
            # A bug in profiles.py must NEVER prevent jacky from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["JACKY_HOME"] = jacky_home
...
load_jacky_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → JACKY_REDACT_SECRETS env
# var BEFORE jacky_logging imports agent.redact (which snapshots the flag at
...
try:
    ...
    _cfg_path = get_jacky_home() / "config.yaml"
    if _cfg_path.exists():
        ...
        try:
            from jacky_cli import managed_scope
            _early_cfg_raw = managed_scope.apply_managed_overlay(_early_cfg_raw)
        except Exception:
            pass
        if "JACKY_REDACT_SECRETS" not in os.environ:
            _early_sec_cfg = _early_cfg_raw.get("security", {})
            if isinstance(_early_sec_cfg, dict):
                _early_redact = _early_sec_cfg.get("redact_secrets")
                if _early_redact is not None:
                    os.environ["JACKY_REDACT_SECRETS"] = str(_early_redact).lower()
...
# Initialize centralized file logging early — all `jacky` subcommands
...
if _FORCE_IPV4_EARLY:
    try:
        ...
        pass  # best-effort — don't crash if jacky_constants not importable yet
...
from jacky_cli import __version__, __release_date__

# Provider model-selection wizard flows extracted to jacky_cli/model_setup_flows.py
# (god-file decomposition Phase 2). Re-imported here so select_provider_and_model and
# existing test monkeypatches (jacky_cli.main._model_flow_*) keep resolving unchanged.
from jacky_cli.model_setup_flows import (
...
def _termux_bundled_skills_stamp_path() -> Path:
    return get_jacky_home() / "skills" / ".termux_bundled_sync_stamp"


def _termux_bundled_skills_sync_needed() -> bool:
    if not _is_termux_startup_environment():
        return True
    if os.environ.get("JACKY_TERMUX_FORCE_SKILLS_SYNC") == "1":
        return True
    try:
...
def _termux_should_prefetch_update_check() -> bool:
    if not _is_termux_startup_environment():
        return True
    return os.environ.get("JACKY_TERMUX_PREFETCH_UPDATES") == "1"
...
def _has_any_provider_configured() -> bool:
    ...
    from jacky_cli.config import get_env_path, get_jacky_home, load_config
    from jacky_cli.auth import get_auth_status

    # Determine whether Jacky itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from jacky_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_jacky_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_jacky_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Jacky has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants Jacky to use their tokens.
    if _has_jacky_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, jacky_bin
        cli_args: the original CLI arguments (everything after 'jacky')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    jacky_bin = container_info["jacky_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo jacky {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo jacky {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  jacky --tui --resume {target}")
    if title:
        print(f'  jacky --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @jacky/ink is missing or node_modules is behind package-lock.json.
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    ink = ws_root / "node_modules" / "@jacky" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
_TUI_BUILD_INPUT_FILES = (
    ...
    "scripts/build.mjs",
    "packages/jacky-ink/package.json",
    "packages/jacky-ink/index.js",
    "packages/jacky-ink/text-input.js",
)
...
def _tui_need_rebuild(root: Path) -> bool:
    ...
    or local edits. Set ``JACKY_TUI_FORCE_BUILD=1`` to force the old behaviour.
    """
    force = (os.environ.get("JACKY_TUI_FORCE_BUILD") or "").strip().lower()
...
def _ensure_tui_node() -> None:
    ...
    ``JACKY_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("JACKY_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    jacky_home = os.environ.get("JACKY_HOME") or str(Path.home() / ".jacky")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "JACKY_HOME": jacky_home},
            capture_output=True,
            text=True,
            encoding="utf-8",
            errors="replace",
    ...
    extras.extend([Path(jacky_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _find_bundled_tui(jacky_cli_dir: Path | None = None) -> Path | None:
    """Find a pre-built TUI entry.js bundled in the wheel."""
    if jacky_cli_dir is None:
        jacky_cli_dir = Path(__file__).parent
    bundled = jacky_cli_dir / "tui_dist" / "entry.js"
...
def _restore_tui_workspace(tui_dir: Path) -> bool:
    ...
    files deleted in the working tree after ``jacky update`` (HEAD stays
...
def _ensure_tui_workspace(tui_dir: Path) -> None:
    ...
    if _restore_tui_workspace(tui_dir):
        if not os.environ.get("JACKY_QUIET"):
            print(f"Restored missing TUI workspace: {tui_dir}")
        return

    print(
        "Error: the TUI workspace is missing from this Jacky checkout.\n"
        f"Expected directory: {tui_dir}\n"
        "This usually means `jacky update` left tracked ui-tui files deleted.\n"
        "Recovery:\n"
        "  1. From the Jacky checkout, run `git restore -- ui-tui`\n"
        "  2. Run `npm install --silent --no-fund --no-audit --progress=false`\n"
        "  3. Retry `jacky --tui`\n"
        "If the checkout is still inconsistent, run `jacky update --force`.",
        file=sys.stderr,
    )
    sys.exit(1)
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (JACKY_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("JACKY_NODE")
            if env_node and os.path.isfile(env_node) and os.access(env_node, os.X_OK):
                return env_node
        path = shutil.which(bin)
        if not path and bin == "node":
            try:
                from jacky_cli.dep_ensure import ensure_dependency
    ...
    ext_dir = os.environ.get("JACKY_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with JACKY_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset JACKY_TUI_DIR (e.g. `unset JACKY_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if tui_dev:
        # Keep the local @jacky/ink package exports in sync with source.
        # --dev runs src/entry.tsx directly, but @jacky/ink resolves through
        # packages/jacky-ink/dist/entry-exports.js. If that dist bundle is
        ...
        npm = _node_bin("npm")
        ink_dir = tui_dir / "packages" / "jacky-ink"
        result = subprocess.run(
            [npm, "run", "build"],
            cwd=str(ink_dir),
            capture_output=True,
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from jacky_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    env = os.environ.copy()
    try:
        from jacky_cli.config import apply_terminal_config_to_env
        apply_terminal_config_to_env(env=env)
    except Exception:
        logger.debug("Failed to apply terminal config bridge for TUI launch", exc_info=True)
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="jacky-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["JACKY_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["JACKY_PYTHON_SRC_ROOT"] = os.environ.get(
        "JACKY_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("JACKY_PYTHON", sys.executable)
    env.setdefault("JACKY_CWD", os.getcwd())
    ...
    if worktree:
        ...
        if not wt_info:
            sys.exit(1)
        env["JACKY_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["JACKY_MODEL"] = model
        env["JACKY_INFERENCE_MODEL"] = model
    if provider:
        env["JACKY_TUI_PROVIDER"] = provider
        env["JACKY_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["JACKY_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["JACKY_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["JACKY_TUI_SKILLS"] = value
    if query:
        env["JACKY_TUI_QUERY"] = query
    if image:
        env["JACKY_TUI_IMAGE"] = image
    if checkpoints:
        env["JACKY_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["JACKY_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["JACKY_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["JACKY_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["JACKY_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["JACKY_ACCEPT_HOOKS"] = "1"
    ...
    # JACKY_TUI_RESUME is an internal hand-off from the Python wrapper to the
    # Ink app.  Because we start from os.environ.copy(), an exported/stale value
    # in the user's shell would otherwise make a plain `jacky --tui` try to
    ...
    # still set JACKY_TUI_RESUME themselves.
    env.pop("JACKY_TUI_RESUME", None)
    if resume_session_id:
        env["JACKY_TUI_RESUME"] = resume_session_id
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `jacky update` so
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``JACKY_KANBAN_BOARD`` for the chat session.

    Without this, in-process tools (``kanban_*``) and shelled-out CLI calls
    (``jacky kanban …``) resolve the board on different paths: the env-pin if
    set, otherwise the global ``<root>/kanban/current`` file. A concurrent
    ``jacky kanban boards switch`` from another session can flip the file
    ...
    """
    if os.environ.get("JACKY_KANBAN_BOARD"):
        return
    try:
        from jacky_cli.kanban_db import get_current_board

        os.environ["JACKY_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass


def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.jacky/skills/`` with the bundled skill library on first launch.

    Called from any CLI entrypoint that the user might use as their first
    interaction with Jacky — chat, dashboard (the desktop GUI's backend),
    ...
    dependency. Jacky still functions without them; the user just sees an
...
def _resolve_use_tui(args) -> bool:
    ...
    workers, cron jobs, and pipelines run ``jacky … chat -q`` with stdout
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'jacky sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    try:
        from jacky_cli.xai_retirement import (
            MIGRATION_GUIDE_URL,
            RETIREMENT_DATE,
            find_retired_xai_refs,
            format_issue,
        )
        from jacky_cli.config import load_config as _load_config_for_xai_check
        ...
        if _retired_xai_refs:
            ...
            sys.stderr.write("  \033[2mRun 'jacky doctor' for details.\033[0m\n\n")
    except Exception:
        pass
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like Jacky isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  jacky setup")
        print()
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'jacky setup' at any time to configure.")
        sys.exit(1)
    ...
    if _termux_should_prefetch_update_check():
        try:
            from jacky_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["JACKY_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.jacky/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["JACKY_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["JACKY_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["JACKY_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from jacky_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from jacky_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_whatsapp(args):
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Jacky?")
        print()
    ...
    # has no creds.json.  Every subsequent `jacky gateway` then paid a 30s
    ...
    session_dir = get_jacky_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in {"y", "yes"}:
            ...
            print("  Start the gateway with: jacky gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `jacky gateway` skips it cleanly instead of paying a 30s
        ...
        print("⚠ Pairing may not have completed. Run 'jacky whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``jacky whatsapp`` (the Baileys bridge wizard) — the
    two adapters are complementary, not alternatives. See
    ``jacky_cli/setup_whatsapp_cloud.py``.
    """
    _require_tty("whatsapp-cloud")
    from jacky_cli.setup_whatsapp_cloud import run_whatsapp_cloud_setup
...
def cmd_postinstall(args):
    ...
    from jacky_cli.config import stamp_install_method
    from jacky_cli.dep_ensure import ensure_dependency
...
def cmd_model(args):
    ...
    if getattr(args, "refresh", False):
        try:
            from jacky_cli.models import clear_provider_models_cache
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``jacky model``) and the setup wizard
    ...
    from jacky_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("JACKY_INFERENCE_PROVIDER") or "auto"
    )
    compatible_custom_providers = get_compatible_custom_providers(config)
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from jacky_cli.config import read_raw_config
    ...
    if not active and effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'jacky model' for "
                "available providers, or run 'jacky doctor' to diagnose config "
                "issues."
            )
    ...
    # PROVIDER_GROUPS in jacky_cli/models.py). A multi-member group shows one
    ...
    # a leftover OPENAI_BASE_URL in ~/.jacky/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.jacky/.env if the active provider is not 'custom'.
    ...
    """
    from jacky_cli.config import get_env_value, save_env_value, load_config
...
#
# Jacky uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `jacky model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `jacky model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _all_aux_tasks() -> list[tuple[str, str, str]]:
    ...
    :meth:`jacky_cli.plugins.PluginContext.register_auxiliary_task`) appear
    in the same surfaces as built-in ones without core knowing about them.
    """
    tasks = list(_AUX_TASKS)
    try:
        from jacky_cli.plugins import get_plugin_auxiliary_tasks
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from jacky_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  Jacky only falls back to a lightweight backend (OpenRouter,")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``jacky model`` flow, then route aux tasks to them here.
    """
    from jacky_cli.config import load_config
    from jacky_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from jacky_cli.auth import _prompt_model_selection
    from jacky_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    """
    try:
        from jacky_cli.setup import _curses_prompt_choice
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from jacky_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use Jacky URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
...
def _remove_custom_provider(config):
    ...
    try:
        from jacky_cli.curses_ui import curses_radiolist
...
# downstream call sites read `jacky_cli.main._PROVIDER_MODELS` directly,
# so the symbol needs to be reachable as a module attribute. But importing
# the catalog eagerly costs ~55ms on every `jacky` invocation — including
# fast paths like `jacky --version` and slash-command dispatch that never
...
def __getattr__(name):
    ...
    if name in _LAZY_MODEL_EXPORTS:
        from jacky_cli.models import _PROVIDER_MODELS
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``jacky setup`` / ``jacky model``.
    ...
    recover from a malformed paste without editing ``~/.jacky/.env`` by hand.

    Returns ``(resolved_key, abort)``.  ``abort=True`` means the caller should
    ``return`` immediately — the user cancelled entry, declined to replace, or
    cleared the key and is now unconfigured.
    """
    from jacky_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from jacky_cli.config import save_env_value
    from jacky_cli.secret_prompt import masked_secret_prompt
    ...
    from jacky_cli.env_loader import format_secret_source_suffix
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `jacky setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_login(args):
    """Authenticate Jacky CLI with a provider."""
    from jacky_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from jacky_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from jacky_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from jacky_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from jacky_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from jacky_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``jacky slack <subcommand>``. Currently supports:
    ...
    if sub in {None, ""}:
        ...
        print(
            "usage: jacky slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `jacky slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from jacky_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from jacky_cli.kanban import kanban_command
...
def cmd_project(args):
    ...
    from jacky_cli.projects_cmd import projects_command
...
def cmd_hooks(args):
    ...
    from jacky_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from jacky_cli.doctor import run_doctor

    run_doctor(args)


def cmd_security(args):
    """Dispatch `jacky security <subcmd>`."""
    sub = getattr(args, "security_command", None)
    if sub in ("audit", None):
        from jacky_cli.security_audit import cmd_security_audit
...
def cmd_dump(args):
    ...
    from jacky_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from jacky_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from jacky_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up Jacky home directory to a zip file."""
    if getattr(args, "quick", False):
        from jacky_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from jacky_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a Jacky backup from a zip file."""
    from jacky_cli.backup import run_import
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    from jacky_cli.banner import format_banner_version_label
    ...
    try:
        from jacky_cli.banner import check_for_updates
...
def cmd_uninstall(args):
    """Uninstall Jacky Agent (or just the Chat GUI with --gui)."""
    ...
    if getattr(args, "gui_summary", False):
        from jacky_cli.gui_uninstall import gui_install_summary
    ...
    if getattr(args, "gui", False):
        ...
        from jacky_cli.uninstall import run_gui_uninstall
    ...
    from jacky_cli.uninstall import run_uninstall
...
# Critical files that Jacky must be able to import immediately after an
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    These are the files imported on every ``jacky`` startup; if any of them
    ...
    root = Path(root)
    with tempfile.TemporaryDirectory(prefix="jacky-syntax-check-") as tmpdir:
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``jacky update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    still outputs to ``jacky_cli/web_dist/`` (per vite.config.ts
    outDir: "../jacky_cli/web_dist"), NOT to ``web/dist/``, so Python
    ...
    dist_dir = project_root / "jacky_cli" / "web_dist"
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    ``jacky`` launcher present but ``jacky_cli`` not importable.
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    Two-tier resolution:
    1. Fast path — the jacky venv's python3 (present in managed installs)
    ...
    # Tier 1: fast path — jacky venv python3, no nix-shell overhead
    ...
    # a jacky venv (pip / non-managed / bare-git installs).  The resolved
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``jacky update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``jacky web``).
    ...
    # build path usable on a stock `py -m jacky_cli.main web` invocation.
    ...
    if not npm:
        ...
        return not fatal
    build_env = with_jacky_node_path()
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (jacky web will not be available)")
        )
        _relay(r1)
        if fatal:
...
#   - ``jacky update`` can unconditionally call ``jacky desktop --build-only``
#     and it will skip if nothing actually changed
#   - ``jacky desktop`` (interactive launch) skips the build when the
#     stamp matches, making repeated launches fast
#
# Stamp file: $JACKY_HOME/desktop-build-stamp.json
# Schema:
#   {
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``jacky update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_packaged_executable(desktop_dir: Path) -> Optional[Path]:
    ...
    if sys.platform == "darwin":
        candidates = list(release_dir.glob("mac*/Jacky.app/Contents/MacOS/Jacky"))
    elif sys.platform == "win32":
        candidates = [
            release_dir / "win-unpacked" / "Jacky.exe",
            release_dir / "win-ia32-unpacked" / "Jacky.exe",
            release_dir / "win-arm64-unpacked" / "Jacky.exe",
        ]
    else:
        candidates = [
            release_dir / "linux-unpacked" / "jacky",
            release_dir / "linux-unpacked" / "Jacky",
            release_dir / "linux-arm64-unpacked" / "jacky",
            release_dir / "linux-arm64-unpacked" / "Jacky",
        ]
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    '…/linux-unpacked/Jacky'`` desktop build failure: a corrupt zip in the
...
def _redownload_electron_dist(
    ...
    *,
    ...
    dl_env = with_jacky_node_path(env)
    if mirror:
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    On Windows a running ``Jacky.exe`` keeps an exclusive lock on
    ``release/win-unpacked/Jacky.exe``. electron-builder's pack then can't
    delete the stale binary and dies with ``remove …\\Jacky.exe: Access is
    ...
    an unrelated "Jacky" process is never touched. Best-effort: never raises.
...
def _desktop_macos_relaunchable_fixup(desktop_dir: Path) -> None:
    ...
    tampering and macOS reports "Jacky is damaged and can't be opened." The
    ...
    if exe is None:
        return
    # exe = .../Jacky.app/Contents/MacOS/Jacky  ->  app bundle = .../Jacky.app
    app = exe.parents[2]
...
def _force_adhoc_macos_signing(env: dict, *, source_mode: bool) -> bool:
    ...
    machine* (``jacky desktop --build-only`` → electron-builder ``--dir``).
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ Jacky Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ Jacky Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def _desktop_launch_options() -> tuple[list[str], str]:
    ...
    (normalized for the JACKY_DESKTOP_DISABLE_GPU env var the Electron app
...
def cmd_gui(args: argparse.Namespace):
    ...
    if getattr(args, "fake_boot", False):
        env["JACKY_DESKTOP_BOOT_FAKE"] = "1"
    if getattr(args, "ignore_existing", False):
        env["JACKY_DESKTOP_IGNORE_EXISTING"] = "1"
    if getattr(args, "jacky_root", None):
        env["JACKY_DESKTOP_JACKY_ROOT"] = str(Path(args.jacky_root).expanduser().resolve())
    if getattr(args, "cwd", None):
        env["JACKY_DESKTOP_CWD"] = str(Path(args.cwd).expanduser().resolve())
    else:
        env["JACKY_DESKTOP_CWD"] = os.getcwd()
    ...
    # `JACKY_DESKTOP_DISABLE_GPU=... jacky desktop` keeps working.
    config_electron_flags, config_disable_gpu = _desktop_launch_options()
    if config_disable_gpu != "auto" and "JACKY_DESKTOP_DISABLE_GPU" not in os.environ:
        env["JACKY_DESKTOP_DISABLE_GPU"] = config_disable_gpu
    ...
    if source_mode or not skip_build:
        ...
        if not npm:
            ...
            print("Install Node.js, then run:  jacky gui")
            sys.exit(1)
    else:
        npm = None
    ...
    if skip_build:
        ...
        if not build_needed:
            ...
            if not source_mode:
                ...
                # holds Jacky.exe locked on Windows, so the pack can't replace
            ...
            if (
                ...
                if restored:
                    ...
                    for p in purged:
                        print(f"    - {p}")
                    # The purge can't remove a win-unpacked tree whose Jacky.exe
            ...
            if build_result.returncode != 0:
                ...
                if sys.platform == "win32":
                    print("  If this says \"Access is denied\" on Jacky.exe, close any")
                    print("  running Jacky desktop window and retry.")
                print("  If the log shows Electron download retries, rebuild via a mirror:")
                print("    ELECTRON_MIRROR=<mirror-base-url> jacky desktop --force-build")
                sys.exit(build_result.returncode or 1)
            packaged_executable = _desktop_packaged_executable(desktop_dir)
            if not source_mode:
                # Locally-built apps are ad-hoc signed; make them relaunchable after
                # an in-place self-update (otherwise macOS reports "Jacky is
    ...
    if source_mode:
        print("→ Launching Jacky Desktop from source build...")
    ...
    print(f"→ Launching packaged Jacky Desktop: {' '.join(launch_command)}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    """Return PIDs of ``jacky dashboard`` processes other than ourselves.

    ``jacky dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``jacky update`` replaces files on disk, the running
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the Jacky Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``jacky serve`` as
    ...
    environment variable ``JACKY_DESKTOP_CHILD_PID`` on the spawned
    ...
    patterns = [
        "jacky dashboard",
        "jacky_cli.main dashboard",
        "jacky_cli/main.py dashboard",
        # The headless backend (`jacky serve`) is the same long-lived server
        # under a different command name — the desktop app spawns it. Reap it
        # on update for the same frontend/backend-mismatch reason.
        "jacky serve",
        "jacky_cli.main serve",
        "jacky_cli/main.py serve",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "jacky.*dashboard"`) keeps us consistent
            # with `jacky_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `jacky update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``jacky update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``jacky update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: jacky curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``jacky dashboard`` processes.

    Called at the end of ``jacky update`` (default ``reason``) and also
    from ``jacky dashboard --stop`` (which overrides ``reason``).  The
    ...
    """
    # When the Jacky Desktop Electron app spawns this dashboard as a
    # backend child, it sets JACKY_DESKTOP_CHILD_PID so that the update
    # path can skip killing the desktop-managed process.  (#37532)
    exclude: set[int] | None = None
    raw_pid = os.environ.get("JACKY_DESKTOP_CHILD_PID")
    if raw_pid:
    ...
    if killed:
        ...
        print("    jacky dashboard --port <port>")
...
def _atomic_replace_dir(src: str, dst: str) -> None:
    ...
    """
    staging = f"{dst}.jacky-update-staging"
    backup = f"{dst}.jacky-update-old"
...
def _update_via_zip(args):
    """Update Jacky Agent by downloading a ZIP archive.
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `jacky update "
            f"--branch {branch}`, or update against main with `jacky update`."
        )
        sys.exit(1)
    zip_url = (
        f"https://github.com/jaswanthsai1/jacky-cli/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="jacky-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"jacky-agent-{branch}.zip")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # symlink members. A GitHub source ZIP for jacky-agent itself
        ...
        # GitHub ZIPs extract to jacky-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"jacky-agent-{branch}")
    ...
    from jacky_cli.managed_uv import ensure_uv, update_managed_uv
    ...
    try:
        ...
        if result.get("user_modified"):
            ...
            print(
                "    → see them: jacky skills list-modified  "
    ...
    try:
        from jacky_cli.model_catalog import seed_cache_from_checkout
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "jacky-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Jacky behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes jacky completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but Jacky couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but Jacky couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if Jacky behaves unexpectedly.")
    return True
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but Jacky couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but Jacky couldn't drop "
...
# =========================================================================
# Fork detection and upstream management for `jacky update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/jaswanthsai1/jacky-cli.git",
    "git@github.com:jaswanthsai1/jacky-cli.git",
    "https://github.com/jaswanthsai1/jacky-cli",
    "git@github.com:jaswanthsai1/jacky-cli",
}
OFFICIAL_REPO_URL = "https://github.com/jaswanthsai1/jacky-cli.git"
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official Jacky repository.")
        print("  This means you may miss updates from jaswanthsai1/jacky-cli.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/jaswanthsai1/jacky-cli.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/jaswanthsai1/jacky-cli.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``jacky update``, every profile is now current.
    """
    homes = []
...
# Install-scoped breadcrumb dropped right before ``jacky update`` mutates the
...
# survives and the next ``jacky`` launch finishes the install instead of
# limping along on a half-built venv (e.g. pip wiped, a core dep like Pillow
# never landed).  Lives next to the venv (not under $JACKY_HOME) because the
...
def _recover_from_interrupted_install() -> None:
    """Finish a dependency install that a prior ``jacky update`` left half-done.
    ...
    protocol stream (``jacky acp`` speaks JSON-RPC on stdout) must never get
    ...
    # Windows self-lock guard: if jacky.exe is the launcher that spawned
    ...
    if _is_windows():
        ...
        if scripts_dir is not None:
            shims = _jacky_exe_shims(scripts_dir)
            if shims:
                ...
                try:
                    ...
                    for _anc in [_me] + list(_me.parents()):
                        ...
                        if _anc_norm in _shim_set:
                            print(
                                "✗ Jacky is running from the binary that "
                                "needs to be replaced — the auto-recovery "
                                "cannot overwrite a running executable."
                            )
                            print(
                                "  Restart Jacky from a different terminal, "
    ...
    try:
        ...
        print(
            "⚠ A previous `jacky update` was interrupted mid-install — "
            "finishing dependency installation now..."
        )

        try:
            from jacky_cli.managed_uv import ensure_uv
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``jacky update`` is still progressing even if pip/uv itself is silent.
    """
...
def _jacky_exe_shims(scripts_dir: Path) -> list[Path]:
...
def _detect_concurrent_jacky_instances(
    ...
    The Jacky Desktop Electron app spawns ``jacky.EXE`` as a backend child,
    so during ``jacky update`` the user-invoked process and the desktop's
    ...
    shims (``jacky.exe`` / ``jacky-gateway.exe``) and returns ``(pid,
    process_name)`` pairs. The caller's own PID and its entire ancestor
    chain are excluded so the running ``jacky update`` invocation never
    reports itself — this matters on Windows where the setuptools .exe
    launcher (``jacky.exe``) is a separate process from the Python
    ...
    for shim in _jacky_exe_shims(scripts_dir):
        try:
    ...
    # setuptools-generated jacky.exe launcher is a separate native process
    ...
    # file lock) is the parent. Without excluding it, every ``jacky update``
    ...
    #      jacky.exe sitting *under* a non-Jacky parent (e.g. a Jacky
...
def _format_concurrent_instances_message(
    ...
    shim = scripts_dir / "jacky.exe"
    lines = ["✗ Another jacky.exe is running:"]
    ...
    lines.append("")
    lines.append("  Close Jacky Desktop, exit any open `jacky` REPLs, and")
    lines.append("  stop the gateway (`jacky gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `jacky update --force` if you've already")
    lines.append("  confirmed those processes will not write to the venv.")
    return "\n".join(lines)


def _quarantine_running_jacky_exe(
    scripts_dir: Path, *, max_attempts: int = 4
) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``jacky.exe``.
    ...
    when ``jacky update`` runs, ``jacky.exe`` IS the live process, and uv
    fails with ``Access is denied. (os error 5)``.

    We rename live shims to ``jacky.exe.old.<unix-ms>`` first. uv then writes
    fresh shims at the original paths. The ``.old`` files are cleaned up on
    the next jacky invocation by ``_cleanup_quarantined_exes``.
    ...
    handles (recovers in <1s), or the Jacky Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       Jacky Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _jacky_exe_shims(scripts_dir):
        ...
        print(
            "    Close Jacky Desktop, exit other `jacky` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `jacky update`."
        )
...
def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_jacky_exe`` if uv didn't write replacements."""
...
def _run_quarantined_install(
    ...
    *,
    ...
    """Run an editable install, quarantining the running ``jacky.exe`` first.

    Any ``pip install -e .`` (or ``--reinstall``) rewrites the entry-point
    shims, and on Windows the live ``jacky.exe`` is the running process —
    pip can neither delete nor overwrite it, so without quarantine the shim
    is left missing and ``jacky`` drops off PATH. This wraps
    ...
    moved: list[tuple[Path, Path]] = []
    if scripts_dir is not None:
        moved = _quarantine_running_jacky_exe(scripts_dir)
    try:
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``jacky.exe.old.*`` left by prior updates.

    Called early on every jacky invocation. The .old files are unlocked once
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``jacky update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `jacky update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    On Windows, pre-renames live ``jacky.exe`` / ``jacky-gateway.exe`` shims
    in the venv Scripts dir before each install attempt so uv can write fresh
    copies (Windows blocks REPLACE on a running .exe but allows RENAME). See
    ``_quarantine_running_jacky_exe`` for the rationale.
    """
    ...
    # hours later inside ``jacky update``'s desktop-rebuild or skill-sync
...
def _verify_console_scripts_installed(
    ...
    *,
    ...
    On Windows, ``uv pip install -e .`` can register ``jacky.exe`` in the
    wheel RECORD while the file never lands on disk — typically when the live
    ``jacky.exe`` shim is locked during ``jacky update``, or when uv/distlib
    skips a launcher write. The symptom is ``jacky-agent.exe`` and
    ``jacky-acp.exe`` present but ``jacky.exe`` missing, so ``jacky`` drops
    ...
    try:
        ...
        print(
            "  ⚠ Entry point repair failed; try `jacky update --force` after "
            "closing other jacky processes."
        )
        return
    ...
    if still_missing:
        print(
            ...
            "Workaround: python -m jacky_cli.main <command>"
        )
    else:
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    #
    # Quarantine the running ``jacky.exe`` first: ``--reinstall -e .``
    # rewrites the entry-point shims, and on Windows pip can't overwrite the
    # live launcher, which would leave ``jacky`` off PATH.
    scripts_dir = _venv_scripts_dir() if _is_windows() else None
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `jacky update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `jacky update --force` after closing other jacky processes."
        )
        return
...
def _install_psutil_android_compat(
    ...
    *,
    ...
    from jacky_cli.psutil_android import PSUTIL_URL, prepare_patched_psutil_sdist
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    standalone uv into ``$JACKY_HOME/bin/uv``, but on Termux the official
    ...
    """
    from jacky_cli.managed_uv import resolve_uv
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `jacky update`, so we install root-only first
    ...
    nixos_env = with_jacky_node_path(_nixos_build_env())
    ...
    # hung. The chatty npm-deprecation noise during `jacky update` comes from
...
class _UpdateOutputStream:
    """Stream wrapper used during ``jacky update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.jacky/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``jacky update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``jacky update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.jacky/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``jacky update --gateway``) the update is already
    ...
    try:
        ...
        # jacky_cli.config.get_jacky_home to simulate setup failure.
        from jacky_cli.config import get_jacky_home as _get_jacky_home

        logs_dir = _get_jacky_home() / "logs"
...
def _log_only_write(text: str) -> None:
    """Write ``text`` to ``~/.jacky/logs/update.log`` only, never the terminal.

    During ``jacky update`` ``sys.stdout`` is an ``_UpdateOutputStream`` that
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    """Implement ``jacky update --check``: fetch and report without installing.
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``jacky update`` (apply
        # path) uses — telling the user to "reinstall via curl" or that
        # ".git is missing" would point them at the wrong remediation.
        from jacky_cli.config import format_docker_update_message
        print(format_docker_update_message())
        sys.exit(1)
    if method == "pip":
        from jacky_cli.config import recommended_update_command
        from jacky_cli.banner import check_via_pypi
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``jacky update`` without requiring a reinstall.  The
    ...
    ``jacky`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/jacky.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v jacky'`` already resolves.  Idempotent.
    """
    if sys.platform != "linux":
        return
    try:
    ...
    # /usr/local/bin/jacky, code at /usr/local/lib/jacky-agent).
    fhs_link = Path("/usr/local/bin/jacky")
    ...
    path_comment = (
        "# Jacky Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of JACKY_HOME before running the update.
    ...
    JACKY_HOME directories.  The ``--backup`` flag on ``jacky update``
    ...
    # ``jacky_cli/config.py``). Fall back to false if the key is missing
    # so the default behaviour matches the shipped config: zipping a large
    # JACKY_HOME can add minutes to every update. Users who want the
    ...
    try:
        from jacky_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    try:
        ...
        home = get_jacky_home()
        try:
            display_path = f"{display_jacky_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  jacky import {out_path}")
...
def _venv_core_imports_healthy() -> tuple[bool, str]:
    ...
    ``jacky update`` may be driven by a different Python). Catches the
    ...
    Without this probe, ``jacky update`` on a current checkout prints
    ...
    if not venv_python.exists():
        ...
        # dev may run jacky from any interpreter), so report healthy to
        # avoid forcing reinstalls. But on a MANAGED install (the Windows
        # installer / desktop bootstrap stamps `.jacky-bootstrap-complete`,
        ...
        managed_markers = (
            PROJECT_ROOT / ".jacky-bootstrap-complete",
            _update_marker_path(),
        )
...
def _detect_venv_python_processes(
    *, exclude_pids: set[int] | None = None
    ...
    The jacky.exe shim guard misses the biggest lock-holder class on
    Windows: the Desktop app's backend (``python.exe -m jacky_cli.main
    ...
    calling process and its ancestors are always excluded (a CLI ``jacky
...
def _format_venv_python_holders_message(matches: list[tuple[int, str, str]]) -> str:
    ...
    lines = [
        "✗ Other Jacky processes are running from this install's venv:",
    ]
    for pid, name, cmdline in matches[:6]:
        hint = ""
        low = cmdline.lower()
        if "serve" in low or "dashboard" in low:
            hint = "  ← Jacky Desktop backend (close the desktop app)"
        elif "gateway" in low:
            hint = "  ← gateway"
    ...
    lines.append(
        "  Close the Jacky desktop app / other Jacky terminals, then re-run:"
    )
    lines.append("    jacky update")
    lines.append("  (or use `jacky update --force-venv` to proceed anyway at your own risk)")
    return "\n".join(lines)
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    jacky.exe concurrent-instance guard does not see them. They still import
    ...
    if not running_pids:
        ...
        try:
            from jacky_cli import gateway_windows
    ...
    print("→ Stopping Windows gateway process(es) before updating Jacky...")
    try:
    ...
    # — e.g. a Windows Scheduled Task running ``pythonw.exe -m jacky_cli.main
    ...
    if unmapped_pids:
        ...
        if respawnable < len(unmapped_pids):
            ...
            print("    Restart manually after update: jacky gateway run")
...
def _cold_start_windows_gateway_after_update() -> None:
    ...
    ``jacky gateway start`` uses (``gateway_windows._spawn_detached``).
    ...
    try:
        from jacky_cli import gateway_windows
        from jacky_cli.gateway import find_gateway_pids
    except Exception as exc:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``jacky update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    """Update Jacky Agent to the latest version.
    ...
    if is_managed():
        managed_error("update Jacky Agent")
        return
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `jacky update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_pip(args):
    """Update Jacky via pip (for PyPI installs)."""
    from jacky_cli import __version__
    from jacky_cli.config import is_uv_tool_install
    ...
    print("✓ Update complete! Restart jacky to use the new version.")
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if _is_windows() and not getattr(args, "force", False):
        scripts_dir = _venv_scripts_dir()
        if scripts_dir is not None:
            concurrent = _detect_concurrent_jacky_instances(scripts_dir)
            if concurrent:
    ...
    # (most commonly the Desktop app's `jacky serve` backend) will keep .pyd
    ...
    # the jacky.exe shim guard above, but its lock probe only checks the shim
    ...
    try:
        ...
        if commit_count == 0:
            ...
            if not healthy:
                ...
                if healthy_after:
                    ...
                    print("  Close all Jacky windows/gateways and re-run: jacky update")
            else:
                print("✓ Already up to date!")
            _resume_windows_gateways_after_update(_windows_gateway_resume)
            return
        ...
        # though `git pull` can't touch $JACKY_HOME, this is cheap
        ...
        try:
            from jacky_cli.backup import create_quick_snapshot
        ...
        # orphan merge-conflict markers in jacky_cli/config.py bricked
        # every user who ran ``jacky update`` for the 7 minutes between
        ...
        try:
            ...
            # so the CLI stays bootable. The user can then retry ``jacky
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``jacky update`` again later once a fix lands.")
                    else:
        ...
        # the old bytecode (e.g. get_jacky_home added to jacky_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # the marker survives and the next ``jacky`` launch finishes the
        ...
        # build.  ``jacky desktop --build-only`` uses the content-hash stamp
        ...
        # never run ``jacky desktop`` shouldn't be forced into a full
        ...
        if (desktop_dir / "package.json").exists() and find_node_executable("npm") and has_desktop_app:
            ...
            _desktop_build_cmd = [sys.executable, "-m", "jacky_cli.main", "desktop", "--build-only"]
            ...
            if build_result.returncode != 0:
                build_result = _run_logged_subprocess(_desktop_build_cmd, cwd=PROJECT_ROOT)
            if build_result.returncode != 0:
                print("  ⚠ Desktop build failed (non-fatal; run `jacky desktop` to retry)")
        ...
        # current — so copy it straight over ~/.jacky/cache/model_catalog.json
        ...
        # modules in this process.  Reload jacky_constants so that any lazy
        ...
        # seed_profile_skills() uses subprocess with an explicit JACKY_HOME so
        # it is not affected by sync_skills()'s module-level JACKY_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's JACKY_HOME env var points at the default or a named profile.
        try:
            from jacky_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from jacky_cli.profiles import backfill_profile_envs
        ...
        if version_bump_only:
            ...
            try:
                ...
                print("     Run 'jacky config migrate' to retry.")
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: jacky config migrate")
            else:
                print()
                print("Skipped. Run 'jacky config migrate' later to configure.")
        else:
        ...
        # it and warn loudly.
        try:
            from jacky_cli.backup import restore_cron_jobs_if_emptied
        ...
        # check `jacky curator status`. Self-stamps after printing so it
        ...
        # don't have it. Tying the refresh to ``jacky update`` gives users a
        ...
        try:
            ...
            if (
                ...
                from jacky_cli.tools_config import install_cua_driver
        ...
        # When running as ``jacky update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_jacky_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                ``systemctl ... jacky-gateway*`` also qualifies
                ...
                if (
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            ...
                            # (NOPASSWD for systemctl ... jacky-gateway*)
            ...
            # Discover all jacky-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. jacky-gateway.service or jacky-gateway-coder.service
                            ...
                            if _manage_cmd is None:
                                print(
                                    ...
                                    f"    To let `jacky update` restart it automatically, allow\n"
                        ...
                        print(
                            ...
                            f"Check the gateway with: jacky gateway status"
                        )
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: jacky gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: jacky -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy Jacky gateway unit files are still installed.
        # When both jacky.service (from a pre-rename install) and the
        # current jacky-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `jacky update` surfaces the issue until the user migrates.
        try:
            from jacky_cli.gateway import (
                has_legacy_jacky_units,
                _find_legacy_jacky_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_jacky_units():
                print()
                print("⚠ Legacy Jacky gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_jacky_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (jacky.service) fight the current")
                print("  jacky-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    jacky gateway migrate-legacy")
                print()
        ...
        print("  jacky model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``jacky -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `jacky profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_jacky_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} → jacky -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print("Switched to: default (~/.jacky)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  jacky profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     jacky -p {name} chat")
                else:
        ...
        from jacky_cli import profiles as _profiles_mod
        ...
        from jacky_cli import profile_describer as _pd
        ...
        if dist_name:
            ...
            print(f"  (run `jacky profile info {name}` for full manifest)")
        if alias_name:
            is_windows = sys.platform == "win32"
            wrapper = _get_wrapper_dir() / (f"{alias_name}.bat" if is_windows else alias_name)
            print(f"Alias:   {alias_name} → jacky -p {name}  ({wrapper})")
        print()
        ...
        from jacky_cli.profiles import profile_exists, validate_alias_name
        ...
        from jacky_cli.profiles import rename_profile
        ...
        from jacky_cli.profiles import export_profile
        ...
        from jacky_cli.profiles import import_profile
        ...
        import tempfile
        from jacky_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="jacky_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  jacky -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      jacky -p {plan.manifest.name} chat")
        ...
        from jacky_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `jacky profile install` can be updated."
                )
                sys.exit(1)
            ...
            if plan.has_cron:
                print(
                    ...
                    f"jacky -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from jacky_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("jacky_requires"):
            print(f"Requires:     Jacky {data['jacky_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from jacky_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.jacky_requires:
        print(f"  Requires: Jacky {mf.jacky_requires}")
...
def _report_dashboard_status() -> int:
    """Print ``jacky dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``jacky dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No jacky dashboard processes running.")
        return 0

    print(f"{len(pids)} jacky dashboard process(es) running:")
    for pid in pids:
...
def _maybe_setup_dashboard_auth_interactively(args) -> None:
    ...
    ``jacky dashboard register`` for OAuth.
    ...
    try:
        from jacky_cli.web_server import should_require_auth
    ...
    try:
        from jacky_cli.dashboard_auth import list_providers
        if list_providers():
    ...
    print("    [2] OAuth via Nous Portal (run `jacky dashboard register`)")
    print("    [3] Cancel")
    print()
    ...
    try:
        from jacky_cli.plugins import discover_plugins
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No jacky dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `jacky update`.
    ...
    # launches the dashboard (`worker dashboard` → JACKY_HOME points into
    ...
    if (
        ...
        and os.environ.get("JACKY_DESKTOP") != "1"
    ):
        ...
        reexec_argv = [
            sys.executable, "-m", "jacky_cli.main",
            "-p", "default",
        ...
        # JACKY_HOME.  We must resolve the root explicitly instead of just
        # dropping JACKY_HOME: in the Docker layout the machine root is
        # /opt/data (set via `ENV JACKY_HOME=/opt/data`), so an unset
        # JACKY_HOME falls back to $HOME/.jacky = /opt/data/.jacky — an
        ...
        # update-button guard also misfires).  get_default_jacky_root()
        # returns the root for both layouts: ~/.jacky for a standard install
        ...
        try:
            ...
            # behaviour (drop JACKY_HOME) rather than block the reroute.
            env.pop("JACKY_HOME", None)
    ...
    if _headless_backend:
        ...
        os.environ["JACKY_SERVE_HEADLESS"] = "1"
    elif "JACKY_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            Path(os.environ["JACKY_WEB_DIST"])
            if "JACKY_WEB_DIST" in os.environ
            else PROJECT_ROOT / "jacky_cli" / "web_dist"
        )
        ...
        # JACKY_WEB_DIST is set without --skip-build: the build is skipped
        ...
        _dist_root = Path(os.environ["JACKY_WEB_DIST"]).expanduser()
        if not (_dist_root / "index.html").exists():
            print(f"✗ JACKY_WEB_DIST is set but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or unset JACKY_WEB_DIST to build and use the default web UI dist.")
            sys.exit(1)
        # Write the expanded path back: web_server reads JACKY_WEB_DIST raw
        # at import (no expanduser), so a validated "~/dist" would otherwise
        # pass here and still 404 there.
        os.environ["JACKY_WEB_DIST"] = str(_dist_root)
        print(f"→ Using web dist from JACKY_WEB_DIST: {_dist_root}")
    ...
    try:
        from jacky_cli.mcp_startup import start_background_mcp_discovery
    ...
    from jacky_cli.web_server import start_server
...
def cmd_dashboard_register(args):
    ...
    from jacky_cli.dashboard_register import cmd_dashboard_register as _impl
...
def cmd_gateway_enroll(args):
    ...
    from jacky_cli.gateway_enroll import cmd_gateway_enroll as _impl
...
def cmd_completion(args, parser=None):
    ...
    from jacky_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_prompt_size(args):
    ...
    from jacky_cli.prompt_size import cmd_prompt_size as _impl

    _impl(args)


def cmd_logs(args):
    """View and filter Jacky log files."""
    from jacky_cli.logs import tail_log, list_logs
...
def cmd_console(args):
    """Open the safe Jacky command console."""
    from jacky_cli.console_engine import run_console_repl
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from jacky_cli.models import CANONICAL_PROVIDERS as _cp
...
# so that in ``jacky -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``jacky_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``jacky -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``jacky`` or only flags → defaults to ``chat``.
        return False
...
def _is_tui_chat_launch(args) -> bool:
    return bool(getattr(args, "tui", False) or os.environ.get("JACKY_TUI") == "1")
...
def _prepare_agent_startup(args) -> None:
    ...
    # --yolo: chokepoint guarantee that JACKY_YOLO_MODE is set before ANY
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    from jacky_cli._parser import build_top_level_parser
    ...
    if getattr(args, "oneshot", None):
        ...
        from jacky_cli.oneshot import run_oneshot
    ...
    if args.command in {None, "chat"}:
        ...
        if interactive_prompt:
            ...
            os.environ["JACKY_DEFER_AGENT_STARTUP"] = "1"
            os.environ["JACKY_FAST_STARTUP_BANNER"] = "1"
            if getattr(args, "accept_hooks", False):
                os.environ["JACKY_ACCEPT_HOOKS"] = "1"
        else:
            _prepare_agent_startup(args)
        cmd_chat(args)
        return True
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `jacky --tui` is the hot path on phones. The full parser setup imports
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        if not existing:
            print(
                f"\n  Nothing to reset — no memory files found in {display_jacky_home()}/memories/\n"
            )
            return
        ...
        print(f"  Files were in: {display_jacky_home()}/memories/\n")
    else:
        from jacky_cli.memory_setup import memory_command

        memory_command(args)


def cmd_acp(args):
    """Launch Jacky Agent as an ACP server."""
    try:
...
def cmd_tools(args):
    ...
    if action in {"list", "disable", "enable"}:
        from jacky_cli.tools_config import tools_disable_enable_command

        tools_disable_enable_command(args)
    elif action == "post-setup":
        from jacky_cli.tools_config import run_post_setup_command

        sys.exit(run_post_setup_command(args))
    else:
        _require_tty("tools")
        from jacky_cli.tools_config import tools_command
...
def cmd_skills(args):
    ...
    if getattr(args, "skills_action", None) == "config":
        ...
        from jacky_cli.skills_config import skills_command as skills_config_command

        skills_config_command(args)
    else:
        from jacky_cli.skills_hub import skills_command

        skills_command(args)


def cmd_pairing(args):
    from jacky_cli.pairing import pairing_command

    pairing_command(args)


def cmd_plugins(args):
    from jacky_cli.plugins_cmd import plugins_command

    plugins_command(args)


def cmd_mcp(args):
    from jacky_cli.mcp_config import mcp_command

    mcp_command(args)


def cmd_claw(args):
    from jacky_cli.claw import claw_command

    claw_command(args)


def main():
    """Main entry point for jacky CLI."""
    # Cosmetic: make the process show up as 'jacky' instead of 'python3.11'
    # in ps/top/htop.  Non-fatal — just a nicer UX.
    _set_process_title()

    # Force UTF-8 stdio on Windows before anything prints.  No-op elsewhere.
    try:
        from jacky_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    # Sweep stale ``jacky.exe.old.*`` quarantine files left by previous
    # ``jacky update`` runs on Windows. Silent no-op on non-Windows or when
    # there's nothing to clean. See ``_quarantine_running_jacky_exe``.
    try:
        _cleanup_quarantined_exes()
    except Exception:
        pass

    # Self-heal a venv left half-built by an interrupted ``jacky update``
    ...
    # ``jacky skills install update``) merely defers recovery one launch;
    # under-matching (missing ``jacky -p work update``) would race a recovery
    ...
    # =========================================================================
    # model command  (parser built in jacky_cli/subcommands/model.py)
    # =========================================================================
    build_model_parser(subparsers, cmd_model=cmd_model)

    from jacky_cli.moa_cmd import cmd_moa
    ...
    # =========================================================================
    from jacky_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `jacky model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    from jacky_cli import secrets_cli as _secrets_cli
    from jacky_cli import onepassword_secrets_cli as _op_secrets_cli
    ...
    # migrate command
    # =========================================================================
    from jacky_cli.migrate import cmd_migrate, cmd_migrate_xai
    ...
    # =========================================================================
    # gateway + proxy commands  (parsers built in jacky_cli/subcommands/gateway.py)
    # =========================================================================
    ...
    # =========================================================================
    # setup command  (parser built in jacky_cli/subcommands/setup.py)
    # =========================================================================
    build_setup_parser(subparsers, cmd_setup=cmd_setup)

    # =========================================================================
    # postinstall command  (parser built in jacky_cli/subcommands/postinstall.py)
    # =========================================================================
    build_postinstall_parser(subparsers, cmd_postinstall=cmd_postinstall)

    # =========================================================================
    # whatsapp command  (parser built in jacky_cli/subcommands/whatsapp.py)
    # =========================================================================
    ...
    whatsapp_cloud_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "required). Distinct from `jacky whatsapp` which sets up "
    ...
    # =========================================================================
    # slack command  (parser built in jacky_cli/subcommands/slack.py)
    # =========================================================================
    ...
    # =========================================================================
    from jacky_cli.send_cmd import register_send_subparser
    register_send_subparser(subparsers)

    # =========================================================================
    # login command  (parser built in jacky_cli/subcommands/login.py)
    # =========================================================================
    build_login_parser(subparsers, cmd_login=cmd_login)

    # =========================================================================
    # logout command  (parser built in jacky_cli/subcommands/logout.py)
    # =========================================================================
    build_logout_parser(subparsers, cmd_logout=cmd_logout)

    # =========================================================================
    # auth command  (parser built in jacky_cli/subcommands/auth.py)
    # =========================================================================
    build_auth_parser(subparsers, cmd_auth=cmd_auth)

    # =========================================================================
    # status command  (parser built in jacky_cli/subcommands/status.py)
    # =========================================================================
    build_status_parser(subparsers, cmd_status=cmd_status)

    # =========================================================================
    # cron command  (parser built in jacky_cli/subcommands/cron.py)
    # =========================================================================
    build_cron_parser(subparsers, cmd_cron=cmd_cron)

    # =========================================================================
    # webhook command  (parser built in jacky_cli/subcommands/webhook.py)
    # =========================================================================
    ...
    # =========================================================================
    from jacky_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command — multi-profile collaboration board
    # =========================================================================
    from jacky_cli.kanban import build_parser as _build_kanban_parser
    ...
    # =========================================================================
    from jacky_cli.projects_cmd import build_parser as _build_project_parser
    ...
    # =========================================================================
    # hooks command  (parser built in jacky_cli/subcommands/hooks.py)
    # =========================================================================
    build_hooks_parser(subparsers, cmd_hooks=cmd_hooks)

    # =========================================================================
    # doctor command  (parser built in jacky_cli/subcommands/doctor.py)
    # =========================================================================
    ...
    # =========================================================================
    # security command  (parser built in jacky_cli/subcommands/security.py)
    # =========================================================================
    build_security_parser(subparsers, cmd_security=cmd_security)

    # =========================================================================
    # dump command  (parser built in jacky_cli/subcommands/dump.py)
    # =========================================================================
    build_dump_parser(subparsers, cmd_dump=cmd_dump)

    # =========================================================================
    # debug command  (parser built in jacky_cli/subcommands/debug.py)
    # =========================================================================
    build_debug_parser(subparsers, cmd_debug=cmd_debug)

    # =========================================================================
    # backup command  (parser built in jacky_cli/subcommands/backup.py)
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.jacky/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo jacky uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from jacky_cli.checkpoints import register_cli as _register_checkpoints_cli
    _register_checkpoints_cli(checkpoints_parser)

    # =========================================================================
    # import command  (parser built in jacky_cli/subcommands/import_cmd.py)
    # =========================================================================
    build_import_cmd_parser(subparsers, cmd_import=cmd_import)

    # =========================================================================
    # config command  (parser built in jacky_cli/subcommands/config.py)
    # =========================================================================
    build_config_parser(subparsers, cmd_config=cmd_config)

    # =========================================================================
    # console command  (parser built in jacky_cli/subcommands/console.py)
    # =========================================================================
    build_console_parser(subparsers, cmd_console=cmd_console)

    # =========================================================================
    # pairing command  (parser built in jacky_cli/subcommands/pairing.py)
    # =========================================================================
    build_pairing_parser(subparsers, cmd_pairing=cmd_pairing)

    # =========================================================================
    # skills command  (parser built in jacky_cli/subcommands/skills.py)
    # =========================================================================
    ...
    from jacky_cli.bundles import register_cli as _bundles_register, bundles_command
    _bundles_register(bundles_parser)
    bundles_parser.set_defaults(func=bundles_command)

    # =========================================================================
    # plugins command  (parser built in jacky_cli/subcommands/plugins.py)
    # =========================================================================
    ...
    # subcommand — ``jacky --help``, ``jacky version``, ``jacky logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from jacky_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from jacky_cli.curator import register_cli as _register_curator_cli
    ...
    pets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "and Jacky shows it reacting to agent activity across the CLI, "
            "TUI, and desktop app."
        ),
    )
    try:
        from jacky_cli.pets import register_cli as _register_pets_cli
    ...
    try:
        from jacky_cli.journey import register_cli as _register_journey_cli
    ...
    # =========================================================================
    # memory command  (parser built in jacky_cli/subcommands/memory.py)
    # =========================================================================
    build_memory_parser(subparsers, cmd_memory=cmd_memory)

    # =========================================================================
    # tools command  (parser built in jacky_cli/subcommands/tools.py)
    # =========================================================================
    ...
    computer_use_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Linux.\n\n"
            "Use `jacky computer-use install` to fetch and run the\n"
            "upstream cua-driver installer. This is equivalent to the\n"
            "post-setup hook that `jacky tools` runs when you first\n"
            "enable the Computer Use toolset, and is a stable target\n"
            "for re-running the install if it didn't fire (e.g. when\n"
            "toggling the toolset on a returning-user setup).\n\n"
            "Use `jacky computer-use doctor` to run cua-driver's\n"
    ...
    computer_use_perms = computer_use_sub.add_parser(
        ...
        description=(
            ...
            "terminal or the Jacky app. `status` reports the driver's grant\n"
    ...
    def cmd_computer_use(args):
        ...
        if action == "status":
            import shutil
            import subprocess
            from jacky_cli.tools_config import _cua_driver_cmd
            # Honor JACKY_CUA_DRIVER_CMD for local-build testing — same
            ...
            if path:
                ...
                try:
                    ...
                    if st and st.get("update_available"):
                        ...
                        print("    Run: jacky computer-use install --upgrade")
                    elif st:
                        print("  ✓ Up to date.")
                    else:
                        # Older driver (no check-update verb) or offline.
                        print("  Refresh to latest: jacky computer-use install --upgrade")
                except Exception:
                    print("  Refresh to latest: jacky computer-use install --upgrade")
                return
            print("cua-driver: not installed")
            print("  Run: jacky computer-use install")
            return
        if action == "doctor":
        ...
        if action == "permissions":
            ...
            if perms_action == "status":
                ...
                if not st["platform_supported"]:
                    ...
                    sys.exit(1)
                if not st["installed"]:
                    print("cua-driver: not installed. Run: jacky computer-use install")
                    sys.exit(1)
                ...
                if st["can_grant"]:  # macOS TCC permissions
                    ...
                    if not st["ready"]:
                        print("  Grant: jacky computer-use permissions grant")
    ...
    # =========================================================================
    # mcp command  (parser built in jacky_cli/subcommands/mcp.py)
    # =========================================================================
    ...
    sessions_export.add_argument(
        ...
        help=(
            ...
            "md/qmd: output directory (default: <jacky home>/session-exports)"
        ),
    )
    sessions_export.add_argument(
        "--format",
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            from jacky_cli.session_filters import (
                build_prune_filters,
                describe_filters,
            )
            ...
            def _redact(data):
                if not args.redact or data is None:
                    return data
                from jacky_cli.session_export_md import redact_session_data
            ...
            if args.format == "html":
                if not args.output or args.output == "-":
                    ...
                    return
                from jacky_cli.session_export_html import (
                ...
                try:
                    if len(ids) == 1:
                        ...
                        out_dir = (
                            ...
                            else get_jacky_home() / "session-exports"
                        )
            ...
            # Markdown / QMD export
            from jacky_cli.session_export_md import (
            ...
            if args.output == "-":
                ...
                db.close()
                return
            output_dir = Path(args.output).expanduser() if args.output else get_jacky_home() / "session-exports"
            ...
            if args.session_id:
                ...
                if args.delete_after_verified:
                    ...
                    if not ok:
                        ...
                        db.close()
                        return
                    sessions_dir = get_jacky_home() / "sessions"
            ...
            # `jacky sessions prune`: no time window and no filters at all
            ...
            # Launch jacky --resume <id> by replacing the current process
    ...
    # =========================================================================
    # insights command  (parser built in jacky_cli/subcommands/insights.py)
    # =========================================================================
    build_insights_parser(subparsers, cmd_insights=cmd_insights)

    # =========================================================================
    # claw command  (parser built in jacky_cli/subcommands/claw.py)
    # =========================================================================
    build_claw_parser(subparsers, cmd_claw=cmd_claw)

    # =========================================================================
    # version command  (parser built in jacky_cli/subcommands/version.py)
    # =========================================================================
    build_version_parser(subparsers, cmd_version=cmd_version)

    # =========================================================================
    # update command  (parser built in jacky_cli/subcommands/update.py)
    # =========================================================================
    build_update_parser(subparsers, cmd_update=cmd_update)

    # =========================================================================
    # uninstall command  (parser built in jacky_cli/subcommands/uninstall.py)
    # =========================================================================
    build_uninstall_parser(subparsers, cmd_uninstall=cmd_uninstall)

    # =========================================================================
    # acp command  (parser built in jacky_cli/subcommands/acp.py)
    # =========================================================================
    build_acp_parser(subparsers, cmd_acp=cmd_acp)

    # =========================================================================
    # profile command  (parser built in jacky_cli/subcommands/profile.py)
    # =========================================================================
    ...
    # =========================================================================
    # dashboard command  (parser built in jacky_cli/subcommands/dashboard.py)
    # =========================================================================
    ...
    # for one release. The Jacky-Setup.exe success screen tells users to
    # run `jacky desktop` from a terminal, so the canonical name needs
    # to be the one that appears in --help (argparse promotes the primary
    # name; aliases stay hidden).
    # =========================================================================
    # gui command  (parser built in jacky_cli/subcommands/gui.py)
    # =========================================================================
    build_gui_parser(subparsers, cmd_gui=cmd_gui)

    # =========================================================================
    # logs command  (parser built in jacky_cli/subcommands/logs.py)
    # =========================================================================
    build_logs_parser(subparsers, cmd_logs=cmd_logs)

    # =========================================================================
    # prompt-size command  (parser built in jacky_cli/subcommands/prompt_size.py)
    # =========================================================================
    ...
    # e.g. ``jacky -c Pokemon Agent Dev`` → ``jacky -c 'Pokemon Agent Dev'``
    ...
    from jacky_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'jacky -c model' where 'model' is consumed as the
    ...
    # --yolo: set JACKY_YOLO_MODE *before* plugin discovery.  The call to
    ...
    # so introspection/management commands (jacky hooks list, cron

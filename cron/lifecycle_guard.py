...
``jacky gateway restart`` (or ``launchctl kickstart ai.jacky.gateway``
or ``systemctl restart jacky-gateway``).  When the cron fires, the
...
``jacky cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``jacky gateway``, ``launchctl ... jacky-gateway``,
``systemctl ... jacky-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_JACKY_GATEWAY=1``, and
``jacky gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `jacky gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:jacky\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a jacky-gateway label. macOS launchd
    # labels look like `ai.jacky.gateway` / `jacky-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated jacky services (e.g.
    # `launchctl unload ai.jacky.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bjacky[.\-]?gateway)"
    # Branch C: systemctl ops on a jacky-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bjacky[.\-]?gateway)"
    # Branch D: pkill / kill targeting the jacky gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<JACKY_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.jacky/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_jacky_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `jacky gateway restart` from a shell outside "

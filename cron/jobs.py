...
Jobs are stored in ~/.jacky/cron/jobs.json
Output is saved to ~/.jacky/cron/output/{job_id}/{timestamp}.md
"""
...
# store under its own JACKY_HOME, and a profile-scoped gateway runs that
# profile's jobs under that same JACKY_HOME — so a job authored in profile
# `coder` lives in `~/.jacky/profiles/coder/cron/jobs.json` and executes with
# `coder`'s `.env`, `config.yaml`, and skills. We deliberately anchor on
# `get_jacky_home()` (the active profile home), NOT `get_default_jacky_root()`
# (the shared root). Anchoring at the root would funnel every profile's jobs
# into one shared `jobs.json` and run them under whatever JACKY_HOME the
...
# `_get_jacky_home()` / `_get_lock_paths()` resolution in cron/scheduler.py.
JACKY_DIR = get_jacky_home().resolve()
...
CRON_DIR = JACKY_DIR / "cron"
JOBS_FILE = CRON_DIR / "jobs.json"
# Heartbeat file the in-process ticker touches on every loop iteration. The
# gateway process and the (separate) ``jacky cron status`` process share it
...
# threshold in `jacky cron status` (jacky_cli/cron.py), so the two never
# drift apart.
...
# the cron inactivity timeout is disabled (JACKY_CRON_TIMEOUT=0 → unlimited),
...
# TTL only recovers a claim left by a tick that DIED mid-run. JACKY_CRON_TIMEOUT
...
def _oneshot_run_claim_ttl_seconds() -> float:
    ...
    Derived from ``JACKY_CRON_TIMEOUT`` (the cron inactivity timeout the
    ...
    """
    raw = os.getenv("JACKY_CRON_TIMEOUT", "").strip()
...
def _jobs_lock():
    ...
    and standalone ``jacky`` CLI invocations, which previously shared no lock
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    if 'T' in schedule or re.match(r'^\d{4}-\d{2}-\d{2}', schedule):
        try:
            ...
            #
            # Anchor to the CONFIGURED Jacky timezone, not the server's local
            # timezone. The due-check (`get_due_jobs`) compares `next_run_at`
            # against `jacky_time.now()`, which uses the configured zone. If a
            ...
            if dt.tzinfo is None:
                jacky_tz = _jacky_now().tzinfo
                dt = dt.replace(tzinfo=jacky_tz)
            return {
                "kind": "once",
                "run_at": dt.isoformat(),
    ...
    try:
        ...
        run_at = _jacky_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Jacky configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Jacky timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _jacky_now().tzinfo
    if dt.tzinfo is None:
...
def _stored_wall_clock_is_future(stored: datetime, current: datetime) -> bool:
    ...
    Cron schedules express local wall-clock intent. If Jacky/system local time
    ...
    if kind == "once":
        ...
        if not HAS_CRONITER:
            logger.warning(
                ...
                "reinstall jacky-agent or run 'pip install croniter' in your "
                "runtime env.",
                expr,
            )
            return None
...
# =============================================================================
# Ticker heartbeat (liveness signal for `jacky cron status`)
# =============================================================================
...
def _atomic_write_epoch(path: Path) -> None:
    ...
    concurrent reader in another process (``jacky cron status``) never sees a
...
def record_ticker_heartbeat(success: bool = False) -> None:
    ...
    `jacky cron status` can tell a thread that is merely *alive and looping*
...
def _save_jobs_unlocked(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _jacky_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def _resolve_default_model_snapshot() -> Optional[str]:
    ...
    try:
        ...
        cfg_path = get_jacky_home() / "config.yaml"
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ...
                ~/.jacky/scripts/; ``.sh`` / ``.bash`` files run via bash,
    ...
    # covered, not just `jacky cron create`.
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _jacky_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job["id"],
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _jacky_now().isoformat(),
        },
    )
...
def _machine_id() -> str:
    ...
    Uses ``JACKY_MACHINE_ID`` if set, else hostname + pid. The CAS correctness
    comes from the file lock + the fresh-claim check, not from this value.
    """
    explicit = os.getenv("JACKY_MACHINE_ID", "").strip()
    if explicit:
        return explicit
    try:
        import socket
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    # (derived from JACKY_CRON_TIMEOUT). See _oneshot_run_claim_ttl_seconds.
    ...
    for job in jobs:
        ...
        try:
            ...
            if next_run_dt <= now:
                ...
                # desktop both run in-process 60s tickers on one JACKY_HOME)
                ...
                # (_oneshot_run_claim_ttl_seconds, derived from JACKY_CRON_TIMEOUT),
...
# execution. Unlike the quick-snapshot store (`jacky_cli.backup`, capped at 20)
...
def _prune_job_output(job_output_dir: Path, keep: int) -> int:
    ...
    Mirrors the quick-snapshot retention in ``jacky_cli.backup._prune_quick_snapshots``:
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _jacky_now().strftime("%Y-%m-%d_%H-%M-%S")

...
Uses a file-based lock (~/.jacky/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `jacky update` reloads
# the module) fail with ModuleNotFoundError for jacky_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from jacky_cli.jacky_constants import get_jacky_home
from jacky_cli._subprocess_compat import windows_hide_flags
from jacky_cli.config import load_config, _expand_env_vars
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy jacky_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from jacky_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``jacky tools`` config for the ``cron`` platform.
    ...
    try:
        from jacky_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``jacky update`` / ``jacky gateway stop`` / systemd restart, or an
...
_jacky_home: Path | None = None


def _get_jacky_home() -> Path:
    """Resolve Jacky home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active JACKY_HOME at call time
    ...
    """
    return _jacky_home or get_jacky_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    jacky_home = _get_jacky_home()
    lock_dir = jacky_home / "cron"
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("JACKY_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid JACKY_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within JACKY_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Jacky-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against JACKY_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_jacky_home() / "scripts"
    ...
    # escape — scripts MUST reside within JACKY_HOME/scripts/.
    try:
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _jacky_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_jacky_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_jacky_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["JACKY_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed JACKY_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads JACKY_SESSION_PLATFORM
    #     and JACKY_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via JACKY_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     JACKY_SESSION_PLATFORM == "telegram".
    ...
    #     JACKY_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read JACKY_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the JACKY_CRON_AUTO_DELIVER_* vars set
    # below, so clearing JACKY_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "JACKY_CRON_AUTO_DELIVER_PLATFORM",
        "JACKY_CRON_AUTO_DELIVER_CHAT_ID",
        "JACKY_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        # load_jacky_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this JACKY_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_jacky_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from jacky_cli.env_loader import (
            load_jacky_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_jacky_dotenv(jacky_home=_get_jacky_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["JACKY_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["JACKY_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["JACKY_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > JACKY_MODEL env >
        ...
        model = job.get("model") or os.getenv("JACKY_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_jacky_home() / "config.yaml")
        ...
        prefill_file = (
            os.getenv("JACKY_PREFILL_MESSAGES_FILE", "")
        ...
        try:
            # Do not inject JACKY_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        agent = AIAgent(
            ...
            # JACKY_HOME. When a workdir is configured, also inject project
        ...
        # override via JACKY_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("JACKY_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid JACKY_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
...
**Job ID:** {job_id}
**Run Time:** {_jacky_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_jacky_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_jacky_home())
        )
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _jacky_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _jacky_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set JACKY_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("JACKY_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid JACKY_CRON_MAX_PARALLEL value; defaulting to unbounded")

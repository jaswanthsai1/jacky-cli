...
1. Bundled plugins: ``plugins/model-providers/<name>/`` (shipped with jacky-agent)
2. User plugins: ``$JACKY_HOME/plugins/model-providers/<name>/``
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$JACKY_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$JACKY_HOME/plugins/model-providers/`` if it exists."""
    try:
        from jacky_constants import get_jacky_home

        d = get_jacky_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # multiple JACKY_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"plugins.model_providers.{safe_name}"
    else:
        module_name = f"_jacky_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$JACKY_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with jacky-agent.
    ...
    # 2. User plugins — under $JACKY_HOME/plugins/model-providers/<name>/.

# Jacky Security Policy

This document describes Jacky's trust model, names the one
...
or **collegegamerff@gmail.com**. Do not open public issues for
security vulnerabilities. **Jacky does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Jacky access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Jacky's documentation
...
Jacky supports two OS-level isolation postures. They address
...
Jacky supports this in two ways:

- **Jacky's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Jacky's in-process heuristics
...
Jacky filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Jacky
itself. Bugs in Jacky's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Jacky does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Jacky's own documentation, or reasonable
  operator expectations would predict — including cases where
  Jacky has documented a stance about how its output should
...
- Jacky includes supply-chain guards for MCP server

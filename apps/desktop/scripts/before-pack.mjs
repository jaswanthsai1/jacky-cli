 ...
 * product name (`Jacky`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Jacky'
 *
 * This is a hard failure with no obvious cause for the user — `jacky desktop`
 ...
 * (the mac-unpacked Jacky.app bundle) and Windows (win-unpacked), so we

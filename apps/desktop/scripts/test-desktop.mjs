...
// no longer bundles the Jacky Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Jacky.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Jacky'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Jacky.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'jacky'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default JACKY_HOME for non-sandboxed runs -- matches main.ts's
// resolveJackyHome(). On Windows it's %LOCALAPPDATA%\jacky; elsewhere
// it's ~/.jacky. The fresh-install sandbox launchFresh() sets its own
// JACKY_HOME and never touches this.
const DEFAULT_JACKY_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'jacky')
  }
  return path.join(os.homedir(), '.jacky')
})()
const VENV_ROOT = path.join(DEFAULT_JACKY_HOME, 'jacky-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'jacky-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.JACKY_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Jacky-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Jacky-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Jacky-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Jacky-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.JACKY_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.JACKY_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const jackyHome = path.join(sandbox, 'jacky-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(jackyHome, { recursive: true })
  ...
  env.JACKY_DESKTOP_CWD = cwd
  env.JACKY_DESKTOP_IGNORE_EXISTING = '1'
  env.JACKY_DESKTOP_TEST_MODE = 'fresh-install'
  env.JACKY_DESKTOP_USER_DATA_DIR = userDataDir
  env.JACKY_HOME = jackyHome
  delete env.JACKY_DESKTOP_JACKY
  delete env.JACKY_DESKTOP_JACKY_ROOT
  ...
  console.log(`  JACKY_HOME: ${jackyHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(jackyHome, 'jacky-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Jacky Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of jacky_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'jacky-agent', 'jacky_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Jacky
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + JACKY_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  JACKY_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}

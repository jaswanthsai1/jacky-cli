 ...
 *
 * Stamps the Jacky icon + identity onto the packed Windows Jacky.exe via
 * rcedit (delegated to set-exe-identity.mjs). This runs for EVERY packed build
 * — first install, `jacky desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Jacky')
 */
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Jacky'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Jacky.exe keeps the stock Electron icon`)
  }
}

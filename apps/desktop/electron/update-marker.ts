...
export function markerPath(jackyHome) {
  return path.join(jackyHome, '.jacky-update-in-progress')
}
...
export function readLiveUpdateMarker(
  ...
  const file = markerPath(jackyHome)
  let raw
 ...
 *
 * The Tauri-based jacky-setup.exe takes several seconds to initialise its
 ...
 */
export function writeUpdateMarker(jackyHome, pid, { now = Date.now } = {}) {
  const file = markerPath(jackyHome)

...
function encryptDesktopSecret(value, safeStorageApi) {
  ...
  if (!encryptionAvailable) {
    throw new Error(
      'Secure token storage is unavailable, so Jacky Desktop cannot save remote gateway tokens. ' +
        'Set JACKY_DESKTOP_REMOTE_URL and JACKY_DESKTOP_REMOTE_TOKEN in your environment, or enable OS keychain access and try again.'
    )
  }
  ...
  try {
    ...
    throw new Error(
      `Failed to encrypt the remote gateway token for secure storage${detail}. ` +
        'Set JACKY_DESKTOP_REMOTE_URL and JACKY_DESKTOP_REMOTE_TOKEN in your environment as a fallback.'
    )
  }
}

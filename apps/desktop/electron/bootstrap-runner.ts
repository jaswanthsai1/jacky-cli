 ...
 * bootstrap-runner.ts
 *
 * Drives apps/desktop's first-launch install of Jacky Agent by spawning
...
function bootstrapCacheDir(jackyHome) {
  return path.join(jackyHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.jacky/jacky-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(jackyHome) {
  if (!jackyHome) {
    return null
  }
  const candidate = path.join(jackyHome, 'jacky-agent', 'scripts', installScriptName())
...
function cachedScriptPath(jackyHome, commit) {
  return path.join(bootstrapCacheDir(jackyHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/jaswanthsai1/jacky-cli/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({
  ...
  const cached = cachedScriptPath(jackyHome, installStamp.commit)
  ...
  try {
    ...
    const installed = installedAgentInstallScript(jackyHome)
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, jackyHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
    ...
    const child = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Pass JACKY_HOME through so install.ps1 respects the caller's
          // choice rather than re-computing the default.
          JACKY_HOME: jackyHome || process.env.JACKY_HOME || ''
        }
      })
    )
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, jackyHome }: any = {}) {
  return new Promise<any>((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, jackyHome, pinCommit = true }) {
  const args = ['--dir', activeRoot, '--jacky-home', jackyHome]
...
async function fetchManifest({
  ...
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, jackyHome, pinCommit })]
...
async function runStage({
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, jackyHome, pinCommit })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(jackyHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, jackyHome, emit })

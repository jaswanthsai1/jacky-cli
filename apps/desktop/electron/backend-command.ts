...
// Backend subcommand routing for the desktop-managed Jacky process.
//
// The desktop app launches its own headless backend via `jacky serve` — it
...
// app hasn't updated yet, or an older `jacky` resolved from PATH) only knows
 ...
 * @param {string} [profile] optional Jacky profile to pin via `--profile`.
 */
 ...
 * `-m jacky_cli.main` and any `--profile <name>`). Returns a copy; if there is
...
/**
 * True when a runtime's `jacky_cli/subcommands/dashboard.py` source registers

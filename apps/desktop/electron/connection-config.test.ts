...
test('profileRemoteOverride returns the per-profile remote with defaulted auth mode', () => {
  const config = {
    profiles: {
      coder: { mode: 'remote', url: '  https://coder.example.com/jacky  ', token: { value: 'sek' } }
    }
  }

  assert.deepEqual(profileRemoteOverride(config, 'coder'), {
    url: 'https://coder.example.com/jacky',
    authMode: 'token',
    token: { value: 'sek' }
  })
})
...
test('normalizeRemoteBaseUrl strips trailing slashes, hash, and query', () => {
  ...
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/jacky/'), 'https://gw.example.com/jacky')
  assert.equal(normalizeRemoteBaseUrl('https://gw.example.com/jacky?x=1#frag'), 'https://gw.example.com/jacky')
})

test('normalizeRemoteBaseUrl preserves a path prefix', () => {
  assert.equal(normalizeRemoteBaseUrl('https://host/jacky'), 'https://host/jacky')
})
...
test('buildGatewayWsUrl honors a path prefix', () => {
  assert.equal(buildGatewayWsUrl('https://host/jacky', 't'), 'wss://host/jacky/api/ws?token=t')
})
...
test('buildGatewayWsUrlWithTicket uses ?ticket= not ?token=', () => {
  const url = buildGatewayWsUrlWithTicket('https://gw.example.com/jacky', 'tkt-9')
  assert.equal(url, 'wss://gw.example.com/jacky/api/ws?ticket=tkt-9')
...
test('cookiesHaveSession detects the bare access-token cookie', () => {
  assert.equal(cookiesHaveSession([{ name: 'jacky_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession detects the __Host- and __Secure- prefixed variants', () => {
  assert.equal(cookiesHaveSession([{ name: '__Host-jacky_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveSession([{ name: '__Secure-jacky_session_at', value: 'x' }]), true)
})

test('cookiesHaveSession is false for an empty value', () => {
  assert.equal(cookiesHaveSession([{ name: 'jacky_session_at', value: '' }]), false)
})
...
test('cookiesHaveSession ignores unrelated cookies (AT-only by design)', () => {
  ...
  assert.equal(cookiesHaveSession([{ name: 'jacky_session_rt', value: 'x' }]), false)
...
test('AT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(AT_COOKIE_VARIANTS, ['__Host-jacky_session_at', '__Secure-jacky_session_at', 'jacky_session_at'])
})

test('RT_COOKIE_VARIANTS covers all three deploy shapes', () => {
  assert.deepEqual(RT_COOKIE_VARIANTS, ['__Host-jacky_session_rt', '__Secure-jacky_session_rt', 'jacky_session_rt'])
})

// --- cookiesHaveLiveSession (AT or RT — the connectivity check) ---

test('cookiesHaveLiveSession is true for a live access-token cookie', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'jacky_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-jacky_session_at', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-jacky_session_at', value: 'x' }]), true)
})
...
test('cookiesHaveLiveSession is true for an RT cookie even with NO access-token cookie', () => {
  ...
  assert.equal(cookiesHaveLiveSession([{ name: 'jacky_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Host-jacky_session_rt', value: 'x' }]), true)
  assert.equal(cookiesHaveLiveSession([{ name: '__Secure-jacky_session_rt', value: 'x' }]), true)
})

test('cookiesHaveLiveSession is true when both AT and RT are present', () => {
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'jacky_session_at', value: 'a' },
      { name: 'jacky_session_rt', value: 'r' }
    ]),
    true
  )
})
...
test('cookiesHaveLiveSession is false for empty values', () => {
  assert.equal(cookiesHaveLiveSession([{ name: 'jacky_session_at', value: '' }]), false)
  assert.equal(cookiesHaveLiveSession([{ name: 'jacky_session_rt', value: '' }]), false)
  assert.equal(
    cookiesHaveLiveSession([
      { name: 'jacky_session_at', value: '' },
      { name: 'jacky_session_rt', value: '' }
    ]),
    false
  )
})
...
test('cookiesHavePrivySession does NOT treat jacky gateway cookies as a portal session', () => {
  // The whole point of Q7: a gateway session cookie is NOT a portal sign-in.
  assert.equal(cookiesHavePrivySession([{ name: 'jacky_session_at', value: 'x' }]), false)
  assert.equal(cookiesHavePrivySession([{ name: '__Host-jacky_session_rt', value: 'x' }]), false)
})

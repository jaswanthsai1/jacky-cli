 ...
 * null to keep it enabled. `JACKY_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options: { env?: NodeJS.ProcessEnv; platform?: NodeJS.Platform } = {}) {
  ...
  const override = String(env.JACKY_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()

  if (GPU_OVERRIDE_ON.has(override)) {
    return 'override (JACKY_DESKTOP_DISABLE_GPU)'
  }

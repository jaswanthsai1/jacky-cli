 ...
 *     `X-Jacky-Session-Token` header; WS uses `?token=`.
...
// jacky_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - jacky_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - jacky_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / jacky #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-jacky_session_at', '__Secure-jacky_session_at', 'jacky_session_at']
const RT_COOKIE_VARIANTS = ['__Host-jacky_session_rt', '__Secure-jacky_session_rt', 'jacky_session_rt']

// The Nous portal (NAS) does NOT use Jacky gateway session cookies — it is a
...
// Jacky Cloud connection (cloud-auto-discovery Q3/Q6): it carries a
...
function pathWithGlobalRemoteProfile(path, profile, opts: any = {}) {
  ...
  try {
    parsed = new URL(rawPath, 'http://jacky.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a jacky session ACCESS-token cookie
 ...
 * the Jacky gateway session cookies, so cloud sign-in / discovery liveness

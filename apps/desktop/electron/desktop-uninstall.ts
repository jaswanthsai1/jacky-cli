 ...
 * the three user-facing uninstall modes to the `jacky uninstall` CLI flags,
...
/**
 * Map an uninstall mode to the `python -m jacky_cli.uninstall` argv (after the
 * python executable). Uses the dedicated lightweight module entrypoint (not
 * `jacky_cli.main`) so it can run under a system Python OUTSIDE the venv that
...
function uninstallArgsForMode(mode) {
  ...
  return ['-m', 'jacky_cli.uninstall', '--mode', mode]
}
 ...
 *
 *   macOS:   …/Jacky.app/Contents/MacOS/Jacky  → …/Jacky.app
 *   Windows: …\Jacky\Jacky.exe                 → …\Jacky  (install dir)
...
function resolveRemovableAppPath(execPath, platform, env: any = {}) {
  ...
  if (platform === 'darwin') {
    // …/Jacky.app/Contents/MacOS/Jacky → strip 3 segments to the .app
    const macOsDir = p.dirname(exe) // …/Contents/MacOS
    const contents = p.dirname(macOsDir) // …/Contents
    const appBundle = p.dirname(contents) // …/Jacky.app
  ...
  if (platform === 'win32') {
    // NSIS per-user installs Jacky.exe directly in the install dir.
    const dir = p.dirname(exe)

    if (/[\\/]Jacky$/i.test(dir) || /[\\/]jacky-desktop$/i.test(dir)) {
      return dir
    }
  ...
  // Unpacked electron-builder tree: …/linux-unpacked/jacky
  const dir = p.dirname(exe)
 ...
 * being deleted); `pythonPath` is prepended to PYTHONPATH so `import jacky_cli`
 ...
 */
function buildPosixCleanupScript({ desktopPid, pythonExe, pythonPath, agentRoot, uninstallArgs, appPath, jackyHome }) {
 ...
 * modes + `pythonPath`=agentRoot so `import jacky_cli` resolves from source
...
function buildWindowsCleanupScript({
  ...
  // a problem, but Jacky install paths don't use them.
  const q = s => `"${String(s).replace(/"/g, '')}"`

  const lines = [
    '@echo off',
    'setlocal enableextensions',
    `set "JACKY_HOME=${String(jackyHome).replace(/"/g, '')}"`,
    `set "PID=${pid}"`
  ]

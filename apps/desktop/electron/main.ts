...
import { buildDesktopBackendEnv, normalizeJackyHomeRoot } from './backend-env'
import { canImportJackyCli, verifyJackyCli } from './backend-probes'
...
const USER_DATA_OVERRIDE = process.env.JACKY_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.JACKY_DESKTOP_DEV_SERVER
const IS_PACKAGED = app.isPackaged || Boolean(process.env.JACKY_DESKTOP_IS_PACKAGED)
...
// switches only apply pre-launch. Override with JACKY_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[jacky] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}
...
if (IS_WSL && !REMOTE_DISPLAY_REASON && fs.existsSync('/dev/dxg')) {
  ...
  console.log('[jacky] WSL GPU passthrough (/dev/dxg) detected; enabling GPU acceleration')
}

ipcMain.handle('jacky:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[jacky] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
      ...
      console.warn(`[jacky] install-stamp.json found at ${p} , but parsing failed with ${e}`)
...
if (INSTALL_STAMP) {
  console.log(
    `[jacky] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[jacky] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// JACKY_HOME — the user-facing root for everything Jacky-related. Mirrors
// scripts/install.ps1's $JackyHome and scripts/install.sh's $JACKY_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\jacky (matches install.ps1)
//   macOS / Linux: ~/.jacky (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.jacky directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\jacky yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// JACKY_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// JACKY_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.jacky / %LOCALAPPDATA%\jacky.
function resolveJackyHome() {
  if (process.env.JACKY_HOME) {
    return normalizeJackyHomeRoot(process.env.JACKY_HOME)
  }

  if (USER_DATA_OVERRIDE) {
    return path.join(path.resolve(USER_DATA_OVERRIDE), 'jacky-home')
  }

  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a JACKY_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\jacky and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('JACKY_HOME')

    if (fromRegistry) {
      return normalizeJackyHomeRoot(fromRegistry)
    }
  }

  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'jacky')
    const legacy = path.join(app.getPath('home'), '.jacky')

    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.jacky setup (no LOCALAPPDATA install yet) so users don't lose state.
  ...
  return path.join(app.getPath('home'), '.jacky')
}

const JACKY_HOME = resolveJackyHome()

function jackyManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_jacky_node_dirs() in
  // jacky_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(JACKY_HOME, 'node')
...
function pathWithJackyManagedNode(...entries) {
  return [...jackyManagedNodePathEntries(), ...entries, process.env.PATH].filter(Boolean).join(path.delimiter)
}

// ACTIVE_JACKY_ROOT — the canonical mutable Jacky install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_JACKY_ROOT = path.join(JACKY_HOME, 'jacky-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_JACKY_ROOT, 'venv')
...
// boots, going straight to `resolveJackyBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_JACKY_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_JACKY_ROOT, '.jacky-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
const DESKTOP_WINDOW_STATE_PATH = path.join(app.getPath('userData'), 'window-state.json')
// active-profile.json records which Jacky profile the desktop launches its
// local backend as. When set, startJacky() passes `jacky --profile <name>
// dashboard …`, which deterministically pins JACKY_HOME (see
// _apply_profile_override in jacky_cli/main.py) and bypasses the sticky
// ~/.jacky/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors jacky_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// jackyDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under JACKY_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by jacky_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(JACKY_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (jacky_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.JACKY_DESKTOP_BOOT_FAKE === '1'

const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.JACKY_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
const APP_NAME = process.env.JACKY_DESKTOP_APP_NAME || 'Jacky'
...
// 'jacky:native-theme' ('dark' | 'light' | 'system'); we pin
...
// electron-builder derives from the build `appId` (com.nousresearch.jacky) —
// keep this string in sync with package.json `build.appId`. macOS/Linux don't
// need this, so gate it on Windows. (Fixes: desktop approval/turn notifications
// never firing on Windows.)
if (IS_WINDOWS) {
  app.setAppUserModelId('com.nousresearch.jacky')
}

// Seed the native About panel with the live Jacky version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `jacky update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveJackyVersion(),
...
const MEDIA_PROTOCOL = 'jacky-media'

// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(jacky-media://…)`.
...
let mainWindow = null
let jackyProcess = null
let connectionPromise = null
// True while connection-config:apply soft-rehomes the primary — suppresses the
// backend-exit toast so an intentional kill doesn't look like a crash.
let softRehomeInProgress = false
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by jackyProcess +
// connectionPromise + startJacky(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.JACKY_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.JACKY_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startJacky() calls (e.g. the renderer's
// ensureGatewayOpen retrying after the WS won't open) return the same error
// instead of re-running install.ps1 in a hot loop. Cleared explicitly by
// the renderer's "Reload and retry" path or by quitting the app.
let bootstrapFailure = null
// Latched non-bootstrap backend spawn failure — stops getConnection() from
// respawning jacky serve backend children in a tight loop while boot is broken.
...
let bootProgressState = {
  ...
  message: 'Waiting to start Jacky backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  if (jackyLog.length > 300) {
    jackyLog.splice(0, jackyLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-jacky-wsl-windows-fonts.conf')
    let existing = ''
...
function broadcastBootProgress() {
  ...
  webContents.send('jacky:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the jacky:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  webContents.send('jacky:bootstrap:event', ev)
}
...
// The Tauri updater writes JACKY_HOME/.jacky-update-in-progress for the whole
...
// (`force_kill_other_jacky`, taskkill /IM jacky.exe) kills it, the launch
...
async function waitForUpdateToFinish() {
  let marker = readLiveUpdateMarker(JACKY_HOME)
  ...
  while (marker && Date.now() < deadline) {
    await advanceBootProgress(
      'backend.update-wait',
      'An update is finishing — Jacky will start automatically when it completes…',
      12
    )
    await new Promise(r => setTimeout(r, UPDATE_WAIT_POLL_MS))
    marker = readLiveUpdateMarker(JACKY_HOME)
  }
...
function findOnPath(command) {
  ...
  // shell-script shim named `jacky` — must not shadow `jacky.cmd`/`jacky.exe`.
...
function unwrapWindowsVenvJackyCommand(command, backendArgs) {
  ...
  if (!/^jacky(?:\.exe)?$/i.test(path.basename(resolved))) {
    return null
  }
  ...
  // died mid-`pip install` still has python.exe + jacky.exe on disk, but the
  ...
  if (
    ...
    rememberLog(
      `Ignoring venv Jacky at ${python}: runtime import probe failed (broken/partial venv); falling through to bootstrap.`
    )
  ...
  return {
    label: `existing Jacky Python at ${python}`,
    command: python,
    args: ['-m', 'jacky_cli.main', ...backendArgs],
    bootstrap: false,
    env: buildDesktopBackendEnv({
      jackyHome: JACKY_HOME,
...
// spawns `jacky serve`; runtimes older than serve only have `dashboard`. We
...
// (covers a bare `jacky` resolved from PATH with no known source root). Result
...
function backendSupportsServe(backend) {
  ...
  if (backend.root) {
    try {
      const src = fs.readFileSync(path.join(backend.root, 'jacky_cli', 'subcommands', 'dashboard.py'), 'utf8')
  ...
  if (supported === null) {
    try {
      ...
      execFileSync(backend.command, [...prefix, 'serve', '--help'], {
        ...
        env: { ...process.env, JACKY_HOME, ...(backend.env || {}) },
        timeout: 15000,
        stdio: 'ignore',
        windowsHide: true
      })
...
function isJackySourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'jacky_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.JACKY_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Jacky deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Jacky' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\jacky\git\... — checked
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'jacky', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'jacky', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// normal JACKY_DASHBOARD_READY stdout line and no ready-file side channel is
// needed.
...
// install only has PortableGit under %LOCALAPPDATA%\jacky\git (never on
...
function resolveGitBinary() {
  ...
  if (localAppData) {
    candidates.push(path.join(localAppData, 'jacky', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'jacky', 'git', 'bin', 'git.exe'))
  }
...
function recentJackyLog() {
  return jackyLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's jacky root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_JACKY_ROOT.
// JACKY_DESKTOP_JACKY_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.JACKY_DESKTOP_JACKY_ROOT && path.resolve(process.env.JACKY_DESKTOP_JACKY_ROOT),
    !IS_PACKAGED && isJackySourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isJackySourceRoot(ACTIVE_JACKY_ROOT) ? ACTIVE_JACKY_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_JACKY_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('jacky:updates:progress', merged)
  }
}
...
// JACKY_HOME/jacky-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_jacky_home). That binary owns
// ALL repo mutation — running `jacky update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'jacky-setup.exe' : 'jacky-setup'
  const candidate = path.join(JACKY_HOME, name)
...
// Path to the venv shim whose lock decides whether `jacky update` can write
// fresh entry points. On Windows this is the file the running backend
// `jacky.exe` holds open; on POSIX it's never mandatory-locked.
function venvJackyShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'jacky.exe')
    : path.join(updateRoot, 'venv', 'bin', 'jacky')
}
...
// only signals the direct child, so on Windows a backend `jacky.exe` that
// spawned its own grandchildren (a `jacky` REPL, a pty terminal session, the
...
// `jackyProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `jacky.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a jacky
// REPL, a pty terminal, the gateway) keep `jacky.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  if (jackyProcess && Number.isInteger(jackyProcess.pid)) {
    pids.push(jackyProcess.pid)
  }
  ...
  if (jackyProcess && !jackyProcess.killed) {
    try {
      jackyProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvJackyShimPath(updateRoot)
  ...
  while (Date.now() < deadlineMs) {
    ...
    if (jackyProcess && Number.isInteger(jackyProcess.pid)) {
      stragglers.push(jackyProcess.pid)
    }
...
// `jacky update`). Instead we spawn the staged Jacky-Setup binary with
// --update and quit, so it can run `jacky update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri jacky-setup. Unlike Windows
      ...
      // whole update itself: `jacky update` (backend) + `jacky desktop
    ...
    if (!updater) {
      ...
      // `jacky desktop`, never the Tauri installer that self-copies
      // jacky-setup.exe into JACKY_HOME). They DO have a working `jacky`
      ...
      // checkout they're on — bare `jacky update` defaults to main and would
      ...
      try {
        ...
        if (head.code === 0 && current && current !== 'HEAD') {
          ...
          if (branch !== 'main') {
            command = `jacky update --branch ${branch}`
          }
        }
      } catch {
        // Best-effort: fall back to bare `jacky update` if branch detection fails.
      }

      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })

      return { ok: true, manual: true, command, jackyRoot: updateRoot }
    }

    emitUpdateProgress({
      stage: 'restart',
      message:
        'Updating Jacky — this window will close and the updater will open. Don’t reopen Jacky yourself; it restarts automatically when the update finishes.',
      percent: 100
    })
    ...
    // jacky.exe (held by the backend child / its grandchildren) and the update
    ...
    if (!lock.unlocked) {
      ...
      // terminal running jacky, an unkillable child). Handing off anyway
      ...
      const message =
        'Update aborted: another process is holding the Jacky install open ' +
        '(a second Jacky window or a terminal running jacky?). Close it and retry.'
    ...
    // `jacky update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: JACKY_HOME,
      env: {
        ...process.env,
        JACKY_HOME,
        PATH: pathWithJackyManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
    ...
    if (Number.isInteger(child.pid)) {
      writeUpdateMarker(JACKY_HOME, child.pid)
    }
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvJacky = path.join(venvBin, IS_WINDOWS ? 'jacky.exe' : 'jacky')
  ...
  // not just the `jacky.exe` console-script shim. That shim is generated at the
  ...
  const haveRealInstall =
    fileExists(venvPython) || fileExists(venvJacky) || fileExists(path.join(updateRoot, '.jacky-bootstrap-complete'))
...
// Resolve the jacky CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `jacky` on PATH.
function resolveJackyCliBinary(updateRoot) {
  const venvJacky = path.join(updateRoot, 'venv', 'bin', 'jacky')

  if (fileExists(venvJacky)) {
    return venvJacky
  }

  return findOnPath('jacky') || null
}
...
// macOS/Linux in-app update: backend (`jacky update`) + OS-aware GUI rebuild
// (`jacky desktop --build-only`), then atomically swap the running .app bundle
...
async function applyUpdatesPosixInApp(opts: any) {
  const updateRoot = resolveUpdateRoot()
  const jacky = resolveJackyCliBinary(updateRoot)

  if (!jacky) {
    emitUpdateProgress({ stage: 'manual', message: 'jacky update', percent: null })

    return { ok: true, manual: true, command: 'jacky update', jackyRoot: updateRoot }
  }

  // Put the Jacky-managed Node and the venv on PATH so `jacky desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\jacky\node, not node\bin.
  const env: Record<string, string> = {
    JACKY_HOME,
    PATH: pathWithJackyManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `jacky update` reaps stale `jacky serve` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads JACKY_DESKTOP_CHILD_PID and excludes
  ...
  if (jackyProcess && Number.isInteger(jackyProcess.pid)) {
    desktopChildPids.push(jackyProcess.pid)
  }
  ...
  if (desktopChildPids.length) {
    env.JACKY_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Jacky (git + dependencies)…', percent: 10 })

  const updated = (await runStreamedUpdate(jacky, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })) as any
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'jacky update failed.', error: updated.error || 'update-failed' })

    return { ok: false, error: 'jacky update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(jacky, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })

  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Jacky to retry.',
  ...
  // Linux in-app update terminal state (#45205). `jacky desktop --build-only`
  ...
  if (!IS_MAC) {
    ...
    if (outcome === 'relaunch') {
      emitUpdateProgress({ stage: 'restart', message: 'Restarting Jacky…', percent: 100 })
      ...
      const scriptPath = path.join(app.getPath('temp'), `jacky-desktop-update-${Date.now()}.sh`)
      ...
      try {
        ...
        return {
          ...
          guiUpdated: false,
          manualRestart: true,
          message: 'Backend updated. Quit and reopen Jacky to load the new version.'
        }
      }
    }
    ...
    if (outcome === 'guiSkew') {
      emitUpdateProgress({
        ...
        message:
          ...
          'Update or reinstall the Jacky desktop app to match.',
        percent: 100
      })
      rememberLog(
    ...
    return {
      ...
      message:
        ...
        '(sandbox helper needs root). Quit and reopen Jacky to finish.'
    }
  }

  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Jacky.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Jacky.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Jacky to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.jacky-update-new"; then
    rm -rf "$DST.jacky-update-old" 2>/dev/null || true
    mv "$DST" "$DST.jacky-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.jacky-update-new" "$DST"
    rm -rf "$DST.jacky-update-old" 2>/dev/null || true
  fi
fi
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Jacky to load the new version.',
      percent: 100
    })
...
// Marker-independent: is the canonical install at ACTIVE_JACKY_ROOT actually
...
function isActiveRuntimeUsable() {
  ...
  return (
    isJackySourceRoot(ACTIVE_JACKY_ROOT) &&
    fileExists(venvPython) &&
    canImportJackyCli(venvPython, {
      env: {
        PYTHONPATH: [ACTIVE_JACKY_ROOT, process.env.PYTHONPATH].filter(Boolean).join(path.delimiter)
      }
    })
  )
}
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `jacky
...
function resolveWebDist() {
  const override = process.env.JACKY_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set JACKY_DESKTOP_WEB_DIST.`
    )
  }
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: jacky desktop --force-build`
  )
...
function resolveJackyCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Jacky.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveJackyCwd(), sanitized: Boolean(trimmed) }
  }
...
// createActiveBackend — build a backend pointing at ACTIVE_JACKY_ROOT, the
...
function createActiveBackend(backendArgs) {
  ...
  return {
    kind: 'python',
    label: `Jacky at ${ACTIVE_JACKY_ROOT}`,
    command,
    args: ['-m', 'jacky_cli.main', ...backendArgs],
    env: buildDesktopBackendEnv({
      jackyHome: JACKY_HOME,
      pythonPathEntries: [ACTIVE_JACKY_ROOT, ...getVenvSitePackagesEntries(VENV_ROOT)],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_JACKY_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveJackyBackend(backendArgs) {
  // 1. Explicit override -- JACKY_DESKTOP_JACKY_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.JACKY_DESKTOP_JACKY_ROOT && path.resolve(process.env.JACKY_DESKTOP_JACKY_ROOT)

  if (overrideRoot && isJackySourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Jacky source at ${overrideRoot}`, backendArgs)
  ...
  //    installed `jacky` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isJackySourceRoot.)
  if (!IS_PACKAGED && isJackySourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Jacky source at ${SOURCE_REPO_ROOT}`, backendArgs)
  ...
  // 3. Bootstrap-complete ACTIVE_JACKY_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\jacky\jacky-agent (Windows) or ~/.jacky/jacky-agent.
  ...
  //    to spawning jacky. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `jacky update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(backendArgs)
  }

  // 4. Existing `jacky` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    JACKY_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.JACKY_DESKTOP_IGNORE_EXISTING !== '1') {
    let jackyCommand = null
    const jackyOverride = process.env.JACKY_DESKTOP_JACKY

    if (jackyOverride) {
      const resolvedOverride = findOnPath(jackyOverride)

      if (resolvedOverride) {
        jackyCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(jackyOverride, { isWsl: IS_WSL })) {
        jackyCommand = jackyOverride
      } else {
        rememberLog(`Ignoring Windows Jacky override under WSL: ${jackyOverride}`)
      }
    } else {
      jackyCommand = findOnPath('jacky')
    }

    if (jackyCommand) {
      if (looksLikeDesktopAppBinary(jackyCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Jacky CLI: ${jackyCommand}`)
        jackyCommand = null
      }
    }

    if (jackyCommand) {
      const unwrapped = unwrapWindowsVenvJackyCommand(jackyCommand, backendArgs)

      if (unwrapped) {
        return unwrapped
      }

      // Smoke-test the candidate before trusting it. A `jacky` shim
      ...
      const shellForProbe = isCommandScript(jackyCommand)

      if (verifyJackyCli(jackyCommand, { shell: shellForProbe })) {
        return (
          unwrapWindowsVenvJackyCommand(jackyCommand, backendArgs) || {
            label: `existing Jacky CLI at ${jackyCommand}`,
            command: jackyCommand,
            args: backendArgs,
            bootstrap: false,
            env: {},
            kind: 'command',
      ...
      rememberLog(
        `Ignoring existing Jacky CLI at ${jackyCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed jacky_cli module via system Python.
  ...
  if (python) {
    ...
    // having jacky_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\jacky\jacky-agent\venv.
    if (canImportJackyCli(python)) {
      return {
        kind: 'python',
        label: `installed jacky_cli module via ${python}`,
        command: python,
        args: ['-m', 'jacky_cli.main', ...backendArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: jacky_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveJackyBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Jacky Agent not installed yet; bootstrap required',
    command: null,
    args: backendArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveJackyBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Jacky install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError: Error & { isBootstrapFailure?: boolean; bootstrapHandedOff?: boolean } = new Error(
        'Jacky recovery was handed off to Jacky Setup. The desktop will restart when recovery completes.'
      )
    ...
    const bootstrapResult = await runBootstrap({
      ...
      jackyHome: JACKY_HOME,
      logRoot: path.join(JACKY_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Jacky install was cancelled.') as any
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Jacky bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(JACKY_HOME, 'logs', 'desktop.log')} for the full transcript.`
      ) as any

      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startJacky() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // jacky:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveJackyBackend(backend.args))
  }
  ...
  if (!isJackySourceRoot(ACTIVE_JACKY_ROOT)) {
    throw new Error(
      `Jacky install at ${ACTIVE_JACKY_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Jacky' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\jacky\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `jacky` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Jacky on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isJackySourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Jacky venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = getVenvPython(VENV_ROOT)
  backend.label = `Jacky at ${ACTIVE_JACKY_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Jacky runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Jacky backend URL protocol: ${parsed.protocol}`))
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Jacky backend.'
              )
            )
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to Jacky backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options: any = {}) {
  ...
  // NO ``X-Jacky-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('jacky:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveJackyCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  webContents.send('jacky:preview-file-changed', payload)
}
...
async function waitForJacky(baseUrl, token) {
  ...
  throw new Error(`Jacky backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  webContents.send('jacky:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  webContents.send('jacky:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Jacky...".
function sendPowerResume() {
  ...
  webContents.send('jacky:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  webContents.send('jacky:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen?: boolean) {
  ...
  webContents.send('jacky:window-state-changed', state)
}
...
function setAndPersistZoomLevel(window, zoomLevel) {
  ...
  window.webContents.send('jacky:zoom:changed', { level: next, percent: zoomLevelToPercent(next) })
  window.webContents
    .executeJavaScript(
...
//
// Hosted Jacky gateways gate the dashboard behind an OAuth provider (e.g.
// Nous Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``jacky_session_at``),
...
//     alongside the ~15-min access token (Portal NAS #293 / jacky #37247).
//     Both are set as HttpOnly cookies (``jacky_session_at`` ~15 min,
//     ``jacky_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:jacky-remote-oauth'
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: silent ? 'Connecting to Jacky Cloud agent…' : 'Sign in to Jacky gateway',
...
function fetchJsonViaOauthSession(url, options: any = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Jacky backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// --- Jacky Cloud discovery + silent per-agent sign-in (cloud-auto-discovery
// Phase 3) ---------------------------------------------------------------
//
...
// portal session cookie living in `persist:jacky-remote-oauth`:
...
// convention (jacky_cli/auth.py DEFAULT_NOUS_PORTAL_URL + the same env names)
// so a single override flips every Jacky surface to the same portal.
const DEFAULT_NOUS_PORTAL_URL = 'https://portal.nousresearch.com'

function resolvePortalBaseUrl() {
  const raw = process.env.JACKY_PORTAL_BASE_URL || process.env.NOUS_PORTAL_BASE_URL || DEFAULT_NOUS_PORTAL_URL
...
// authenticates via PRIVY, not the Jacky gateway session cookies, so this
// checks for the `privy-token` cookie on the portal host (NOT
// hasLiveOauthSession, which looks for jacky_session_at/rt that the portal
...
function openPortalLoginWindow() {
  ...
  return new Promise((resolve, reject) => {
    if (!app.isReady()) {
      reject(new Error('Desktop is not ready to start a Jacky Cloud sign-in.'))
...
// Discover the hosted (Jacky Cloud) agents the signed-in user can see. Calls
...
async function discoverCloudAgents(org?: string) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error(
      'You are not signed in to Jacky Cloud. Open Settings → Gateway, choose Jacky Cloud, and sign in.'
    ) as any
  ...
  try {
    ...
    if (error && error.statusCode === 401) {
      const err = new Error('Your Jacky Cloud session has expired. Open Settings → Gateway and sign in again.') as any
      err.needsCloudLogin = true
      err.cause = error
      throw err
    }
...
async function cloudAgentSilentSignIn(dashboardUrl) {
  ...
  if (!(await hasLivePortalSession())) {
    const err = new Error('Your Jacky Cloud session has expired. Sign in to Jacky Cloud again.') as any
    err.needsCloudLogin = true
    throw err
  }
...
function sanitizeConnectionProfiles(raw: Record<string, any>) {
  ...
  for (const [name, entry] of Object.entries(raw)) {
    ...
    // Preserve the Jacky Cloud org tag on cloud-mode entries so Settings can
...
// a valid stored value (pins the root JACKY_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.JACKY_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.JACKY_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  // The env override forces a plain remote connection. Otherwise reflect the
  // saved mode, preserving 'cloud' (a Jacky Cloud connection — Q6) so the UI
  ...
  return {
    ...
    remoteUrl,
    // The persisted Jacky Cloud org (slug/id) for a cloud connection, or '' for
    ...
    // scope is never overridden by JACKY_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
// `org` (optional) is the Jacky Cloud org slug/id the instance was discovered
...
function coerceDesktopConnectionConfig(input: any = {}, existing = readDesktopConnectionConfig(), options: any = {}) {
  ...
  // describe a discovered Jacky Cloud instance, NOT a user-owned remote gateway,
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (jacky #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Jacky gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Jacky gateway is selected, but no session token is saved. ' +
...
//   2. env override (JACKY_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.JACKY_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.JACKY_DESKTOP_REMOTE_TOKEN

  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'JACKY_DESKTOP_REMOTE_URL is set but JACKY_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Jacky backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.JACKY_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Jacky
...
async function testDesktopConnectionConfig(input: any = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startJacky())
  ...
  // connect to Jacky gateway". Mirror the renderer's connect here so the test
...
function resetJackyConnection({ soft = false } = {}) {
  connectionPromise = null
  backendStartFailure = null

  stopBackendChild(jackyProcess)
...
// startJacky() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait({ soft = false } = {}) {
  // Capture the reference before resetJackyConnection() nulls jackyProcess.
  const dying = jackyProcess && !jackyProcess.killed ? jackyProcess : null
...
function sendConnectionApplied() {
  ...
  webContents.send('jacky:connection:applied')
}
...
// profile to startJacky() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startJacky() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForJacky(remote.baseUrl, remote.token)

    return {
      ...remote,
      profile,
      logs: jackyLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited JACKY_HOME env (see _apply_profile_override
  // step 3 in jacky_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const backendArgs = ['--profile', profile, 'serve', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveJackyBackend(backendArgs))
  // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
  backend.args = getBackendArgsForRuntime(backend)
  const jackyCwd = resolveJackyCwd()
  const webDist = resolveWebDist()
  const readyFile = backend.readyFile ? makeDashboardReadyFile() : null

  rememberLog(`Starting Jacky backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: jackyCwd,
        JACKY_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        JACKY_DESKTOP: '1',
        JACKY_WEB_DIST: webDist,
        ...(readyFile ? { JACKY_DESKTOP_READY_FILE: readyFile } : {})
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Jacky backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Jacky backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)

    if (!ready) {
      rejectStart?.(
        new Error(`Jacky backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForJacky(baseUrl, token), startFailed])
  ready = true

  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Jacky backend for profile "${profile}"`,
    rememberLog
  })
...
// backend whose ensure_jacky_home() recreates the deleted profile directory.
...
async function startJacky() {
  // Latched-failure short-circuit: once bootstrap has failed in this
  // process, every subsequent startJacky() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Jacky backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())

    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Jacky backend at ${remote.baseUrl}`, 24)
      await waitForJacky(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Jacky backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.jacky/active_profile file) and
    // resolves JACKY_HOME the same way `jacky -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Jacky runtime', 28)
    const backend = await ensureRuntime(resolveJackyBackend(backendArgs))
    // Route old runtimes (no `serve`) through the legacy `dashboard --no-open`.
    backend.args = getBackendArgsForRuntime(backend)
    const jackyCwd = resolveJackyCwd()
    ...
    await advanceBootProgress('backend.spawn', `Starting Jacky backend via ${backend.label}`, 84)
    rememberLog(`Starting Jacky backend via ${backend.label}`)
    ...
    jackyProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin JACKY_HOME for the child so Python's get_jacky_home()
          // resolves to the SAME location our resolveJackyHome() picked. Without
          // this pin, Python falls back to ~/.jacky on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\jacky, which differs from C:\Users\<u>\.jacky.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets JACKY_HOME via setx; the desktop
    ...
    jackyProcess.stdout.on('data', rememberLog)
    jackyProcess.stderr.on('data', rememberLog)
    ...
    jackyProcess.once('error', error => {
      rememberLog(`Jacky backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Jacky backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    jackyProcess.once('exit', (code, signal) => {
      rememberLog(`Jacky backend exited (${signal || code})`)
      jackyProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })

      if (!backendReady) {
        const message = `Jacky backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Jacky backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentJackyLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Jacky backend to launch', 86)

    // Discover the ephemeral port the child bound to
    const port = await Promise.race([
      waitForDashboardPortAnnouncement(jackyProcess, { readyFile }),
      backendStartFailed
    ])
    ...
    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Jacky backend to become ready', 90)
    await Promise.race([waitForJacky(baseUrl, token), backendStartFailed])
    backendReady = true
    backendStartFailure = null

    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null jackyProcess when the child dies.
      childAlive: () => jackyProcess !== null && jackyProcess.exitCode === null && !jackyProcess.killed,
      rememberLog
    })

    updateBootProgress({
      phase: 'backend.ready',
      message: 'Jacky backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// here so it can leave the app's bounds and stay visible while Jacky is
...
// pushes pet state over IPC (jacky:pet-overlay:state); the overlay just renders
// it. Control flows back (pop-in, composer submit) via jacky:pet-overlay:control.
...
function spawnPetOverlayWindow(bounds) {
  const win = new BrowserWindow({
    ...
    // instead, leaving the main Jacky app as the Dock/cmd-tab anchor.
    skipTaskbar: !IS_MAC,
    hasShadow: false,
    alwaysOnTop: true,
    ...
    // anchor and the Jacky icon drops out of cmd/alt-tab — especially when the
    // main window is minimized. We flip this on only while the composer needs
    // the keyboard (see jacky:pet-overlay:set-focusable).
    focusable: false,
    show: false,
  ...
  try {
    ...
    // which briefly hides the Dock/cmd-tab presence. Keep Jacky in the normal
  ...
  win.on('closed', () => {
    ...
    if (mainWindow && !mainWindow.isDestroyed()) {
      mainWindow.webContents.send('jacky:pet-overlay:control', { type: 'pop-in' })
    }
  })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startJacky().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('jacky:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Jacky…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('jacky:connection:revalidate', async () => {
  if (!connectionPromise) {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetJackyConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Jacky backend failed liveness probe; dropping stale connection.')
    resetJackyConnection()

    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('jacky:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)

  return { ok: true }
})
ipcMain.handle('jacky:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('jacky:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('jacky:window:openNewSession', async () => {
...
ipcMain.handle('jacky:zoom:get', event => {
...
ipcMain.on('jacky:zoom:set-percent', (event, percent) => {
  const window = BrowserWindow.fromWebContents(event.sender)
...
ipcMain.handle('jacky:pet-overlay:open', async (_event, request) => {
...
ipcMain.handle('jacky:pet-overlay:close', async () => {
  closePetOverlay()
...
ipcMain.on('jacky:pet-overlay:set-bounds', (_event, bounds) => {
...
ipcMain.on('jacky:pet-overlay:ignore-mouse', (_event, ignore) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
...
ipcMain.on('jacky:pet-overlay:set-focusable', (_event, focusable) => {
  if (!petOverlayWindow || petOverlayWindow.isDestroyed()) {
    return
  }
...
ipcMain.on('jacky:pet-overlay:state', (_event, payload) => {
  if (petOverlayWindow && !petOverlayWindow.isDestroyed()) {
    petOverlayWindow.webContents.send('jacky:pet-overlay:state', payload)
  }
})
// Overlay → main renderer: control messages (pop back in, composer submit).
ipcMain.on('jacky:pet-overlay:control', (_event, payload) => {
  if (!mainWindow || mainWindow.isDestroyed()) {
    return
  }
  ...
  mainWindow.webContents.send('jacky:pet-overlay:control', payload)
})
ipcMain.handle('jacky:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startJacky() call restarts the
...
ipcMain.handle('jacky:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startJacky() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('jacky:bootstrap:cancel', async () => {
...
ipcMain.handle('jacky:boot-progress:get', async () => bootProgressState)
ipcMain.handle('jacky:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('jacky:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('jacky:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('jacky:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('jacky:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('jacky:connection-config:oauth-logout', async (_event, rawUrl) => {
...
// --- Jacky Cloud (cloud-auto-discovery Phase 3) ---
...
ipcMain.handle('jacky:cloud:agent-sign-in', async (_event, dashboardUrl) => {
...
ipcMain.handle('jacky:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('jacky:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
  ...
  if (key && key !== primaryProfileKey()) {
    ...
    // wipes session lists (skeletons) and re-dials on jacky:connection:applied.
...
ipcMain.handle('jacky:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('jacky:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new JACKY_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('jacky:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('jacky:requestMicrophoneAccess', async () => {
...
ipcMain.handle('jacky:api', async (_event, request) => {
  ...
  // backend calls ensure_jacky_home() which recreates the profile directory,
...
ipcMain.handle('jacky:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('jacky:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('jacky:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
...
ipcMain.handle('jacky:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('jacky:readFileText', async (_event, filePath) => {
...
ipcMain.handle('jacky:selectPaths', async (_event, options: any = {}) => {
...
ipcMain.handle('jacky:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))

  return true
})

ipcMain.handle('jacky:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('jacky:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('jacky:saveClipboardImage', async () => {
...
ipcMain.handle('jacky:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('jacky:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('jacky:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('jacky:titlebar-theme', (_event, payload) => {
...
ipcMain.on('jacky:native-theme', (_event, mode) => {
...
ipcMain.on('jacky:translucency', (_event, payload) => {
...
ipcMain.handle('jacky:openExternal', (_event, url) => {
  if (!openExternalUrl(url)) {
    throw new Error('Invalid external URL')
  }
})

ipcMain.handle('jacky:openPreviewInBrowser', async (_event, url) => {
...
// it via writeDefaultProjectDir so resolveJackyCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('jacky:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveJackyCwd()
}))

ipcMain.handle('jacky:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('jacky:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('jacky:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('jacky:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('jacky:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('jacky:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: jackyLog.slice(-200) }))
...
function terminalShellCommand() {
  // JACKY_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.JACKY_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Jacky' TUI into
  ...
  env.TERM_PROGRAM = 'Jacky'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a jacky/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from JACKY_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.JACKY_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `jacky:terminal:${id}:${suffix}`
}
...
ipcMain.handle('jacky:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('jacky:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

// Reveal a path in the OS file manager (Finder / Explorer / Files).
ipcMain.handle('jacky:fs:reveal', async (_event, targetPath) => {
...
ipcMain.handle('jacky:fs:rename', async (_event, targetPath, newName) => {
...
ipcMain.handle('jacky:fs:writeText', async (_event, filePath, content) => {
...
ipcMain.handle('jacky:fs:trash', async (_event, targetPath) => {
  const target = String(targetPath || '').trim()
...
ipcMain.handle('jacky:git:worktreeList', async (_event, repoPath) => listWorktrees(repoPath, resolveGitBinary()))

ipcMain.handle('jacky:git:worktreeAdd', async (_event, repoPath, options) =>
  addWorktree(repoPath, options || {}, resolveGitBinary())
)

ipcMain.handle('jacky:git:worktreeRemove', async (_event, repoPath, worktreePath, options) =>
  removeWorktree(repoPath, worktreePath, options || {}, resolveGitBinary())
)

ipcMain.handle('jacky:git:branchSwitch', async (_event, repoPath, branch) =>
  switchBranch(repoPath, branch, resolveGitBinary())
)

ipcMain.handle('jacky:git:branchList', async (_event, repoPath) => listBranches(repoPath, resolveGitBinary()))
...
ipcMain.handle('jacky:git:repoStatus', async (_event, repoPath) => repoStatus(repoPath, resolveGitBinary()))
...
ipcMain.handle('jacky:git:review:list', async (_event, repoPath, scope, baseRef) =>
  reviewList(repoPath, scope, baseRef, resolveGitBinary())
)
ipcMain.handle('jacky:git:review:diff', async (_event, repoPath, filePath, scope, baseRef, staged) =>
...
ipcMain.handle('jacky:git:fileDiff', async (_event, repoPath, filePath) =>
  fileDiffVsHead(repoPath, filePath, resolveGitBinary())
)
ipcMain.handle('jacky:git:review:stage', async (_event, repoPath, filePath) =>
  reviewStage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('jacky:git:review:unstage', async (_event, repoPath, filePath) =>
  reviewUnstage(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('jacky:git:review:revert', async (_event, repoPath, filePath) =>
  reviewRevert(repoPath, filePath ?? null, resolveGitBinary())
)
ipcMain.handle('jacky:git:review:revParse', async (_event, repoPath, ref) =>
  reviewRevParse(repoPath, ref, resolveGitBinary())
)
ipcMain.handle('jacky:git:review:commit', async (_event, repoPath, message, push) =>
  reviewCommit(repoPath, message, Boolean(push), resolveGitBinary())
)
ipcMain.handle('jacky:git:review:commitContext', async (_event, repoPath) =>
  reviewCommitContext(repoPath, resolveGitBinary())
)
ipcMain.handle('jacky:git:review:push', async (_event, repoPath) => reviewPush(repoPath, resolveGitBinary()))
ipcMain.handle('jacky:git:review:shipInfo', async (_event, repoPath) => reviewShipInfo(repoPath, resolveGhBinary()))
ipcMain.handle('jacky:git:review:createPr', async (_event, repoPath) =>
...
ipcMain.handle('jacky:git:scanRepos', async (_event, roots, options) => {
  try {
...
ipcMain.handle('jacky:terminal:start', async (event, payload = {}) => {
...
ipcMain.handle('jacky:terminal:write', (_event, id, data) => {
...
ipcMain.handle('jacky:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('jacky:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('jacky:updates:check', async () =>
...
ipcMain.handle('jacky:updates:apply', async (_event, payload) =>
...
ipcMain.handle('jacky:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('jacky:updates:branch:set', async (_event, name) => {
...
// Resolve the canonical Jacky version (the one `release.py` bumps in
// jacky_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Jacky version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveJackyVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'jacky_cli', '__init__.py')
...
// Re-resolve the live Jacky version and push it into the native About panel
// just before showing it, so an in-place `jacky update` is reflected without
...
ipcMain.handle('jacky:version', async () => ({
  appVersion: resolveJackyVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  jackyRoot: resolveUpdateRoot()
}))
...
// the actual removal via `jacky uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (jacky_cli/uninstall.py + jacky_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_JACKY_ROOT
  ...
  const fallback = () => ({
    jacky_home: JACKY_HOME,
    agent_installed: isJackySourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'jacky_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, JACKY_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Jacky agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import jacky_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_JACKY_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `jacky-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `jacky-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('jacky:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('jacky:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('jacky:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('jacky:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// jacky:// deep links (e.g. jacky://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const JACKY_PROTOCOL = 'jacky'
...
function _extractDeepLink(argv) {
  ...
  return argv.find(a => typeof a === 'string' && a.startsWith(`${JACKY_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // jacky://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('jacky:deep-link', payload)
...
ipcMain.handle('jacky:deep-link-ready', () => {
  ...
  if (_pendingDeepLink) {
    ...
    handleDeepLink(
      `${JACKY_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(JACKY_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(JACKY_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `jacky://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching jacky:// URL is in our own argv.

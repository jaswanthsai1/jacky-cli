...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'jacky-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/jacky-runner-test',
    sourceRepoRoot: null,
    jackyHome: '/tmp/jacky-runner-test',
    logRoot: '/tmp/jacky-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'jacky-agent', 'scripts')
...
test('existing checkout detection requires git metadata', () => {
  ...
  try {
    const activeRoot = path.join(home, 'jacky-agent')
...
test('existing-checkout bootstrap args keep branch but skip the packaged commit pin', () => {
  ...
  assert.deepEqual(
    ...
    ['--dir', '/tmp/jacky-agent', '--jacky-home', '/tmp/jacky', '--branch', 'main']
  )
})

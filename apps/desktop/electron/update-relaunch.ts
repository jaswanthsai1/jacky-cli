 ...
 * Background
 * ----------
 * After `jacky update` + `jacky desktop --build-only`, the freshly-rebuilt
...
// backend/profile/root it talks to). Anything JACKY_DESKTOP_* is preserved
// plus JACKY_HOME. We snapshot the values, not the live env, so the new
...
const PRESERVED_ENV_KEYS = ['JACKY_HOME', 'ELECTRON_DISABLE_SANDBOX']
const PRESERVED_ENV_PREFIXES = ['JACKY_DESKTOP_']

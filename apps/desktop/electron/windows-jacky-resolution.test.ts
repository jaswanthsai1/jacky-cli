// Regression guards for Windows `jacky` resolution in main.ts.
//
...
//      Git-Bash `jacky` shim shadowed the real jacky.cmd/jacky.exe; the
...
//      --repair by checking ONLY venv\Scripts\jacky.exe (the console-script
...
//      installs.
//   3. unwrapWindowsVenvJackyCommand() returned the venv python with NO
...
test('findOnPath tries PATHEXT extensions before the bare (empty) name on Windows', () => {
  ...
  assert.doesNotMatch(
    source,
    /\['', \.\.\.\(process\.env\.PATHEXT/,
    'empty-extension-first order regressed: an extensionless shim can shadow jacky.cmd/.exe'
  )
})
...
test('Windows bootstrap recovery chooses --update when any real-install signal is present', () => {
  ...
  // The old too-narrow check (only venv\Scripts\jacky.exe) must not return.
  assert.doesNotMatch(
    source,
    /updaterArgs = fileExists\(venvJacky\) \?/,
    'recovery regressed to gating only on the jacky.exe shim, which forces destructive --repair'
  )
})

test('unwrapWindowsVenvJackyCommand smoke-tests the venv python before trusting it', () => {
  const source = readMain()
  const fnStart = source.indexOf('function unwrapWindowsVenvJackyCommand(')
  assert.notEqual(fnStart, -1, 'unwrapWindowsVenvJackyCommand must exist in main.ts')

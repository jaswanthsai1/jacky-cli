...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.jackyDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.jackyDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.jackyDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }
  ...
  const signInRemote = async () => {
    ...
    try {
      const result = await window.jackyDesktop?.oauthLoginConnectionConfig(remoteReauth.url)
  ...
  const openLogs = () => void window.jackyDesktop?.revealLogs().catch(() => undefined)

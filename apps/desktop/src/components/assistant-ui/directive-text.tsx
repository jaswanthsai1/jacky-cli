...
const JACKY_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type JackyRefType = (typeof JACKY_REF_TYPES)[number]
...
const ICON_PATHS: Record<JackyRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as JackyRefType] ?? ICON_FALLBACK
...
const JACKY_DIRECTIVE_RE = new RegExp(
...
export const jackyDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(JACKY_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as JackyRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: JackyRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...jackyDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Jacky directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.jackyDesktop && isRemoteGateway() ? gatewayMediaDataUrl(id) : window.jackyDesktop?.readFileDataUrl(id)

...
import { jackyDirectiveFormatter } from '@/components/assistant-ui/directive-text'
import {
...
export const UserEditComposer: FC<UserEditComposerProps> = ({ cwd, gateway, sessionId }) => {
  ...
  const replaceTriggerWithChip = useCallback(
    (item: Unstable_TriggerItem) => {
      ...
      const serialized = jackyDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLElement>) => {
    if (!dragHasAttachments(event.dataTransfer, JACKY_PATHS_MIME)) {
      return
    }

...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.jackyDesktop && ['audio', 'video'].includes(mediaKind(path))) {
  ...
  if (window.jackyDesktop && isRemoteGateway()) {
    return gatewayMediaDataUrl(path)
  }

  if (!window.jackyDesktop?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.jackyDesktop.readFileDataUrl(filePathFromMediaPath(path))
}
...
function MarkdownLink({ children, className, href, ...props }: ComponentProps<'a'>) {
  ...
  if (window.jackyDesktop && text && normalizeExternalUrl(text) === target) {

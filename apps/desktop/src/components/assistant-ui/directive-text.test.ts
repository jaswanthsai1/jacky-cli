...
import { formatRefValue, jackyDirectiveFormatter } from './directive-text'
...
describe('jackyDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = jackyDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = jackyDirectiveFormatter.parse('@file:src/main.tsx the entry point')

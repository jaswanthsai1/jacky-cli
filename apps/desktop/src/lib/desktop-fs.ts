import type {
  JackyConnection,
  JackyReadDirResult,
  JackyReadFileTextResult,
  JackySelectPathsOptions
} from '@/global'
...
function connectionCacheKey(connection: JackyConnection | null) {
  if (!connection) {
    return 'local:'
  }
...
export async function readDesktopDir(path: string): Promise<JackyReadDirResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readDir(path)
  }

  return remoteFsApi<JackyReadDirResult>(fsPath('list', path))
}

export async function readDesktopFileText(path: string): Promise<JackyReadFileTextResult> {
  if (!isDesktopFsRemoteMode()) {
    return bridge().readFileText(path)
  }

  return remoteFsApi<JackyReadFileTextResult>(fsPath('read-text', path))
}
...
export async function selectDesktopPaths(options?: JackySelectPathsOptions): Promise<string[]> {
  const desktop = bridge()

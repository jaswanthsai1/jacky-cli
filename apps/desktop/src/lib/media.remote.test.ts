...
describe('filePathFromMediaPath', () => {
  it('passes through a plain path', () => {
    expect(filePathFromMediaPath('/home/u/.jacky/images/a.png')).toBe('/home/u/.jacky/images/a.png')
  })
...
describe('gatewayMediaDataUrl', () => {
  ...
  beforeEach(() => {
    api.mockClear()
    vi.stubGlobal('window', { jackyDesktop: { api } })
    $connection.set({ mode: 'remote' } as never)
  })
  ...
  it('reads gateway media through the desktop fs bridge instead of /api/media roots', async () => {
    const url = await gatewayMediaDataUrl('/home/u/.jacky/skills/demo/images/a b.png')

    expect(url).toBe('data:image/png;base64,ZHVtbXk=')
    expect(api).toHaveBeenCalledWith({
      path: '/api/fs/read-data-url?path=%2Fhome%2Fu%2F.jacky%2Fskills%2Fdemo%2Fimages%2Fa%20b.png'
    })
  })
})
...
describe('downloadGatewayMediaFile', () => {
  ...
  beforeEach(() => {
    api.mockClear()
    vi.stubGlobal('window', { jackyDesktop: { api }, setTimeout: vi.fn() })
    vi.stubGlobal(
      'fetch',

...
/** Why a known Jacky command has no desktop UI surface. */
...
const DESKTOP_COMMAND_SPECS: readonly DesktopCommandSpec[] = [
  ...
  { name: '/profile', description: 'Switch the active Jacky profile', surface: action('profile') },
  ...
  { name: '/version', description: 'Show Jacky Agent version', surface: exec() },
...
function isKnownJackySlashCommand(command: string): boolean {
  const normalized = normalizeCommand(command)
 ...
 * Jacky' built-in slash commands — i.e. skill commands (`/gif-search`,
...
export function isDesktopSlashExtensionCommand(command: string): boolean {
  ...
  return !isKnownJackySlashCommand(normalized)
}

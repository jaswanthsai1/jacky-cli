...
describe('Jacky REST session helpers', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks()
    Reflect.deleteProperty(window, 'jackyDesktop')
  })
  ...
  it('gives the whole startup data burst the long timeout, not just profiles', async () => {
    ...
    const bootCalls: [() => Promise<unknown>, string][] = [
      [getJackyConfig, '/api/config'],
      [getJackyConfigDefaults, '/api/config/defaults'],

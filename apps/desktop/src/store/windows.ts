...
export function canOpenSessionWindow(): boolean {
  return typeof window !== 'undefined' && typeof window.jackyDesktop?.openSessionWindow === 'function'
}
...
export async function openSessionInNewWindow(sessionId: string, opts?: { watch?: boolean }): Promise<void> {
  ...
  await openWindow(() => window.jackyDesktop.openSessionWindow(sessionId, opts), 'Could not open chat in a new window')
}

// Open a fresh compact window on the new-session draft.
export async function openNewSessionInNewWindow(): Promise<void> {
  if (!canOpenSessionWindow() || typeof window.jackyDesktop.openNewSessionWindow !== 'function') {
    return
  }

  await openWindow(() => window.jackyDesktop.openNewSessionWindow(), 'Could not open new session window')
}

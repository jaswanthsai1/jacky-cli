...
import { getProfiles, setApiRequestProfile, STARTUP_REQUEST_TIMEOUT_MS } from '@/jacky'
...
import type { ProfileInfo } from '@/types/jacky'
...
// /api/profiles/active `current`). "default" is the root ~/.jacky. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'jacky.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'jacky.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.jackyDesktop.api<ActiveProfileResponse>({
...
// Persist the choice and relaunch the backend under the new JACKY_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.jackyDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.jackyDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'jacky.desktop.showAllProfiles'
...
// Switch to the default (root ~/.jacky) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.jackyDesktop?.touchBackend?.(target).catch(() => undefined)
}

...
import type { JackyGitBranch } from '@/global'
...
import type { ProjectInfo, ProjectsPayload } from '@/types/jacky'
...
const PROJECT_SCOPE_KEY = 'jacky.desktop.projectScope'
...
async function gatewayRequest<T>(method: string, params: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!gateway) {
    throw new Error('Jacky gateway is not connected')
  }
...
// repo, returning where Jacky should start working. Git is the source of
...
export async function listRepoBranches(repoPath: string): Promise<JackyGitBranch[]> {
  const git = desktopGit()
...
export async function revealPath(path: null | string): Promise<void> {
  if (path) {
    await window.jackyDesktop?.revealPath?.(path)
  }
}
...
export async function copyPath(path: null | string): Promise<void> {
  if (path) {
    await window.jackyDesktop?.writeClipboard?.(path)
  }
}

...
import { getJackyConfigRecord, saveJackyConfig } from '@/jacky'
...
export async function setAutoSpeakReplies(enabled: boolean): Promise<void> {
  ...
  try {
    const record = await getJackyConfigRecord()
    const voice = record.voice && typeof record.voice === 'object' ? (record.voice as Record<string, unknown>) : {}

    await saveJackyConfig({ ...record, voice: { ...voice, auto_tts: enabled } })
  } catch (error) {

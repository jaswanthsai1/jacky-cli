...
import type { JackyRepoStatus } from '@/global'

import { $repoStatus, refreshRepoStatus } from './coding-status'
import { $currentCwd } from './session'

const sampleStatus: JackyRepoStatus = {
...
function stubProbe(impl: (cwd: string) => Promise<JackyRepoStatus | null>) {
  ;(window as unknown as { jackyDesktop?: unknown }).jackyDesktop = { git: { repoStatus: impl } }
}

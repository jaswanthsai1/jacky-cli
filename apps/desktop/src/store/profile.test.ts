...
const { getProfiles } = await import('@/jacky')
...
const remoteConn = (over: Partial<JackyConnection> = {}): JackyConnection =>
  ({ baseUrl: 'https://jacky-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as JackyConnection

const localConn = (over: Partial<JackyConnection> = {}): JackyConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as JackyConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<JackyConnection>>()
...
beforeEach(() => {
  ...
  $profiles.set([])
  vi.stubGlobal('window', { jackyDesktop: { getConnection } })

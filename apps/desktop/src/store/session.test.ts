...
describe('workspaceCwdForNewSession', () => {
  afterEach(() => {
    ...
    window.localStorage.removeItem('jacky.desktop.workspace-cwd')
    window.localStorage.removeItem('jacky.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-a.default')
    window.localStorage.removeItem('jacky.desktop.workspace-cwd.remote.http%3A%2F%2Fbackend-b.default')
  })

  it('prefers the configured default over the sticky remembered workspace', () => {
    window.localStorage.setItem('jacky.desktop.workspace-cwd', '/home/user/sticky')
  ...
  it('keeps remote workspace memory separate from local and other remotes', () => {
    window.localStorage.setItem('jacky.desktop.workspace-cwd', '/local/project')
    $currentCwd.set('/live/session/path')

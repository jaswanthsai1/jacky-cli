...
const STORAGE_KEY = 'jacky:native-notifications'
...
// "Backgrounded" = the user isn't on Jacky. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.jackyDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.jackyDesktop

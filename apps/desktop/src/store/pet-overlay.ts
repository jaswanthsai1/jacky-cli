 ...
 * app's bounds and stays visible while Jacky is minimized. That window carries
...
const OVERLAY_ACTIVE_KEY = 'jacky.desktop.pet-overlay-active.v1'
const OVERLAY_BOUNDS_KEY = 'jacky.desktop.pet-overlay-bounds.v1'
...
function pushNow(): void {
  window.jackyDesktop?.petOverlay?.pushState(currentPayload())
}
...
function openOverlay(request: PetOverlayOpenRequest): void {
  const api = window.jackyDesktop?.petOverlay
...
export function restorePetOverlay(): void {
  if (!window.jackyDesktop?.petOverlay || !$petOverlayActive.get() || stateUnsubs.length) {
    return
  }
...
export function popInPet(): void {
  ...
  stateUnsubs = []
  $petOverlayActive.set(false)
  void window.jackyDesktop?.petOverlay?.close()
}

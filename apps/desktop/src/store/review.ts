...
import type { JackyReviewFile, JackyReviewShipInfo } from '@/global'
...
//
// Scope is always "uncommitted" — Jacky' flow is agent edits you review BEFORE
...
const OPEN_KEY = 'jacky.desktop.reviewOpen'
const COMMIT_DEFAULT_KEY = 'jacky.desktop.reviewCommitDefault'
const TREE_MODE_KEY = 'jacky.desktop.reviewTreeMode'
const SELECTED_KEY = 'jacky.desktop.reviewSelectedPath'
...
export const $reviewFiles = atom<JackyReviewFile[]>([])
...
export const $reviewShipInfo = atom<JackyReviewShipInfo>({ ghReady: false, pr: null })
...
type ReviewBridge = NonNullable<NonNullable<NonNullable<Window['jackyDesktop']>['git']>['review']>
...
export async function selectReviewFile(file: JackyReviewFile): Promise<void> {
...
export async function createOrOpenPr(): Promise<void> {
  ...
  if (existing?.url) {
    void window.jackyDesktop?.openExternal?.(existing.url)
  ...
  await runShip(async () => {
    ...
    if (url) {
      void window.jackyDesktop?.openExternal?.(url)
    }

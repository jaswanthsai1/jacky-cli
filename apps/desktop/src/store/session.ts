...
import type { SessionInfo, UsageStats } from '@/types/jacky'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'jacky.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'jacky.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'jacky.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'jacky.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'jacky.desktop.composer.fast'
...
const LAST_SESSION_KEY = 'jacky.desktop.lastSessionId'
...
function workspaceCwdKey(connection: JackyConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.jackyDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.jackyDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<JackyConnection | null>(null)
...
export const setConnection = (next: Updater<JackyConnection | null>) => updateAtom($connection, next)

import { type ConnectionState, type GatewayEvent, resolveGatewayWsUrl } from '@jacky/shared'
...
const isOpen = (gateway: JackyGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<JackyGateway | null>(null)
...
let primaryGateway: JackyGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: JackyGateway | null, profile = 'default'): void {
...
export function activeGateway(): JackyGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<JackyGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }

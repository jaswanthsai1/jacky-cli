...
declare global {
  interface Window {
    jackyDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<JackyConnection>
      ...
      // Jacky Cloud: one portal login powers discovery + silent per-agent
      ...
      profile: {
        ...
        // backend under the new JACKY_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: JackyApiRequest) => Promise<T>
      notify: (payload: JackyNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<JackyReadFileTextResult>
      selectPaths: (options?: JackySelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<JackyPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<JackyPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: JackyTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<JackyReadDirResult>
      ...
      git?: {
        worktreeList: (repoPath: string) => Promise<JackyGitWorktree[]>
        worktreeAdd: (
          repoPath: string,
        ...
        branchList: (repoPath: string) => Promise<JackyGitBranch[]>
        ...
        repoStatus: (repoPath: string) => Promise<JackyRepoStatus | null>
        ...
        review: {
          list: (repoPath: string, scope: JackyReviewScope, baseRef?: null | string) => Promise<JackyReviewList>
          diff: (
            repoPath: string,
            filePath: string,
            scope: JackyReviewScope,
            baseRef?: null | string,
            staged?: boolean
          ) => Promise<string>
          ...
          shipInfo: (repoPath: string) => Promise<JackyReviewShipInfo>
      ...
      terminal: {
        ...
        onExit: (id: string, callback: (payload: JackyTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<JackyTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: JackyWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: JackyPreviewFileChanged) => void) => () => void
...
export interface JackyTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface JackyTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `jacky update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  jackyRoot?: string
...
export interface JackyConnection {
  baseUrl: string
...
export interface JackyTitleBarTheme {
  background: string
  foreground: string
}

export interface JackyWindowState {
  isFullscreen: boolean
  nativeOverlayWidth: number
  windowButtonPosition: { x: number; y: number } | null
}
...
export interface DesktopConnectionConfig {
  envOverride: boolean
  // The saved connection mode. 'cloud' is a Jacky Cloud connection: it carries
  ...
  remoteUrl: string
  // For a 'cloud' connection: the persisted Jacky Cloud org (slug or id) the
...
export interface DesktopConnectionConfigInput {
  ...
  remoteToken?: string
  remoteUrl?: string
  // For a 'cloud' connection: the selected Jacky Cloud org (slug or id) to
...
export interface DesktopCloudStatus {
  ...
  // the jacky gateway session cookies. See cookiesHavePrivySession.
  signedIn: boolean
}

// A discovered Jacky Cloud agent — the trimmed DTO from NAS GET /api/agents.
...
export interface JackyApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface JackyNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface JackyPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface JackyReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface JackyPreviewWatch {
  id: string
  path: string
}
...
export interface JackyGitWorktree {
  path: string
  branch: null | string
  isMain: boolean
  detached: boolean
...
export interface JackyGitBranch {
  name: string
  checkedOut: boolean
  isDefault: boolean
...
export interface JackyRepoStatusFile {
  path: string
  staged: boolean
  unstaged: boolean
  untracked: boolean
...
export interface JackyRepoStatus {
  branch: null | string
...
// turn began.
export type JackyReviewScope = 'branch' | 'lastTurn' | 'uncommitted'

// One changed file in the review pane (status letter, +/- lines, staged flag).
export interface JackyReviewFile {
  path: string
  added: number
  removed: number
...
export interface JackyReviewList {
  files: JackyReviewFile[]
...
export interface JackyReviewPr {
  url: string
  state: string
  number: number
}
...
export interface JackyReviewShipInfo {
  ghReady: boolean
  pr: JackyReviewPr | null
}

export interface JackyReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface JackyReadDirResult {
  entries: JackyReadDirEntry[]
  error?: string
}

export interface JackyPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface JackySelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean

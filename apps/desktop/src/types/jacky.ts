...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Jacky doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in jacky_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `jacky model` uses — instead of
...
export type JackyConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `jacky model` CLI / onboarding OAuth flow. */
  auth_type?: string
...
// folders. Mirrors jacky_cli/projects_db.Project.to_dict().
...
export interface SkillInfo {
  ...
  usage?: number
  /** 'agent' = learned/local (editable), 'bundled' = ships with Jacky, 'hub' = installed. */
...
export interface ToolProvider {
   ...
   *  CLI `jacky tools` active-provider detection). */
  is_active: boolean
}
 ...
 *  not Jacky); elsewhere, driver health from `cua-driver doctor`. `null`
...
/** Shape of `GET /api/jacky/update/check` — the backend's own update state.

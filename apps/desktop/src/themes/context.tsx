...
const SKIN_KEY = 'jacky-desktop-theme-v2'
const MODE_KEY = 'jacky-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'jacky-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'jacky-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'jacky-desktop-active-profile-v1'
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  root.dataset.jackyTheme = skinName
  root.dataset.jackyMode = rendered
  ...
  window.jackyDesktop?.setTitleBarTheme?.({
    background: chromeBg,
  ...
  try {
    window.localStorage.setItem('jacky-boot-background', chromeBg)
    window.localStorage.setItem('jacky-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.jackyThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.jackyDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Jacky backend theme on connect. */

import { JsonRpcGatewayClient } from '@jacky/shared'
...
// "Timed out connecting to Jacky backend" that hangs the UI (#48504).
//
...
export class JackyGateway extends JsonRpcGatewayClient {
  constructor() {
    super({
      closedErrorMessage: 'Jacky gateway connection closed',
      connectErrorMessage: 'Could not connect to Jacky gateway',
      createRequestId: nextId => nextId,
      notConnectedErrorMessage: 'Jacky gateway is not connected',
...
// the call; each pooled backend already has its own JACKY_HOME, so no backend
...
export async function listSessions(
  ...
  const result = await window.jackyDesktop.api<PaginatedSessions>({
    path:
...
export function setSessionArchived(id: string, archived: boolean, profile?: string | null): Promise<{ ok: boolean }> {
  return window.jackyDesktop.api<{ ok: boolean }>({
    ...(profile ? { profile } : {}),
...
export function searchSessions(query: string): Promise<SessionSearchResponse> {
  return window.jackyDesktop.api<SessionSearchResponse>({
...
export function getSession(id: string, profile?: string | null): Promise<SessionInfo> {
  ...
  return window.jackyDesktop.api<SessionInfo>({
    ...(profile ? { profile } : {}),
...
export function getSessionMessages(id: string, profile?: string | null): Promise<SessionMessagesResponse> {
  ...
  return window.jackyDesktop.api<SessionMessagesResponse>({
    ...(profile ? { profile } : {}),
...
export function renameSession(
  ...
  return window.jackyDesktop.api<{ ok: boolean; title: string }>({
    ...(profile ? { profile } : {}),
    path: `/api/sessions/${encodeURIComponent(id)}`,
    method: 'PATCH',
...
export function getGlobalModelInfo(): Promise<ModelInfoResponse> {
  return window.jackyDesktop.api<ModelInfoResponse>({
    ...profileScoped(),
    path: '/api/model/info',
...
export function getStatus(): Promise<StatusResponse> {
  return window.jackyDesktop.api<StatusResponse>({
    ...profileScoped(),
    path: '/api/status'
  })
}
...
export function getLogs(params: {
  ...
  return window.jackyDesktop.api<LogsResponse>({
    ...profileScoped(),
    path: suffix ? `/api/logs?${suffix}` : '/api/logs'
  })
}

export function getJackyConfig(): Promise<JackyConfig> {
  return window.jackyDesktop.api<JackyConfig>({
    ...profileScoped(),
    path: '/api/config',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
...
export function getJackyConfigRecord(): Promise<JackyConfigRecord> {
  return window.jackyDesktop.api<JackyConfigRecord>({
    ...profileScoped(),
    path: '/api/config'
  })
}

export function getJackyConfigDefaults(): Promise<JackyConfigRecord> {
  return window.jackyDesktop.api<JackyConfigRecord>({
    ...profileScoped(),
...
export function getJackyConfigSchema(): Promise<ConfigSchemaResponse> {
  return window.jackyDesktop.api<ConfigSchemaResponse>({
    ...profileScoped(),
    path: '/api/config/schema'
  })
}

export function saveJackyConfig(config: JackyConfigRecord): Promise<{ ok: boolean }> {
  return window.jackyDesktop.api<{ ok: boolean }>({
    ...profileScoped(),
    path: '/api/config',
    method: 'PUT',
...
export function getMemoryProviderConfig(provider: string): Promise<MemoryProviderConfig> {
  return window.jackyDesktop.api<MemoryProviderConfig>({
...
export function getEnvVars(): Promise<Record<string, EnvVarInfo>> {
  return window.jackyDesktop.api<Record<string, EnvVarInfo>>({
    ...profileScoped(),
    path: '/api/env'
  })
}
...
export function validateProviderCredential(
  ...
  return window.jackyDesktop.api<{ ok: boolean; reachable: boolean; message: string; models?: string[] }>({
    ...profileScoped(),
...
export function revealEnvVar(key: string): Promise<{ key: string; value: string }> {
  return window.jackyDesktop.api<{ key: string; value: string }>({
    ...profileScoped(),
    path: '/api/env/reveal',
    method: 'POST',
    body: { key }
...
export function listOAuthProviders(): Promise<OAuthProvidersResponse> {
  return window.jackyDesktop.api<OAuthProvidersResponse>({
    ...profileScoped(),
...
export function disconnectOAuthProvider(providerId: string): Promise<{ ok: boolean; provider: string }> {
  return window.jackyDesktop.api<{ ok: boolean; provider: string }>({
    ...profileScoped(),
...
export function startOAuthLogin(providerId: string): Promise<OAuthStartResponse> {
  return window.jackyDesktop.api<OAuthStartResponse>({
    ...profileScoped(),
...
export function submitOAuthCode(providerId: string, sessionId: string, code: string): Promise<OAuthSubmitResponse> {
  return window.jackyDesktop.api<OAuthSubmitResponse>({
    ...profileScoped(),
...
export function pollOAuthSession(providerId: string, sessionId: string): Promise<OAuthPollResponse> {
  return window.jackyDesktop.api<OAuthPollResponse>({
    ...profileScoped(),
...
export function startMemoryProviderOAuth(provider: string): Promise<MemoryProviderOAuthStatus> {
  return window.jackyDesktop.api<MemoryProviderOAuthStatus>({
    ...profileScoped(),
...
export function getSkills(): Promise<SkillInfo[]> {
  return window.jackyDesktop.api<SkillInfo[]>({
    ...profileScoped(),
    path: '/api/skills'
  })
}
...
export function getStarmapGraph(): Promise<StarmapGraph> {
  return window.jackyDesktop.api<StarmapGraph>({
    ...profileScoped(),
...
export function getLearningNode(id: string): Promise<LearningNodeDetail> {
  return window.jackyDesktop.api<LearningNodeDetail>({
    ...profileScoped(),
...
export function deleteLearningNode(id: string): Promise<{ message: string; ok: boolean }> {
  return window.jackyDesktop.api<{ message: string; ok: boolean }>({
    ...profileScoped(),
...
export function toggleSkill(name: string, enabled: boolean): Promise<{ ok: boolean; name: string; enabled: boolean }> {
  return window.jackyDesktop.api<{ ok: boolean; name: string; enabled: boolean }>({
    ...profileScoped(),
...
export function testMcpServer(name: string): Promise<McpTestResult> {
  return window.jackyDesktop.api<McpTestResult>({
    ...profileScoped(),
 ...
 *  `saveJackyConfig`, this REPLACES rather than deep-merges, so deletes,
...
export function getToolsets(): Promise<ToolsetInfo[]> {
  return window.jackyDesktop.api<ToolsetInfo[]>({
    ...profileScoped(),
...
export function getToolsetConfig(name: string): Promise<ToolsetConfig> {
  return window.jackyDesktop.api<ToolsetConfig>({
    ...profileScoped(),
...
export function getToolsetModels(name: string, provider?: string): Promise<ToolsetModelsResponse> {
  ...
  return window.jackyDesktop.api<ToolsetModelsResponse>({
    ...profileScoped(),
...
export function selectToolsetModel(
  ...
  return window.jackyDesktop.api<{ ok: boolean; name: string; model: string }>({
    ...profileScoped(),
...
export function selectToolsetProvider(
  ...
  return window.jackyDesktop.api<{ ok: boolean; name: string; provider: string }>({
    ...profileScoped(),
...
export function runToolsetPostSetup(name: string, key: string): Promise<ActionResponse & { key: string }> {
  return window.jackyDesktop.api<ActionResponse & { key: string }>({
    ...profileScoped(),
...
export function getComputerUseStatus(): Promise<ComputerUseStatus> {
  return window.jackyDesktop.api<ComputerUseStatus>({
    ...profileScoped(),
...
export function grantComputerUsePermissions(): Promise<ActionResponse> {
  return window.jackyDesktop.api<ActionResponse>({
    ...profileScoped(),
...
export function getMessagingPlatforms(): Promise<MessagingPlatformsResponse> {
  return window.jackyDesktop.api<MessagingPlatformsResponse>({
...
export function updateMessagingPlatform(
  ...
  return window.jackyDesktop.api<{ ok: boolean; platform: string }>({
...
export function testMessagingPlatform(platformId: string): Promise<MessagingPlatformTestResponse> {
  return window.jackyDesktop.api<MessagingPlatformTestResponse>({
...
export function getCronJobs(): Promise<CronJob[]> {
  return window.jackyDesktop.api<CronJob[]>({
    path: '/api/cron/jobs',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export async function getCronJobRuns(jobId: string, limit = 20): Promise<SessionInfo[]> {
  const { runs } = await window.jackyDesktop.api<{ runs: SessionInfo[] }>({
...
export function getProfiles(): Promise<ProfilesResponse> {
  return window.jackyDesktop.api<ProfilesResponse>({
    path: '/api/profiles',
    timeoutMs: STARTUP_REQUEST_TIMEOUT_MS
  })
}
...
export function createProfile(body: ProfileCreatePayload): Promise<{ name: string; ok: boolean; path: string }> {
  return window.jackyDesktop.api<{ name: string; ok: boolean; path: string }>({
    path: '/api/profiles',
    method: 'POST',
    body
  })
...
export function deleteProfile(name: string): Promise<{ ok: boolean; path: string }> {
  return window.jackyDesktop.api<{ ok: boolean; path: string }>({
    path: `/api/profiles/${encodeURIComponent(name)}`,
    method: 'DELETE'
  })
}
...
export function getProfileSoul(name: string): Promise<ProfileSoul> {
  return window.jackyDesktop.api<ProfileSoul>({
...
export function getProfileSetupCommand(name: string): Promise<ProfileSetupCommand> {
  return window.jackyDesktop.api<ProfileSetupCommand>({
    path: `/api/profiles/${encodeURIComponent(name)}/setup-command`
  })
}

export function getUsageAnalytics(days = 30): Promise<AnalyticsResponse> {
  return window.jackyDesktop.api<AnalyticsResponse>({
    ...profileScoped(),
...
export function getGlobalModelOptions(opts?: {
  ...
  return window.jackyDesktop.api<ModelOptionsResponse>({
    ...profileScoped(),
...
// curation `jacky model` does — for Nous it honors the free/paid tier so a
// free user gets a free model instead of a paid default.
export function getRecommendedDefaultModel(provider: string): Promise<RecommendedDefaultModel> {
  return window.jackyDesktop.api<RecommendedDefaultModel>({
    ...profileScoped(),
...
export function setGlobalModel(
  ...
  return window.jackyDesktop.api<{ ok: boolean; provider: string; model: string }>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body: {
...
export function getAuxiliaryModels(): Promise<AuxiliaryModelsResponse> {
  return window.jackyDesktop.api<AuxiliaryModelsResponse>({
    ...profileScoped(),
...
export function getMoaModels(): Promise<MoaConfigResponse> {
  return window.jackyDesktop.api<MoaConfigResponse>({
    ...profileScoped(),
    path: '/api/model/moa'
  })
}
...
export function saveMoaModels(body: MoaConfigResponse): Promise<MoaConfigResponse & { ok: boolean }> {
  return window.jackyDesktop.api<MoaConfigResponse & { ok: boolean }>({
    ...profileScoped(),
    path: '/api/model/moa',
    method: 'PUT',
    body
...
export function setModelAssignment(body: ModelAssignmentRequest): Promise<ModelAssignmentResponse> {
  return window.jackyDesktop.api<ModelAssignmentResponse>({
    ...profileScoped(),
    path: '/api/model/set',
    method: 'POST',
    body
...
export function updateJacky(): Promise<ActionResponse> {
  return window.jackyDesktop.api<ActionResponse>({
    ...profileScoped(),
    path: '/api/jacky/update',
    method: 'POST'
  })
}
...
export function checkJackyUpdate(force = false): Promise<BackendUpdateCheckResponse> {
  return window.jackyDesktop.api<BackendUpdateCheckResponse>({
    ...profileScoped(),
    path: `/api/jacky/update/check${force ? '?force=true' : ''}`
  })
}

export function getActionStatus(name: string, lines = 200): Promise<ActionStatusResponse> {
  return window.jackyDesktop.api<ActionStatusResponse>({
    ...profileScoped(),
...
export function transcribeAudio(dataUrl: string, mimeType?: string): Promise<AudioTranscriptionResponse> {
  return window.jackyDesktop.api<AudioTranscriptionResponse>({
...
export function speakText(text: string): Promise<AudioSpeakResponse> {
  return window.jackyDesktop.api<AudioSpeakResponse>({
...
export function getElevenLabsVoices(): Promise<ElevenLabsVoicesResponse> {
  return window.jackyDesktop.api<ElevenLabsVoicesResponse>({
    path: '/api/audio/elevenlabs/voices'
  })
}

// ---------------------------------------------------------------------------
// Skills hub — search / preview / scan / install (parity with `jacky skills`
...
export function getSkillHubSources(): Promise<SkillHubSourcesResponse> {
  return window.jackyDesktop.api<SkillHubSourcesResponse>({
    ...profileScoped(),
...
export function searchSkillsHub(query: string, source = 'all', limit = 20): Promise<SkillHubSearchResponse> {
  ...
  return window.jackyDesktop.api<SkillHubSearchResponse>({
    ...profileScoped(),
...
export function previewSkillHub(identifier: string): Promise<SkillHubPreview> {
  return window.jackyDesktop.api<SkillHubPreview>({
    ...profileScoped(),
...
export function scanSkillHub(identifier: string): Promise<SkillHubScanResult> {
  return window.jackyDesktop.api<SkillHubScanResult>({
    ...profileScoped(),
...
// `jacky mcp` and the dashboard MCP page). Raw JSON editing stays in
// config.yaml via saveJackyConfig.
// ---------------------------------------------------------------------------

export function listMcpServers(): Promise<{ servers: McpServerSummary[] }> {
  return window.jackyDesktop.api<{ servers: McpServerSummary[] }>({
    ...profileScoped(),
    path: '/api/mcp/servers'
  })
}
...
export function getMcpCatalog(): Promise<McpCatalogResponse> {
  return window.jackyDesktop.api<McpCatalogResponse>({
    ...profileScoped(),
...
export function installMcpCatalogEntry(
  ...
  return window.jackyDesktop.api<{ ok: boolean; name?: string; pid?: number; action?: string; background?: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Memory data + curator (parity with `jacky memory` / `jacky curator`).
// ---------------------------------------------------------------------------

export function getMemoryStatus(): Promise<MemoryStatusResponse> {
  return window.jackyDesktop.api<MemoryStatusResponse>({
    ...profileScoped(),
    path: '/api/memory'
  })
}
...
export function resetMemory(target: 'all' | 'memory' | 'user'): Promise<{ ok: boolean; deleted: string[] }> {
  return window.jackyDesktop.api<{ ok: boolean; deleted: string[] }>({
    ...profileScoped(),
...
export function getCuratorStatus(): Promise<CuratorStatusResponse> {
  return window.jackyDesktop.api<CuratorStatusResponse>({
    ...profileScoped(),
    path: '/api/curator'
  })
}
...
export function setCuratorPaused(paused: boolean): Promise<{ ok: boolean; paused: boolean }> {
  return window.jackyDesktop.api<{ ok: boolean; paused: boolean }>({
    ...profileScoped(),
...
// ---------------------------------------------------------------------------
// Maintenance operations (parity with `jacky doctor` / `jacky security
// audit` / `jacky backup` / `jacky debug share` and the dashboard System
...
export function runDoctor(): Promise<ActionResponse> {
  return window.jackyDesktop.api<ActionResponse>({ path: '/api/ops/doctor', method: 'POST', body: {} })
}

export function runSecurityAudit(): Promise<ActionResponse> {
  return window.jackyDesktop.api<ActionResponse>({ path: '/api/ops/security-audit', method: 'POST', body: {} })
}

export function runBackup(): Promise<ActionResponse & { archive?: string }> {
  return window.jackyDesktop.api<ActionResponse & { archive?: string }>({
    path: '/api/ops/backup',
    method: 'POST',
    body: {}
  })
...
export function runDebugShare(): Promise<DebugShareResponse> {
  return window.jackyDesktop.api<DebugShareResponse>({

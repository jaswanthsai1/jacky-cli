...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'jacky:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.jackyDesktop?.saveImageFromUrl) {
        if (await window.jackyDesktop.saveImageFromUrl(src)) {

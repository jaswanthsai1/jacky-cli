...
const branchActionLabel = (branch: JackyGitBranch, copy: BranchActionCopy) => {
...
// inside it. Naming is explicit — no auto-generated `jacky/work-<ts>` trees.
export function StartWorkButton({ repoPath, onStarted }: { repoPath: string; onStarted: (path: string) => void }) {
  ...
  const [branches, setBranches] = useState<JackyGitBranch[]>([])
  ...
  const convert = async (branch: JackyGitBranch) => {
    if (pending || !repoPath || !branch) {
      return
    }

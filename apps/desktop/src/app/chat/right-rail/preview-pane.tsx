        ...
        <>
          <a
            ...
            onClick={event => {
              ...
              void window.jackyDesktop?.openExternal(error.url)
            }}
          >
...
export function PreviewPane({
  ...
  const restartServer = useCallback(async () => {
    ...
    // "Ask Jacky to restart the server" looked like it did nothing —
  ...
  useEffect(() => {
    if (
      ...
      !window.jackyDesktop?.watchPreviewFile ||
      !window.jackyDesktop?.onPreviewFileChanged
    ) {
      return
    }
    ...
    const unsubscribe = window.jackyDesktop.onPreviewFileChanged(payload => {
    ...
    void window.jackyDesktop
      .watchPreviewFile(target.url)
      .then(watch => {
        if (!active) {
          void window.jackyDesktop?.stopPreviewFileWatch?.(watch.id)
    ...
    return () => {
      ...
      if (watchId) {
        void window.jackyDesktop?.stopPreviewFileWatch?.(watchId)
      }
    }
  ...
  useEffect(() => {
    ...
    webview.setAttribute('partition', 'persist:jacky-preview')

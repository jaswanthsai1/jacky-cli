...
export async function attachmentPreviewDataUrl(filePath: string): Promise<string> {
  try {
    const local = await window.jackyDesktop?.readFileDataUrl?.(filePath)
...
export const JACKY_PATHS_MIME = 'application/x-jacky-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Jacky sources (e.g. the project tree) ride on a custom
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.jackyDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(JACKY_PATHS_MIME)
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.jackyDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(
    async ({ silent = false }: { silent?: boolean } = {}) => {
      try {
        const path = await window.jackyDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.jackyDesktop?.getPathForFile ? window.jackyDesktop.getPathForFile(file) : ''

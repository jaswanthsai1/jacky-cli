...
export function PetOverlayApp() {
  ...
  const setIgnore = (ignore: boolean) => {
    if (ignoreRef.current !== ignore) {
      ...
      window.jackyDesktop?.petOverlay?.setIgnoreMouse(ignore)
    }
  }

  // Mirror pushed state into the shared atoms so PetSprite/PetBubble just work.
  useEffect(() => {
    const off = window.jackyDesktop?.petOverlay?.onState(payload => {
    ...
    window.jackyDesktop?.petOverlay?.control({ type: 'ready' })
  ...
  useEffect(() => {
    ...
    window.jackyDesktop?.petOverlay?.setFocusable(composerOpen)
  ...
  const onPetPointerMove = (e: React.PointerEvent) => {
    ...
    window.jackyDesktop?.petOverlay?.setBounds({
      height: drag.height,
      width: drag.width,
  ...
  const onPetPointerUp = (e: React.PointerEvent) => {
    ...
    if (drag.moved) {
      ...
      window.jackyDesktop?.petOverlay?.control({
    ...
    if (e.shiftKey) {
      window.jackyDesktop?.petOverlay?.control({ type: 'pop-in' })
    ...
    if (clickTimerRef.current) {
      clearTimeout(clickTimerRef.current)
      clickTimerRef.current = undefined
      window.jackyDesktop?.petOverlay?.control({ type: 'toggle-app' })
  ...
  const send = () => {
    ...
    if (text) {
      window.jackyDesktop?.petOverlay?.control({ text, type: 'submit' })
    }
  ...
  const openApp = () => {
    ...
    setUnread(false)
    window.jackyDesktop?.petOverlay?.control({ type: 'open-app' })
  }
  ...
  const onScale = useCallback((next: number, anchor: PetZoomAnchor) => {
    ...
    window.jackyDesktop?.petOverlay?.control({ scale: next, type: 'scale' })
  }, [])
  ...
  useEffect(() => {
    ...
    window.jackyDesktop?.petOverlay?.setBounds(bounds)
    window.jackyDesktop?.petOverlay?.control({ bounds, type: 'bounds' })

...
import { getSessionMessages, type SessionMessage, triggerCronJob } from '../jacky'
...
import { useJackyConfig } from './session/hooks/use-jacky-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.jackyDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.jackyDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.jackyDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.jackyDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // jacky:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.jackyDesktop?.onDeepLink?.(payload => {
    ...
    void window.jackyDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.jackyDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  const { refreshJackyConfig, sttEnabled, voiceMaxRecordingSeconds } = useJackyConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshJackyConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshJackyConfig])

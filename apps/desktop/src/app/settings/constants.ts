...
export const EMPTY_SELECT_VALUE = '__jacky_empty__'
...
export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'NOUS_',
    name: 'Nous Portal',
    description: 'Hosted Jacky & Nous-trained models',
  ...
  { prefix: 'JACKY_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // jacky_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Jacky needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Jacky stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Jacky can read from one file request.',
  approvals: {
    mode: 'How Jacky handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Jacky updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})

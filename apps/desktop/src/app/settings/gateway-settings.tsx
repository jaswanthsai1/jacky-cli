...
// Jacky Cloud discovery lifecycle for the cloud-mode panel.
...
export function GatewaySettings() {
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.jackyDesktop
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.jackyDesktop.applyConnectionConfig(payload())
        : await window.jackyDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.jackyDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.jackyDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.jackyDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.jackyDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.jackyDesktop.getConnectionConfig(scope)
      setState(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.jackyDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.jackyDesktop?.getConnectionConfig) {
  ...
  return (
    <SettingsContent>
      ...
      {/* Jacky Cloud panel: one portal sign-in, then a discovered-agent picker
          ...
          <ListRow
            action={
              <Input
                ...
                placeholder="https://gateway.example.com/jacky"
      ...
      <div className="mt-6 grid gap-1">
        <ListRow
          action={
            <Button onClick={() => void window.jackyDesktop?.revealLogs()} size="sm" variant="textStrong">
              <FileText />
              {g.openLogs}
            </Button>
          }

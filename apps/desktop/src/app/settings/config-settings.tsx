...
import { getElevenLabsVoices, getJackyConfigSchema, saveJackyConfig } from '@/jacky'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, JackyConfigRecord } from '@/types/jacky'

import { setJackyConfigCache, useJackyConfigRecord } from '../hooks/use-config-record'
...
export function voiceFieldVisible(key: string, config: JackyConfigRecord): boolean {
...
export function ConfigSettings({
  ...
  const [config, setConfig] = useState<JackyConfigRecord | null>(null)
  const { data: loadedConfig, isError: configLoadFailed, refetch: refetchConfig } = useJackyConfigRecord()
  ...
  const {
    ...
    queryKey: ['jacky-config-schema'],
    queryFn: getJackyConfigSchema,
    staleTime: 5 * 60 * 1000
  })
  ...
  const updateConfig = (next: JackyConfigRecord) => {

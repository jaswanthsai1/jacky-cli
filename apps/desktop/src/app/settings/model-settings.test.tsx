...
const getJackyConfigRecord = vi.fn()
const saveJackyConfig = vi.fn()
...
vi.mock('@/jacky', () => ({
  ...
  getJackyConfigRecord: () => getJackyConfigRecord(),
  saveJackyConfig: (config: unknown) => saveJackyConfig(config)
}))
...
beforeEach(() => {
  ...
  getJackyConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveJackyConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getJackyConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveJackyConfig).toHaveBeenCalledWith(

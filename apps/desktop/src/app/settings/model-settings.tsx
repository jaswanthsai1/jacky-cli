...
import { invalidateJackyConfig, setJackyConfigCache, useJackyConfigRecord } from '../hooks/use-config-record'
...
// Jacky' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Jacky default (medium), shown as Medium.
...
// backend now surfaces the full `jacky model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in jacky_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const { data: config } = useJackyConfigRecord()
  const setConfig = setJackyConfigCache
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `jacky model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Jacky runs the flow for you.`}
          </p>
        )}

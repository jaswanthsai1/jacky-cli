...
import { disconnectOAuthProvider, listOAuthProviders } from '@/jacky'
...
import type { EnvVarInfo, OAuthProvider } from '@/types/jacky'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.jackyDesktop?.terminal)
...
//      in jacky_cli/provider_catalog.py) — the SAME provider identity
//      `jacky model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Jacky can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Jacky can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Jacky never deletes creds another tool owns behind a silent API call.

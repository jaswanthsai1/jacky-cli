...
import { getJackyConfig, getJackyConfigDefaults } from '@/jacky'
...
interface JackyConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useJackyConfig({ activeSessionIdRef, refreshProjectBranch }: JackyConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshJackyConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getJackyConfig(), getJackyConfigDefaults().catch(() => ({}))])
  ...
  return { refreshJackyConfig, sttEnabled, voiceMaxRecordingSeconds }
}

...
import type { JackyReadDirResult } from '@/global'
import { $connection } from '@/store/session'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<JackyReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { jackyDesktop: { readDir: typeof readDir } }).jackyDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { jackyDesktop?: unknown }).jackyDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): JackyReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { jackyDesktop: unknown }).jackyDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: JackyReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<JackyReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { jackyDesktop: unknown }).jackyDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.jackyDesktop is missing', async () => {
    delete (window as unknown as { jackyDesktop?: unknown }).jackyDesktop

...
import type { JackyReadDirEntry, JackyReadDirResult } from '@/global'
...
export type ProjectTreeEntry = JackyReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: JackyReadDirEntry) {
...
async function filterIgnored(entries: JackyReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<JackyReadDirResult> {
  if (!window.jackyDesktop) {

...
import type { ModelOptionProvider, ModelOptionsResponse } from '@/types/jacky'
...
export function ModelMenuPanel({ gateway, onSelectModel, requestGateway }: ModelMenuPanelProps) {
  ...
  // session (effort/fast), gated by capability. Unset → Jacky defaults.
          ...
          {groups.map(group => (
            <DropdownMenuGroup className="py-0.5" key={group.provider.slug}>
              ...
              {group.families.map(family => {
                ...
                // the active model, otherwise its remembered preset (Jacky

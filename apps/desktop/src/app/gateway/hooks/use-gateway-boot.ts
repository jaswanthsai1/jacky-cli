import { isGatewayReauthRequired, resolveGatewayWsUrl } from '@jacky/shared'
import { useEffect, useRef } from 'react'

import type { JackyConnection } from '@/global'
import { JackyGateway } from '@/jacky'
...
import type { RpcEvent } from '@/types/jacky'
...
interface GatewayBootOptions {
  ...
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.jackyDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: JackyGateway | null) => void
  refreshJackyConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    const publish = (next: JackyConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Jacky...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Jacky…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Jacky gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshJackyConfig().catch(() => undefined)
    ...
    const offBootProgress = desktop.onBootProgress(payload => {
      // Soft switch / post-boot startJacky re-emits progress — ignore so the
    ...
    const gateway = new JackyGateway()
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshJackyConfig()

...
// bug that drives the REAL useGatewayBoot hook + REAL JackyGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshJackyConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { jackyDesktop?: unknown }).jackyDesktop = fakeDesktop()
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForJacky) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startJacky()'s remote branch awaits waitForJacky() for 45s before it
    ...
    ;(window as { jackyDesktop?: unknown }).jackyDesktop = desktop
    ...
    // After ~45s waitForJacky gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Jacky backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })

...
import type { JackyGateway } from '@/jacky'
...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<JackyGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.jackyDesktop>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<JackyGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as JackyGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}, timeoutMs?: number, signal?: AbortSignal) => {
      ...
      if (!gateway) {
        throw new Error('Jacky gateway unavailable')
      }

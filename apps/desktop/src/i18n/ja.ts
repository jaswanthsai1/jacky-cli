...
export const ja = defineLocale({
  ...
  boot: {
    ready: 'Jacky Desktop の準備ができました',
    desktopBootFailedWithMessage: message => `デスクトップの起動に失敗しました: ${message}`,
    steps: {
      connectingGateway: 'ライブデスクトップゲートウェイに接続中',
      loadingSettings: 'Jacky の設定を読み込み中',
      loadingSessions: '最近のセッションを読み込み中',
      startingDesktopConnection: 'デスクトップ接続を開始中',
      startingJackyDesktop: 'Jacky Desktop を起動中…'
    },
    errors: {
      backgroundExited: 'Jacky バックグラウンドプロセスが終了しました。',
      backgroundExitedDuringStartup: '起動中に Jacky バックグラウンドプロセスが終了しました。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage:
      'Jacky バックエンドがこのデスクトップビルドより古く、正常に動作しない場合があります。更新して揃えてください。',
    installMethodUnsupportedTitle: 'サポート対象外のインストール方法',
    updateJacky: 'Jacky を更新',
    ...
    errors: {
      ...
      methodNotAllowed:
        'デスクトップバックエンドがそのリクエストを拒否しました (405 Method Not Allowed)。Jacky Desktop を再起動してください。',
    ...
    native: {
      ...
      rejectAction: '拒否',
      inputTitle: '入力が必要です',
      inputBody: 'Jacky が応答を待っています。',
      turnDoneTitle: 'Jacky が完了しました',
  ...
  settings: {
    ...
    resetConfirm: 'すべての設定を Jacky のデフォルトに戻しますか？',
    ...
    notifications: {
      ...
      focusedHint: '完了通知は Jacky がバックグラウンドにあるときのみ表示されます。',
      kinds: {
        approval: {
          label: '承認が必要',
        ...
        input: {
          label: '入力が必要',
          description: 'Jacky が質問したか、パスワードやシークレットを必要としています。'
        },
        turnDone: {
          label: '応答完了',
          description: 'Jacky がバックグラウンドのときにターンが完了しました。'
        },
        turnError: {
          label: 'ターン失敗',
    ...
    appearance: {
      ...
      colorMode: 'カラーモード',
      colorModeDesc: '固定モードを選ぶか、Jacky をシステム設定に合わせます。',
      ...
      pet: {
        title: 'ペット',
        intro:
          'アプリ上に浮かぶ petdex のアニメーションマスコットを採用しましょう。ツール実行中は走り、成功すると喜び、エラーでしょんぼりと、Jacky の状態に反応します。',
        restartHint:
          'ペット機能には再起動が必要です。この機能が追加される前に起動したアプリが動作中です。Jacky を終了して再度開き、このページに戻ってください。',
        scaleTitle: 'サイズ',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone:
        'Jacky がローカル時刻のコンテキストを必要とするときに使用します。空欄ならシステムのタイムゾーンを使います。',
      agent: {
        imageInputMode: '画像添付をモデルへ送る方法を制御します。',
        maxTurns: 'Jacky が 1 回の実行を停止するまでのツール呼び出しターン上限です。'
      },
      terminal: {
      ...
      fileReadMaxChars: 'Jacky が 1 回のファイル読み取りで取得できる最大文字数です。',
      approvals: {
        mode: '明示的な承認が必要なコマンドを Jacky がどう扱うかを設定します。',
      ...
      updates: {
        nonInteractiveLocalChanges:
          'アプリから Jacky 自身を更新するとき、ローカルのソース変更を保持するか破棄するかを選びます。ターミナル更新では常に確認されます。'
      }
    }),
    about: {
      ...
      automaticUpdatesDesc: 'Jacky はバックグラウンドで自動的に更新を確認し、利用可能になったら通知します。',
    ...
    gateway: {
      ...
      intro:
        'Jacky Desktop はデフォルトで独自のローカルゲートウェイを起動します。別のマシンや信頼できるプロキシの背後で既に動作している Jacky バックエンドをこのアプリで制御する場合は、リモートゲートウェイを使用してください。以下でプロファイルを選択して、それぞれのリモートホストを設定します。',
      appliesTo: '適用対象',
      ...
      envOverrideDesc:
        '保存された設定を使用するには JACKY_DESKTOP_REMOTE_URL と JACKY_DESKTOP_REMOTE_TOKEN の設定を解除してください。',
      localTitle: 'ローカルゲートウェイ',
      localDesc:
        'ローカルホストでプライベートな Jacky バックエンドを起動します。これがデフォルトで、オフラインでも動作します。',
      remoteTitle: 'リモートゲートウェイ',
      remoteDesc:
        'このデスクトップシェルをリモートの Jacky バックエンドに接続します。ホスト型ゲートウェイは OAuth またはユーザー名とパスワードを使用します。自己ホスト型はセッショントークンを使用する場合があります。',
      remoteUrlTitle: 'リモート URL',
      remoteUrlDesc:
        'リモートダッシュボードバックエンドのベース URL。/jacky などのパスプレフィックスもサポートしています。',
      ...
      restartingMessage: 'Jacky Desktop は保存された設定を使用して再接続します。',
      savedMessage: '次回起動時に保存されます。',
      connectedTo: (baseUrl, version) => `${baseUrl}${version ? ` · Jacky ${version}` : ''} に接続しました`,
    ...
    providers: {
      ...
      intro:
        'サブスクリプションでサインインします。API キーのコピーは不要です。Jacky がアプリ内でブラウザーサインインを代行します。',
      connected: '接続済み',
      collapse: '折りたたむ',
  ...
  skills: {
    ...
    skillArchivedMessage: 'jacky curator restore で復元できます。'
  },
  ...
  starmap: {
    ...
    emptyDesc: 'Jacky がスキルやメモリを蓄積すると、ここに表示されます。'
  },
  agents: {
    close: 'エージェントを閉じる',
    title: 'スポーンツリー',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: 'ペット機能を使うには Jacky を再起動してください。',
      empty: '一致するペットがありません。',
      turnOff: 'オフ',
      turnOn: 'オン',
      installed: 'インストール済み',
    ...
    generatePet: {
      ...
      staleBackend: 'ペットを生成するには Jacky を更新してください。',
      backgroundHint: 'このウィンドウは閉じても大丈夫です。完了したら Jacky が通知します。',
    ...
    nav: {
      ...
      settings: { title: '設定', detail: 'Jacky デスクトップを設定' },
    ...
    jackyActiveSessions: (version, count) => `Jacky ${version} · アクティブセッション ${count}`,
  ...
  messaging: {
    ...
    fieldCopy: {
      ...
      MATRIX_USER_ID: { label: 'ボットユーザー ID', placeholder: '@jacky:example.org' },
  ...
  profiles: {
    ...
    deleting: '削除中...',
    createDesc: 'プロファイルは独立した Jacky 環境です：設定、スキル、SOUL.md が別々になります。',
    nameLabel: '名前',
    cloneFrom: '複製元',
    cloneFromNone: 'なし（空）',
  ...
  cron: {
    ...
    newCron: '新しい Cron',
    emptyDescNew:
      'Cron 式でプロンプトを実行するスケジュールを設定します。Jacky が実行して、選択した宛先に結果を送信します。',
  ...
  sidebar: {
    ...
    projects: {
      ...
      staleBackend:
        'プロジェクトを作成するには Jacky バックエンドを更新してください。バックエンドがこのデスクトップアプリより古いです（設定 → 更新 → バックエンド）。',
      deleteConfirm:
        'Jacky から保存済みプロジェクトを削除します。ファイル・git リポジトリ・ワークツリーはそのまま残ります。',
      startWork: '新しいワークツリー',
  ...
  composer: {
    ...
    placeholderStarting: 'Jacky を起動中...',
    placeholderReconnecting: 'Jacky に再接続中…',
    ...
    attachUrlDesc: 'Jacky がページを取得し、このターンのコンテキストとして含めます。',
  ...
  statusStack: {
    ...
    coding: {
      ...
      agentShip: 'Jacky にコミットと PR を任せる',
      agentShipPrompt:
  ...
  updates: {
    ...
    unsupportedMessage: 'このバージョンの Jacky はアプリ内から自分を更新できません。',
    ...
    availableBody: '新しいバージョンの Jacky をインストールする準備ができています。',
    availableTitleBackend: 'バックエンドの更新があります',
    availableBodyBackend: '接続中の Jacky バックエンドの新しいバージョンをインストールできます。',
    ...
    manualBody:
      'Jacky をコマンドラインからインストールしたため、更新もそこで実行されます。これをターミナルに貼り付けてください:',
    manualPickedUp: 'Jacky は次回起動時に新しいバージョンを読み込みます。',
    guiSkewTitle: 'デスクトップアプリを更新してください',
    guiSkewBody:
      'バックエンドは更新されましたが、このデスクトップアプリのパッケージは変更されていません。一致させるために Jacky デスクトップアプリ（AppImage / .deb / .rpm）を更新または再インストールしてください。',
    copy: 'コピー',
    copied: 'コピーしました',
    done: '完了',
    applyingBody:
      'Jacky アップデーターが独自のウィンドウで引き継ぎ、完了後に自動的に Jacky を再度開きます。更新中はご自分で Jacky を開き直さないでください。',
    applyingBodyBackend: 'リモートバックエンドが更新を適用して再起動します。復帰すると Jacky が自動的に再接続します。',
    applyingClose: 'このウィンドウは更新中に閉じ、その後 Jacky が自動的に再度開きます。',
  ...
  install: {
    ...
    oneTimeTitle: 'Jacky には一度限りのインストールが必要です',
    ...
    settingUpTitle: 'Jacky Agent を設定中',
    finishingTitle: '仕上げ中',
    failedDesc:
      'インストール手順のいずれかが失敗しました。Windows では、別の Jacky CLI またはデスクトップインスタンスが実行中の場合に発生することがあります。実行中の Jacky インスタンスをすべて停止してから再試行してください。詳細は以下またはデスクトップログで確認できます。',
    activeDesc:
      'これは一回限りのセットアップです。Jacky インストーラーが依存関係をダウンロードしてマシンを設定しています。以降の起動ではこの手順はスキップされます。',
  ...
  onboarding: {
    headerTitle: 'Jacky Agent のセットアップをしましょう',
    headerDesc: 'チャットを始めるにはモデルプロバイダーを接続してください。ほとんどのオプションはワンクリックです。',
    preparingInstall: 'Jacky はインストールを完了中です。初回実行では通常 1 分以内に完了します。',
    starting: 'Jacky を起動中…',
    ...
    recommended: '推奨',
    connected: '接続済み',
    featuredPitch: '1 つのサブスクリプションで 300 以上の最先端モデル — Jacky を実行するための推奨方法',
    ...
    apiKeyOptions: {
      ...
      local: {
        short: 'セルフホスト',
        description:
          'ローカルまたはセルフホストの OpenAI 互換エンドポイント（vLLM、llama.cpp、Ollama など）に Jacky を接続。'
      }
    },
    ...
    flowSubtitles: {
      ...
      device_code: 'ブラウザーで確認ページを開きます — Jacky が自動接続します',
    ...
    authorizeThere: 'そこで Jacky を承認してください。',
    copyAuthCode: '認証コードをコピーして以下に貼り付けてください。',
    pasteAuthCode: '認証コードを貼り付け',
    reopenAuthPage: '認証ページを再度開く',
    autoBrowser: provider =>
      `${provider} をブラウザーで開きました。Jacky をそこで承認すれば自動接続されます。コピーや貼り付けは不要です。`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      gatewayTitle: 'Jacky 推論ゲートウェイのステータス',
      agents: 'エージェント',
  ...
  preview: {
    ...
    largeBody: (label, size) => `${label} は ${size} です。Jacky は最初の 512 KB のみを表示します。`,
    ...
    web: {
      ...
      tryAgain: '再試行',
      restarting: 'Jacky を再起動中...',
      askRestart: 'Jacky にサーバーの再起動を依頼',
      lookingRestart: taskId => `Jacky は再起動するプレビューサーバーを検索中です (${taskId})`,
      restartingTitle: 'プレビューサーバーを再起動中',
      restartingMessage: 'Jacky はバックグラウンドで作業中です。進捗はプレビューコンソールで確認してください。',
      ...
      finishedRestarting: message =>
        `Jacky がプレビューサーバーの再起動を完了しました${message ? `: ${message}` : ''}`,
      ...
      restartFailedMessage: 'Jacky がサーバーを再起動できませんでした。',
      stillWorking:
        'Jacky はまだ作業中ですが、再起動の結果がまだ届いていません。サーバーコマンドがフォアグラウンドで実行されている可能性があります。',
  ...
  assistant: {
    thread: {
      ...
      loadingResponse: 'Jacky が応答を読み込み中',
    ...
    approval: {
      gatewayDisconnected: 'Jacky ゲートウェイが接続されていません',
      ...
      alwaysDescription: pattern =>
        `これにより "${pattern}" パターンが永続的な許可リスト (~/.jacky/config.yaml) に追加されます。Jacky はこのセッションや将来のセッションで、このようなコマンドについて再度尋ねません。`,
      alwaysAllow: '常に許可'
    },
    clarify: {
  ...
  prompts: {
    ...
    sudoTitle: '管理者パスワード',
    sudoDesc:
      'Jacky は特権コマンドを実行するために sudo パスワードが必要です。ローカルエージェントにのみ送信されます。',
    sudoPlaceholder: 'sudo パスワード',
    secretTitle: 'シークレットが必要です',
    secretDesc: 'Jacky は続行するための認証情報が必要です。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '画像を保存するには Jacky Desktop を再起動してください。',
    restartToSaveImages: '画像を保存するには Jacky Desktop を再起動してください',
    ...
    handoff: {
      ...
      timedOut: 'ゲートウェイの待機がタイムアウトしました。`jacky gateway` は起動していますか？'
    }
  },

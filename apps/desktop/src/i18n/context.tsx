...
import { getJackyConfigRecord, type JackyConfigRecord, saveJackyConfig } from '@/jacky'
...
export interface I18nConfigClient {
  getConfig: () => Promise<JackyConfigRecord>
  saveConfig: (config: JackyConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.jackyDesktop?.api) {
      return Promise.resolve({})
    }

    return getJackyConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.jackyDesktop?.api) {
...
export function getConfigDisplayLanguage(config: JackyConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: JackyConfigRecord, locale: Locale): JackyConfigRecord {

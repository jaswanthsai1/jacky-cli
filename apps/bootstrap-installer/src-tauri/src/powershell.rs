...
///
/// `jacky_home_override` propagates to the child as $JACKY_HOME so the
...
pub async fn run_script(
    ...
    if let Some(cwd) = stable_script_cwd(script_path, jacky_home_override) {
        cmd.current_dir(cwd);
    }

    if let Some(home) = jacky_home_override {
        cmd.env("JACKY_HOME", home);
    }
...
fn stable_script_cwd<'a>(script_path: &'a Path, jacky_home_override: Option<&'a str>) -> Option<&'a Path> {
    if let Some(home) = jacky_home_override {
...
"#;
    ...
    #[test]
    fn stable_script_cwd_prefers_existing_jacky_home() {

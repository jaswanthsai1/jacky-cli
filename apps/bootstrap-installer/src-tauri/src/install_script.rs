...
//!
//! Resolution order:
//!   1. Dev shortcut: a sibling repo checkout via $JACKY_SETUP_DEV_REPO_ROOT
...
//! Electron app's APP_ROOT/../.. trick, because Jacky-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either Jacky-Setup's build-time
...
pub async fn resolve(
    ...
    // 1. Dev shortcut.
    if let Ok(repo_root) = std::env::var("JACKY_SETUP_DEV_REPO_ROOT") {
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/jaswanthsai1/jacky-cli/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::new()
        .get(&url)
        .header("User-Agent", "jacky-setup/0.0.1")
        .send()
        .await

...
//!
//! Driven when the installer is launched as `Jacky-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Jacky desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `jacky update`
//!      or repair bootstrap can race locked files),
//!   2. run `jacky update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in jacky_cli/main.py),
//!   3. run `jacky desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `jacky update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_jacky) and
...
/// `jacky update` exit code meaning "another jacky process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// jacky_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `jacky update`'s own guard decide.
...
async fn run_update(app: AppHandle) -> Result<()> {
    let jacky_home = crate::paths::jacky_home();
    let install_root = jacky_home.join("jacky-agent");
    ...
    // it, that backend re-locks the venv shim, our `force_kill_other_jacky`
    ...
    let jacky = resolve_jacky(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the jacky CLI under {}. Is Jacky installed? \
    ...
    // async on Windows. If it still holds the venv shim, `jacky update`
    ...
    // ---- stage 2: jacky update -----------------------------------------
    // Pass --branch so `jacky update` targets the branch this installer was
    ...
    // without --branch, `jacky update` switches the checkout to `main` (a
    ...
    // --force skips `jacky update`'s Windows running-exe guard (which would
    ...
    // time `jacky update` runs there is no legitimate jacky.exe to protect,
    // and the guard would only produce a false "Jacky is still running" stop.
    //
    ...
    // probe only checks the jacky.exe shim and app.asar, so an external venv
    ...
    // below surfaces the correct "close all Jacky windows" message.
    ...
    // Retry-once for the update-boundary crash. `jacky update` lazily imports
    ...
    // second `jacky update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Jacky" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Jacky is still running. Close all Jacky windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "jacky update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::jacky_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 3: jacky desktop --build-only ----------------------------
    // `jacky update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `jacky update` already does above, and `jacky update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `jacky desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Jacky manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_jacky_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Jacky to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend jacky.exe (or the desktop Jacky.exe
            ...
            // force-kill every Jacky.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Jacky still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_jacky();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_jacky(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Jacky.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Jacky.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `jacky.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `jacky.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\jacky.exe update`. And a
/// desktop the user relaunches mid-update will NOT have spawned a backend —
/// `startJacky()` in the desktop gates local-backend startup on our
/// update-in-progress marker and parks until we finish (#50238). So the only
/// jacky.exe images here are stragglers from the old desktop — exactly what
/// we want gone. (`/FI PID ne <self>` also spares this Tauri process, though it
/// isn't named jacky.exe.)
fn force_kill_other_jacky() {
    if !cfg!(target_os = "windows") {
        return;
    }
...
/// Spawn `jacky <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv jacky shim under an install root, regardless of existence.
fn venv_jacky(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("jacky.exe")
    } else {
        install_root.join("venv").join("bin").join("jacky")
    }
}

/// Resolve the jacky CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `jacky` on PATH.
fn resolve_jacky(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_jacky(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "jacky.exe" } else { "jacky" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    let jacky_home = crate::paths::jacky_home();
    let mut envs = vec![(
        "JACKY_HOME".to_string(),
        jacky_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        jacky_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_jacky_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Jacky.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.jacky-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.jacky-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::jacky_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_jacky_is_under_install_root() {
        let root = Path::new("/x/jacky-agent");
        let shim = venv_jacky(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_jacky(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/jacky-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn update_marker_guard_writes_then_removes_on_drop() {
        ...
        let marker = dir.join(".jacky-update-in-progress");
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Jacky.app"]),
            Some(PathBuf::from("/Applications/Jacky.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Jacky.app");
        let tmp = base.join("Jacky.app.jacky-update-new");
        let old = base.join("Jacky.app.jacky-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Jacky.app");
        let tmp = base.join("Jacky.app.jacky-update-new"); // intentionally absent
        let old = base.join("Jacky.app.jacky-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Jacky.app");
        let tmp = base.join("Jacky.app.jacky-update-new"); // absent
        let old = base.join("Jacky.app.jacky-update-old");
        write_marker(&target, "OLD");

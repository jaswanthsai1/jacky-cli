...
//!
//! Mirrors `jacky_constants.get_jacky_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\jacky
//!   macOS:   ~/.jacky
//!   Linux:   ~/.jacky  (override via $JACKY_HOME)
//!
//! NOTE (macOS): Python's get_jacky_home(), scripts/install.sh, and the
//! Electron desktop's resolveJackyHome() ALL use ~/.jacky on macOS — there
...
/// Returns the canonical Jacky home directory, respecting $JACKY_HOME if set.
pub fn jacky_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("JACKY_HOME") {
    ...
    {
        // %LOCALAPPDATA%\jacky — matches scripts/install.ps1's $JackyHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("jacky");
        }
    }

    // macOS + Linux + fallback: ~/.jacky (matches Python get_jacky_home(),
    // install.sh, and the Electron desktop's resolveJackyHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".jacky");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    jacky_home().join("bootstrap-cache")
}
...
/// JACKY_HOME so it survives repo checkout deletion (unlike anything under
/// jacky-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\jacky\jacky-setup.exe`; on other
...
/// mid-update re-locks the venv shim and triggers `force_kill_other_jacky`,
/// which then kills that legitimate backend in a respawn loop (#50238).
///
/// Lives directly under JACKY_HOME (same rationale as `installer_dest`) so the
/// Electron desktop — which resolves JACKY_HOME identically and pins it into
/// the updater's env — agrees on the exact path.
pub fn update_in_progress_marker() -> PathBuf {
    jacky_home().join(".jacky-update-in-progress")
}
...
pub fn copy_self_to_jacky_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to JACKY_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_JACKY_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".jacky-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under JACKY_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[jacky-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("JACKY_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_jacky_home() -> String {
    jacky_home().to_string_lossy().into_owned()
}

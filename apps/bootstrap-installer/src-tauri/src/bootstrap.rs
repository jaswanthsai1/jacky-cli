...
//! JACKY_HOME/logs/bootstrap-<timestamp>.log.
//!
//! Lifecycle:
...
pub struct StartBootstrapArgs {
    ...
    /// Optional override for JACKY_HOME. Tests use this; production
    /// almost always falls back to the OS default.
    pub jacky_home: Option<String>,
}
...
/// Spawn the locally-built Jacky desktop binary, then close the installer
...
#[tauri::command]
pub async fn launch_jacky_desktop(
    app: AppHandle,
    install_root: String,
) -> Result<(), String> {
    let install_root = PathBuf::from(install_root);
    let exe_path = resolve_jacky_desktop_exe(&install_root).ok_or_else(|| {
        format!(
            "Couldn't find a built Jacky desktop at {}. The desktop build step \
             may have been skipped or failed. Run `jacky desktop` from a \
             terminal to build and launch it.",
            install_root.join("apps").join("desktop").join("release").display()
        )
    })?;

    tracing::info!(?exe_path, "launching Jacky desktop");

    // Detach from us — the installer is about to exit. On macOS launch the
    // bundle through LaunchServices instead of exec'ing Contents/MacOS/Jacky
...
/// <os>-unpacked/<exe>).
pub(crate) fn resolve_jacky_desktop_exe(install_root: &std::path::Path) -> Option<PathBuf> {
    let release_dir = install_root.join("apps").join("desktop").join("release");
    let candidates: &[(&str, &str)] = if cfg!(target_os = "windows") {
        &[
            ("win-unpacked", "Jacky.exe"),
            ("win-arm64-unpacked", "Jacky.exe"),
        ]
    } else if cfg!(target_os = "macos") {
        &[
            ("mac/Jacky.app/Contents/MacOS", "Jacky"),
            ("mac-arm64/Jacky.app/Contents/MacOS", "Jacky"),
        ]
    } else {
        &[("linux-unpacked", "jacky")]
    };
...
pub(crate) fn resolve_jacky_desktop_app(install_root: &std::path::Path) -> Option<PathBuf> {
    let exe = resolve_jacky_desktop_exe(install_root)?;
    #[cfg(target_os = "macos")]
    {
        // .../Jacky.app/Contents/MacOS/Jacky -> .../Jacky.app
...
/// path so a bare re-open just opens Jacky instead of re-running setup.
pub(crate) fn jacky_is_installed(install_root: &std::path::Path) -> bool {
    install_root.join(".jacky-bootstrap-complete").exists()
        && resolve_jacky_desktop_exe(install_root).is_some()
}
...
pub(crate) fn spawn_installed_desktop(install_root: &std::path::Path) -> std::io::Result<()> {
    let exe = resolve_jacky_desktop_exe(install_root).ok_or_else(|| {
        std::io::Error::new(std::io::ErrorKind::NotFound, "no built Jacky desktop app")
    })?;
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        // installer exits, mirroring launch_jacky_desktop. Kept correct here
...
pub(crate) fn open_macos_app_detached(app_bundle: &std::path::Path) -> std::io::Result<()> {
    ...
    cmd.arg(app_bundle);
    cmd.current_dir(crate::paths::jacky_home());
    cmd.spawn().map(|_child| ())
}
...
async fn run_bootstrap(
    ...
    // explicitly; we infer it from $JackyHome which Stage-Repository clones
    // the repo INTO at $JackyHome\jacky-agent. Mirrors jacky_constants.
    let jacky_home = args
        .jacky_home
        .clone()
        .unwrap_or_else(|| crate::paths::jacky_home().to_string_lossy().into_owned());
    let install_root = PathBuf::from(&jacky_home).join("jacky-agent");

    // Copy ourselves to JACKY_HOME/jacky-setup.exe so the desktop app can
    ...
    if let Err(err) = crate::paths::copy_self_to_jacky_home() {
        tracing::warn!(?err, "failed to copy installer into JACKY_HOME (non-fatal)");
        emit_log(&format!(
...
async fn run_install_script(
    ...
    args: &[String],
    jacky_home_override: Option<&str>,
    ...
    powershell::run_script(script_path, args, sink, jacky_home_override, cancel_rx)
        .await
        .map_err(|e| {
...
mod tests {
    ...
    fn make_release_tree(install_root: &Path) -> PathBuf {
        ...
        if cfg!(target_os = "macos") {
            ...
            std::fs::write(macos_dir.join("Jacky"), b"#!/bin/sh\n").unwrap();
            macos_dir.parent().unwrap().parent().unwrap().to_path_buf() // .../Jacky.app
        } else if cfg!(target_os = "windows") {
            let dir = release.join("win-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("Jacky.exe");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        } else {
            let dir = release.join("linux-unpacked");
            std::fs::create_dir_all(&dir).unwrap();
            let exe = dir.join("jacky");
            std::fs::write(&exe, b"stub").unwrap();
            exe
        }
    }
    ...
    // what the updater ditto's over /Applications/Jacky.app). A regression in
    // this derivation breaks the post-update auto-relaunch, so guard it.
    #[test]
    fn resolve_jacky_desktop_app_finds_built_bundle() {
        let root = unique_tmp_dir("app-ok");
        let expected = make_release_tree(&root);

        let resolved = resolve_jacky_desktop_app(&root)
    ...
    #[test]
    fn resolve_jacky_desktop_app_is_none_without_a_build() {
        let root = unique_tmp_dir("app-none");
        // No release tree created.
        assert!(
            resolve_jacky_desktop_app(&root).is_none(),

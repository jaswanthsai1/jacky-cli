...
export const $jackyHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  if (fake) {
    unlisten = () => {}
    $logPath.set('~/.jacky/logs/bootstrap-installer.log')
    $jackyHome.set('~/.jacky')
  ...
  try {
    const [logPath, jackyHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_jacky_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $jackyHome.set(jackyHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Jacky" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  ...
  // Update is driven by the desktop handing off (Jacky-Setup.exe --update);
...
export async function launchJackyDesktop(): Promise<void> {
  ...
  await invoke('launch_jacky_desktop', { installRoot })
}
...
const FAKE_INSTALL_STAGES: FakeStage[] = [
  ...
  { name: 'repo', title: 'Jacky repository' },

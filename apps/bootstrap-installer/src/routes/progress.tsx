...
export default function ProgressScreen({ bootstrap }: ProgressProps) {
  ...
  const title = bootstrap.status === 'completed' ? 'Done' : isUpdate ? 'Updating Jacky' : 'Setting up Jacky Agent'
  const description = isUpdate
    ? 'Jacky is updating to the latest version — this only takes a moment.'
    : 'This is a one-time setup. The Jacky installer is downloading dependencies and configuring your machine. Subsequent launches will skip this step.'
  const pct = Math.round(progress.fraction * 100)

  return (
    <div className="jacky-fade-in flex h-full flex-col">

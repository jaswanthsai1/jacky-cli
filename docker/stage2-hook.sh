...
#
# Wired into the image as /etc/cont-init.d/01-jacky-setup by the
...
JACKY_HOME="${JACKY_HOME:-/opt/data}"
INSTALL_DIR="/opt/jacky"

# Drop to jacky via s6-setuidgid, but skip it when already non-root.
as_jacky() { [ "$(id -u)" = 0 ] || { "$@"; return; }; s6-setuidgid jacky "$@"; }
...
# starts non-root. The baked install tree under /opt/jacky is intentionally
# root-owned and non-writable; mutable runtime state must live under
# $JACKY_HOME. An arbitrary `--user` UID therefore cannot repair or populate
...
# default) and pass JACKY_UID/JACKY_GID — or the PUID/PGID aliases — which the
...
# `id -u jacky` is the unremapped build UID (10000) because no root-only remap
# could run. root starts (id -u = 0) and the normal supervised drop to the
# jacky UID are both unaffected.
cur_uid="$(id -u)"
if [ "$cur_uid" != 0 ] && [ "$cur_uid" != "$(id -u jacky)" ]; then
    cat >&2 <<EOF
[stage2] ERROR: container started with --user $cur_uid (an arbitrary, non-jacky UID).
...
and the baked /opt/jacky install tree is intentionally root-owned and
...
Start the container as root (the default) and pass your host UID/GID instead:
    ...
    docker run -e JACKY_UID=\$(id -u) -e JACKY_GID=\$(id -g) ...

NAS users (Synology / unRAID / UGOS) can use the PUID/PGID aliases:

    docker run -e PUID=\$(id -u) -e PGID=\$(id -g) ...

The image remaps the jacky user to that UID/GID at boot and chowns the data
...
# --- Bootstrap JACKY_HOME as root ---
# Create the directory (and any missing parents) while we still have root
# privileges so the chown checks below see real metadata and the later
# `s6-setuidgid jacky mkdir -p` block doesn't EACCES on root-owned
# ancestors. Without this, custom JACKY_HOME paths whose parents only
# root can create (e.g. `JACKY_HOME=/home/jacky/.jacky` in a Compose
...
# --- UID/GID remap ---
# Accept PUID/PGID as aliases for JACKY_UID/JACKY_GID.  NAS users (UGOS,
...
# UID 10000 leaves the runtime unable to read the volume.  JACKY_UID/
# JACKY_GID still win when both are set.  See #15290, salvages #25872.
JACKY_UID="${JACKY_UID:-${PUID:-}}"
JACKY_GID="${JACKY_GID:-${PGID:-}}"

if [ -n "${JACKY_UID:-}" ] && validate_uid_gid "$JACKY_UID" && [ "$JACKY_UID" != "$(id -u jacky)" ]; then
    echo "[stage2] Changing jacky UID to $JACKY_UID"
    usermod -u "$JACKY_UID" jacky
fi
if [ -n "${JACKY_GID:-}" ] && validate_uid_gid "$JACKY_GID" && [ "$JACKY_GID" != "$(id -g jacky)" ]; then
    echo "[stage2] Changing jacky GID to $JACKY_GID"
    # -o allows non-unique GID (e.g. macOS GID 20 "staff" may already
    # exist as "dialout" in the Debian-based container image).
    groupmod -o -g "$JACKY_GID" jacky 2>/dev/null || true
fi
...
# host's `docker` group (or root). The supervised jacky user (UID 10000)
...
# jacky process with `Groups: 10000` (998 gone); after this hook adds
...
# matching entry that includes jacky. Idempotent across container
...
#   - jacky is already a member of the right group (idempotent restart)
...
for sock in /var/run/docker.sock /run/docker.sock; do
    ...
    if id -G jacky 2>/dev/null | tr ' ' '\n' | grep -qx "$sock_gid"; then
        echo "[stage2] jacky already in group $sock_gid for $sock"
        break
    fi
    ...
    if usermod -aG "$sock_group" jacky 2>/dev/null; then
        echo "[stage2] Added jacky to group $sock_group (GID $sock_gid) for $sock"
    else
        echo "[stage2] Warning: usermod -aG $sock_group jacky failed; docker backend may fail with EACCES"
    fi
    break
done

# --- Fix ownership of data volume ---
# When JACKY_UID is remapped or the top-level $JACKY_HOME isn't owned by
# the runtime jacky UID, restore ownership to jacky — but ONLY for the
# directories jacky actually writes to. The full $JACKY_HOME may be a
# host-mounted bind containing unrelated user files; `chown -R` would
# silently destroy host ownership of those (see issue #19788).
#
# The canonical list of jacky-owned subdirs is the same one the s6-setuidgid
# mkdir -p block below seeds. Keep them in sync if the seed list changes.
actual_jacky_uid=$(id -u jacky)
...
chown_jacky_tree() {
    ...
    chown -R jacky:jacky "$target" 2>/dev/null || \
        echo "[stage2] Warning: chown $target failed (rootless container?) — continuing"
}

needs_chown=false
if [ "$(stat -c %u "$JACKY_HOME" 2>/dev/null)" != "$actual_jacky_uid" ]; then
    needs_chown=true
fi
if [ "$needs_chown" = true ]; then
    echo "[stage2] Fixing ownership of $JACKY_HOME (targeted) to jacky ($actual_jacky_uid)"
    # In rootless Podman the container's "root" is mapped to an
    # unprivileged host UID — chown will fail. That's fine: the volume
    # is already owned by the mapped user on the host side.
    #
    # Top-level $JACKY_HOME: chown the directory itself (not its contents)
    # so jacky can mkdir new subdirs but bind-mounted host files keep
    # their existing ownership.
    if refuse_symlinked_path "chown" "$JACKY_HOME"; then
        :
    else
        chown jacky:jacky "$JACKY_HOME" 2>/dev/null || \
            echo "[stage2] Warning: chown $JACKY_HOME failed (rootless container?) — continuing"
    fi
    # Jacky-owned subdirs: recursive chown is safe here because these are
    # created and managed exclusively by jacky (see the s6-setuidgid mkdir
    ...
    for sub in cron sessions logs hooks memories skills skins plans workspace home profiles pairing platforms/pairing lazy-packages; do
        if [ -e "$JACKY_HOME/$sub" ]; then
            chown_jacky_tree "$JACKY_HOME/$sub"
        fi
    done
fi
...
# jacky user. Hosted/container instances keep mutable state under
# $JACKY_HOME (/opt/data) and run with PYTHONDONTWRITEBYTECODE plus
# JACKY_DISABLE_LAZY_INSTALLS=1. Keeping /opt/jacky root-owned and
...
# $JACKY_HOME/lazy-packages dir on the data volume (Dockerfile sets
# JACKY_LAZY_INSTALL_TARGET). That dir is appended to the END of sys.path,
...
# even though installs are re-enabled. The dir is seeded + chowned to jacky
...
# Always reset ownership of $JACKY_HOME/profiles to jacky on every
# boot. Profile dirs and files can land owned by root when commands
# are invoked via `docker exec <container> jacky …` (which defaults
# to root unless `-u` is passed), and that breaks the cont-init
# reconciler (02-reconcile-profiles) which runs as jacky and walks
# the profiles dir. Idempotent; skipped on rootless containers where
# chown would fail.
if [ -d "$JACKY_HOME/profiles" ]; then
    chown_jacky_tree "$JACKY_HOME/profiles"
fi

# Always reset ownership of $JACKY_HOME/cron on every boot for the same
# docker-exec/root-write reason as profiles/. The cron scheduler state
# (jobs.json) must stay readable by the unprivileged jacky runtime even
# after root-context maintenance commands or scheduler writes.
if [ -d "$JACKY_HOME/cron" ]; then
    chown_jacky_tree "$JACKY_HOME/cron"
fi
...
# jacky pairing approve …` defaults to uid=0 and writes 0600 root-owned
# approval files that the unprivileged jacky gateway cannot read,
# silently leaving the approved user unauthorized (#10270). The targeted
# data-volume chown above only runs when the top-level $JACKY_HOME is
...
# is negligible.
if [ -d "$JACKY_HOME/platforms/pairing" ]; then
    chown_jacky_tree "$JACKY_HOME/platforms/pairing"
fi
# Legacy location (pre-consolidated layout).
if [ -d "$JACKY_HOME/pairing" ]; then
    chown_jacky_tree "$JACKY_HOME/pairing"
fi

# Reset ownership of jacky-owned top-level state files on every boot.
# The targeted data-volume chown above only covers jacky-owned
# *subdirectories*; loose state files living directly under $JACKY_HOME
# are missed. When those files are created or rewritten by
# `docker exec <container> jacky …` (root unless `-u` is passed) they
# land root-owned, and the unprivileged jacky runtime then hits
...
# sweep so host-owned files in a bind-mounted $JACKY_HOME are never
...
# entries of jacky_cli.profile_distribution.USER_OWNED_EXCLUDE plus the
...
for f in \
    ...
    active_profile; do
    if [ -e "$JACKY_HOME/$f" ]; then
        if refuse_symlinked_path "chown" "$JACKY_HOME/$f"; then
            :
        else
            chown jacky:jacky "$JACKY_HOME/$f" 2>/dev/null || true
        fi
    fi
done

# --- config.yaml permissions ---
# Ensure config.yaml is readable by the jacky runtime user even if it
# was edited on the host after initial ownership setup.
if [ -f "$JACKY_HOME/config.yaml" ]; then
    if refuse_symlinked_path "chown/chmod" "$JACKY_HOME/config.yaml"; then
        :
    else
        chown jacky:jacky "$JACKY_HOME/config.yaml" 2>/dev/null || true
        chmod 640 "$JACKY_HOME/config.yaml" 2>/dev/null || true
    fi
fi

# --- Seed directory structure as jacky user ---
# Run as jacky via s6-setuidgid so dirs end up owned correctly (matters
# under rootless Podman where chown back to root would fail).
#
# Use direct `mkdir -p` invocation (no `sh -c "..."` wrapper) so the
# shell isn't a second interpreter — defends against $JACKY_HOME values
...
as_jacky mkdir -p \
    ...
    "$JACKY_HOME/sessions" \
    "$JACKY_HOME/logs" \
    "$JACKY_HOME/logs/gateways" \
    "$JACKY_HOME/hooks" \
    "$JACKY_HOME/memories" \
    "$JACKY_HOME/skills" \
    "$JACKY_HOME/skins" \
    "$JACKY_HOME/plans" \
    "$JACKY_HOME/workspace" \
    "$JACKY_HOME/home" \
    "$JACKY_HOME/pairing" \
    "$JACKY_HOME/platforms/pairing" \
    "$JACKY_HOME/lazy-packages"

# --- Install-method stamp ---
# The 'docker' stamp is baked into the immutable install tree at
# /opt/jacky/.install_method (see Dockerfile), NOT written here into
# $JACKY_HOME. detect_install_method() reads the code-scoped stamp first.
#
# Why we no longer stamp $JACKY_HOME: it is a shared DATA volume, commonly
# bind-mounted from the host (~/.jacky:/opt/data) and sometimes shared with a
...
# 'jacky update'. To heal homes already poisoned by older images, remove a
# stale 'docker' stamp from $JACKY_HOME if one is present (the host install's
# own installer re-creates its code-scoped stamp; a genuine container relies on
# the baked /opt/jacky stamp, so deleting the data-dir copy is safe).
if [ -f "$JACKY_HOME/.install_method" ]; then
    stamped="$(tr -d '[:space:]' < "$JACKY_HOME/.install_method" 2>/dev/null || true)"
    if [ "$stamped" = "docker" ]; then
        rm -f "$JACKY_HOME/.install_method" 2>/dev/null || true
    fi
fi
...
seed_one() {
    dest=$1
    src=$2
    if [ ! -f "$JACKY_HOME/$dest" ] && [ -f "$INSTALL_DIR/$src" ]; then
        if refuse_symlinked_path "seed" "$JACKY_HOME/$dest"; then
            :
        else
            as_jacky cp "$INSTALL_DIR/$src" "$JACKY_HOME/$dest"
        fi
    fi
}
...
if [ -f "$JACKY_HOME/.env" ]; then
    if refuse_symlinked_path "chown/chmod" "$JACKY_HOME/.env"; then
        :
    else
        chown jacky:jacky "$JACKY_HOME/.env" 2>/dev/null || true
        chmod 600 "$JACKY_HOME/.env" 2>/dev/null || true
    fi
fi

# --- Migrate persisted config schema ---
# Docker image upgrades replace the code under $INSTALL_DIR but preserve
# $JACKY_HOME on the mounted volume. Run the same safe, non-interactive
# config-schema migrations that `jacky update` runs for non-Docker installs,
# after first-boot seeding and before supervised gateway services start.
# Set JACKY_SKIP_CONFIG_MIGRATION=1 for controlled/manual migrations.
if [ -f "$JACKY_HOME/config.yaml" ]; then
    s6-setuidgid jacky "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/scripts/docker_config_migrate.py" \
...
if [ ! -f "$JACKY_HOME/auth.json" ] && [ -n "${JACKY_AUTH_JSON_BOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "seed" "$JACKY_HOME/auth.json"; then
        :
    else
        printf '%s' "$JACKY_AUTH_JSON_BOOTSTRAP" > "$JACKY_HOME/auth.json"
        chown jacky:jacky "$JACKY_HOME/auth.json" 2>/dev/null || true
        chmod 600 "$JACKY_HOME/auth.json"
    fi
fi
...
# supply a freshly-issued session via JACKY_AUTH_JSON_REBOOTSTRAP (distinct
...
if [ -f "$JACKY_HOME/auth.json" ] && [ -n "${JACKY_AUTH_JSON_REBOOTSTRAP:-}" ]; then
    if refuse_symlinked_path "reseed" "$JACKY_HOME/auth.json"; then
        :
    else
        s6-setuidgid jacky "$INSTALL_DIR/.venv/bin/python" \
            "$INSTALL_DIR/scripts/docker_rebootstrap_nous_session.py" \
            "$JACKY_HOME/auth.json" \
...
# can set JACKY_GATEWAY_BOOTSTRAP_STATE=running; we seed the state file
...
# JACKY_AUTH_JSON_BOOTSTRAP seeds auth.json. The [ ! -f ] guard is the
...
if [ ! -f "$JACKY_HOME/gateway_state.json" ] && \
        [ "${JACKY_GATEWAY_BOOTSTRAP_STATE:-}" = "running" ]; then
    if refuse_symlinked_path "seed" "$JACKY_HOME/gateway_state.json"; then
        :
    else
        printf '{"gateway_state":"running"}\n' > "$JACKY_HOME/gateway_state.json"
        chown jacky:jacky "$JACKY_HOME/gateway_state.json" 2>/dev/null || true
        chmod 644 "$JACKY_HOME/gateway_state.json"
    fi
fi
...
if [ -d "$INSTALL_DIR/skills" ]; then
    as_jacky "$INSTALL_DIR/.venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" \
        || echo "[stage2] Warning: skills_sync.py failed; continuing"
fi

# --- Discover agent-browser's Chromium binary ---
# The image's Dockerfile runs `npx playwright install chromium`, which
# populates ``$PLAYWRIGHT_BROWSERS_PATH`` (=/opt/jacky/.playwright) with
# a ``chromium_headless_shell-<build>/chrome-headless-shell-linux64/``
# directory. agent-browser (the runtime CLI Jacky spawns for the
...
# main-wrapper.sh propagates it into the supervised ``jacky`` process
...
if [ -z "${AGENT_BROWSER_EXECUTABLE_PATH:-}" ] && \
    ...
    if [ -n "$browser_bin" ]; then
        ...
        # up for all supervised services (main-jacky, dashboard, etc.).

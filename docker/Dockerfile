...
# Do not write .pyc files at runtime: /opt/jacky is immutable in the
...
ENV PLAYWRIGHT_BROWSERS_PATH=/opt/jacky/.playwright

# Install system dependencies in one layer, clear APT cache.
# tini was previously PID 1 to reap orphaned zombie processes (MCP stdio
# subprocesses, git, bun, etc.) that would otherwise accumulate when jacky
# ran as PID 1. See #15012. Phase 2 of the s6-overlay supervision plan
# replaces tini with s6-overlay's /init (PID 1 = s6-svscan), which reaps
# zombies non-blockingly on SIGCHLD and additionally supervises the main
# jacky process, the dashboard, and per-profile gateways.
RUN apt-get update && \
...
# ---------- s6-overlay install ----------
# s6-overlay provides supervision for the main jacky process, the dashboard,
...
RUN set -eu; \
    ...
    # 'Jacky WebUI' catalog). The image has moved to s6-overlay /init\
...
# Non-root user for runtime; UID can be overridden via JACKY_UID at runtime
RUN useradd -u 10000 -m -d /opt/data jacky
...
#
# ui-tui/packages/jacky-ink/ is copied IN FULL (not just its manifests)
...
COPY ui-tui/packages/jacky-ink/ ui-tui/packages/jacky-ink/
# apps/shared/ is copied IN FULL because web/package.json references it as a
# `file:` workspace dependency (same pattern as jacky-ink above).
...
# lock on the @jacky/ink entry, tripped the TUI launcher's
...
# for the same reason: it lazy-installs into /opt/jacky/.venv at first
...
# gives the non-root jacky user read + traverse but no write; root retains
# write so the build steps below don't need chmod u+w dances.
COPY --link --chmod=a+rX,go-w . .

# ---------- Permissions ----------
# Link jacky-agent itself (editable). Deps are already installed in the
# cached layer above; `--no-deps` makes this a fast egg-link creation with no
# resolution or downloads.
RUN uv pip install --no-cache-dir --no-deps -e "."

# Wire the exec shim and install-method stamp.  Files under /opt/jacky are
# already root-owned (COPY, uv sync, npm install all run as root) and
# read-only for the jacky user (go-w from the --chmod above).

USER root
RUN mkdir -p /opt/jacky/bin && \
    cp /opt/jacky/docker/jacky-exec-shim.sh /opt/jacky/bin/jacky && \
    chmod 0755 /opt/jacky/bin/jacky && \
    printf 'docker\n' > /opt/jacky/.install_method
# The ``.install_method`` stamp is baked next to the running code (the install
# tree), NOT into $JACKY_HOME. $JACKY_HOME (/opt/data) is a shared data
# volume that is commonly bind-mounted from the host and even shared with a
# host-side Desktop/CLI install; stamping it at boot used to clobber that
# host install's marker and wrongly block its ``jacky update``. A code-scoped
# stamp is read first by detect_install_method() and is immune to the share.
# Start as root so the s6-overlay stage2 hook can usermod/groupmod and chown
# the data volume. Each supervised service then drops to the jacky user via
# `s6-setuidgid jacky` in its run script. If JACKY_UID is unset, services
# run as the default jacky user (UID 10000).

# ---------- Bake build-time git revision ----------
# .dockerignore excludes .git, so `git rev-parse HEAD` from inside the
# container always returns nothing — meaning `jacky dump` reports
# "(unknown)" and the startup banner drops its `· upstream <sha>` suffix.
# That makes support triage from container bug reports impossible:
# we can't tell which commit the user is actually running.
#
# Fix: write the commit SHA passed via the JACKY_GIT_SHA build-arg to
# /opt/jacky/.jacky_build_sha at build time, and have
# jacky_cli/build_info.py read it at runtime.  Both `jacky dump` and
...
ARG JACKY_GIT_SHA=
RUN if [ -n "${JACKY_GIT_SHA}" ]; then \
        printf '%s\n' "${JACKY_GIT_SHA}" > /opt/jacky/.jacky_build_sha; \
    fi

# ---------- s6-overlay service wiring ----------
# Static services declared at build time: main-jacky + dashboard.
...
# `exec jacky`. Wired in as cont-init.d/01- so it
# runs before user services start.
#
# 02-reconcile-profiles re-creates per-profile gateway s6 service
# slots from $JACKY_HOME/profiles/<name>/ after a container restart
# (the /run/service/ scandir is tmpfs and wiped on restart). Phase 4.
RUN mkdir -p /etc/cont-init.d && \
    printf '#!/command/with-contenv sh\nexec /opt/jacky/docker/stage2-hook.sh\n' \
        > /etc/cont-init.d/01-jacky-setup && \
    chmod +x /etc/cont-init.d/01-jacky-setup
COPY --chmod=0755 docker/cont-init.d/015-supervise-perms /etc/cont-init.d/015-supervise-perms
COPY --chmod=0755 docker/cont-init.d/02-reconcile-profiles /etc/cont-init.d/02-reconcile-profiles

# ---------- Runtime ----------
ENV JACKY_WEB_DIST=/opt/jacky/jacky_cli/web_dist
# Point the TUI launcher at the prebuilt bundle baked at build time (Layer 8:
# `ui-tui && npm run build`). This makes _make_tui_argv take the prebuilt-bundle
# fast path (`node --expose-gc /opt/jacky/ui-tui/dist/entry.js`) and skip the
...
ENV JACKY_TUI_DIR=/opt/jacky/ui-tui
ENV JACKY_HOME=/opt/data
ENV JACKY_WRITE_SAFE_ROOT=/opt/data
ENV JACKY_DISABLE_LAZY_INSTALLS=1
# The published image seals /opt/jacky (root-owned, read-only) so a runtime
...
# is seeded + chowned to the jacky user by docker/stage2-hook.sh and lives
...
ENV JACKY_LAZY_INSTALL_TARGET=/opt/data/lazy-packages

# `docker exec` privilege-drop shim. When operators run
# `docker exec <c> jacky ...` they default to root, and any file the
# command writes under $JACKY_HOME (auth.json, .env, config.yaml) ends
# up root-owned and unreadable to the supervised gateway (UID 10000).
# The shim lives at /opt/jacky/bin/jacky, sits earliest on PATH, and
# transparently re-exec's the real venv binary via `s6-setuidgid jacky`
# when invoked as root. Non-root callers (supervised processes,
# `--user jacky`, etc.) hit the short-circuit path with no overhead.
# Recursion is impossible because the shim exec's the venv binary by
# absolute path (/opt/jacky/.venv/bin/jacky). See the shim source for
# the opt-out env var (JACKY_DOCKER_EXEC_AS_ROOT=1).

# Pre-s6 entrypoint.sh did `source .venv/bin/activate` which exported
# the venv bin onto PATH; Architecture B's main-wrapper.sh does the
# same for the container's main process, but `docker exec` and our
# cont-init.d scripts don't pass through the wrapper. Expose the venv
# bin globally so `docker exec <container> jacky ...` and any
# subprocess that doesn't activate the venv first still find jacky.
#
# /opt/jacky/bin is prepended ahead of the venv so the privilege-drop
# shim wins PATH resolution. The shim's last act is to exec the venv
# binary by absolute path, so this PATH ordering is transparent to
# every other consumer.
ENV PATH="/opt/jacky/bin:/opt/jacky/.venv/bin:/opt/data/.local/bin:${PATH}"
...
#
# main-wrapper.sh handles arg routing (bare-exec vs. jacky
# subcommand vs. no-args), drops to the jacky user via s6-setuidgid,
# and exec's the final program so its exit code becomes the container
# exit code. Without the wrapper-as-ENTRYPOINT, leading-dash args
# like `--version` would be intercepted by /init's POSIX shell.
ENTRYPOINT [ "/init", "/opt/jacky/docker/main-wrapper.sh" ]
CMD [ ]

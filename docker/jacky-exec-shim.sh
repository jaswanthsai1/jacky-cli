#!/bin/sh
# shellcheck shell=sh
# /opt/jacky/bin/jacky — `docker exec` privilege-drop shim.
#
# Background
# ----------
# The s6 image runs the supervised gateway/main process as the unprivileged
# `jacky` user (UID 10000). When an operator runs `docker exec <c> jacky ...`
# the default UID is root (0), and any file the command writes under
# $JACKY_HOME — auth.json, .env, config.yaml — ends up root-owned and
# unreadable to the supervised gateway. The most common manifestation: the
# user runs `docker exec <c> jacky login`, this writes
# /opt/data/auth.json as root:root mode 0600, and from then on the gateway
# returns "Provider authentication failed: Jacky is not logged into Nous
# Portal" on every incoming message — even though `docker exec <c> jacky
...
# Fix
# ---
# This shim sits at /opt/jacky/bin/jacky and is placed earliest on PATH.
# When invoked as root, it drops to the jacky user (via s6-setuidgid)
# before exec'ing the real venv binary, so anything that writes under
# $JACKY_HOME is uid-aligned with the supervised processes. When invoked
# as any non-root UID — including the supervised processes themselves,
# `docker exec --user jacky`, kanban subagents, etc. — it short-circuits
...
# (/opt/jacky/.venv/bin/jacky), so the second hop cannot re-enter this
# shim regardless of PATH state. No sentinel env var needed.
#
# Opt-out: set JACKY_DOCKER_EXEC_AS_ROOT=1 (1/true/yes, case-insensitive)
...
# state via the jacky CLI. Default is to drop.

set -e

REAL=/opt/jacky/.venv/bin/jacky

# Defensive: if the venv binary is missing (corrupted image, partial
# install), fail loudly rather than silently masking it.
if [ ! -x "$REAL" ]; then
    echo "jacky-shim: $REAL not found or not executable" >&2
    exit 127
fi

# Already non-root? Just exec the real binary. This is the hot path for
# supervised processes (uid 10000) and for `docker exec --user jacky`.
if [ "$(id -u)" != "0" ]; then
    exec "$REAL" "$@"
fi

# Root, with opt-out set? Honor it.
case "${JACKY_DOCKER_EXEC_AS_ROOT:-}" in
    1|true|TRUE|True|yes|YES|Yes)
        exec "$REAL" "$@"
        ;;
esac

# Root, no opt-out. Drop to the jacky user.
#
...
if [ ! -x "$S6_SUID" ]; then
    ...
    echo "jacky-shim: $S6_SUID not found; refusing to silently run as root." >&2
    echo "jacky-shim: re-run with --user jacky or set JACKY_DOCKER_EXEC_AS_ROOT=1." >&2
    exit 126
fi

# Reset HOME to the jacky user's home before dropping privileges. Without

...
class GatewayConfig:
    ...
    # Storage paths
    sessions_dir: Path = field(default_factory=lambda: get_jacky_home() / "sessions")
    ...
    # gateway behaves exactly as before — single JACKY_HOME, no profile stamping.
...
def load_gateway_config() -> GatewayConfig:
    ...
    2. ~/.jacky/config.yaml (primary user-facing config)
    3. ~/.jacky/gateway.json (legacy — provides defaults under config.yaml)
    4. Built-in defaults
    """
    _home = get_jacky_home()
    gw_data: dict = {}
    ...
    try:
        ...
        if config_yaml_path.exists():
            ...
            # through jacky_cli.config.load_config, so without this a managed
            ...
            if isinstance(gateway_section, dict):
                if "multiplex_profiles" in gateway_section and "multiplex_profiles" not in gw_data:
                    # gateway.multiplex_profiles written by `jacky config set gateway.multiplex_profiles true`
            ...
            if not isinstance(streaming_cfg, dict):
                ...
                # ``jacky config set gateway.streaming.*``
                streaming_cfg = (

...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.jacky to the active profile home."""
    ...
    if raw == "~/.jacky":
        return jacky_home
    if raw.startswith("~/.jacky/"):
        return jacky_home / raw.removeprefix("~/.jacky/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return jacky_home / path
...
def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    ...
    if raw_text == "~/.jacky" or raw_text.startswith("~/.jacky/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__jacky_ignore__") is True
        ):
            return False, None

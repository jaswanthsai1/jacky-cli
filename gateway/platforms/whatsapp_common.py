...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to JACKY_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/jacky), this function
    mirrors the bridge source to a writable JACKY_HOME location and returns that
    ...
    jacky_home_bridge = jacky_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to JACKY_HOME if needed
    if jacky_home_bridge.exists():
        return jacky_home_bridge

    # Mirror the bridge source to JACKY_HOME
    try:
        jacky_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            jacky_home_bridge,
            dirs_exist_ok=False,

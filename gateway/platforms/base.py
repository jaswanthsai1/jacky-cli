...
# Audio file extensions Jacky recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Jacky' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Jacky-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.jacky/.env manually."
)
...
IMAGE_CACHE_DIR = get_jacky_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_jacky_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_jacky_dir(new_subpath, old_name)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; JackyAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_jacky_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_jacky_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_jacky_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_jacky_dir("cache/screenshots", "browser_screenshots")
...
_JACKY_ROOT = get_default_jacky_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "JACKY_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "JACKY_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "JACKY_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "JACKY_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _JACKY_HOME / "image_cache",
    _JACKY_HOME / "audio_cache",
    _JACKY_HOME / "video_cache",
    _JACKY_HOME / "document_cache",
    _JACKY_HOME / "browser_screenshots",
    ...
    _JACKY_HOME / "cache" / "images",
    _JACKY_HOME / "cache" / "audio",
    _JACKY_HOME / "cache" / "videos",
    _JACKY_HOME / "cache" / "documents",
    _JACKY_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Jacky root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* JACKY_HOME's cache, so a gateway running at
    the root (e.g. ``JACKY_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when JACKY_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _JACKY_ROOT / "profiles"
    try:
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Jacky profile and shared Jacky root both contain control
    ...
    # JACKY_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.jacky
    ...
    for jacky_root in (_JACKY_HOME, _JACKY_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(jacky_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Jacky
    secrets (``~/.jacky/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``JACKY_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Jacky-managed cache, under an operator-allowlisted root
    (``JACKY_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Jacky root (~/.jacky/.env,
    ...
    # ``MEDIA:~/.jacky/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?jacky\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+jacky[.!?\s]*$", re.IGNORECASE),
)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``JACKY_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Jacky commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("JACKY_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "JACKY_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "JACKY_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Jacky sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Jacky home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Jacky home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.jacky/media/generated/stale.png"}
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        if session_key in self._active_sessions:
            ...
            from jacky_cli.commands import should_bypass_active_session
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          JACKY_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          JACKY_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          JACKY_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("JACKY_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("JACKY_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("JACKY_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            # not at the top of this task.  _jacky_run_generation is set on

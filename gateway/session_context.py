"""
Session-scoped context variables for the Jacky gateway.

Replaces the previous ``os.environ``-based session state
(``JACKY_SESSION_PLATFORM``, ``JACKY_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["JACKY_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("JACKY_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("JACKY_SESSION_PLATFORM", "")
...
_SESSION_PLATFORM: ContextVar = ContextVar("JACKY_SESSION_PLATFORM", default=_UNSET)
_SESSION_SOURCE: ContextVar = ContextVar("JACKY_SESSION_SOURCE", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("JACKY_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("JACKY_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("JACKY_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("JACKY_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("JACKY_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("JACKY_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("JACKY_SESSION_ID", default=_UNSET)
# In-process UI session/window id for multi-session desktop/TUI hosts. This is
# intentionally separate from JACKY_SESSION_ID: the latter is the durable
...
_SESSION_UI_SESSION_ID: ContextVar = ContextVar("JACKY_UI_SESSION_ID", default=_UNSET)
# ID of the message that triggered the current turn. Used as a reply anchor
# so background-process notifications stay inside the originating Telegram
# private-chat topic (those lanes route only with thread id + reply anchor).
_SESSION_MESSAGE_ID: ContextVar = ContextVar("JACKY_SESSION_MESSAGE_ID", default=_UNSET)

_SESSION_PROFILE: ContextVar = ContextVar("JACKY_SESSION_PROFILE", default=_UNSET)
...
_SESSION_ASYNC_DELIVERY: ContextVar = ContextVar("JACKY_SESSION_ASYNC_DELIVERY", default=_UNSET)

# Cron auto-delivery vars — set per-job in run_job() so concurrent jobs
# don't clobber each other's delivery targets.
_CRON_AUTO_DELIVER_PLATFORM: ContextVar = ContextVar("JACKY_CRON_AUTO_DELIVER_PLATFORM", default=_UNSET)
_CRON_AUTO_DELIVER_CHAT_ID: ContextVar = ContextVar("JACKY_CRON_AUTO_DELIVER_CHAT_ID", default=_UNSET)
_CRON_AUTO_DELIVER_THREAD_ID: ContextVar = ContextVar("JACKY_CRON_AUTO_DELIVER_THREAD_ID", default=_UNSET)

_VAR_MAP = {
    "JACKY_SESSION_PLATFORM": _SESSION_PLATFORM,
    "JACKY_SESSION_SOURCE": _SESSION_SOURCE,
    "JACKY_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "JACKY_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "JACKY_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "JACKY_SESSION_USER_ID": _SESSION_USER_ID,
    "JACKY_SESSION_USER_NAME": _SESSION_USER_NAME,
    "JACKY_SESSION_KEY": _SESSION_KEY,
    "JACKY_SESSION_ID": _SESSION_ID,
    "JACKY_UI_SESSION_ID": _SESSION_UI_SESSION_ID,
    "JACKY_SESSION_MESSAGE_ID": _SESSION_MESSAGE_ID,
    "JACKY_SESSION_PROFILE": _SESSION_PROFILE,
    "JACKY_CRON_AUTO_DELIVER_PLATFORM": _CRON_AUTO_DELIVER_PLATFORM,
    "JACKY_CRON_AUTO_DELIVER_CHAT_ID": _CRON_AUTO_DELIVER_CHAT_ID,
    "JACKY_CRON_AUTO_DELIVER_THREAD_ID": _CRON_AUTO_DELIVER_THREAD_ID,
}


def set_current_session_id(session_id: str) -> None:
    """Synchronize ``JACKY_SESSION_ID`` across ContextVar and ``os.environ``.
    ...
    ``get_session_env("JACKY_SESSION_ID")`` with an ``os.environ`` fallback,
...
def reset_session_vars() -> None:
    ...
    *A's* ``JACKY_SESSION_*`` identity via the subprocess-env bridge.  The
    ...
    ``JACKY_SESSION_*`` env var read via :func:`get_session_env`), so it is
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``JACKY_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("JACKY_SESSION_*", default)``.

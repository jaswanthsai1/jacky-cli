...
Contract (presence-based, mirroring ``.restart_notify.json``):
  ...
  * begin-drain  → write ``{JACKY_HOME}/.drain_request.json`` with
...
Why the epoch (NS-570). ``JACKY_HOME`` is a **durable** store — on Jacky
...
def current_instantiation_epoch() -> str:
    ...
    Together they discriminate every restart mode that matters:
      ...
      | host ``jacky gateway restart``| same    | same(init) | SAME   | honour |
...
def drain_request_path(home: Optional[Path] = None) -> Path:
    """Absolute path to the drain-request marker, respecting JACKY_HOME."""
    base = home if home is not None else get_jacky_home()
...
def write_drain_request(
    *,
    ...
    later survives a machine restart on the durable JACKY_HOME volume can be
...
def drain_requested(*, home: Optional[Path] = None) -> bool:
    ...
    treated as absent: it survived a container/VM restart (JACKY_HOME is a
    durable Fly volume on Jacky Cloud) and the lifecycle action that triggered
...
def drain_notification_suppressed(*, home: Optional[Path] = None) -> bool:
    ...
    marker that survived a machine restart on the durable JACKY_HOME volume is

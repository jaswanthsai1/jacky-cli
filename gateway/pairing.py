...
PAIRING_DIR = get_jacky_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from jacky_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{JACKY_HOME}/pairing`` while newer code/docs may
    write ``{JACKY_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<JACKY_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<JACKY_HOME>/pairing/``
    directory (backward-compat for the ``jacky pairing`` CLI).
    """
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `jacky` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> jacky pairing approve ...` as root, "
                    "re-run with `docker exec -u jacky <container> ...` and "
                    "chown the existing file to the jacky user, or restart the "

...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``jacky setup`` display).
    ...
    # built-ins).  Used by ``jacky gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `jacky`
        # invocation -- including plain `jacky chat`, which never touches any
        ...
        # `jacky setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``jacky setup``/``gateway status``, channel

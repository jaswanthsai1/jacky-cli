"""Relay/connector support package for the Jacky gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``jacky gateway enroll`` writes them to
    ``~/.jacky/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``JACKY_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``jacky gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``jacky gateway enroll`` CLI. Two modes, in precedence order:
    ...
    if not token_url:
        ...
        from jacky_cli.auth import resolve_nous_access_token
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.jacky/.env``.
    ...
    sets neither ``JACKY_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``jacky gateway enroll``: has a PINNED
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'jacky'}"

...
window before ``jacky update``'s graceful restart fires), a first-time lazy
...
def _fingerprint() -> str | None:
    ...
    ``jacky_cli.main`` is always already imported in a gateway process (it's
    the entry point), so this import is free and avoids duplicating the
    worktree-aware ref resolution.
    """
    try:
        from jacky_cli.main import _read_git_revision_fingerprint

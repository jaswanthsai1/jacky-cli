...
# from shadowing Jacky's own top-level modules.  ``jacky_bootstrap`` lives at
...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `JACKY_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("JACKY_TUI_SIDECAR_URL")
...
# tested; override via ``JACKY_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("JACKY_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``JACKY_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
...
# ``jacky --tui``) imports cleanly there.  SIGBREAK (Windows' Ctrl+Break)
...
def wait_for_mcp_discovery(timeout: "float | None" = None) -> None:
    ...
    CLI path via ``jacky_cli.mcp_startup``); ``timeout`` overrides it.
    """
    thread = _mcp_discovery_thread
    if thread is None or not thread.is_alive():
        return
    try:
        from jacky_cli.mcp_startup import _resolve_discovery_timeout
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``jacky --tui`` path spawns ITS thread here (``_mcp_discovery_thread``),
    while the desktop app + dashboard WebSocket sidecar (``tui_gateway/ws.py``)
    and ``jacky dashboard`` spawn theirs via
    ``jacky_cli.mcp_startup.start_background_mcp_discovery``. The late-refresh
    ...
    if thread is not None and thread.is_alive():
        return True
    try:
        from jacky_cli.mcp_startup import (
...
def join_mcp_discovery(timeout: float | None = None) -> bool:
    ...
    entry thread first, then the ``jacky_cli.mcp_startup`` thread used by the
    ...
    try:
        from jacky_cli.mcp_startup import join_mcp_discovery as _startup_join
...
def main():
    ...
    # server stalled the whole shell on "summoning jacky…" for the full

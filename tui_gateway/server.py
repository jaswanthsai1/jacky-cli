...
load_jacky_dotenv(
    jacky_home=_jacky_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.jacky/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_jacky_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("JACKY_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("JACKY_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("JACKY_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent JackyCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Jacky agent → needs provider credentials.
        ...
        if profile_home:
            ...
            # gateway's launch JACKY_HOME (#40677).
            env["JACKY_HOME"] = str(profile_home)
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from jacky_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from jacky_cli.plugins import invoke_hook
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `jacky --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("JACKY_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `jacky --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from jacky_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its JACKY_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_jacky_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s JACKY_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_jacky_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_jacky_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``jacky dashboard`` was launched) instead of the configured
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `jacky` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # JACKY_HOME so config/skills/model resolve to it, and hand the
            ...
            if home_token is not None:
                reset_jacky_home_override(home_token)
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from jacky_cli.runtime_provider import canonical_custom_identity
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _jacky_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_jacky_home_override()
        home = override if isinstance(override, str) and override else _jacky_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    jacky_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["JACKY_GATEWAY_SESSION"] = "1"
    os.environ["JACKY_EXEC_ASK"] = "1"
...
def resolve_skin() -> dict:
    try:
        from jacky_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("JACKY_MODEL", "")
        or os.environ.get("JACKY_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``JACKY_DESKTOP=1`` and ``JACKY_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``JACKY_DESKTOP_TERMINAL=1`` → "tui"
        (``jacky --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``jacky --tui``.)
    """
    if is_truthy_value(os.environ.get("JACKY_DESKTOP")) and not is_truthy_value(
        os.environ.get("JACKY_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads JACKY_MODEL /
    JACKY_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`jacky --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads JACKY_MODEL /
    # JACKY_INFERENCE_MODEL, which `jacky --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("JACKY_TUI_PROVIDER", "").strip()
    ...
    try:
        from jacky_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("JACKY_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("JACKY_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("JACKY_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Jacky): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `jacky --tui` both land here. See
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] JACKY_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from jacky_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown JACKY_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in JACKY_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid JACKY_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _JACKY_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off jacky-cli's tool universe — can't
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid JACKY_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _apply_model_switch(
    ...
    *,
    ...
    custom_provs = None
    try:
        from jacky_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (JACKY_MODEL /
    # JACKY_INFERENCE_MODEL / JACKY_TUI_PROVIDER / JACKY_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `jacky --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    # JACKY_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("JACKY_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from jacky_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from jacky_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from jacky_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from jacky_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("JACKY_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("JACKY_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``JackyCLI.__init__`` and ``gateway/run.py``:
...
def _resolve_runtime_with_fallback(
    ...
    ``jacky_cli/cli_agent_setup_mixin.py``: when the primary provider
    raises ``AuthError``, walk the configured ``fallback_providers`` /
    ``fallback_model`` chain before giving up.
    """
    from jacky_cli.auth import AuthError
    from jacky_cli.runtime_provider import resolve_runtime_provider
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses jacky_cli.mcp_startup; TUI stdio keeps
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("JACKY_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("JACKY_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("JACKY_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("JACKY_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind JACKY_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `JACKY_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    _enable_gateway_prompts()
    home_token = (
        set_jacky_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        try:
            init_home_token = (
                set_jacky_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_jacky_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds JACKY_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``jacky --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``jacky gateway`` process, whose
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Jacky has to support both the legacy 8-row petdex atlas and the current
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from jacky_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from jacky_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from jacky_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from jacky_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from jacky_cli.pets import set_pet_scale
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("JACKY_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
# (agent/billing_view.py + jacky_cli/nous_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from jacky_cli.nous_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from jacky_cli.nous_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from jacky_cli.nous_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Jacky profile home
    # (~/.jacky/sessions/saved/) rather than the project/workspace CWD, and
...
#
# Layout:  $JACKY_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn JACKY_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_jacky_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from jacky_cli.config import load_config as _tui_load_config
            ...
            if (
                ...
                try:
                    from jacky_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: jacky_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from jacky_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _jacky_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".jacky" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.jacky/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.jacky/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Jacky Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Jacky/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Jacky Desktop. If it serves Jacky/Desktop UI or another unrelated app, stop that process and report failure.",
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from jacky_cli.model_switch import parse_model_flags
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from jacky_cli.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("JACKY_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["JACKY_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("JACKY_YOLO_MODE", None)
                    nv = "0"
...
def _projects_payload(conn) -> dict:
    from jacky_cli import projects_db as pdb
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under JACKY_HOME (~/.jacky by default) — config/sessions/skills,
    ...
    jacky_home = os.path.realpath(str(get_jacky_home()))

    return real == home or real == jacky_home or real.startswith(jacky_home + os.sep)
...
def _discover_repos_payload(db, *, conn=None, backfill: bool = True) -> list[dict]:
    ...
    repos even with zero jacky sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (jacky home subtree + bare
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from jacky_cli.models import list_available_providers, normalize_provider
    ...
    if key == "mtime":
        cfg_path = _jacky_home / "config.yaml"
        try:
            return _ok(
...
def _(rid, params: dict) -> dict:
    try:
        from jacky_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from jacky_cli.runtime_provider import resolve_runtime_provider
        from jacky_cli.auth import has_usable_secret
        from jacky_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Jacky provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.jacky/.env`` into the gateway process via
    ``jacky_cli.config.reload_env``, matching classic CLI's ``/reload``
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from jacky_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `jacky` is interactive — use `/jacky chat -q …` or run `jacky` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`jacky setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`jacky gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`jacky sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`jacky config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m jacky_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "jacky_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=min(int(params.get("timeout", 240)), 600),
            cwd=os.getcwd(),
            # cli.exec runs `python -m jacky_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from jacky_cli.moa_config import moa_usage, normalize_moa_config
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from jacky_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from jacky_cli.config import is_managed, save_env_value
        from jacky_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `jacky model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.jacky/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from jacky_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from jacky_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("JACKY_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("JACKY_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["JACKY_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from jacky_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["JACKY_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from jacky_cli.voice import start_continuous
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from jacky_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from jacky_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from jacky_cli.browser_connect import launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("JACKY_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("JACKY_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_jacky_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``jacky journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from jacky_cli.skills_hub import do_install
        ...
        if action == "browse":
            from jacky_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from jacky_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``jacky plugins`` / the dashboard, so the three surfaces

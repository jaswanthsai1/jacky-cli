"""Tests for jacky_constants module."""
...
from jacky_cli.jacky_constants import (
    ...
    get_default_jacky_root,
    get_jacky_dir,
    get_jacky_home,
    heal_jacky_managed_node,
    jacky_managed_node_tree_present,
    iter_jacky_node_dirs,
    is_container,
    node_tool_runnable,
...
class TestGetDefaultJackyRoot:
    """Tests for get_default_jacky_root() — Docker/custom deployment awareness."""

    def test_no_jacky_home_returns_native(self, tmp_path, monkeypatch):
        """When JACKY_HOME is not set, returns ~/.jacky."""
        monkeypatch.delenv("JACKY_HOME", raising=False)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        assert get_default_jacky_root() == tmp_path / ".jacky"

    def test_jacky_home_is_native(self, tmp_path, monkeypatch):
        """When JACKY_HOME = ~/.jacky, returns ~/.jacky."""
        native = tmp_path / ".jacky"
        native.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(native))
        assert get_default_jacky_root() == native

    def test_jacky_home_is_profile(self, tmp_path, monkeypatch):
        """When JACKY_HOME is a profile under ~/.jacky, returns ~/.jacky."""
        native = tmp_path / ".jacky"
    ...
    def test_jacky_home_is_docker(self, tmp_path, monkeypatch):
        """When JACKY_HOME points outside ~/.jacky (Docker), returns JACKY_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(docker_home))
        assert get_default_jacky_root() == docker_home

    def test_jacky_home_is_custom_path(self, tmp_path, monkeypatch):
        """Any JACKY_HOME outside ~/.jacky is treated as the root."""
        custom = tmp_path / "my-jacky-data"
        custom.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(custom))
        assert get_default_jacky_root() == custom

    def test_docker_profile_active(self, tmp_path, monkeypatch):
        """When a Docker profile is active (JACKY_HOME=<root>/profiles/<name>),
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(profile))
        assert get_default_jacky_root() == docker_root

    def test_no_jacky_home_returns_localappdata_root_on_windows(self, tmp_path, monkeypatch):
        """Native Windows falls back to %LOCALAPPDATA%\\jacky, not ~/.jacky."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("JACKY_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(jacky_constants.sys, "platform", "win32")

        assert get_default_jacky_root() == local_appdata / "jacky"

    def test_no_jacky_home_uses_windows_path_when_localappdata_missing(self, tmp_path, monkeypatch):
        """Windows fallback still uses AppData/Local/jacky without LOCALAPPDATA."""
        home = tmp_path / "Home"
        monkeypatch.delenv("JACKY_HOME", raising=False)
        ...
        assert get_default_jacky_root() == home / "AppData" / "Local" / "jacky"


class TestGetJackyHome:
    """Tests for get_jacky_home() platform-aware fallback."""

    def test_windows_fallback_uses_localappdata(self, tmp_path, monkeypatch):
        """When JACKY_HOME is unset on Windows, use %LOCALAPPDATA%\\jacky."""
        local_appdata = tmp_path / "LocalAppData"
        monkeypatch.delenv("JACKY_HOME", raising=False)
        monkeypatch.setenv("LOCALAPPDATA", str(local_appdata))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "Home")
        monkeypatch.setattr(jacky_constants.sys, "platform", "win32")
        monkeypatch.setattr(jacky_constants, "_profile_fallback_warned", False)

        assert get_jacky_home() == local_appdata / "jacky"


class TestJackyManagedNode:
    def test_windows_node_dir_prefers_portable_root(self, tmp_path, monkeypatch):
        ...
        assert iter_jacky_node_dirs() == [node_dir, bin_dir]
    ...
    def test_windows_finds_npm_cmd_before_path(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(jacky_constants.sys, "platform", "win32")
        monkeypatch.setenv("JACKY_HOME", str(home))
        monkeypatch.setattr(jacky_constants, "node_tool_runnable", lambda path: True)

        assert find_jacky_node_executable("npm") == str(npm_cmd)
    ...
    def test_windows_skips_broken_managed_npm_without_path_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_HOME", str(home))
        monkeypatch.setenv("PATH", str(bin_dir))
        monkeypatch.setattr(jacky_constants, "_managed_node_heal_attempted", False)
        monkeypatch.setattr(jacky_constants, "heal_jacky_managed_node", lambda: False)
        monkeypatch.setattr(
            jacky_constants,
            "node_tool_runnable",
            lambda path: False,
        ...
        assert jacky_managed_node_tree_present() is True
        assert find_node_executable("npm") is None
        assert find_node_executable("npm") != str(path_npm)

    def test_with_jacky_node_path_prepends_existing_managed_dirs(self, tmp_path, monkeypatch):
        home = tmp_path / "jacky"
        node_dir = home / "node"
        bin_dir = node_dir / "bin"
        node_dir.mkdir(parents=True)
        ...
        env = with_jacky_node_path({"PATH": "system-node"})
...
class TestNodeToolRunnable:
    """node_tool_runnable() rejects broken Jacky-managed npm/node wrappers."""
    ...
    def test_broken_managed_npm_heals_when_node_still_runs(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(jacky_constants, "heal_jacky_managed_node", _heal)
    ...
    def test_broken_managed_npm_heals_instead_of_path_fallback(self, tmp_path, monkeypatch):
        ...
        assert find_jacky_node_executable("npm") == str(healed_npm)
...
class TestIsContainer:
    ...
    def _reset_cache(self, monkeypatch):
        ...
        monkeypatch.setattr(jacky_constants, "_container_detected", None)
    ...
    def test_caches_result(self, monkeypatch):
        ...
        monkeypatch.setattr(jacky_constants, "_container_detected", True)
        assert is_container() is True
...
class TestSecureParentDir:
    ...
    def test_safe_path_calls_chmod(self, tmp_path, monkeypatch):
        ...
        safe_dir = tmp_path / "home" / "user" / ".jacky"
...
class TestAgentBrowserRunnable:
    ...
    def test_version_probe_uses_windows_hide_flags(self, tmp_path, monkeypatch):
        ...
        import jacky_cli._subprocess_compat as subprocess_compat
...
class TestGetJackyDir:
    """Tests for ``get_jacky_dir(new_subpath, old_name)``.
    ...
    def test_neither_exists_returns_new(self, tmp_path, monkeypatch):
        ...
        result = get_jacky_dir("platforms/pairing", "pairing")
    ...
    def test_legacy_populated_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_jacky_dir("cache/images", "image_cache")
    ...
    def test_legacy_populated_with_subdir_returns_legacy(self, tmp_path, monkeypatch):
        ...
        result = get_jacky_dir("platforms/matrix/store", "matrix/store")
        assert result == legacy
    ...
    def test_legacy_empty_and_new_missing_returns_new(self, tmp_path, monkeypatch):
        ...
        legacy.mkdir()
        result = get_jacky_dir("cache/audio", "audio_cache")

...
from jacky_cli.cli import JackyCLI, _build_compact_banner, _rich_text_from_ansi
from jacky_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("jacky_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
    ...
    def test_handle_skin_command_refreshes_live_tui(self, capsys):
        ...
        with patch("jacky_cli.cli.save_config_value", return_value=True):
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_hermes_branding(self):
        ...
        with patch("jacky_cli.cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Jacky Agent v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "NOUS JACKY" in banner
    ...
    def test_poseidon_compact_banner_uses_skin_branding_instead_of_nous_hermes(self):
        ...
        assert "NOUS JACKY" not in banner
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("jacky_cli.cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Jacky Agent v1.0 (test) · upstream abc12345"}):
            banner = _build_compact_banner()

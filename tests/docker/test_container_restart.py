...
$JACKY_HOME/profiles/<name>/gateway_state.json on every boot and
...
def restart_container(request, built_image: str):
    ...
    preserves $JACKY_HOME/profiles/."""
    safe = request.node.name.replace("[", "_").replace("]", "_")
    name = f"jacky-restart-{safe}"
    volume = f"jacky-restart-vol-{safe}"
    _docker("rm", "-f", name)
...
def test_running_gateway_survives_container_restart(restart_container: str) -> None:
    ...
    r = docker_exec(container, "jacky", "profile", "create", "coder")
    assert r.returncode == 0, f"profile create failed: {r.stderr}"

    r = docker_exec(container, "jacky", "-p", "coder", "gateway", "start", timeout=60)
...
def test_stopped_gateway_stays_stopped_after_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "jacky", "profile", "create", "writer").check_returncode()
...
def test_stale_gateway_pid_cleaned_up_on_restart(restart_container: str) -> None:
    ...
    docker_exec(container, "jacky", "profile", "create", "ghost").check_returncode()
...
def test_live_gateway_autostarts_after_real_restart_without_manual_state_stamp(
    ...
    docker_exec(container, "jacky", "profile", "create", "live").check_returncode()
    r = docker_exec(container, "jacky", "-p", "live", "gateway", "start", timeout=60)
    assert r.returncode == 0, f"gateway start failed: {r.stderr}"
    ...
    # operator `jacky gateway stop`), so the shutdown is signal-initiated.
    _docker("restart", container, timeout=60).check_returncode()

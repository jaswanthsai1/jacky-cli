...
The shim (docker/jacky-exec-shim.sh, installed at /opt/jacky/bin/jacky)
exists to prevent the auth.json ownership-mismatch bug where
`docker exec <c> jacky login` would write /opt/data/auth.json as
...
failed: Jacky is not logged into Nous Portal" on every message.

These tests verify:

1. ``docker exec <c> jacky …`` (defaulting to root) gets dropped to the
   jacky user before the real binary runs.
2. ``docker exec --user jacky <c> jacky …`` (already non-root) short-
   circuits and doesn't try to drop again.
3. Files written under $JACKY_HOME from a ``docker exec`` session land
   as jacky:jacky — the actual user-visible invariant.
4. The JACKY_DOCKER_EXEC_AS_ROOT opt-out lets diagnostic sessions keep
...
def _wait_for_cont_init(container: str) -> None:
    ...
    ``01-jacky-setup`` cont-init hook (docker/stage2-hook.sh) has
    finished seeding + ``chown jacky:jacky`` config.yaml and running the
    ...
    clobbered the root-written file's ownership back to jacky:jacky,
    failing ``test_shim_opt_out_keeps_root`` non-deterministically.

    The reliable "cont-init is done" signal is
    ``$JACKY_HOME/logs/container-boot.log``: it is written by
    ``02-reconcile-profiles`` (jacky_cli.container_boot), which s6 runs
    *strictly after* ``01-jacky-setup`` in lexicographic order. The
...
def test_shim_drops_root_to_jacky_uid(sleep_container: str) -> None:
    """docker exec defaults to root; the shim should drop to uid 10000.

    We invoke `jacky` with a Python-style `-c` shim equivalent — there's no
    pure-jacky "print my uid" command, so we use the venv's python directly
    ...
    fact that the venv's `jacky` is a console_scripts entry — under the
    hood it's a tiny Python wrapper. We can't easily inject "print my uid"
    into it without forking subcommands. Simplest approach: have `jacky`
    do anything that writes to disk, then check the file's owner.

    Use `jacky config set` which writes config.yaml under JACKY_HOME.
    ...
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "jacky", "config", "set", "_test.shim_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: stdout={r.stdout!r} stderr={r.stderr!r}"

    # The written file must be owned by jacky, not root.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
    ...
    assert r.stdout.strip() == "jacky:jacky", (
        f"config.yaml owned by {r.stdout.strip()!r}, expected jacky:jacky. "
        "The shim did not drop privileges before invoking jacky."
    )


def test_shim_short_circuits_for_non_root_exec(sleep_container: str) -> None:
    """docker exec --user jacky already runs as 10000; shim should be a no-op.
    ...
    r = subprocess.run(
        ["docker", "exec", "--user", "jacky", sleep_container,
         "jacky", "config", "set", "_test.shim_short_circuit", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, (
        f"docker exec --user jacky failed: {r.stderr!r} stdout={r.stdout!r}. "
        "If the shim mis-handled the non-root path, this would fail with EPERM."
    )

    # File still ends up jacky:jacky — orthogonally confirms uid.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "stat", "-c", "%U:%G", "/opt/data/config.yaml"],
        capture_output=True, text=True, timeout=10,
...
def test_shim_opt_out_keeps_root(sleep_container: str) -> None:
    """JACKY_DOCKER_EXEC_AS_ROOT=1 should suppress the privilege drop.
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", "JACKY_DOCKER_EXEC_AS_ROOT=1",
         sleep_container,
         "jacky", "config", "set", "_test.opt_out", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "root:root", (
        f"With JACKY_DOCKER_EXEC_AS_ROOT=1, expected root:root, "
...
def test_shim_opt_out_strict_truthiness(
    ...
    Strict truthiness so a typo (``JACKY_DOCKER_EXEC_AS_ROOT=0``) doesn't
    silently keep the user as root. Mirrors the policy used by
    ``JACKY_GATEWAY_NO_SUPERVISE`` in #33583.
    """
    subprocess.run(
        ["docker", "exec", "--user", "root", sleep_container,
         "rm", "-f", "/opt/data/config.yaml"],
    ...
    r = subprocess.run(
        ["docker", "exec",
         "-e", f"JACKY_DOCKER_EXEC_AS_ROOT={falsy_value}",
         sleep_container,
         "jacky", "config", "set", "_test.falsy", "1"],
        capture_output=True, text=True, timeout=30,
    )
    ...
    assert r.stdout.strip() == "jacky:jacky", (
        ...
        f"file owner is {r.stdout.strip()!r}, expected jacky:jacky"
    )
...
def test_main_cmd_path_unaffected(built_image: str) -> None:
    ...
    The shim sits at /opt/jacky/bin earliest on PATH; main-wrapper.sh
    invokes `s6-setuidgid jacky jacky <args>` which resolves `jacky`
...
def test_e2e_login_then_supervised_gateway_can_read_auth(
    ...
    Pre-shim: ``docker exec <c> jacky login`` (root) wrote
    ...
    "Jacky is not logged into Nous Portal" on every message.

    We can't do a real OAuth login in a unit test, but we can stand in
    for it by writing the same file shape via `jacky config set`-style
    writes — what matters is the *file ownership invariant* downstream
    of `_save_auth_store`. If the shim works, every file the
    `docker exec` path produces is jacky-readable.

    Specifically: pretend the operator ran `jacky login` (writes
    auth.json) and verify (a) the file exists and (b) it's readable by
    the jacky UID. We use `jacky auth list` since that touches the
    ...
    # `jacky auth list` is read-only but still exercises _load_auth_store
    # under the shim's UID. We invoke `jacky config set` first to
    # provoke a write into JACKY_HOME so we have something concrete to
    # owner-check.
    r = subprocess.run(
        ["docker", "exec", sleep_container,
         "jacky", "config", "set", "_test.e2e_marker", "1"],
        capture_output=True, text=True, timeout=30,
    )
    assert r.returncode == 0, f"config set failed: {r.stderr}"

    # The supervised UID (10000) must be able to read everything under
    # JACKY_HOME that docker exec just wrote.
    r = subprocess.run(
        ["docker", "exec", "--user", "jacky", sleep_container,
    ...
    assert not unreadable, (
        "Files written by `docker exec` are unreadable to the jacky user "

...
Override the image with ``JACKY_TEST_IMAGE`` env var to point at a pre-built
...
IMAGE_TAG = os.environ.get("JACKY_TEST_IMAGE", "jacky-agent-harness:latest")
...
def built_image() -> str:
    ...
    Override with ``JACKY_TEST_IMAGE`` env var to point at a pre-built
    image (faster local iteration).
    """
    if os.environ.get("JACKY_TEST_IMAGE"):
        return IMAGE_TAG
...
# ---------------------------------------------------------------------------
# docker_exec — default to the unprivileged jacky user
# ---------------------------------------------------------------------------
#
# Background: every Jacky runtime path inside the container drops to UID
# 10000 (the ``jacky`` user) via ``s6-setuidgid jacky``. ``docker exec``
...
# fails (PermissionError swallowed) for jacky, so a test that ran as root
...
# helpers below run every probe under ``-u jacky`` unless a specific
...
def docker_exec(
    ...
    *args: str,
    ...
    """Run a command inside ``container`` as ``user`` (default: jacky).

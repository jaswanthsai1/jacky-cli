"""Harness: dashboard opt-in via JACKY_DASHBOARD.

Today (tini): dashboard starts once when JACKY_DASHBOARD=1; if it crashes
...
Every ``docker exec`` here runs as the unprivileged ``jacky`` user
...
def test_dashboard_not_running_by_default(
    ...
    """Without JACKY_DASHBOARD, no dashboard process should be running."""
    start_container(built_image, container_name, cmd="sleep 60")
    r = docker_exec(container_name, "pgrep", "-f", "jacky dashboard")
    # pgrep exits non-zero when no match found
    assert r.returncode != 0, (
        "Dashboard should not be running without JACKY_DASHBOARD"
    )
...
def test_dashboard_slot_reports_down_when_disabled(
    ...
    """Without JACKY_DASHBOARD, s6-svstat should report the dashboard
    slot as DOWN (not up-with-sleep-infinity, which would
    false-positive `jacky doctor` and any other health check).
    ...
    JACKY_DASHBOARD is unset, so the slot reflects reality.
    """
    start_container(built_image, container_name, cmd="sleep 60")
    ...
    assert "down" in r.stdout, (
        f"Dashboard slot should be 'down' without JACKY_DASHBOARD; "
...
def test_dashboard_slot_reports_up_when_enabled(
    ...
    """Symmetry: with JACKY_DASHBOARD=1, s6-svstat reports the slot as up."""
    # The default dashboard host is 0.0.0.0, which now engages the
    # OAuth auth gate. Without a provider registered (no
    # JACKY_DASHBOARD_OAUTH_CLIENT_ID in this test env), start_server
    ...
    start_container(
        ...
        "JACKY_DASHBOARD_BASIC_AUTH_PASSWORD=test-dashboard-pw",
        cmd="sleep 120",
    )
...
def test_dashboard_opt_in_starts(
    ...
    """With JACKY_DASHBOARD=1, a dashboard process should be visible."""
    ...
    ok, _ = poll_container(
        container_name, "pgrep -f 'jacky dashboard'", deadline_s=30.0,
    )
    assert ok, "Dashboard should be running with JACKY_DASHBOARD=1"


def test_dashboard_port_override(
    built_image: str, container_name: str,
) -> None:
    """JACKY_DASHBOARD_PORT changes the dashboard's listen port."""
...
def test_dashboard_restarts_after_crash(
    ...
    for _attempt in range(10):
        first_pid_result = docker_exec(
            container_name, "pgrep", "-f", "jacky dashboard",
        )
    ...
    # Kill the dashboard. The dashboard process runs as jacky, so the
    # jacky user can kill it (same UID).
...
# whenever `JACKY_DASHBOARD_HOST` was non-loopback, silently disabling
...
def test_dashboard_oauth_gate_engages_on_non_loopback_bind(
    ...
    provider activates whenever ``JACKY_DASHBOARD_OAUTH_CLIENT_ID`` is
    set).
    ...
    start_container(
        built_image, container_name,
        "JACKY_DASHBOARD=1",
        "JACKY_DASHBOARD_HOST=0.0.0.0",
        "JACKY_DASHBOARD_OAUTH_CLIENT_ID=agent:test-instance",
        cmd="sleep 120",
    )
    ...
    assert "nous" in provider_names, (
        ...
        f"JACKY_DASHBOARD_OAUTH_CLIENT_ID is set. Got: {payload!r}"
    )
    ...
    assert status_code == 401, (
        ...
        "when a provider is registered and JACKY_DASHBOARD_INSECURE "
...
def test_dashboard_insecure_env_var_no_longer_bypasses_gate(
    ...
    """``JACKY_DASHBOARD_INSECURE=1`` NO LONGER disables the auth gate

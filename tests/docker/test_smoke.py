...
Converted from the former ``.github/actions/jacky-smoke-test`` composite
...
def test_jacky_help(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"jacky --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )
    assert "Traceback" not in r.stderr, (
        f"jacky --help produced a traceback: {r.stderr[-2000:]!r}"
    )
...
def test_dashboard_subcommand_present(built_image: str) -> None:
    ...
    assert r.returncode == 0, (
        f"jacky dashboard --help failed (exit {r.returncode}): "
        f"stdout={r.stdout[-2000:]!r} stderr={r.stderr[-2000:]!r}"
    )

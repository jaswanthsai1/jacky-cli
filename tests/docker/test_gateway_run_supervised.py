...
Before the s6 migration, ``docker run jaswanthsai1/jacky-cli gateway
...
crash and a supervised dashboard alongside (when ``JACKY_DASHBOARD=1``).
...
These tests verify the three load-bearing properties of that redirect:
  ...
  2. ``--no-supervise`` / ``JACKY_GATEWAY_NO_SUPERVISE=1`` opts out.
  3. The supervised process itself does NOT recurse — the
     ``JACKY_S6_SUPERVISED_CHILD`` sentinel breaks the loop.

Every ``docker exec`` runs as ``jacky`` per the conftest module
...
def _wait_for_gateway_or_exit(
    ...
    *,
    ...
    ``pgrep -f 'jacky.*gateway'`` (the gateway is running) or
    ...
    while time.monotonic() < end:
        ...
        if status == "running":
            ...
            pgrep = docker_exec_sh(
                container, "pgrep -f 'jacky.*gateway' >/dev/null 2>&1",
            )
...
def test_gateway_run_redirects_to_supervised(
    ...
    # process (main-wrapper.sh → python → jacky gateway run) can take
    ...
    # (main-wrapper.sh → python → jacky gateway run) can take well over
    ...
    # top), NOT the static main-jacky service's sleep — a bare grep
    # for `sleep infinity` would false-positive on the main-jacky
...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``jacky gateway run`` invocation
        (not ``sleep infinity``).
...
def test_gateway_run_no_supervise_env_var(
    ...
    if status == "running":
        assert not _svstat_wants_up(container_name, "gateway-default"), (
            "JACKY_GATEWAY_NO_SUPERVISE=1: gateway-default has "
...
def test_supervised_gateway_does_not_recurse(
    ...
    """The JACKY_S6_SUPERVISED_CHILD sentinel must prevent the
    supervised ``jacky gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``jacky gateway
    ...
    # (main-wrapper.sh → python → jacky gateway run) can take well
    ...
    # started either — so we'd see the CMD's `jacky gateway run`
    ...
    # `jacky gateway run`. If the recursion guard fails, s6 would
    # respawn fresh `gateway run` processes on every cycle, leaving
    # multiple Python-process descendants under the gateway-default
    # supervise tree.
    r = docker_exec_sh(container_name, "ps -eo pid,cmd | grep -v grep | grep -E 'python.*jacky.*gateway run' | wc -l")
    assert r.returncode == 0
    n = int(r.stdout.strip() or 0)
    assert n <= 1, (
        f"expected at most one supervised python `jacky gateway run` "
    ...
    # CMD process (PID 17 typically). The static `main-jacky`
...
def test_dashboard_supervised_when_env_set(
    ...
    """When ``JACKY_DASHBOARD=1`` is set, ``docker run <image> gateway
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    log file under ``${JACKY_HOME}/logs/gateways/<profile>/current``.
    ...
    a poor user experience: the iconic "Jacky Gateway Starting…"
    banner with the "Jacky Gateway Starting" text is the most visible "yes, your gateway
    ...
    We assert by looking for the literal banner text (``Jacky Gateway
    Starting``) — a distinctive string that won't appear in stderr-routed
    Python-logging output, so its presence in ``docker logs`` proves
    the stdout-tee is working.
    """
    start_container(built_image, container_name, cmd="gateway run")

    # Poll docker logs for the "Jacky Gateway Starting" banner text —
    # the gateway's rich-console startup banner. A fixed
    ...
    wait_for_docker_logs(container_name, "Jacky Gateway Starting", deadline_s=60.0)
    ...
    # The banner text is the load-bearing assertion — it's unique
    # to gateway startup stdout output and won't appear in stderr
    # (Python logging) or s6 boot messages.
    assert "Jacky Gateway Starting" in combined, (
    ...
    assert "Jacky Gateway Starting" in file_contents, (

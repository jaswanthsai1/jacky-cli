...
def test_container_sets_hosted_write_policy_env(built_image: str) -> None:
    script = (
        'test "$JACKY_HOME" = "/opt/data" && '
        'test "$JACKY_WRITE_SAFE_ROOT" = "/opt/data" && '
...
def test_jacky_user_cannot_modify_install_but_can_write_data(built_image: str) -> None:
    script = textwrap.dedent(
        ...
        install_file = Path("/opt/jacky/agent/message_sanitization.py")
        try:

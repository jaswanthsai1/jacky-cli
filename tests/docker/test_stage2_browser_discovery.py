...
def test_stage2_browser_path_accessible_to_jacky_user(
    built_image: str, container_name: str,
) -> None:
    """The discovered browser binary must be accessible to the
    unprivileged jacky user (UID 10000), since that's who runs
    ...
    assert r.returncode == 0, (
        f"browser binary not readable+executable by jacky user: {r.stderr}"
    )

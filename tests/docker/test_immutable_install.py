...
Build the real image and verify at runtime:
  ...
  1. /opt/jacky is not writable by the jacky user (immutable install tree)
  2. PYTHONDONTWRITEBYTECODE and JACKY_DISABLE_LAZY_INSTALLS are set
  3. /opt/jacky/.install_method contains "docker" (code-scoped stamp)
  4. $JACKY_HOME/.install_method is NOT stamped as "docker" by stage2
  5. A stale "docker" stamp in $JACKY_HOME is healed (removed) on boot
"""
...
def test_install_tree_not_writable_by_jacky(
    built_image: str, container_name: str,
) -> None:
    """The jacky user must not be able to modify /opt/jacky.
    ...
    r = docker_exec_sh(
        container_name,
        # Try to create a file under /opt/jacky as the jacky user
        "touch /opt/jacky/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"jacky user can write to /opt/jacky (install tree not immutable): "
        f"{r.stdout}"
    )
    ...
    r = docker_exec_sh(
        container_name,
        "touch /opt/jacky/.venv/test_write 2>&1 && "
        "echo WRITE_SUCCEEDED || echo WRITE_FAILED",
        timeout=10,
    )
    assert "WRITE_FAILED" in r.stdout, (
        f"jacky user can write to /opt/jacky/.venv: {r.stdout}"
    )


def test_jacky_disable_lazy_installs_and_dont_write_bytecode(
    built_image: str, container_name: str,
) -> None:
    """The container must set PYTHONDONTWRITEBYTECODE and
    JACKY_DISABLE_LAZY_INSTALLS=1 so no .pyc files are written to the
    ...
    r = docker_exec_sh(
        ...
        'test "$JACKY_DISABLE_LAZY_INSTALLS" = "1" && '
        'echo ENV_OK || echo ENV_MISSING',
        timeout=10,
    )
    assert "ENV_OK" in r.stdout, (
        f"expected PYTHONDONTWRITEBYTECODE=1 and "
        f"JACKY_DISABLE_LAZY_INSTALLS=1, got: {r.stdout} stderr={r.stderr}"
    )
...
def test_install_method_stamp_is_code_scoped(
    ...
    /opt/jacky/.install_method (code-scoped), NOT in $JACKY_HOME."""
    start_container(built_image, container_name)

    # Code-scoped stamp must exist and say "docker"
    r = docker_exec_sh(
        container_name,
        "cat /opt/jacky/.install_method",
        timeout=10,
    )
    assert r.returncode == 0, (
        f"/opt/jacky/.install_method not found: {r.stderr}"
    )
    assert r.stdout.strip() == "docker", (
        f"expected 'docker' stamp, got: {r.stdout.strip()!r}"
    )

    # $JACKY_HOME must NOT have a 'docker' stamp
    r = docker_exec_sh(
        container_name,
    ...
    assert r.stdout.strip() != "docker", (
        "$JACKY_HOME/.install_method is stamped 'docker' - stage2 must "
...
def test_stale_docker_stamp_in_home_is_healed_on_boot(
    ...
    """A stale 'docker' stamp left in $JACKY_HOME by an older image
    ...
    assert "HEALED" in r.stdout or r.stdout.strip() != "docker", (
        f"stale 'docker' stamp in $JACKY_HOME was not healed on boot: "
        f"{r.stdout}"
    )

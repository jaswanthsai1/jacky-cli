...
- jacky_cli/plugins register_skill API + registry
...
class TestPluginContextRegisterSkill:
    ...
    def ctx(self, tmp_path, monkeypatch):
        from jacky_cli import plugins as plugins_mod
        from jacky_cli.plugins import PluginContext, PluginManager, PluginManifest
...
class TestSkillViewPluginGuards:
    ...
    def test_disabled_plugin(self, tmp_path, monkeypatch):
        ...
        self._reg(tmp_path, "---\nname: foo\n---\nBody.\n")
        monkeypatch.setattr("jacky_cli.plugins._get_disabled_plugins", lambda: {"myplugin"})

...
This validates the IPC + lifecycle story that mocks can't:
  ...
  - the child process resolves jacky_cli.kanban_db on its own
  ...
  - worker logs are captured to JACKY_HOME/kanban/logs/<task>.log
    ...
    def _spawn(task, workspace):
        ...
        env = {
            **os.environ,
            ...
            "HOME": home,
            "PYTHONPATH": WT,
            "JACKY_KANBAN_TASK": task.id,
            "JACKY_KANBAN_WORKSPACE": workspace,
...
def main():
    home = tempfile.mkdtemp(prefix="jacky_e2e_")
    os.environ["JACKY_HOME"] = home
    os.environ["HOME"] = home
    sys.path.insert(0, WT)
    from jacky_cli import kanban_db as kb

    # Point the `jacky` CLI child processes will run at the worktree
    # jacky_cli.main. We do this by putting a shim on PATH.
    shim_dir = os.path.join(home, "bin")
    os.makedirs(shim_dir, exist_ok=True)
    shim_path = os.path.join(shim_dir, "jacky")
    with open(shim_path, "w") as f:
        f.write(f"""#!/bin/sh
exec {PY} -m jacky_cli.main "$@"
""")
    ...
    print("=" * 60)
    # Scenario A workers wrote to /tmp/jacky_e2e_*/worker_*.log
    import glob

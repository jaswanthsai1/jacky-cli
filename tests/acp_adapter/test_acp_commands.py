...
class FakeAgent:
    def __init__(self):
        ...
        self.enabled_toolsets = ["jacky-acp"]
...
def test_acp_real_agent_gets_session_db_for_recall(monkeypatch):
    ...
    monkeypatch.setitem(sys.modules, "jacky_cli.run_agent", mod("jacky_cli.run_agent", AIAgent=CapturingAgent))
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.config",
        mod("jacky_cli.config", load_config=lambda: {"model": {"default": "m", "provider": "p"}}),
    )
    monkeypatch.setitem(
        sys.modules,

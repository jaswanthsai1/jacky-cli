...
from jacky_cli.toolsets import (
    TOOLSETS,
    get_toolset,
    resolve_toolset,
...
class TestToolsetConsistency:
    ...
    def test_jacky_platforms_share_core_tools(self):
        """All jacky-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on jacky-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["jacky-cli", "jacky-telegram", "jacky-discord", "jacky-whatsapp", "jacky-slack", "jacky-signal", "jacky-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_jacky_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("jacky-whatsapp")

    def test_jacky_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("jacky-api-server")

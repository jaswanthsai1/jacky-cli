...
profile's JACKY_HOME and EXECUTE under it.

- Storage: a job created under profile ``coder`` writes to
  ``~/.jacky/profiles/coder/cron/jobs.json`` — NOT the shared default root.
- Execution: the profile-scoped gateway's in-process ticker resolves the
  active JACKY_HOME (profile home) at call time, so jobs run with that
...
one store and runs them under whatever JACKY_HOME the ticker happens to have —
...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    """Under a profile JACKY_HOME (<root>/profiles/<name>), the cron store
    ...
    assert jacky_constants.get_default_jacky_root().resolve() == root.resolve()

    # cron/jobs.py computes JACKY_DIR from get_jacky_home() at import, so a
    # fresh import under this env anchors the store at <profile>/cron.
    import cron.jobs as jobs

    importlib.reload(jobs)
    try:
        assert jobs.JACKY_DIR.resolve() == profile_home.resolve()
        assert (
...
def test_cron_execution_home_follows_active_profile(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(scheduler, "_jacky_home", None, raising=False)
    assert scheduler._get_jacky_home().resolve() == profile_home.resolve()
    assert scheduler._get_jacky_home().resolve() != root.resolve()


def test_cron_storage_unaffected_when_no_profile(tmp_path, monkeypatch):
    """With no profile (JACKY_HOME == root), the store is the root's cron dir
    ...
    importlib.reload(jobs)
    try:
        assert jobs.JACKY_DIR.resolve() == root.resolve()

...
the shared /blueprint command handler (jacky_cli/blueprint_cmd.py), and
the docs generator. Uses an isolated JACKY_HOME for anything that touches the
cron job store.
"""
...
class TestRenderers:
    ...
    def test_deeplink_shape(self):
        ...
        assert url.startswith("jacky://blueprint/morning-brief?")
    ...
    def test_catalog_entry_has_all_surfaces(self):
        ...
        assert entry["appUrl"].startswith("jacky://")
...
class TestCommandHandler:
    ...
    def test_name_match_is_forgiving(self, isolated_home):
        from jacky_cli.blueprint_cmd import handle_blueprint_command, match_blueprint

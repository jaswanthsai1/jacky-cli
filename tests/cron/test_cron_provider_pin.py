...
resolve_runtime_provider against a temp JACKY_HOME) and the create_job
...
class TestCreateJobSnapshot:
    ...
    def test_pinned_job_skips_snapshot(self, monkeypatch):
        ...
        with patch("jacky_cli.runtime_provider.resolve_runtime_provider", resolver):
            job = jobs.create_job(
    ...
    def test_unpinned_model_captures_model_snapshot(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "cron.jobs.get_jacky_home", lambda: tmp_path, raising=True
        )
        with patch(
            "jacky_cli.runtime_provider.resolve_runtime_provider",
            return_value={"provider": "openrouter"},
...
def _run_with_current_provider_and_model(job, current_provider, current_model, tmp_path):
    ...
    fake_db = MagicMock()
    with patch("cron.scheduler._jacky_home", tmp_path), \
         patch("cron.scheduler._get_jacky_home", return_value=tmp_path), \
         patch("cron.scheduler._resolve_origin", return_value=None), \
         patch("jacky_cli.env_loader.load_jacky_dotenv"), \
         patch("jacky_cli.env_loader.reset_secret_source_cache"), \
         patch("jacky_cli.jacky_state.SessionDB", return_value=fake_db), \

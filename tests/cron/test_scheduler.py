...
class TestPerJobToolsetMcpMerge:
    ...
    def test_resolver_empty_per_job_falls_through_to_platform(self):
        ...
        with patch("jacky_cli.tools_config._get_platform_tools",
...
class TestRunJobSessionPersistence:
    ...
    def _run_job_patches(self, tmp_path, extra=()):
        ...
        mock_agent.run_conversation.return_value = {"final_response": "ok"}
        base = [
            patch("cron.scheduler._jacky_home", tmp_path),
            patch("cron.scheduler._resolve_origin", return_value=None),
            patch("jacky_cli.env_loader.load_jacky_dotenv"),
            patch("jacky_cli.env_loader.reset_secret_source_cache"),
            patch("jacky_state.SessionDB", return_value=fake_db),
    ...
    def test_run_job_enabled_toolsets_resolves_from_platform_config_when_not_set(self, tmp_path):
        ...
        resolves them from ``jacky tools`` platform config for ``cron``
        ...
        # The cron default is _JACKY_CORE_TOOLS with _DEFAULT_OFF_TOOLSETS
    ...
    def test_run_job_per_job_toolsets_win_over_platform_config(self, tmp_path):
        ...
        over the platform-level ``jacky tools`` config."""
        job = {
            "id": "override-job",
            "name": "test",
            "prompt": "hello",
        ...
        # Even if the user has ``jacky tools`` configured to enable web+file
        # for cron, the per-job override wins.
        extra = [patch("jacky_cli.tools_config._get_platform_tools", return_value={"web", "file"})]
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("JACKY_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("JACKY_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("JACKY_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen["platform"] = get_session_env("JACKY_CRON_AUTO_DELIVER_PLATFORM") or None
                seen["chat_id"] = get_session_env("JACKY_CRON_AUTO_DELIVER_CHAT_ID") or None
                seen["thread_id"] = get_session_env("JACKY_CRON_AUTO_DELIVER_THREAD_ID") or None
        ...
        assert os.getenv("JACKY_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("JACKY_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("JACKY_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
    ...
    def test_run_job_resets_secret_source_cache_before_reload(self, tmp_path, monkeypatch):
        ...
        this JACKY_HOME in ``_APPLIED_HOMES``, so the external-secret pull
        ...
        ``load_jacky_dotenv`` (which then re-applies external secret sources).
        """
    ...
    def test_run_job_clears_stale_auto_delivery_thread_id_between_jobs(self, tmp_path, monkeypatch):
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                ...
                seen.append(
                    {
                        "platform": get_session_env("JACKY_CRON_AUTO_DELIVER_PLATFORM") or None,
                        "chat_id": get_session_env("JACKY_CRON_AUTO_DELIVER_CHAT_ID") or None,
                        "thread_id": get_session_env("JACKY_CRON_AUTO_DELIVER_THREAD_ID") or None,
                    }
                )
                return {"final_response": "ok"}
...
class TestRunJobConfigEnvVarExpansion:
    ...
    def test_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_JACKY_TEST_CRON_MODEL}\n")
        monkeypatch.setenv("_JACKY_TEST_CRON_MODEL", "gpt-4o-mini-cron-test")
    ...
    def test_fallback_model_env_ref_in_config_yaml_is_expanded(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text(
            ...
            "fallback_providers:\n"
            "  - provider: openrouter\n"
            "    model: ${_JACKY_TEST_CRON_FALLBACK}\n"
        )
        monkeypatch.setenv("_JACKY_TEST_CRON_FALLBACK", "gpt-4o-fallback-test")
    ...
    def test_unexpanded_ref_passthrough_when_var_unset(self, tmp_path, monkeypatch):
        ...
        (tmp_path / "config.yaml").write_text("model: ${_JACKY_TEST_CRON_UNSET_VAR}\n")
        monkeypatch.delenv("_JACKY_TEST_CRON_UNSET_VAR", raising=False)
        ...
        assert kwargs["model"] == "${_JACKY_TEST_CRON_UNSET_VAR}"
...
class TestRunJobModelResolution:
    ...
    ``model: null``. At fire time the scheduler must:
      1. fall back to ``JACKY_MODEL`` env if set,
    ...
    def test_null_job_model_falls_back_to_env(self, tmp_path, monkeypatch):
        """``model: null`` on the job uses JACKY_MODEL when set."""
        (tmp_path / "config.yaml").write_text("")
        monkeypatch.setenv("JACKY_MODEL", "env-model")
    ...
    def test_explicit_null_model_block_in_config_does_not_overwrite_env(self, tmp_path, monkeypatch):
        """``model: null`` in config.yaml must not overwrite a resolved JACKY_MODEL.
    ...
    def test_config_model_alias_key_resolves(self, tmp_path, monkeypatch):
        ...
        ``jacky_cli/oneshot.py``, ``fallback_cmd.py`` and ``prompt_size.py``
...
class TestParallelTick:
    ...
    def test_max_parallel_env_var(self, monkeypatch):
        """JACKY_CRON_MAX_PARALLEL=1 should restore serial behaviour."""
        monkeypatch.setenv("JACKY_CRON_MAX_PARALLEL", "1")
        call_times = []
...
class TestHomeTargetEnvVarRegistry:
    ...
    entry means ``jacky cron create --deliver=<platform>`` silently

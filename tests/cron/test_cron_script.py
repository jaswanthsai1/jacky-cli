...
- Path resolution (absolute, relative to JACKY_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp JACKY_HOME."""
    jacky_home = tmp_path / ".jacky"
    jacky_home.mkdir()
    (jacky_home / "cron").mkdir()
    (jacky_home / "cron" / "output").mkdir()
    (jacky_home / "scripts").mkdir()
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "JACKY_DIR", jacky_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", jacky_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", jacky_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", jacky_home / "cron" / "output")
...
class TestRunJobScript:
    ...
    def test_script_subprocess_env_sanitized(self, cron_env, monkeypatch):
        """Cron scripts must not inherit Jacky provider env (SECURITY.md §2.3)."""
        from tools.environments.local import _JACKY_PROVIDER_ENV_BLOCKLIST
        from cron.scheduler import _run_job_script

        # sorted() so the probed var is deterministic across runs
        # (frozenset iteration order varies with PYTHONHASHSEED).
        blocked_var = sorted(_JACKY_PROVIDER_ENV_BLOCKLIST)[0]
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.jacky/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("JACKY_SESSION_PLATFORM") is None
        assert os.environ.get("JACKY_SESSION_CHAT_ID") is None
        assert os.environ.get("JACKY_SESSION_CHAT_NAME") is None

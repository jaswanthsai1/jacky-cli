...
class TestHandleUpdateCommand:
    ...
    async def test_managed_install_returns_package_manager_guidance(self, monkeypatch):
        ...
        # `jacky update --gateway` against the CI checkout. The managed-install
        ...
        assert "brew upgrade jacky-agent" in result
    ...
    async def test_no_git_directory(self, tmp_path):
        ...
        runner = _make_runner()
        event = _make_event()
        # Point _jacky_home to tmp_path and project_root to a dir without .git
        fake_root = tmp_path / "project"
        fake_root.mkdir()
        with patch("gateway.run._jacky_home", tmp_path), \
        ...
        with patch("gateway.run._jacky_home", tmp_path):
            ...
            # project_root will be the real jacky-agent dir (which HAS .git).
    ...
    @pytest.mark.asyncio
    async def test_no_jacky_binary(self, tmp_path):
        """Returns error when jacky is not on PATH and jacky_cli is not importable."""
        runner = _make_runner()
        event = _make_event()
        ...
        assert "jacky update" in result

    @pytest.mark.asyncio
    async def test_fallback_to_sys_executable(self, tmp_path):
        """Falls back to sys.executable -m jacky_cli.main when jacky not on PATH."""
        runner = _make_runner()
        event = _make_event()
        ...
        with patch("gateway.run._jacky_home", jacky_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", return_value=None), \
        ...
        assert "Starting Jacky update" in result
        call_args = mock_popen.call_args[0][0]
        # The update_cmd uses sys.executable -m jacky_cli.main
        joined = " ".join(call_args) if isinstance(call_args, list) else call_args
        assert "jacky_cli.main" in joined or "bash" in call_args[0]

    @pytest.mark.asyncio
    async def test_resolve_jacky_bin_prefers_which(self, tmp_path):
        """_resolve_jacky_bin returns argv parts from shutil.which when available."""
        from gateway.run import _resolve_jacky_bin

        with patch("shutil.which", return_value="/custom/path/jacky"):
            result = _resolve_jacky_bin()

        assert result == ["/custom/path/jacky"]

    @pytest.mark.asyncio
    async def test_resolve_jacky_bin_fallback(self):
        """_resolve_jacky_bin falls back to sys.executable argv when which fails."""
        import sys
        from gateway.run import _resolve_jacky_bin
        ...
        assert result == [sys.executable, "-m", "jacky_cli.main"]

    @pytest.mark.asyncio
    async def test_resolve_jacky_bin_returns_none_when_both_fail(self):
        """_resolve_jacky_bin returns None when both strategies fail."""
        from gateway.run import _resolve_jacky_bin
    ...
    async def test_writes_pending_marker(self, tmp_path):
        ...
        with patch("gateway.run._jacky_home", jacky_home), \
             patch("gateway.run.__file__", fake_file), \
             patch("shutil.which", side_effect=lambda x: "/usr/bin/jacky" if x == "jacky" else "/usr/bin/setsid"), \
             patch("subprocess.Popen"):
            result = await runner._handle_update_command(event)

        pending_path = jacky_home / ".update_pending.json"
        ...
        assert not (jacky_home / ".update_exit_code").exists()
    ...
    async def test_writes_pending_marker_with_thread_id(self, tmp_path):
        ...
        data = json.loads((jacky_home / ".update_pending.json").read_text())
    ...
    async def test_popen_failure_cleans_up(self, tmp_path):
        ...
        assert not (jacky_home / ".update_pending.json").exists()
        assert not (jacky_home / ".update_exit_code").exists()
...
class TestUpdateCommandPlatformGate:
    ...
    async def test_blocks_programmatic_interface(self, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_MANAGED", "")
    ...
    async def test_allows_plugin_platform_via_registry_fallback(self, monkeypatch):
        ...
        from jacky_cli.plugins import PluginManager
        ...
        # Later steps may legitimately return success ("Starting Jacky
...
class TestSendUpdateNotification:
    ...
    async def test_defers_notification_while_update_still_running(self, tmp_path):
        ...
        (jacky_home / ".update_output.txt").write_text("still running")
    ...
    async def test_recovers_from_claimed_pending_file(self, tmp_path):
        ...
        claimed_path = jacky_home / ".update_pending.claimed.json"
        claimed_path.write_text(json.dumps({
            "platform": "telegram", "chat_id": "67890", "user_id": "12345",
        }))
        (jacky_home / ".update_output.txt").write_text("done")
        (jacky_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_notification_with_output(self, tmp_path):
        ...
        (jacky_home / ".update_pending.json").write_text(json.dumps(pending))
        (jacky_home / ".update_output.txt").write_text(
    ...
    async def test_truncates_long_output(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (jacky_home / ".update_pending.json").write_text(json.dumps(pending))
        (jacky_home / ".update_output.txt").write_text("x" * 5000)
        (jacky_home / ".update_exit_code").write_text("0")
    ...
    async def test_sends_failure_message_when_update_fails(self, tmp_path):
        ...
        pending = {"platform": "telegram", "chat_id": "111", "user_id": "222"}
        (jacky_home / ".update_pending.json").write_text(json.dumps(pending))
        (jacky_home / ".update_output.txt").write_text("Traceback: boom")
        (jacky_home / ".update_exit_code").write_text("1")
    ...
    async def test_cleans_up_files_after_notification(self, tmp_path):
        ...
        pending_path = jacky_home / ".update_pending.json"
        output_path = jacky_home / ".update_output.txt"
        exit_code_path = jacky_home / ".update_exit_code"
        pending_path.write_text(json.dumps({
    ...
    async def test_no_adapter_for_platform_preserves_markers(self, tmp_path):
        ...
        assert not (jacky_home / ".update_pending.claimed.json").exists()

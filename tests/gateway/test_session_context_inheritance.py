...
variant that the ``_UNSET``-strip guard does NOT catch:
    ...
    *concurrent* message A had ALREADY called ``set_session_vars``, B inherits
    ...
    ``JACKY_SESSION_*`` identity through the subprocess-env bridge. The bridge's
...
def _spawn_view():
    ...
    env = _make_run_env({})
    return {
        "JACKY_SESSION_CHAT_ID": env.get("JACKY_SESSION_CHAT_ID"),
        "JACKY_SESSION_THREAD_ID": env.get("JACKY_SESSION_THREAD_ID"),
        "JACKY_SESSION_KEY": env.get("JACKY_SESSION_KEY"),
    }
...
def test_child_task_inherits_foreign_session_without_reset():
    ...
    assert captured["window"]["JACKY_SESSION_CHAT_ID"] == "MINE_CHAT", (
...
def test_reset_session_vars_closes_inheritance_leak():
    ...
    for var in ("JACKY_SESSION_CHAT_ID", "JACKY_SESSION_THREAD_ID", "JACKY_SESSION_KEY"):
    ...
    assert captured["bound"]["JACKY_SESSION_CHAT_ID"] == "FOREIGN_CHAT"
    assert captured["bound"]["JACKY_SESSION_KEY"] == FOREIGN["session_key"]
...
# flag read via ``async_delivery_supported()``, not a string ``JACKY_SESSION_*``

...
class LongPreviewAgent:
    ...
    LONG_CMD = "cd /home/teknium/.jacky/jacky-agent/.worktrees/jacky-d8860339 && source .venv/bin/activate && python -m pytest tests/gateway/test_run_progress_topics.py -n0 -q"
...
async def test_run_agent_progress_stays_in_originating_topic(monkeypatch, tmp_path):
    monkeypatch.setenv("JACKY_TOOL_PROGRESS_MODE", "all")
    ...
    monkeypatch.setitem(sys.modules, "jacky_cli.run_agent", fake_run_agent)
...
async def test_terminal_progress_verbose_shows_full_command(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("JACKY_TOOL_PROGRESS_MODE", "verbose")
...
async def test_run_agent_relays_thinking_when_tool_progress_off(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setenv("JACKY_TOOL_PROGRESS_MODE", "off")
    adapter, result = await _run_with_agent(
        monkeypatch,
        tmp_path,
        ThinkingAgent,

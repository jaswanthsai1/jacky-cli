...
def _patch_config(monkeypatch, cfg):
    """Stub ``jacky_cli.config.read_raw_config`` to return ``cfg``."""
    import jacky_cli.config
    monkeypatch.setattr(jacky_cli.config, "read_raw_config", lambda: cfg)
...
def test_config_read_exception_falls_back_to_default(monkeypatch):
    ...
    monkeypatch.setattr(jacky_cli.config, "read_raw_config", _boom)
    assert _read_discord_prompt_timeout() == _DISCORD_PROMPT_TIMEOUT_DEFAULT

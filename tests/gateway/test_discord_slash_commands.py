...
def adapter():
    ...
    adapter._client = SimpleNamespace(
        ...
        user=SimpleNamespace(id=99999, name="JackyBot"),
    )
...
async def test_auto_registers_plugin_commands_for_discord(adapter):
    ...
    with patch(
        "jacky_cli.plugins.get_plugin_commands",
        return_value={
            "metricas": {
...
async def test_slash_command_registration_stays_under_discord_limit(adapter):
    ...
    with patch("jacky_cli.plugins.get_plugin_commands", return_value=many_plugins):
        adapter._register_slash_commands()
...
async def test_handle_thread_create_slash_dispatches_session_when_message_provided(adapter):
    ...
    await adapter._handle_thread_create_slash(interaction, "Planning", "Hello Jacky", 1440)

    adapter._dispatch_thread_session.assert_awaited_once_with(
        interaction, "555", "Planning", "Hello Jacky",
    )
...
async def test_auto_create_thread_uses_message_content_as_name(adapter):
    ...
    assert thread._jacky_auto_thread_initial_name == "Hello world, how are you?"
...
@pytest.mark.asyncio
async def test_auto_create_thread_falls_back_to_jacky_when_only_mentions(adapter):
    """If a message contains only mention syntax, the stripped content is
    empty — fall back to the 'Jacky' default rather than ''."""
    thread = SimpleNamespace(id=999, name="Jacky")
    message = SimpleNamespace(
...
async def test_auto_create_thread_falls_back_to_seed_message(adapter):
    ...
    result = await adapter._auto_create_thread(message)
    assert result is thread
    message.channel.send.assert_awaited_once_with("🧵 Thread created by Jacky: **Hello**")
    seed_message.create_thread.assert_awaited_once_with(
        name="Hello",
        auto_archive_duration=1440,
...
async def test_rename_thread_edits_only_when_current_name_matches(adapter):
    ...
    thread.edit.assert_awaited_once_with(
        ...
        reason="Jacky semantic session title",
    )
...
async def test_auto_thread_source_carries_initial_name_for_semantic_rename(adapter, monkeypatch):
    ...
    thread = SimpleNamespace(
        id=999,
        name="raw user prompt",
        _jacky_auto_thread_initial_name="raw user prompt",
    )
    adapter._auto_create_thread = AsyncMock(return_value=thread)
...
def test_discord_auto_thread_config_bridge(monkeypatch, tmp_path):
    ...
    jacky_dir.mkdir()
    config_path = jacky_dir / "config.yaml"
    config_path.write_text(yaml.dump({
        "discord": {"auto_thread": True},
    }))

    monkeypatch.delenv("DISCORD_AUTO_THREAD", raising=False)
    monkeypatch.setenv("JACKY_HOME", str(jacky_dir))
...
def test_register_skill_command_is_flat_not_nested(adapter):
    ...
    with patch(
        "jacky_cli.commands.discord_skill_commands_by_category",

...
class TestGatewayPidState:
    def test_write_pid_file_records_gateway_metadata(self, tmp_path, monkeypatch):
        ...
        assert payload["kind"] == "jacky-gateway"
    ...
    def test_get_running_pid_cleans_stale_record_from_dead_process(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": dead_pid,
            "kind": "jacky-gateway",
            "argv": ["python", "-m", "jacky_cli.main", "gateway", "run"],
            "start_time": 111,
        }))
    ...
    def test_get_running_pid_accepts_gateway_metadata_when_cmdline_unavailable(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": os.getpid(),
            "kind": "jacky-gateway",
            "argv": ["python", "-m", "jacky_cli.main", "gateway"],
            "start_time": 123,
        }))
    ...
    def test_get_running_pid_accepts_script_style_gateway_cmdline(self, tmp_path, monkeypatch):
        ...
        pid_path.write_text(json.dumps({
            "pid": os.getpid(),
            "kind": "jacky-gateway",
            "argv": ["/venv/bin/python", "/repo/jacky_cli/main.py", "gateway", "run", "--replace"],
            "start_time": 123,
        }))
        ...
        monkeypatch.setattr(
            ...
            lambda pid: "/venv/bin/python /repo/jacky_cli/main.py gateway run --replace",
        )
    ...
    def test_get_running_pid_accepts_no_supervisor_restart_runtime(self, tmp_path, monkeypatch):
        ...
        record = {
            "pid": os.getpid(),
            "kind": "jacky-gateway",
            "argv": ["python", "-m", "jacky_cli.main", "gateway", "restart"],
            "start_time": 123,
        }
        ...
        monkeypatch.setattr(
            status,
            "_read_process_cmdline",
            lambda pid: "python -m jacky_cli.main gateway restart",
        )
    ...
    def test_gateway_identity_files_use_process_home_not_context_override(
        ...
        """Regression: pid/lock/state files must use process-level JACKY_HOME.
        ...
        monkeypatch.setenv("JACKY_HOME", str(process_home))

        # Simulate a profile context override being active during write.
        token = set_jacky_home_override(str(profile_home))
        try:
            status.write_pid_file()
        finally:
...
class TestGatewayRuntimeStatus:
    ...
    def test_write_runtime_status_overwrites_stale_argv_on_restart(self, tmp_path, monkeypatch):
        ...
        state_path.write_text(json.dumps({
            ...
            "start_time": 1000.0,
            "kind": "jacky-gateway",
            "argv": ["/old/path/jacky", "gateway", "run"],
            "platforms": {},
            "updated_at": "2025-01-01T00:00:00Z",
        }))

        monkeypatch.setattr(status.sys, "argv", ["/new/path/jacky", "gateway", "run"])
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 2000)

        status.write_runtime_status(gateway_state="running")

        payload = status.read_runtime_status()
        assert payload["argv"] == ["/new/path/jacky", "gateway", "run"]
        assert payload["pid"] == os.getpid()
    ...
    def test_runtime_status_running_pid_rejects_stale_record_for_supervisor_pid(self, monkeypatch):
        ...
        payload = {
            ...
            "gateway_state": "running",
            "kind": "jacky-gateway",
            "argv": ["/opt/jacky/.venv/bin/jacky", "gateway", "run", "--replace"],
        }
    ...
    def test_runtime_status_running_pid_rejects_pid_reused_by_other_profile(self, monkeypatch):
        ...
        onto the live *default* gateway (``jacky gateway run``).  The recorded
        ...
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "jacky gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_accepts_matching_profile_cmdline(self, monkeypatch):
        ...
        for cmdline in (
            "jacky -p coder gateway run --replace",
            "/opt/jacky/.venv/bin/jacky --profile coder gateway run --replace",
            "jacky_home=/opt/data/profiles/coder jacky gateway run --replace",
        ):
    ...
    def test_runtime_status_running_pid_default_profile_rejects_named_cmdline(self, monkeypatch):
        ...
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: None)
        monkeypatch.setattr(
            status, "_read_process_cmdline", lambda pid: "jacky -p coder gateway run --replace"
        )
    ...
    def test_runtime_status_running_pid_default_profile_accepts_bare_cmdline(self, monkeypatch):
        """The default/root gateway (bare ``jacky gateway run``) is reported
        running for the default profile."""
        payload = {
            "pid": 139,
            "gateway_state": "running",
...
class TestScopedLocks:
    ...
    def test_acquire_scoped_lock_rejects_live_other_process(self, tmp_path, monkeypatch):
        monkeypatch.setenv("JACKY_GATEWAY_LOCK_DIR", str(tmp_path / "locks"))
    ...
    def test_acquire_scoped_lock_replaces_pid_reused_by_unrelated_process(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "jacky-gateway",
            "argv": ["/Users/user/.jacky/jacky-agent/jacky_cli/main.py", "gateway", "run", "--replace"],
        }))
    ...
    def test_acquire_scoped_lock_keeps_lock_when_cmdline_unreadable_but_record_is_gateway(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": None,
            "kind": "jacky-gateway",
            "argv": ["jacky_cli/main.py", "gateway", "run"],
        }))
    ...
    def test_acquire_scoped_lock_replaces_reused_pid_even_with_matching_start_time(self, tmp_path, monkeypatch):
        ...
        lock_path.write_text(json.dumps({
            ...
            "start_time": 123,
            "kind": "jacky-gateway",
            "argv": ["/usr/bin/python", "-m", "jacky_cli.main", "gateway", "run"],
        }))
...
class TestTakeoverMarker:
    ...
    def test_write_marker_records_replacer_jacky_home(self, tmp_path, monkeypatch):
        """The marker stamps the replacer's JACKY_HOME for cross-profile guard (#29092)."""
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        monkeypatch.setattr(status, "_get_process_start_time", lambda pid: 42)
        ...
        payload = json.loads((tmp_path / ".gateway-takeover.json").read_text())
        assert payload["replacer_jacky_home"] == str(tmp_path)
    ...
    def test_consume_rejects_marker_from_different_profile(self, tmp_path, monkeypatch):
        ...
        JACKY_HOME must be rejected even when PID + start_time coincidentally
        match — otherwise two profile services sharing a default ~/.jacky flap
        ...
        marker_path.write_text(json.dumps({
            ...
            "replacer_pid": 99999,
            "replacer_jacky_home": str(tmp_path / "profiles" / "other"),
            "written_at": datetime.now(timezone.utc).isoformat(),
        }))
    ...
    def test_consume_accepts_legacy_marker_without_jacky_home(self, tmp_path, monkeypatch):
        """Back-compat (#29092): markers written by older Jacky versions have no
        ``replacer_jacky_home`` field; an absent field is treated as same-home so
...
class TestPlannedStopMarker:
    ...
    def test_consume_returns_true_on_windows_when_start_time_unavailable(
        ...
        it would always return False — and ``jacky gateway stop`` would be
...
class TestReadProcessCmdlinePsFallback:
    ...
    def test_proc_cmdline_takes_priority_over_ps(self, monkeypatch):
        ...
        def fake_read_bytes(self):
            calls.append("proc")
            return b"python\x00jacky_cli/main.py\x00gateway\x00"

        monkeypatch.setattr(status.Path, "read_bytes", fake_read_bytes)
        result = status._read_process_cmdline(12345)
        assert "jacky_cli/main.py" in result
        assert calls == ["proc"]
    ...
    def test_ps_fallback_used_when_proc_returns_empty(self, monkeypatch):
        ...
        monkeypatch.setattr(
            status.subprocess, "run",
            lambda args, **kwargs: SimpleNamespace(returncode=0, stdout="python jacky_cli/main.py gateway run\n"),
        )
        result = status._read_process_cmdline(12345)
        assert "jacky_cli/main.py" in result
    ...
    def test_windows_skips_ps_fallback_and_uses_psutil(self, monkeypatch):
        ...
        class _Proc:
            ...
            def cmdline(self):
                return ["pythonw.exe", "-m", "jacky_cli.main", "gateway", "run"]
        ...
        assert result == "pythonw.exe -m jacky_cli.main gateway run"
        assert ps_calls == []

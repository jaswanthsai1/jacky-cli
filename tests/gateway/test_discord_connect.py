...
class FakeBot:
    def __init__(self, *, intents, proxy=None, allowed_mentions=None, **_):
        ...
        self.user = SimpleNamespace(id=999, name="Jacky")
        self._events = {}
        self.tree = FakeTree()
...
async def test_connect_ready_wait_uses_gateway_platform_connect_timeout(monkeypatch):
    ...
    monkeypatch.setenv("JACKY_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "90")
    monkeypatch.setattr("gateway.status.acquire_scoped_lock", lambda scope, identity, metadata=None: (True, None))
    monkeypatch.setattr("gateway.status.release_scoped_lock", lambda scope, identity: None)
...
async def test_safe_sync_slash_commands_only_mutates_diffs():
    ...
    desired_same = {
        "name": "status",
        "description": "Show Jacky session status",
        "type": 1,
        "options": [],
        "nsfw": False,
        "dm_permission": True,
...
async def test_post_connect_initialization_reraises_non_rate_limit_exceptions(tmp_path, monkeypatch):
    ...
    class _DesiredCommand:
        def to_dict(self, tree):
            return {"name": "status", "description": "Show Jacky status", "type": 1, "options": []}

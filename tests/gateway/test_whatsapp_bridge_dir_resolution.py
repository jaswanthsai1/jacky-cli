...
(/opt/jacky/scripts/whatsapp-bridge) is read-only, so `npm install` fails
with EACCES. The resolver must detect the read-only install dir and mirror the
bridge source into a writable JACKY_HOME location instead.
"""
import importlib
...
def test_writable_install_returns_install_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.jacky_constants.get_jacky_home", lambda: jacky_home
    )

    resolved = whatsapp_common.resolve_whatsapp_bridge_dir()
    assert resolved == install_bridge
    # Nothing mirrored into JACKY_HOME.
    assert not (jacky_home / "scripts" / "whatsapp-bridge").exists()


def test_readonly_install_mirrors_to_jacky_home(tmp_path, monkeypatch):
    """A read-only install tree is mirrored into a writable JACKY_HOME."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)
    ...
    expected = jacky_home / "scripts" / "whatsapp-bridge"
...
def test_readonly_install_reuses_existing_mirror(tmp_path, monkeypatch):
    """If the JACKY_HOME mirror already exists, return it without re-copying."""
    install_root = tmp_path / "install"
    install_bridge = install_root / "scripts" / "whatsapp-bridge"
    _seed_install_tree(install_bridge)

    jacky_home = tmp_path / "jacky_home"
    mirror = jacky_home / "scripts" / "whatsapp-bridge"

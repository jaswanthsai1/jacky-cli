...
Precedence verified here:
    JACKY_MAX_TOKENS env  >  model.max_tokens  >  per-provider
...
def isolated_home(tmp_path, monkeypatch):
    """Isolated JACKY_HOME with a writable config.yaml and a clean module cache.

    These tests deliberately re-import ``jacky_cli`` / ``gateway`` so each
    ...
    monkeypatch.delenv("JACKY_MAX_TOKENS", raising=False)

    _saved = {
        k: v
        for k, v in sys.modules.items()
        if k.startswith(("jacky_cli", "gateway"))
    }

    def write_cfg(body: str) -> None:
        (jacky_home / "config.yaml").write_text(textwrap.dedent(body))

    def fresh_gateway():
        for mod in list(sys.modules.keys()):
            if mod.startswith(("jacky_cli", "gateway")):
                del sys.modules[mod]
...
def test_env_override_beats_everything(isolated_home, monkeypatch):
    """JACKY_MAX_TOKENS is the internal override mechanism (highest priority)."""
    write_cfg, fresh_gateway = isolated_home
    monkeypatch.setenv("JACKY_MAX_TOKENS", "2048")
    write_cfg(
        """
        model:
          default: glm-5.1
...
def test_lift_helper_accepts_alias_and_rejects_garbage(isolated_home):
    ...
    write_cfg("model:\n  provider: openrouter\n")
    for mod in list(sys.modules.keys()):
        if mod.startswith("jacky_cli"):
            del sys.modules[mod]
    rp = importlib.import_module("jacky_cli.runtime_provider")

...
``jacky_cli/setup.py::_setup_slack`` before the Slack adapter migrated to a
bundled plugin (#41112). ``interactive_setup`` lazy-imports its CLI helpers
from ``jacky_cli.config`` (get_env_value / save_env_value) and
``jacky_cli.cli_output`` (prompt / prompt_yes_no / print_*), so we patch those
source modules.
"""
import jacky_cli.config as config_mod
import jacky_cli.cli_output as cli_output_mod
...
def _patch_setup_io(monkeypatch, prompts, saved):
    ...
    # Manifest writing reaches out to jacky_cli.slack_cli + filesystem; stub it.
    import jacky_cli.slack_cli as slack_cli_mod

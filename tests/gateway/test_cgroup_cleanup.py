...
class TestOwnCgroupPath:
    def test_parses_v2_cgroup_path(self, tmp_path, monkeypatch):
        ...
        proc_self.write_text("0::/user.slice/user-1000.slice/jacky-gateway.service\n")
        monkeypatch.setattr(
            cgroup_cleanup,
            "Path",
        ...
        assert cgroup_cleanup._own_cgroup_path() == "/user.slice/user-1000.slice/jacky-gateway.service"
...
class TestReapCgroup:
    def test_skips_own_pid_and_kills_the_rest(self, tmp_path, monkeypatch):
        own = os.getpid()
        cgroup_path = "/test.slice/jacky-gateway.service"
    ...
    def test_noop_when_procs_file_missing(self, tmp_path, monkeypatch):
        cgroup_path = "/missing.slice/jacky-gateway.service"
        monkeypatch.setattr(
            cgroup_cleanup,
            "Path",

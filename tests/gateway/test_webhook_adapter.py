...
class TestPayloadFilters:
    ...
    async def test_filter_accepts_nested_any_and_in_file(self, tmp_path, monkeypatch):
        """Nested any groups can match dynamic watchlists under JACKY_HOME."""
                        ...
                        "any": [
                            {
                                ...
                                "in_file": "~/.jacky/data/watchlist.json",
                            },
                            {
    ...
    @pytest.mark.asyncio
    async def test_script_tilde_jacky_path_resolves_to_active_profile_home(self, tmp_path, monkeypatch):
        """~/.jacky/scripts paths must resolve through JACKY_HOME for profiles."""
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        scripts = tmp_path / "scripts"
        scripts.mkdir()
        ...
        routes = {
            "todoist": {
                "secret": _INSECURE_NO_AUTH,
                "script": "~/.jacky/scripts/todoist_filter.py",
                "prompt": "Task: {body}",
            }
        }
        adapter = _make_adapter(routes=routes)

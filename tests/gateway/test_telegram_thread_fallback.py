...
Supergroup forum topics route with ``message_thread_id``. Jacky-created
...
async def test_send_typing_attempts_api_call_for_dm_topic_reply_fallback():
    """Jacky-created DM topic lanes should still attempt scoped typing.
...
@pytest.mark.asyncio
async def test_send_uses_reply_fallback_for_jacky_dm_topics():
    """Jacky-created Telegram DM topics route with thread id plus reply anchor."""
    adapter = _make_adapter()
    call_log = []
...
async def test_send_uses_metadata_reply_fallback_for_streaming_dm_topics():
    """Metadata-only sends still stay in Jacky-created Telegram DM topics."""
    adapter = _make_adapter()
    call_log = []

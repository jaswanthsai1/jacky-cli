...
def test_discord_setup_existing_token_warns_fail_closed_not_fail_open(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.config.get_env_value", fake_get_env_value)
    monkeypatch.setattr("jacky_cli.config.save_env_value", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("jacky_cli.cli_output.print_header", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("jacky_cli.cli_output.print_success", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("jacky_cli.cli_output.print_info", lambda msg="", **_kwargs: info_lines.append(str(msg)))
    monkeypatch.setattr("jacky_cli.cli_output.prompt", lambda *_args, **_kwargs: "")
    monkeypatch.setattr("jacky_cli.cli_output.prompt_yes_no", lambda *_args, **_kwargs: next(yes_no_answers))
...
def test_discord_setup_new_token_empty_allowlist_warns_denied_until_configured(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.config.get_env_value", lambda _key: "")
    monkeypatch.setattr("jacky_cli.config.save_env_value", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("jacky_cli.cli_output.print_header", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("jacky_cli.cli_output.print_success", lambda *_args, **_kwargs: None)
    monkeypatch.setattr("jacky_cli.cli_output.print_info", lambda msg="", **_kwargs: info_lines.append(str(msg)))
    monkeypatch.setattr("jacky_cli.cli_output.prompt", lambda *_args, **_kwargs: next(prompts))
    monkeypatch.setattr("jacky_cli.cli_output.prompt_yes_no", lambda *_args, **_kwargs: False)

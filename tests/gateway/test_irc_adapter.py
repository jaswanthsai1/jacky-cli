...
class TestIRCProtocolHelpers:
    ...
    def test_parse_numeric_reply(self):
        msg = _parse_irc_message(":server 001 jacky-bot :Welcome to IRC")
        assert msg["prefix"] == "server"
        assert msg["command"] == "001"
        assert msg["params"] == ["jacky-bot", "Welcome to IRC"]

    def test_parse_nick_collision(self):
        msg = _parse_irc_message(":server 433 * jacky-bot :Nickname is already in use")
...
class TestIRCAdapterInit:
    ...
    def test_init_from_config_extra(self, monkeypatch):
        ...
        assert adapter.nickname == "jacky"
        assert adapter.channel == "#jacky-dev"
...
class TestIRCAdapterMessageParsing:
    ...
    async def test_handle_welcome(self, adapter):
        ...
        await adapter._handle_line(":server 001 jacky :Welcome to IRC")
    ...
    async def test_handle_nick_collision(self, adapter):
        ...
        await adapter._handle_line(":server 433 * jacky :Nickname in use")
        assert adapter._current_nick == "jacky_"
        sent = writer.write.call_args[0][0]
        assert b"NICK jacky_" in sent
    ...
    async def test_handle_addressed_channel_message(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG #test :jacky: hello there")
    ...
    async def test_handle_dm(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG jacky :private message")
        assert len(dispatched) == 1
    ...
    async def test_ignores_own_messages(self, adapter):
        ...
        await adapter._handle_line(":jacky!bot@host PRIVMSG #test :my own msg")
        assert len(dispatched) == 0
    ...
    async def test_ctcp_action_converted(self, adapter):
        ...
        await adapter._handle_line(":user!u@host PRIVMSG jacky :\x01ACTION waves\x01")
        assert len(dispatched) == 1
    ...
    async def test_allowed_users_case_insensitive(self, monkeypatch):
        ...
        await adapter._handle_line(":admin!u@host PRIVMSG #test :jacky: hello")
        assert len(dispatched) == 1
    ...
    async def test_unauthorized_user_blocked(self, monkeypatch):
        ...
        await adapter._handle_line(":eve!u@host PRIVMSG #test :jacky: hello")
        assert len(dispatched) == 0
    ...
    async def test_nick_collision_retry(self, adapter):
        ...
        await adapter._handle_line(":server 433 * jacky :Nickname in use")
        assert adapter._current_nick == "jacky_"
        await adapter._handle_line(":server 433 * jacky_ :Nickname in use")
        assert adapter._current_nick == "jacky_1"
        await adapter._handle_line(":server 433 * jacky_1 :Nickname in use")
        assert adapter._current_nick == "jacky_2"
...
class TestIRCStandaloneSend:
    ...
    async def test_standalone_send_completes_handshake_and_sends_privmsg(self, monkeypatch):
        ...
        monkeypatch.setenv("IRC_NICKNAME", "jackybot")
        monkeypatch.setenv("IRC_USE_TLS", "false")

        # Server greets us with 001 RPL_WELCOME, then nothing for QUIT drain.
        conn = _FakeIRCConnection([b":server 001 jackybot-cron :Welcome"])
        ...
        assert any(line.startswith("NICK jackybot-cron") for line in sent_lines)
        assert any(line.startswith("USER jackybot-cron 0 * :Jacky Agent (cron)")
    ...
    async def test_standalone_send_strips_crlf_from_message_body(self, monkeypatch):
        ...
        assert all(line.startswith("NICK jackybot-cron") for line in nick_lines)
    ...
    async def test_standalone_send_joins_channel_before_privmsg(self, monkeypatch):
        ...
        conn = _FakeIRCConnection([
            b":server 001 jackybot-cron :Welcome",
            b":server 366 jackybot-cron #cron :End of /NAMES list.",
        ])

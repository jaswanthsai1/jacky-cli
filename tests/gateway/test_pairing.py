...
class TestSplitPairingDirMigration:
    def test_merges_new_approved_into_active_legacy_dir(self, tmp_path):
        ...
        with patch("gateway.pairing.PAIRING_DIR", legacy), patch("gateway.pairing.get_jacky_home", return_value=home):
            store = PairingStore()
...
class TestProfileScopedStorage:
    ...
    under <JACKY_HOME>/profiles/<name>/pairing/ so a multiplexing gateway
    ...
    def test_default_store_uses_global_dir(self, tmp_path, monkeypatch):
        ...
        ``jacky pairing`` CLI continues to work without a profile context."""
        from jacky_cli.jacky_constants import get_jacky_home
        monkeypatch.setattr("jacky_cli.jacky_constants.get_jacky_home", lambda: tmp_path)
    ...
    def test_profile_store_uses_profiles_subdir(self, tmp_path, monkeypatch):
        ...
        <JACKY_HOME>/profiles/yangyang/pairing/."""
        from jacky_cli.jacky_constants import get_jacky_home
        monkeypatch.setattr("jacky_cli.jacky_constants.get_jacky_home", lambda: tmp_path)

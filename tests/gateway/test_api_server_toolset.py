"""Tests for jacky-api-server toolset and API server tool availability."""
...
class TestJackyApiServerToolset:
    """Tests for the jacky-api-server toolset definition."""

    def test_toolset_exists(self):
        ts = get_toolset("jacky-api-server")
        assert ts is not None

    def test_toolset_validates(self):
        assert validate_toolset("jacky-api-server")

    def test_toolset_includes_web_tools(self):
        tools = resolve_toolset("jacky-api-server")
...
class TestApiServerPlatformConfig:
    def test_platforms_dict_includes_api_server(self):
        ...
        assert PLATFORMS["api_server"]["default_toolset"] == "jacky-api-server"
    ...
    def test_default_api_server_includes_terminal_toolset(self):
        ...
        jacky-api-server composite, so the old all-tools subset test dropped
...
class TestApiServerAdapterToolset:
    ...
    def test_create_agent_reads_config_toolsets(self):
        ...
        with patch("gateway.run._resolve_runtime_agent_kwargs") as mock_kwargs, \
            ...
            # No platform_toolsets override — should fall back to jacky-api-server default

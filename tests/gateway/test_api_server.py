...
class TestAdapterInit:
    ...
    def test_create_agent_forwards_config_reasoning_effort(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.tools_config._get_platform_tools", lambda *_: set())
...
class TestConcurrencyCap:
    def test_resolve_defaults_to_10_when_unset(self):
        with patch("jacky_cli.config.load_config", return_value={}):
    ...
    def test_resolve_reads_config_value(self):
        ...
        with patch("jacky_cli.config.load_config", return_value=cfg):
...
class TestHealthEndpoint:
    ...
    async def test_health_returns_ok(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["platform"] == "jacky-agent"
...
class TestModelsEndpoint:
    @pytest.mark.asyncio
    async def test_models_returns_jacky_agent(self, adapter):
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            ...
            assert data["data"][0]["id"] == "jacky-agent"
            assert data["data"][0]["owned_by"] == "jacky"
    ...
    def test_resolve_model_name_default_profile(self):
        """Default profile falls back to 'jacky-agent'."""
        with patch("jacky_cli.profiles.get_active_profile_name", return_value="default"):
            assert APIServerAdapter._resolve_model_name("") == "jacky-agent"

    def test_resolve_model_name_named_profile(self):
        """Named profile uses the profile name as model name."""
        with patch("jacky_cli.profiles.get_active_profile_name", return_value="lucas"):
...
class TestCapabilitiesEndpoint:
    ...
    async def test_capabilities_advertises_plugin_safe_contract(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["object"] == "jacky.api_server.capabilities"
            assert data["platform"] == "jacky-agent"
            assert data["model"] == "jacky-agent"
            ...
            assert data["features"]["session_continuity_header"] == "X-Jacky-Session-Id"
...
class TestToolsetsEndpoint:
    ...
    async def test_toolsets_returns_resolved_tools(self, adapter):
        ...
        with patch(
            "jacky_cli.tools_config._get_effective_configurable_toolsets",
            return_value=fake_toolsets,
        ), patch(
            "jacky_cli.tools_config._get_platform_tools",
...
class TestChatCompletionsEndpoint:
    ...
    async def test_stream_includes_tool_progress(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                ...
                assert "event: jacky.tool.progress" in body
    ...
    async def test_stream_emits_tool_lifecycle_with_call_id(self, adapter):
        ...
        ``tool.started``-style ``jacky.tool.progress`` event; clients
        ...
        of the lifecycle pair on the same ``event: jacky.tool.progress``
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            for i, line in enumerate(lines):
                if line.strip() != "event: jacky.tool.progress":
                    continue
...
class TestResponsesEndpoint:
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp1 = await cli.post(
                    "/v1/responses",
                    json={"model": "jacky-agent", "input": "What is 1+1?"},
                )
    ...
    async def test_previous_response_id_preserves_session(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp1 = await cli.post(
                    "/v1/responses",
                    json={"model": "jacky-agent", "input": "Hello"},
                )
            assert resp1.status == 200
    ...
    async def test_invalid_input_type_returns_400(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/responses",
                json={"model": "jacky-agent", "input": 42},
            )
            assert resp.status == 400
    ...
    async def test_stream_true_returns_responses_sse(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "jacky-agent", "input": "hi", "stream": True},
                )
                assert resp.status == 200
                assert "text/event-stream" in resp.headers.get("Content-Type", "")
                body = await resp.text()
    ...
    async def test_stream_emits_function_call_and_output_items(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "jacky-agent", "input": "read the file", "stream": True},
                )
                assert resp.status == 200
                body = await resp.text()
    ...
    async def test_streamed_response_is_stored_for_get(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", side_effect=_mock_run_agent):
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "jacky-agent", "input": "store this", "stream": True},
                )
                body = await resp.text()
                response_id = None
                for line in body.splitlines():
    ...
    async def test_get_stored_response(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "jacky-agent", "input": "Hi"},
                )
    ...
    async def test_tool_calls_in_output(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    json={"model": "jacky-agent", "input": "What is 6*7?"},
                )
...
class TestChatCompletionsAgentIncomplete:
    ...
    async def test_truncation_with_partial_text_uses_length_finish_reason(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "tell me everything"}]},
                )
            assert resp.status == 200
            data = await resp.json()
            assert data["choices"][0]["finish_reason"] == "length"
            assert data["choices"][0]["message"]["content"] == "Here is part one of the answer"
            assert data["jacky"]["partial"] is True
            assert data["jacky"]["completed"] is False
            assert data["jacky"]["error_code"] == "output_truncated"
            assert resp.headers.get("X-Jacky-Completed") == "false"
            assert resp.headers.get("X-Jacky-Partial") == "true"
    ...
    async def test_hard_failure_redacts_secret_like_error_text(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "hello"}]},
                )
            ...
            body = json.dumps(data)
            assert raw_secret not in body
            assert raw_secret not in resp.headers.get("X-Jacky-Error", "")
            assert "OPENAI_API_KEY=" in body
            assert data["error"]["jacky"]["failed"] is True
    ...
    async def test_failure_with_no_text_returns_502_error_envelope(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "x"}]},
                )
            ...
            assert data["error"]["jacky"]["partial"] is True
            assert data["error"]["jacky"]["failed"] is True
            assert resp.headers.get("X-Jacky-Completed") == "false"
    ...
    async def test_normal_completion_unchanged(self, adapter):
        ...
        and no jacky extras (preserves the existing happy-path contract)."""
        mock_result = {
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            data = await resp.json()
            assert data["choices"][0]["finish_reason"] == "stop"
            assert data["choices"][0]["message"]["content"] == "All good."
            assert "jacky" not in data
            assert "X-Jacky-Completed" not in resp.headers
...
# ---------------------------------------------------------------------------
# X-Jacky-Session-Id header (session continuity)
# ---------------------------------------------------------------------------
...
class TestSessionIdHeader:
    ...
    async def test_new_session_response_includes_session_id_header(self, adapter):
        """Without X-Jacky-Session-Id, a new session is created and returned in the header."""
        mock_result = {"final_response": "Hello!", "messages": [], "api_calls": 1}
        app = _create_app(adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
            ...
            assert resp.status == 200
            assert resp.headers.get("X-Jacky-Session-Id") is not None

    @pytest.mark.asyncio
    async def test_provided_session_id_is_used_and_echoed(self, auth_adapter):
        """When X-Jacky-Session-Id is provided, it's passed to the agent and echoed in the response."""
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Jacky-Session-Id": "my-session-123", "Authorization": "Bearer sk-secret"},
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "Continue"}]},
                )

            assert resp.status == 200
            assert resp.headers.get("X-Jacky-Session-Id") == "my-session-123"
            call_kwargs = mock_run.call_args.kwargs
            assert call_kwargs["session_id"] == "my-session-123"

    @pytest.mark.asyncio
    async def test_traversal_session_id_header_rejected(self, auth_adapter):
        """Security (#5958): a path-traversal X-Jacky-Session-Id must be
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                for bad in ("../../../../etc/pwned", "/abs/path", "..\\win"):
                    resp = await cli.post(
                        "/v1/chat/completions",
                        headers={"X-Jacky-Session-Id": bad, "Authorization": "Bearer sk-secret"},
                        json={"model": "jacky-agent", "messages": [{"role": "user", "content": "hi"}]},
                    )
    ...
    async def test_provided_session_id_loads_history_from_db(self, auth_adapter):
        """When X-Jacky-Session-Id is provided, history comes from SessionDB not request body."""
        mock_result = {"final_response": "OK", "messages": [], "api_calls": 1}
        db_history = [
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Jacky-Session-Id": "existing-session", "Authorization": "Bearer sk-secret"},
    ...
    async def test_db_failure_falls_back_to_empty_history(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run, \
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={"X-Jacky-Session-Id": "some-session", "Authorization": "Bearer sk-secret"},
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "Hi"}]},
                )
...
# ---------------------------------------------------------------------------
# X-Jacky-Session-Key header (long-term memory scoping)
# ---------------------------------------------------------------------------
...
class TestSessionKeyHeader:
    ...
    async def test_session_key_passed_to_agent_and_echoed(self, auth_adapter):
        """X-Jacky-Session-Key reaches _run_agent as gateway_session_key and is echoed back."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Jacky-Session-Key": "webui:user-42",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Jacky-Session-Key") == "webui:user-42"
            call_kwargs = mock_run.call_args.kwargs
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Jacky-Session-Key": "channel-abc",
                        "X-Jacky-Session-Id": "transcript-xyz",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "hi"}]},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Jacky-Session-Key") == "channel-abc"
            assert resp.headers.get("X-Jacky-Session-Id") == "transcript-xyz"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_session_key_absent_yields_none(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            assert "X-Jacky-Session-Key" not in resp.headers
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_session_key_rejected_without_api_key(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={"X-Jacky-Session-Key": "whatever"},
                json={"model": "jacky-agent", "messages": [{"role": "user", "content": "hi"}]},
            )
            assert resp.status == 403
    ...
    async def test_session_key_rejects_control_chars(self, auth_adapter):
        ...
        """
        mock_request = MagicMock()
        mock_request.headers = {"X-Jacky-Session-Key": "bad\rvalue"}
    ...
    async def test_session_key_rejects_oversized(self, auth_adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            resp = await cli.post(
                "/v1/chat/completions",
                headers={"X-Jacky-Session-Key": "x" * 1000, "Authorization": "Bearer sk-secret"},
                json={"model": "jacky-agent", "messages": [{"role": "user", "content": "hi"}]},
            )
            assert resp.status == 400
        ...
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_create_agent", side_effect=_fake_create_agent):
                resp = await cli.post(
                    "/v1/chat/completions",
                    headers={
                        "X-Jacky-Session-Key": "agent:main:webui:dm:user-7",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "jacky-agent", "messages": [{"role": "user", "content": "hi"}]},
                )
    ...
    async def test_responses_endpoint_accepts_session_key(self, auth_adapter):
        """Responses API honors the same X-Jacky-Session-Key contract."""
        mock_result = {"final_response": "ok", "messages": [], "api_calls": 1}
        app = _create_app(auth_adapter)
        async with TestClient(TestServer(app)) as cli:
            with patch.object(auth_adapter, "_run_agent", new_callable=AsyncMock) as mock_run:
                ...
                resp = await cli.post(
                    "/v1/responses",
                    headers={
                        "X-Jacky-Session-Key": "webui:chan-1",
                        "Authorization": "Bearer sk-secret",
                    },
                    json={"model": "jacky-agent", "input": "hello", "store": False},
                )
            assert resp.status == 200
            assert resp.headers.get("X-Jacky-Session-Key") == "webui:chan-1"
            call_kwargs = mock_run.call_args.kwargs
    ...
    async def test_capabilities_advertises_session_key_header(self, adapter):
        ...
        async with TestClient(TestServer(app)) as cli:
            ...
            assert resp.status == 200
            data = await resp.json()
            assert data["features"]["session_key_header"] == "X-Jacky-Session-Key"
...
class TestModelRoutesParsing:
    ...
    def test_no_routes_configured(self):
        adapter = _make_routing_adapter({})
        assert adapter._resolve_route("jacky-agent") is None

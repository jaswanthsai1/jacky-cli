...
def _redirect_cache(tmp_path, monkeypatch):
    """Point document cache to tmp_path so tests don't touch ~/.jacky."""
    monkeypatch.setattr(
...
class TestAppMentionHandler:
    ...
    def test_app_mention_registered_on_connect(self):
        ...
        # covering every COMMAND_REGISTRY entry (e.g. /jacky, /btw, /stop,
        ...
        for expected in ("/jacky", "/btw", "/stop", "/model", "/help"):
...
class TestBangPrefixCommands:
    ...
    async def test_bang_with_bot_suffix_resolves(self, adapter):
        """``!stop@jacky`` matches the get_command() ``@suffix`` stripping."""
        await adapter._handle_slack_message(self._make_event("!stop@jacky"))

        msg_event = adapter.handle_message.call_args[0][0]
        assert msg_event.text.startswith("/stop@jacky")
        assert msg_event.message_type == MessageType.COMMAND
...
class TestSlashCommands:
    ...
    # instead of as /jacky subcommands. This is the Discord/Telegram parity
    # fix: the slash name itself becomes the command.
    # ------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_native_btw_slash(self, adapter):
        """/btw with args must dispatch to /background, not /jacky btw."""
        command = {
            "command": "/btw",
    ...
    @pytest.mark.asyncio
    async def test_legacy_jacky_prefix_still_works(self, adapter):
        """Backward compat: /jacky btw foo must still route to /btw foo.

        Old workspace manifests only declared /jacky as the single slash.
    ...
    @pytest.mark.asyncio
    async def test_legacy_jacky_freeform_question(self, adapter):
        """/jacky <free-form text> must stay as the raw text (non-command)."""
        command = {
            "command": "/jacky",
...
class TestSlashEphemeralAck:
    ...
    @pytest.mark.asyncio
    async def test_legacy_jacky_slash_stashes_context(self, adapter):
        """Legacy /jacky <subcommand> also stashes context."""
        command = {
            "command": "/jacky",
            "text": "help",
            "user_id": "U_H",
    ...
    @pytest.mark.asyncio
    async def test_freeform_jacky_question_does_not_stash_context(self, adapter):
        """Free-form /jacky <question> must NOT route agent reply ephemeral."""
        command = {
            "command": "/jacky",

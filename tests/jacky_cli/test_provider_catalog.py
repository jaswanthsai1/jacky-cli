"""Tests for the unified provider catalog (jacky_cli.provider_catalog).

These are invariant tests, not snapshots: they assert the parity *contract*
between what ``jacky model`` shows (``CANONICAL_PROVIDERS``) and what the
...
from jacky_cli.provider_catalog import (
    ProviderDescriptor,
    provider_catalog,
...
def test_catalog_covers_every_jacky_model_provider():
    """PARITY CONTRACT: the catalog == the `jacky model` universe."""
    slugs = {d.slug for d in provider_catalog()}
    for entry in CANONICAL_PROVIDERS:
        assert entry.slug in slugs, (
            f"{entry.slug} is shown in `jacky model` but missing from provider_catalog()"
        )


def test_catalog_has_no_providers_outside_jacky_model():
    """The catalog must not invent providers `jacky model` doesn't show."""

"""Tests for ``jacky gui`` desktop launcher wiring."""
...
from jacky_cli import main as cli_main
...
def _make_packaged_executable(root: Path, monkeypatch, platform: str = "darwin") -> Path:
    ...
    desktop_dir = root / "apps" / "desktop"
    if platform == "darwin":
        exe = desktop_dir / "release" / "mac-arm64" / "Jacky.app" / "Contents" / "MacOS" / "Jacky"
    elif platform == "win32":
        exe = desktop_dir / "release" / "win-unpacked" / "Jacky.exe"
    else:
        exe = desktop_dir / "release" / "linux-unpacked" / "jacky"
...
def test_gui_installs_packages_and_launches_desktop_app(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("jacky_cli.main._desktop_build_needed", return_value=True), \
         patch("jacky_cli.main._write_desktop_build_stamp"), \
         patch("jacky_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("jacky_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
...
def test_gui_forwards_desktop_environment_overrides(tmp_path, monkeypatch):
    root = _make_desktop_tree(tmp_path)
    jacky_root = tmp_path / "custom-jacky"
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=ok), \
         patch("jacky_cli.main._desktop_build_needed", return_value=True), \
         patch("jacky_cli.main._write_desktop_build_stamp"), \
         patch("jacky_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("jacky_cli.main.subprocess.run", side_effect=[ok, ok]) as mock_run, \
         pytest.raises(SystemExit):
        cli_main.cmd_gui(_ns(
            fake_boot=True,
            ignore_existing=True,
            jacky_root=str(jacky_root),
            cwd=str(cwd),
        ))

    launch_env = mock_run.call_args_list[1].kwargs["env"]
    assert launch_env["JACKY_DESKTOP_BOOT_FAKE"] == "1"
    assert launch_env["JACKY_DESKTOP_IGNORE_EXISTING"] == "1"
    assert launch_env["JACKY_DESKTOP_JACKY_ROOT"] == str(jacky_root)
    assert launch_env["JACKY_DESKTOP_CWD"] == str(cwd)
...
def test_gui_exits_when_npm_missing(tmp_path, monkeypatch, capsys):
    ...
    with patch("jacky_cli.main.shutil.which", return_value=None), \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_skip_build_launches_existing_packaged_app_without_npm(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value=None), \
         patch("jacky_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("jacky_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_configures_sandbox_before_launch(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("jacky_cli.main.subprocess.run", return_value=ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_rejects_symlink_sandbox(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/sudo"), \
         patch("jacky_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_falls_back_to_no_sandbox_when_userns_is_restricted(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
         patch("jacky_cli.main._desktop_linux_needs_no_sandbox", return_value=True), \
         patch("jacky_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_linux_exits_when_sandbox_fixup_fails_without_safe_fallback(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main._desktop_linux_sandbox_fixup", return_value=False), \
         patch("jacky_cli.main._desktop_linux_needs_no_sandbox", return_value=False), \
         patch("jacky_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns(skip_build=True))
...
def test_gui_source_mode_uses_renderer_build_and_electron(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("jacky_cli.main._desktop_build_needed", return_value=True), \
         patch("jacky_cli.main._write_desktop_build_stamp"), \
         patch("jacky_cli.main.subprocess.run", side_effect=[build_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_build_stamp_skips_build_when_up_to_date(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main._desktop_build_needed", return_value=False), \
         patch("jacky_cli.main._run_npm_install_deterministic") as mock_install, \
         patch("jacky_cli.main.subprocess.run", return_value=launch_ok) as mock_run, \
         patch("jacky_cli.main._desktop_macos_relaunchable_fixup"), \
         pytest.raises(SystemExit) as exc:
...
def test_desktop_force_build_overrides_stamp(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=install_ok) as mock_install, \
         patch("jacky_cli.main._desktop_build_needed", return_value=False), \
         patch("jacky_cli.main._write_desktop_build_stamp") as mock_stamp, \
         patch("jacky_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("jacky_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
...
def test_compute_desktop_content_hash_stable(tmp_path, monkeypatch):
    ...
    (root / "package.json").write_text('{"name":"jacky"}', encoding="utf-8")
...
def test_gui_retries_pack_once_after_purging_build_cache(tmp_path, monkeypatch):
    ...
    packaged_exe = root / "apps" / "desktop" / "release" / "linux-unpacked" / "jacky"
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("jacky_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("jacky_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("jacky_cli.main._write_desktop_build_stamp"), \
         patch("jacky_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("jacky_cli.main._electron_dist_ok", return_value=False), \
         patch("jacky_cli.main._redownload_electron_dist", return_value=True), \
         patch("jacky_cli.main.subprocess.run", side_effect=run_side_effect) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_redownloads_electron_via_mirror_then_repacks(tmp_path, monkeypatch, capsys):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("jacky_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("jacky_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("jacky_cli.main._electron_dist_ok", return_value=False), \
         patch("jacky_cli.main._redownload_electron_dist", side_effect=[False, True]) as mock_dl, \
         patch("jacky_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_retries_pack_under_mirror_even_when_prefetch_blocked(tmp_path, monkeypatch, capsys):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("jacky_cli.main._purge_electron_build_cache", return_value=[]), \
         patch("jacky_cli.main._electron_dist_ok", return_value=False), \
         patch("jacky_cli.main._redownload_electron_dist", return_value=False), \
         patch("jacky_cli.main.subprocess.run", side_effect=[pack_fail, pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_self_heals_electron_and_continues(tmp_path, monkeypatch, capsys):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("jacky_cli.main._desktop_linux_sandbox_fixup", return_value=True), \
         patch("jacky_cli.main._write_desktop_build_stamp"), \
         patch("jacky_cli.main._electron_dist_ok", return_value=False), \
         patch("jacky_cli.main._try_redownload_electron_dist", return_value=True) as mock_dl, \
         patch("jacky_cli.main.subprocess.run", side_effect=[pack_ok, launch_ok]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_install_failure_hard_fails_when_electron_dist_exists(tmp_path, monkeypatch, capsys):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=install_fail), \
         patch("jacky_cli.main._electron_dist_ok", return_value=True), \
         patch("jacky_cli.main.subprocess.run") as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_retry_after_packaged_executable_exists(tmp_path, monkeypatch, capsys):
    ...
    Jacky.app/Contents/MacOS/Jacky in place. Re-downloading Electron can't
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         ...
         patch("jacky_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("jacky_cli.main._purge_electron_build_cache", return_value=[Path("/c/electron.zip")]) as mock_purge, \
         patch("jacky_cli.main._redownload_electron_dist", return_value=True) as mock_dl, \
         patch("jacky_cli.main.subprocess.run", return_value=pack_fail) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_gui_does_not_override_user_electron_mirror(tmp_path, monkeypatch, capsys):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/npm"), \
         patch("jacky_cli.main._run_npm_install_deterministic", return_value=install_ok), \
         patch("jacky_cli.main._desktop_macos_relaunchable_fixup"), \
         patch("jacky_cli.main._purge_electron_build_cache", return_value=[]) as mock_purge, \
         patch("jacky_cli.main.subprocess.run", side_effect=[pack_fail]) as mock_run, \
         pytest.raises(SystemExit) as exc:
        cli_main.cmd_gui(_ns())
...
def test_redownload_electron_dist_missing_installer(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("jacky_cli.main.subprocess.run") as mock_run:
...
def test_redownload_electron_dist_runs_installer_with_mirror(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("jacky_cli.main.subprocess.run", side_effect=fake_run):
...
def test_redownload_electron_dist_returns_false_when_download_fails(tmp_path, monkeypatch):
    ...
    with patch("jacky_cli.main.shutil.which", return_value="/usr/bin/node"), \
         patch("jacky_cli.main.subprocess.run",
...
def test_stop_desktop_build_lock_terminates_only_release_procs(tmp_path, monkeypatch):
    ...
    locker_exe = release / "Jacky.exe"
    locker_exe.write_text("", encoding="utf-8")
    other_exe = tmp_path / "elsewhere" / "Jacky.exe"
...
def test_desktop_launch_options_survives_config_error():
    with patch("jacky_cli.config.load_config", side_effect=RuntimeError("boom")):
        flags, gpu = cli_main._desktop_launch_options()
    assert flags == []
    assert gpu == "auto"

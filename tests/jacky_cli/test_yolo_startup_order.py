"""Regression tests for #60328: --yolo must set JACKY_YOLO_MODE in
...
def _run_main_and_capture_yolo_at_startup(monkeypatch, argv):
    """Run main() with *argv*, capturing JACKY_YOLO_MODE at the
    ...
    def spy_prepare_startup(args):
        yolo_at_startup["value"] = os.environ.get("JACKY_YOLO_MODE")

    monkeypatch.setattr(
        "jacky_cli.main._prepare_agent_startup", spy_prepare_startup
    )
    # Stub cmd_chat so main() returns cleanly without entering chat.
    monkeypatch.setattr("jacky_cli.main.cmd_chat", lambda args: None)
    monkeypatch.delenv("JACKY_YOLO_MODE", raising=False)
    monkeypatch.setattr(sys, "argv", argv)

    from jacky_cli.main import main as cli_main
...
def test_top_level_yolo_flag_sets_env_before_startup(monkeypatch):
    """jacky --yolo must set JACKY_YOLO_MODE before
    _prepare_agent_startup imports tools.approval."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["jacky", "--yolo"]
    )
    assert result == "1", (
        "JACKY_YOLO_MODE was not '1' when _prepare_agent_startup was "
...
def test_chat_subcommand_yolo_flag_sets_env_before_startup(monkeypatch):
    """jacky chat --yolo must also set JACKY_YOLO_MODE before
    _prepare_agent_startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["jacky", "chat", "--yolo"]
    )
    assert result == "1", (
        "JACKY_YOLO_MODE was not '1' when _prepare_agent_startup was "
...
def test_no_yolo_flag_leaves_env_unset_at_startup(monkeypatch):
    """Without --yolo, JACKY_YOLO_MODE must not be set at startup."""
    result = _run_main_and_capture_yolo_at_startup(
        monkeypatch, ["jacky"]
    )
    assert result is None, (
        "JACKY_YOLO_MODE was unexpectedly set at startup without --yolo."
    )

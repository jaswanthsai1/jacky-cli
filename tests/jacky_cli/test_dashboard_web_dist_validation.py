"""Regression tests: `jacky dashboard` validates JACKY_WEB_DIST before serving.

A custom JACKY_WEB_DIST without --skip-build previously skipped BOTH the
...
def test_env_dist_without_index_exits(main_mod, monkeypatch, tmp_path, capsys):
    """JACKY_WEB_DIST pointing at a dist with no index.html must exit 1,
    not start a server that 404s."""
    _wire_common(main_mod, monkeypatch)
    empty_dist = tmp_path / "empty_dist"
    empty_dist.mkdir()
    monkeypatch.setenv("JACKY_WEB_DIST", str(empty_dist))
    ...
    assert "JACKY_WEB_DIST" in out and str(empty_dist) in out


def test_env_dist_with_index_starts_server(main_mod, monkeypatch, tmp_path):
    """A valid JACKY_WEB_DIST (has index.html) proceeds to start_server
    without building."""
    _wire_common(main_mod, monkeypatch)
    dist = tmp_path / "dist"
    dist.mkdir()
    (dist / "index.html").write_text("<html></html>", encoding="utf-8")
    monkeypatch.setenv("JACKY_WEB_DIST", str(dist))
...
def test_env_dist_tilde_expanded_for_web_server(main_mod, monkeypatch, tmp_path):
    """A '~/...' JACKY_WEB_DIST must be written back expanded so
    ...
    monkeypatch.setenv("JACKY_WEB_DIST", "~/mydist")
    ...
    import os
    assert os.environ["JACKY_WEB_DIST"] == str(dist)

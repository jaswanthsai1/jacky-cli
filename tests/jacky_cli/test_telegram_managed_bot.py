"""Tests for jacky_cli.telegram_managed_bot — QR codes, deep links, pairing."""
...
class TestGenerateBotUsername:
    def test_secure_default_format(self):
        ...
        assert name.startswith("jacky_")
        assert name.endswith("_bot")
        assert len(name) == len("jacky_") + 16 + len("_bot")
        assert len(name) <= 32
    ...
    def test_slug_uses_telegram_safe_base32_chars(self):
        name = generate_bot_username()
        slug = name.removeprefix("jacky_").removesuffix("_bot")
        assert len(slug) == 16
...
class TestGenerateDeepLink:
    ...
    def test_defaults(self):
        ...
        assert "jacky_" in link

    def test_name_url_encoded(self):
        link = generate_deep_link(
            manager_bot="Bot",
            suggested_username="test_bot",
            suggested_name="Jacky & Friends",
        )
        assert "Jacky+%26+Friends" in link
...
class TestCreatePairing:
    def test_success(self):
        ...
        mock_resp.json.return_value = {
            ...
            "suggested_username": "jacky_abcdefghijklmnop_bot",
            "deep_link": "https://t.me/newbot/JackySetupBot/jacky_abcdefghijklmnop_bot?name=Jacky+Agent",
            "qr_payload": "https://t.me/newbot/JackySetupBot/jacky_abcdefghijklmnop_bot?name=Jacky+Agent",
            "expires_at": "2026-05-18T00:00:00.000Z",
        }

        with patch(
            "jacky_cli.telegram_managed_bot.httpx.post", return_value=mock_resp
        ) as post:
            pairing = create_pairing("https://api.example.com", bot_name="Jacky Agent")
...
class TestPollForToken:
    def pairing(self):
        return TelegramPairing(
            ...
            poll_token="secret-token",
            suggested_username="jacky_abcdefghijklmnop_bot",
            deep_link="https://t.me/newbot/JackySetupBot/jacky_abcdefghijklmnop_bot",
            qr_payload="https://t.me/newbot/JackySetupBot/jacky_abcdefghijklmnop_bot",
        )
    ...
    def test_immediate_success(self):
        ...
        mock_resp.json.return_value = {
            "bot_username": "jacky_abcdefghijklmnop_bot",
            "owner_user_id": 42,
            "status": "ready",
            "token": VALID_TOKEN,
        }
        ...
        with patch(
            "jacky_cli.telegram_managed_bot.httpx.get", return_value=mock_resp
        ) as get:
            with patch("jacky_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_setup_result_includes_owner_user_id(self):
        ...
        with patch("jacky_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("jacky_cli.telegram_managed_bot.time.sleep"):
    ...
    def test_invalid_ready_token_returns_none(self):
        ...
        with patch("jacky_cli.telegram_managed_bot.httpx.get", return_value=mock_resp):
            with patch("jacky_cli.telegram_managed_bot.time.sleep"):
                with patch(
                    "jacky_cli.telegram_managed_bot.time.monotonic"
                ) as mock_time:
    ...
    def test_eventual_success(self):
        ...
        with patch("jacky_cli.telegram_managed_bot.httpx.get", side_effect=fake_get):
            with patch("jacky_cli.telegram_managed_bot.time.sleep"):
                token = poll_for_token(
...
class TestSetupTelegramAuto:
    def test_setup_helper_exists(self):
        from jacky_cli.setup import _setup_telegram_auto

        assert callable(_setup_telegram_auto)

    def test_setup_result_passes_profile_name_for_profile_home(self, monkeypatch, tmp_path):
        from jacky_cli import setup

        seen = {}
        profile_home = tmp_path / ".jacky" / "profiles" / "oracle"
        profile_home.mkdir(parents=True)

        monkeypatch.setattr(setup, "get_jacky_home", lambda: profile_home)
        ...
        monkeypatch.setattr(
            "jacky_cli.telegram_managed_bot.auto_setup_telegram_bot_result",
    ...
    def test_profile_name_from_home_path_handles_windows_separators(self):
        from jacky_cli.setup import _profile_name_from_jacky_home

        assert (
            _profile_name_from_jacky_home(
                PureWindowsPath(r"C:\Users\test\AppData\Local\jacky\profiles\oracle")
            )
            == "oracle"
        )

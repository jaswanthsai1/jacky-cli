"""Tests for issue #26670 — concurrent jacky.exe detection and improved
quarantine retry / reboot-deferred fallback during `jacky update` on Windows.
...
# Tests in this module either exercise the REAL _detect_concurrent_jacky_instances
# helper (and need the autouse stub in tests/jacky_cli/conftest.py disabled),
...
# ---------------------------------------------------------------------------
# _detect_concurrent_jacky_instances
# ---------------------------------------------------------------------------


def _make_proc(pid: int, exe: str, name: str = "jacky.exe"):
...
def test_detect_concurrent_returns_empty_when_no_other_processes(_winp, tmp_path):
    ...
    (scripts_dir / "jacky.exe").write_bytes(b"")
    (scripts_dir / "jacky-gateway.exe").write_bytes(b"")

    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter([]))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_jacky_instances(scripts_dir)
...
def test_detect_concurrent_excludes_self_pid(_winp, tmp_path):
    ...
    procs = [_make_proc(my_pid, str(shim), "jacky.exe")]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_finds_other_jacky_process(_winp, tmp_path):
    scripts_dir = tmp_path
    shim = scripts_dir / "jacky.exe"
    shim.write_bytes(b"")
    ...
    procs = [
        _make_proc(other_pid, str(shim), "jacky.exe"),
    ...
    assert result == [(other_pid, "jacky.exe")]
...
def test_detect_concurrent_matches_case_insensitively(_winp, tmp_path):
    ...
    # Simulate the desktop spawning jacky.EXE (uppercase ext) from same path
    upper = str(shim).replace("jacky.exe", "JACKY.EXE")
    procs = [_make_proc(9999, upper, "JACKY.EXE")]
    fake_psutil = types.SimpleNamespace(process_iter=lambda attrs: iter(procs))
    with patch.dict(sys.modules, {"psutil": fake_psutil}):
        result = cli_main._detect_concurrent_jacky_instances(scripts_dir)

    assert result == [(9999, "JACKY.EXE")]
...
def test_detect_concurrent_is_noop_off_windows(_winp, tmp_path):
    ...
    assert cli_main._detect_concurrent_jacky_instances(tmp_path) == []
...
def test_detect_concurrent_excludes_parent_chain(_winp, tmp_path):
    ...
    Simulates the real Windows topology: jacky.exe launcher (PID L) spawns
    ...
    rows = [
        ...
        _make_proc(launcher_pid, str(shim), "jacky.exe"),
    ]
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_concurrent_still_finds_unrelated_other_jacky(_winp, tmp_path):
    """A sibling jacky.exe outside our ancestor chain must still be reported."""
    scripts_dir = tmp_path
    shim = scripts_dir / "jacky.exe"
    shim.write_bytes(b"")
    me = os.getpid()
    launcher_pid = me + 100  # our .exe launcher (parent — must be excluded)
    sibling_pid = me + 200  # an UNRELATED jacky.exe (must still be reported)

    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(launcher_pid, str(shim), "jacky.exe"),
        _make_proc(sibling_pid, str(shim), "jacky.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
        parent_chain=[launcher_pid],
        proc_iter_rows=rows,
    ...
    assert result == [(sibling_pid, "jacky.exe")]
...
def test_detect_concurrent_parent_chain_walks_deep(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "python.exe"),
        _make_proc(parent_pid, str(shim), "jacky.exe"),
        _make_proc(grandparent_pid, str(shim), "jacky.exe"),
        _make_proc(greatgrandparent_pid, str(shim), "jacky.exe"),
    ]
    fake_psutil = _fake_psutil_with_parent_chain(
...
def test_detect_concurrent_parents_call_robust_to_one_bad_hop(_winp, tmp_path):
    ...
    assert result == [(launcher_pid, "jacky.exe")]
...
def test_detect_concurrent_parent_walk_handles_stub_without_process(_winp, tmp_path):
    ...
    rows = [
        _make_proc(me, str(shim), "jacky.exe"),
        _make_proc(other_pid, str(shim), "jacky.exe"),
    ]
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    matches = [(1234, "jacky.exe"), (5678, "jacky.exe")]
    msg = cli_main._format_concurrent_instances_message(matches, tmp_path)

    assert "1234" in msg
    assert "5678" in msg
    assert "jacky.exe" in msg
    assert "Jacky Desktop" in msg
    assert "--force" in msg
    # Mentions the file that would have been overwritten
    assert str(tmp_path / "jacky.exe") in msg
...
# ---------------------------------------------------------------------------
# _quarantine_running_jacky_exe — retry + reboot-deferred fallback
# ---------------------------------------------------------------------------
...
def test_quarantine_succeeds_first_attempt(_winp, tmp_path):
    ...
    shim = tmp_path / "jacky.exe"
    shim.write_bytes(b"old")

    pairs = cli_main._quarantine_running_jacky_exe(tmp_path)

    assert len(pairs) == 1
    orig, quarantine = pairs[0]
    assert orig == shim
    assert quarantine.name.startswith("jacky.exe.old.")
...
def test_quarantine_retries_then_succeeds(_winp, tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cli_main, "_jacky_exe_shims", lambda d: [shim])
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "Jacky Desktop" in captured or "gateway" in captured.lower()
...
def test_pause_windows_gateways_for_update_stops_profile_and_unmapped_pids(
    ...
    monkeypatch.setattr(
        ...
        lambda pid: ["pythonw.exe", "-m", "jacky_cli.main", "gateway", "run"]
        if pid == 202
        else None,
    )
    ...
    assert token == {
        ...
        "unmapped": [
            {
                "pid": 202,
                "argv": ["pythonw.exe", "-m", "jacky_cli.main", "gateway", "run"],
            }
        ],
    }
...
def test_resume_windows_gateways_after_update_respawns_unmapped_by_cmdline(
    ...
    scheduled_argv = ["pythonw.exe", "-m", "jacky_cli.main", "gateway", "run"]
    token = {
        "resume_needed": True,
        "profiles": {},
        "unmapped_pids": [7560],
...
def test_cmd_update_aborts_on_concurrent_instance(_winp, tmp_path, capsys):
    """If another jacky.exe is running, the update bails out before
...
def test_cmd_update_force_bypasses_concurrent_check(_winp, tmp_path):
    ...
    detect = MagicMock(return_value=[(9, "jacky.exe")])
    ...
    with patch.object(
        ...
        cli_main, "_detect_concurrent_jacky_instances", detect
    ), patch.object(

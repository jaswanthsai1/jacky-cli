"""Tests for the update check mechanism in jacky_cli.banner."""
...
def test_check_for_updates_invalidates_on_version_change(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "jacky_cli" / "banner.py"
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JACKY_HOME", str(tmp_path))
    with patch("jacky_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_official_ssh_origin_uses_https_probe(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="git@github.com:jaswanthsai1/jacky-cli.git\n")
    ...
    with patch("jacky_cli.banner.subprocess.run", side_effect=fake_run):
...
def test_check_via_local_git_shallow_clone_behind_reports_no_count(tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append(cmd)
        if cmd == ["git", "remote", "get-url", "origin"]:
            return MagicMock(returncode=0, stdout="https://github.com/jaswanthsai1/jacky-cli.git\n")
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when JACKY_HOME has no git repo."""
    import jacky_cli.banner as banner
    ...
    # Point JACKY_HOME at a temp dir with no jacky-agent/.git
    monkeypatch.setenv("JACKY_HOME", str(tmp_path))
    with patch("jacky_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_docker_returns_none(tmp_path, monkeypatch):
    ...
    JACKY_REVISION (nix-only), so without a docker guard check_for_updates()
    ...
    container, and `jacky update` correctly refusing to run there. The guard
    ...
    with patch("jacky_cli.config.detect_install_method", return_value="docker"), \
         patch("jacky_cli.banner.subprocess.run") as mock_run, \
         patch("jacky_cli.banner.check_via_pypi") as mock_pypi:
        result = banner.check_for_updates()
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from jacky_cli.main import _invalidate_update_cache

    # Build a fake ~/.jacky with default + two named profiles
    ...
    with patch.object(Path, "home", return_value=tmp_path), \
         patch.dict(os.environ, {"JACKY_HOME": str(default_home)}):

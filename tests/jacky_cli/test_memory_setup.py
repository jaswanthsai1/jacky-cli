...
import jacky_cli.memory_setup as memory_setup
from jacky_cli.memory_setup import _CANCELLED, _curses_select
...
def test_curses_select_cancel_defaults_to_selected(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.curses_ui.curses_radiolist", fake_radiolist)
...
def test_cmd_setup_top_level_cancel_writes_nothing(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.config.load_config", load_config)
...
def test_cmd_setup_builtin_selection_still_saves_builtin(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.config.load_config", lambda: config)
    monkeypatch.setattr("jacky_cli.config.save_config", save_config)
...
def test_cmd_setup_clears_interactive_picker_before_provider_post_setup(monkeypatch):
    ...
    class PostSetupProvider:
        def post_setup(self, jacky_home, config):
    ...
    monkeypatch.setattr(memory_setup, "get_jacky_home", lambda: "/tmp/jacky-test")
    monkeypatch.setattr("jacky_cli.config.load_config", lambda: {"memory": {}})
...
def test_cmd_setup_generic_choice_cancel_writes_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(memory_setup, "get_jacky_home", lambda: tmp_path)
    monkeypatch.setattr("jacky_cli.config.load_config", lambda: {"memory": {}})
    monkeypatch.setattr("jacky_cli.config.save_config", save_config)

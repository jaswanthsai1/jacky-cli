"""Tests for jacky_cli.model_catalog — remote manifest fetch + cache + fallback."""
...
def isolated_home(tmp_path, monkeypatch):
    """Isolate JACKY_HOME + reset any module-level catalog cache per test."""
    home = tmp_path / ".jacky"
    home.mkdir()
    ...
    import importlib
    from jacky_cli import model_catalog
...
class TestValidation:
    def test_accepts_well_formed_manifest(self, isolated_home):
        from jacky_cli.model_catalog import _validate_manifest
...
class TestIntegrationWithModelsModule:
    """Exercise the fallback paths via the real callers in jacky_cli.models."""

    def test_curated_nous_ids_falls_back_to_hardcoded_on_empty_catalog(
        self, isolated_home
    ):
        from jacky_cli import model_catalog
        from jacky_cli.models import get_curated_nous_model_ids, _PROVIDER_MODELS
    ...
    def test_curated_nous_ids_prefers_manifest(self, isolated_home):
        from jacky_cli import model_catalog
        from jacky_cli.models import get_curated_nous_model_ids
    ...
    def test_picker_nous_row_uses_curated_list(self, tmp_path, monkeypatch):
        ...
        list in curated order — matching the ``jacky model`` CLI — not the live
        ...
        # ``JACKY_HOME / auth.json`` then resolves to the same path the
        # seat-belt thinks is the "real" user store. Use the autouse
        # ``_hermetic_environment`` JACKY_HOME directly instead.
        import importlib
        from jacky_cli import model_catalog
        from jacky_cli.models import get_curated_nous_model_ids
        importlib.reload(model_catalog)
        try:
            from jacky_cli.model_switch import list_picker_providers

            active_home = Path(os.environ["JACKY_HOME"])
            ...
            with patch.object(
                model_catalog, "_fetch_manifest", return_value=_valid_manifest()
            ), patch("jacky_cli.models.check_nous_free_tier", return_value=False), patch(
                "jacky_cli.models.union_with_portal_free_recommendations",

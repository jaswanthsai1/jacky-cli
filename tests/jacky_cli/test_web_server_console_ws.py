"""Dashboard Jacky Console websocket tests."""
...
@pytest.fixture
def console_client(monkeypatch, _isolate_jacky_home):
...
def test_console_ws_uses_hosted_context_for_opt_data_policy(console_client, monkeypatch):
    monkeypatch.setattr(web_server, "_default_jacky_root_is_opt_data", lambda: True)
    ...
    with console_client.websocket_connect(_url()) as conn:
        ...
        assert "hosted Jacky Console" in error["message"]


def test_console_ws_cancel_returns_to_prompt(console_client, monkeypatch):
    from jacky_cli.console_engine import ConsoleResult, JackyConsoleEngine

    def slow_execute(self, line: str, *, confirmed: bool = False):
        time.sleep(0.5)
        return ConsoleResult("ok", output="late", command=line)

    monkeypatch.setattr(JackyConsoleEngine, "execute", slow_execute)

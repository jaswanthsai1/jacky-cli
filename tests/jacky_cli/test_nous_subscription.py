...
from jacky_cli.nous_account import NousPortalAccountInfo, NousToolAccessInfo
from jacky_cli import nous_subscription as ns
...
def _capture_checklist(monkeypatch, *, selected_idx):
    ...
    import jacky_cli.setup as setup_mod

    monkeypatch.setattr(setup_mod, "prompt_checklist", _fake_checklist, raising=False)
    monkeypatch.setattr(
        "jacky_cli.config.save_config", lambda cfg: None, raising=False
    )
    return captured
...
# ---------------------------------------------------------------------------
# ensure_nous_portal_access — inline login gate for `jacky tools`
# ---------------------------------------------------------------------------

"""Tests for jacky_cli/completion.py — shell completion script generation."""
...
from jacky_cli.completion import _walk, generate_bash, generate_zsh, generate_fish
...
def _make_parser() -> argparse.ArgumentParser:
    """Build a minimal parser that mirrors the real jacky structure."""
    p = argparse.ArgumentParser(prog="jacky")
...
class TestGenerateBash:
    def test_contains_completion_function_and_register(self):
        ...
        assert "_jacky_completion()" in out
        assert "complete -F _jacky_completion jacky" in out
...
class TestGenerateZsh:
    ...
    def test_registers_compdef_instead_of_invoking_completion_function(self):
        out = generate_zsh(_make_parser())
        assert 'compdef _jacky jacky' in out
        assert '_jacky "$@"' not in out
    ...
    def test_preserves_valid_zsh_arguments_alias_syntax(self):
        ...
        assert "'(-)'{-p,--profile}'[Profile name]:profile:_jacky_profiles'" in out
        ...
        try:
            result = subprocess.run(
                [
                    "zsh",
                    "-fc",
                    f"autoload -Uz compinit && compinit -D; source {path}; [[ ${{_comps[jacky]}} == _jacky ]]",
                ],
                capture_output=True,
                text=True,
            )
...
class TestGenerateFish:
    def test_disables_file_completion(self):
        ...
        assert "complete -c jacky -f" in out
...
class TestSubcommandDrift:
    def test_SUBCOMMANDS_covers_required_commands(self):
        ...
        """
        import inspect
        from jacky_cli.main import _coalesce_session_name_args
...
class TestProfileCompletion:
    ...
    def test_bash_has_profiles_helper(self):
        out = generate_bash(_make_parser())
        assert "_jacky_profiles()" in out
        assert 'profiles_dir="$HOME/.jacky/profiles"' in out
    ...
    def test_bash_profile_actions_complete_profile_names(self):
        """After 'jacky profile use', complete with profile names."""
        out = generate_bash(_make_parser())
        # The profile case should have _jacky_profiles for name-taking actions
        lines = out.split("\n")
        ...
        for line in lines:
            ...
            if in_profile_case and "_jacky_profiles" in line:
                has_profiles_in_action = True
                break
        assert has_profiles_in_action, "profile actions should complete with _jacky_profiles"

    def test_zsh_has_profiles_helper(self):
        out = generate_zsh(_make_parser())
        assert "_jacky_profiles()" in out
        assert "$HOME/.jacky/profiles" in out
    ...
    def test_fish_has_profiles_helper(self):
        out = generate_fish(_make_parser())
        assert "__jacky_profiles" in out
        assert "$HOME/.jacky/profiles" in out
    ...
    def test_fish_profile_actions_complete_names(self):
        ...
        assert "__jacky_profiles" in out
        count = out.count("(__jacky_profiles)")

...
def test_managed_install_takes_precedence(tmp_path):
    """When JACKY_MANAGED is set, that takes precedence over git detection."""
    (tmp_path / ".git").mkdir()
    with patch("jacky_cli.config.get_managed_system", return_value="NixOS"), \
         patch("jacky_cli.config.get_jacky_home", return_value=tmp_path):
        from jacky_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_code_scoped_stamp_wins_over_home_stamp(tmp_path):
    """The stamp next to the running code is authoritative over $JACKY_HOME.

    Models a host git install whose $JACKY_HOME is shared with (and stamped
    'docker' by) a co-located container. The code-scoped stamp must win so the
    host install is correctly identified as 'git' and 'jacky update' works.
    """
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    ...
    with patch("jacky_cli.config.get_managed_system", return_value=None), \
         patch("jacky_cli.config.get_jacky_home", return_value=home):
        from jacky_cli.config import detect_install_method
...
def test_home_docker_stamp_ignored_when_not_containerized(tmp_path):
    ...
    'docker' into the shared $JACKY_HOME. With no code-scoped stamp, a host
    ...
    with patch("jacky_cli.config.get_managed_system", return_value=None), \
         patch("jacky_cli.config.get_jacky_home", return_value=home), \
         patch("jacky_cli.config._running_in_container", return_value=False):
        from jacky_cli.config import detect_install_method
        assert detect_install_method(project_root=code) == "git"
...
def test_home_docker_stamp_honored_inside_container(tmp_path):
    ...
    with patch("jacky_cli.config.get_managed_system", return_value=None), \
         patch("jacky_cli.config.get_jacky_home", return_value=home), \
         patch("jacky_cli.config._running_in_container", return_value=True):
        from jacky_cli.config import detect_install_method
...
def test_home_non_docker_stamp_still_honored_for_backcompat(tmp_path):
    ...
    install that historically stamped 'git'/'pip' into $JACKY_HOME keeps
...
def test_stamp_install_method_writes_code_scoped(tmp_path):
    """stamp_install_method writes next to the code, not into $JACKY_HOME."""
    code = tmp_path / "code"
    home = tmp_path / "home"
    code.mkdir()
    home.mkdir()
    with patch("jacky_cli.config.get_jacky_home", return_value=home):
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    image, so ``jacky update`` refused to run. With a ``.git`` checkout it
    must resolve to ``git``.
    """
    (tmp_path / ".git").mkdir()
    with patch("jacky_cli.config.get_managed_system", return_value=None), \

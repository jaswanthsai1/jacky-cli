"""Tests for `jacky chat --safe-mode` isolation."""
...
_VARS = ("JACKY_SAFE_MODE", "JACKY_IGNORE_USER_CONFIG", "JACKY_IGNORE_RULES")
...
def test_cmd_chat_safe_mode_sets_env_before_startup(monkeypatch):
    ...
    def fake_has_provider() -> bool:
        assert os.environ["JACKY_SAFE_MODE"] == "1"
        assert os.environ["JACKY_IGNORE_USER_CONFIG"] == "1"
        assert os.environ["JACKY_IGNORE_RULES"] == "1"
        return True
...
def test_prepare_agent_startup_applies_safe_mode_before_plugin_discovery(monkeypatch):
    ...
    plugins = types.ModuleType("jacky_cli.plugins")
    ...
    monkeypatch.setitem(sys.modules, "jacky_cli.plugins", plugins)
...
def test_plugin_discovery_skipped(monkeypatch):
    monkeypatch.setenv("JACKY_SAFE_MODE", "1")

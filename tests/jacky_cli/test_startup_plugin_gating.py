...
``jacky_cli.main`` skips eager plugin discovery at argparse-setup time
when the invocation is clearly targeting a known built-in subcommand.
This saves 500-650ms on ``jacky --help``, ``jacky version``,
``jacky logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``jacky --help`` in-process and parse the subcommand block.
    ...
    built-in-only set.
    """
    from jacky_cli import main as _main
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["jacky", "-w", "--tui"], None),
        (["jacky", "version"], "version"),
        (["jacky", "--tui", "chat"], "chat"),
        (["jacky", "-w", "logs"], "logs"),
        (["jacky", "chat", "hello world"], "chat"),
        (["jacky", "gateway", "run"], "gateway"),
        # Top-level value-taking flags: the value should be skipped.
        (["jacky", "-m", "gpt5", "chat"], "chat"),
        (["jacky", "--model", "gpt5", "chat", "hi"], "chat"),
        (["jacky", "-m", "gpt5", "--provider", "openai", "chat"], "chat"),
        (["jacky", "-z", "hello world"], None),
        (["jacky", "-z", "hello", "chat"], "chat"),
        (["jacky", "--model=gpt5", "chat"], "chat"),     # inline form
        (["jacky", "--", "chat"], "chat"),               # -- terminator
        (["jacky", "-w", "--"], None),
        # Unknown positional after skipped flags → plugin-cmd candidate.
        (["jacky", "some-plugin-cmd"], "some-plugin-cmd"),
        (["jacky", "-m", "gpt5", "some-plugin-cmd"], "some-plugin-cmd"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ["jacky"],                          # bare → chat
        ["jacky", "--help"],                # top-level help
        ["jacky", "-h"],
        ["jacky", "version"],               # known built-in
        ["jacky", "logs"],
        ["jacky", "gateway", "run"],
        ["jacky", "--tui"],
        ["jacky", "-w", "--tui"],
        ["jacky", "chat", "hi"],
        ["jacky", "help"],                  # accepted built-in-ish
        ["jacky", "-m", "gpt5", "chat"],    # flag-value-skipping
    ],
)
...
@pytest.mark.parametrize(
    "argv",
    [
        ["jacky", "meet", "join"],          # potential google_meet plugin
        ["jacky", "honcho", "status"],      # potential memory plugin
        ["jacky", "unknown-subcmd"],
    ],
)
...
def test_builtin_set_covers_every_registered_subcommand():
    ...
    # in the set so ``jacky help <cmd>`` skips discovery; it won't show
    ...
    assert not missing_from_declaration, (
        ...
        f"jacky_cli/main.py::_BUILTIN_SUBCOMMANDS so plugin discovery "

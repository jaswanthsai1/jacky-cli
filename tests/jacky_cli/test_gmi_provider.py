...
from jacky_cli.auth import resolve_provider
...
class TestGmiAliases:
    ...
    def test_providers_normalize_provider(self):
        from jacky_cli.providers import normalize_provider as normalize_provider_in_providers
...
class TestGmiConfigRegistry:
    def test_optional_env_vars_include_gmi(self):
        ...
        # jacky model, not via upgrade prompts.
...
class TestGmiModelCatalog:
    ...
    def test_provider_model_ids_falls_back_to_static_models(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.models.fetch_api_models", lambda api_key, base_url: None)
...
class TestGmiProvidersModule:
    def test_overlay_exists(self):
        ...
        assert "gmi" in JACKY_OVERLAYS
        overlay = JACKY_OVERLAYS["gmi"]
...
class TestGmiDoctor:
    def test_provider_env_hints_include_gmi(self):
        from jacky_cli.doctor import _PROVIDER_ENV_HINTS
...
class TestGmiAuxiliary:
    def test_resolve_provider_client_uses_gmi_aux_default(self, monkeypatch):
        ...
        # GMI profile declares default_headers with a JackyAgent User-Agent
        ...
        assert headers.get("User-Agent", "").startswith("JackyAgent/")

    def test_gmi_profile_declares_jacky_user_agent(self):
        """The GMI plugin sets a JackyAgent/<ver> User-Agent on its profile."""
        from providers import get_provider_profile

        profile = get_provider_profile("gmi")
        assert profile is not None
        ua = profile.default_headers.get("User-Agent", "")
        assert ua.startswith("JackyAgent/"), (
            f"expected GMI profile User-Agent to start with 'JackyAgent/', got {ua!r}"
        )
...
class TestGmiMainFlow:
    def test_chat_parser_accepts_gmi_provider(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.config.get_container_exec_info", lambda: None)
        monkeypatch.setattr(
            "jacky_cli.main.cmd_chat",
            lambda args: recorded.setdefault("provider", args.provider),
        )
        monkeypatch.setattr(sys, "argv", ["jacky", "chat", "--provider", "gmi"])

        from jacky_cli.main import main
    ...
    def test_select_provider_and_model_routes_gmi_to_generic_flow(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.auth.resolve_provider", lambda *args, **kwargs: None)
        ...
        monkeypatch.setattr("jacky_cli.main._prompt_provider_choice", fake_prompt_provider_choice)
        monkeypatch.setattr("jacky_cli.main._model_flow_api_key_provider", fake_model_flow_api_key_provider)

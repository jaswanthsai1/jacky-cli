"""Tests for jacky_cli.tools_config platform tool persistence."""
...
from jacky_cli.nous_account import NousPortalAccountInfo
...
def test_all_invalid_platform_toolsets_logs_runtime_warning(caplog):
    ...
    (e.g. 'jacky' instead of 'jacky-cli') must warn at resolve time so an
    already-corrupted config is caught at runtime, not just during migration."""
    import jacky_cli.tools_config as _tc
    ...
    config = {"platform_toolsets": {"cli": ["jacky"]}}

    with caplog.at_level(logging.WARNING, logger="jacky_cli.tools_config"):
        _get_platform_tools(config, "cli")

    warnings = [r.getMessage() for r in caplog.records if r.levelno >= logging.WARNING]
    assert any("#38798" in m and "jacky" in m for m in warnings), warnings
...
def test_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["jacky-cli"]}}
...
def test_partially_valid_platform_toolsets_no_runtime_warning(caplog):
    ...
    config = {"platform_toolsets": {"cli": ["jacky-cli", "bogus"]}}
...
def test_get_platform_tools_x_search_auto_enabled_when_xai_oauth_present(monkeypatch):
    ...
    to also click through `jacky tools` → X (Twitter) Search to flip the
    toolset on. Tool's check_fn still gates schema registration if creds
    later go missing.
    """
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "jacky_cli.tools_config._xai_credentials_present", lambda: True
    )
...
# via the platform's native composite. The composite ``jacky-discord``
...
def test_discord_composite_only_enables_discord_toolsets():
    """Layer 1: ``platform_toolsets.discord: [jacky-discord]`` is an explicit
    opt-in to the full Discord bundle (which includes the ``discord`` and
    ``discord_admin`` tools). They must not be silently stripped."""
    config = {"platform_toolsets": {"discord": ["jacky-discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled, "discord toolset missing from jacky-discord composite"
...
def test_discord_composite_plus_configurable_enables_discord_toolsets():
    ...
    config = {"platform_toolsets": {"discord": ["jacky-discord", "spotify"]}}
    enabled = _get_platform_tools(config, "discord")
...
def test_discord_composite_plus_partial_explicit_enables_sibling():
    """Layer 3: ``[jacky-discord, discord]`` lists discord explicitly but
    discord_admin arrives only via the composite. Both must survive."""
    config = {"platform_toolsets": {"discord": ["jacky-discord", "discord"]}}
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_discord_toolsets_do_not_leak_to_other_platforms():
    ...
    config = {"platform_toolsets": {"telegram": ["jacky-telegram", "discord"]}}
...
def test_discord_explicit_workaround_still_works():
    ...
    config = {
        "platform_toolsets": {"discord": ["jacky-discord", "discord", "discord_admin"]}
    }
    enabled = _get_platform_tools(config, "discord")
    assert "discord" in enabled
    assert "discord_admin" in enabled
...
def test_get_platform_tools_x_search_off_when_no_xai_credentials(monkeypatch):
    ...
    monkeypatch.delenv("XAI_API_KEY", raising=False)
    monkeypatch.setattr(
        "jacky_cli.tools_config._xai_credentials_present", lambda: False
    )
...
def test_get_platform_tools_x_search_respects_explicit_config(monkeypatch):
    """Once the user has saved an explicit toolset list via `jacky tools`,
    ...
    # User explicitly opted into spotify but not x_search via `jacky tools`.
    config = {"platform_toolsets": {"cli": ["jacky-cli", "spotify"]}}
    enabled = _get_platform_tools(config, "cli")
...
def test_get_platform_tools_expands_composite_when_mixed_with_configurable():
    """``[jacky-cli, spotify]`` (composite + configurable) must keep the full
    ``jacky-cli`` toolset alongside the explicit Spotify opt-in. The
    has_explicit_config branch used to drop ``jacky-cli`` on the floor,
    ...
    for ts in ("terminal", "file", "web", "browser", "memory", "delegation",
        ...
        assert ts in enabled, f"{ts} should be enabled when jacky-cli is listed"
...
def test_get_platform_tools_composite_only_unchanged():
    ...
    composite_only = _get_platform_tools(
        {"platform_toolsets": {"cli": ["jacky-cli"]}},
        "cli",
...
def test_get_platform_tools_mixed_does_not_resurrect_default_off():
    ...
    pull-in. Without this, ``jacky-cli`` expansion would re-enable
    ``moa`` / ``rl`` / ``homeassistant`` for users who never opted in."""
    config = {"platform_toolsets": {"cli": ["jacky-cli", "terminal"]}}
...
def test_get_platform_tools_preserves_explicit_empty_selection():
    ...
    # live in _JACKY_CORE_TOOLS but aren't user-toggleable) are still
    # auto-recovered by _get_platform_tools so saving via `jacky tools`
...
def test_apply_toolset_change_from_default_does_not_enable_default_off_toolsets():
    ...
    with patch("jacky_cli.tools_config.save_config"):
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/jaswanthsai1/jacky-cli/issues/1247
    """
    config = {
        "platform_toolsets": {
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (jacky-cli, jacky-telegram, etc.) must NOT
    ...
    ``jacky tools``, but jacky-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "jacky-cli",  # <-- the culprit
    ...
    # jacky-cli must NOT survive — it's a platform default, not an MCP server
    assert "jacky-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_jacky_telegram():
    """Same bug for Telegram — jacky-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "jacky-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "jacky-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "jacky-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.nous_subscription.get_nous_portal_account_info",
...
def test_visible_providers_force_fresh_shows_nous_subscription_after_upgrade(monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.tools_config.get_nous_subscription_features",
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.tools_config._run_post_setup", lambda key: None)
...
def test_reconfigure_lists_enabled_web_without_existing_provider_config(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.tools_config._prompt_choice", fake_prompt_choice)
    monkeypatch.setattr(
        "jacky_cli.tools_config._configure_tool_category_for_reconfig",
        lambda ts_key, cat, config, **kwargs: configured.append(ts_key),
    )
    monkeypatch.setattr("jacky_cli.tools_config.save_config", lambda config: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    monkeypatch.setattr("jacky_cli.nous_subscription.managed_nous_tools_enabled", lambda: True)
    config = {
        "model": {"provider": "nous"},
        "platform_toolsets": {"cli": []},
    }
    ...
    monkeypatch.setattr(
        "jacky_cli.tools_config._prompt_toolset_checklist",
    ...
    monkeypatch.setattr(
        "jacky_cli.tools_config._get_enabled_platforms",
        lambda: ["cli"],
    )
    monkeypatch.setattr(
        "jacky_cli.nous_subscription.get_nous_portal_account_info",
    ...
    configured = []
    monkeypatch.setattr(
        "jacky_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from jacky_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from jacky_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
...
def test_numeric_mcp_server_name_does_not_crash_sorted():
    ...
    Regression test for https://github.com/jaswanthsai1/jacky-cli/issues/6901
    """
    config = {
...
def test_computer_use_needs_configuration_when_cua_driver_post_setup_pending():
    ...
    Returning users enabling Computer Use through `jacky tools` must reach the
...
def test_computer_use_respects_custom_cua_driver_command():
    """The setup gate should match runtime's JACKY_CUA_DRIVER_CMD override."""
    def fake_which(name: str):
        return "/opt/bin/custom-cua" if name == "custom-cua" else None

    with patch.dict("os.environ", {"JACKY_CUA_DRIVER_CMD": "custom-cua"}), \
...
def test_computer_use_blank_custom_driver_command_falls_back_to_default():
    ...
    with patch.dict("os.environ", {"JACKY_CUA_DRIVER_CMD": "   "}), \
         patch("shutil.which", side_effect=fake_which):
        assert _toolset_needs_configuration_prompt("computer_use", {}) is False
...
class TestImagegenBackendRegistry:
    ...
    def test_fal_backend_registered(self):
        from jacky_cli.tools_config import IMAGEGEN_BACKENDS
    ...
    def test_image_gen_providers_tagged_with_fal_backend(self):
        ...
        from jacky_cli.tools_config import TOOL_CATEGORIES
...
class TestImagegenModelPicker:
    ...
    def test_picker_writes_chosen_model_to_config(self):
        from jacky_cli.tools_config import _configure_imagegen_model
        config = {}
        # Force _prompt_choice to pick index 1 (second-in-ordered-list).
        with patch("jacky_cli.tools_config._prompt_choice", return_value=1):
    ...
    def test_picker_with_gpt_image_does_not_prompt_quality(self):
        ...
        config = {}
        with patch("jacky_cli.tools_config._prompt_choice", side_effect=fake_prompt):
            _configure_imagegen_model("fal", config)
    ...
    def test_picker_repairs_corrupt_config_section(self):
        ...
        with patch("jacky_cli.tools_config._prompt_choice", return_value=0):
            _configure_imagegen_model("fal", config)
...
def test_save_platform_tools_clears_no_mcp_sentinel():
    """`jacky tools` has no UI for no_mcp, so saving from the picker clears
...
def test_get_platform_tools_recovers_non_configurable_toolsets_from_composite():
    ...
    fake_toolsets["jacky-_test_platform"] = {
    ...
    test_platforms = {
        "_test_platform": {"label": "Test", "default_toolset": "jacky-_test_platform"},
    }
...
def test_get_platform_tools_second_pass_skips_fully_claimed_toolsets():
    ...
    be added by the second pass (prevents 'search', 'jacky-acp' noise).
    """
...
def test_get_platform_tools_discord_both_off_by_default():
    """Both `discord` and `discord_admin` are opt-in via `jacky tools`,
...
def test_discord_toolsets_not_available_on_other_platforms():
    ...
    from jacky_cli.tools_config import _toolset_allowed_for_platform
...
def test_discord_toolsets_user_enabled_are_honored():
    """When the user opts in via `jacky tools`, the toolset appears."""
...
def test_save_platform_tools_strips_restricted_toolsets():
    ...
    from jacky_cli.tools_config import _save_platform_tools
    config = {}
...
def test_get_effective_configurable_toolsets_dedupes_bundled_plugins():
    ...
    them twice — otherwise `jacky tools` → "reconfigure existing" shows
    the same toolset two rows in a row.
    """
    from jacky_cli.tools_config import _get_effective_configurable_toolsets
...
def test_reconfigure_provider_syncs_use_gateway(monkeypatch, provider, config_key, expected):
    ...
    monkeypatch.setattr(
        "jacky_cli.nous_subscription.ensure_nous_portal_access",
        lambda **kwargs: True,
    )
    config = {}
...
def test_reconfigure_provider_runs_post_setup_for_env_var_providers(
    ...
    called = []
    monkeypatch.setattr("jacky_cli.tools_config._run_post_setup", lambda key: called.append(key))
    monkeypatch.setattr("jacky_cli.tools_config.get_env_value", lambda k: None)
    monkeypatch.setattr("jacky_cli.tools_config._prompt", lambda *a, **kw: "")
    monkeypatch.setattr("jacky_cli.tools_config.save_env_value", lambda k, v: None)
...
def test_configure_non_managed_provider_skips_portal_gate(monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.nous_subscription.ensure_nous_portal_access", _boom
    )
...
def test_apply_provider_selection_web_sets_backend():
    ...
    from jacky_cli.tools_config import apply_provider_selection
...
def test_apply_provider_selection_does_not_prompt_or_post_setup(monkeypatch):
    ...
    from jacky_cli import tools_config
...
#    reported as added/removed by `jacky tools` ──────────────────────────
...
def test_kanban_not_reported_as_removed_in_diff():
    """Reproduces the false-signal bug: `jacky tools` printed ``- kanban``
...
def test_vision_picker_writes_provider_and_model(tmp_path, monkeypatch):
    ...
    as ``jacky model`` and writes the selection to the auxiliary config keys
    the resolver reads.
    """
    monkeypatch.setenv("JACKY_HOME", str(tmp_path))
    import jacky_cli.tools_config as tc
    ...
    seq = iter([1, 0, 1])
    with patch("jacky_cli.model_switch.list_authenticated_providers",

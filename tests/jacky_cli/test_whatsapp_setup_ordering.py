...
Before the fix, ``jacky whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``jacky gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def isolated_home(tmp_path, monkeypatch):
    home = tmp_path / "home"
    jacky = home / ".jacky"
    jacky.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: home)
    monkeypatch.setenv("JACKY_HOME", str(jacky))
...
def _env_value(jacky_home: Path, key: str) -> str | None:
...
def test_aborted_setup_does_not_enable_whatsapp(isolated_home, monkeypatch):
    ...
    """
    from jacky_cli.main import cmd_whatsapp
    ...
    monkeypatch.setattr("jacky_cli.main._require_tty", lambda *_a, **_kw: None)
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``jacky whatsapp`` with an existing paired session and

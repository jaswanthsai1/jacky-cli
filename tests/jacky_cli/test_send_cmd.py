"""Tests for the ``jacky send`` CLI subcommand.

Covers the argument parsing / stdin / file / list behavior of
``jacky_cli.send_cmd``. The underlying ``send_message_tool`` is stubbed so
...
from jacky_cli import send_cmd
...
def test_load_jacky_env_bridges_config_yaml_scalars(tmp_path, monkeypatch):
    ...
    ``jacky send`` from a fresh shell cannot resolve the home channel
    because ``TELEGRAM_HOME_CHANNEL`` (saved by ``jacky config set``) lives
    ...
    jacky_home.mkdir()
    (jacky_home / ".env").write_text("SOME_TOKEN=abc123\n")
    ...
    # Force get_jacky_home() to re-resolve under the patched env.
    from importlib import reload

    import jacky_cli.config as _hc_config
    reload(_hc_config)

    send_cmd._load_jacky_env()

    assert os.environ.get("SOME_TOKEN") == "abc123"
    assert os.environ.get("TELEGRAM_HOME_CHANNEL") == "5550001111"


def test_load_jacky_env_does_not_override_existing(tmp_path, monkeypatch):
    """Existing env vars must not be clobbered by config.yaml values."""
    import os

    jacky_home = tmp_path / ".jacky"
    jacky_home.mkdir()
    (jacky_home / "config.yaml").write_text("TELEGRAM_HOME_CHANNEL: yaml_value\n")
...
def test_load_jacky_env_handles_missing_files(tmp_path, monkeypatch):

...
jacky_cli.config.load_config, so the managed overlay has to be wired into each.
...
def homes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JACKY_MANAGED_DIR", str(managed))
...
def test_gateway_run_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(gr, "_jacky_home", home, raising=False)
...
def test_gateway_config_loader_honors_managed(homes, monkeypatch):
    ...
    # load_gateway_config resolves home via get_jacky_home() (JACKY_HOME env).
...
def test_tui_loader_honors_managed(homes, monkeypatch):
    ...
    monkeypatch.setattr(ts, "_jacky_home", home, raising=False)
    monkeypatch.setattr(ts, "_cfg_cache", None, raising=False)
    monkeypatch.setattr(ts, "_cfg_mtime", None, raising=False)
    monkeypatch.setattr(ts, "get_jacky_home_override", lambda: None, raising=False)
    cfg = ts._load_cfg()
...
def test_timezone_honors_managed(homes, monkeypatch):
    home, managed = homes
    # jacky_time checks an env override first; ensure it's unset so config wins.
...
def test_gateway_env_bridge_honors_managed(homes, monkeypatch):
    ...
    every turn (JACKY_TIMEZONE, JACKY_REDACT_SECRETS, JACKY_MAX_ITERATIONS,
    ...
    # writing JACKY_TIMEZONE = cfg["timezone"]. Prove the overlay flips the value.
    import yaml

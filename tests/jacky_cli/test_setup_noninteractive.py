...
import pytest
from jacky_cli.config import DEFAULT_CONFIG, load_config, save_config
...
class TestNonInteractiveSetup:
    ...
    def test_cmd_setup_allows_noninteractive_flag_without_tty(self):
        ...
        from jacky_cli.main import cmd_setup

        args = _make_setup_args(non_interactive=True)

        with (
            patch("jacky_cli.setup.run_setup_wizard") as mock_run_setup,
    ...
    def test_cmd_setup_defers_no_tty_handling_to_setup_wizard(self):
        """Bare `jacky setup` should reach setup.py, which prints headless guidance."""
        from jacky_cli.main import cmd_setup
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("jacky_cli.setup.ensure_jacky_home"),
            patch("jacky_cli.setup.load_config", return_value={}),
            patch("jacky_cli.setup.get_jacky_home", return_value="/tmp/.jacky"),
            patch("jacky_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "jacky config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `jacky` should not prompt for input when no provider exists and stdin is headless."""
        from jacky_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("jacky_cli.main._has_any_provider_configured", return_value=False),
            patch("jacky_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_main_accepts_tts_setup_section(self, monkeypatch):
        """`jacky setup tts` should parse and dispatch like other setup sections."""
        ...
        monkeypatch.setattr("sys.argv", ["jacky", "setup", "tts"])

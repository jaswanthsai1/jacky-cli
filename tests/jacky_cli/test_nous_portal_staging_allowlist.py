...
``JACKY_PORTAL_BASE_URL=https://portal.staging-nousresearch.com`` in its
...
class TestPortalEnvOverrideHelper:
    def test_none_when_unset(self, monkeypatch):
        monkeypatch.delenv("JACKY_PORTAL_BASE_URL", raising=False)
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert _nous_portal_env_override() is None

    def test_jacky_portal_base_url_wins(self, monkeypatch):
        monkeypatch.setenv(
            "JACKY_PORTAL_BASE_URL", "https://portal.staging-nousresearch.com/"
        )
        monkeypatch.delenv("NOUS_PORTAL_BASE_URL", raising=False)
        assert (
...
class TestResolveAccessTokenEnvOverrideWins:
    ...
    def _run_and_capture(self, monkeypatch, auth):
        ...
        caplog_records = []
        logger = logging.getLogger("jacky_cli.auth")
    ...
    def test_env_override_wins_even_with_staging_state_stored(
        ...
        a prior JACKY_AUTH_JSON_BOOTSTRAP seed), and the env var is set to
        ...
        monkeypatch.setenv("JACKY_PORTAL_BASE_URL", staging_portal)

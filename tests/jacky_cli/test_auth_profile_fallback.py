...
When ``JACKY_HOME`` points to a named profile, ``read_credential_pool()``
...
def profile_env(tmp_path, monkeypatch):
    """Set up a global root + an active profile under Path.home()/.jacky/profiles/coder.

    * Path.home() -> tmp_path
    * Global root -> tmp_path/.jacky            (has its own auth.json fixture)
    * Profile     -> tmp_path/.jacky/profiles/coder   (active, JACKY_HOME points here)
...
# ``_load_provider_state`` itself, those helpers raise ``"Jacky is not
...
def test_load_provider_state_falls_back_to_global(profile_env):
    ...
    from jacky_cli.auth import _load_auth_store, _load_provider_state
...
def test_load_provider_state_classic_mode_no_fallback(tmp_path, monkeypatch):
    ...
    jacky_home = tmp_path / "classic"
    jacky_home.mkdir()
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))

    _write(jacky_home / "auth.json", _make_auth_store(providers={
...
def test_classic_mode_does_not_double_read_same_file(tmp_path, monkeypatch):
    """In classic mode (JACKY_HOME == global root), no fallback path runs.
    ...
    # sees tmp_path/home/.jacky as the "real home" — which is NOT equal
    # to the JACKY_HOME we set (tmp_path/classic), so the guard passes.
    fake_home = tmp_path / "home"
    fake_home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: fake_home)
    jacky_home = tmp_path / "classic"
    ...
    _write(jacky_home / "auth.json", _make_auth_store(pool={
        "openrouter": [{
            "id": "only",
            "label": "classic",
            "auth_type": "api_key",
    ...
    from jacky_cli.auth import read_credential_pool, _global_auth_file_path

    # Classic mode: JACKY_HOME is set to a custom path that is NOT under
    # ~/.jacky/profiles/ — get_default_jacky_root() returns JACKY_HOME

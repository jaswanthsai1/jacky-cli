...
prefix (e.g. ``mission-control.tilos.com/jacky/*`` -> local Caddy ->
:9119), injecting ``X-Forwarded-Prefix: /jacky`` on every request.

The dashboard already honours this for the SPA bundle (rewriting asset
URLs and the bootstrap ``__JACKY_BASE_PATH__``). The OAuth gate must
honour it too:

  1. The gate's ``Location:`` redirect to /login (in
     ``_unauth_response``) needs to be ``/jacky/login`` so the browser
  ...
  3. ``_redirect_uri`` (the OAuth callback URL handed to the IDP) must
     ...
     ``/jacky/auth/callback`` and the user gets 404.
  4. Cookies must use ``Path=/jacky`` when behind a prefix so they
...
from jacky_cli.dashboard_auth import clear_providers, register_provider
from jacky_cli.dashboard_auth import prefix as prefix_mod
from tests.jacky_cli.conftest_dashboard_auth import StubAuthProvider
...
class TestForwardedPrefixNormalisation:
    def test_home_assistant_ingress_prefix_with_subpath_is_accepted(
        ...
        the SPA receives the correct __JACKY_BASE_PATH__ and asset prefix."""
...
class TestGateRedirectsCarryPrefix:
    def test_html_redirect_to_login_carries_prefix(self, gated_app_proxied):
        r = gated_app_proxied.get(
            "/sessions",
            headers={"x-forwarded-prefix": "/jacky"},
        ...
        assert r.headers["location"].startswith("/jacky/auth/login"), (
    ...
    def test_api_401_envelope_login_url_carries_prefix(self, gated_app_proxied):
        ...
        # include the prefix.
        assert body["login_url"].startswith("/jacky/login"), (
...
class TestOAuthRedirectUriRespectsPrefix:
    def test_redirect_uri_includes_prefix_in_authorize_url(
        ...
        ``https://mission-control.tilos.com/jacky/auth/callback`` — the
        ...
        assert parsed.path == "/jacky/auth/callback", (
...
# ---------------------------------------------------------------------------
# JACKY_DASHBOARD_PUBLIC_URL / dashboard.public_url override
# ---------------------------------------------------------------------------


class TestPublicUrlOverride:
    """``dashboard.public_url`` (env override:
    ``JACKY_DASHBOARD_PUBLIC_URL``) lets an operator force the absolute
    ...
    def patch_config(self, monkeypatch):
        """Replace ``jacky_cli.config.load_config`` with a stub
        ...
        def _set(public_url) -> None:
            ...
            monkeypatch.setattr(
                "jacky_cli.config.load_config", lambda: cfg
            )
    ...
    def test_public_url_config_yaml_used_when_env_unset(
        ...
        monkeypatch.delenv("JACKY_DASHBOARD_PUBLIC_URL", raising=False)
    ...
    def test_env_overrides_config_public_url(
        ...
        monkeypatch.setenv(
            "JACKY_DASHBOARD_PUBLIC_URL", "https://from-env.example",
        )
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_public_url_with_path_prefix_baked_in(
        ...
        (``https://example.com/jacky``), the OAuth callback URL is
        the path appended verbatim. The operator is declaring the
        whole authority; we trust them."""
        patch_config(None)
        monkeypatch.setenv(
            "JACKY_DASHBOARD_PUBLIC_URL", "https://example.com/jacky",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://example.com/jacky/auth/callback"
    ...
    def test_public_url_ignores_x_forwarded_prefix(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "JACKY_DASHBOARD_PUBLIC_URL", "https://example.com/already-prefixed",
        )
    ...
    def test_public_url_strips_trailing_slash(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "JACKY_DASHBOARD_PUBLIC_URL", "https://example.com/",
        )
        redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_malformed_public_url_falls_through_to_reconstruction(
        ...
        for bad in [
            ...
            monkeypatch.setenv("JACKY_DASHBOARD_PUBLIC_URL", bad)
            redirect_uri = self._redirect_uri(gated_app_direct)
    ...
    def test_empty_public_url_env_treated_as_unset(
        ...
        entry."""
        monkeypatch.setenv("JACKY_DASHBOARD_PUBLIC_URL", "")
        patch_config("https://from-config.example")
        redirect_uri = self._redirect_uri(gated_app_direct)
        assert redirect_uri == "https://from-config.example/auth/callback"
    ...
    def test_scheme_less_public_url_env_warns_operator(
        ...
        of "I set JACKY_DASHBOARD_PUBLIC_URL but the callback is still
        ...
        patch_config(None)
        monkeypatch.setenv("JACKY_DASHBOARD_PUBLIC_URL", "jacky.domain.com")
        ...
        assert any(
            "JACKY_DASHBOARD_PUBLIC_URL" in m
            and "jacky.domain.com" in m
            and "scheme" in m
            for m in warnings
    ...
    def test_scheme_less_public_url_warning_is_deduplicated(
        ...
        scheme_warnings = [
            ...
            for r in caplog.records
            if r.levelno == logging.WARNING
            and "jacky.domain.com" in r.getMessage()
        ]
    ...
    def test_valid_public_url_emits_no_warning(
        ...
        patch_config(None)
        monkeypatch.setenv(
            "JACKY_DASHBOARD_PUBLIC_URL", "https://jacky.domain.com"
        )

        with caplog.at_level(logging.WARNING, logger=prefix_mod.__name__):
            result = prefix_mod.resolve_public_url()

        assert result == "https://jacky.domain.com"
        assert not [
...
class TestCookiePathRespectsPrefix:
    ...
    def test_pkce_cookie_uses_prefix_path(self, gated_app_proxied):
        ...
        pkce = next(c for c in cookies if "jacky_session_pkce" in c)
        # Browser only sends cookie back if the request path is under
        # the cookie's Path attribute, so we need /jacky here. Bare
        # /-rooted cookies would still be sent but would also be sent
        # to /billing/... etc.
        assert "Path=/jacky" in pkce, (
    ...
    def test_pkce_cookie_uses_secure_prefix_when_proxied(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Secure-jacky_session_pkce=")
        ]
    ...
    def test_pkce_cookie_uses_host_prefix_when_direct(
        ...
        pkce_candidates = [
            c for c in cookies
            if c.startswith("__Host-jacky_session_pkce=")
        ]
        assert pkce_candidates, (
    ...
    def test_loopback_cookies_unprefixed(self):
        ...
        from jacky_cli.dashboard_auth.cookies import set_pkce_cookie
        ...
        assert any(c.startswith("jacky_session_pkce=") for c in cookies), (
    ...
    def test_cookies_read_back_round_trip_through_prefix(
        ...
        __Secure- prefix AND Path=/jacky, so the next request under
        ...
        (``/jacky/auth/login``, ``/jacky/auth/callback``). A cookie
        set with ``Path=/jacky`` would therefore NOT be sent back on
        the second request through TestClient even though it WOULD be
        sent by a real browser hitting ``/jacky/auth/callback``. To
        ...
        pkce_set = next(
            ...
            if "jacky_session_pkce" in c
        )
        # Parse "__Secure-jacky_session_pkce=...; HttpOnly; ...".
        pkce_kv = pkce_set.split(";", 1)[0]  # "__Secure-jacky_session_pkce=value"
        state = r1.headers["location"].split("state=")[1]

        # Round-trip the cookie by hand because TestClient's jar won't
        # automatically send a Path=/jacky cookie to a /auth/callback
        # request path.
        r2 = gated_app_proxied.get(
            f"/auth/callback?code=stub_code&state={state}",
            headers={
                "x-forwarded-prefix": "/jacky",
                "cookie": pkce_kv,
            },
            follow_redirects=False,
        )
        ...
        at_cookies = [
            c for c in cookies
            if c.startswith("__Secure-jacky_session_at=")
        ]
        assert at_cookies, (
            f"session_at missing __Secure- prefix: {cookies!r}"
        )
        assert "Path=/jacky" in at_cookies[0]

"""Tests for jacky_cli.service_manager — the abstract ServiceManager
...
def test_detect_service_manager_s6_keys_off_s6_running_not_is_container(
    ...
    the dispatch path inert on Fly, so `jacky gateway restart` spawned a
    foreground gateway that fought the supervised one. Detection must key off
    s6 being PID 1 (`_s6_running`) alone."""
    monkeypatch.setattr(
        "jacky_cli.service_manager._s6_running", lambda: True,
    )
...
def test_s6_running_true_when_comm_and_basedir_match(
    ...
    from jacky_cli.service_manager import _s6_running
...
def test_s6_running_false_when_comm_unreadable(
    ...
    always-False inside the container under the jacky user. The new
...
def test_systemd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "jacky_cli.gateway.systemd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "jacky_cli.gateway.systemd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "jacky_cli.gateway.systemd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "jacky_cli.gateway._probe_systemd_service_running",
...
def test_launchd_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    called: list[str] = []
    monkeypatch.setattr(
        "jacky_cli.gateway.launchd_start", lambda: called.append("start"),
    )
    monkeypatch.setattr(
        "jacky_cli.gateway.launchd_stop", lambda: called.append("stop"),
    )
    monkeypatch.setattr(
        "jacky_cli.gateway.launchd_restart", lambda: called.append("restart"),
    )
    monkeypatch.setattr(
        "jacky_cli.gateway._probe_launchd_service_running", lambda: False,
    )
    mgr = LaunchdServiceManager()
    mgr.start("ignored")
    mgr.stop("ignored")
...
def test_windows_manager_lifecycle_delegates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # against the `jacky_cli` package succeeds — gateway_windows is
    # imported lazily inside the wrapper and may not yet be loaded.
    import jacky_cli.gateway_windows  # noqa: F401
    ...
    monkeypatch.setattr("jacky_cli.gateway_windows", _FakeWindowsModule)
    monkeypatch.setattr(
        "jacky_cli.gateway.find_gateway_pids",
        lambda **kw: [12345],
    )
    mgr = WindowsServiceManager()
    mgr.start("ignored")
...
def test_get_service_manager_returns_correct_backend(
    ...
    monkeypatch.setattr(
        "jacky_cli.service_manager.detect_service_manager", lambda: kind,
    )
...
def test_get_service_manager_raises_when_unsupported(
    ...
    monkeypatch.setattr(
        "jacky_cli.service_manager.detect_service_manager", lambda: "none",
    )
...
def test_get_service_manager_returns_s6_instance(
    ...
    monkeypatch.setattr(
        "jacky_cli.service_manager.detect_service_manager", lambda: "s6",
    )
...
# unprivileged jacky user from every lifecycle op. These tests run
...
def test_seed_supervise_skeleton_creates_expected_layout(tmp_path) -> None:
    ...
    from jacky_cli.service_manager import _seed_supervise_skeleton
...
def test_s6_register_creates_service_dir_and_triggers_scan(
    ...
    assert "jacky -p coder gateway run" in run_text
    assert "s6-setuidgid jacky" in run_text
...
def test_s6_register_staging_dir_is_dotfile_hidden_from_svscan(
    ...
    """
    import jacky_cli.service_manager as sm
...
def test_render_run_script_resets_home_before_exec() -> None:
    ...
    assert "export HOME=/opt/data" in run_text
    assert "exec s6-setuidgid jacky jacky -p coder gateway run --replace" in run_text
...
def test_render_run_script_uses_replace_to_take_over_stale_holder() -> None:
    ...
    ``jacky gateway run``, an agent action, the Open WebUI helper) holds
    the per-JACKY_HOME PID lock; the supervised slot then execs a bare
    ...
    Covers both the default (root JACKY_HOME, no ``-p``) and named-profile
    render paths.
    """
    default_text = S6ServiceManager._render_run_script("default", {})
    # Root profile: bare `jacky gateway run --replace` (no -p flag).
    assert "jacky gateway run --replace" in default_text
    assert "jacky -p default" not in default_text
...
def test_s6_lifecycle_persists_named_profile_desired_state(
    ...
    profile_dir = jacky_home / "profiles" / "coder"
...
def test_s6_lifecycle_persists_default_profile_desired_state(
    ...
    monkeypatch.setenv("JACKY_HOME", str(jacky_home / "profiles" / "coder"))

    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.start("gateway-default")
    state = json.loads((jacky_home / "gateway_state.json").read_text())
...
def test_lifecycle_raises_gateway_not_registered_for_missing_slot(
    ...
    assert "'typo'" in msg
    assert "jacky profile create typo" in msg
...
def test_lifecycle_raises_s6_command_error_on_subprocess_failure(
    ...
    import subprocess as _sp
    from jacky_cli.service_manager import S6CommandError
...
#
# `jacky gateway stop` inside a container dispatches through
...
def test_s6_log_run_chowns_gateways_parent(s6_scandir, fake_subprocess_run) -> None:
    ...
    dropped jacky user and s6-log crash-loops on `mkdir: Permission denied`.
    """
    mgr = S6ServiceManager(scandir=s6_scandir)
    mgr.register_profile_gateway("coder")
    ...
    parent_chown = 'chown jacky:jacky "$JACKY_HOME/logs/gateways"'
    ...
    assert 'chown -R jacky:jacky "$JACKY_HOME/logs/gateways"' not in log_text
    ...
    leaf_idx = log_text.index('chown -R jacky:jacky "$log_dir"')

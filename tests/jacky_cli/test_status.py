...
def test_show_status_termux_gateway_section_skips_systemctl(monkeypatch, capsys, tmp_path):
    from jacky_cli import status as status_mod
    ...
    monkeypatch.setattr(status_mod, "get_jacky_home", lambda: tmp_path, raising=False)
    ...
    assert "Start with:   jacky gateway" in output
...
def test_show_status_reports_nous_auth_error(monkeypatch, capsys, tmp_path):
    ...
    output = capsys.readouterr().out
    assert "Nous Portal   ✗ not logged in (run: jacky portal)" in output
...
class TestShowStatusXaiOAuth:
    ...
    def test_logged_in_shows_auth_store(self, monkeypatch, capsys, tmp_path):
        ...
        status_mod = _base_xai_mocks(monkeypatch, tmp_path)
        monkeypatch.setattr(auth_mod, "get_xai_oauth_auth_status",
                            lambda: {"logged_in": True, "auth_store": "/home/u/.jacky/auth.json"},
                            raising=False)

        status_mod.show_status(SimpleNamespace(all=False, deep=False))
        out = capsys.readouterr().out

        assert "Auth file:  /home/u/.jacky/auth.json" in out
    ...
    def test_not_logged_in_shows_login_command(self, monkeypatch, capsys, tmp_path):
        ...
        assert "not logged in (run: jacky auth add xai-oauth)" in out

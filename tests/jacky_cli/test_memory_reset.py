"""Tests for the `jacky memory reset` CLI command.
...
- Profile-scoped reset (uses JACKY_HOME)
"""
...
def memory_env(tmp_path, monkeypatch):
    """Set up a fake JACKY_HOME with memory files."""
    jacky_home = tmp_path / ".jacky"
    memories = jacky_home / "memories"
    memories.mkdir(parents=True)
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))

    # Create sample memory files
    (memories / "MEMORY.md").write_text(
        "§\nJacky repo is at ~/.jacky/jacky-agent\n§\nUser prefers dark themes",
        encoding="utf-8",
    )
...
class TestMemoryReset:
    ...
    def test_reset_no_files_exist(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        (jacky_home / "memories").mkdir(parents=True)
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    ...
    def test_reset_profile_scoped(self, tmp_path, monkeypatch):
        """Reset should work on the active profile's JACKY_HOME."""
    ...
    def test_reset_empty_memories_dir(self, tmp_path, monkeypatch):
        ...
        # The memories dir won't exist; get_jacky_home() / "memories" won't have files
        result = _run_memory_reset(target="all", yes=True)
        assert result == "nothing"

...
from jacky_cli import plugins_cmd
...
def test_discover_all_plugins_includes_entrypoint_plugins(monkeypatch, tmp_path):
    ...
    dist = SimpleNamespace(
        version="0.1.0",
        metadata={"Summary": "Karpathy-style LLM Wikis for Jacky"},
    )
    entry_point = SimpleNamespace(
        name="wiki",
        value="adapters.jacky.cli_plugin",
        group="jacky_agent.plugins",
        dist=dist,
    )

    monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: user_dir)
    monkeypatch.setattr(
        "jacky_cli.plugins.get_bundled_plugins_dir",
        lambda: bundled_dir,
    )
    monkeypatch.setattr(
    ...
    assert entries == [
        (
            "wiki",
            "0.1.0",
            "Karpathy-style LLM Wikis for Jacky",
            "entrypoint",
            "adapters.jacky.cli_plugin",
            "wiki",
        )
    ]
    ...
    assert payload == [
        {
            ...
            "status": "enabled",
            "version": "0.1.0",
            "description": "Karpathy-style LLM Wikis for Jacky",
            "source": "entrypoint",
        }
    ]

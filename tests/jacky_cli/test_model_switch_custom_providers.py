...
The terminal `jacky model` flow already exposes `custom_providers`, but the
shared slash-command pipeline (`/model` in CLI/gateway/Telegram) historically
only looked at `providers:`.
"""

import jacky_cli.providers as providers_mod
...
def test_list_authenticated_providers_can_skip_custom_provider_live_probe(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.models.fetch_api_models", fetch)
...
def test_switch_model_accepts_explicit_bare_custom_current_endpoint(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.models.validate_requested_model", lambda *a, **k: _MOCK_VALIDATION)
    monkeypatch.setattr("jacky_cli.model_switch.get_model_info", lambda *a, **k: None)
    monkeypatch.setattr("jacky_cli.model_switch.get_model_capabilities", lambda *a, **k: None)
...
def test_custom_provider_no_key_singular_model_still_probes_live_models(monkeypatch):
    ...
    so /model matches the terminal ``jacky model`` flow.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr(providers_mod, "JACKY_OVERLAYS", {})
...
def test_list_enumerates_dict_format_models_alongside_default(monkeypatch):
    ...
    Regression: Jacky's own writer stores configured models as a dict
...
def test_lmstudio_picker_probes_active_config_base_url(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.models.fetch_lmstudio_models", _fake_fetch)
...
def test_resolve_custom_provider_passes_key_env():
    ...
    """
    from jacky_cli.providers import resolve_custom_provider

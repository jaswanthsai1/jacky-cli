"""Unit tests for jacky_cli.managed_scope (resolver + loaders + key helpers)."""
import textwrap
...
def test_get_managed_dir_absent_override_returns_none(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JACKY_MANAGED_DIR", str(tmp_path / "nope"))
...
def test_get_managed_dir_empty_override_falls_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JACKY_MANAGED_DIR", "   ")  # whitespace = unset
    # Under pytest the /etc/jacky default is ignored, so this is None; the
...
def test_get_managed_dir_default_ignored_under_pytest(monkeypatch):
    ...
    monkeypatch.delenv("JACKY_MANAGED_DIR", raising=False)
    assert managed_scope.get_managed_dir() is None
...
def test_managed_dir_env_scrubbed_by_default():
    """conftest must scrub JACKY_MANAGED_DIR so a dev-shell value can't leak in."""
    import os

    assert "JACKY_MANAGED_DIR" not in os.environ

"""Tests for provider-aware `/model` validation in jacky_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("jacky_cli.models.fetch_api_models", return_value=api_models), \
         patch("jacky_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("nous:jacky-3", "openrouter")
        assert provider == "nous"
        assert model == "jacky-3"
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("jacky_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("jacky_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_anthropic_provider_uses_configured_base_url_for_live_catalog(self):
        ...
        with patch(
            ...
            "jacky_cli.models._urlopen_model_catalog_request",
            return_value=_Resp(),
        ) as mock_urlopen:
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("jacky_cli.models._urlopen_model_catalog_request", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("jacky_cli.models._urlopen_model_catalog_request", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("jacky_cli.models._urlopen_model_catalog_request", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("jacky_cli.models._urlopen_model_catalog_request", return_value=_Resp()):
...
class TestValidateApiFallback:
    ...
    def test_known_model_accepted_via_catalog_when_api_down(self):
        ...
        with patch(
            "jacky_cli.models.provider_model_ids",
    ...
    def test_unknown_provider_soft_accepted_when_api_down(self):
        ...
        with patch("jacky_cli.models.provider_model_ids", return_value=[]):
    ...
    def test_fetch_lmstudio_models_filters_embedding_type(self):
        ...
        with patch("jacky_cli.models._urlopen_model_catalog_request", return_value=mock_resp):
    ...
    def test_fetch_lmstudio_models_normalizes_native_api_base_url(self):
        ...
        with patch("jacky_cli.models._urlopen_model_catalog_request", return_value=mock_resp) as mock_urlopen:
    ...
    def test_fetch_lmstudio_models_raises_auth_error_on_401(self):
        ...
        with patch("jacky_cli.models._urlopen_model_catalog_request", side_effect=http_error):
...
class TestValidateCodexAutoCorrection:
    ...
    def test_missing_dash_auto_corrects(self):
        ...
        with patch("jacky_cli.models.provider_model_ids", return_value=codex_models):
...
class TestProbeApiModelsUserAgent:
    """Probing custom /v1/models must send a Jacky User-Agent.
    ...
    def test_probe_sends_jacky_user_agent(self):
        ...
        assert ua.startswith("jacky-cli/"), (
            f"User-Agent must advertise jacky-cli, got {ua!r}"
        )
    ...
    def test_probe_user_agent_sent_without_api_key(self):
        ...
        assert ua and ua.startswith("jacky-cli/")

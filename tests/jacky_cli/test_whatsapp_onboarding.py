...
def test_whatsapp_pairing_watcher_records_qr_and_connected():
    ...
    proc = _FakeProc([
        ...
        '{"event":"connected","user":{"id":"15551234567:1@s.whatsapp.net","name":"Jacky Bot"}}\n',
    ])
    ...
    assert record.account_name == "Jacky Bot"
...
def test_whatsapp_pairing_payload_includes_linked_account():
    ...
    assert payload["account_name"] == "Jacky Bot"
...
def test_start_whatsapp_onboarding_existing_creds_returns_linked_account(monkeypatch, tmp_path):
    ...
    (session_dir / "creds.json").write_text(
        '{"me":{"id":"15551234567:1@s.whatsapp.net","name":"Jacky Bot"}}',
        encoding="utf-8",
    )
    ...
    assert result["account_name"] == "Jacky Bot"
...
def test_spawn_whatsapp_pairing_process_uses_json_mode(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(jacky_constants, "find_node_executable", lambda command: "/usr/bin/node")
    monkeypatch.setattr(jacky_constants, "with_jacky_node_path", lambda env=None: {})

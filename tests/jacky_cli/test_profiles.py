"""Comprehensive tests for jacky_cli.profiles module.
...
# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and JACKY_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.jacky/profiles)
    * JACKY_HOME  -> tmp_path/.jacky  (so get_jacky_home() agrees)
    * Creates the bare-minimum ~/.jacky directory.
    """
...
class TestValidateProfileName:
    ...
    @pytest.mark.parametrize("name", ["jacky", "test", "tmp", "root", "sudo"])
    def test_reserved_names_rejected(self, name):
        """Reserved names collide with the Jacky install itself or with
...
class TestCreateProfile:
    ...
    def test_clone_all_excludes_sibling_profiles_tree(self, profile_env):
        """--clone-all from default ~/.jacky must not copy profiles/* (nested explosion)."""
        tmp_path = profile_env
        default_home = tmp_path / ".jacky"
    ...
    def test_clone_all_excludes_default_infrastructure(self, profile_env):
        """--clone-all from default profile excludes jacky-agent, .worktrees,
        ...
        (default_home / "jacky-agent" / ".git").mkdir(parents=True)
        (default_home / "jacky-agent" / "venv" / "bin").mkdir(parents=True)
        (default_home / "jacky-agent" / "README.md").write_text("repo")
        ...
        assert not (profile_dir / "jacky-agent").exists()
...
class TestNoSkillsOptOut:
    """Tests for `jacky profile create --no-skills` and the opt-out marker."""
    ...
    def test_seed_profile_skills_respects_marker(self, profile_env):
        ...
        called directly (e.g. by `jacky update`'s all-profile sync loop)."""
        profile_dir = create_profile("orchestrator", no_alias=True, no_skills=True)
...
class TestBackfillProfileEnvs:
    """Tests for backfill_profile_envs() — the `jacky update` pass that
    ...
    def test_copies_default_env_into_envless_profiles(self, profile_env):
        import stat
        tmp_path = profile_env
        (tmp_path / ".jacky" / ".env").write_text("OPENROUTER_API_KEY=root-key\n")
    ...
    def test_never_overwrites_existing_profile_env(self, profile_env):
        tmp_path = profile_env
        (tmp_path / ".jacky" / ".env").write_text("KEY=root\n")
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("jacky_cli.profiles._cleanup_gateway_service"):
    ...
    def test_rmtree_failure_raises(self, profile_env):
        ...
        with patch("jacky_cli.profiles._cleanup_gateway_service"), \
             patch("jacky_cli.profiles.time.sleep"), \
             patch("jacky_cli.profiles.shutil.rmtree", side_effect=PermissionError("locked")):
    ...
    def test_stops_profile_bound_backends_before_removal(self, profile_env):
        ...
        with patch("jacky_cli.profiles._cleanup_gateway_service"), \
             patch("jacky_cli.profiles._profile_bound_backend_pids", return_value=[4242]) as pids, \
    ...
    def test_rmtree_retries_transient_enotempty_then_succeeds(self, profile_env):
        ...
        with patch("jacky_cli.profiles._cleanup_gateway_service"), \
             patch("jacky_cli.profiles._profile_bound_backend_pids", return_value=[]), \
             patch("jacky_cli.profiles.time.sleep"), \
             patch("jacky_cli.profiles.shutil.rmtree", side_effect=flaky_rmtree):
            delete_profile("coder", yes=True)
    ...
    def test_backend_scan_only_matches_this_profile(self, profile_env, monkeypatch):
        ...
        procs = [
            ...
            FakeProc(101, ["python", "-m", "jacky_cli.main", "--profile", "coder", "serve"]),
            # Interactive chat for coder → NOT a backend subcommand, skipped.
            FakeProc(102, ["python", "-m", "jacky_cli.main", "--profile", "coder", "chat"]),
            # Backend for a different profile → skipped.
            FakeProc(103, ["python", "-m", "jacky_cli.main", "--profile", "other", "serve"]),
            # This very process → skipped even if it matched.
            FakeProc(self_pid, ["python", "-m", "jacky_cli.main", "--profile", "coder", "serve"]),
        ]
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".jacky" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_jacky_home_returns_default(self, profile_env):
        # JACKY_HOME points to tmp_path/.jacky which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".jacky" / "profiles" / "coder"
    ...
    def test_custom_path_returns_default(self, profile_env, monkeypatch):
        """A custom JACKY_HOME (Docker, etc.) IS the default root."""
        tmp_path = profile_env
        custom = tmp_path / "some" / "other" / "path"
        custom.mkdir(parents=True)
        monkeypatch.setenv("JACKY_HOME", str(custom))
        # With Docker-aware roots, a custom JACKY_HOME is the default —
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("jacky")
        assert result is not None
...
class TestWrapperScript:
    ...
    def test_creates_sh_on_posix(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        monkeypatch.setattr("jacky_cli.profiles.shutil.which", lambda name: "/opt/jacky/bin/jacky")
        from jacky_cli.profiles import create_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
        assert wrapper.name == "mybot"
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "exec /opt/jacky/bin/jacky -p mybot" in content
    ...
    def test_creates_bat_on_windows(self, profile_env, monkeypatch):
        ...
        assert "jacky -p mybot" in content
        assert "%*" in content

    def test_remove_finds_bat_on_windows(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        from jacky_cli.profiles import create_wrapper_script, remove_wrapper_script
        wrapper = create_wrapper_script("mybot")
        assert wrapper is not None
    ...
    def test_remove_returns_false_when_absent(self, profile_env):
        from jacky_cli.profiles import remove_wrapper_script
    ...
    def test_custom_alias_target_on_posix(self, profile_env, monkeypatch):
        ...
        content = wrapper.read_text()
        assert content.startswith("#!/bin/sh")
        assert "jacky -p redqueen" in content
...
class TestFindAliasForProfile:
    ...
    def test_profile_named_alias(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from jacky_cli.profiles import create_wrapper_script, find_alias_for_profile
    ...
    def test_no_alias_returns_none(self, profile_env, monkeypatch):
        monkeypatch.setattr("sys.platform", "darwin")
        from jacky_cli.profiles import find_alias_for_profile
    ...
    def test_ignores_unrelated_files(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr("sys.platform", "darwin")
        from jacky_cli.profiles import _get_wrapper_dir, find_alias_for_profile
...
class TestRenameProfile:
    ...
    def test_renames_root_honcho_host_without_changing_ai_peer(self, profile_env):
        ...
        assert "jacky.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["jacky_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["jacky_heimdall"]["peerName"] == "user-peer"
    ...
    def test_pins_ai_peer_when_absent_on_honcho_host_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "jacky.ssi_health": {"workspace": "jacky", "enabled": True}
            }
        }))
        ...
        assert "jacky.ssi_health" not in cfg["hosts"]
        assert cfg["hosts"]["jacky_heimdall"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["jacky_heimdall"]["workspace"] == "jacky"
    ...
    def test_does_not_overwrite_existing_honcho_host_on_rename(self, profile_env):
        ...
        honcho_path.write_text(json.dumps({
            "hosts": {
                "jacky.ssi_health": {"aiPeer": "ssi_health"},
                "jacky_heimdall": {"aiPeer": "heimdall"},
            }
        }))
        ...
        cfg = json.loads(honcho_path.read_text())
        assert cfg["hosts"]["jacky.ssi_health"]["aiPeer"] == "ssi_health"
        assert cfg["hosts"]["jacky_heimdall"]["aiPeer"] == "heimdall"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("jacky-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".jacky_history",
        ...
        excluded_prefixes = [
            "default/jacky-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.jacky_history",
    ...
    def test_export_default_uses_allowlist_for_unrelated_dirs(self, profile_env, tmp_path):
        """Unrelated directories under JACKY_HOME are excluded by allow-list (#58394).

        Docker/custom deployments often set JACKY_HOME to a working
        ...
        Jacky artifacts end up in the archive. Replaces the old
...
# ===================================================================
# TestGetProfilesRoot / TestGetDefaultJackyHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_jacky_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".jacky" / "profiles"

    def test_default_jacky_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_jacky_home()
        assert home == tmp_path / ".jacky"

    def test_profiles_root_docker_deployment(self, tmp_path, monkeypatch):
        """In Docker (JACKY_HOME outside ~/.jacky), profiles go under JACKY_HOME."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(docker_home))
        root = _get_profiles_root()
        assert root == docker_home / "profiles"

    def test_default_jacky_home_docker(self, tmp_path, monkeypatch):
        """In Docker, _get_default_jacky_home() returns JACKY_HOME itself."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(docker_home))
    ...
    def test_profiles_root_profile_mode(self, tmp_path, monkeypatch):
        """In profile mode (JACKY_HOME under ~/.jacky), profiles root is still ~/.jacky/profiles."""
        native = tmp_path / ".jacky"
    ...
    def test_active_profile_path_docker(self, tmp_path, monkeypatch):
        """In Docker, active_profile file lives under JACKY_HOME."""
        from jacky_cli.profiles import _get_active_profile_path
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(docker_home))
    ...
    def test_create_profile_docker(self, tmp_path, monkeypatch):
        """Profile created in Docker lands under JACKY_HOME/profiles/."""
        docker_home = tmp_path / "opt" / "data"
        docker_home.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(docker_home))
    ...
    def test_active_profile_name_docker_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(profile))
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".jacky" / "profiles" / "mybot"
        assert result == expected
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from jacky_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".jacky"
    ...
    def test_gateway_running_check_falls_back_to_runtime_state(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            return_value="jacky gateway run --replace",
        ):
            assert _check_gateway_running(default_home) is True
    ...
    def test_gateway_running_check_rejects_pid_reused_by_other_profile(self, profile_env):
        ...
        gateway green while ``jacky -p <name> gateway status`` showed it
        stopped.
        ...
        tmp_path = profile_env
        coder_home = tmp_path / ".jacky" / "profiles" / "coder"
    ...
    def test_gateway_running_check_detects_matching_named_profile(self, profile_env):
        ...
        with patch("gateway.status.get_running_pid", return_value=None), patch(
            ...
            "gateway.status._read_process_cmdline",
            return_value="jacky -p coder gateway run --replace",
        ):
...
class TestProfilesToServe:
    ...
    def test_off_returns_only_active_default(self, profile_env):
        ...
        name, home = serve[0]
        assert name == "default"
        assert home == _get_default_jacky_home()

    def test_off_returns_only_active_named(self, profile_env, monkeypatch):
        # A named profile's gateway runs with JACKY_HOME pointing at the
        # profile dir; get_active_profile_name() infers the name from there.
        create_profile("coder", no_alias=True)
        monkeypatch.setenv("JACKY_HOME", str(get_profile_dir("coder")))
        serve = profiles_to_serve(multiplex=False)
        assert len(serve) == 1
    ...
    def test_on_returns_default_plus_all_named(self, profile_env):
        ...
        assert serve["default"] == _get_default_jacky_home()

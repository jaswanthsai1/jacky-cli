"""Tests for ``jacky update`` / ``--check`` inside the Docker container.
...
fix, ``jacky update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Jacky, not an update to the running container, so the message
...
from jacky_cli.main import _cmd_update_check, cmd_update
...
@patch("jacky_cli.config.detect_install_method", return_value="docker")
@patch("subprocess.run")
def test_cmd_update_in_docker_prints_guidance_and_exits(
    mock_run, _mock_method, _mock_managed, capsys
):
    """``jacky update`` inside Docker → friendly message + exit 1, no git calls."""
    ...
    assert "docker pull jaswanthsai1/jacky-cli:latest" in out
...
def test_cmd_update_check_in_docker_prints_guidance_and_exits(
    ...
    """``jacky update --check`` inside Docker → same message + exit 1, no fetch."""
    with pytest.raises(SystemExit) as excinfo:
...
def test_format_docker_update_message_contents():
    ...
    assert "docker pull jaswanthsai1/jacky-cli:latest" in msg
    ...
    assert "JACKY_HOME" in msg or "/opt/data" in msg, (

...
def test_single_handler_builders(name, builder, kw, argv):
    parser = argparse.ArgumentParser(prog="jacky")
...
# ── deprecated `jacky login` fails gracefully, not with argparse error ────
#
# `jacky login` is a removed command; its handler (`login_command` in
# `jacky_cli/auth.py`) prints a deprecation notice pointing at `jacky auth` /
# `jacky model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `jacky --help`.

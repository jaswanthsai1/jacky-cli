"""Tests for ``jacky debug`` CLI command and debug utilities."""
...
def jacky_home(tmp_path, monkeypatch):
    """Set up an isolated JACKY_HOME with minimal logs."""
    home = tmp_path / ".jacky"
    home.mkdir()
    ...
    (logs_dir / "gui.log").write_text(
        "2026-04-12 17:00:12 INFO jacky_cli.web_server: dashboard request\n"
    )
...
class TestUploadPasteRs:
    ...
    def test_upload_paste_rs_success(self):
        from jacky_cli.debug import _upload_paste_rs
        ...
        with patch("jacky_cli.debug.urllib.request.urlopen", return_value=mock_resp):
    ...
    def test_upload_paste_rs_network_error(self):
        ...
        with patch(
            "jacky_cli.debug.urllib.request.urlopen",
...
class TestUploadDpasteCom:
    ...
    def test_upload_dpaste_com_success(self):
        from jacky_cli.debug import _upload_dpaste_com
...
class TestUploadToPastebin:
    ...
    def test_tries_paste_rs_first(self):
        from jacky_cli.debug import upload_to_pastebin

        with patch("jacky_cli.debug._upload_paste_rs",
    ...
    def test_falls_back_to_dpaste_com(self):
        ...
        with patch("jacky_cli.debug._upload_paste_rs",
             ...
             patch("jacky_cli.debug._upload_dpaste_com",
...
class TestCaptureLogSnapshot:
    ...
    def test_reads_small_file(self, jacky_home):
        from jacky_cli.debug import _capture_log_snapshot
    ...
    def test_empty_primary_reports_file_empty(self, jacky_home):
        """Empty primary (no .1 fallback) surfaces as '(file empty)', not missing."""
        (jacky_home / "logs" / "agent.log").write_text("")
    ...
    def test_race_truncate_after_resolve_reports_empty(self, jacky_home, monkeypatch):
        """If the log is truncated between resolve and stat, say 'empty', not 'missing'."""
        log_path = jacky_home / "logs" / "agent.log"
        from jacky_cli import debug
    ...
    def test_keeps_first_line_when_truncation_on_boundary(self, jacky_home):
        ...
        (jacky_home / "logs" / "agent.log").write_text(line * num_lines)
    ...
    def test_drops_partial_when_truncation_mid_line(self, jacky_home):
    ...
    def test_unknown_log_returns_none(self, jacky_home):
        from jacky_cli.debug import _capture_log_snapshot
        snap = _capture_log_snapshot("nonexistent", tail_lines=10)
        assert snap.full_text is None

    def test_falls_back_to_rotated_file(self, jacky_home):
        """When gateway.log doesn't exist, falls back to gateway.log.1."""
        from jacky_cli.debug import _capture_log_snapshot

        logs_dir = jacky_home / "logs"
    ...
    def test_prefers_primary_over_rotated(self, jacky_home):
    ...
    def test_falls_back_when_primary_empty(self, jacky_home):
...
# ---------------------------------------------------------------------------
# Capture log redaction (force=True applies regardless of JACKY_REDACT_SECRETS)
# ---------------------------------------------------------------------------
...
class TestCaptureLogSnapshotRedaction:
    ...
    @pytest.fixture
    def jacky_home_with_secret(self, tmp_path, monkeypatch):
        """Isolated JACKY_HOME whose agent.log contains a vendor-prefixed token."""
        home = tmp_path / ".jacky"
        home.mkdir()
        monkeypatch.setenv("JACKY_HOME", str(home))
        ...
        monkeypatch.delenv("JACKY_REDACT_SECRETS", raising=False)
    ...
    def test_default_redacts_tail_and_full_text(self, jacky_home_with_secret):
        from jacky_cli.debug import _capture_log_snapshot
    ...
    def test_redact_false_passes_through(self, jacky_home_with_secret):
        from jacky_cli.debug import _capture_log_snapshot
    ...
    def test_force_true_works_when_redaction_disabled(
        self, jacky_home_with_secret, monkeypatch
    ):
        ...
        input unchanged when JACKY_REDACT_SECRETS=false, and the share-time
        ...
        monkeypatch.setenv("JACKY_REDACT_SECRETS", "false")

        from jacky_cli.debug import _capture_log_snapshot

        assert os.environ.get("JACKY_REDACT_SECRETS", "") == "false"
    ...
    def test_default_redacts_email_addresses_for_public_share(
        self, jacky_home_with_secret
    ):
        from jacky_cli.debug import _capture_log_snapshot

        log_path = jacky_home_with_secret / "logs" / "agent.log"
        log_path.write_text(
    ...
    def test_no_redact_preserves_email_addresses(self, jacky_home_with_secret):
        from jacky_cli.debug import _capture_log_snapshot
    ...
    def test_capture_default_log_snapshots_threads_redact(
        ...
        from jacky_cli.debug import _capture_default_log_snapshots
...
class TestCollectDebugReport:
    ...
    def test_report_includes_dump_output(self, jacky_home):
        from jacky_cli.debug import collect_debug_report

        with patch("jacky_cli.dump.run_dump") as mock_dump:
            mock_dump.side_effect = lambda args: print(
                "--- jacky dump ---\nversion: 0.8.0\n--- end dump ---"
            )
            report = collect_debug_report(log_lines=50)

        assert "--- jacky dump ---" in report
        assert "version: 0.8.0" in report

    def test_report_includes_agent_log(self, jacky_home):
        from jacky_cli.debug import collect_debug_report

        with patch("jacky_cli.dump.run_dump"):
            report = collect_debug_report(log_lines=50)

        assert "--- agent.log" in report
        assert "session started" in report

    def test_report_includes_errors_log(self, jacky_home):
        from jacky_cli.debug import collect_debug_report
    ...
    def test_report_includes_gateway_log(self, jacky_home):
        from jacky_cli.debug import collect_debug_report
    ...
    def test_report_includes_gui_log(self, jacky_home):
        from jacky_cli.debug import collect_debug_report
    ...
    def test_report_includes_desktop_log(self, jacky_home):
        from jacky_cli.debug import collect_debug_report
...
class TestRunDebugShare:
    ...
    def test_share_sweeps_expired_pastes(self, jacky_home, capsys):
        """Slash-command path should sweep old pending deletes before uploading."""
        from jacky_cli.debug import run_debug_share
        ...
        with patch("jacky_cli.dump.run_dump"), \
             patch("jacky_cli.debug._sweep_expired_pastes", return_value=(0, 0)) as mock_sweep, \
             patch("jacky_cli.debug.upload_to_pastebin",
    ...
    def test_share_survives_sweep_failure(self, jacky_home, capsys):
        ...
        with patch("jacky_cli.dump.run_dump"), \
             patch(
                 "jacky_cli.debug._sweep_expired_pastes",
    ...
    def test_local_flag_prints_full_logs(self, jacky_home, capsys):
    ...
    def test_share_uploads_five_pastes(self, jacky_home, capsys):
        ...
        with patch("jacky_cli.dump.run_dump") as mock_dump, \
            ...
            mock_dump.side_effect = lambda a: print("--- jacky dump ---\nversion: test\n--- end dump ---")
            run_debug_share(args)
        ...
        assert "--- jacky dump ---" in agent_paste
        assert "--- full agent.log ---" in agent_paste
        gateway_paste = uploaded_content[2]
        assert "--- jacky dump ---" in gateway_paste
        assert "--- full gateway.log ---" in gateway_paste
        gui_paste = uploaded_content[3]
        assert "--- jacky dump ---" in gui_paste
        assert "--- full gui.log ---" in gui_paste
        desktop_paste = uploaded_content[4]
        assert "--- jacky dump ---" in desktop_paste
        assert "--- full desktop.log ---" in desktop_paste

    def test_share_keeps_report_and_full_log_on_same_snapshot(self, jacky_home, capsys):
        """A mid-run rotation must not make full agent.log older than the report."""
        from jacky_cli.debug import run_debug_share, collect_debug_report as real_collect_debug_report
        ...
        with patch("jacky_cli.dump.run_dump"), \
             patch("jacky_cli.debug.collect_debug_report", side_effect=_wrapped_collect_debug_report), \
             patch("jacky_cli.debug.upload_to_pastebin", side_effect=_mock_upload):
            run_debug_share(args)
    ...
    def test_share_continues_on_log_upload_failure(self, jacky_home, capsys):
    ...
    def test_share_exits_on_report_upload_failure(self, jacky_home, capsys):
...
class TestRunDebugShareRedaction:
    ...
    def test_default_share_redacts_uploaded_content(
        self, jacky_home_with_secret, capsys
    ):
        ...
        with patch("jacky_cli.dump.run_dump"), \
             patch("jacky_cli.debug._sweep_expired_pastes", return_value=(0, 0)), \
             patch("jacky_cli.debug.upload_to_pastebin", side_effect=fake_upload):
            run_debug_share(args)
...
class TestRunDebug:
    def test_no_subcommand_shows_usage(self, capsys):
        ...
        out = capsys.readouterr().out
        assert "jacky debug" in out
        assert "share" in out
        assert "delete" in out

    def test_share_subcommand_routes(self, jacky_home):
        from jacky_cli.debug import run_debug
...
class TestExtractPasteId:
    def test_paste_rs_url(self):
        from jacky_cli.debug import _extract_paste_id
...
class TestDeletePaste:
    def test_delete_sends_delete_request(self):
        from jacky_cli.debug import delete_paste
        ...
        with patch("jacky_cli.debug.urllib.request.urlopen",
...
class TestScheduleAutoDelete:
    ...
    ``~/.jacky/pastes/pending.json`` and lets ``_sweep_expired_pastes``
    handle the DELETE requests synchronously on the next ``jacky debug``
    invocation.
    """

    def test_does_not_spawn_subprocess(self, jacky_home):
        ...
        import ast
        import inspect
        from jacky_cli.debug import _schedule_auto_delete
    ...
    def test_records_pending_to_json(self, jacky_home):
        """Scheduled URLs are persisted to pending.json with expiration."""
        from jacky_cli.debug import _schedule_auto_delete, _pending_file
        import json
    ...
    def test_skips_non_paste_rs_urls(self, jacky_home):
    ...
    def test_merges_with_existing_pending(self, jacky_home):
        """Subsequent calls merge into existing pending.json."""
        from jacky_cli.debug import _schedule_auto_delete, _load_pending
    ...
    def test_dedupes_same_url(self, jacky_home):
...
class TestSweepExpiredPastes:
    ...
    def test_sweep_empty_is_noop(self, jacky_home):
        from jacky_cli.debug import _sweep_expired_pastes

        deleted, remaining = _sweep_expired_pastes()
        assert deleted == 0
        assert remaining == 0

    def test_sweep_deletes_expired_entries(self, jacky_home):
        from jacky_cli.debug import (
        ...
        with patch("jacky_cli.debug.delete_paste", side_effect=fake_delete):
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_leaves_future_entries_alone(self, jacky_home):
        from jacky_cli.debug import _sweep_expired_pastes, _save_pending
        import time
        ...
        with patch("jacky_cli.debug.delete_paste") as mock_delete:
            deleted, remaining = _sweep_expired_pastes()
    ...
    def test_sweep_survives_network_failure(self, jacky_home):
    ...
    def test_sweep_drops_entries_past_grace_window(self, jacky_home):
...
class TestRunDebugSweepsOnInvocation:
    ...
    def test_run_debug_calls_sweep(self, jacky_home):
        from jacky_cli.debug import run_debug

        args = MagicMock()
        args.debug_command = None  # default → prints help

        with patch("jacky_cli.debug._sweep_expired_pastes") as mock_sweep:
            run_debug(args)

        mock_sweep.assert_called_once()

    def test_run_debug_survives_sweep_failure(self, jacky_home, capsys):
        ...
        out = capsys.readouterr().out
        assert "Usage: jacky debug" in out


class TestRunDebugDelete:
    def test_deletes_valid_url(self, capsys):
        from jacky_cli.debug import run_debug_delete

        args = MagicMock()
        args.urls = ["https://paste.rs/abc"]

        with patch("jacky_cli.debug.delete_paste", return_value=True):
    ...
    def test_handles_delete_failure(self, capsys):
        ...
        with patch("jacky_cli.debug.delete_paste",
...
class TestShareIncludesAutoDelete:
    ...
    def test_share_schedules_auto_delete(self, jacky_home, capsys):
        from jacky_cli.debug import run_debug_share
        ...
        with patch("jacky_cli.dump.run_dump"), \
             ...
             patch("jacky_cli.debug._schedule_auto_delete") as mock_sched:
            run_debug_share(args)
    ...
    def test_share_shows_privacy_notice(self, jacky_home, capsys):
        from jacky_cli.debug import run_debug_share
        ...
        with patch("jacky_cli.dump.run_dump"), \
             patch("jacky_cli.debug.upload_to_pastebin",
                    return_value="https://paste.rs/test"), \
             patch("jacky_cli.debug._schedule_auto_delete"):
            run_debug_share(args)
    ...
    def test_local_no_privacy_notice(self, jacky_home, capsys):
        from jacky_cli.debug import run_debug_share
...
class TestBuildDebugShare:
    ...
    Backs both ``jacky debug share`` (CLI) and ``POST /api/ops/debug-share``
    (dashboard). The dashboard renders ``urls`` as real, copyable links, so the
    contract here is the return value, not stdout.
    """

    def test_returns_structured_urls(self, jacky_home):
        from jacky_cli.debug import build_debug_share, DebugShareResult
        ...
        with patch("jacky_cli.dump.run_dump"), patch(
            "jacky_cli.debug.upload_to_pastebin", side_effect=_upload
        ), patch("jacky_cli.debug._schedule_auto_delete"):
    ...
    def test_skips_missing_logs_without_failure(self, jacky_home):
        from jacky_cli.debug import build_debug_share

        # Remove desktop.log so it should be neither uploaded nor reported failed.
        (jacky_home / "logs" / "desktop.log").unlink()

        with patch("jacky_cli.dump.run_dump"), patch(
            "jacky_cli.debug.upload_to_pastebin",
    ...
    def test_redaction_keeps_secrets_out_of_payload(self, jacky_home):
        from jacky_cli.debug import build_debug_share
    ...
    def test_optional_log_failure_is_collected_not_raised(self, jacky_home):
        from jacky_cli.debug import build_debug_share
    ...
    def test_required_report_failure_raises(self, jacky_home):
        from jacky_cli.debug import build_debug_share
...
class TestCollectShareBundle:
    def test_returns_report_and_logs(self, jacky_home):
        from jacky_cli.debug import collect_share_bundle
    ...
    def test_no_redact_omits_banner(self, jacky_home):
        from jacky_cli.debug import collect_share_bundle
    ...
    def test_redaction_keeps_secrets_out(self, jacky_home):
        from jacky_cli.debug import collect_share_bundle
    ...
    def test_build_debug_share_uses_collector(self, jacky_home):
        # build_debug_share must produce the same report text the collector does
        # (i.e. the refactor preserved paste.rs behaviour).
        from jacky_cli.debug import build_debug_share, collect_share_bundle
...
class TestBuildNousBundle:
    def test_envelope_shape_and_gzip(self, jacky_home):
        import gzip
        import json as _json

        from jacky_cli.debug import build_nous_bundle
        ...
        assert envelope["format"] == "jacky-debug-share/1"
...
class TestRunDebugShareNous:
    ...
    def test_nous_success_prints_view_url(self, jacky_home, capsys):
        from jacky_cli.debug import run_debug_share
        ...
        with patch("jacky_cli.dump.run_dump"), patch(
            "jacky_cli.diagnostics_upload.share_to_nous", return_value=res
        ) as share:
    ...
    def test_nous_failure_suggests_local(self, jacky_home, capsys):
        from jacky_cli.debug import run_debug_share

        with patch("jacky_cli.dump.run_dump"), patch(
            "jacky_cli.diagnostics_upload.share_to_nous",
    ...
    def test_nous_does_not_touch_pastebin(self, jacky_home):
        from jacky_cli.debug import run_debug_share

        res = {"id": "id-1", "viewUrl": "https://v"}
        with patch("jacky_cli.dump.run_dump"), patch(
            "jacky_cli.diagnostics_upload.share_to_nous", return_value=res
        ), patch("jacky_cli.debug.upload_to_pastebin") as paste:
            run_debug_share(self._args())
...
class TestDebugSlashCommand:
    ...
    ``JackyCLI.process_command`` → ``_handle_debug_command(cmd_original)``,
    ...
    def _captured(self, cmd_original):
        ...
        with patch("jacky_cli.debug.run_debug_share", _fake_run):
...
class TestShareConsentGate:
    """`jacky debug share` requires explicit consent before uploading.
    ...
    def test_aborts_on_user_decline(self, jacky_home, capsys, monkeypatch):
        ...
        with patch("jacky_cli.dump.run_dump"), \
             patch("jacky_cli.debug.upload_to_pastebin") as mock_upload:
            run_debug_share(self._args())

        mock_upload.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_proceeds_on_user_accept(self, jacky_home, capsys, monkeypatch):
    ...
    def test_yes_flag_skips_prompt(self, jacky_home, capsys, monkeypatch):
    ...
    def test_non_interactive_requires_yes(self, jacky_home, capsys, monkeypatch):
    ...
    def test_non_interactive_with_yes_succeeds(self, jacky_home, capsys, monkeypatch):
    ...
    def test_nous_path_also_gated(self, jacky_home, capsys, monkeypatch):
        ...
        with patch("jacky_cli.dump.run_dump"), \
             patch("jacky_cli.diagnostics_upload.share_to_nous") as mock_nous:
            run_debug_share(self._args(nous=True))

        mock_nous.assert_not_called()
        assert "Aborted" in capsys.readouterr().out

    def test_local_never_prompts(self, jacky_home, capsys, monkeypatch):

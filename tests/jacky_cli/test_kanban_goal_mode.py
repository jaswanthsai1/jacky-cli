...
2. Spawn: _default_spawn sets the JACKY_KANBAN_GOAL_MODE env vars only
...
def test_spawn_sets_goal_env_only_when_enabled(kanban_home, monkeypatch):
    ...
    env = captured["env"]
    assert env.get("JACKY_KANBAN_GOAL_MODE") == "1"
    assert env.get("JACKY_KANBAN_GOAL_MAX_TURNS") == "5"
...
def test_spawn_no_goal_env_for_plain_task(kanban_home, monkeypatch):
    ...
    kb._default_spawn(task, str(kanban_home))
    env = captured["env"]
    assert "JACKY_KANBAN_GOAL_MODE" not in env
    assert "JACKY_KANBAN_GOAL_MAX_TURNS" not in env

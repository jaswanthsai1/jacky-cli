    ...
    token = _jwt(
        {
            ...
            "nous_client": "jacky-agent",
            "exp": int(time.time()) + 900,
            "paid_access": True,
            "subscription_tier": 2,
        }
    )
    monkeypatch.setattr("jacky_cli.auth.get_provider_auth_state", lambda provider: _state(token))
...
def test_expired_jwt_falls_back_to_fresh_account(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.auth.get_provider_auth_state", lambda provider: _state(token))
    monkeypatch.setattr("jacky_cli.auth.resolve_nous_access_token", lambda: "fresh-token")
    monkeypatch.setattr("jacky_cli.nous_account._fetch_nous_account_info", lambda *a, **kw: payload)
...
def test_entitlement_message_for_inference_key_without_portal_login():
    ...
    assert "Log in with `jacky model`" in message
...
def test_entitlement_message_for_unknown_entitlement_is_explicit():
    ...
    assert "Run `jacky model`" in message

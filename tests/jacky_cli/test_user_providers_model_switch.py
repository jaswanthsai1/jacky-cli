...
import pytest
from jacky_cli.model_switch import list_authenticated_providers, switch_model
from jacky_cli import runtime_provider as rp
...
def test_list_authenticated_providers_enumerates_dict_format_models(monkeypatch):
    ...
    (canonical Jacky write format) should surface every key in the picker.

    Regression: the ``providers:`` dict path previously only accepted
    list-format ``models:`` and silently dropped dict-format entries,
    even though Jacky's own writer and downstream readers use dict format.
    """
    monkeypatch.setattr("agent.models_dev.fetch_models_dev", lambda: {})
    monkeypatch.setattr("jacky_cli.providers.JACKY_OVERLAYS", {})
...
def test_list_authenticated_providers_uses_live_models_for_user_provider(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_list_authenticated_providers_accepts_base_url_and_singular_model(monkeypatch):
    """providers: dict entries written in canonical Jacky shape
    ...
    ``default_model``, so new-shape entries written by Jacky's own writer
...
def test_switch_model_resolves_user_provider_credentials(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "jacky_cli.models.validate_requested_model",
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("jacky_cli.model_switch.resolve_alias", return_value=None), \
         patch("jacky_cli.model_switch.list_provider_models", return_value=[]), \
         patch("jacky_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("jacky_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         patch("jacky_cli.models.detect_provider_for_model", return_value=None), \
         patch("jacky_cli.model_switch.get_model_info", return_value=None), \
         patch("jacky_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("jacky_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.models.fetch_api_models", _fake_fetch)
...
def test_section3_skips_probe_when_no_key_but_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.models.fetch_api_models", _fail_fetch)
...
def test_current_custom_model_is_surfaced_in_builtin_provider_row(monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.models.cached_provider_model_ids",

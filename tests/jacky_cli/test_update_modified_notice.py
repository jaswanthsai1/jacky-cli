"""Guard: every `jacky update` path that reports user-modified skills must
also tell the user how to find them.

`jacky update` keeps (does not overwrite) bundled skills the user edited and
prints a ``~ N user-modified (kept)`` count. There are two independent update
code paths in ``jacky_cli/main.py`` that print this notice (the git-pull path
in ``_cmd_update_impl`` and the unpack/install path). Both must point the user
at ``jacky skills list-modified`` so the count is actionable — otherwise,
...
_HINT_RE = re.compile(r"jacky skills list-modified")
...
def test_every_user_modified_notice_points_at_list_modified():
    ...
    for idx in count_sites:
        ...
        assert _HINT_RE.search(window), (
            ...
            "`jacky skills list-modified` within the following lines — the "

...
from jacky_cli.console_engine import JackyConsoleEngine, run_console_repl
...
MUTATING_CONFIRMATION_SMOKE_COMMANDS = [
    ...
    "mcp picker",
    "backup --quick -o /tmp/jacky-console-test.zip",
    "import /tmp/jacky-console-test.zip",
...
def test_console_parses_bare_and_jacky_prefixed_commands(_isolate_jacky_home):
    engine = JackyConsoleEngine()

    bare = engine.execute("config path")
    prefixed = engine.execute("jacky config path")
...
def test_console_status_hides_cli_next_step_footer(
    ...
    import jacky_cli.status as status_mod
    ...
    def fake_show_status(_args):
        ...
        print("\x1b[2m  Run 'jacky doctor' for detailed diagnostics\x1b[0m")
        print("\x1b[2m  Run 'jacky setup' to configure\x1b[0m")
        print()

    monkeypatch.setattr(status_mod, "show_status", fake_show_status)

    result = JackyConsoleEngine().execute("status")

    assert result.status == "ok"
    assert "Sessions" in result.output
    assert "Active: 3 session(s)" in result.output
    assert "jacky doctor" not in result.output
    assert "jacky setup" not in result.output
...
def test_console_status_hides_osc_linked_cli_next_step_footer(
    ...
    def fake_show_status(_args):
        ...
        print()
        print(osc_link("\u2500" * 60))
        print(osc_link("  Run 'jacky doctor' for detailed diagnostics"))
        print(osc_link("  Run 'jacky setup' to configure"))
        print()
...
def test_console_help_uses_cli_subcommand_summaries():
    help_text = JackyConsoleEngine().help_text()
    ...
    assert "Run `jacky skills list`" not in help_text
    assert "Run `jacky tools list`" not in help_text
...
def test_console_help_for_command_uses_cli_summary():
    help_text = JackyConsoleEngine().help_text("skills list")

    assert help_text == "skills list\nList installed skills"


def test_console_registry_covers_non_admin_cli_surface():
    registered = set(JackyConsoleEngine().commands)
...
def test_hosted_console_registry_exposes_only_hosted_safe_surface():
    engine = JackyConsoleEngine(context="hosted")
    hosted = {
...
def test_hosted_console_rejects_local_only_or_dangerous_commands(line):
    result = JackyConsoleEngine(context="hosted").execute(line)
...
def test_console_rejects_destructive_and_shell_like_commands(line):
    result = JackyConsoleEngine().execute(line)
...
def test_help_lists_supported_commands_and_not_full_cli():
    result = JackyConsoleEngine().execute("help")
...
def test_config_set_requires_confirmation_then_writes(_isolate_jacky_home):
    engine = JackyConsoleEngine()
...
def test_cron_pause_resume_and_run_require_confirmation(_isolate_jacky_home):
...
def test_repl_runs_non_interactive_lines_without_prompts(_isolate_jacky_home):
    ...
    assert code == 0
    assert "Jacky Console" in stdout.getvalue()
    assert "jacky>" not in stdout.getvalue()
    assert stderr.getvalue() == ""


def test_repl_refuses_non_interactive_confirmation(_isolate_jacky_home):
...
def test_main_console_subcommand_smoke(_isolate_jacky_home):
    import subprocess

    result = subprocess.run(
        [sys.executable, "-m", "jacky_cli.main", "console"],
    ...
    assert "Jacky Console" in result.stdout

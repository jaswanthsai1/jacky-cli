...
The cache exists to keep `jacky tools` → "All Platforms" fast: every
...
def test_save_env_value_invalidates_cache(tmp_path, monkeypatch):
    ...
    from jacky_cli.config import invalidate_env_cache, load_env, save_env_value
    ...
    monkeypatch.setattr(config_mod, "ensure_jacky_home", lambda: None)
...
def test_load_env_handles_missing_file():
    ...
    nonexistent = Path(tempfile.gettempdir()) / "jacky-test-no-such-env-xyz123.env"
    nonexistent.unlink(missing_ok=True)

    try:
        with patch("jacky_cli.config.get_env_path", return_value=nonexistent):
            assert load_env() == {}

"""Tests for jacky_cli.gui_uninstall — GUI-only uninstall + install discovery.
...
import jacky_cli.gui_uninstall as gu


def _make_agent(jacky_home: Path) -> Path:
    """Create a fake agent install: source package + venv."""
    agent_root = jacky_home / "jacky-agent"
    (agent_root / "jacky_cli").mkdir(parents=True)
    (agent_root / "jacky_cli" / "__init__.py").write_text("")
    (agent_root / "venv" / "bin").mkdir(parents=True)
    return agent_root


def _make_gui_build(jacky_home: Path) -> None:
    """Create the source-built GUI artifacts a `jacky desktop` run produces."""
    desktop = jacky_home / "jacky-agent" / "apps" / "desktop"
    ...
    (jacky_home / "jacky-agent" / "node_modules").mkdir(parents=True)
    (jacky_home / "desktop-build-stamp.json").write_text("{}")


def _make_user_data(jacky_home: Path) -> None:
    (jacky_home / "config.yaml").write_text("x: 1\n")
    (jacky_home / ".env").write_text("KEY=secret\n")
    (jacky_home / "sessions").mkdir()


def test_agent_is_installed_detects_source_and_venv(tmp_path):
    jacky_home = tmp_path / ".jacky"
    jacky_home.mkdir()
    assert gu.agent_is_installed(jacky_home) is False
    _make_agent(jacky_home)
    assert gu.agent_is_installed(jacky_home) is True


def test_agent_is_installed_venv_only(tmp_path):
    """A checkout with only a venv (no package dir yet) still counts."""
    jacky_home = tmp_path / ".jacky"
    (jacky_home / "jacky-agent" / "venv").mkdir(parents=True)
    assert gu.agent_is_installed(jacky_home) is True


def test_source_built_artifacts_lists_known_paths(tmp_path):
    jacky_home = tmp_path / ".jacky"
    _make_gui_build(jacky_home)
    artifacts = gu.source_built_gui_artifacts(jacky_home)
...
def test_gui_is_installed_true_when_built(tmp_path, monkeypatch):
    ...
    assert gu.gui_is_installed(jacky_home) is True
...
def test_gui_is_installed_false_when_nothing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(gu, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu, "desktop_userdata_dir", lambda: tmp_path / "nope")
    assert gu.gui_is_installed(jacky_home) is False
...
def test_uninstall_gui_removes_only_gui_artifacts(tmp_path, monkeypatch):
    ...
    jacky_home = tmp_path / ".jacky"
    agent_root = _make_agent(jacky_home)
    _make_gui_build(jacky_home)
    _make_user_data(jacky_home)
    ...
    removed = gu.uninstall_gui(jacky_home)
    ...
    assert not (jacky_home / "desktop-build-stamp.json").exists()
    assert "dist" in removed_names

    # Agent + user data preserved.
    assert (agent_root / "jacky_cli" / "__init__.py").exists()
    assert (agent_root / "venv").exists()
    assert (jacky_home / "config.yaml").exists()
    assert (jacky_home / ".env").exists()
    assert (jacky_home / "sessions").exists()
...
def test_uninstall_gui_removes_userdata(tmp_path, monkeypatch):
    jacky_home = tmp_path / ".jacky"
    _make_agent(jacky_home)
    userdata = tmp_path / "Jacky-userdata"
    userdata.mkdir()
...
def test_uninstall_gui_keeps_userdata_when_requested(tmp_path, monkeypatch):
    ...
    gu.uninstall_gui(jacky_home, remove_userdata=False)
...
def test_gui_install_summary_shape(tmp_path, monkeypatch):
    ...
    summary = gu.gui_install_summary(jacky_home)
    ...
    assert summary["jacky_home"] == str(jacky_home)
    assert summary["platform"] == sys.platform


def test_userdata_dir_per_platform(monkeypatch):
    """userData path matches Electron's app.getPath('userData') for "Jacky"."""
    home = Path("/home/tester")
    monkeypatch.setattr(Path, "home", classmethod(lambda cls: home))

    monkeypatch.setattr(gu.sys, "platform", "darwin")
    assert gu.desktop_userdata_dir() == home / "Library" / "Application Support" / "Jacky"

    monkeypatch.setattr(gu.sys, "platform", "linux")
    monkeypatch.delenv("XDG_CONFIG_HOME", raising=False)
    assert gu.desktop_userdata_dir() == home / ".config" / "Jacky"
...
def test_userdata_dir_windows(monkeypatch):
    ...
    assert gu.desktop_userdata_dir() == Path(r"C:\Users\tester\AppData\Roaming") / "Jacky"
...
def test_run_uninstall_yes_keep_data_is_non_interactive(tmp_path, monkeypatch):
    ...
    JACKY_HOME with all the destructive externals stubbed out.
    """
    import jacky_cli.uninstall as uninstall
    ...
    monkeypatch.setattr(uninstall, "get_jacky_home", lambda: jacky_home)
    ...
    from jacky_cli import gui_uninstall as gu_mod
    ...
    assert not (desktop / "release").exists()
    assert (jacky_home / "config.yaml").exists()
    assert jacky_home.exists()


def test_run_uninstall_yes_full_wipes_home(tmp_path, monkeypatch):
    """``--yes --full`` removes the whole JACKY_HOME non-interactively."""
    import jacky_cli.uninstall as uninstall

    jacky_home = tmp_path / ".jacky"
    (jacky_home / "jacky-agent" / "jacky_cli").mkdir(parents=True)
    (jacky_home / "config.yaml").write_text("x: 1\n")
    fake_code = tmp_path / "checkout"
    fake_code.mkdir()
    ...
    assert not jacky_home.exists()


def test_uninstall_module_main_gui_mode(tmp_path, monkeypatch):
    """`python -m jacky_cli.uninstall --mode gui` runs the GUI-only path.
    ...
    monkeypatch.setattr(gu_mod, "packaged_gui_app_paths", lambda: [])
    monkeypatch.setattr(gu_mod, "desktop_userdata_dir", lambda: tmp_path / "none")
    monkeypatch.setattr(gu_mod, "get_jacky_home", lambda: jacky_home)
    ...
    assert not (desktop / "release").exists()
    assert not (jacky_home / "desktop-build-stamp.json").exists()
    assert (agent_root / "jacky_cli").exists()
    assert (jacky_home / "config.yaml").exists()

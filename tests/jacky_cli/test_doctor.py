...
import jacky_cli.doctor as doctor
import jacky_cli.gateway as gateway_cli
from jacky_cli import doctor as doctor_mod
from jacky_cli.doctor import _has_provider_env_config
...
class TestDoctorEnvFileEncoding:
    """Regression for #18637 (bug 3): `jacky doctor` crashed on Windows
    ...
    def test_doctor_reads_env_as_utf8_even_when_locale_is_not_utf8(
        ...
        monkeypatch.setattr(doctor_mod, "JACKY_HOME", jacky_home)
...
class TestDoctorToolAvailabilityOverrides:
    ...
    def test_leaves_kanban_unavailable_when_worker_env_is_set(self, monkeypatch):
        monkeypatch.setenv("JACKY_KANBAN_TASK", "probe")
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("JACKY_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_jacky_home(self, tmp_path, provider=""):
        """Create a minimal JACKY_HOME with config.yaml."""
        home = tmp_path / ".jacky"
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_jacky_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "JACKY_HOME", home)
...
def test_run_doctor_accepts_jacky_provider_ids_that_catalog_aliases(
...
class TestGitHubTokenCheck:
    ...
    def _isolate_home(monkeypatch, home):
        """Point doctor at the temp JACKY_HOME.

        ``run_doctor`` reads the module-level ``JACKY_HOME`` constant (cached
        at import time), NOT the env var — so ``setenv("JACKY_HOME")`` alone
        leaves doctor probing the REAL ~/.jacky. On a dev machine with a
...
def test_has_healthy_oauth_fallback_returns_false_for_unknown_provider():
    from jacky_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
...
class TestHasHealthyOauthFallbackForXai:
    ...
    def test_returns_false_when_xai_import_unavailable(self, monkeypatch):
        ...
        monkeypatch.delattr("jacky_cli.auth.get_xai_oauth_auth_status", raising=False)
        # Force doctor module to re-import the function
        monkeypatch.delitem(sys.modules, "jacky_cli.doctor", raising=False)
        from jacky_cli.doctor import _has_healthy_oauth_fallback_for_apikey_provider
        assert _has_healthy_oauth_fallback_for_apikey_provider("xai") is False
...
class TestDoctorStaleMaxIterationsDrift:
    """Regression for #17534: a stale JACKY_MAX_ITERATIONS in .env shadows
    ...
    def _run_config_section(self, monkeypatch, tmp_path, *, fix, ghost, cfg_turns,
        ...
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir(parents=True)
        (jacky_home / "config.yaml").write_text(
            f"agent:\n  max_turns: {cfg_turns}\n", encoding="utf-8"
        )
        env_lines = ["OPENAI_API_KEY=sk-test\n"]
        if ghost is not None:
            env_lines.append(f"JACKY_MAX_ITERATIONS={ghost}\n")
        (jacky_home / ".env").write_text("".join(env_lines), encoding="utf-8")

        monkeypatch.setattr(doctor_mod, "JACKY_HOME", jacky_home)
        monkeypatch.setattr(doctor_mod, "get_jacky_home", lambda: jacky_home)
        ...
        if os_environ_value is not None:
            ...
            monkeypatch.setenv("JACKY_MAX_ITERATIONS", str(os_environ_value))
        else:
            monkeypatch.delenv("JACKY_MAX_ITERATIONS", raising=False)
    ...
    def test_fix_removes_ghost(self, monkeypatch, tmp_path):
        ...
        assert "Removed stale JACKY_MAX_ITERATIONS" in out
        env_after = (jacky_home / ".env").read_text(encoding="utf-8")
        assert "JACKY_MAX_ITERATIONS" not in env_after
...
def test_npm_audit_fix_hint_avoids_crashing_workspace_flag(monkeypatch, tmp_path):
    """`jacky doctor` must not hand users `npm audit fix --workspace <name>`:
    ...
    # left thinking a crashing command means a broken Jacky install.

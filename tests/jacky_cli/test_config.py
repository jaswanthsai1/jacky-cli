"""Tests for jacky_cli configuration management."""
...
class TestGetJackyHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            ...
            home = get_jacky_home()
            assert home == Path.home() / ".jacky"

    def test_env_override(self):
        with patch.dict(os.environ, {"JACKY_HOME": "/custom/path"}):
            home = get_jacky_home()
            assert home == Path("/custom/path")


class TestEnsureJackyHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path)}):
            ensure_jacky_home()
    ...
    def test_upgrades_legacy_template_soul_md(self, tmp_path):
        ...
        from jacky_cli.default_soul import DEFAULT_SOUL_MD, _LEGACY_TEMPLATE_SOULS
    ...
    def test_preserves_legacy_template_with_user_persona(self, tmp_path):
        ...
        from jacky_cli.default_soul import _LEGACY_TEMPLATE_SOULS
    ...
    def test_existing_named_profile_still_bootstraps_subdirs(self, tmp_path):
        profile_home = tmp_path / ".jacky" / "profiles" / "coder"
        profile_home.mkdir(parents=True)
        with patch.dict(os.environ, {"JACKY_HOME": str(profile_home)}):
            ensure_jacky_home()
...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``jacky logs`` surfaces it)
    ...
    def test_logs_and_warns_on_parse_failure(self, tmp_path, caplog, capsys):
        ...
        from jacky_cli import config as cfg_mod
        ...
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path)}):
            ...
            import logging
            with caplog.at_level(logging.WARNING, logger="jacky_cli.config"):
                config = load_config()
            ...
            # stands out at jacky startup before logging is configured)
            captured = capsys.readouterr()
            assert "jacky config:" in captured.err
    ...
    def test_rewarns_after_file_edit(self, tmp_path, capsys):
        ...
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path)}):
            ...
            assert "jacky config:" in after_edit, "edited file should re-warn"
...
class TestEmptyConfigSections:
    ...
    def test_null_override_of_non_dict_default_still_applies(self, tmp_path):
        ...
        from jacky_cli.config import _deep_merge
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
    ...
    def test_remove_env_value_preserves_existing_file_mode_on_posix(self, tmp_path):
        ...
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path), "DROP": "gone"}):
...
class TestSaveConfigAtomicity:
    ...
    def test_no_partial_write_on_crash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path)}):
            ...
            with patch("jacky_cli.utils.yaml.dump", side_effect=OSError("disk full")):
                try:
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from jacky_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        """JACKY_MAX_ITERATIONS must NOT be in OPTIONAL_ENV_VARS (issue #17534).

        Offering it as an editable env var (dashboard, `jacky setup`) lets a
        ...
        via config.yaml; JACKY_MAX_ITERATIONS remains a read-only backward-compat
        fallback in the gateway/CLI, never a promoted write target.
        """
        from jacky_cli.config import OPTIONAL_ENV_VARS
        assert "JACKY_MAX_ITERATIONS" not in OPTIONAL_ENV_VARS
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `jacky setup` surface it (previously only Honcho was listed, leaving
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or any ``JACKY_*`` runtime flag.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Jacky
    process on next startup via the dotenv → ``os.environ`` chain in
    ``jacky_cli/env_loader.py``.
    ...
    def _jacky_home(self, tmp_path, monkeypatch):
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            "JACKY_SPOTIFY_CLIENT_ID",
            "JACKY_QWEN_BASE_URL",
            "JACKY_MAX_ITERATIONS",
        ],
    )
    def test_jacky_integration_keys_still_writable(self, allowed_key):
        """``JACKY_*`` overall is NOT blocked — only the four runtime
        location names (HOME/PROFILE/CONFIG/ENV) are. Integration
        credentials following the ``JACKY_*`` convention must keep
    ...
    def test_arbitrary_user_key_still_works(self):
        ...
        denylist and outside ``JACKY_*``) keep working. The denylist
    ...
    def test_preserves_existing_codex_app_server_auto_value(self, tmp_path):
        with patch.dict(os.environ, {"JACKY_HOME": str(tmp_path)}):
            self._write(
                ...
                "_config_version: 31\n"
                "compression:\n"
                "  codex_app_server_auto: jacky\n",
            )

            migrate_config(interactive=False, quiet=True)

            raw = yaml.safe_load((tmp_path / "config.yaml").read_text())
            assert raw["compression"]["codex_app_server_auto"] == "jacky"

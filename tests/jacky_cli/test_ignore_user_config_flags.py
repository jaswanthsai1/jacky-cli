"""Tests for --ignore-user-config and --ignore-rules flags on `jacky chat`.
...
files. In Jacky the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.jacky/config.yaml`` in ``load_cli_config()``
...
argparse → cmd_chat → cli.main() → JackyCLI → AIAgent call chain.
"""
...
def _clean_env(monkeypatch):
    ...
    """
    for var in ("JACKY_IGNORE_USER_CONFIG", "JACKY_IGNORE_RULES"):
...
class TestIgnoreUserConfigEnvGate:
    """``load_cli_config()`` must honour ``JACKY_IGNORE_USER_CONFIG=1``.

    When the env var is set, user config at ``<jacky_home>/config.yaml`` is
    ...
    def _write_user_config(self, tmp_path, model_default):
        ...
        # config. With JACKY_IGNORE_USER_CONFIG=1, load_cli_config() falls
    ...
    def test_flag_ignored_when_set_to_other_value(self, tmp_path, monkeypatch):
        ...
        self._write_user_config(tmp_path, "test-vendor/ignore-user-config-sentinel")
        monkeypatch.setenv("JACKY_IGNORE_USER_CONFIG", "true")  # not "1"
...
class TestIgnoreRulesEnvGate:
    """The constructor / env var must propagate to ``JackyCLI.ignore_rules``
    so ``AIAgent`` is built with ``skip_context_files=True`` and
    ``skip_memory=True``.
    """

    def test_env_var_enables_ignore_rules(self, monkeypatch):
        """Setting JACKY_IGNORE_RULES=1 flips JackyCLI.ignore_rules True."""
        monkeypatch.setenv("JACKY_IGNORE_RULES", "1")
        ...
        # Replicate the exact logic from cli.py JackyCLI.__init__:
...
class TestCmdChatWiring:
    """The wiring inside ``cmd_chat()`` in ``jacky_cli/main.py`` must set
    ...
    def test_both_flags_set_both_env_vars(self, monkeypatch):
        monkeypatch.delenv("JACKY_IGNORE_USER_CONFIG", raising=False)
        monkeypatch.delenv("JACKY_IGNORE_RULES", raising=False)
        ...
        assert os.environ.get("JACKY_IGNORE_USER_CONFIG") == "1"
        assert os.environ.get("JACKY_IGNORE_RULES") == "1"
    ...
    def test_only_ignore_user_config(self, monkeypatch):
        ...
        assert os.environ.get("JACKY_IGNORE_USER_CONFIG") == "1"
        assert "JACKY_IGNORE_RULES" not in os.environ
    ...
    def test_flags_absent_sets_nothing(self, monkeypatch):
        ...
        assert "JACKY_IGNORE_USER_CONFIG" not in os.environ
        assert "JACKY_IGNORE_RULES" not in os.environ
...
class TestArgparseFlagsRegistered:
    ...
    def test_main_py_registers_both_flags(self):
        """E2E: the real jacky parser accepts both flags."""
        ...
        import inspect
        import jacky_cli.main as hm
        src = inspect.getsource(hm)
        assert "JACKY_IGNORE_USER_CONFIG" in src
        assert "JACKY_IGNORE_RULES" in src

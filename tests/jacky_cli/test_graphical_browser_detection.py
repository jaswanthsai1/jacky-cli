"""Tests for `_can_open_graphical_browser()` in jacky_cli.auth.

Guards the fix for the May 2026 report where `jacky auth add xai-oauth`
...
from jacky_cli.auth import _can_open_graphical_browser
...
def _force_platform_linux(monkeypatch):
    monkeypatch.setattr("jacky_cli.auth.sys.platform", "linux")
...
def test_non_linux_with_gui_allows(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.auth.sys.platform", "darwin")

"""Tests for the Kanban DB layer (jacky_cli.kanban_db)."""
...
def kanban_home(tmp_path, monkeypatch):
    """Isolated JACKY_HOME with an empty kanban DB."""
    home = tmp_path / ".jacky"
    home.mkdir()
...
def test_connect_honors_kanban_busy_timeout_env(kanban_home, monkeypatch):
    ...
    """
    monkeypatch.setenv("JACKY_KANBAN_BUSY_TIMEOUT_MS", "123456")
...
def test_connect_rejects_tls_record_in_sqlite_header(tmp_path, monkeypatch):
    ...
    home.mkdir()
    monkeypatch.setenv("JACKY_HOME", str(home))
    monkeypatch.delenv("JACKY_KANBAN_DB", raising=False)
    monkeypatch.delenv("JACKY_KANBAN_HOME", raising=False)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
def test_claim_uses_env_default_ttl(kanban_home, monkeypatch):
    monkeypatch.setenv("JACKY_KANBAN_CLAIM_TTL_SECONDS", "3600")
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x", assignee="a")
...
def test_stale_claim_reclaimed(kanban_home, monkeypatch):
    import signal
    import jacky_cli.kanban_db as _kb
...
def test_detect_crashed_workers_skips_freshly_claimed_tasks(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.delenv("JACKY_KANBAN_CRASH_GRACE_SECONDS", raising=False)
...
def test_detect_crashed_workers_grace_period_env_override(
    ...
    """JACKY_KANBAN_CRASH_GRACE_SECONDS env var adjusts the window."""
    import jacky_cli.kanban_db as _kb

    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("JACKY_KANBAN_CRASH_GRACE_SECONDS", "5")
...
def test_resolve_crash_grace_seconds_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv("JACKY_KANBAN_CRASH_GRACE_SECONDS", bad_val)
...
def test_rate_limit_exit_requeues_without_counting_failure(
    ...
    monkeypatch.setattr(_kb, "_pid_alive", lambda _pid: False)
    monkeypatch.setenv("JACKY_KANBAN_CRASH_GRACE_SECONDS", "0")
...
def test_respawn_guard_defers_rate_limited_within_cooldown(
    ...
    monkeypatch.setenv("JACKY_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "300")
    now = 5_000_000
...
def test_respawn_guard_rate_limit_cooldown_zero_allows_immediately(
    ...
    monkeypatch.setenv("JACKY_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", "0")
    now = 6_000_000
...
def test_resolve_rate_limit_cooldown_handles_bad_env(monkeypatch):
    ...
    for bad_val in ("notanumber", "-5", ""):
        monkeypatch.setenv(
            "JACKY_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", bad_val
        )
        assert (
...
def test_has_spawnable_ready_true_when_real_profile_present(kanban_home, monkeypatch):
    ...
    has an assignee that maps to a real Jacky profile — preserves the
    ...
    with kb.connect() as conn:
        ...
        kb.create_task(conn, title="jacky-task", assignee="daily")
...
def test_scratch_workspace_created_under_jacky_home(kanban_home):
    with kb.connect() as conn:
        t = kb.create_task(conn, title="x")
        task = kb.get_task(conn, t)
        assert task is not None
...
def test_dispatch_worktree_task_persists_materialized_workspace_and_branch(kanban_home, tmp_path, monkeypatch):
    ...
    import jacky_cli.profiles as profiles
...
def test_cleanup_workspace_removes_managed_scratch_dir(kanban_home):
    ...
    with kb.connect() as conn:
        ...
        assert ws.is_dir()
        kb.complete_task(conn, t, result="ok")
    assert not ws.exists(), "Jacky-managed scratch dir should be cleaned up"
...
def test_cleanup_workspace_honors_workspaces_root_env_override(tmp_path, monkeypatch):
    """``JACKY_KANBAN_WORKSPACES_ROOT`` extends the managed-scratch set.
    ...
    monkeypatch.setenv("JACKY_KANBAN_WORKSPACES_ROOT", str(workspaces_override))
    kb.init_db()
...
def test_is_managed_scratch_path_rejects_kanban_metadata_subtrees(kanban_home):
    """Jacky' own DB/metadata/log subtrees under ``<kanban_home>/kanban`` are NOT managed.
    ...
    ``shutil.rmtree`` Jacky' DB/metadata/logs on task completion.
    """
...
# spawned with `jacky -p <profile>` must read/write the same kanban.db
...
# where `kanban_db_path()` resolved to the active profile's JACKY_HOME.
# ---------------------------------------------------------------------------

class TestSharedBoardPaths:
    """`kanban_home`/`kanban_db_path`/`workspaces_root`/`worker_log_path`
    must anchor at the **shared root**, not the active profile's JACKY_HOME."""

    def _set_home(self, monkeypatch, tmp_path, jacky_home):
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        monkeypatch.delenv("JACKY_KANBAN_HOME", raising=False)

    def test_default_install_anchors_at_home_dot_jacky(
        self, tmp_path, monkeypatch
    ):
        # Standard install: JACKY_HOME == ~/.jacky, no profile active.
        default_home = tmp_path / ".jacky"
        default_home.mkdir()
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.jacky/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.jacky/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.jacky/kanban.db.
        default_home = tmp_path / ".jacky"
        default_home.mkdir()
    ...
    def test_dispatcher_and_profile_worker_converge(
        ...
        # JACKY_HOME and confirm equality. This is the property the
        ...
        # Worker's perspective (profile activated by `jacky -p coder`).
    ...
    def test_docker_custom_jacky_home_uses_env_path_directly(
        self, tmp_path, monkeypatch
    ):
        # Docker / custom deployment: JACKY_HOME points outside ~/.jacky.
        # `get_default_jacky_root()` returns env_home directly when it
        # is not a `<root>/profiles/<name>` shape and not under
        # `Path.home() / ".jacky"`.
        custom_root = tmp_path / "opt" / "jacky"
    ...
    def test_docker_profile_layout_uses_grandparent(
        ...
        # Docker profile shape: JACKY_HOME=/opt/jacky/profiles/coder;
        # `get_default_jacky_root()` walks up to /opt/jacky because
    ...
    def test_explicit_override_via_jacky_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # Explicit override: JACKY_KANBAN_HOME beats every other
        # resolution rule.
        default_home = tmp_path / ".jacky"
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(profile_home))
        monkeypatch.setenv("JACKY_KANBAN_HOME", str(override))
    ...
    def test_empty_override_falls_through(self, tmp_path, monkeypatch):
        ...
        default_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(default_home))
        monkeypatch.setenv("JACKY_KANBAN_HOME", "   ")
    ...
    def test_dispatcher_and_worker_share_a_real_database(
        ...
        # JACKY_HOME perspectives via a real SQLite file. Without the
        ...
        # Worker switches to the profile JACKY_HOME and reads.
        monkeypatch.setenv("JACKY_HOME", str(profile_home))
        with kb.connect() as conn:
    ...
    def test_jacky_kanban_db_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # JACKY_KANBAN_DB pins the file path directly and beats both
        # JACKY_KANBAN_HOME and the `get_default_jacky_root()` path.
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(default_home))
        monkeypatch.setenv("JACKY_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("JACKY_KANBAN_DB", str(pinned_db))

        assert kb.kanban_db_path() == pinned_db
        # workspaces_root still follows JACKY_KANBAN_HOME -- the pins
        # are independent.
        assert kb.workspaces_root() == umbrella / "kanban" / "workspaces"

    def test_jacky_kanban_workspaces_root_pin_beats_kanban_home(
        self, tmp_path, monkeypatch
    ):
        # JACKY_KANBAN_WORKSPACES_ROOT pins the workspaces root directly.
        default_home = tmp_path / ".jacky"
        default_home.mkdir()
        umbrella = tmp_path / "umbrella"
        umbrella.mkdir()
        ...
        monkeypatch.setenv("JACKY_HOME", str(default_home))
        monkeypatch.setenv("JACKY_KANBAN_HOME", str(umbrella))
        monkeypatch.setenv("JACKY_KANBAN_WORKSPACES_ROOT", str(pinned_ws))

        assert kb.workspaces_root() == pinned_ws
        # kanban_db_path still follows JACKY_KANBAN_HOME.
    ...
    def test_empty_per_path_overrides_fall_through(
        ...
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(default_home))
        monkeypatch.setenv("JACKY_KANBAN_DB", "   ")
        monkeypatch.setenv("JACKY_KANBAN_WORKSPACES_ROOT", "")
    ...
    def test_dispatcher_spawn_injects_kanban_db_and_workspaces_root(
        ...
        # The dispatcher's `_default_spawn` must inject JACKY_KANBAN_DB
        # and JACKY_KANBAN_WORKSPACES_ROOT into the worker env so the
        # worker converges on the dispatcher's paths even when the
        # `-p <profile>` flag rewrites JACKY_HOME.
        default_home = tmp_path / ".jacky"
        default_home.mkdir()
        self._set_home(monkeypatch, tmp_path, default_home)
        ...
        env = captured["env"]
        assert env["JACKY_KANBAN_DB"] == str(default_home / "kanban.db")
        assert env["JACKY_KANBAN_WORKSPACES_ROOT"] == str(
            default_home / "kanban" / "workspaces"
        )
        assert env["JACKY_KANBAN_TASK"] == "t_dispatch_env"
        assert env["JACKY_KANBAN_BRANCH"] == "wt/t_dispatch_env"
...
# ---------------------------------------------------------------------------
# NFS / network-filesystem fallback (see jacky_state.apply_wal_with_fallback)
# ---------------------------------------------------------------------------
...
def test_unlink_tasks_triggers_recompute_ready(kanban_home):
    ...
    next dispatcher tick or a manual 'jacky kanban recompute' would promote it.
    """
    with kb.connect() as conn:
        # A is done.
...
# ---------------------------------------------------------------------------
# Dispatcher spawn invocation — _resolve_jacky_argv()
#
# Workers spawned by the dispatcher must use a `jacky` invocation that does
...
# $PATH that doesn't include the venv's bin/, so a bare `["jacky", ...]`
...
def test_resolve_jacky_argv_prefers_path_shim(monkeypatch):
    """When `jacky` is on PATH, use the shim — preserves familiar ps output."""
    import shutil
    import jacky_cli.kanban_db as kb

    monkeypatch.delenv("JACKY_BIN", raising=False)
    monkeypatch.setattr(shutil, "which", lambda name: "/usr/local/bin/jacky")
    argv = kb._resolve_jacky_argv()
    assert argv == ["/usr/local/bin/jacky"]


def test_resolve_jacky_argv_absolutizes_relative_exe_shim(monkeypatch, tmp_path):
    """A relative executable override must not remain workspace-cwd-dependent."""
    import jacky_cli.kanban_db as kb

    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("JACKY_BIN", ".\\jacky.exe")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_jacky_argv() == [os.path.abspath(".\\jacky.exe")]


def test_resolve_jacky_argv_avoids_implicit_windows_batch_shim(monkeypatch, tmp_path):
    ...
    bin_dir.mkdir()
    (bin_dir / "jacky.CMD").write_text("@echo off\n", encoding="utf-8")
    monkeypatch.delenv("JACKY_BIN", raising=False)
    monkeypatch.setenv("PATH", str(bin_dir))
    monkeypatch.setenv("PATHEXT", ".CMD")
    monkeypatch.setattr(kb, "_IS_WINDOWS", True)

    assert kb._resolve_jacky_argv() == [sys.executable, "-m", "jacky_cli.main"]


def test_resolve_jacky_argv_honors_jacky_bin_path_override(monkeypatch, tmp_path):
    """An explicit path-like JACKY_BIN lets service managers pin the executable."""
    import shutil
    import jacky_cli.kanban_db as kb

    shim = tmp_path / "bin" / "jacky"
    shim.parent.mkdir()
    shim.write_text("#!/bin/sh\n", encoding="utf-8")
    monkeypatch.setenv("JACKY_BIN", str(shim))
    monkeypatch.setattr(shutil, "which", lambda name: None)

    assert kb._resolve_jacky_argv() == [str(shim)]


def test_resolve_jacky_argv_jacky_bin_bare_name_uses_path(monkeypatch, tmp_path):
    """Bare JACKY_BIN values keep PATH semantics instead of cwd shadowing."""
    import stat
    import jacky_cli.kanban_db as kb

    cwd_jacky = tmp_path / "jacky"
    cwd_jacky.write_text("wrong\n", encoding="utf-8")
    cwd_jacky.chmod(cwd_jacky.stat().st_mode | stat.S_IXUSR)
    path_jacky = tmp_path / "bin" / "jacky"
    path_jacky.parent.mkdir()
    path_jacky.write_text("right\n", encoding="utf-8")
    path_jacky.chmod(path_jacky.stat().st_mode | stat.S_IXUSR)
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("PATH", str(path_jacky.parent))
    monkeypatch.setenv("JACKY_BIN", "jacky")

    assert kb._resolve_jacky_argv() == [str(path_jacky)]


def test_resolve_jacky_argv_jacky_bin_bare_name_ignores_cwd(monkeypatch, tmp_path):
    """Bare JACKY_BIN does not accept current-directory shadow executables."""
    import sys
    import jacky_cli.kanban_db as kb

    (tmp_path / "jacky.exe").write_text("wrong\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
...
def test_resolve_jacky_argv_jacky_bin_bare_cmd_uses_module_fallback(monkeypatch, tmp_path):
    """A PATH-resolved JACKY_BIN batch shim is not used as worker argv[0]."""
    import sys
    import jacky_cli.kanban_db as kb
...
def test_resolve_jacky_argv_jacky_bin_unresolved_bare_name_falls_back(monkeypatch):
    """Unresolved JACKY_BIN command names do not delegate cwd search to Popen."""
    import sys
    import jacky_cli.kanban_db as kb
...
def test_resolve_jacky_argv_falls_back_to_module_form_when_no_path_shim(monkeypatch):
    """When the shim is not on PATH, fall back to `python -m jacky_cli.main`.

    Pins the correct module name (NOT `jacky` — there is no top-level
    `jacky` package). Regression for #23198: the original PR shipped
    `python -m jacky` which fails with `No module named jacky` on every
    invocation.
    """
    import shutil
    import sys
    ...
    monkeypatch.setattr(shutil, "which", lambda name: None)
    argv = kb._resolve_jacky_argv()
    assert argv == [sys.executable, "-m", "jacky_cli.main"]


def test_resolve_jacky_argv_module_actually_runs():
    """The fallback module name must be importable + runnable.

    A unit test that pins the literal string is necessary but not
    sufficient — if `jacky_cli.main` ever loses `if __name__ == "__main__"`
    handling or its argparse setup, `python -m jacky_cli.main --version`
    ...
    assert "Jacky Agent" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
...
def test_maybe_emit_scratch_tip_fires_once_per_install(kanban_home, caplog):
    ...
    with caplog.at_level(logging.WARNING, logger="jacky_cli.kanban_db"):
        with kb.connect() as conn:
...
def test_write_txn_healthy_commit_no_exception(tmp_path):
    ...
    from jacky_cli.kanban_db import connect, write_txn
    db = tmp_path / "test.db"
...
def test_write_txn_raises_on_truncated_file(tmp_path):
    ...
    with pytest.raises(sqlite3.DatabaseError, match="torn-extend|page count mismatch"):
        with unittest.mock.patch("jacky_cli.kanban_db.os.path.getsize", side_effect=fake_getsize):
            with write_txn(conn) as c:
                c.execute(
                    "INSERT INTO tasks (id, title, assignee, status, priority, created_at) "
...
def test_write_txn_post_commit_check_fires_every_call(tmp_path):
    ...
    from jacky_cli.kanban_db import connect, write_txn
    import jacky_cli.kanban_db as kanban_db_module
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
    call_count = 0
...
def test_connect_sets_wal_autocheckpoint_100(tmp_path):
    ...
    from jacky_cli.kanban_db import connect
    db = tmp_path / "test.db"
    conn = connect(db_path=db)
...
def test_write_txn_check_reads_correct_header_fields(tmp_path):
    ...
    import struct
    from jacky_cli.kanban_db import connect, _check_file_length_invariant
...
def test_reap_worker_zombies_returns_count():
    ...
    with patch("jacky_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("jacky_cli.kanban_db._record_worker_exit"):
...
def test_reap_worker_zombies_noop_on_windows(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.kanban_db.os.name", "nt")
    with patch("jacky_cli.kanban_db.os.waitpid") as mock_waitpid:
...
def test_reap_worker_zombies_noop_no_children():
    ...
    with patch("jacky_cli.kanban_db.os.waitpid", side_effect=ChildProcessError):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_reap_worker_zombies_records_exit_status():
    ...
    with patch("jacky_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch(
            "jacky_cli.kanban_db._record_worker_exit",
...
def test_reap_worker_zombies_handles_waitpid_os_error():
    ...
    with patch("jacky_cli.kanban_db.os.waitpid", side_effect=OSError("test error")):
        result = kb.reap_worker_zombies()
    assert result == []
...
def test_zombie_reaper_survives_all_boards_failing():
    ...
    for tick in range(5):
        ...
        with patch(
            "jacky_cli.kanban_db.os.waitpid", side_effect=make_fake_waitpid(pids)
        ):
            with patch("jacky_cli.kanban_db._record_worker_exit"):
                pids = kb.reap_worker_zombies()
...
def test_dispatch_once_still_reaps_via_extracted_fn(kanban_home):
    ...
    with patch("jacky_cli.kanban_db.os.waitpid", side_effect=fake_waitpid):
        with patch("jacky_cli.kanban_db._record_worker_exit"):
            with patch("jacky_cli.kanban_db.os.name", "posix"):
                pids = kb.reap_worker_zombies()

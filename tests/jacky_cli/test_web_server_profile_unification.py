...
profile switcher can target any profile's JACKY_HOME. These tests pin:
reads/writes land in the REQUESTED profile, the dashboard's own profile
stays untouched, and the chat PTY env is scoped via JACKY_HOME.
"""
import pytest
import yaml
...
def isolated_profiles(tmp_path, monkeypatch, _isolate_jacky_home):
    ...
    monkeypatch.setattr(profiles, "_get_default_jacky_home", lambda: default_home)
...
class TestProfileScopedPostSetup:
    def test_post_setup_spawns_with_profile_flag(
        ...
        config / write per-profile state see the same JACKY_HOME the rest
        ...
        monkeypatch.setattr(
            "jacky_cli.tools_config.valid_post_setup_keys",
...
class TestProfileScopedGateway:
    ...
    def test_status_reads_requested_profile_home(
        ...
        def fake_get_running_pid():
            seen_homes.append(str(get_jacky_home()))
            return None
        ...
        assert resp.json()["jacky_home"] == str(isolated_profiles["worker_beta"])
...
class TestProfileScopedChatPty:
    def test_chat_argv_scopes_jacky_home(self, isolated_profiles, monkeypatch):
        import jacky_cli.web_server as web_server

        monkeypatch.setattr(
            "jacky_cli.main._make_tui_argv",
            lambda root, tui_dev=False: (["cat"], None),
            raising=False,
        )
        argv, cwd, env = web_server._resolve_chat_argv(profile="worker_beta")
        assert env is not None
        assert env["JACKY_HOME"] == str(isolated_profiles["worker_beta"])
        # Scoped chat must NOT attach to the dashboard's in-memory gateway.
        assert "JACKY_TUI_GATEWAY_URL" not in env
    ...
    def test_chat_argv_unscoped_keeps_legacy_env(self, isolated_profiles, monkeypatch):
        ...
        assert env is not None
        assert env.get("JACKY_HOME") != str(isolated_profiles["worker_beta"])

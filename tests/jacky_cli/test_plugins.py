"""Tests for the Jacky plugin system (jacky_cli.plugins)."""
...
from jacky_cli.middleware import (
    VALID_MIDDLEWARE,
...
def _make_plugin_dir(base: Path, name: str, *, register_body: str = "pass",
    ...
    ``<jacky_home>/config.yaml`` under ``plugins.enabled``. Plugins are
    ...
    *base* is expected to be ``<jacky_home>/plugins/``; we derive
    ``<jacky_home>`` from it by walking one level up.
    """
    ...
    if auto_enable:
        # Write/merge plugins.enabled in <JACKY_HOME>/config.yaml.
        # Config is always read from JACKY_HOME (not from the project
        # dir for project plugins), so that's where we opt in.
        import os
        jacky_home_str = os.environ.get("JACKY_HOME")
        if jacky_home_str:
            jacky_home = Path(jacky_home_str)
        else:
            jacky_home = base.parent
        jacky_home.mkdir(parents=True, exist_ok=True)
        cfg_path = jacky_home / "config.yaml"
        cfg: dict = {}
        if cfg_path.exists():
            try:
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.jacky/plugins/ are discovered."""
        plugins_dir = tmp_path / "jacky_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("JACKY_HOME", str(tmp_path / "jacky_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".jacky" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_missing_init(self, tmp_path, monkeypatch):
        ...
        # missing-init error.
        jacky_home = tmp_path / "jacky_test"
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under jacky_plugins.<name>."""
        plugins_dir = tmp_path / "jacky_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("JACKY_HOME", str(tmp_path / "jacky_test"))

        # Clean up any prior namespace module
        sys.modules.pop("jacky_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "jacky_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="jacky_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPreToolCallDirective:
    ...
    def test_approve_directive_returned(self, monkeypatch):
        from jacky_cli.plugins import get_pre_tool_call_directive
        monkeypatch.setattr(
            "jacky_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: [
...
class TestResolvePreToolBlock:
    ...
    def test_block_returns_message(self, monkeypatch):
        from jacky_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "jacky_cli.plugins.invoke_hook",
    ...
    def test_no_directive_returns_none(self, monkeypatch):
        from jacky_cli.plugins import resolve_pre_tool_block
        monkeypatch.setattr(
            "jacky_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
...
class TestGetPreVerifyContinueMessage:
    ...
    def test_none_when_no_hooks(self, monkeypatch):
        monkeypatch.setattr("jacky_cli.plugins.invoke_hook", lambda hook_name, **kwargs: [])
        assert get_pre_verify_continue_message() is None
    ...
    def test_forwards_scope_signals_to_hooks(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.plugins.invoke_hook", capture)
...
class TestPluginContext:
    ...
    def test_register_tool_override_blocked_without_operator_opt_in(self, tmp_path, monkeypatch):
        ...
        """
        from tools.registry import registry
        from jacky_cli.plugins import PluginToolOverrideError
        ...
        try:
            ...
            from jacky_cli.plugins import PluginContext, PluginManifest
    ...
    def test_register_tool_override_blocked_via_delayed_callback(self, tmp_path, monkeypatch):
        ...
        try:
            ...
            import sys as _sys
            mod = _sys.modules.get("jacky_plugins.delayed_override_plugin")
...
class TestPluginManagerList:
    ...
    def test_shared_hook_name_credited_to_every_plugin(self, tmp_path, monkeypatch):
        ...
        `jacky plugins list`. Attribution now counts what each plugin's own
...
class TestPluginCommands:
    ...
    def test_get_plugin_command_handler_found(self):
        ...
        with patch("jacky_cli.plugins._plugin_manager", mgr):
    ...
    def test_get_plugin_context_engine_discovers_plugins_lazily(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / "jacky_test"
        plugins_dir = jacky_home / "plugins"
    ...
    def test_commands_in_list_plugins_output(self, tmp_path, monkeypatch):
        ...
        plugins_dir = tmp_path / "jacky_test" / "plugins"
        # Set JACKY_HOME BEFORE _make_plugin_dir so auto-enable targets
...
class TestPluginCommandResultResolution:
    ...
    def test_awaits_async_result_with_running_loop(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
    ...
    def test_running_loop_timeout_does_not_hang_forever(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.plugins.asyncio.get_running_loop", lambda: _Loop())
        monkeypatch.setattr("jacky_cli.plugins._PLUGIN_COMMAND_AWAIT_TIMEOUT_SECS", 0.1)
...
class TestPluginDispatchTool:
    ...
    def test_dispatch_tool_calls_registry(self):
        ...
        with patch("jacky_cli.plugins.PluginContext.dispatch_tool.__module__", "jacky_cli.plugins"):
...
class TestPluginDebugLogging:
    """JACKY_PLUGINS_DEBUG opt-in stderr handler for plugin developers."""
    ...
    def test_debug_handler_not_installed_when_env_var_absent(self, monkeypatch):
        ...
        monkeypatch.delenv("JACKY_PLUGINS_DEBUG", raising=False)
        from jacky_cli import plugins as plugins_mod
    ...
    def test_debug_handler_installed_when_env_var_set(self, monkeypatch):
        """With JACKY_PLUGINS_DEBUG=1, a DEBUG-level stderr handler is attached."""
        monkeypatch.setenv("JACKY_PLUGINS_DEBUG", "1")
        from jacky_cli import plugins as plugins_mod
...
class TestPluginContextProfileName:
    """ctx.profile_name resolves from JACKY_HOME in every context."""
    ...
    def test_default_profile(self, tmp_path, monkeypatch):
        """JACKY_HOME at the root resolves to 'default'."""
        home = tmp_path / ".jacky"
        home.mkdir()
    ...
    def test_named_profile(self, tmp_path, monkeypatch):
        """JACKY_HOME under profiles/<name> resolves to that name."""
        prof = tmp_path / ".jacky" / "profiles" / "coder"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(prof))
    ...
    def test_works_without_cli_ref(self, tmp_path, monkeypatch):
        ...
        prof = tmp_path / ".jacky" / "profiles" / "worker1"
        prof.mkdir(parents=True)
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(prof))
        ctx = self._ctx()

"""Tests for the kanban CLI surface (jacky_cli.kanban)."""
...
from jacky_cli import kanban as kc
...
def test_run_slash_session_filter(kanban_home):
    """`jacky kanban list --session <id>` filters by the originating
...
def test_board_override_is_isolated_per_concurrent_call(kanban_home, monkeypatch):
    ...
    parser = argparse.ArgumentParser(prog="jacky", add_help=False)
...
def test_kanban_in_autocomplete_table():
    from jacky_cli.commands import COMMANDS, SUBCOMMANDS
...
def test_run_slash_board_override_restores_prior_env(kanban_home, monkeypatch):
    kb.create_board("alpha")
    kb.create_board("beta")
    monkeypatch.setenv("JACKY_KANBAN_BOARD", "beta")

    kc.run_slash("--board alpha list")

    assert os.environ.get("JACKY_KANBAN_BOARD") == "beta"

"""Tests for IRC gateway configuration via `jacky setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Jacky install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Jacky install."""
    ...
    def test_irc_status_configured_when_env_set(self, monkeypatch):
        ...
        try:
            ...
            monkeypatch.setenv("IRC_CHANNEL", "#jacky")
            monkeypatch.setenv("IRC_NICKNAME", "jacky-bot")
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `jacky setup gateway` experience with IRC present."""

...
subparser, placing the flag BEFORE the subcommand (e.g. 'jacky --yolo chat')
...
def _build_parser():
    """Build the jacky argument parser from the real code.
...
class TestChatVerboseArg:
    ...
    def test_cmd_chat_forwards_none_when_verbose_is_absent(self, monkeypatch):
        ...
        fake_banner = types.ModuleType("jacky_cli.banner")
        ...
        monkeypatch.setitem(sys.modules, "jacky_cli.banner", fake_banner)
...
class TestYoloEnvVar:
    """Verify --yolo sets JACKY_YOLO_MODE regardless of flag position.
...
class TestAcceptHooksOnAgentSubparsers:
    ...
    parser and `chat`, so `jacky gateway run --accept-hooks` failed
    ...
    # One driver subprocess parses ALL argvs: jacky_cli.main is a very heavy
    # import (previously 11 separate `python -m jacky_cli.main` spawns with a
...
"""
    ...
    def test_accepted_at_every_position(self):
        """Every `jacky <argv>` must exit 0 (help) rather than failing
        with `unrecognized arguments`."""
        import json
        import subprocess
...
class TestChatSubparserInheritedValueFlags:
    ...
    Regression test for #28780: `jacky -t web chat` silently dropped the
    ...
    Uses the real `jacky_cli._parser.build_top_level_parser()` rather than
    ...
    def test_flag_before_chat_is_preserved(self, real_parser, flag, attr, value):
        ...
        assert getattr(args, attr, None) == value, (
            f"`jacky {flag} {value} chat` lost the flag — got "
    ...
    def test_store_true_flag_before_chat_is_preserved(
        ...
        would clobber the parent's `True` when used as `jacky --tui chat`."""
        args, _ = real_parser.parse_known_args([flag, "chat"])
        assert getattr(args, attr, None) is True, (
            f"`jacky {flag} chat` lost the flag — got "
    ...
    def test_chat_subparser_inherited_value_flags_use_suppress(self):
        ...
        assert not offenders, (
            ...
            "top-level value when used as `jacky <flag> <value> chat`:\n  "

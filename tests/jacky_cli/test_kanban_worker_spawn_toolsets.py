...
def test_default_spawn_pins_assignee_profile_cli_toolsets(monkeypatch, tmp_path):
    ...
    JACKY_KANBAN_TASK: the worker must not collapse to only kanban lifecycle
...
""".lstrip(),
    ...
    monkeypatch.setattr(kb, "_resolve_jacky_argv", lambda: ["jacky"])
    ...
    assert pid == 4242
    assert captured["env"]["JACKY_HOME"] == str(profile)
    assert captured["env"]["JACKY_KANBAN_TASK"] == "t_spawn_tools"
...
def test_default_spawn_never_boots_the_tui(monkeypatch, tmp_path):
    """Workers are headless: an inherited JACKY_TUI=1 (or a TUI-default
    ...
    flag) and strips JACKY_TUI from the child env."""
    root = tmp_path / ".jacky"
    ...
    assert "JACKY_TUI" not in captured["env"]

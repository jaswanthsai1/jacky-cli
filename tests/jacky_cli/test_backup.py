"""Tests for jacky backup and import commands."""
...
def _make_jacky_tree(root: Path) -> None:
    """Create a realistic ~/.jacky directory structure for testing."""
    (root / "config.yaml").write_text("model:\n  provider: openrouter\n")
    (root / ".env").write_text("OPENROUTER_API_KEY=sk-test-123\n")
    (root / "memory_store.db").write_bytes(b"fake-sqlite")
    (root / "jacky_state.db").write_bytes(b"fake-state")
    ...
    # jacky-agent repo (should be EXCLUDED)
    (root / "jacky-agent").mkdir(exist_ok=True)
    (root / "jacky-agent" / "run_agent.py").write_text("# big file\n")
    (root / "jacky-agent" / ".git").mkdir()
    (root / "jacky-agent" / ".git" / "HEAD").write_text("ref: refs/heads/main\n")
...
class TestShouldExclude:
    def test_excludes_jacky_agent(self):
        from jacky_cli.backup import _should_exclude
        assert _should_exclude(Path("jacky-agent/run_agent.py"))
        assert _should_exclude(Path("jacky-agent/.git/HEAD"))
    ...
    def test_includes_nested_jacky_agent_in_skills(self):
        """skills/autonomous-ai-agents/jacky-agent/ must NOT be excluded —
        only the root-level jacky-agent/ repo is skipped."""
        from jacky_cli.backup import _should_exclude
        assert not _should_exclude(Path("skills/autonomous-ai-agents/jacky-agent/SKILL.md"))
        assert not _should_exclude(Path("skills/autonomous-ai-agents/jacky-agent/sub/item.txt"))
    ...
    def test_excludes_regeneratable_dependency_and_cache_dirs(self, rel):
        """Python dep trees and tool caches under JACKY_HOME must be skipped —
...
class TestBackup:
    def test_creates_zip(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        _make_jacky_tree(jacky_home)

        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        # get_default_jacky_root needs this
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import jacky_cli.backup as backup_mod
        staged_dirs = []
    ...
    def test_pre_update_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        out_zip = jacky_home / "backups" / "pre-update-test.zip"
        ...
        monkeypatch.setattr(backup_mod.tempfile, "NamedTemporaryFile", _spy)
        result = backup_mod._write_full_zip_backup(out_zip, jacky_home)

        assert result is not None
        assert staged_dirs, "no SQLite snapshot was staged"
        assert all(d == str(out_zip.parent) for d in staged_dirs), staged_dirs

    def test_excludes_jacky_agent(self, tmp_path, monkeypatch):
        """Backup does NOT include jacky-agent/ directory."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        _make_jacky_tree(jacky_home)
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            agent_files = [n for n in names if "jacky-agent" in n]
            assert agent_files == [], f"jacky-agent files leaked into backup: {agent_files}"

    def test_excludes_dependency_and_cache_trees(self, tmp_path, monkeypatch):
        """A plugin venv / site-packages / pip cache under JACKY_HOME must be
        ...
        venv_pkg = jacky_home / "plugins" / "heavy" / ".venv" / "lib" / "site-packages" / "dep"
        venv_pkg.mkdir(parents=True)
        (venv_pkg / "__init__.py").write_text("# dep\n")
        pip_cache = jacky_home / ".cache" / "uv" / "wheels"
    ...
    def test_includes_nested_jacky_agent_in_skills(self, tmp_path, monkeypatch):
        """Backup includes skills/.../jacky-agent/ but NOT root jacky-agent/."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        _make_jacky_tree(jacky_home)

        # Add a nested jacky-agent directory inside skills (like the real layout)
        nested = jacky_home / "skills" / "autonomous-ai-agents" / "jacky-agent"
        nested.mkdir(parents=True)
        (nested / "SKILL.md").write_text("# Jacky Agent Skill\n")
        (nested / "sub").mkdir()
        ...
        with zipfile.ZipFile(out_zip, "r") as zf:
            names = zf.namelist()
            # Root jacky-agent must be excluded
            root_agent = [n for n in names if n.startswith("jacky-agent/")]
            assert root_agent == [], f"root jacky-agent leaked: {root_agent}"
            # Nested skill jacky-agent must be included
            assert "skills/autonomous-ai-agents/jacky-agent/SKILL.md" in names
            assert "skills/autonomous-ai-agents/jacky-agent/sub/item.txt" in names
    ...
    def test_default_output_path(self, tmp_path, monkeypatch):
        """When no output path given, zip goes to ~/jacky-backup-*.zip."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / "config.yaml").write_text("model: test\n")
        ...
        zips = list(tmp_path.glob("jacky-backup-*.zip"))
        assert len(zips) == 1

    def test_skips_symlinked_files(self, tmp_path, monkeypatch):
        """Backup must not dereference symlinks and leak files outside JACKY_HOME."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        _make_jacky_tree(jacky_home)
        outside = tmp_path / "outside-secret.txt"
        outside.write_text("outside secret\n")
        _symlink_file_or_skip(jacky_home / "skills" / "outside-link.txt", outside)
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Jacky backup."""
        from jacky_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        """A zip with only jacky_state.db (old wrong name) is rejected."""
        from jacky_cli.backup import _validate_backup_zip
        zip_path = tmp_path / "old.zip"
        self._make_zip(zip_path, ["jacky_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestImport:
    ...
    def test_restores_files(self, tmp_path, monkeypatch):
        """Import extracts files into jacky home."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (jacky_home / "config.yaml").read_text() == "model:\n  provider: openrouter\n"
        assert (jacky_home / ".env").read_text() == "OPENROUTER_API_KEY=sk-test\n"
        assert (jacky_home / "skills" / "my-skill" / "SKILL.md").read_text() == "# My Skill\n"
        assert (jacky_home / "profiles" / "coder" / "config.yaml").exists()

    def test_strips_jacky_prefix(self, tmp_path, monkeypatch):
        """Import strips .jacky/ prefix if all entries share it."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        zip_path = tmp_path / "backup.zip"
        self._make_backup_zip(zip_path, {
            ".jacky/config.yaml": "model: test\n",
            ".jacky/skills/a/SKILL.md": "# A\n",
        })
        ...
        assert (jacky_home / "config.yaml").read_text() == "model: test\n"
        assert (jacky_home / "skills" / "a" / "SKILL.md").read_text() == "# A\n"
    ...
    def test_rejects_non_jacky_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a jacky backup."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_blocks_path_traversal(self, tmp_path, monkeypatch):
        ...
        assert (jacky_home / "config.yaml").exists()
        # traversal file should NOT exist outside jacky home
    ...
    def test_preserves_live_gateway_state(self, tmp_path, monkeypatch):
        ...
        (jacky_home / "gateway_state.json").write_text(live_state)
        ...
        assert (jacky_home / "gateway_state.json").read_text() == live_state
    ...
    def test_does_not_seed_gateway_state_when_absent(self, tmp_path, monkeypatch):
        ...
        assert (jacky_home / "config.yaml").exists()
        assert not (jacky_home / "gateway_state.json").exists()
    ...
    def test_preserves_per_profile_gateway_state(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        (jacky_home / "profiles" / "coder").mkdir(parents=True)
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)

        live_state = '{"gateway_state": "running"}'
        (jacky_home / "profiles" / "coder" / "gateway_state.json").write_text(live_state)
        ...
        assert (jacky_home / "profiles" / "coder" / "config.yaml").read_text() == "model: anthropic\n"
        assert (
            jacky_home / "profiles" / "coder" / "gateway_state.json"
    ...
    def test_preserves_runtime_pid_and_process_files(self, tmp_path, monkeypatch):
        ...
        (jacky_home / "gateway.pid").write_text("4242")
        (jacky_home / "processes.json").write_text('{"live": true}')
        ...
        assert (jacky_home / "gateway.pid").read_text() == "4242"
        assert (jacky_home / "processes.json").read_text() == '{"live": true}'
        # cron.pid / gateway.lock had no live copy and were not seeded.
        assert not (jacky_home / "cron.pid").exists()
        assert not (jacky_home / "gateway.lock").exists()
    ...
    def test_confirmation_prompt_abort(self, tmp_path, monkeypatch):
        ...
        (jacky_home / "config.yaml").write_text("existing: true\n")
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        ...
        assert (jacky_home / "config.yaml").read_text() == "existing: true\n"
    ...
    def test_force_skips_confirmation(self, tmp_path, monkeypatch):
        ...
        assert (jacky_home / "config.yaml").read_text() == "model: restored\n"
    ...
    def test_restores_secret_files_with_0600_perms(self, tmp_path, monkeypatch):
        ...
        for rel in (".env", "auth.json", "state.db", "profiles/coder/.env"):
            mode = (jacky_home / rel).stat().st_mode & 0o777
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        # Source
        src_home = tmp_path / "source" / ".jacky"
        src_home.mkdir(parents=True)
        _make_jacky_tree(src_home)

        monkeypatch.setenv("JACKY_HOME", str(src_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "source")

        # Backup
        out_zip = tmp_path / "roundtrip.zip"
        from jacky_cli.backup import run_backup, run_import

        run_backup(Namespace(output=str(out_zip)))
        assert out_zip.exists()

        # Import into a different location
        dst_home = tmp_path / "dest" / ".jacky"
        dst_home.mkdir(parents=True)
        monkeypatch.setenv("JACKY_HOME", str(dst_home))
        ...
        # jacky-agent should NOT be present
        assert not (dst_home / "jacky-agent").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from jacky_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_validate_rejects_random(self):
        """Zip without jacky markers fails validation."""
        import io
        from jacky_cli.backup import _validate_backup_zip
    ...
    def test_detect_prefix_jacky(self):
        """Detects .jacky/ prefix wrapping all entries."""
        import io
        from jacky_cli.backup import _detect_prefix

        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr(".jacky/config.yaml", "test")
            zf.writestr(".jacky/skills/a/SKILL.md", "skill")
        buf.seek(0)
        with zipfile.ZipFile(buf, "r") as zf:
            assert _detect_prefix(zf) == ".jacky/"
...
class TestBackupEdgeCases:
    def test_nonexistent_jacky_home(self, tmp_path, monkeypatch):
        """Backup exits when jacky home doesn't exist."""
        fake_home = tmp_path / "nonexistent" / ".jacky"
        monkeypatch.setenv("JACKY_HOME", str(fake_home))
    ...
    def test_output_is_directory(self, tmp_path, monkeypatch):
        ...
        zips = list(out_dir.glob("jacky-backup-*.zip"))
        assert len(zips) == 1
    ...
    def test_empty_jacky_home(self, tmp_path, monkeypatch):
        """Backup handles empty jacky home (no files to back up)."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        # Only excluded dirs, no actual files
        (jacky_home / "__pycache__").mkdir()
        (jacky_home / "__pycache__" / "foo.pyc").write_bytes(b"\x00")
    ...
    def test_pre1980_timestamp_skipped(self, tmp_path, monkeypatch):
        ...
        old_file = jacky_home / "ancient.txt"
    ...
    def test_skips_output_zip_inside_jacky(self, tmp_path, monkeypatch):
        """Backup skips its own output zip if it's inside jacky root."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / "config.yaml").write_text("model: test\n")
        ...
        # Output inside jacky home
        out_zip = jacky_home / "backup.zip"
        args = Namespace(output=str(out_zip))
...
class TestImportEdgeCases:
    ...
    def test_eof_during_confirmation(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / "config.yaml").write_text("existing\n")
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_keyboard_interrupt_during_confirmation(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / ".env").write_text("KEY=val\n")
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
    ...
    def test_progress_with_many_files(self, tmp_path, monkeypatch):
        ...
        assert (jacky_home / "config.yaml").exists()
        assert (jacky_home / "sessions" / "s0599.json").exists()
...
class TestProfileRestoration:
    ...
    def test_import_creates_profile_wrappers(self, tmp_path, monkeypatch):
        ...
        assert (jacky_home / "profiles" / "coder" / "config.yaml").exists()
        assert (jacky_home / "profiles" / "researcher" / "config.yaml").exists()
        ...
        assert "jacky -p coder" in coder_wrapper
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from jacky_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    @pytest.fixture
    def jacky_home(self, tmp_path):
        """Create a fake JACKY_HOME with critical state files."""
        home = tmp_path / ".jacky"
        home.mkdir()
    ...
    def test_creates_snapshot(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        assert snap_id is not None
        snap_dir = jacky_home / "state-snapshots" / snap_id
        assert snap_dir.is_dir()
        assert (snap_dir / "manifest.json").exists()

    def test_label_in_id(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(label="before-upgrade", jacky_home=jacky_home)
        assert "before-upgrade" in snap_id

    def test_state_db_safely_copied(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        db_copy = jacky_home / "state-snapshots" / snap_id / "state.db"
    ...
    def test_copies_nested_files(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        assert (jacky_home / "state-snapshots" / snap_id / "cron" / "jobs.json").exists()

    def test_copies_channel_aliases(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        copied = jacky_home / "state-snapshots" / snap_id / "channel_aliases.json"
        assert copied.exists()
        assert "120363408391911677@g.us" in copied.read_text()

    def test_missing_files_skipped(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        with open(jacky_home / "state-snapshots" / snap_id / "manifest.json") as f:
            meta = json.load(f)
    ...
    def test_empty_home_returns_none(self, tmp_path):
        ...
        empty.mkdir()
        assert create_quick_snapshot(jacky_home=empty) is None

    def test_list_snapshots(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot, list_quick_snapshots
        id1 = create_quick_snapshot(label="first", jacky_home=jacky_home)
        id2 = create_quick_snapshot(label="second", jacky_home=jacky_home)

        snaps = list_quick_snapshots(jacky_home=jacky_home)
        assert len(snaps) == 2
        assert snaps[0]["id"] == id2  # most recent first
        assert snaps[1]["id"] == id1

    def test_list_limit(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot, list_quick_snapshots
        for i in range(5):
            create_quick_snapshot(label=f"s{i}", jacky_home=jacky_home)
        snaps = list_quick_snapshots(limit=3, jacky_home=jacky_home)
        assert len(snaps) == 3

    def test_restore_config(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)

        (jacky_home / "config.yaml").write_text("model:\n  provider: anthropic\n")
        assert "anthropic" in (jacky_home / "config.yaml").read_text()

        result = restore_quick_snapshot(snap_id, jacky_home=jacky_home)
        assert result is True
        assert "openrouter" in (jacky_home / "config.yaml").read_text()

    def test_restore_state_db(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)

        conn = sqlite3.connect(str(jacky_home / "state.db"))
        conn.execute("INSERT INTO sessions VALUES ('s2', 'new')")
        conn.commit()
        conn.close()

        restore_quick_snapshot(snap_id, jacky_home=jacky_home)
    ...
    def test_restore_nonexistent(self, jacky_home):
        from jacky_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", jacky_home=jacky_home) is False

    def test_auto_prune(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
        for i in range(_QUICK_DEFAULT_KEEP + 5):
            create_quick_snapshot(label=f"snap-{i:03d}", jacky_home=jacky_home)
        snaps = list_quick_snapshots(limit=100, jacky_home=jacky_home)
        assert len(snaps) <= _QUICK_DEFAULT_KEEP

    def test_manual_prune(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", jacky_home=jacky_home)
        deleted = prune_quick_snapshots(keep=3, jacky_home=jacky_home)
        assert deleted == 7
        assert len(list_quick_snapshots(jacky_home=jacky_home)) == 3

    def test_snapshot_includes_pairing_directories(self, jacky_home):
        ...
        (jacky_home / "platforms" / "pairing").mkdir(parents=True)
        (jacky_home / "platforms" / "pairing" / "telegram-approved.json").write_text(
            '{"12345": {"user_name": "alice"}}'
        )
        (jacky_home / "platforms" / "pairing" / "discord-approved.json").write_text(
            '{"67890": {"user_name": "bob"}}'
        )
        # Legacy pairing store (old location)
        (jacky_home / "pairing").mkdir()
        (jacky_home / "pairing" / "matrix-approved.json").write_text(
            '{"@charlie:server": {"user_name": "charlie"}}'
        )
        # Feishu's separate JSON
        (jacky_home / "feishu_comment_pairing.json").write_text(
    ...
    def test_restore_recovers_pairing_data(self, jacky_home):
        ...
        assert restore_quick_snapshot(snap_id, jacky_home=jacky_home) is True
        assert approved.exists()
        assert '"alice"' in approved.read_text()
        assert feishu.exists()
        assert '"user_xyz"' in feishu.read_text()

    def test_empty_pairing_dir_does_not_fail(self, jacky_home):
...
# ---------------------------------------------------------------------------
# Pre-update backup (jacky update safety net)
# ---------------------------------------------------------------------------
    ...
    def test_restore_rejects_snapshot_id_traversal(self, jacky_home):
    ...
    def test_restore_rejects_manifest_rel_traversal(self, jacky_home):
        ...
        escape_dst = jacky_home.parent.parent / "outside.txt"
        ...
        assert not escape_dst.exists(), (
            f"manifest rel traversal escaped JACKY_HOME: {escape_dst} exists"
        )
...
class TestQuickSnapshotProjectsKanban:
    ...
    def test_in_quick_state_files(self):
        from jacky_cli.backup import _QUICK_STATE_FILES
    ...
    def test_projects_db_snapshotted(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        copy = jacky_home / "state-snapshots" / snap_id / "projects.db"
        assert copy.exists()
    ...
    def test_kanban_db_snapshotted(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        copy = jacky_home / "state-snapshots" / snap_id / "kanban.db"
        assert copy.exists()
        conn = sqlite3.connect(str(copy))
    ...
    def test_restore_recreates_emptied_projects_db(self, jacky_home):
        from jacky_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(jacky_home=jacky_home)

        # Simulate the upgrade wiping the store back to an empty schema.
        conn = sqlite3.connect(str(jacky_home / "projects.db"))
    ...
    def test_non_default_kanban_board_snapshotted(self, jacky_home):
        ...
        board_dir = jacky_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        copy = (
            jacky_home / "state-snapshots" / snap_id
    ...
    def test_additional_per_profile_dbs_round_trip(self, jacky_home):
        ...
        for name, (table, row) in seeded.items():
            conn = sqlite3.connect(str(jacky_home / name))
            conn.execute(f"CREATE TABLE {table} (id TEXT PRIMARY KEY, data TEXT)")
            conn.execute(f"INSERT INTO {table} VALUES (?, ?)", row)
            conn.commit()
            conn.close()
    ...
    def test_board_workspaces_and_attachments_are_skipped(self, jacky_home):
        ...
        board = jacky_home / "kanban" / "boards" / "work"
        ...
        snap_id = create_quick_snapshot(jacky_home=jacky_home)
        snap = jacky_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work"
    ...
    def test_board_db_copied_wal_safely(self, jacky_home, monkeypatch):
        ...
        copy = jacky_home / "state-snapshots" / snap_id / "kanban" / "boards" / "work" / "kanban.db"
        rows = sqlite3.connect(str(copy)).execute("SELECT * FROM tasks").fetchall()
        assert rows == [("w1", "ship")]


class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``jacky update``
    ...
    def jacky_home(self, tmp_path):
        root = tmp_path / ".jacky"
        root.mkdir()
        _make_jacky_tree(root)
        return root

    def test_creates_backup_under_backups_dir(self, jacky_home):
        from jacky_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(jacky_home=jacky_home)
        assert out is not None
        assert out.exists()
        assert out.parent == jacky_home / "backups"
        assert out.name.startswith("pre-update-")
        assert out.suffix == ".zip"

    def test_backup_contents_match_full_backup(self, jacky_home):
        """Pre-update backup should include the same user data that
        ``jacky backup`` would, and should exclude the same directories."""
        from jacky_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(jacky_home=jacky_home)
        assert out is not None
        ...
        assert "skills/my-skill/SKILL.md" in names
        assert "profiles/coder/config.yaml" in names
        # jacky-agent repo excluded
        assert not any(n.startswith("jacky-agent/") for n in names)
        # __pycache__ excluded
        assert not any("__pycache__" in n for n in names)
        # pid files excluded
        assert "gateway.pid" not in names

    def test_does_not_recurse_into_prior_backups(self, jacky_home):
        """The ``backups/`` directory must be excluded so that each backup
        doesn't grow exponentially by including all prior backups."""
        from jacky_cli.backup import create_pre_update_backup
        # First backup
        out1 = create_pre_update_backup(jacky_home=jacky_home)
        assert out1 is not None
        # Second backup — must not include the first
        out2 = create_pre_update_backup(jacky_home=jacky_home)
    ...
    def test_rotation_keeps_only_n(self, jacky_home):
        ...
        created = []
        for _ in range(5):
            out = create_pre_update_backup(jacky_home=jacky_home, keep=3)
            created.append(out)
            _t.sleep(1.05)  # ensure distinct seconds in timestamp

        remaining = sorted(
            p.name for p in (jacky_home / "backups").iterdir()
    ...
    def test_rotation_preserves_manual_files(self, jacky_home):
        """Hand-dropped zips in ``backups/`` must not be touched by
        rotation — it only prunes files matching ``pre-update-*.zip``."""
        import time as _t
        from jacky_cli.backup import create_pre_update_backup

        (jacky_home / "backups").mkdir(exist_ok=True)
        manual = jacky_home / "backups" / "my-manual.zip"
        manual.write_bytes(b"manual backup")

        for _ in range(5):
            create_pre_update_backup(jacky_home=jacky_home, keep=2)
            _t.sleep(1.05)
    ...
    def test_returns_none_if_root_missing(self, tmp_path):
        from jacky_cli.backup import create_pre_update_backup
        assert create_pre_update_backup(jacky_home=tmp_path / "does-not-exist") is None

    def test_keep_zero_does_not_delete_freshly_created_backup(self, jacky_home):
        ...
        """
        from jacky_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(jacky_home=jacky_home, keep=0)
        assert out is not None
        assert out.exists(), (
    ...
    def test_keep_negative_does_not_delete_freshly_created_backup(self, jacky_home):
    ...
    def test_keep_zero_still_prunes_older_backups(self, jacky_home):
        ...
        first = create_pre_update_backup(jacky_home=jacky_home, keep=5)
        _t.sleep(1.05)
        second = create_pre_update_backup(jacky_home=jacky_home, keep=5)
        _t.sleep(1.05)
        third = create_pre_update_backup(jacky_home=jacky_home, keep=0)
    ...
    def test_skips_symlinked_files(self, jacky_home, tmp_path):
        """Pre-update backups must not dereference symlinks outside JACKY_HOME."""
        from jacky_cli.backup import create_pre_update_backup
...
class TestRunPreUpdateBackup:
    ...
    @pytest.fixture
    def jacky_home(self, tmp_path, monkeypatch):
        root = tmp_path / ".jacky"
        root.mkdir()
        _make_jacky_tree(root)
        # Point JACKY_HOME at the temp dir so config + backup paths resolve here
        monkeypatch.setenv("JACKY_HOME", str(root))
        # Make Path.home() point at tmp_path for anything that uses it
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        # Bust caches for jacky_cli.config + jacky_constants so they pick up JACKY_HOME
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("jacky_cli.config") or mod == "jacky_constants":
                del __import__("sys").modules[mod]
        return root

    def test_backup_flag_creates_backup(self, jacky_home, capsys):
        """--backup forces the pre-update backup for one run even when config is off."""
        from jacky_cli.main import _run_pre_update_backup
        _run_pre_update_backup(Namespace(no_backup=False, backup=True))
        out = capsys.readouterr().out
        assert "Creating pre-update backup" in out
        assert "Saved:" in out
        assert "Restore:" in out
        assert "jacky import" in out
        assert "Disable:" in out
        # Actual backup was created
        backups = list((jacky_home / "backups").glob("pre-update-*.zip"))
        assert len(backups) == 1

    def test_default_disabled_is_silent(self, jacky_home, capsys):
        """With the default (``pre_update_backup: false``), ``jacky update``
        ...
        out = capsys.readouterr().out
        assert out == ""
        assert not list((jacky_home / "backups").glob("pre-update-*.zip")) \
            if (jacky_home / "backups").exists() else True

    def test_no_backup_flag_skips(self, jacky_home, capsys):
        from jacky_cli.main import _run_pre_update_backup
        ...
        # No backup written
        assert not (jacky_home / "backups").exists() or not list(
            (jacky_home / "backups").glob("pre-update-*.zip")
        )

    def test_config_enabled_creates_backup(self, jacky_home, capsys):
        ...
        import yaml
        (jacky_home / "config.yaml").write_text(yaml.safe_dump({
            "_config_version": 22,
            "updates": {"pre_update_backup": True},
        }))
        import sys as _sys
        for mod in list(_sys.modules.keys()):
            if mod.startswith("jacky_cli.config"):
    ...
    def test_config_disabled_is_silent(self, jacky_home, capsys):
    ...
    def test_cli_flag_overrides_enabled_config(self, jacky_home, capsys):
...
# ---------------------------------------------------------------------------
# Pre-migration backup (jacky claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``jacky claw migrate`` runs before mutating ~/.jacky/."""
    ...
    def test_creates_backup_under_backups_dir(self, jacky_home):
        from jacky_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(jacky_home=jacky_home)
        assert out is not None
        assert out.exists()
        # Shares the backups/ directory with pre-update backups so `jacky
        # import` and the update-backup listing both pick them up.
        assert out.parent == jacky_home / "backups"
        assert out.name.startswith("pre-migration-")
        assert out.suffix == ".zip"

    def test_backup_uses_shared_exclusion_rules(self, jacky_home):
        """Pre-migration backup reuses the same exclusion rules as
        ``jacky backup`` / ``create_pre_update_backup`` — no drift."""
        from jacky_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(jacky_home=jacky_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_restorable_with_jacky_import(self, jacky_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Jacky
        backup — `jacky import` should accept it."""
        from jacky_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(jacky_home=jacky_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_recurse_into_prior_backups(self, jacky_home):
        from jacky_cli.backup import create_pre_migration_backup
        out1 = create_pre_migration_backup(jacky_home=jacky_home)
        assert out1 is not None
        out2 = create_pre_migration_backup(jacky_home=jacky_home)
        assert out2 is not None
        with zipfile.ZipFile(out2) as zf:
            names = zf.namelist()
        assert not any(n.startswith("backups/") for n in names)
    ...
    def test_rotation_keeps_only_n(self, jacky_home):
        ...
        created = []
        for _ in range(7):
            out = create_pre_migration_backup(jacky_home=jacky_home, keep=3)
            if out is not None:
                created.append(out)
            _t.sleep(1.05)  # timestamp resolution

        remaining = sorted((jacky_home / "backups").glob("pre-migration-*.zip"))
        assert len(remaining) <= 3, f"expected <=3 backups retained, got {len(remaining)}"

    def test_missing_jacky_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.jacky yet — nothing to back up."""
        from jacky_cli.backup import create_pre_migration_backup
        missing = tmp_path / "does-not-exist"
        out = create_pre_migration_backup(jacky_home=missing)
        assert out is None

    def test_does_not_touch_pre_update_backups(self, jacky_home):
        """Pre-migration rotation must only prune pre-migration-*.zip files,
        leaving pre-update-*.zip backups untouched."""
        from jacky_cli.backup import create_pre_update_backup, create_pre_migration_backup
        update_backup = create_pre_update_backup(jacky_home=jacky_home, keep=5)
        assert update_backup is not None and update_backup.exists()
        # Spin up a lot of migration backups with keep=1
        import time as _t
        for _ in range(3):
            out = create_pre_migration_backup(jacky_home=jacky_home, keep=1)
            assert out is not None
            _t.sleep(1.05)
...
class TestRestoreCronJobsIfEmptied:
    """`jacky update` config migration can leave cron/jobs.json valid-but-empty,
    ...
    def _make_snapshot(self, jacky_home: Path, label="pre-update"):
        from jacky_cli.backup import create_quick_snapshot
        return create_quick_snapshot(label=label, jacky_home=jacky_home, keep=5)

    def test_restores_when_emptied_after_migration(self, tmp_path):
        from jacky_cli.backup import restore_cron_jobs_if_emptied
        jacky_home = tmp_path / ".jacky"
        jobs_path = jacky_home / "cron" / "jobs.json"
        # Pre-update: 3 real jobs.
        self._seed_jobs(jobs_path, [{"id": "a"}, {"id": "b"}, {"id": "c"}])
        snap_id = self._make_snapshot(jacky_home)
        assert snap_id

        # Migration silently empties the file (valid JSON, zero jobs).
        jobs_path.write_text(json.dumps({"jobs": []}))

        result = restore_cron_jobs_if_emptied(snap_id, jacky_home=jacky_home)
        assert result is not None
    ...
    def test_noop_when_snapshot_id_missing(self, tmp_path):
        ...
        jobs_path = jacky_home / "cron" / "jobs.json"
        self._seed_jobs(jobs_path, [])
        assert restore_cron_jobs_if_emptied(None, jacky_home=jacky_home) is None
        assert restore_cron_jobs_if_emptied("", jacky_home=jacky_home) is None
...
# location, NOT under JACKY_HOME. (backup/import cycle data-loss fix)
# ---------------------------------------------------------------------------

class TestMemoryProviderExternalPaths:
    def _make_min_tree(self, jacky_home: Path) -> None:
        jacky_home.mkdir(parents=True, exist_ok=True)
        (jacky_home / "config.yaml").write_text("model:\n  provider: openrouter\n")
        (jacky_home / ".env").write_text("OPENROUTER_API_KEY=sk-test\n")
        (jacky_home / "state.db").write_bytes(b"x")
    ...
    def test_backup_captures_external_paths_under_external_prefix(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        self._make_min_tree(jacky_home)
        # External provider state living OUTSIDE JACKY_HOME.
    ...
    def test_import_restores_external_to_home_relative_location(self, tmp_path, monkeypatch):
        """_external/ members restore to ~/<relpath>, not under JACKY_HOME,
        and credential-shaped files get 0600."""
        dst_home = tmp_path / "dst"
        dst_home.mkdir()
        jacky_home = dst_home / ".jacky"
        jacky_home.mkdir()
        ...
        # External state did NOT leak into JACKY_HOME.
        assert not (jacky_home / "_external").exists()

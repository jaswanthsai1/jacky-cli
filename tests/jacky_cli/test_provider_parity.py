...
the SAME provider universe as ``jacky model`` (the CLI/TUI picker).
...
def test_every_jacky_model_provider_is_configurable_in_desktop():
    """PARITY CONTRACT: GUI (keys ∪ accounts) ⊇ `jacky model` universe."""
    ...
    assert not missing, (
        "providers shown in `jacky model` but not configurable in the desktop "

"""Tests for `jacky memory setup [provider]` routing.
...
``jacky memory setup honcho``) without the interactive picker — which matters
because the per-provider ``jacky <provider>`` subcommand is only registered
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "jacky memory setup" in out
...
class TestInstallDependenciesRunner:
    ...
    def _run_with_missing_dep(self, tmp_path, which_side_effect, run_behavior=None):
        ...
        with patch("plugins.memory.find_provider_dir", return_value=tmp_path), \
             patch("jacky_cli.tools_config.shutil.which", side_effect=which_side_effect), \
             patch("jacky_cli.tools_config.subprocess.run", fake_run):

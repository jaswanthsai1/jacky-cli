...
Covers:
- Defense 1: gateway stop/restart refuse when _JACKY_GATEWAY=1
...
class TestGatewayLifecyclePattern:
    ...
    @pytest.mark.parametrize("text", [
        "launchctl kickstart gui/501/ai.jacky.gateway",
        "launchctl unload ~/Library/LaunchAgents/ai.jacky.gateway.plist",
        "launchctl stop ai.jacky.gateway",
        "systemctl restart jacky-gateway",
        "systemctl stop jacky-gateway.service",
        "systemctl start jacky-gateway",
    ])
    def test_service_manager_commands(self, text):
        assert _contains_gateway_lifecycle_command(text), f"Should match: {text!r}"

    @pytest.mark.parametrize("text", [
        "kill jacky gateway process",
        "pkill -f jacky.*gateway",
        "pkill -f gateway.*jacky",          # inverse token order
    ])
    ...
    @pytest.mark.parametrize("text", [
        ...
        "jacky cron list",
        "jacky update",
        "jacky config set model claude",
        "echo 'just a normal cron job'",
        "run the backup script",
        "gateway is running fine",
        # `jacky gateway start` is benign — starting a gateway from inside a
        ...
        "jacky gateway start",
        "jacky gateway start --all",
        # Tightened launchctl/systemctl branches: ops on NON-gateway jacky
        # services must not be falsely blocked (the old `.*jacky` matched any
        # jacky token).
        "launchctl unload ai.jacky.update-checker.plist",
        "launchctl restart ai.jacky.daemon",
        "systemctl restart jacky-meta.service",
        "systemctl restart jacky-cron-helper",
...
class TestCronCreateLifecycleBlock:
    ...
    def test_block_jacky_gateway_restart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="30m",
            prompt="Upgrade jacky then run jacky gateway restart",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_launchctl_kickstart(self, capsys):
        args = Namespace(
            cron_command="create",
            schedule="0 9 * * *",
            prompt="Run launchctl kickstart -k gui/501/ai.jacky.gateway",
            name=None,
            deliver=None,
            repeat=None,
            skill=None,
    ...
    def test_block_script_with_lifecycle_command(self, tmp_path, capsys, monkeypatch):
        ...
        # restart_jacky_gateway_once.sh). The script must live under
        # JACKY_HOME/scripts so the scheduler — and the guard — resolve it.
        monkeypatch.setenv("JACKY_HOME", str(tmp_path / ".jacky"))
        scripts_dir = tmp_path / ".jacky" / "scripts"
...
class TestGatewaySelfTargetingGuard:
    """Verify jacky gateway stop/restart refuse when _JACKY_GATEWAY=1."""

    def test_stop_refuses_inside_gateway(self, monkeypatch):
        monkeypatch.setenv("_JACKY_GATEWAY", "1")
    ...
    def test_stop_allows_outside_gateway(self, monkeypatch):
        ...
        monkeypatch.delenv("_JACKY_GATEWAY", raising=False)
...
class TestTerminalToolGatewayLifecycleGuard:
    """terminal_tool must refuse gateway lifecycle commands when _JACKY_GATEWAY=1.

    Issue #37453: systemctl --user restart jacky-gateway runs as a child of the
    ...
    @pytest.mark.parametrize("cmd", [
        "systemctl restart jacky-gateway",
        "systemctl --user restart jacky-gateway",
        "systemctl stop jacky-gateway.service",
        "jacky gateway restart",
        "launchctl kickstart gui/501/ai.jacky.gateway",
        "pkill -f jacky.*gateway",
    ...
    def test_force_true_cannot_bypass_block(self, monkeypatch):
        ...
        result = json.loads(tt.terminal_tool(
            command="systemctl restart jacky-gateway", force=True
        ))
    ...
    def test_safe_systemctl_commands_pass_through(self, monkeypatch):
        """Non-jacky systemctl commands must not be blocked by this guard."""
        import tools.terminal_tool as tt
    ...
    def test_guard_inactive_outside_gateway(self, monkeypatch):
        """Without _JACKY_GATEWAY=1 the lifecycle guard must not fire."""
        import tools.terminal_tool as tt
        ...
        result = json.loads(tt.terminal_tool(command="systemctl restart jacky-gateway"))

        # Outside the gateway the lifecycle guard doesn't block — the normal
        # approval flow handles it (here mocked as approved).
        assert result["exit_code"] == 0
        assert calls == ["systemctl restart jacky-gateway"]
...
class TestLifecycleGuardModule:
    ...
    def test_prompt_with_command_raises(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked) as exc:
            check_gateway_lifecycle("please run jacky gateway restart", None)
    ...
    def test_split_across_prompt_and_script_still_blocks(self, tmp_path):
        ...
        script.write_text("jacky gateway stop\n")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_binary_script_does_not_silently_bypass(self, tmp_path):
        ...
        script.write_bytes(b"\xfejacky gateway restart\xff")
        with pytest.raises(GatewayLifecycleBlocked):
    ...
    def test_relative_script_resolved_under_scripts_dir(self, tmp_path, monkeypatch):
        """A bare/relative script name resolves under JACKY_HOME/scripts (the
        ...
        (scripts_dir / "restart.sh").write_text(
            "launchctl kickstart -k gui/501/ai.jacky.gateway\n"
        )
        with pytest.raises(GatewayLifecycleBlocked):
...
class TestCreateJobBlocksLifecycleCommands:
    ...
    jacky_cli.cron.cron_create. Enforcing at create_job covers both."""
    ...
    def test_create_job_blocks_prompt_command(self):
        ...
        with pytest.raises(GatewayLifecycleBlocked):
            create_job(prompt="then run jacky gateway restart", schedule="30m")
    ...
    def test_cronjob_tool_surfaces_block_as_error(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_HOME", str(tmp_path / ".jacky"))
        (tmp_path / ".jacky").mkdir(parents=True)
        from tools.cronjob_tools import cronjob
        result = json.loads(cronjob(
            action="create", schedule="0 9 * * *",
            prompt="please run jacky gateway restart nightly",
        ))

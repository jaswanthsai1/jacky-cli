...
from jacky_cli.urllib_security import (
...
def test_cross_host_redirect_drops_arbitrary_credentials_on_wire():
    ...
    assert headers["user-agent"] == "jacky-test"
    for name in (
        "authorization",
        "cookie",
...
def test_installed_custom_opener_policy_is_preserved(monkeypatch):
    ...
    from jacky_cli.urllib_security import _secure_opener_from_installed_policy

    secured = _secure_opener_from_installed_policy(
        "foo://models.example.test/catalog"
    )
    assert secured.addheaders == []
    assert getattr(secured, "_jacky_initial_addheaders") == installed.addheaders

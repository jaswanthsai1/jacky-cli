...
jacky_cli.config._load_config_impl, so the managed-scope merge has to be
applied in BOTH places or the interactive CLI/TUI surface (skin, display prefs)
silently ignores administrator-pinned values while `jacky config`/`doctor`
...
def _load_cli_config(home):
    ...
    cli.py binds ``_jacky_home = get_jacky_home()`` at import time (module
    singleton), so monkeypatching JACKY_HOME after import doesn't move it.

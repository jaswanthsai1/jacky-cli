"""Tests for xAI Grok OAuth — tokens stored in Jacky auth store (~/.jacky/auth.json)."""
...
def _setup_jacky_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Jacky auth store at the given root."""
    ...
    auth_file = jacky_home / "auth.json"
...
def _patch_httpx_client(monkeypatch, response):
    ...
    monkeypatch.setattr("jacky_cli.auth.httpx.Client", _factory)
    return holder
...
def test_xai_oauth_poll_device_token_waits_until_authorized(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.auth.time.sleep", lambda _: None)
...
def test_save_and_read_xai_oauth_tokens_roundtrip(tmp_path, monkeypatch):
    ...
    jacky_home.mkdir(parents=True, exist_ok=True)
    (jacky_home / "auth.json").write_text(json.dumps({"version": 1, "providers": {}}))
...
def test_read_xai_oauth_tokens_missing_access_token(tmp_path, monkeypatch):
    jacky_home = tmp_path / "jacky"
    _setup_jacky_auth(jacky_home, access_token="")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
...
def test_read_xai_oauth_tokens_missing_refresh_token(tmp_path, monkeypatch):
    jacky_home = tmp_path / "jacky"
    _setup_jacky_auth(jacky_home, refresh_token="")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
...
def test_resolve_xai_runtime_credentials_returns_singleton_state(tmp_path, monkeypatch):
    jacky_home = tmp_path / "jacky"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_jacky_auth(jacky_home, access_token=fresh)
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    monkeypatch.delenv("JACKY_XAI_BASE_URL", raising=False)
    ...
    assert creds["source"] == "jacky-auth-store"
...
def test_resolve_xai_runtime_credentials_refreshes_expiring_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.auth._refresh_xai_oauth_tokens", _fake_refresh)
...
def test_resolve_xai_runtime_credentials_honours_env_base_url(tmp_path, monkeypatch):
    ...
    _setup_jacky_auth(jacky_home, access_token=fresh)
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    monkeypatch.setenv("JACKY_XAI_BASE_URL", "https://custom.x.ai/v1/")
...
# ``XAI_BASE_URL`` / ``JACKY_XAI_BASE_URL`` are a credential-leak vector
...
def _seed_xai_oauth_state(
    jacky_home: Path, state: dict, *, active_provider: str = "xai-oauth"
) -> None:
    jacky_home.mkdir(parents=True, exist_ok=True)
    auth_store = {
        "version": 1,
    ...
    (jacky_home / "auth.json").write_text(json.dumps(auth_store, indent=2))
...
def test_resolve_credentials_quarantines_dead_tokens_on_terminal_refresh_failure(
    ...
    """
    jacky_home = tmp_path / "jacky"
    _seed_xai_oauth_state(jacky_home, dict(_STALE_XAI_OAUTH_STATE), active_provider="nous")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    ...
    monkeypatch.setattr("jacky_cli.auth._refresh_xai_oauth_tokens", _terminal_refresh)
    ...
    raw = json.loads((jacky_home / "auth.json").read_text())
...
def test_resolve_credentials_does_not_quarantine_on_transient_refresh_failure(
    ...
    """
    jacky_home = tmp_path / "jacky"
    _seed_xai_oauth_state(jacky_home, dict(_STALE_XAI_OAUTH_STATE))
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    ...
    monkeypatch.setattr("jacky_cli.auth._refresh_xai_oauth_tokens", _transient_refresh)
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``jacky model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``jacky model``
    ...
    assert "jacky model" not in rendered.lower()
...
def test_xai_oauth_discovery_raises_typed_error_on_malformed_json(monkeypatch):
    ...
    from jacky_cli.auth import _xai_oauth_discovery
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Jacky that didn't validate) must not be silently honored on the refresh
...
def test_xai_oauth_discovery_validates_endpoints(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.auth.httpx.get", _fake_get)
    with pytest.raises(AuthError) as exc:
        _xai_oauth_discovery()
    assert exc.value.code == "xai_discovery_invalid"
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `jacky model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `jacky auth list` reflects truth and
    ...
    jacky_home = tmp_path / "jacky"
    fresh = _jwt_with_exp(int(time.time()) + 2 * 60 * 60)
    _setup_jacky_auth(jacky_home, access_token=fresh, refresh_token="rt-1")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
...
def test_credential_pool_device_code_seed_respects_suppression(tmp_path, monkeypatch):
    from agent.credential_pool import load_pool
    from jacky_cli.auth import suppress_credential_source
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``jacky auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `jacky auth remove xai-oauth 1`.
    ...
    # nothing to import.
    raw_after = json.loads((jacky_home / "auth.json").read_text())
    ...
    assert not pool_after.has_credentials(), (
        ...
        "and rebuilds the entry on every Jacky invocation."
    )


def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``jacky model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``jacky model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``jacky auth
    ...
    monkeypatch.setattr(
        "jacky_cli.auth._update_config_for_provider",
...
def test_pool_sync_back_writes_to_singleton(tmp_path, monkeypatch):
    ...
    _setup_jacky_auth(jacky_home, access_token=expired, refresh_token="rt-old")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    ...
    monkeypatch.setattr("jacky_cli.auth.refresh_xai_oauth_pure", _fake_refresh)
    ...
    auth_path = jacky_home / "auth.json"
...
def test_pool_entry_needs_refresh_when_jwt_within_skew(tmp_path, monkeypatch):
    ...
    from agent.credential_pool import load_pool, AUTH_TYPE_OAUTH, PooledCredential
    from jacky_cli.auth import XAI_ACCESS_TOKEN_REFRESH_SKEW_SECONDS
    import uuid
...
def test_pool_refresh_marks_entry_exhausted_on_failure(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.auth.refresh_xai_oauth_pure", _fake_refresh_fail)
...
def test_pool_seeded_entry_sync_back_after_refresh(tmp_path, monkeypatch):
    ...
    jacky_home = tmp_path / "jacky"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_jacky_auth(jacky_home, access_token=near_expiry, refresh_token="rt-singleton")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
...
def test_pool_refresh_adopts_singleton_tokens_when_consumed_elsewhere(tmp_path, monkeypatch):
    """Multi-process race: another Jacky process refreshed the singleton
    ...
    profiles + Jacky processes."""
    from agent.credential_pool import load_pool

    jacky_home = tmp_path / "jacky"
    in_memory_at = _jwt_with_exp(int(time.time()) + 30)  # near-expiry
    _setup_jacky_auth(jacky_home, access_token=in_memory_at, refresh_token="rt-stale")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    ...
    (jacky_home / "auth.json").write_text(json.dumps(raw))
...
def test_pool_refresh_recovers_when_other_process_already_refreshed(tmp_path, monkeypatch):
    ...
    _setup_jacky_auth(jacky_home, access_token=in_memory_at, refresh_token="rt-shared")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
...
def test_pool_exhausted_xai_entry_recovers_after_singleton_refresh(tmp_path, monkeypatch):
    ...
    user runs ``jacky model`` -> xAI Grok OAuth (or another process
    ...
    _setup_jacky_auth(jacky_home, access_token=stale_at, refresh_token="rt-stale")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    ...
    # Simulate the user re-running `jacky model` -> xAI Grok OAuth: the
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``jacky auth add xai-oauth``) own their own refresh-token lifecycle
    ...
    _setup_jacky_auth(jacky_home, access_token=singleton_at, refresh_token="rt-singleton")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`jacky auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``jacky auth status``, ``jacky setup``, and the
    ``jacky`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
    no future refactor regresses to the legacy semantic."""
    from agent.credential_pool import load_pool

    jacky_home = tmp_path / "jacky"
    near_expiry = _jwt_with_exp(int(time.time()) + 30)
    _setup_jacky_auth(jacky_home, access_token=near_expiry, refresh_token="rt-xai")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))

...
class TestUnifiedDashboardRouting:
    def test_profile_launch_attaches_to_running_dashboard(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "jacky_cli.profiles.get_active_profile_name", lambda: "worker_x"
        )
    ...
    def test_reexec_pins_docker_machine_root(self, main_mod, monkeypatch):
        """In the Docker layout (JACKY_HOME=/opt/data, profiles under
        /opt/data/profiles/<name>) the reroute must pin the child to the
        machine root /opt/data — NOT drop JACKY_HOME.

        Dropping it makes the child fall back to $HOME/.jacky
        (= /opt/data/.jacky), an empty auto-seeded home, so the dashboard
        ...
        """
        monkeypatch.setenv("JACKY_HOME", "/opt/data/profiles/oracle")
        monkeypatch.setattr(
            "jacky_cli.profiles.get_active_profile_name", lambda: "oracle"
        )
        monkeypatch.setattr(main_mod, "_dashboard_listening", lambda host, port: False)
        execs = []
    ...
    def test_default_profile_launch_skips_routing(self, main_mod, monkeypatch):
        monkeypatch.setattr(
            "jacky_cli.profiles.get_active_profile_name", lambda: "default"
        )
        listening_calls = []
        monkeypatch.setattr(
            main_mod, "_dashboard_listening",
    ...
    def test_dashboard_starts_mcp_discovery_for_ws_backend(self, main_mod, monkeypatch):
        ...
        monkeypatch.delenv("JACKY_WEB_DIST", raising=False)
        ...
        calls = []
        monkeypatch.setattr(
            "jacky_cli.mcp_startup.start_background_mcp_discovery",

"""Tests for the per-profile Projects store (jacky_cli/projects_db)."""
...
def test_create_get_list(conn):
    pid = pdb.create_project(conn, name="Jacky Agent", folders=["/tmp/jacky"])
    proj = pdb.get_project(conn, pid)

    assert proj is not None
    assert proj.slug == "jacky-agent"
    assert proj.name == "Jacky Agent"
    # First folder becomes primary.
    assert proj.primary_path == "/tmp/jacky"
    assert [f.path for f in proj.folders] == ["/tmp/jacky"]
    assert proj.folders[0].is_primary is True

    # Lookup by slug too.
    assert pdb.get_project(conn, "jacky-agent").id == pid
    assert len(pdb.list_projects(conn)) == 1


def test_slug_collision_disambiguates(conn):
    pdb.create_project(conn, name="Jacky Agent")
    pdb.create_project(conn, name="Jacky Agent")
    slugs = sorted(p.slug for p in pdb.list_projects(conn))

    assert slugs == ["jacky-agent", "jacky-agent-2"]
...
def test_per_profile_isolation(tmp_path):
    # Two distinct DB paths stand in for two profiles' JACKY_HOME.
...
def test_db_path_under_jacky_home():
    # Resolves under JACKY_HOME (set by the autouse isolation fixture).

"""Behavior tests for jacky_cli.inventory.
...
from jacky_cli.inventory import (
    ConfigContext,
...
def test_build_models_payload_does_not_call_provider_model_ids():
    ...
    """
    rows = [{"slug": "nous", "name": "Nous", "models": ["jacky-4-405b"],
             "total_models": 1, "is_current": False, "is_user_defined": False,
             "source": "built-in"}]
    ctx = _empty_ctx()
    with _list_auth_returning(rows), \
         patch("jacky_cli.models.provider_model_ids") as mock_pm:
...
def test_pricing_uses_cached_nous_tier_by_default():
    ...
    with (
        ...
        patch(
            "jacky_cli.models.get_pricing_for_provider",
            return_value={
                "openai/gpt-5.5": {
                    "prompt": "0.000001",
                    "completion": "0.000002",
        ...
        patch("jacky_cli.models.check_nous_free_tier", return_value=False) as mock_free,
    ):
...
def test_explicit_only_keeps_moa_when_raw_config_has_enabled_preset():
    ...
    with (
        _list_auth_returning(rows),
        patch("jacky_cli.config.load_config", return_value=raw_config),
        patch("jacky_cli.config.read_raw_config", return_value=raw_config),
        patch("jacky_cli.auth.is_provider_explicitly_configured", return_value=False),
    ):
        payload = build_models_payload(ctx, explicit_only=True)
...
def test_picker_hints_adds_warning_to_skeleton_rows():
    ...
    for row in skeleton_rows:
        ...
        # jacky model fallback.
        assert (
            row["warning"].startswith("paste ")
            or row["warning"].startswith("run `jacky model`")
        )
...
def test_build_models_payload_keeps_static_provider_models_from_providers_dict():
    ...
    with (
        ...
        patch("jacky_cli.providers.JACKY_OVERLAYS", {}),
        patch(
...
def test_build_models_payload_forwards_refresh_flag():
    ...
    with patch("jacky_cli.model_switch.list_authenticated_providers", side_effect=_capture):
...
def test_list_authenticated_providers_refresh_busts_cache():
    ...
    from jacky_cli import model_switch

    with patch("jacky_cli.models.clear_provider_models_cache") as clear:

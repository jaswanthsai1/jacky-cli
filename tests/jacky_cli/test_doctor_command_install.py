"""Tests for the Command Installation check in jacky doctor."""
...
import jacky_cli.doctor as doctor_mod


def _setup_doctor_env(monkeypatch, tmp_path, venv_name="venv"):
    """Create a minimal JACKY_HOME + PROJECT_ROOT for doctor tests."""
    home = tmp_path / ".jacky"
    ...
    jacky_bin = venv_bin_dir / "jacky"
    jacky_bin.write_text("#!/usr/bin/env python\n# entry point\n")
    jacky_bin.chmod(0o755)
...
class TestDoctorCommandInstallation:
    ...
    def test_correct_symlink_shows_ok(self, monkeypatch, tmp_path):
        home, project, jacky_bin = _setup_doctor_env(monkeypatch, tmp_path)

        # Create the command link dir with correct symlink
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "jacky"
        cmd_link.symlink_to(jacky_bin)
    ...
    def test_missing_symlink_shows_fail(self, monkeypatch, tmp_path):
        ...
        assert "jacky doctor --fix" in out
    ...
    def test_fix_creates_missing_symlink(self, monkeypatch, tmp_path):
        ...
        cmd_link = tmp_path / ".local" / "bin" / "jacky"
        assert cmd_link.is_symlink()
        assert cmd_link.resolve() == jacky_bin.resolve()
    ...
    def test_wrong_target_symlink_shows_warn(self, monkeypatch, tmp_path):
        ...
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "jacky"
        wrong_target = tmp_path / "wrong_jacky"
    ...
    def test_dot_venv_dir_is_found(self, monkeypatch, tmp_path):
        ...
        jacky_bin = project / ".venv" / "bin" / "jacky"
        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "jacky"
        cmd_link.symlink_to(jacky_bin)
        ...
        out = _run_doctor(fix=False)
        assert "Venv entry point exists" in out
        assert ".venv/bin/jacky" in out

    @pytest.mark.skipif(sys.platform == "win32", reason="Symlink check is Unix-only")
    def test_non_symlink_regular_file_shows_ok(self, monkeypatch, tmp_path):
        """If ~/.local/bin/jacky is a regular file (not symlink), accept it."""
        home, project, jacky_bin = _setup_doctor_env(monkeypatch, tmp_path)

        cmd_link_dir = tmp_path / ".local" / "bin"
        cmd_link_dir.mkdir(parents=True)
        cmd_link = cmd_link_dir / "jacky"
        cmd_link.write_text("#!/bin/sh\nexec python -m jacky_cli.main \"$@\"\n")

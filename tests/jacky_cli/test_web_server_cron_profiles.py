...
def test_call_cron_for_profile_routes_storage_without_mutating_globals(isolated_profiles):
    ...
    assert job["jacky_home"] == str(isolated_profiles["worker_alpha"])
...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    monkeypatch.setattr(scheduler, "_jacky_home", None)
    captured = {}

    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            captured["job_id"] = job_id
            captured["runtime_home"] = scheduler._get_jacky_home()
    ...
    outer_token = set_jacky_home_override(default_home)
    try:
        ...
        assert scheduler._get_jacky_home() == default_home
    finally:
        reset_jacky_home_override(outer_token)
...
async def test_list_cron_jobs_all_includes_default_and_named_profiles(isolated_profiles):
    ...
    assert by_id[default_job["id"]]["jacky_home"] == str(isolated_profiles["default"])
    assert by_id[worker_job["id"]]["profile"] == "worker_alpha"
    assert by_id[worker_job["id"]]["is_default_profile"] is False
    assert by_id[worker_job["id"]]["jacky_home"] == str(isolated_profiles["worker_alpha"])
...
async def test_update_cron_job_refreshes_snapshots_when_unpinning(
    ...
    from jacky_cli import runtime_provider, web_server

...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (jacky_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from jacky_cli.auth_commands import auth_add_command
    ...
    payload = json.loads((tmp_path / "jacky" / "auth.json").read_text())
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "agent.anthropic_adapter.run_jacky_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
    ...
    entry = next(item for item in entries if item["source"] == "manual:jacky_pkce")
    assert entry["label"] == "claude@example.com"
    assert entry["source"] == "manual:jacky_pkce"
    assert entry["refresh_token"] == "refresh-token"
    assert entry["expires_at_ms"] == 1711234567000


def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """jacky auth add qwen-oauth must set active_provider in auth.json.
    ...
    monkeypatch.setattr(
        "jacky_cli.auth.resolve_qwen_runtime_credentials",
    ...
    # pool entry from pool.add_entry() still present for jacky auth list
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.auth._nous_device_code_login",
        lambda **kwargs: {
    ...
    # `jacky auth add nous` must also populate providers.nous so the
    ...
    # raises "Jacky is not logged into Nous Portal" and the agent dies.
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`jacky auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    """Two ``jacky auth add openai-codex`` runs for different ChatGPT
    ...
    second independent one. ``jacky auth list`` showed two labels sharing
    ...
    monkeypatch.setattr("jacky_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from jacky_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from jacky_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """jacky auth add xai-oauth must write providers singleton and set active_provider.
    ...
    monkeypatch.setattr(
        "jacky_cli.auth._xai_oauth_device_code_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from jacky_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from jacky_cli.auth_commands import auth_reset_command
...
def test_logout_resets_codex_config_when_auth_state_already_cleared(tmp_path, monkeypatch, capsys):
    """`jacky logout --provider openai-codex` must still clear model.provider.
    ...
    config_text = (jacky_home / "config.yaml").read_text()
...
def test_logout_defaults_to_configured_codex_when_no_active_provider(tmp_path, monkeypatch, capsys):
    """Bare `jacky logout` should target configured Codex if auth has no active provider."""
    jacky_home = tmp_path / "jacky"
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    _write_auth_store(tmp_path, {"version": 1, "providers": {}, "credential_pool": {}})
    (jacky_home / "config.yaml").write_text(
...
def test_logout_clears_stale_active_codex_without_provider_credentials(tmp_path, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    assert "Logged out of OpenAI Codex." in out
    auth_payload = json.loads((jacky_home / "auth.json").read_text())
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from jacky_cli.auth import _reset_config_provider
    ...
    with patch("jacky_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from jacky_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "jacky_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("jacky_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("jacky_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("jacky_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "jacky_cli.auth_commands.time.time",
...
def test_auth_remove_claude_code_suppresses_reseed(tmp_path, monkeypatch):
    ...
    (jacky_home / "auth.json").write_text(json.dumps(auth_store))

    from types import SimpleNamespace
    from jacky_cli.auth_commands import auth_remove_command
    auth_remove_command(SimpleNamespace(provider="anthropic", target="1"))

    updated = json.loads((jacky_home / "auth.json").read_text())
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from jacky_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from jacky_cli.auth import unsuppress_credential_source
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `jacky auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("JACKY_HOME", str(tmp_path / "jacky"))
    jacky_home = tmp_path / "jacky"
    jacky_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `jacky auth remove`)
    (jacky_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
    ...
    payload = json.loads((jacky_home / "auth.json").read_text())
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.auth._import_codex_cli_tokens", _fake_import)
    ...
    after = json.loads((jacky_home / "auth.json").read_text())
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`jacky auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.jacky/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.jacky/.env (not the shell), the
    ...
    (jacky_home / ".env").write_text("DEEPSEEK_API_KEY=sk-ds-only\n")
    ...
    assert (jacky_home / ".env").read_text().strip() == ""


def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `jacky auth add <provider>` clears any
    ...
    from types import SimpleNamespace
    from jacky_cli.auth import is_source_suppressed
    from jacky_cli.auth_commands import auth_add_command
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `jacky auth remove`.  This is the gate that prevents shell-exported
...
# =============================================================================
# Unified credential-source stickiness — every source Jacky reads from has a
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import jacky_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_jacky_pkce_suppression(tmp_path, monkeypatch):
    """anthropic jacky_pkce must not re-seed from ~/.jacky/.anthropic_oauth.json when suppressed."""
    jacky_home = tmp_path / "jacky"
    jacky_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))

    import yaml
    (jacky_home / "config.yaml").write_text(yaml.dump({"model": {"provider": "anthropic", "model": "claude"}}))
    (jacky_home / "auth.json").write_text(json.dumps({
        "version": 1,
        "providers": {},
        "suppressed_sources": {"anthropic": ["jacky_pkce"]},
    }))

    # Stub the readers so only jacky_pkce is "available"; claude_code returns None
    import agent.anthropic_adapter as aa
    monkeypatch.setattr(aa, "read_jacky_oauth_credentials", lambda: {
    ...
    # jacky_pkce suppressed, claude_code returns None → nothing should be seeded
    assert entries == []
    assert "jacky_pkce" not in active
...
def test_seed_custom_pool_respects_config_suppression(tmp_path, monkeypatch):
    ...
    import yaml
    (jacky_home / "config.yaml").write_text(yaml.dump({
        "model": {},
        "custom_providers": [
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        "jacky_cli.copilot_auth.resolve_copilot_token",
        return_value=("ghp_fake", "gh"),
    ), patch(
        "jacky_cli.copilot_auth.get_copilot_api_token",
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `jacky auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `jacky auth add openai-codex`)

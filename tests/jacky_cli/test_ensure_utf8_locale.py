"""Regression tests for jacky_cli._ensure_utf8().

Covers the crash class where the setup wizard (and other banner-printing
commands) emit box-drawing characters and the >_ glyph, which raise
...
in ``jacky setup`` because the repair returned early. See the Raspberry Pi
...
def test_latin1_stdout_is_repaired_to_utf8(monkeypatch):
    ...
    assert ">_".encode("utf-8") in sys.stdout.getvalue()
...
def test_fallback_when_reconfigure_unavailable(monkeypatch, tmp_path):
    ...
    sys.stdout.flush()
    fh.close()
    assert ">_".encode("utf-8") in real_path.read_bytes()

"""`jacky status` provider label honors config.yaml model.base_url (#3296)."""

from unittest.mock import patch

from jacky_cli.status import _effective_provider_label


def _label_with(config, env_base_url=""):
    with patch("jacky_cli.status.resolve_requested_provider", return_value="auto"), \
         patch("jacky_cli.status.resolve_provider", return_value="openrouter"), \
         patch("jacky_cli.status.load_config", return_value=config), \
         patch("jacky_cli.status.get_env_value",
...
def test_non_openrouter_provider_untouched():
    with patch("jacky_cli.status.resolve_requested_provider", return_value="anthropic"), \
         patch("jacky_cli.status.resolve_provider", return_value="anthropic"):

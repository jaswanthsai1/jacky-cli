...
from jacky_cli.auth import AuthError, get_provider_auth_state, resolve_nous_runtime_credentials
...
class TestResolveVerifyFallback:
    ...
    def test_missing_ca_bundle_in_auth_state_falls_back(self):
        from jacky_cli.auth import _resolve_verify
    ...
    def test_missing_ssl_cert_file_env_falls_back(self, monkeypatch):
        ...
        monkeypatch.delenv("JACKY_CA_BUNDLE", raising=False)
        result = _resolve_verify(auth_state={"tls": {}})
        assert result is True

    def test_missing_jacky_ca_bundle_env_falls_back(self, monkeypatch):
        from jacky_cli.auth import _resolve_verify

        monkeypatch.setenv("JACKY_CA_BUNDLE", "/nonexistent/jacky-ca.pem")
...
def test_resolve_nous_runtime_credentials_env_override_wins_live_not_persisted(
    ...
    monkeypatch.setattr("jacky_cli.auth._refresh_access_token", _fake_refresh_access_token)
...
def test_removed_legacy_session_env_var_does_not_change_jwt_auth(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    monkeypatch.setenv("JACKY_AGENT_USE_LEGACY_SESSION_KEYS", "true")
...
def test_nous_inference_auth_logs_do_not_include_secret_values(
    ...
    caplog.set_level(logging.INFO, logger="jacky_cli.auth")
    auth_mod.resolve_nous_runtime_credentials(
        force_refresh=True,
    )
...
def test_get_nous_auth_status_pool_opaque_key_is_not_inference_credential(tmp_path, monkeypatch):
    from jacky_cli.auth import get_nous_auth_status, invalidate_nous_auth_status_cache
...
def test_get_nous_auth_status_auth_store_fallback(tmp_path, monkeypatch):
    ...
    jacky_home = tmp_path / "jacky"
    _setup_nous_auth(jacky_home, access_token="at-123")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    monkeypatch.setattr(
...
def test_get_nous_auth_status_prefers_runtime_auth_store_over_stale_pool(tmp_path, monkeypatch):
    ...
    jacky_home = tmp_path / "jacky"
    _setup_nous_auth(jacky_home, access_token="at-fresh")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
...
def test_get_nous_auth_status_reports_revoked_refresh_session(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.auth.resolve_nous_runtime_credentials", _boom)
...
def test_terminal_refresh_failure_quarantines_tokens(
    ...
    from jacky_cli import auth as auth_mod
...
def test_managed_access_token_refresh_failure_quarantines_tokens(
    ...
    jacky_home = tmp_path / "jacky"
    _setup_nous_auth(jacky_home, refresh_token="refresh-old")
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    from agent.credential_pool import load_pool
...
class TestLoginNousSkipKeepsCurrent:
    """When a user runs `jacky model` → Nous Portal → Skip (keep current) after
    ...
    def _setup_home_with_openrouter(self, tmp_path, monkeypatch):
        ...
        return jacky_home, config_path, auth_path
    ...
    def test_skip_keep_current_preserves_provider_and_model(self, tmp_path, monkeypatch):
        ...
        import argparse
        import yaml
        from jacky_cli.auth import PROVIDER_REGISTRY, _login_nous

        jacky_home, config_path, auth_path = self._setup_home_with_openrouter(
...
def test_persist_nous_credentials_writes_both_pool_and_providers(tmp_path, monkeypatch):
    ...
    Regression guard: before this helper existed, `jacky auth add nous`
    ...
    """
    from jacky_cli.auth import persist_nous_credentials, NOUS_DEVICE_CODE_SOURCE
...
def test_persist_nous_credentials_allows_recovery_from_401(tmp_path, monkeypatch):
    ...
    must succeed (not raise "Jacky is not logged into Nous Portal").
    ...
    # doesn't raise "Jacky is not logged into Nous Portal".
    def _fake_refresh_access_token(*, client, portal_base_url, client_id, refresh_token):
        return {
...
def test_persist_nous_credentials_embeds_custom_label(tmp_path, monkeypatch):
    ...
    Previously `jacky auth add nous --type oauth --label <name>` silently
...
def test_refresh_token_reuse_detection_surfaces_actionable_message():
    ...
    ``error_description`` containing "reuse detected", Jacky must surface an
    ...
    re-authenticate" string led users to report this as a Jacky persistence
    bug when the true cause is external RT consumption (monitoring scripts,
    custom self-heal hooks).
    """
    from jacky_cli.auth import _refresh_access_token
    ...
    assert "jacky auth add nous" in message.lower()
...
def shared_store_env(tmp_path, monkeypatch):
    """Redirect JACKY_SHARED_AUTH_DIR to a tmp_path.
    ...
    monkeypatch.setenv("JACKY_SHARED_AUTH_DIR", str(shared_dir))
    return shared_dir


def test_shared_store_seat_belt_refuses_real_home_under_pytest(monkeypatch):
    """Without JACKY_SHARED_AUTH_DIR override, the seat belt must trip.
    ...
    writing to the user's real ``~/.jacky/shared/`` across CI runs.
    """
    from jacky_cli.auth import _nous_shared_store_path

    monkeypatch.delenv("JACKY_SHARED_AUTH_DIR", raising=False)
...
def test_shared_store_honors_env_override(tmp_path, monkeypatch):
    """JACKY_SHARED_AUTH_DIR must redirect the path."""
    from jacky_cli.auth import _nous_shared_store_path, NOUS_SHARED_STORE_FILENAME

    custom_dir = tmp_path / "custom_shared"
    monkeypatch.setenv("JACKY_SHARED_AUTH_DIR", str(custom_dir))
...
def test_shared_store_read_missing_returns_none(shared_store_env):
    ...
    from jacky_cli.auth import _read_shared_nous_state
...
def test_shared_store_read_malformed_returns_none(shared_store_env):
    ...
    from jacky_cli.auth import _nous_shared_store_path, _read_shared_nous_state
...
def test_shared_store_write_skips_when_refresh_token_missing(shared_store_env):
    ...
    from jacky_cli.auth import _nous_shared_store_path, _write_shared_nous_state
...
def test_persist_nous_credentials_mirrors_to_shared_store(
    ...
    AND the shared store, so a future profile's `jacky auth add nous
    ...
    jacky_home = tmp_path / "jacky"
    jacky_home.mkdir(parents=True, exist_ok=True)
    (jacky_home / "auth.json").write_text(
...
def test_try_import_shared_returns_none_when_store_missing(shared_store_env):
    ...
    from jacky_cli.auth import _try_import_shared_nous_state
...
def test_try_import_shared_rehydrates_on_success(shared_store_env, monkeypatch):
    ...
    assert result["client_id"] == "jacky-cli"
...
def test_shared_store_survives_across_profile_switch(
    ...
    (different JACKY_HOME) sees the same shared state and can rehydrate
    ...
    monkeypatch.setenv("JACKY_HOME", str(profile_a))
    ...
    # Profile B: fresh JACKY_HOME, no auth yet, but the shared store
    ...
    monkeypatch.setenv("JACKY_HOME", str(profile_b))
...
class TestStalePortalBaseUrlMigration:
    ...
    def test_migrates_stale_portal_url_on_load(self, tmp_path, monkeypatch):
        from jacky_cli.auth import _load_auth_store, DEFAULT_NOUS_PORTAL_URL

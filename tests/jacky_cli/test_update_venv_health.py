...
Covers three additions to ``jacky update``:
...
def test_venv_health_missing_venv_unhealthy_on_managed_install(tmp_path):
    ...
    (tmp_path / ".jacky-bootstrap-complete").write_text("done")
    with patch.object(cli_main, "PROJECT_ROOT", tmp_path):
        healthy, detail = cli_main._venv_core_imports_healthy()
...
def test_detect_venv_python_finds_backend(_winp, tmp_path):
    ...
    fake_psutil = types.SimpleNamespace(
        process_iter=lambda attrs: iter(
            [
                _proc(101, venv_py, "python.exe", ["python.exe", "-m", "jacky_cli.main", "serve"]),
...
def test_format_venv_holders_message_flags_desktop_backend(tmp_path):
    matches = [
        (101, "python.exe", "python.exe -m jacky_cli.main serve --host 127.0.0.1"),
        (102, "pythonw.exe", "pythonw.exe -m jacky_cli.main gateway run"),
    ]
    msg = cli_main._format_venv_python_holders_message(matches)
    assert "101" in msg
    assert "desktop app" in msg.lower()
    assert "gateway" in msg
    assert "jacky update" in msg
...
def test_detect_venv_python_catches_outside_venv_trampoline(_winp, tmp_path):
    ...
    clearly runs Jacky from this install → must still be flagged as a holder
    ...
    fake_psutil = types.SimpleNamespace(
        process_iter=lambda attrs: iter(
            [
                ...
                # `-m jacky_cli.main serve` with the install root as cwd
                _proc(
                    202,
                    base_py,
                    "python.exe",
                    [base_py, "-m", "jacky_cli.main", "serve"],
                    cwd=str(tmp_path),
                ),
...
@patch.object(cli_main, "_is_windows", return_value=True)
def test_detect_venv_jacky_cli_cmdline_outside_install_not_matched(_winp, tmp_path):
    """A jacky_cli.main process belonging to a DIFFERENT install (neither
...
def _run_update_until_guard(args):
    ...
    with patch.object(cli_main, "_is_windows", return_value=True), patch.object(
        ...
        cli_main,
        "_detect_venv_python_processes",
        return_value=[(101, "python.exe", "python.exe -m jacky_cli.main serve")],
    ), patch.object(

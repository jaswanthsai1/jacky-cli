...
class TestSlackNativeSlashes:
    ...
    def test_jacky_catchall_is_first(self):
        """``/jacky`` must be reserved as the first slot so the legacy
        ``/jacky <subcommand>`` form keeps working after we add new
        commands and hit the 50-slash cap."""
        slashes = slack_native_slashes()
        assert slashes[0][0] == "jacky"
    ...
    def test_excludes_slack_reserved_commands(self):
        ...
        Users can still reach them via /jacky <command>."""
        names = {n for n, _d, _h in slack_native_slashes()}
    ...
    def test_includes_aliases_as_first_class_slashes(self):
        ...
        aliases stay reachable via ``/jacky <alias>``.
        """
        slashes = slack_native_slashes()
    ...
    def test_telegram_parity(self):
        ...
        # Commands deliberately routed through /jacky <command> on Slack only
        # (Slack's 50-slash cap) are expected to be absent from native slashes.
        via_jacky_norm = {_norm(n) for n in _SLACK_VIA_JACKY_ONLY}
        missing = (tg_norm - slack_norm) - reserved_norm - via_jacky_norm
        assert not missing, (
...
class TestSlackAppManifest:
    """Generated Slack app manifest (used by `jacky slack manifest`)."""
...
class TestSubcommandCompletion:
    ...
    def test_tools_enable_completes_toolset_names(self, monkeypatch):
        ...
        from jacky_cli import commands as commands_mod
        ...
        monkeypatch.setattr(
            "jacky_cli.tools_config._get_plugin_toolset_keys",
            lambda: set(),
        )
...
from jacky_cli.commands import discord_skill_commands_by_category  # noqa: E402
...
class TestPluginCommandEnumeration:
    ...
    def _patch_plugin_commands(self, monkeypatch, commands):
        """Monkeypatch jacky_cli.plugins.get_plugin_commands() to a fixed dict."""
        from jacky_cli import plugins as _plugins_mod
    ...
    def test_plugin_command_appears_in_slack_subcommand_map(self, monkeypatch):
        """/jacky metricas must route through the Slack subcommand map."""
        self._patch_plugin_commands(monkeypatch, {
            "metricas": {
                "handler": lambda _a: "ok",
                "description": "Metrics",

"""Tests for jacky_cli.container_boot — the cont-init.d-time
reconciliation that recreates per-profile gateway s6 service slots
from the persistent profiles directory.

These tests run against a fake $JACKY_HOME under tmp_path; no real
...
from jacky_cli.container_boot import (
    ReconcileAction,
...
def _hermetic_container_argv(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fallback). On a host that is *also* running jacky containers, those
    ...
    """
    monkeypatch.setattr(
        "jacky_cli.container_boot._read_container_argv",
        lambda: (),
    )
...
def _make_profile(
    ...
    *,
    ...
    """Create a fake profile directory under jacky_home/profiles/<name>/."""
    p = jacky_home / "profiles" / name
    p.mkdir(parents=True)
    if config:
        # SOUL.md is what the reconciler keys on — it's always seeded by
        # `jacky profile create`. See container_boot._render_run_script.
...
def _seed_default_root(
    ...
    *,
    ...
    JACKY_HOME root (the implicit default profile)."""
    if state is not None:
        (jacky_home / "gateway_state.json").write_text(json.dumps({
            "gateway_state": state, "timestamp": 1234567890,
        }))
    if with_pid:
        (jacky_home / "gateway.pid").write_text(json.dumps(
            {"pid": 99999, "host": "old-container"},
        ))
        (jacky_home / "processes.json").write_text("[]")
...
def test_running_profile_is_registered_and_autostarted(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        jacky_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_reconcile_log_rotates_when_size_exceeded(
    ...
    from jacky_cli import container_boot
...
def test_dry_run_makes_no_filesystem_changes(tmp_path: Path) -> None:
    ...
    actions = reconcile_profile_gateways(
        jacky_home=tmp_path, scandir=scandir, dry_run=True,
    )
...
def test_missing_profiles_root_still_registers_default_slot(
    ...
    """When $JACKY_HOME/profiles doesn't exist (fresh install), the
    ...
    so `jacky gateway start` (no -p) has somewhere to land."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
    actions = reconcile_profile_gateways(
        jacky_home=tmp_path, scandir=scandir, dry_run=False,
    )
...
def test_default_slot_always_registered_on_empty_home(tmp_path: Path) -> None:
    """Bare JACKY_HOME with nothing under it still produces a
...
def test_default_slot_run_script_omits_profile_flag(tmp_path: Path) -> None:
    ...
    that would resolve to $JACKY_HOME/profiles/default/ instead of
    the root profile. It must call `jacky gateway run` directly."""
    scandir = tmp_path / "run-service"; scandir.mkdir()
...
@pytest.mark.parametrize(
    ...
    [
        ("gateway", "run"),
        ("/init", "/opt/jacky/docker/main-wrapper.sh", "gateway", "run"),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("/init", "/opt/jacky/docker/main-wrapper.sh", "gateway", "run", "--no-supervise"),
    ],
)
...
def test_legacy_gateway_run_env_no_supervise_does_not_seed_s6_state(
    ...
    scandir = tmp_path / "run-service"; scandir.mkdir()
    monkeypatch.setenv("JACKY_GATEWAY_NO_SUPERVISE", "1")
...
def test_default_slot_cleans_up_stale_runtime_files_at_root(
    ...
    """gateway.pid and processes.json at the JACKY_HOME root (left
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Through s6 /init + the main-wrapper that re-execs `jacky`.
        ("/init", "/opt/jacky/docker/main-wrapper.sh", "dashboard"),
        (
            "/init",
            "/opt/jacky/docker/main-wrapper.sh",
            "dashboard",
            "--host",
            "127.0.0.1",
            "--no-open",
        ),
        # Wrapper that kept the explicit `jacky` argv0.
        ("/init", "/opt/jacky/docker/main-wrapper.sh", "jacky", "dashboard"),
...
def test_is_dashboard_container_true_for_dashboard_argv(
    ...
    from jacky_cli.container_boot import _is_dashboard_container
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("gateway", "run"),
        ("/init", "/opt/jacky/docker/main-wrapper.sh", "gateway", "run"),
        ("/init", "/opt/jacky/docker/main-wrapper.sh", "jacky", "gateway", "run"),
        ("chat",),
...
def test_main_skips_reconcile_in_dashboard_container(
    ...
    monkeypatch.setattr(
        ...
        lambda: ("/init", "/opt/jacky/docker/main-wrapper.sh", "dashboard"),
    )
...
def test_main_reconciles_in_gateway_container(
    ...
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/jacky/docker/main-wrapper.sh", "gateway", "run"),
    )
...
def test_main_ignores_removed_skip_reconcile_env_var(
    ...
    """The legacy JACKY_SKIP_PROFILE_RECONCILE flag is gone: setting it on a
    ...
    monkeypatch.setenv("JACKY_HOME", str(tmp_path))
    monkeypatch.setenv("S6_PROFILE_GATEWAY_SCANDIR", str(scandir))
    monkeypatch.setenv("JACKY_SKIP_PROFILE_RECONCILE", "1")
    monkeypatch.setattr(
        container_boot,
        "_read_container_argv",
        lambda: ("/init", "/opt/jacky/docker/main-wrapper.sh", "gateway", "run"),

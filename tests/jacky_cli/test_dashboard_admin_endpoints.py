...
def _client():
    ...
    # Keep the state DB under the isolated JACKY_HOME for any handler that
    # touches it.
...
class TestSystemStatsEndpoint:
    ...
    def test_stats_shape(self):
        ...
        for key in ("os", "arch", "hostname", "python_version", "jacky_version"):
    ...
    def test_sources_lists_configured_hubs(self, monkeypatch):
        ...
        class _Src:
            ...
            def search(self, q, limit=10):
                return [_FakeMeta("jacky-index/featured-skill", "trusted")]

        def _fake_router():
            srcs = [_Src("official"), _Src("github")]
            # jacky-index source advertises availability + featured search.
            idx = _Src("jacky-index")
        ...
        assert {"official", "github", "jacky-index"} <= ids
...
class TestSkillsHubPreviewEndpoint:
    ...
    def test_preview_returns_skill_md_text(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "jacky_cli.skills_hub._resolve_source_meta_and_bundle",
...
class TestUpdateCheckEndpoint:
    """``GET /api/jacky/update/check`` reports availability without applying.

    Powers the dashboard's check-before-you-update flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/jacky/update`` runs ``jacky update``.
    """
    ...
    def test_git_install_reports_behind_count(self, monkeypatch):
        ...
        r = self.client.get("/api/jacky/update/check")
        assert r.status_code == 200
        body = r.json()
        assert {
            "install_method",
    ...
    def test_up_to_date(self, monkeypatch):
        ...
        body = self.client.get("/api/jacky/update/check").json()
...
class TestDebugShareEndpoint:
    ...
    def test_returns_structured_urls(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.dump.run_dump", lambda a: None)
...
class TestToolsConfigEndpoints:
    ...
    the dashboard surface that replicates the `jacky tools` configurator."""
    ...
    def test_post_setup_spawns_action(self, monkeypatch):
        ...
        monkeypatch.setattr(ws, "_spawn_jacky_action", _fake_spawn)
        r = self.client.post(

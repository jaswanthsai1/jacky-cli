...
``jacky update`` killed mid-install (Ctrl-C, terminal close, WSL OOM) gets
...
def test_recovery_runs_install_and_clears_marker(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.managed_uv.ensure_uv", lambda: None)
    monkeypatch.setattr(
        m,
        "_install_python_dependencies_with_optional_fallback",
...
def test_recovery_self_lock_guard_clears_marker_without_install(tmp_path, monkeypatch):
    # Windows self-lock: jacky.exe is an ancestor of this Python process, so a
    ...
    monkeypatch.setattr(m, "_jacky_exe_shims", lambda d: [shim])
...
def test_recovery_self_lock_guard_inactive_when_not_ancestor(tmp_path, monkeypatch):
    # Windows, but jacky.exe is NOT in the ancestry (launched via `jacky
    ...
    class FakeProc:
        ...
        def parents(self):
            # Ancestry is plain pythons / cmd — no jacky.exe shim.

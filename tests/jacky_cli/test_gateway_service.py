...
import jacky_cli.gateway as gateway_cli
...
class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "jacky-gateway.service"
    ...
    def test_systemd_stop_timeout_prints_status_guidance(self, monkeypatch, capsys):
        ...
        assert "jacky gateway status" in output

    def test_systemd_restart_timeout_prints_status_guidance(self, monkeypatch, capsys):
        """`jacky gateway restart` must not surface a raw TimeoutExpired traceback.

        The dashboard spawns `jacky gateway restart` in the background; when a
    ...
    def test_run_gateway_refreshes_outdated_unit_on_boot(self, tmp_path, monkeypatch):
        ...
        via exit-code-75 (bypassing `jacky gateway restart`)."""
        unit_path = tmp_path / "jacky-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
    ...
    def test_refresh_refuses_to_bake_pytest_tmpdir_into_real_user_unit(
        ...
        ``generate_systemd_unit()`` bakes ``JACKY_HOME`` into the unit's
        ...
        ``/tmp/pytest-of-.../jacky_test`` JACKY_HOME — silently breaking
        ...
        # Realistic generated unit referencing a pytest tmpdir JACKY_HOME
        polluted_unit = (
            "[Service]\n"
            'Environment="JACKY_HOME=/tmp/pytest-of-alice/pytest-42/'
            'popen-gw0/test_x/jacky_test"\n'
        )
        monkeypatch.setattr(
            gateway_cli,
    ...
    def test_refresh_refuses_to_bake_any_tempdir_home_into_real_user_unit(
        ...
        """Structural guard: a manual E2E JACKY_HOME like
        ``/tmp/jacky-e2e-41264`` carries none of the pytest markers but
        poisons the unit identically (seen live 2026-06-11 — an E2E probe ran
        ``jacky gateway restart`` with a /tmp JACKY_HOME exported; the
        ...
        must refuse ANY temp-dir JACKY_HOME, not just pytest-shaped ones.
        """
        unit_path = tmp_path / "jacky-gateway.service"
        unit_path.write_text("old unit\n", encoding="utf-8")
        ...
        polluted_unit = (
            "[Service]\n"
            'Environment="JACKY_HOME=/tmp/jacky-e2e-41264"\n'
            "WorkingDirectory=/tmp/jacky-e2e-41264\n"
        )
        monkeypatch.setattr(
            gateway_cli,
            "generate_systemd_unit",
...
class TestTempHomeServiceDefinitionGuard:
    ...
    def test_detects_var_tmp_home(self):
        unit = '[Service]\nEnvironment="JACKY_HOME=/var/tmp/jacky-x"\n'
    ...
    def test_detects_tempdir_env_home(self, monkeypatch, tmp_path):
        ...
        unit = f'[Service]\nEnvironment="JACKY_HOME={tmp_path}/jacky-home"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is not None

    def test_detects_tmp_home_in_launchd_plist(self):
        plist = (
            "<dict>\n  <key>JACKY_HOME</key>\n"
            "  <string>/tmp/jacky-e2e-99999</string>\n</dict>\n"
        )
        assert (
    ...
    def test_tmp_prefixed_non_temp_path_is_accepted(self):
        ...
        unit = '[Service]\nEnvironment="JACKY_HOME=/tmpfs-data/.jacky"\n'
        assert gateway_cli._temp_home_in_service_definition(unit) is None
...
class TestRequireServiceInstalled:
    def test_exits_with_install_hint_when_unit_missing(self, tmp_path, monkeypatch, capsys):
        ...
        assert "jacky gateway install" in out
...
class TestGeneratedSystemdUnits:
    ...
    def test_user_unit_does_not_leak_profile_node_symlink_target(self, tmp_path, monkeypatch):
        ...
        profile_node_bin = tmp_path / ".jacky" / "profiles" / "jarvis" / "node" / "bin"
    ...
    def test_system_unit_includes_wsl_windows_interop_paths(self, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "_jacky_home_for_target_user", lambda home: "/home/alice/.jacky")
        monkeypatch.setenv("PATH", "/usr/local/bin:/mnt/c/WINDOWS/system32")
        monkeypatch.setattr(gateway_cli.shutil, "which", lambda cmd: None)
...
class TestLaunchdServiceRecovery:
    def test_get_restart_drain_timeout_prefers_env_then_config_then_default(self, monkeypatch):
        monkeypatch.delenv("JACKY_RESTART_DRAIN_TIMEOUT", raising=False)
        ...
        monkeypatch.setenv("JACKY_RESTART_DRAIN_TIMEOUT", "9")
        assert gateway_cli._get_restart_drain_timeout() == 9.0

        monkeypatch.setenv("JACKY_RESTART_DRAIN_TIMEOUT", "invalid")
        assert (
            gateway_cli._get_restart_drain_timeout()
            == DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT
        )
    ...
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.jacky.gateway.plist"
        ...
        # JACKY_HOME resolves under the (pytest tmp) test JACKY_HOME.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist>--replace\n<key>JACKY_HOME</key>"
                "<string>/Users/alice/.jacky</string></plist>"
            ),
        )
    ...
    def test_launchd_install_falls_back_to_detached_on_bootstrap_5(self, tmp_path, monkeypatch, capsys):
        ...
        # (which would trip on the pytest-tmp test JACKY_HOME) stays out of
        # the way — this test exercises the bootstrap-error fallback.
        monkeypatch.setattr(
            gateway_cli,
            "generate_launchd_plist",
            lambda: (
                "<plist><key>JACKY_HOME</key>"
                "<string>/Users/alice/.jacky</string></plist>"
            ),
        )
    ...
    def test_launchd_fallback_exits_when_spawn_fails(self, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "nohup jacky gateway run" in out
    ...
    def test_launchd_unsupported_marker_write_and_clear(self, tmp_path, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_jacky_home", lambda: tmp_path)
...
class TestGatewaySystemServiceRouting:
    ...
    def test_systemd_status_surfaces_planned_restart_failure(self, monkeypatch, capsys):
        ...
        monkeypatch.setattr(gateway_cli, "has_legacy_jacky_units", lambda: False)
    ...
    def test_gateway_install_reports_termux_manual_mode(self, monkeypatch, capsys):
        ...
        assert "Run manually: jacky gateway" in out
    ...
    def test_gateway_status_on_termux_shows_manual_guidance(self, monkeypatch, capsys):
        ...
        assert "nohup jacky gateway" in out
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.jacky.gateway"])
            ),
        )
...
class TestSystemUnitJackyHome:
    """JACKY_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'JACKY_HOME=/home/alice/.jacky' in unit
        assert '/root/.jacky' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: JACKY_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JACKY_HOME", "/root/.jacky/profiles/coder")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'JACKY_HOME=/home/alice/.jacky/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_jacky_home(self, monkeypatch):
        # Custom JACKY_HOME not under any user's home — keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JACKY_HOME", "/opt/jacky-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'JACKY_HOME=/opt/jacky-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's JACKY_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        jacky_home = str(gateway_cli.get_jacky_home().resolve())
        assert f'JACKY_HOME={jacky_home}' in unit


class TestSystemUnitRefreshSyncsJackyHome:
    """sudo system refresh must not flip TimeoutStopSec via /root/.jacky."""

    def test_refresh_adopts_unit_jacky_home_before_rewriting(self, tmp_path, monkeypatch):
        root_home = tmp_path / "root"
        alice_home = tmp_path / "alice"
        root_jacky = root_home / ".jacky"
        alice_jacky = alice_home / ".jacky"
        root_jacky.mkdir(parents=True)
        alice_jacky.mkdir(parents=True)
        (root_jacky / "config.yaml").write_text(
            "agent:\n  restart_drain_timeout: 60\n", encoding="utf-8"
        )
        (alice_jacky / "config.yaml").write_text(
        ...
        # Correct installed unit (operator's JACKY_HOME + drain timeout).
        monkeypatch.setenv("JACKY_HOME", str(alice_jacky))
        ...
        # Simulate sudo without inherited JACKY_HOME (falls back to root).
        monkeypatch.setenv("JACKY_HOME", str(root_jacky))
        ...
        assert os.environ["JACKY_HOME"] == str(alice_jacky)
    ...
    def test_is_current_syncs_before_reading_unit(self, tmp_path, monkeypatch):
        ...
        unit's pinned JACKY_HOME *before* it reads/compares the unit.
        ...
        monkeypatch.setattr(gateway_cli, "_sync_jacky_home_from_systemd_unit", tracking_sync)
    ...
    def test_start_and_restart_delegate_sync_to_chokepoint(self, monkeypatch):
        ...
        for entry in ("systemd_start", "systemd_restart"):
            ...
            monkeypatch.setattr(
                gateway_cli,
                "_sync_jacky_home_from_systemd_unit",
...
class TestJackyHomeForTargetUser:
    """Unit tests for _jacky_home_for_target_user()."""

    def test_remaps_default_home(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.delenv("JACKY_HOME", raising=False)

        result = gateway_cli._jacky_home_for_target_user("/home/alice")
        assert result == "/home/alice/.jacky"
    ...
    def test_remaps_profile_path(self, monkeypatch):
        ...
        result = gateway_cli._jacky_home_for_target_user("/home/alice")
        assert result == "/home/alice/.jacky/profiles/coder"

    def test_keeps_custom_path(self, monkeypatch):
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("JACKY_HOME", "/opt/jacky")

        result = gateway_cli._jacky_home_for_target_user("/home/alice")
        assert result == "/opt/jacky"
...
class TestPreflightUserSystemd:
    ...
    def test_raises_when_linger_disabled_and_loginctl_denied(self, monkeypatch):
        ...
        assert "jacky gateway run" in msg  # foreground fallback mentioned
...
class TestProfileArg:
    ...
    def test_default_jacky_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.jacky should not produce a --profile flag."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        result = gateway_cli._profile_arg(str(jacky_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.jacky/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".jacky" / "profiles" / "mybot"
    ...
    def test_named_profile_under_target_user_root_returns_flag(self, tmp_path):
        ...
        target_root = tmp_path / "home" / "alice" / ".jacky"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile JACKY_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "jacky"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.jacky/profiles/mybot/subdir should NOT match — too deep."""
        nested = tmp_path / ".jacky" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".jacky" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_jacky_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_systemd_unit_for_target_user_includes_named_profile(self, tmp_path, monkeypatch):
        ...
        root_profile = root_home / ".jacky" / "profiles" / "mybot"
        root_profile.mkdir(parents=True)

        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("JACKY_HOME", str(root_profile))
        monkeypatch.setattr(gateway_cli, "get_jacky_home", lambda: root_profile)
        monkeypatch.setattr(
            gateway_cli,
            "_system_service_identity",
        ...
        assert f'JACKY_HOME={target_home / ".jacky" / "profiles" / "mybot"}' in unit
    ...
    def test_launchd_plist_path_uses_real_user_home_not_profile_home(self, tmp_path, monkeypatch):
        profile_dir = tmp_path / ".jacky" / "profiles" / "orcha"
        profile_dir.mkdir(parents=True)
        ...
        assert plist_path == machine_home / "Library" / "LaunchAgents" / "ai.jacky.gateway-orcha.plist"
...
class TestRemapPathForUser:
    ...
    def test_remaps_path_under_current_home(self, monkeypatch, tmp_path):
        ...
        result = gateway_cli._remap_path_for_user(
            str(tmp_path / "root" / ".jacky" / "jacky-agent"),
            str(tmp_path / "alice"),
        )
        assert result == str(tmp_path / "alice" / ".jacky" / "jacky-agent")

    def test_keeps_system_path_unchanged(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "root")
        (tmp_path / "root").mkdir()
        result = gateway_cli._remap_path_for_user("/opt/jacky", str(tmp_path / "alice"))
        assert result == "/opt/jacky"

    def test_noop_when_same_user(self, monkeypatch, tmp_path):
        monkeypatch.setattr(Path, "home", lambda: tmp_path / "alice")
        (tmp_path / "alice").mkdir()
        original = str(tmp_path / "alice" / ".jacky" / "jacky-agent")
...
class TestSystemUnitPathRemapping:
    ...
    def test_system_unit_has_no_root_paths(self, monkeypatch, tmp_path):
        root_home = tmp_path / "root"
        root_home.mkdir()
        project = root_home / ".jacky" / "jacky-agent"
        ...
        monkeypatch.setattr(Path, "home", lambda: root_home)
        monkeypatch.setenv("JACKY_HOME", str(root_home / ".jacky"))
        monkeypatch.setattr(gateway_cli, "get_jacky_home", lambda: root_home / ".jacky")
        ...
        # WorkingDirectory is anchored at the target user's JACKY_HOME (stable,
        ...
        assert "WorkingDirectory=/home/alice/.jacky" in unit
        assert "WorkingDirectory=/home/alice/.jacky/jacky-agent" not in unit
...
class TestDockerAwareGateway:
    ...
    def test_run_systemctl_raises_runtimeerror_when_missing(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="systemctl is not available"):
            gateway_cli._run_systemctl(["start", "jacky-gateway"])
    ...
    def test_run_systemctl_passes_through_on_success(self, monkeypatch):
        ...
        result = gateway_cli._run_systemctl(["status", "jacky-gateway"])
        assert result.returncode == 0
        assert len(calls) == 1
        assert "status" in calls[0]

    def test_install_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'jacky gateway install' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_uninstall_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'jacky gateway uninstall' inside Docker exits 0 with container guidance."""
        import pytest
    ...
    def test_start_in_container_prints_docker_guidance(self, monkeypatch, capsys):
        """'jacky gateway start' inside Docker exits 0 with container guidance."""
        import pytest
        ...
        out = capsys.readouterr().out
        assert "docker" in out.lower()
        assert "jacky gateway run" in out


class TestLegacyJackyUnitDetection:
    """Tests for _find_legacy_jacky_units / has_legacy_jacky_units.

    These guard against the scenario that tripped Luis in April 2026: an
    older install left a ``jacky.service`` unit behind when the service was
    renamed to ``jacky-gateway.service``. After PR #5646 (signal recovery
    via systemd), the two services began SIGTERM-flapping over the same
    Telegram bot token in a 30-second cycle.

    The detector must flag ``jacky.service`` ONLY when it actually runs our
    gateway, and must NEVER flag profile units
    (``jacky-gateway-<profile>.service``) or unrelated third-party services.
    """

    # Minimal ExecStart that looks like our gateway
    _OUR_UNIT_TEXT = (
        "[Unit]\nDescription=Jacky Gateway\n[Service]\n"
        "ExecStart=/usr/bin/python -m jacky_cli.main gateway run --replace\n"
    )
    ...
    def test_detects_legacy_jacky_service_in_user_scope(self, tmp_path, monkeypatch):
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = user_dir / "jacky.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")

        results = gateway_cli._find_legacy_jacky_units()

        assert len(results) == 1
        name, path, is_system = results[0]
        assert name == "jacky.service"
        assert path == legacy
        assert is_system is False
        assert gateway_cli.has_legacy_jacky_units() is True

    def test_detects_legacy_jacky_service_in_system_scope(self, tmp_path, monkeypatch):
        _, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        legacy = system_dir / "jacky.service"
        legacy.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_ignores_profile_unit_jacky_gateway_coder(self, tmp_path, monkeypatch):
        """CRITICAL: profile units must NOT be flagged as legacy.

        Teknium's concern — ``jacky-gateway-coder.service`` is our standard
        ...
        for base in (user_dir, system_dir):
            (base / "jacky-gateway-coder.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "jacky-gateway-orcha.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
            (base / "jacky-gateway.service").write_text(
                self._OUR_UNIT_TEXT, encoding="utf-8"
            )
        ...
        assert results == []
        assert gateway_cli.has_legacy_jacky_units() is False

    def test_ignores_unrelated_jacky_service(self, tmp_path, monkeypatch):
        """Third-party ``jacky.service`` that isn't ours stays untouched.

        If a user has some other package named ``jacky`` installed as a
        service, we must not flag it.
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "jacky.service").write_text(
            "[Unit]\nDescription=Some Other Jacky\n[Service]\n"
            "ExecStart=/opt/other-jacky/bin/daemon --foreground\n",
            encoding="utf-8",
        )
    ...
    def test_returns_empty_when_no_legacy_files_exist(self, tmp_path, monkeypatch):
        ...
        assert gateway_cli._find_legacy_jacky_units() == []
        assert gateway_cli.has_legacy_jacky_units() is False
    ...
    def test_detects_both_scopes_simultaneously(self, tmp_path, monkeypatch):
        ...
        user_dir, system_dir = self._setup_search_paths(tmp_path, monkeypatch)
        (user_dir / "jacky.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        (system_dir / "jacky.service").write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
    ...
    def test_accepts_alternate_execstart_formats(self, tmp_path, monkeypatch):
        ...
        ExecStart variants we've seen in the wild:
          - python -m jacky_cli.main gateway run
          - python path/to/jacky_cli/main.py gateway run
          - jacky gateway run   (direct binary)
          - python path/to/gateway/run.py
        """
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        variants = [
            "ExecStart=/venv/bin/python -m jacky_cli.main gateway run --replace",
            "ExecStart=/venv/bin/python /opt/jacky/jacky_cli/main.py gateway run",
            "ExecStart=/usr/local/bin/jacky gateway run --replace",
            "ExecStart=/venv/bin/python /opt/jacky/gateway/run.py",
        ]
        for i, execstart in enumerate(variants):
            name = "jacky.service" if i == 0 else "jacky.service"  # same name
            # Test each variant fresh
            (user_dir / "jacky.service").write_text(
                f"[Unit]\nDescription=Old Jacky\n[Service]\n{execstart}\n",
                encoding="utf-8",
            )
            results = gateway_cli._find_legacy_jacky_units()
    ...
    def test_print_legacy_unit_warning_shows_migration_hint(self, tmp_path, monkeypatch, capsys):
        ...
        assert "jacky.service" in out
        assert "jacky gateway migrate-legacy" in out
    ...
    def test_handles_unreadable_unit_file_gracefully(self, tmp_path, monkeypatch):
        ...
        user_dir, _ = self._setup_search_paths(tmp_path, monkeypatch)
        unreadable = user_dir / "jacky.service"
...
class TestRemoveLegacyJackyUnits:
    """Tests for remove_legacy_jacky_units (the migration action)."""
    ...
    def test_returns_zero_when_no_legacy_units(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_jacky_units(interactive=False)
    ...
    def test_dry_run_lists_without_removing(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_jacky_units(
    ...
    def test_removes_user_scope_legacy_unit(self, tmp_path, monkeypatch, capsys):
        ...
        assert any("--user stop jacky.service" in c for c in cmds_joined)
        assert any("--user disable jacky.service" in c for c in cmds_joined)
    ...
    def test_system_scope_without_root_defers_removal(self, tmp_path, monkeypatch, capsys):
        ...
        out = capsys.readouterr().out
        assert "sudo jacky gateway migrate-legacy" in out
    ...
    def test_system_scope_with_root_removes(self, tmp_path, monkeypatch, capsys):
        ...
        assert any(
            c.startswith("systemctl stop jacky.service") for c in cmds_joined
        )
        assert any(
            c.startswith("systemctl disable jacky.service") for c in cmds_joined
        )
    ...
    def test_removes_both_scopes_with_root(self, tmp_path, monkeypatch, capsys):
        ...
        user_legacy = user_dir / "jacky.service"
        system_legacy = system_dir / "jacky.service"
    ...
    def test_does_not_touch_profile_units_during_migration(
        ...
        """Teknium's constraint: profile units (jacky-gateway-coder.service)
        ...
        profile_unit = user_dir / "jacky-gateway-coder.service"
        profile_unit.write_text(self._OUR_UNIT_TEXT, encoding="utf-8")
        default_unit = user_dir / "jacky-gateway.service"
    ...
    def test_interactive_prompt_no_skips_removal(self, tmp_path, monkeypatch, capsys):
        ...
        removed, remaining = gateway_cli.remove_legacy_jacky_units(interactive=True)
...
class TestMigrateLegacyCommand:
    """Tests for the `jacky gateway migrate-legacy` subcommand dispatch."""
    ...
    def test_migrate_legacy_subparser_accepts_dry_run_and_yes(self):
        ...
        import jacky_cli.main as cli_main
        ...
        # public, just confirm that `jacky gateway --help` shows it.
        import subprocess
        import sys

        project_root = cli_main.PROJECT_ROOT if hasattr(cli_main, "PROJECT_ROOT") else None
        if project_root is None:
            import jacky_cli.gateway as gw
            project_root = gw.PROJECT_ROOT

        result = subprocess.run(
            [sys.executable, "-m", "jacky_cli.main", "gateway", "--help"],
    ...
    def test_gateway_command_migrate_legacy_dispatches(
        ...
        monkeypatch.setattr(gateway_cli, "remove_legacy_jacky_units", fake_remove)
        monkeypatch.setattr(gateway_cli, "supports_systemd_services", lambda: True)
        monkeypatch.setattr(gateway_cli, "is_macos", lambda: False)
...
class TestGatewayStatusParser:
    def test_gateway_status_subparser_accepts_full_flag(self):
        ...
        result = subprocess.run(
            [sys.executable, "-m", "jacky_cli.main", "gateway", "status", "-l", "--help"],
...
class TestSystemdInstallOffersLegacyRemoval:
    ...
    def test_install_offers_removal_when_legacy_detected(
        ...
        # has_legacy_jacky_units must return True
        monkeypatch.setattr(gateway_cli, "has_legacy_jacky_units", lambda: True)
        monkeypatch.setattr(gateway_cli, "remove_legacy_jacky_units", fake_remove)
...
class TestSystemScopeWizardPreCheck:
    ...
    def _setup_units(tmp_path, monkeypatch, system_present: bool, user_present: bool):
        ...
        usr_dir.mkdir()
        if system_present:
            (sys_dir / "jacky-gateway.service").write_text("[Unit]\n")
        if user_present:
            (usr_dir / "jacky-gateway.service").write_text("[Unit]\n")
        monkeypatch.setattr(
            gateway_cli,
            "get_systemd_unit_path",
            lambda system=False: (sys_dir if system else usr_dir) / "jacky-gateway.service",
        )
    ...
    def test_non_root_with_explicit_system_arg_returns_true(self, tmp_path, monkeypatch):
        # Caller passed system=True explicitly (e.g. ``jacky gateway start --system``).
        self._setup_units(tmp_path, monkeypatch, system_present=False, user_present=False)
        monkeypatch.setattr(gateway_cli.os, "geteuid", lambda: 1000)
...
class TestSystemScopeRemediationOutput:
    ...
    def test_start_remediation_mentions_sudo_systemctl_and_uninstall(self, capsys, monkeypatch):
        monkeypatch.setattr(gateway_cli, "get_service_name", lambda: "jacky-gateway")
        ...
        assert "sudo systemctl start jacky-gateway" in out
        assert "sudo jacky gateway uninstall --system" in out
        assert "jacky gateway install" in out
    ...
    def test_restart_remediation_uses_systemctl_restart(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl restart jacky-gateway" in out
    ...
    def test_stop_remediation_uses_systemctl_stop(self, capsys, monkeypatch):
        ...
        assert "sudo systemctl stop jacky-gateway" in out


class TestGatewayCommandCatchesSystemScopeError:
    """The direct CLI path (``jacky gateway start --system`` etc.) must
...
class TestServiceWorkingDirIsStable:
    ...
    (JACKY_HOME), never the source checkout / worktree, so a relocated or
    deleted checkout can't crash-loop the unit on CHDIR (status=200).
    """

    def test_stable_working_dir_uses_jacky_home(self, tmp_path, monkeypatch):
        home = tmp_path / ".jacky"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_jacky_home", lambda: home)
    ...
    def test_stable_working_dir_falls_back_to_project_root(self, tmp_path, monkeypatch):
        # JACKY_HOME points somewhere that does not exist -> fall back.
        missing = tmp_path / "does-not-exist" / ".jacky"
        monkeypatch.setattr(gateway_cli, "get_jacky_home", lambda: missing)
        assert gateway_cli._stable_service_working_dir() == str(gateway_cli.PROJECT_ROOT)

    def test_user_unit_workingdirectory_is_jacky_home_not_checkout(self, tmp_path, monkeypatch):
        home = tmp_path / ".jacky"
        home.mkdir()
        monkeypatch.setattr(gateway_cli, "get_jacky_home", lambda: home)
        unit = gateway_cli.generate_systemd_unit(system=False)
    ...
    def test_launchd_workingdirectory_is_jacky_home(self, tmp_path, monkeypatch):
        import re
...
class TestLaunchctlBootstrapEioRetry:
    ...
    PLIST = "/tmp/ai.jacky.gateway.plist"
    DOMAIN = "gui/501"
    LABEL = "ai.jacky.gateway"

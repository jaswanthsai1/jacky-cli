...
def test_main_top_level_tui_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["jacky", "--tui", "--toolsets", "web,terminal"])
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.plugins",
...
def test_termux_fast_tui_launch_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["jacky", "--tui", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
        main_mod,
        "cmd_chat",
...
def test_termux_fast_tui_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(sys, "argv", ["jacky", "--tui", "--help"])
...
def test_fast_tui_launch_is_termux_only(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(sys, "argv", ["jacky", "--tui"])
...
def test_termux_fast_cli_launch_chat_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("JACKY_TUI", raising=False)
    monkeypatch.setattr(
        sys, "argv", ["jacky", "chat", "-q", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setattr(
...
def test_termux_fast_cli_launch_bare_defers_agent_startup(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("JACKY_TUI", raising=False)
    monkeypatch.delenv("JACKY_DEFER_AGENT_STARTUP", raising=False)
    monkeypatch.delenv("JACKY_FAST_STARTUP_BANNER", raising=False)
    monkeypatch.setattr(sys, "argv", ["jacky"])
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
    monkeypatch.setattr(
    ...
    assert os.environ["JACKY_DEFER_AGENT_STARTUP"] == "1"
    assert os.environ["JACKY_FAST_STARTUP_BANNER"] == "1"
...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["jacky", "-z", "hello", "--model", "gpt-test", "--provider", "openai"],
    )
    monkeypatch.setattr(
        main_mod, "_prepare_agent_startup", lambda args: prepared.append(args.command)
    )
...
def test_termux_fast_cli_launch_version_skips_update_check(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("JACKY_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["jacky", "version"])
    monkeypatch.setattr(
...
def test_termux_ultrafast_version_runs_before_heavy_startup(
    ...
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("JACKY_TERMUX_DISABLE_FAST_CLI", raising=False)
    monkeypatch.setattr(sys, "argv", ["jacky", "--version"])

    assert main_mod._try_termux_ultrafast_version() is True

    out = capsys.readouterr().out
    assert "Jacky Agent v" in out
...
def test_termux_fast_cli_launch_skips_help(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.delenv("JACKY_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["jacky", "chat", "--help"])
...
def test_termux_fast_cli_launch_can_be_disabled(monkeypatch, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setenv("JACKY_TERMUX_DISABLE_FAST_CLI", "1")
    monkeypatch.delenv("JACKY_TUI", raising=False)
    monkeypatch.setattr(sys, "argv", ["jacky", "version"])
...
def test_termux_bundled_skills_stamp_controls_sync(monkeypatch, tmp_path, main_mod):
    monkeypatch.setenv("TERMUX_VERSION", "1")
    monkeypatch.setattr(main_mod, "get_jacky_home", lambda: tmp_path)
    ...
    monkeypatch.setenv("JACKY_TERMUX_FORCE_SKILLS_SYNC", "1")
    assert main_mod._termux_bundled_skills_sync_needed() is True
...
def test_main_top_level_oneshot_accepts_toolsets(monkeypatch, main_mod):
    ...
    monkeypatch.setattr(
        sys, "argv", ["jacky", "-z", "hello", "--toolsets", "web,terminal"]
    )
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.plugins",
...
def test_oneshot_fails_closed_on_empty_final_response(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    import jacky_cli.oneshot as oneshot_mod
...
def test_oneshot_rejects_disabled_mcp_toolset(monkeypatch, capsys):
    ...
    assert valid is None
    assert error == "jacky -z: --toolsets did not contain any valid toolsets.\n"
    err = capsys.readouterr().err
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    """jacky -z bypasses JackyCLI, but recall still needs SessionDB."""
    from jacky_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.config",
        mod("jacky_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.models",
        mod("jacky_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.tools_config",
        mod("jacky_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exports_model_provider_and_toolsets(monkeypatch, main_mod):
    ...
    def fake_call(argv, cwd=None, env=None):
        ...
        active_path_during_call = Path(env["JACKY_TUI_ACTIVE_SESSION_FILE"])
    ...
    with pytest.raises(SystemExit):
        main_mod._launch_tui(
            model="nous/jacky-test", provider="nous", toolsets="web, terminal"
        )

    env = captured["env"]
    assert env["JACKY_MODEL"] == "nous/jacky-test"
    assert env["JACKY_INFERENCE_MODEL"] == "nous/jacky-test"
    assert env["JACKY_TUI_PROVIDER"] == "nous"
    assert env["JACKY_INFERENCE_PROVIDER"] == "nous"
    assert env["JACKY_TUI_TOOLSETS"] == "web,terminal"
    active_path = Path(env["JACKY_TUI_ACTIVE_SESSION_FILE"])
    assert active_path.name.startswith("jacky-tui-active-session-")
...
def test_launch_tui_applies_terminal_backend_config(
    monkeypatch, main_mod, _isolate_jacky_home
):
    captured = {}
    ...
    assert captured["env"]["TERMINAL_DOCKER_IMAGE"] == "example/jacky-tools:latest"
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("jacky_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_launch_tui_drops_stale_resume_env_without_resume_arg(monkeypatch, main_mod):
    ...
    monkeypatch.setenv("JACKY_TUI_RESUME", "stale-missing-session")
    monkeypatch.setattr(
        main_mod,
        "_make_tui_argv",
        lambda tui_dir, tui_dev: (["node", "dist/entry.js"], Path(".")),
    ...
    assert "JACKY_TUI_RESUME" not in captured["env"]
...
def test_launch_tui_sets_resume_env_from_resume_arg(monkeypatch, main_mod):
    ...
    assert captured["env"]["JACKY_TUI_RESUME"] == "20260518_000000_goodid"


def test_make_tui_argv_dev_prebuilds_jacky_ink(monkeypatch, main_mod, tmp_path):
    ...
    monkeypatch.delenv("JACKY_TUI_DIR", raising=False)
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    assert "jacky --tui --resume 20260409_000001_abc123" in out
    assert 'jacky --tui -c "demo title"' in out
...
def test_print_tui_exit_summary_prefers_actual_active_session_file(
    ...
    assert "jacky --tui --resume actual_session" in out

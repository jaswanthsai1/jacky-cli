...
Covers:
 - Normal path (tokens already in Jacky auth store)
 - Claude Code fallback (tokens only in ~/.claude/.credentials.json)
 - Negative case (no credentials anywhere)

Note: auto-import from ~/.codex/auth.json was removed in #12360 — Jacky
now owns its own openai-codex auth state, and users explicitly adopt
existing Codex CLI tokens via `jacky auth openai-codex`. The old
...
@pytest.fixture()
def jacky_auth_only_env(tmp_path, monkeypatch):
    """Tokens already in Jacky auth store (no Codex CLI needed)."""
...
def test_normal_path_still_works(jacky_auth_only_env):
    """openai-codex appears when tokens are already in Jacky auth store."""
...
def test_codex_picker_uses_live_codex_catalog(jacky_auth_only_env, tmp_path, monkeypatch):
...
def claude_code_only_env(tmp_path, monkeypatch):
    ...
    ~/.claude/.credentials.json (Claude Code) — not in env vars or Jacky
    auth store."""
    jacky_home = tmp_path / ".jacky"
    jacky_home.mkdir()

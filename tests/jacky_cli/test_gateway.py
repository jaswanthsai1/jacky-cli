...
def _install_fake_gateway_run(monkeypatch, start_gateway):
    ...
    # /jacky-gateway.service"`` and runs ``systemctl --user daemon-reload``
    # — both target the *real* user environment because the conftest only
    # sandboxes ``JACKY_HOME``, not ``HOME``. Tests that drive
    ...
    # unit (baking in the test's pytest-tmp ``JACKY_HOME`` value, which
...
def test_run_gateway_refuses_root_in_official_docker(monkeypatch, tmp_path, capsys):
    project_root = tmp_path / "opt" / "jacky"
    ...
    monkeypatch.delenv("JACKY_ALLOW_ROOT_GATEWAY", raising=False)
    ...
    assert exc_info.value.code == 1
    out = capsys.readouterr().out
    assert "Refusing to run the Jacky gateway as root" in out
    assert "/opt/jacky/docker/entrypoint.sh" in out
...
def test_run_gateway_root_guard_has_escape_hatch(monkeypatch):
    ...
    monkeypatch.setattr(gateway.os, "geteuid", lambda: 0)
    monkeypatch.setattr(gateway, "_is_official_docker_checkout", lambda: True)
    monkeypatch.setenv("JACKY_ALLOW_ROOT_GATEWAY", "1")
...
def _clear_supervisor_markers(monkeypatch):
    ...
    monkeypatch.delenv("JACKY_S6_SUPERVISED_CHILD", raising=False)
...
def test_run_gateway_refuses_when_service_supervising(monkeypatch, capsys):
    ...
    assert "jacky gateway restart" in out
    assert "--force" in out
...
def test_run_gateway_refuses_existing_process_before_importing_gateway_run(monkeypatch, capsys):
    ...
    assert "jacky gateway run --replace" in out
...
def test_running_under_gateway_supervisor_markers(monkeypatch):
    ...
    monkeypatch.setenv("XPC_SERVICE_NAME", "org.nousresearch.jacky.gateway")
...
def test_run_gateway_windows_foreground_keeps_ctrl_c_enabled(monkeypatch):
    ...
    monkeypatch.delenv("JACKY_GATEWAY_DETACHED", raising=False)
...
def test_run_gateway_windows_detached_absorbs_console_controls(monkeypatch):
    ...
    monkeypatch.setattr(gateway, "supports_systemd_services", lambda: False)
    monkeypatch.setattr(gateway.sys, "stdin", _TTY())
    monkeypatch.setenv("JACKY_GATEWAY_DETACHED", "1")
    monkeypatch.setattr(gateway.signal, "signal", fake_signal)
    monkeypatch.setattr(gateway.asyncio, "run", lambda coro: True)
...
def test_gateway_restart_on_windows_without_service_uses_detached_backend(monkeypatch):
    ...
    A Telegram-hosted agent may run `jacky gateway restart` via the terminal
...
def test_systemd_install_checks_linger_status(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "systemd" / "user" / "jacky-gateway.service"

    monkeypatch.setattr(gateway, "get_systemd_unit_path", lambda system=False: unit_path)
    # Synthetic unit with a non-temp home: the real generator bakes the
    # hermetic test JACKY_HOME (a tmp dir), which the temp-home write
...
def test_systemd_install_can_skip_enable_on_startup(monkeypatch, tmp_path, capsys):
    ...
    # hermetic test JACKY_HOME) stays out of the way.
    monkeypatch.setattr(
        gateway,
        "generate_systemd_unit",
        lambda system=False, run_as_user=None: (
...
def test_systemd_install_system_scope_skips_linger_and_uses_systemctl(monkeypatch, tmp_path, capsys):
    unit_path = tmp_path / "etc" / "systemd" / "system" / "jacky-gateway.service"
...
def test_conflicting_systemd_units_warning(monkeypatch, tmp_path, capsys):
    user_unit = tmp_path / "user" / "jacky-gateway.service"
    system_unit = tmp_path / "system" / "jacky-gateway.service"
    ...
    assert "jacky gateway uninstall" in out
...
def test_install_linux_gateway_from_setup_non_root_never_offers_system(monkeypatch, capsys):
    ...
    # handed a `sudo jacky …` self-elevation recipe.
    captured = {}
...
def test_gateway_install_noninteractive_skips_legacy_unit_prompt(monkeypatch, tmp_path):
    ...
    """
    monkeypatch.setattr(gateway, "has_legacy_jacky_units", lambda: True)
    ...
    monkeypatch.setattr(gateway, "remove_legacy_jacky_units", lambda interactive=False: calls.append(("remove_legacy",)))
    monkeypatch.setattr(gateway, "print_legacy_unit_warning", lambda: None)

    fake_path = tmp_path / "jacky-gateway.service"
...
def test_scan_gateway_pids_detects_windows_jacky_exe_case_variants(monkeypatch):
    monkeypatch.setattr(gateway, "is_windows", lambda: True)
    ...
    def fake_run(cmd, **kwargs):
        if cmd[:4] == ["wmic.exe", "process", "get", "ProcessId,CommandLine"]:
            return SimpleNamespace(
                returncode=0,
                stdout=(
                    "CommandLine=C:\\Program Files\\Jacky\\Jacky.EXE gateway run --replace\n"
                    "ProcessId=2468\n\n"
                ),
                stderr="",
            )
...
def test_module_has_logger():
    ...
    assert gateway.logger.name == "jacky_cli.gateway"

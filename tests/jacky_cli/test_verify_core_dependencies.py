...
base deps) during ``jacky update``, leaving the venv in a broken state
...
def temp_pyproject(tmp_path, monkeypatch):
    """Point jacky_cli.main.PROJECT_ROOT at a tmp dir with a minimal pyproject.
...
class TestVerifyCoreDependencies:
    def test_no_action_when_all_deps_present(self, temp_pyproject, fake_venv_python):
        ...
        with patch("jacky_cli.main._resolve_install_target_python", return_value=py), \
             patch("jacky_cli.main.subprocess.run") as mock_run, \
             patch("jacky_cli.main._run_install_with_heartbeat") as mock_install:
            mock_run.return_value = MagicMock(returncode=0, stdout="", stderr="")

            from jacky_cli.main import _verify_core_dependencies_installed
    ...
    def test_triggers_reinstall_when_dep_missing(self, temp_pyproject, fake_venv_python):
        ...
        with patch("jacky_cli.main._resolve_install_target_python", return_value=py), \
             patch("jacky_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("jacky_cli.main._run_install_with_heartbeat") as mock_install:
    ...
    def test_skips_deps_excluded_by_environment_markers(self, temp_pyproject, fake_venv_python):
        ...
        with patch("jacky_cli.main._resolve_install_target_python", return_value=py), \
             patch("jacky_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("jacky_cli.main._run_install_with_heartbeat"), \
    ...
    def test_no_pyproject_is_noop(self, tmp_path, monkeypatch):
        ...
        with patch("jacky_cli.main._resolve_install_target_python") as mock_resolve, \
             patch("jacky_cli.main._run_install_with_heartbeat") as mock_install:
            from jacky_cli.main import _verify_core_dependencies_installed
    ...
    def test_repair_reinstall_quarantines_running_shim_on_windows(
        ...
        quarantine the running ``jacky.exe`` on Windows before installing.
        ...
        is left missing and ``jacky`` drops off PATH. Previously this path
        ...
        with patch("jacky_cli.main._resolve_install_target_python", return_value=py), \
             patch("jacky_cli.main.subprocess.run", side_effect=fake_subprocess_run), \
             patch("jacky_cli.main._is_windows", return_value=True), \
             patch("jacky_cli.main._venv_scripts_dir", return_value=fake_scripts), \
             patch("jacky_cli.main._run_install_with_heartbeat"), \
             patch("jacky_cli.main._quarantine_running_jacky_exe", return_value=[]) as mock_quar:
...
class TestResolveInstallTargetPython:
    def test_uses_virtual_env_from_environment(self, tmp_path):
        ...
        venv's interpreter — not the outer Python that drove `jacky update`.
        ...
        with patch("jacky_cli.main._is_windows", return_value=True):
            from jacky_cli.main import _resolve_install_target_python

...
from jacky_cli.codex_runtime_plugin_migration import (
    MIGRATION_MARKER,
    MIGRATION_END_MARKER,
    _build_jacky_tools_mcp_entry,
    _format_toml_value,
...
class TestMigrate:
    ...
    def test_no_servers_still_writes_permissions_default(self, tmp_path):
        ...
        report = migrate({}, codex_home=tmp_path, discover_plugins=False, expose_jacky_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_plugin_discovery_writes_plugin_blocks(self, tmp_path, monkeypatch):
        ...
        from jacky_cli import codex_runtime_plugin_migration as crpm
    ...
    def test_plugin_discovery_skips_unavailable_plugins(self):
        ...
        from jacky_cli.codex_runtime_plugin_migration import _query_codex_plugins
    ...
    def test_plugin_discovery_failure_non_fatal(self, tmp_path, monkeypatch):
        ...
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, discover_plugins=True, expose_jacky_tools=False)
        assert report.written
    ...
    def test_discover_plugins_false_skips_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, discover_plugins=False, expose_jacky_tools=False)
    ...
    def test_dry_run_skips_plugin_query(self, tmp_path, monkeypatch):
        ...
        migrate({"mcp_servers": {"x": {"command": "y"}}},
                codex_home=tmp_path, dry_run=True, discover_plugins=True, expose_jacky_tools=False)
        assert called["yes"] is False
    ...
    def test_expose_jacky_tools_writes_callback_mcp_entry(self, tmp_path):
        """When expose_jacky_tools=True (production default), an
        [mcp_servers.jacky-tools] entry is written so codex calls back
        into Jacky for browser/web/delegate_task/vision/memory tools.

        This is the fix for 'all other tools that codex doesn't provide
        should be useable by jacky' — quirk #7."""
        report = migrate({}, codex_home=tmp_path,
                         discover_plugins=False,
                         default_permission_profile=None,
                         expose_jacky_tools=True)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.jacky-tools]" in text
        assert "jacky_tools_mcp_server" in text
        ...
        assert "jacky-tools" in report.migrated

    def test_expose_jacky_tools_disabled_skips_entry(self, tmp_path):
        """expose_jacky_tools=False suppresses the callback registration."""
        migrate({}, codex_home=tmp_path,
                discover_plugins=False,
                default_permission_profile=None,
                expose_jacky_tools=False)
        text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.jacky-tools]" not in text
        assert "jacky_tools_mcp_server" not in text

    def test_dry_run_doesnt_write(self, tmp_path):
        report = migrate({"mcp_servers": {"x": {"command": "y"}}},
                         codex_home=tmp_path, dry_run=True, expose_jacky_tools=False)
    ...
    def test_full_migration_round_trip(self, tmp_path):
        ...
        report = migrate(jacky_cfg, codex_home=tmp_path, expose_jacky_tools=False)
        assert report.written
        text = (tmp_path / "config.toml").read_text()
    ...
    def test_idempotent_re_run_replaces_managed_block(self, tmp_path):
        # First migration
        migrate({"mcp_servers": {"a": {"command": "x"}}}, codex_home=tmp_path, expose_jacky_tools=False)
        first_text = (tmp_path / "config.toml").read_text()
        assert "[mcp_servers.a]" in first_text
        # Second migration with different servers
        migrate({"mcp_servers": {"b": {"command": "y"}}}, codex_home=tmp_path, expose_jacky_tools=False)
    ...
    def test_managed_root_keys_stay_top_level_when_config_ends_in_table(self, tmp_path):
        """TOML has no explicit 'leave current table' syntax. If Jacky appends
        ...
        migrate({}, codex_home=tmp_path, discover_plugins=False, expose_jacky_tools=False)
        new_text = target.read_text()
    ...
    def test_preserves_user_mcp_server_outside_managed_block(self, tmp_path):
        ...
        to ~/.codex/config.toml outside Jacky' managed block, re-running
        ...
        migrate({"mcp_servers": {"jacky-mcp": {"command": "npx"}}},
        ...
        assert "[mcp_servers.jacky-mcp]" in final
    ...
    def test_skipped_keys_reported(self, tmp_path):
        ...
        }, codex_home=tmp_path, expose_jacky_tools=False)
    ...
    def test_invalid_mcp_servers_value(self, tmp_path):
        report = migrate({"mcp_servers": "notadict"}, codex_home=tmp_path, expose_jacky_tools=False)
...
class TestStripUnmanagedPluginTables:
    ...
    ``jacky codex-runtime migrate`` would re-emit them inside the managed
    ...
    def test_migrate_dedups_codex_owned_plugin_tables(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "jacky_cli.codex_runtime_plugin_migration._query_codex_plugins",
            fake_query,
        )
        migrate({}, codex_home=tmp_path, discover_plugins=True, expose_jacky_tools=False)
        new_text = target.read_text()
...
# ---- Bug C: JACKY_HOME tempdir leak into ~/.codex/config.toml ----


class TestJackyHomeLeakGuard:
    """Regression tests for issue #26250 Bug C.

    Previously ``_build_jacky_tools_mcp_entry()`` read ``JACKY_HOME``
    ...
    once codex spawned the jacky-tools MCP subprocess.
    """

    def test_tempdir_detector_recognizes_pytest_paths(self):
        assert _looks_like_test_tempdir(
            "/private/var/folders/abc/pytest-of-kshitij/pytest-137/popen-gw2/test_X/jacky_test"
        )
        assert _looks_like_test_tempdir(
            "/tmp/pytest-of-user/pytest-12/test_X/jacky"
        )
        assert _looks_like_test_tempdir(
            "/private/var/folders/zz/T/pytest-of-bob/pytest-1"
        )

    def test_tempdir_detector_accepts_real_jacky_home(self):
        assert not _looks_like_test_tempdir("/Users/alice/.jacky")
        assert not _looks_like_test_tempdir("/home/bob/.jacky")
        assert not _looks_like_test_tempdir("/opt/jacky")
        assert not _looks_like_test_tempdir("")

    def test_pytest_tempdir_not_burned_into_mcp_env(self, monkeypatch):
        """The headline regression: even when JACKY_HOME points at a pytest
        tempdir, _build_jacky_tools_mcp_entry() must NOT propagate it."""
        monkeypatch.setenv(
            "JACKY_HOME",
            "/private/var/folders/xx/pytest-of-user/pytest-99/test_x/jacky_test",
        )
        entry = _build_jacky_tools_mcp_entry()
        env = entry.get("env", {})
        assert "JACKY_HOME" not in env, (
            f"pytest-tempdir JACKY_HOME leaked into codex MCP entry: "
            f"{env.get('JACKY_HOME')!r}"
        )

    def test_real_jacky_home_propagates(self, monkeypatch, tmp_path):
        """A legitimate JACKY_HOME (not a tempdir path) DOES propagate so the
        ...
        real_path = "/Users/alice/.jacky"
        monkeypatch.setenv("JACKY_HOME", real_path)
        entry = _build_jacky_tools_mcp_entry()
        env = entry.get("env", {})
        assert env.get("JACKY_HOME") == real_path

    def test_unset_jacky_home_omits_env_key(self, monkeypatch):
        """When JACKY_HOME is unset in the environment, the MCP entry MUST
        NOT bake in a resolved-default path. The codex subprocess should
        inherit whatever JACKY_HOME its launcher (systemd, gateway, shell)
        ...
        env = entry.get("env", {})
        assert "JACKY_HOME" not in env, (
            f"JACKY_HOME should not be set when env var is unset, got: "
            f"{env.get('JACKY_HOME')!r}"
        )

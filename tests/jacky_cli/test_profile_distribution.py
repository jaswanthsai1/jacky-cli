"""Tests for jacky_cli.profile_distribution — git-based profile installs.
...
# ---------------------------------------------------------------------------
# Isolated profile env (matches tests/jacky_cli/test_profiles.py)
# ---------------------------------------------------------------------------
...
class TestVersionRequires:
    ...
    def test_check_matrix(self, spec, cur, ok):
        if ok:
            check_jacky_requires(spec, cur)
        else:
            with pytest.raises(DistributionError, match="requires Jacky"):
                check_jacky_requires(spec, cur)
...
class TestEnvTemplate:
    ...
    def test_empty_env_requires_is_header_only(self):
        m = DistributionManifest(name="x")
        out = _env_template_from_manifest(m)
        assert "Jacky distribution" in out
...
class TestInstall:
    ...
    def test_install_enforces_jacky_requires(self, profile_env, monkeypatch):
        # Pin current Jacky version to something well below the requirement
        import jacky_cli
        monkeypatch.setattr(jacky_cli, "__version__", "0.1.0", raising=False)

        mf = DistributionManifest(
            name="future",
            version="1.0.0",
            jacky_requires=">=99.0.0",
        )
...
class TestSecurity:
    ...
    def test_install_rejects_symlinked_distribution_files(self, profile_env, tmp_path):
        ...
        from jacky_cli.profiles import get_profile_dir
...
class TestInstalledAtStamp:
    ...
    def test_update_refreshes_installed_at(self, profile_env, monkeypatch):
        ...
        monkeypatch.setattr(
            "jacky_cli.profile_distribution.datetime", _FakeDT, raising=True
        )

        from jacky_cli.profile_distribution import update_distribution
...
class TestProfileInfoDistribution:
    ...
    def test_plain_profile_has_no_distribution_fields(self, profile_env):
        from jacky_cli.profiles import create_profile, list_profiles
        create_profile(name="plain", no_alias=True)
        rows = {p.name: p for p in list_profiles()}
    ...
    def test_malformed_manifest_does_not_break_list(self, profile_env):
        from jacky_cli.profiles import create_profile, list_profiles, get_profile_dir

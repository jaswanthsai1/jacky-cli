"""
Tests for mcp_serve — Jacky MCP server.
...
def _isolate_jacky_home(tmp_path, monkeypatch):
    """Redirect JACKY_HOME to a temp directory."""
    monkeypatch.setenv("JACKY_HOME", str(tmp_path))
    try:
        import jacky_cli.jacky_constants as jacky_constants
        monkeypatch.setattr(jacky_constants, "get_jacky_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking jacky_state schema."""
...
def fake_mcp_server(populated_sessions_dir, mock_session_db, monkeypatch):
    import jacky_cli.mcp_serve as mcp_serve
...
class TestContentExtraction:
    def test_text(self):
        from jacky_cli.mcp_serve import _extract_message_content
...
class TestAttachmentExtraction:
    def test_image_url_block(self):
        from jacky_cli.mcp_serve import _extract_attachments
        msg = {"content": [
...
class TestEventBridge:
    def test_create(self):
        from jacky_cli.mcp_serve import EventBridge
        b = EventBridge()
        assert b._cursor == 0
        assert b._queue == []

    def test_enqueue_and_poll(self):
        from jacky_cli.mcp_serve import EventBridge, QueueEvent
        b = EventBridge()
    ...
    def test_queue_limit(self):
        from jacky_cli.mcp_serve import EventBridge, QueueEvent, QUEUE_LIMIT
        b = EventBridge()
...
class TestE2EEventsPoll:
    ...
    def test_poll_with_events(self, mcp_server_e2e, _event_loop):
        from jacky_cli.mcp_serve import QueueEvent
        server, bridge = mcp_server_e2e
...
class TestCliIntegration:
    ...
    def test_dispatcher_routes_serve(self, monkeypatch, tmp_path):
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        mock_run = MagicMock()
        monkeypatch.setattr("jacky_cli.mcp_serve.run_mcp_server", mock_run)
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <JACKY_HOME>/state.db for its mtime gate; the autouse
        # fixture points JACKY_HOME at tmp_path.
        db_path = tmp_path / "state.db"
    ...
    def test_poll_interval_is_200ms(self):
        ...
        from jacky_cli.mcp_serve import POLL_INTERVAL

...
user-initiated via the ``x-initiator: user`` header. Jacky previously sent
...
def _make_agent(monkeypatch, base_url, api_mode="chat_completions"):
    ...
    monkeypatch.setattr("jacky_cli.run_agent.get_tool_definitions", lambda **kw: _tool_defs("web_search"))

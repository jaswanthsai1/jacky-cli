"""Tests for jacky_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Jacky entry point
(jacky, jacky-agent, jacky-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Jacky entry point imports jacky_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported jacky_bootstrap module.
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Jacky CLI
...
class TestEntryPointsImportBootstrap:
    """Every Jacky entry point must import jacky_bootstrap as its
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import jacky_bootstrap' and that
        ...
        half-finished ``jacky update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``jacky_bootstrap`` as a top-level module) leaves jacky
        ...
        """
        # Resolve relative to the jacky-agent repo root.  Tests live
        # at tests/test_jacky_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # jacky start even when jacky_bootstrap hasn't been
...
class TestHardenImportPath:
    ...
    directory from shadowing Jacky's own top-level modules — covering both
    ...
    def _run(self, hb, path_seed, env=None):
        ...
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["JACKY_PYTHON_SRC_ROOT"] = env
            elif "JACKY_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["JACKY_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/jacky")
            return sys.path[:]
        finally:
            sys.path[:] = original
    ...
    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/jacky", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Jacky root.  The guard must relocate Jacky to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/jacky"])
        assert result[0] == "/opt/jacky"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Jacky root.
        assert result.index("/opt/jacky") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/jacky", "/opt/jacky", ""])
        assert result.count("/opt/jacky") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["JACKY_PYTHON_SRC_ROOT"] = "/env/jacky"
            hb.harden_import_path()
            assert sys.path[0] == "/env/jacky"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("JACKY_PYTHON_SRC_ROOT", None)

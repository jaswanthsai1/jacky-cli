...
def jacky_home(tmp_path, monkeypatch):
    ...
    # Bust the goal-module DB cache so it re-resolves JACKY_HOME.
...
def test_moa_bare_returns_usage(server, session, jacky_home):
    _write_moa_config(jacky_home, """
moa:
...
def test_moa_arg_is_always_one_shot(server, session, jacky_home):
...
def test_moa_non_preset_returns_one_shot_send(server, session, jacky_home):
    _write_moa_config(jacky_home, """
moa:
  default_preset: default
  presets:

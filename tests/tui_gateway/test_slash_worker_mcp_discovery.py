...
def test_profile_local_mcp_tool_is_visible_in_slash_worker(tmp_path):
    ...
    server.write_text(
        textwrap.dedent(
            ...
            @mcp.tool()
            def jacky_61922_profile_probe() -> str:
                return {marker!r}
    ...
    env["JACKY_SLASH_WATCHDOG_GRACE_S"] = "0"
    env["JACKY_SLASH_WATCHDOG_POLL_S"] = "0.05"
    ...
    try:
        ...
        assert "mcp__profileprobe__jacky_61922_profile_probe" in response["output"]
    finally:
        proc.terminate()
        try:
            proc.wait(timeout=5)

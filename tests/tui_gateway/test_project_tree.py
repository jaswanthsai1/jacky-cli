...
def test_user_worktree_under_dotworktrees_is_its_own_lane_not_kanban():
    ...
    sessions = [
        _session("/repo", branch="main"),
        _session("/repo/.worktrees/test-gui-stuff", branch="jacky/test-gui-stuff"),
    ]
...
def test_junk_root_never_becomes_an_auto_project():
    # A session whose git root is JACKY_HOME (config/state) must not spawn a
    # phantom project; it falls through to flat Recents (unscoped). A real repo
    # alongside it still groups normally.
    resolve = _resolver(
        {
            "/home/me/.jacky": ("/home/me/.jacky", "/home/me/.jacky"),
            "/www/app": ("/www/app", "/www/app"),
        }
    )
    junk = _session("/home/me/.jacky", branch="main")
    real = _session("/www/app", branch="main")
    is_junk = lambda root: root == "/home/me/.jacky"
...
def test_junk_root_is_dropped_from_the_discovered_tier():
    discovered = [{"root": "/home/me/.jacky", "label": ".jacky", "sessions": 0, "last_active": 9}]

    tree = pt.build_tree([], [], discovered, resolve=None, is_junk_root=lambda r: r == "/home/me/.jacky")

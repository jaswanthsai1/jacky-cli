...
def test_make_agent_passes_resolved_provider():
    ...
    with (
        ...
        patch("jacky_cli.run_agent.AIAgent") as mock_agent,
    ):
...
def test_make_agent_honors_tui_launch_env_flags():
    ...
    with (
        patch.dict(
            os.environ,
            {
                "JACKY_TUI_MAX_TURNS": "7",
                "JACKY_TUI_CHECKPOINTS": "1",
                "JACKY_TUI_PASS_SESSION_ID": "1",
                "JACKY_IGNORE_RULES": "1",
            },
        ),
        patch("tui_gateway.server._load_cfg", return_value=fake_cfg),
        patch("tui_gateway.server._get_db", return_value=MagicMock()),
...
def test_make_agent_tolerates_null_config_sections():
    """Bare `agent:` / `display:` keys in ~/.jacky/config.yaml parse as
...
def test_make_agent_tolerates_null_personalities_with_active_personality():
    ...
    with (
        ...
        patch("tui_gateway.server._get_db", return_value=MagicMock()),
        patch("jacky_cli.cli.load_cli_config", return_value={"agent": {"personalities": None}}),
        patch(
            "jacky_cli.runtime_provider.resolve_runtime_provider",
            return_value=fake_runtime,
        ),
...
def test_apply_model_switch_does_not_leak_process_env():
    ...
    with (
        patch("jacky_cli.model_switch.parse_model_flags",
              return_value=("glm-5.1", None, False, False, True)),
        patch("jacky_cli.model_switch.resolve_persist_behavior",
              return_value=False),
        patch("jacky_cli.model_switch.switch_model", return_value=_FakeResult()),

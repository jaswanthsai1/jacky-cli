...
def server():
    with patch.dict(
        "sys.modules",
        {
            "jacky_constants": MagicMock(
                get_jacky_home=MagicMock(return_value="/tmp/jacky_test_review_summary")
            ),
...
def test_init_session_attaches_background_review_callback(server, monkeypatch):
    ...
    cb("💾 Self-improvement review: Skill 'jacky-release' patched")
    ...
    assert sid == "sid-abc"
    assert payload == {
        "text": "💾 Self-improvement review: Skill 'jacky-release' patched"
    }

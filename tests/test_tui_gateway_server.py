...
def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "Jacky is at the active session limit (1/1). "
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["JACKY_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("JACKY_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("JACKY_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("JACKY_VOICE", "1")
    monkeypatch.delenv("JACKY_VOICE_TTS", raising=False)
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("JACKY_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("JACKY_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.plugins",
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("JACKY_TUI_TOOLSETS", raising=False)
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("JACKY_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import jacky_cli.config as config_mod
    ...
    # _get_platform_tools (a non-configurable platform toolset in jacky-cli's
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("JACKY_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("JACKY_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("JACKY_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("JACKY_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("JACKY_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "jacky_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown JACKY_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("JACKY_MODEL", raising=False)
    monkeypatch.setenv("JACKY_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("JACKY_MODEL", raising=False)
    monkeypatch.delenv("JACKY_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " nous/jacky-test "}}
    )

    assert server._resolve_model() == "nous/jacky-test"
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set JACKY_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("JACKY_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("JACKY_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `jacky --tui -m <model>` sets JACKY_MODEL/JACKY_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("JACKY_MODEL", "one-shot/model")
    monkeypatch.setenv("JACKY_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("JACKY_MODEL", "seed/model")
    monkeypatch.setenv("JACKY_INFERENCE_MODEL", "seed/model")
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "jacky_cli.model_switch.resolve_persist_behavior",
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("JACKY_MODEL", "nous/jacky-test")
    monkeypatch.setenv("JACKY_TUI_PROVIDER", "nous")
    monkeypatch.delenv("JACKY_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("nous/jacky-test", "nous")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("JACKY_MODEL", "nous/jacky-test")
    monkeypatch.delenv("JACKY_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("JACKY_INFERENCE_PROVIDER", "nous")
    monkeypatch.setattr(
        "jacky_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("nous/jacky-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("JACKY_MODEL", "sonnet")
    monkeypatch.delenv("JACKY_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("JACKY_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "jacky_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with JackyCLI / gateway: fallback_providers stays first and keeps
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "jacky_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "jacky_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_jacky_home", tmp_path)
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("JACKY_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("JACKY_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("JACKY_EXEC_ASK", raising=False)
    monkeypatch.delenv("JACKY_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("JACKY_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["JACKY_GATEWAY_SESSION"] == "1"
    assert server.os.environ["JACKY_EXEC_ASK"] == "1"
    assert server.os.environ["JACKY_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("jacky_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("jacky_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "jacky_cli.runtime_provider.resolve_runtime_provider",
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "jacky_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing JACKY_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("JACKY_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "jacky_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["JACKY_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "JACKY_TUI_PROVIDER" not in os.environ
        assert "JACKY_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write JACKY_MODEL / JACKY_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("JACKY_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("JACKY_MODEL", raising=False)
    monkeypatch.delenv("JACKY_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("jacky_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert os.environ["JACKY_TUI_PROVIDER"] == "openai-codex"
        assert "JACKY_MODEL" not in os.environ
        assert "JACKY_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors JackyCLI._manual_compress's
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".jacky" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.jacky/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".jacky" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.jacky/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".jacky" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.jacky/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Jacky TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("jacky_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "jacky_cli.commands.SlashCommandCompleter",
...
def test_session_info_includes_install_warning_for_pip(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.config.detect_install_method", lambda: "pip")
...
def test_session_info_omits_install_warning_for_git(monkeypatch):
    monkeypatch.setattr("jacky_cli.config.detect_install_method", lambda: "git")
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("jacky_state")
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_jacky_home
    # fixture pins JACKY_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `jacky model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `jacky model` and the classic CLI /model picker.
    ...
    with patch(
        "jacky_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("jacky_cli.models.provider_model_ids") as live_fetch:
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from jacky_cli.inventory import ConfigContext

    calls = []

    monkeypatch.setattr(server, "_resolve_model", lambda: "")
    monkeypatch.setattr(
        "jacky_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "jacky_cli.inventory.build_models_payload",
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``jacky --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"jacky_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "jacky_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("jacky_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
                "jacky_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "jacky_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_jacky_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.jacky/.env
    ...
    with patch.dict(sys.modules, {"jacky_cli.config": fake}):
...
def test_session_save_writes_under_jacky_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Jacky profile
    ...
    Regression: the gateway handler wrote ``jacky_conversation_*.json`` to
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Jacky.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("jacky_conversation_*.json"))
    ...
    assert payload["model"] == "jacky-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Jacky."
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._jacky_home, which
    # is captured at import time — patch it directly (set_jacky_home_override

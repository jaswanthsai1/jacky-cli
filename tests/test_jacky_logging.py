"""Tests for jacky_logging — centralized logging setup."""
import io
import logging
import os
import stat
...
# Use whatever RotatingFileHandler class jacky_logging actually resolved so
# the autouse fixture's isinstance checks (which strip rotating handlers
# between tests) match the real handlers on every platform. jacky_logging
...
from jacky_cli.jacky_logging import RotatingFileHandler
...
def _reset_logging_state():
    ...
    """
    jacky_logging._logging_initialized = False
    ...
    jacky_logging._reset_queued_handlers()
    ...
    jacky_logging._install_session_record_factory()
    yield
    ...
    jacky_logging._logging_initialized = False
    jacky_logging.clear_session_context()


@pytest.fixture
def jacky_home(tmp_path, monkeypatch):
    """Provide an isolated JACKY_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_jacky_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["JACKY_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, jacky_home):
        log_dir = jacky_logging.setup_logging(jacky_home=jacky_home)
        assert log_dir == jacky_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
        root = logging.getLogger()

        agent_handlers = [
            h for h in jacky_logging.rotating_file_handlers()
    ...
    def test_creates_errors_log_handler(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
        jacky_logging.setup_logging(jacky_home=jacky_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        jacky_logging.setup_logging(jacky_home=jacky_home, force=True)
    ...
    def test_custom_log_level(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, jacky_home):
        jacky_logging.setup_logging(
            jacky_home=jacky_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
    ...
    def test_writes_to_agent_log(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)

        test_logger = logging.getLogger("test_jacky_logging.write_test")
        test_logger.info("test message for agent.log")

        # Flush handlers
        jacky_logging.flush_log_queue()

        agent_log = jacky_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)

        test_logger = logging.getLogger("test_jacky_logging.warning_test")
        test_logger.warning("this is a warning")

        jacky_logging.flush_log_queue()

        agent_log = jacky_home / "logs" / "agent.log"
        errors_log = jacky_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)

        test_logger = logging.getLogger("test_jacky_logging.info_test")
    ...
    def test_explicit_params_override_config(self, jacky_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (jacky_home / "config.yaml").write_text(yaml.dump(config))

        jacky_logging.setup_logging(jacky_home=jacky_home, log_level="WARNING")
    ...
    def test_record_factory_installed(self, jacky_home):
        """The custom record factory injects session_tag on all records."""
        jacky_logging.setup_logging(jacky_home=jacky_home)
        factory = logging.getLogRecordFactory()
        assert getattr(factory, "_jacky_session_injector", False), (
            "Record factory should have _jacky_session_injector marker"
        )
...
class TestGatewayMode:
    ...
    def test_gateway_log_created(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home, mode="gateway")
        root = logging.getLogger()
    ...
    def test_gateway_log_not_created_in_cli_mode(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home, mode="cli")
        root = logging.getLogger()
    ...
    def test_gateway_log_created_after_cli_init(self, jacky_home):
        ...
        gw_log = jacky_home / "logs" / "gateway.log"
        assert gw_log.exists()
        assert "gateway connected after cli init" in gw_log.read_text()

    def test_gateway_log_created_after_cli_init_without_duplicate_handlers(self, jacky_home):
    ...
    def test_gateway_log_receives_gateway_records(self, jacky_home):
    ...
    def test_gateway_log_rejects_non_gateway_records(self, jacky_home):
    ...
    def test_agent_log_still_receives_all(self, jacky_home):
...
class TestGuiMode:
    ...
    def test_gui_log_created(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home, mode="gui")
        root = logging.getLogger()
    ...
    def test_gui_log_created_after_cli_init(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home, mode="cli")
        jacky_logging.setup_logging(jacky_home=jacky_home, mode="gui")
    ...
    def test_gui_log_receives_only_gui_components(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home, mode="gui")

        logging.getLogger("jacky_cli.web_server").info("dashboard online")
        logging.getLogger("tui_gateway.ws").info("ws connected")
        logging.getLogger("gateway.run").info("gateway event")

        jacky_logging.flush_log_queue()

        gui_log = jacky_home / "logs" / "gui.log"
...
class TestSessionContext:
    ...
    def test_session_tag_in_log_output(self, jacky_home):
        """When session context is set, log lines include [session_id]."""
        jacky_logging.setup_logging(jacky_home=jacky_home)
        jacky_logging.set_session_context("abc123")
    ...
    def test_no_session_tag_without_context(self, jacky_home):
    ...
    def test_clear_session_context(self, jacky_home):
        """After clearing, session tag disappears."""
        jacky_logging.setup_logging(jacky_home=jacky_home)
        jacky_logging.set_session_context("xyz789")
        jacky_logging.clear_session_context()
    ...
    def test_session_context_thread_isolated(self, jacky_home):
        ...
        def thread_a():
            jacky_logging.set_session_context("thread_a_session")
            logging.getLogger("test.thread_a").info("from thread A")
            jacky_logging.flush_log_queue()

        def thread_b():
            jacky_logging.set_session_context("thread_b_session")
...
class TestRecordFactory:
    ...
    def test_tag_with_context(self):
        jacky_logging.set_session_context("sess_42")
        factory = logging.getLogRecordFactory()
        record = factory("test", logging.INFO, "", 0, "msg", (), None)
    ...
    def test_works_with_any_handler(self):
        ...
        jacky_logging.set_session_context("any_handler_test")
...
class TestComponentFilter:
    ...
    def test_passes_matching_prefix(self):
        f = jacky_logging._ComponentFilter(("gateway",))
    ...
    def test_passes_nested_matching_prefix(self):
        ...
        f = jacky_logging._ComponentFilter(
            jacky_logging.COMPONENT_PREFIXES["gateway"]
        )
        record = logging.LogRecord(
    ...
    def test_multiple_prefixes(self):
        f = jacky_logging._ComponentFilter(("agent", "run_agent", "model_tools"))
...
class TestComponentPrefixes:
    ...
    def test_gateway_prefix(self):
        assert "gateway" in jacky_logging.COMPONENT_PREFIXES
        # The gateway component captures core gateway logs, the jacky_plugins
        ...
        # break this test.
        gateway_prefixes = jacky_logging.COMPONENT_PREFIXES["gateway"]
        assert "gateway" in gateway_prefixes
        assert "jacky_plugins" in gateway_prefixes
        assert "plugins.platforms" in gateway_prefixes

    def test_agent_prefix(self):
        prefixes = jacky_logging.COMPONENT_PREFIXES["agent"]
        assert "agent" in prefixes
        assert "run_agent" in prefixes
        assert "model_tools" in prefixes

    def test_tools_prefix(self):
        assert ("tools",) == jacky_logging.COMPONENT_PREFIXES["tools"]

    def test_cli_prefix(self):
        prefixes = jacky_logging.COMPONENT_PREFIXES["cli"]
        assert "jacky_cli" in prefixes
        assert "cli" in prefixes

    def test_cron_prefix(self):
        assert ("cron",) == jacky_logging.COMPONENT_PREFIXES["cron"]

    def test_gui_prefix(self):
        prefixes = jacky_logging.COMPONENT_PREFIXES["gui"]
        assert "jacky_cli.web_server" in prefixes
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
        jacky_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_jacky_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
        jacky_logging.setup_verbose_logging()
        jacky_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        jacky_logging._add_rotating_handler(
            logger, log_path,
    ...
    def test_log_filter_attached(self, tmp_path):
        ...
        formatter = logging.Formatter("%(message)s")
        component_filter = jacky_logging._ComponentFilter(("test",))
        ...
        handlers = [h for h in jacky_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)]
    ...
    def test_no_session_filter_on_handler(self, tmp_path):
        ...
        jacky_logging.set_session_context("factory_test")
    ...
    def test_managed_mode_initial_open_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("jacky_cli.config.is_managed", return_value=True):
                jacky_logging._add_rotating_handler(
                    logger, log_path,
                    level=logging.INFO, max_bytes=1024, backup_count=1,
                    formatter=formatter,
    ...
    def test_managed_mode_rollover_sets_group_writable(self, tmp_path):
        ...
        try:
            with patch("jacky_cli.config.is_managed", return_value=True):
                ...
                handler = next(
                    h for h in jacky_logging.rotating_file_handlers() if isinstance(h, RotatingFileHandler)
                )
                logger.info("a" * 256)
                jacky_logging.flush_log_queue()
        finally:
...
class TestWindowsConcurrentLogLockTimeout:
    ...
    def _make_logger_and_handler(self, log_path: Path):
        ...
        handler = jacky_logging._ManagedRotatingFileHandler(
    ...
    def test_helper_only_matches_windows_concurrent_lock_timeout(self):
        with patch.object(jacky_logging.sys, "platform", "win32"):
            assert jacky_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
            assert not jacky_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("some other logging failure")
            )

        with patch.object(jacky_logging.sys, "platform", "linux"):
            assert not jacky_logging._is_windows_concurrent_log_lock_timeout(
                RuntimeError("Cannot acquire lock after 20 attempts")
            )
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, jacky_home):
        level, max_size, backup = jacky_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, jacky_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, jacky_home):
        import yaml
        config = {"model": "test"}
        (jacky_home / "config.yaml").write_text(yaml.dump(config))
...
class TestExternalRotationRecovery:
    ...
    def _make_handler(self, log_path: Path) -> jacky_logging._ManagedRotatingFileHandler:
        handler = jacky_logging._ManagedRotatingFileHandler(
    ...
    def _emit(self, handler: logging.Handler, msg: str) -> None:
        ...
        # Match the record factory that jacky_logging installs at import time.
        record.session_tag = ""
        handler.emit(record)
        jacky_logging.flush_log_queue()
    ...
    def test_gateway_log_attached_after_external_rotation_then_re_setup(
        ...
        """
        jacky_logging.setup_logging(jacky_home=jacky_home, mode="gateway")
        gw_path = jacky_home / "logs" / "gateway.log"
        rotated = jacky_home / "logs" / "gateway.log.1"
...
class TestSafeStderr:
    ...
    def test_returns_stderr_on_utf8_system(self, monkeypatch):
        ...
        result = jacky_logging._safe_stderr()
...
class TestAsyncQueueLogging:
    ...
    def test_file_handlers_not_on_root(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
        root = logging.getLogger()
        ...
        queue_handlers = [
            h for h in root.handlers if getattr(h, "_jacky_queue", False)
        ]
        assert len(queue_handlers) == 1
        # The real file handlers are discoverable via the accessor.
        assert any(
            "agent.log" in getattr(h, "baseFilename", "")
            for h in jacky_logging.rotating_file_handlers()
        )

    def test_records_reach_file_through_queue(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)
    ...
    def test_queue_preserves_per_handler_levels(self, jacky_home):
        jacky_logging.setup_logging(jacky_home=jacky_home)

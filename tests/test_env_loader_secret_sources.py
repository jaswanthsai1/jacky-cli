"""Tests for the secret-source tracking in ``jacky_cli.env_loader``.

These cover the small public surface that lets `jacky model` / `jacky setup`
...
from jacky_cli import env_loader  # noqa: E402
...
def test_apply_external_secret_sources_dedupes_within_process(tmp_path, monkeypatch):
    """``load_jacky_dotenv()`` is called at module-import time from several
    hot modules (cli.py, jacky_cli/main.py, run_agent.py, ...).  The
    ...
    runs exactly once per JACKY_HOME per process.
    """
    ...
    # cli.py, jacky_cli/main.py, run_agent.py, trajectory_compressor.py,

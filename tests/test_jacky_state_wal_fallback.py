...
(SQLITE_PROTOCOL — typical on NFS/SMB), Jacky must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``JACKY_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    jacky_state._set_last_init_error(None)
    yield
    jacky_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    jacky_state._wal_fallback_warned_paths.clear()
    yield
    jacky_state._wal_fallback_warned_paths.clear()
...
class TestApplyWalWithFallback:
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see jacky_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        jacky_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("jacky_cli.jacky_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        jacky_state._set_last_init_error("OperationalError: generic SQLite error")

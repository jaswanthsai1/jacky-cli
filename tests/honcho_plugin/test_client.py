...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "jacky"
        assert config.workspace_id == "jacky"
...
class TestFromGlobalConfig:
    ...
    def test_reads_full_config(self, tmp_path, monkeypatch):
        ...
        # Isolate from real ~/.jacky/honcho.json
        monkeypatch.setenv("JACKY_HOME", str(tmp_path / "isolated"))
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"jacky": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.context_tokens == 2000
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"jacky": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"jacky": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="jacky-agent"
        ):
            result = config.resolve_session_name("/home/user/jacky-agent/subdir")
        assert result == "jacky-agent"
...
class TestResolveConfigPath:
    def test_prefers_jacky_home_when_exists(self, tmp_path):
        jacky_home = tmp_path / "jacky"
        jacky_home.mkdir()
        local_cfg = jacky_home / "honcho.json"
    ...
    def test_falls_back_to_default_profile_when_no_local(self, tmp_path, monkeypatch):
        # Profile mode: JACKY_HOME points at ~/.jacky/profiles/<name>, so
        # _get_default_jacky_home() must resolve back to ~/.jacky — that's
        # the bug the HOME-anchored helper fixes (vs. blindly using Path.home()).
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".jacky"
        ...
        assert _get_default_jacky_home() == default_home
        assert result == default_cfg

    def test_falls_back_to_global_without_jacky_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()

        with patch.dict(os.environ, {}, clear=False), \
             patch.object(Path, "home", return_value=fake_home):
            os.environ.pop("JACKY_HOME", None)
            result = resolve_config_path()
    ...
    def test_from_global_config_uses_default_profile_fallback(self, tmp_path, monkeypatch):
        ...
        # via the HOME-anchored helper, not Path.home() / ".jacky".
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
        default_home = fake_home / ".jacky"
        profile_home = default_home / "profiles" / "work"
...
class TestResolveActiveHost:
    def test_profile_host_key_uses_honcho_safe_separator(self):
        assert profile_host_key("coder") == "jacky_coder"
        assert profile_host_key("default") == "jacky"

    def test_default_returns_jacky(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("JACKY_HONCHO_HOST", None)
            os.environ.pop("JACKY_HOME", None)
            assert resolve_active_host() == "jacky"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"JACKY_HONCHO_HOST": "jacky.coder"}):
            assert resolve_active_host() == "jacky.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JACKY_HONCHO_HOST", None)
            with patch("jacky_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "jacky_coder"

    def test_default_profile_returns_jacky(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JACKY_HONCHO_HOST", None)
            with patch("jacky_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "jacky"

    def test_custom_profile_returns_jacky(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JACKY_HONCHO_HOST", None)
            with patch("jacky_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "jacky"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("JACKY_HONCHO_HOST", None)
            # Temporarily remove jacky_cli.profiles to simulate import failure
            saved = sys.modules.get("jacky_cli.profiles")
            sys.modules["jacky_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "jacky"
            finally:
                if saved is not None:
                    sys.modules["jacky_cli.profiles"] = saved
                else:
                    sys.modules.pop("jacky_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jacky_coder")
        assert config.host == "jacky_coder"
        assert config.workspace_id == "jacky"  # shared workspace
        assert config.ai_peer == "jacky_coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="jacky")
        assert config.host == "jacky"
        assert config.workspace_id == "jacky"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "jacky": {"aiPeer": "jacky", "peerName": "alice"},
                "jacky_coder": {
                    "aiPeer": "jacky_coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="jacky_coder", config_path=config_file,
        )
        assert config.host == "jacky_coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "jacky_dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="jacky_dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "jacky_dreamer"
    ...
    def test_from_global_config_reads_legacy_dot_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "jacky.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        config = HonchoClientConfig.from_global_config(
            host="jacky_dreamer",
        ...
        assert config.host == "jacky_dreamer"
        assert config.peer_name == "dreamer-user"
        assert config.workspace_id == "jacky_dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"jacky": {"enabled": True, "aiPeer": "jacky"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"jacky": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
...
class TestGetHonchoClient:
    ...
    def test_jacky_config_timeout_override_used_when_config_timeout_missing(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="jacky",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("jacky_cli.config.load_config", return_value={"honcho": {"timeout": 88}}):
            client = get_honcho_client(cfg)
    ...
    def test_jacky_request_timeout_alias_used(self):
        fake_honcho = MagicMock(name="Honcho")
        cfg = HonchoClientConfig(
            api_key="test-key",
            workspace_id="jacky",
        ...
        with patch("honcho.Honcho", return_value=fake_honcho) as mock_honcho, \
             patch("jacky_cli.config.load_config", return_value={"honcho": {"request_timeout": "77.5"}}):
            client = get_honcho_client(cfg)

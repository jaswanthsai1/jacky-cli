"""
Tests for timezone support (jacky_time module + integration points).
...
def _reset_jacky_time_cache():
    """Reset the jacky_time module cache (replacement for removed reset_cache)."""
    jacky_time._cached_tz = None
    jacky_time._cached_tz_name = None
    jacky_time._cache_resolved = False


# =========================================================================
# jacky_time.now() — core helper
# =========================================================================

class TestJackyTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_jacky_time_cache()
    ...
    def test_valid_timezone_applies(self):
        ...
        os.environ["JACKY_TIMEZONE"] = "Asia/Kolkata"
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["JACKY_TIMEZONE"] = "Europe/London"
        tz = jacky_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["JACKY_TIMEZONE"] = "Not/A/Timezone"
        tz = jacky_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When JACKY_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) JACKY_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("JACKY_TIMEZONE=" + os.environ.get("JACKY_TIMEZONE", "NOT_SET"))'
        )
        with patch("jacky_cli.model_tools.handle_function_call", side_effect=self._mock_handle):
        ...
        assert "JACKY_TIMEZONE=NOT_SET" in result["output"], (
            "JACKY_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When JACKY_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("JACKY_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=jacky_tz) which shifted
        absolute time when system-local tz != Jacky tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_jacky_tz(self):
        """Already-aware datetimes should be normalized to Jacky tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Jacky tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Jacky tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Jacky is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Jacky tz
        ...
        # Use a Jacky timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _jacky_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["JACKY_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_jacky_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Jacky timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["JACKY_TIMEZONE"] = "US/Eastern"
        _reset_jacky_time_cache()

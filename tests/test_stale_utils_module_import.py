...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.jacky/jacky-agent/utils.py)
...
2. ``jacky update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``jacky_cli/main.py`` (the ``jacky update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``jacky_constants`` after the

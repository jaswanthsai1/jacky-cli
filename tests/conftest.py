"""Shared fixtures for the jacky-agent test suite.
...
2. **Isolated JACKY_HOME.** JACKY_HOME points to a per-test tempdir so
   code reading ``~/.jacky/*`` via ``get_jacky_home()`` can't see the
   real one. (We do NOT also redirect HOME — that broke subprocesses in
   CI. Code using ``Path.home() / ".jacky"`` instead of the canonical
   ``get_jacky_home()`` is a bug to fix at the callsite.)
3. **Deterministic runtime.** TZ=UTC, LANG=C.UTF-8, PYTHONHASHSEED=0.
4. **No JACKY_SESSION_* inheritance** — the agent's current gateway
...
# JACKY_* vars that change test behavior by being set. Unset all of these
# unconditionally — individual tests that need them set do so explicitly.
_JACKY_BEHAVIORAL_VARS = frozenset({
    "JACKY_YOLO_MODE",
    "JACKY_INTERACTIVE",
    "JACKY_QUIET",
    "JACKY_TOOL_PROGRESS",
    "JACKY_TOOL_PROGRESS_MODE",
    "JACKY_MAX_ITERATIONS",
    "JACKY_SESSION_PLATFORM",
    "JACKY_SESSION_CHAT_ID",
    "JACKY_SESSION_CHAT_NAME",
    "JACKY_SESSION_THREAD_ID",
    "JACKY_SESSION_SOURCE",
    "JACKY_SESSION_KEY",
    "JACKY_GATEWAY_SESSION",
    "JACKY_CRON_SESSION",
    "_JACKY_GATEWAY",
    "JACKY_PLATFORM",
    "JACKY_MODEL",
    "JACKY_INFERENCE_MODEL",
    "JACKY_INFERENCE_PROVIDER",
    "JACKY_TUI_PROVIDER",
    "JACKY_MANAGED",
    "JACKY_MANAGED_DIR",
    "JACKY_DEV",
    ...
    "JACKY_TIMEZONE",
    "JACKY_REDACT_SECRETS",
    "JACKY_BACKGROUND_NOTIFICATIONS",
    "JACKY_EXEC_ASK",
    "JACKY_HOME_MODE",
    "JACKY_AGENT_USE_LEGACY_SESSION_KEYS",
    # Kanban path/board pins must never leak from a developer shell or
    # dispatched worker into tests; otherwise tests can write fake tasks to
    # the real ~/.jacky/kanban.db instead of the per-test JACKY_HOME.
    "JACKY_KANBAN_DB",
    "JACKY_KANBAN_BOARD",
    "JACKY_KANBAN_HOME",
    "JACKY_KANBAN_WORKSPACES_ROOT",
    "JACKY_KANBAN_LOGS_ROOT",
    "JACKY_KANBAN_TASK",
    "JACKY_KANBAN_WORKSPACE",
    "JACKY_KANBAN_RUN_ID",
    "JACKY_KANBAN_CLAIM_LOCK",
    "JACKY_KANBAN_DISPATCH_IN_GATEWAY",
    "JACKY_TENANT",
    ...
    "JACKY_DASHBOARD_OAUTH_CLIENT_ID",
    "JACKY_DASHBOARD_PORTAL_URL",
    "TERMINAL_CWD",
    "TERMINAL_ENV",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    Also redirects HOME and JACKY_HOME to per-test tempdirs so code that
    reads ``~/.jacky/*`` can't touch the real one, and pins TZ/LANG so
    ...
    # 2. Blank behavioral JACKY_* vars that could change test semantics.
    for name in _JACKY_BEHAVIORAL_VARS:
        monkeypatch.delenv(name, raising=False)

    # 3. Redirect JACKY_HOME to a per-test tempdir. Code that reads
    #    ``~/.jacky/*`` via ``get_jacky_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.jacky/*`` via
    #    ``Path.home() / ".jacky"`` instead of ``get_jacky_home()``
    #    is a bug to fix at the callsite.
    fake_jacky_home = tmp_path / "jacky_test"
    fake_jacky_home.mkdir()
    (fake_jacky_home / "sessions").mkdir()
    (fake_jacky_home / "cron").mkdir()
    (fake_jacky_home / "memories").mkdir()
    (fake_jacky_home / "skills").mkdir()
    monkeypatch.setenv("JACKY_HOME", str(fake_jacky_home))
    ...
    #    ~/.jacky/plugins/ (which, per step 3, is now empty — but the
    #    singleton might still be cached from a previous test).
    try:
        import jacky_cli.plugins as _plugins_mod
...
@pytest.fixture(autouse=True)
def _isolate_jacky_home(_hermetic_environment):
...
@pytest.fixture()
def mock_config():
    """Return a minimal jacky config dict suitable for unit tests."""
    return {
...
# environment and finds the developer's live ``jacky-gateway`` process
...
# correlated with a ``tests/jacky_cli/`` pytest run starting up.
#
...
#    ``check_output`` reject any ``systemctl ... <verb> jacky-gateway``
...
def _live_system_guard(request, monkeypatch):
    ...
    tokens[0]), so ``bash -c "systemctl restart jacky-gateway"``,
    ``sudo systemctl ...``, ``env systemctl ...``, ``setsid systemctl ...``
    are all caught. ``pkill``/``killall``/``taskkill`` invocations
    targeting jacky/python patterns are also blocked.
    """
    ...
    def _matches_jacky_gateway(cmd_str: str) -> bool:
        low = cmd_str.lower()
        return any(tok in low for tok in _JACKY_TOKENS)

    def _is_blocked_systemctl(cmd) -> bool:
        cmd_str = _cmd_to_string(cmd)
        if "systemctl" not in cmd_str:
            return False
        if not _matches_jacky_gateway(cmd_str):
            return False
        try:
    ...
    def _is_process_killer(cmd) -> bool:
        ...
        for tok in tokens:
            ...
            if head in _PROCESS_KILLERS:
                low = cmd_str.lower()
                # pkill -f pattern: catch jacky-themed patterns + a
                # plain "python" -f which would catch the live gateway
                # whose cmdline contains "python -m jacky_cli.main".
                if (
                    "jacky" in low
                    or "gateway" in low
    ...
    def _check_subprocess_cmd(name, cmd):
        if _is_blocked_systemctl(cmd):
            raise RuntimeError(
                ...
                "live jacky-gateway systemd unit. Mock "
        ...
        if _is_process_killer(cmd):
            raise RuntimeError(
                ...
                "targeting jacky/python could hit the live gateway. "
                "Mark with @pytest.mark.live_system_guard_bypass if "
                "intentional."
            )
        # Block any subprocess that would run `jacky update` (or the
        # equivalent `python -m jacky_cli.main update`).  These commands
        ...
        if "update" in low and (
            # jacky update / jacky update --gateway / setsid bash -c ... jacky update
            ("jacky" in low and "update" in low.split())
            or
            # python -m jacky_cli.main update --gateway
            ("jacky_cli" in low and "update" in low.split())
            or
            # venv/bin/jacky update  (absolute path variant used in tests)
            (".venv/bin/jacky" in low and "update" in low)
        ):
            raise RuntimeError(
                f"tests/conftest.py live-system guard: blocked "
                f"subprocess.{name}({cmd!r}) — this command would run "
                "`jacky update` against the real checkout, fetching "

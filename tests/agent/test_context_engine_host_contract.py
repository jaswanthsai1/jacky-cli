...
engine plugins (e.g. jacky-lcm) rely on:
...
def test_discover_context_engines_includes_plugin_registered_engines(monkeypatch):
    """Plugin-registered context engines appear in the ``jacky plugins`` picker."""
    ...
    monkeypatch.setattr(
        "jacky_cli.plugins.discover_plugins",
        lambda *_a, **_kw: None,
    )
    monkeypatch.setattr(
        "jacky_cli.plugins.get_plugin_context_engine",

...
def test_env_var_override(monkeypatch):
    """JACKY_LANGUAGE wins over config."""
    i18n.reset_language_cache()
    monkeypatch.setenv("JACKY_LANGUAGE", "ja")
    assert i18n.get_language() == "ja"


def test_env_var_normalized(monkeypatch):
    i18n.reset_language_cache()
    monkeypatch.setenv("JACKY_LANGUAGE", "Chinese")
    assert i18n.get_language() == "zh"


def test_default_when_nothing_set(monkeypatch):
    """With no env var and no config override, falls back to English."""
    monkeypatch.delenv("JACKY_LANGUAGE", raising=False)
...
def test_locales_dir_env_override_used_when_dir_exists(tmp_path, monkeypatch):
    """JACKY_BUNDLED_LOCALES wins when it points at a real directory."""
    bundled = tmp_path / "bundled-locales"
    bundled.mkdir()
    monkeypatch.setenv("JACKY_BUNDLED_LOCALES", str(bundled))
    assert i18n._locales_dir() == bundled


def test_locales_dir_env_override_ignored_when_missing(tmp_path, monkeypatch):
    """A bogus JACKY_BUNDLED_LOCALES falls through to source/wheel resolution
    instead of returning a path that doesn't exist."""
    monkeypatch.setenv("JACKY_BUNDLED_LOCALES", str(tmp_path / "does-not-exist"))
...
def test_locales_dir_falls_back_to_data_scheme(tmp_path, monkeypatch):
    ...
    # No env override.
    monkeypatch.delenv("JACKY_BUNDLED_LOCALES", raising=False)

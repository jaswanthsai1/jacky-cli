...
def test_translate_native_response_surfaces_reasoning_and_tool_calls():
        ...
        "candidates": [
            {
                "content": {
                    "parts": [
                        ...
                        {"functionCall": {"name": "search", "args": {"q": "jacky"}}},
                    ]
                },
                "finishReason": "STOP",
            }
    ...
    assert json.loads(choice.message.tool_calls[0].function.arguments) == {"q": "jacky"}
...
def test_max_tokens_none_defaults_to_gemini_output_ceiling():
    ...
    maxOutputTokens is absent, truncating tool calls mid-stream. Jacky passes

...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Jacky Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_jacky_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async

    monkeypatch.setenv("HOME", str(tmp_path))
    monkeypatch.setenv("JACKY_HOME", str(tmp_path / ".jacky"))

    jacky_env = tmp_path / ".jacky" / ".env"
    jacky_env.parent.mkdir(parents=True)
    jacky_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.jacky/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
...
async def test_blocks_canonical_read_denylist_credential_stores(tmp_path: Path, monkeypatch):
    ...
    a chat peer could otherwise attach `@file:~/.jacky/auth.json` and read the
    ...
    oauth = jacky_home / ".anthropic_oauth.json"
    oauth.write_text('{"access_token": "OAUTH-SECRET"}\n', encoding="utf-8")

    mcp_token = jacky_home / "mcp-tokens" / "github.json"
    ...
    result = await preprocess_context_references_async(
        "inspect @file:.jacky/auth.json and @file:.jacky/.anthropic_oauth.json "
        "and @file:.jacky/mcp-tokens/github.json and @file:project/.env",
        cwd=tmp_path,
        allowed_root=tmp_path,
        context_length=100_000,
    )

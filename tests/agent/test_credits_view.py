...
from jacky_cli.nous_account import NousPortalAccountInfo, NousPaidServiceAccessInfo
...
def test_view_logged_out_when_no_token(monkeypatch):
    monkeypatch.setattr("jacky_cli.auth.get_provider_auth_state", lambda provider: {})
...
def test_view_fetch_failure_is_logged_out(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.nous_account.get_nous_portal_account_info", _boom)
...
def test_credits_command_registered():
    from jacky_cli.commands import resolve_command, COMMAND_REGISTRY
...
def test_cli_show_credits_non_interactive_renders_text_not_modal(monkeypatch, capsys):
    ...
    monkeypatch.setattr(JackyCLI, "_prompt_text_input_modal", _boom_modal, raising=False)

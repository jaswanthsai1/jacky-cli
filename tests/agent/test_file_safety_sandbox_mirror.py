...
Those paths look like ``…/sandboxes/<backend>/<task>/home/.jacky/…`` and
they accumulate divergent copies of authoritative profile state (SOUL.md,
config.yaml, memories/*.md) because the host Jacky process never reads
...
class TestClassifySandboxMirrorTarget:
    def test_docker_mirror_soul_md_classified(self, tmp_path):
        ...
        target = (
            tmp_path
            / "profiles" / "group1"
            / "sandboxes" / "docker" / "default" / "home" / ".jacky"
    ...
    def test_other_backends_and_inner_files_match(self, tmp_path, backend, inner):
        ...
        target = (
            tmp_path
            / "sandboxes" / backend / "task-42" / "home" / ".jacky"
            / Path(inner)
        )
    ...
    def test_path_outside_sandbox_returns_none(self, tmp_path):
        """A plain Jacky path is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target

        target = tmp_path / ".jacky" / "profiles" / "group1" / "SOUL.md"
        target.parent.mkdir(parents=True)
        target.write_text("# real SOUL\n")

        assert classify_sandbox_mirror_target(str(target)) is None

    def test_sandboxes_segment_without_home_jacky_returns_none(self, tmp_path):
        """A ``sandboxes/`` directory unrelated to Jacky-state mirroring (e.g.
    ...
    def test_sandboxes_segment_with_home_but_no_jacky_returns_none(self, tmp_path):
        """``sandboxes/<backend>/<task>/home/anything-not-jacky`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
    ...
    def test_truncated_sandbox_path_returns_none(self, tmp_path):
        """``…/sandboxes/<backend>/<task>`` without ``home/.jacky/<thing>`` is not a mirror."""
        from agent.file_safety import classify_sandbox_mirror_target
...
class TestGetSandboxMirrorWarning:
    ...
    def test_mirror_warning_names_mirror_root_and_inner_path(self, tmp_path):
        ...
        assert "sandboxes/docker/default/home/.jacky" in warn
...
class TestSandboxMirrorIsOrthogonalToCrossProfile:
    ...
    def test_same_profile_mirror_still_flagged(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(fs, "_jacky_root_path", lambda: tmp_path)
        monkeypatch.setattr(fs, "_jacky_home_path", lambda: tmp_path / "profiles" / "group1")

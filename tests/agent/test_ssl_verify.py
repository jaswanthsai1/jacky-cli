...
_CA_ENV_VARS = ("JACKY_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE")
...
def test_jacky_ca_bundle_returns_ssl_context(clean_ca_env, monkeypatch):
    monkeypatch.setenv("JACKY_CA_BUNDLE", certifi.where())
...
def test_missing_ca_bundle_falls_back_to_true(clean_ca_env, monkeypatch):
    monkeypatch.setenv("JACKY_CA_BUNDLE", "/nonexistent/root-ca.pem")

...
def test_jacky_client_tag_includes_current_version():
    """The client tag must reflect jacky_cli.__version__ verbatim."""
    from jacky_cli import __version__
    from agent.portal_tags import jacky_client_tag

    assert jacky_client_tag() == f"client=jacky-client-v{__version__}"


def test_jacky_client_tag_format():
    """The client tag has the exact shape Nous Portal expects."""
    from agent.portal_tags import jacky_client_tag

    tag = jacky_client_tag()
    assert tag.startswith("client=jacky-client-v")
...
def test_nous_portal_tags_contains_product_and_client():
    ...
    from agent.portal_tags import jacky_client_tag, nous_portal_tags

    tags = nous_portal_tags()
    assert "product=jacky-agent" in tags
    assert jacky_client_tag() in tags
    assert len(tags) == 2

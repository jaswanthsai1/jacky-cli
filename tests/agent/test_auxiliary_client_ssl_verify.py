...
``JACKY_CA_BUNDLE`` via ``agent.ssl_verify.resolve_httpx_verify``. Auxiliary calls
...
_CA_ENV_VARS = ("JACKY_CA_BUNDLE", "SSL_CERT_FILE", "REQUESTS_CA_BUNDLE", "HTTPS_PROXY")
...
def test_resolve_aux_verify_uses_per_provider_ssl_ca_cert(clean_tls_env, monkeypatch):
    ...
    # jacky_cli.config, so patch it at the source module.
    monkeypatch.setattr(
        cfg,

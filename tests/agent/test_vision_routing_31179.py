...
def isolated_home(monkeypatch):
    """Temp JACKY_HOME with config + clean credential env vars."""
    test_home = tempfile.mkdtemp(prefix="jacky_test_31179_")
    jacky_home = os.path.join(test_home, ".jacky")
    os.makedirs(jacky_home)
...
def _fresh_modules():
    """Drop cached jacky modules so each test reloads against current env."""

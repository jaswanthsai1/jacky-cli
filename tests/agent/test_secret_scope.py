...
class TestMultiplexActiveFailClosed:
    ...
    def test_global_env_still_reads_environ_under_multiplex(self, monkeypatch):
        ...
        ss.set_multiplex_active(True)
        # No scope, multiplex on — but JACKY_HOME is global, so no raise.
        assert ss.get_secret("JACKY_HOME") == "/opt/data"

    def test_kanban_prefix_is_global(self, monkeypatch):
        monkeypatch.setenv("JACKY_KANBAN_DB", "/x/kanban.db")
        ss.set_multiplex_active(True)
        assert ss.get_secret("JACKY_KANBAN_DB") == "/x/kanban.db"

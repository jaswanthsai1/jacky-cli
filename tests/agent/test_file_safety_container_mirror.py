...
``…/sandboxes/<backend>/<task>/home/.jacky/…`` prefix. This covers the
complementary inner-container case: when file tools execute inside Docker,
the bind-mount strips that prefix and the guard sees plain ``/root/.jacky/…``.
...
class TestClassifyContainerMirrorTarget:
    def test_returns_none_without_context(self):
        """No Docker context — /root/.jacky/… must not be flagged."""
        from agent.file_safety import classify_container_mirror_target

        assert classify_container_mirror_target("/root/.jacky/profiles/group1/SOUL.md") is None
    ...
    def test_catches_soul_md_with_context(self):
        ...
        result = classify_container_mirror_target(
            "/root/.jacky/profiles/group1/SOUL.md",
            mirror_prefix="/root/.jacky",
        )
        assert result is not None
        assert result["mirror_root"].replace("\\", "/").endswith("root/.jacky")
    ...
    def test_non_jacky_path_not_flagged(self):
        """/root/workspace/… is not .jacky state and must not be blocked."""
        from agent.file_safety import classify_container_mirror_target
...
class TestOrthogonality:
    ...
    def test_inner_container_path_caught_by_context_guard(self):
        ...
        path = "/root/.jacky/profiles/group1/SOUL.md"

        assert classify_container_mirror_target(path) is None  # no context
        assert classify_container_mirror_target(path, mirror_prefix="/root/.jacky") is not None

...
def test_transient_retry_count_default(monkeypatch):
    ...
    with patch("jacky_cli.config.load_config", return_value={}), \
         patch("jacky_cli.config.cfg_get", return_value=None):
...
def test_transient_retry_count_configurable_and_clamped():
    ...
    with patch("jacky_cli.config.cfg_get", return_value=4):
        assert ac._transient_retry_count() == 4
    with patch("jacky_cli.config.cfg_get", return_value=100):
        assert ac._transient_retry_count() == 6  # clamped high
    with patch("jacky_cli.config.cfg_get", return_value=-3):
        assert ac._transient_retry_count() == 0  # clamped low
    with patch("jacky_cli.config.cfg_get", side_effect=RuntimeError):
        assert ac._transient_retry_count() == ac._DEFAULT_TRANSIENT_RETRIES

...
def jacky_home(monkeypatch):
    """Isolate JACKY_HOME for ``reload_skills`` tests.

    Rather than popping cache-bearing modules from ``sys.modules``,
    we monkeypatch the module-level ``JACKY_HOME`` / ``SKILLS_DIR``
    constants in place so the isolation is local to this fixture's scope.
    """
    td = tempfile.mkdtemp(prefix="jacky-reload-skills-")
    monkeypatch.setenv("JACKY_HOME", td)
    home = Path(td)
    ...
    monkeypatch.setattr(_st, "JACKY_HOME", home, raising=False)
...
class TestReloadSkillsHelper:
    ...
    def test_returns_expected_keys(self, jacky_home):
    ...
    def test_detects_newly_added_skill_with_description(self, jacky_home):
        from agent.skill_commands import reload_skills, get_skill_commands

        # Prime the cache so subsequent diff is meaningful
        get_skill_commands()

        _write_skill(jacky_home / "skills", "demo", "a demo skill")
        result = reload_skills()
    ...
    def test_detects_removed_skill_carries_description(self, jacky_home):
        from agent.skill_commands import reload_skills

        skill_dir = _write_skill(jacky_home / "skills", "demo", "soon to be gone")
    ...
    def test_description_passes_through_verbatim(self, jacky_home):
        ...
        long_desc = "x" * 200
        _write_skill(jacky_home / "skills", "longdesc", long_desc)

        result = reload_skills()
        assert len(result["added"]) == 1
        assert result["added"][0]["description"] == long_desc

    def test_unchanged_skills_appear_in_unchanged_list(self, jacky_home):
        from agent.skill_commands import reload_skills, get_skill_commands

        _write_skill(jacky_home / "skills", "alpha")
        # Prime cache
        get_skill_commands()
    ...
    def test_does_not_invalidate_prompt_cache_snapshot(self, jacky_home):

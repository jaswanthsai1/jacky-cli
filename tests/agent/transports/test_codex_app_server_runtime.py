...
class TestSpawnEnvIsolation:
    ...
    def test_kanban_worker_adds_only_kanban_writable_root(self, monkeypatch):
        ...
        danger-full-access. Jacky passes a narrow app-server config override
        ...
        monkeypatch.setattr(subprocess, "Popen", FakePopen)
        monkeypatch.setenv("HOME", "/users/alice")
        monkeypatch.setenv("JACKY_HOME", "/users/alice/.jacky/profiles/backend-worker")
        monkeypatch.setenv("JACKY_KANBAN_TASK", "t_smoke")
        monkeypatch.setenv(
            "JACKY_KANBAN_DB",
            "/users/alice/.jacky/kanban/boards/smoke/kanban.db",
        )
        ...
        assert (
            'sandbox_workspace_write.writable_roots=["/users/alice/.jacky/kanban/boards/smoke"]'
            in cmd
        )
...
class TestSpawnEnvSecretStripping:
    """codex app-server routes its spawn env through jacky_subprocess_env(
    ...
    credentials to authenticate, but it must NOT inherit Tier-1 Jacky secrets
    ...
    def test_tier1_and_internal_secrets_stripped_from_spawn_env(self, monkeypatch):
        for var, val in {
            ...
            "JACKY_DASHBOARD_SESSION_TOKEN": "dash-secret",
        ...
        for var in (
            ...
            "JACKY_DASHBOARD_SESSION_TOKEN", "AUXILIARY_VISION_API_KEY",
    ...
    def test_home_still_preserved_through_helper(self, monkeypatch):
        """Regression guard: routing through jacky_subprocess_env must not

...
def curator_env(tmp_path, monkeypatch):
    """Isolated JACKY_HOME + freshly reloaded curator + skill_usage modules."""
    home = tmp_path / ".jacky"
    ...
    # save_state() resolves the state path from JACKY_HOME at write time, so a
    # straggler thread that outlives this test would write into whatever home
    # the *next* test has configured (or the default ~/.jacky once monkeypatch
    ...
    # thread here, while JACKY_HOME is still pinned to this test's tmp home
...
def test_dry_run_does_not_advance_state(curator_env, monkeypatch):
    ...
    `jacky curator status`. Fixes #18373.
    """
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
...
def test_run_review_consolidate_override_runs_llm(curator_env, monkeypatch):
    ...
    the LLM consolidation pass — mirrors `jacky curator run --consolidate`."""
    c = curator_env["curator"]
    u = curator_env["usage"]
    skills_dir = curator_env["home"] / "skills"
    _write_skill(skills_dir, "a")
...
def test_cli_unpin_refuses_bundled_skill(curator_env, capsys):
    """jacky curator unpin must refuse bundled/hub skills too (matches pin)."""
    from jacky_cli import curator as cli
    skills_dir = curator_env["home"] / "skills"
...
# `jacky model` → auxiliary picker, the dashboard Models tab, and the full
...
def test_curator_slot_is_canonical_aux_task():
    ...
    from jacky_cli.main import _AUX_TASKS
    from jacky_cli.web_server import _AUX_TASK_SLOTS
    ...
    # 2. jacky_cli/main.py _AUX_TASKS — CLI picker
    aux_keys = {k for k, _name, _desc in _AUX_TASKS}
    assert "curator" in aux_keys, "curator missing from _AUX_TASKS (CLI picker)"

    # 3. jacky_cli/web_server.py _AUX_TASK_SLOTS — REST API allowlist
...
def test_review_fork_runs_under_background_review_origin(curator_env, monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.run_agent.AIAgent", _StubAgent)

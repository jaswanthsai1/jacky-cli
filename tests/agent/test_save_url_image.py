...
def http_server(tmp_path, monkeypatch):
    """Spin up a localhost HTTP server and isolate JACKY_HOME under tmp_path."""
    monkeypatch.setenv("JACKY_HOME", str(tmp_path / ".jacky"))
    (tmp_path / ".jacky").mkdir()

    # Force the constants/image cache helpers to re-read JACKY_HOME.
    import sys
    for mod in list(sys.modules):
        if mod.startswith("jacky_constants") or mod.startswith("agent.image_gen_provider"):
...
class TestSaveUrlImage:
    def test_writes_real_bytes_to_jacky_home_cache(self, http_server):
        base, _ = http_server
        ...
        # The cache directory must be under JACKY_HOME — gateway cleanup

...
path via ``jacky chat --query``) couldn't see what the aggregator actually
produced without joining to the session DB by hand.

These exercise the real ``consume_and_save_trace`` → ``save_moa_turn`` path
with real file I/O against a temp JACKY_HOME — no mocks on the write path.
"""
...
def _enable_traces(tmp_path, monkeypatch):
    """Point JACKY_HOME at a temp dir and turn moa.save_traces on."""
    jacky_home = tmp_path / ".jacky"
    jacky_home.mkdir()
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))

    # save_moa_turn reads config via jacky_cli.config.load_config; stub it to

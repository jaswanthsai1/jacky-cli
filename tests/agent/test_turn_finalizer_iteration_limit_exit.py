...
def test_pending_verify_response_is_preserved_for_cron_delivery(monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    agent = _LimitAgent()
...
def test_pending_response_records_kanban_timeout(monkeypatch):
    monkeypatch.setattr("jacky_cli.plugins.invoke_hook", lambda *_a, **_kw: [])
    monkeypatch.setenv("JACKY_KANBAN_TASK", "task-123")
    record = MagicMock(name="record_task_failure")
    conn = SimpleNamespace(close=lambda: None)
    monkeypatch.setattr("jacky_cli.kanban_db.connect", lambda: conn)
    monkeypatch.setattr("jacky_cli.kanban_db._record_task_failure", record)
    agent = _LimitAgent()

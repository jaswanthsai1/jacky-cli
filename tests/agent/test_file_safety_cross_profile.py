"""Tests for the cross-Jacky-profile write guard in agent/file_safety.

The guard fires when a tool tries to write into another Jacky profile's
...
Reference: May 2026 incident — a jacky-security profile session
accidentally edited skills under both ~/.jacky/profiles/jacky-security/skills/
AND ~/.jacky/skills/ (the default profile's skills), realizing only
...
# ---------------------------------------------------------------------------
# Helpers — set up a fake Jacky root with two profiles, monkeypatch the
# resolver helpers so the classifier sees the test layout.
# ---------------------------------------------------------------------------


@pytest.fixture
def fake_jacky(tmp_path, monkeypatch):
    """Build a fake Jacky layout:
    ...
    sec_home = root / "profiles" / "jacky-security"
    ...
    monkeypatch.setattr(fs, "_jacky_root_path", lambda: root)
...
def _set_active_home(monkeypatch, jacky_home: Path):
    """Point file_safety._jacky_home_path at a specific profile dir."""
    import agent.file_safety as fs
...
class TestResolveActiveProfileName:
    def test_default_when_home_is_root(self, fake_jacky, monkeypatch):
        _set_active_home(monkeypatch, fake_jacky["default_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "default"

    def test_named_profile(self, fake_jacky, monkeypatch):
        _set_active_home(monkeypatch, fake_jacky["security_home"])
        from agent.file_safety import _resolve_active_profile_name
        assert _resolve_active_profile_name() == "jacky-security"

    def test_falls_back_to_default_on_resolution_failure(self, fake_jacky, monkeypatch):
        """If JACKY_HOME resolution raises, return 'default' rather than crashing the tool."""
        import agent.file_safety as fs

        def _boom():
            raise RuntimeError("simulated")

        monkeypatch.setattr(fs, "_jacky_home_path", _boom)
...
class TestClassifyCrossProfileTarget:
    def test_same_profile_write_returns_none(self, fake_jacky, monkeypatch):
        _set_active_home(monkeypatch, fake_jacky["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_jacky["security_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is None

    def test_security_writing_default_skill(self, fake_jacky, monkeypatch):
        """The exact incident from May 2026."""
        _set_active_home(monkeypatch, fake_jacky["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_jacky["default_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["active_profile"] == "jacky-security"
        assert result["target_profile"] == "default"
        assert result["area"] == "skills"

    def test_default_writing_security_skill(self, fake_jacky, monkeypatch):
        ...
        assert result["target_profile"] == "jacky-security"

    def test_named_to_named_cross_profile(self, fake_jacky, monkeypatch):
        _set_active_home(monkeypatch, fake_jacky["security_home"])
        from agent.file_safety import classify_cross_profile_target
        result = classify_cross_profile_target(
            str(fake_jacky["coder_home"] / "skills" / "foo" / "SKILL.md")
        )
        assert result is not None
        assert result["target_profile"] == "coder"

    @pytest.mark.parametrize("area", ["skills", "plugins", "cron", "memories"])
    def test_all_profile_scoped_areas_classified(self, fake_jacky, monkeypatch, area):
        _set_active_home(monkeypatch, fake_jacky["security_home"])
        from agent.file_safety import classify_cross_profile_target
        target = fake_jacky["default_home"] / area / "foo.txt"
        result = classify_cross_profile_target(str(target))
        assert result is not None
        assert result["area"] == area

    def test_non_jacky_path_returns_none(self, fake_jacky, monkeypatch, tmp_path):
        _set_active_home(monkeypatch, fake_jacky["security_home"])
        from agent.file_safety import classify_cross_profile_target
        # Path outside any Jacky root
        assert classify_cross_profile_target(str(tmp_path / "random.txt")) is None

    def test_jacky_config_not_classified_as_cross_profile(self, fake_jacky, monkeypatch):
        ...
        result = classify_cross_profile_target(
            str(fake_jacky["default_home"] / "config.yaml")
        )
        assert result is None
...
class TestGetCrossProfileWarning:
    def test_in_profile_returns_none(self, fake_jacky, monkeypatch):
        _set_active_home(monkeypatch, fake_jacky["security_home"])
    ...
    def test_cross_profile_warning_names_both_profiles(self, fake_jacky, monkeypatch):
        _set_active_home(monkeypatch, fake_jacky["security_home"])
        from agent.file_safety import get_cross_profile_warning
        ...
        assert "jacky-security" in warn
        # Must name the bypass kwarg.
        assert "cross_profile=True" in warn
        # Must reference the area.
        assert "skills" in warn

    def test_warning_is_defense_in_depth_not_boundary(self, fake_jacky, monkeypatch):
        _set_active_home(monkeypatch, fake_jacky["security_home"])
        from agent.file_safety import get_cross_profile_warning
        warn = get_cross_profile_warning(
            str(fake_jacky["default_home"] / "skills" / "foo" / "SKILL.md")

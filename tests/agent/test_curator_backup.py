...
def backup_env(monkeypatch, tmp_path):
    """Isolate JACKY_HOME + reload modules so every test starts clean."""
    home = tmp_path / ".jacky"
    home.mkdir()
    (home / "skills").mkdir()
...
def test_real_run_takes_pre_snapshot(backup_env, monkeypatch):
    ...
    # Reload curator module against the freshly-env'd jacky_constants
...
def _write_cron_jobs(home: Path, jobs: list) -> Path:
    """Write a synthetic cron/jobs.json under JACKY_HOME. Returns the path.
...
def test_snapshot_without_cron_jobs_file_still_succeeds(backup_env):
    ...
    cb = backup_env["cb"]
    _write_skill(backup_env["skills"], "alpha")
    # Deliberately do not create ~/.jacky/cron/jobs.json

...
Reports live under ``~/.jacky/logs/curator/{YYYYMMDD-HHMMSS}/`` alongside
...
def curator_env(tmp_path, monkeypatch):
    """Isolated JACKY_HOME with a skills/ dir + reset curator module state."""
    home = tmp_path / ".jacky"
    home.mkdir()
    (home / "skills").mkdir()
    (home / "logs").mkdir()
...
def curator_env_with_cron(curator_env, monkeypatch):
    ...
    monkeypatch.setattr(jobs_mod, "JACKY_DIR", home)

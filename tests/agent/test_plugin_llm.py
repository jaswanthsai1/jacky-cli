...
""",
        ...
        from jacky_cli import config as _config_mod
    ...
    def test_missing_plugin_entry_yields_default_deny(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / "config.yaml").write_text("plugins: {}\n", encoding="utf-8")
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        from jacky_cli import config as _config_mod
        _config_mod._config_cache = None  # type: ignore[attr-defined]
...
class TestPluginContextIntegration:
    def test_ctx_llm_is_lazy_singleton(self):
        from jacky_cli.plugins import PluginContext, PluginManifest, PluginManager

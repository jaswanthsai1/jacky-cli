...
def _isolate_env(tmp_path, monkeypatch):
    """Isolate JACKY_HOME for each test.

    The global hermetic fixture already redirects JACKY_HOME to a tempdir,
    but we want the plugin to work with a predictable subpath. We reset
    JACKY_HOME here for clarity.
    """
...
def _load_plugin_init():
    ...
    spec = importlib.util.spec_from_file_location(
        "jacky_plugins.disk_cleanup",
        plugin_dir / "__init__.py",
        submodule_search_locations=[str(plugin_dir)],
    )
    # Ensure parent namespace package exists for the relative `. import disk_cleanup`
    import types
    if "jacky_plugins" not in sys.modules:
        ns = types.ModuleType("jacky_plugins")
        ns.__path__ = []
        sys.modules["jacky_plugins"] = ns
    mod = importlib.util.module_from_spec(spec)
    mod.__package__ = "jacky_plugins.disk_cleanup"
    mod.__path__ = [str(plugin_dir)]
    sys.modules["jacky_plugins.disk_cleanup"] = mod
    spec.loader.exec_module(mod)
    return mod
...
class TestIsSafePath:
    def test_accepts_path_under_jacky_home(self, _isolate_env):
        dg = _load_lib()
    ...
    def test_rejects_outside_jacky_home(self, _isolate_env):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/etc/passwd")) is False

    def test_accepts_tmp_jacky_prefix(self, _isolate_env, tmp_path):
        dg = _load_lib()
        assert dg.is_safe_path(Path("/tmp/jacky-abc/x.log")) is True
...
class TestTrackForgetQuick:
    ...
    def test_quick_does_not_descend_into_protected_top_level_dirs(
        ...
        dg = _load_lib()
        protected_empty = (
            _isolate_env / "jacky-agent" / "node_modules" / "pkg" / "empty"
        )
        ...
        def guarded_iterdir(path):
            if path == _isolate_env / "jacky-agent":
                raise AssertionError("quick() descended into protected jacky-agent/")
...
class TestBundledDiscovery:
    def _write_enabled_config(self, jacky_home, names):
    ...
    def test_disk_cleanup_discovered_but_not_loaded_by_default(self, _isolate_env):
        ...
        from jacky_cli import plugins as pmod

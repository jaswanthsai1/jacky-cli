...
_ISSUER = "https://auth.example.com/application/o/jacky"
_CLIENT_ID = "jacky-dashboard"
...
class TestStartLogin:
    ...
    def test_returns_login_start(self, provider):
        result = provider.start_login(
            redirect_uri="https://jacky.example/auth/callback"
        )
    ...
    def test_authorize_url_has_required_params(self, provider):
        ...
        assert params["redirect_uri"] == "https://jacky.example/auth/callback"
    ...
    def test_two_calls_differ(self, provider):
        a = provider.start_login(redirect_uri="https://jacky.example/auth/callback")
        b = provider.start_login(redirect_uri="https://jacky.example/auth/callback")
        assert (
            a.cookie_payload["jacky_session_pkce"]
            != b.cookie_payload["jacky_session_pkce"]
        )
    ...
    def test_allows_http_with_arbitrary_host(self, provider):
        ...
        provider.start_login(redirect_uri="http://jacky.example/auth/callback")
...
class TestPluginRegister:
    ...
    def clear_env(self, monkeypatch):
        for var in (
            "JACKY_DASHBOARD_OIDC_ISSUER",
            "JACKY_DASHBOARD_OIDC_CLIENT_ID",
            "JACKY_DASHBOARD_OIDC_SCOPES",
            "JACKY_DASHBOARD_OIDC_CLIENT_SECRET",
        ):
    ...
    def test_skips_when_unconfigured(self, patch_config):
        ...
        assert "JACKY_DASHBOARD_OIDC_ISSUER" in oidc_plugin.LAST_SKIP_REASON
        assert "self_hosted" in oidc_plugin.LAST_SKIP_REASON

    def test_skips_when_only_issuer_set(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_ISSUER", _ISSUER)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_not_called()
    ...
    def test_registers_from_env(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
    ...
    def test_empty_env_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_ISSUER", "")
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_CLIENT_ID", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        ctx.register_dashboard_auth_provider.assert_called_once()
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_config_load_failure_falls_through(self, monkeypatch):
        ...
        monkeypatch.setattr("jacky_cli.config.load_config", _broken)
        ctx = MagicMock()
    ...
    def test_non_https_issuer_skips_with_reason(self, patch_config, monkeypatch):
        patch_config(None)
        monkeypatch.setenv(
            "JACKY_DASHBOARD_OIDC_ISSUER", "http://insecure.example"
        )
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        ctx = MagicMock()
        oidc_plugin.register(ctx)
    ...
    def test_secret_from_env(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_CLIENT_SECRET", "env-secret")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
    ...
    def test_empty_env_secret_does_not_shadow_config(self, patch_config, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_CLIENT_SECRET", "")
        ctx = MagicMock()
        oidc_plugin.register(ctx)
        registered = ctx.register_dashboard_auth_provider.call_args.args[0]
        assert registered._client_secret == "cfg-secret"
    ...
    def test_register_log_reports_confidential_not_secret(
        ...
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_ISSUER", _ISSUER)
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_CLIENT_ID", _CLIENT_ID)
        monkeypatch.setenv("JACKY_DASHBOARD_OIDC_CLIENT_SECRET", "logme-secret")
        ctx = MagicMock()
        with caplog.at_level(logging.INFO):
            oidc_plugin.register(ctx)

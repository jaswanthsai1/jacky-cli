...
def test_post_setup_existing_profile_picker_validates_and_links_saved_profile(tmp_path, monkeypatch):
    ...
    from jacky_cli import memory_setup
    ...
    OpenVikingMemoryProvider().post_setup(str(jacky_home), config)
...
def test_post_setup_create_remote_user_profile_can_mirror_to_openviking_store(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            ...
            "Jacky peer ID in OpenViking": "jacky",
...
def test_post_setup_create_remote_user_can_keep_jacky_only(tmp_path, monkeypatch):
    _clear_openviking_env(monkeypatch)
    jacky_home = tmp_path / "jacky"
    jacky_home.mkdir()
    monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    ...
    monkeypatch.setattr(
        ...
        _prompt_from_values({
            "OpenViking server URL": "https://openviking.example",
            "OpenViking user API key": "user-secret",
            "Jacky peer ID in OpenViking": "agent",
        }),
    )
    config = {"memory": {}}
    ...
    env_text = (jacky_home / ".env").read_text(encoding="utf-8")
...
def test_post_setup_remote_blank_api_key_cancels_without_saving(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(jacky_config, "save_config", save_config)
    choices = iter([1, 0, 1])
    monkeypatch.setattr(memory_setup, "_curses_select", lambda *args, **kwargs: next(choices))
    monkeypatch.setattr(
        memory_setup,
...
def test_post_setup_user_key_path_can_route_detected_root_key_to_root_setup(tmp_path, monkeypatch):
    ...
    assert prompt_events.count("Jacky peer ID in OpenViking") == 1
    env_text = (jacky_home / ".env").read_text(encoding="utf-8")
...
def test_start_local_openviking_server_writes_output_to_log(tmp_path, monkeypatch):
    ...
    def fake_popen(args, **kwargs):
        ...
        assert kwargs["stdout"].name == str(jacky_home / "logs" / "openviking-server.log")
    ...
    assert started is True
    assert str(jacky_home / "logs" / "openviking-server.log") in message
    assert popen_calls
...
def test_https_local_endpoint_is_not_runtime_autostart_eligible(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory disabled for this Jacky run. "
...
def test_runtime_openviking_waiter_warns_when_background_start_times_out(monkeypatch):
    ...
    assert warnings == [
        ...
        "within 60 seconds. OpenViking memory disabled for this Jacky run."
    ]
...
def test_tool_add_resource_rejects_jacky_credential_file_upload(tmp_path, monkeypatch):
    import agent.file_safety as fs

    jacky_home = tmp_path / "jacky_home"
    jacky_home.mkdir()
    auth_json = jacky_home / "auth.json"
    auth_json.write_text('{"OPENROUTER_API_KEY":"sk-test-secret"}', encoding="utf-8")
    monkeypatch.setattr(fs, "_jacky_home_path", lambda: jacky_home)
...
def test_tool_add_resource_directory_zip_skips_jacky_credential_files(tmp_path, monkeypatch):
    import agent.file_safety as fs

    jacky_home = tmp_path / "jacky_home"
    jacky_home.mkdir()
    (jacky_home / "guide.md").write_text("# Guide\n", encoding="utf-8")
    ...
    def inspect_upload(path):
        ...
        return "upload_jacky_home.zip"

    provider._client.upload_temp_file.side_effect = inspect_upload
    provider._client.post.return_value = {
        "status": "ok",
        "result": {"root_uri": "viking://resources/jacky_home"},
    }

    result = json.loads(provider._tool_add_resource({"url": str(jacky_home)}))
...
def test_handle_tool_call_forget_deletes_exact_memory_file_uri():
    uri = "viking://user/peers/jacky/memories/preferences/mem_abc123.md"
    provider = OpenVikingMemoryProvider()
    provider._client = MagicMock()
...
@pytest.mark.parametrize("uri", [
    ...
    "viking://agent/jacky/memories/preferences/mem_abc123.md",
...
def test_viking_client_delete_uses_identity_headers(monkeypatch):
    ...
    assert captured["kwargs"]["headers"]["X-OpenViking-Actor-Peer"] == "jacky"
...
def test_viking_client_headers_include_bearer_when_api_key_set():
    ...
    assert headers["X-OpenViking-Actor-Peer"] == "jacky"
    assert "X-OpenViking-Agent" not in headers
    assert "X-OpenViking-Account" not in headers
    assert "X-OpenViking-User" not in headers
...
def test_viking_client_health_sends_auth_headers(monkeypatch):
    ...
    assert captured["headers"]["X-OpenViking-Actor-Peer"] == "jacky"
...
def test_validate_openviking_setup_values_local_dev_no_key_uses_health_only(monkeypatch):
    ...
    ok, message, role = openviking_module._validate_openviking_setup_values(
        {"endpoint": "localhost", "agent": "jacky"}
    )
...
# ---------------------------------------------------------------------------
# on_session_switch — flush + commit + rotate behavior (jacky-agent#28296)
# ---------------------------------------------------------------------------
...
def test_sync_turn_captures_session_id_before_worker_runs():
    ...
    assert captured_payloads == [{
        "messages": [
            ...
            {"role": "assistant", "parts": [{"type": "text", "text": "a"}], "peer_id": "jacky"},
        ]
    }]
...
def test_sync_turn_structured_messages_include_assistant_peer_id():
    ...
    assert captured == [(
        "/api/v1/sessions/sid-structured/messages/batch",
        {
            "messages": [
                {"role": "user", "parts": [{"type": "text", "text": "u"}]},
                {"role": "assistant", "parts": [{"type": "text", "text": "Looking."}], "peer_id": "jacky"},
                {
                    "role": "assistant",
                    "parts": [
                        {
...
def test_session_needs_commit_guard_wins_over_stale_turn_count():
    """Regression for jacky-agent#28296 review (M3): once a session is marked
...
def test_on_session_switch_does_not_block_caller_on_slow_drain():
    """Regression for jacky-agent#28296 review (H1): on_session_switch must
...
def test_on_session_switch_defers_old_commit_to_finalizer_thread():
    ...
    -_DEFERRED_COMMIT_TIMEOUT drain or the commit POST. See jacky-agent#28296
...
def test_on_memory_write_uses_content_write_independent_of_session_rotation():
    ...
    assert captured_payloads[0]["uri"].startswith(
        "viking://user/peers/jacky/memories/preferences/mem_"
    )
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/jacky/memories/events/mem_1.md",
                            "score": 0.9,
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/jacky/memories/events/mem_2.md",
                            "score": 0.8,
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/jacky/memories/events/mem_3.md",
                            "score": 0.9,
                            "level": 2,
                            "category": "events",
...
def test_prefetch_reads_empty_abstract_content_within_budget(monkeypatch):
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/jacky/memories/one.md",
                            "score": 0.9,
                            "abstract": "",
                        },
                    ],
    ...
    assert [params["uri"] for _path, params in captured_reads] == [
        "viking://user/peers/jacky/memories/one.md",
    ]
    assert (
        "content for viking://user/peers/jacky/memories/one.md"
        in context
    )
...
def test_prefetch_caps_full_content_reads(monkeypatch):
        ...
        def post(self, path, payload=None, **kwargs):
            return {
                "result": {
                    "memories": [
                        {
                            "uri": f"viking://user/peers/jacky/memories/events/mem_{idx}.md",
                            "score": 0.9 - (idx * 0.01),
                            "level": 2,
                            "category": "events",
    ...
    assert "full content for viking://user/peers/jacky/memories/events/mem_0.md" in context
    assert "full content for viking://user/peers/jacky/memories/events/mem_1.md" in context
        ...
        def post(self, path, payload=None, **kwargs):
            ...
            return {
                "result": {
                    "memories": [
                        {
                            "uri": "viking://user/peers/jacky/memories/events/mem_timeout.md",
                            "score": 0.9,
                            "level": 2,
                            "category": "events",
                            "abstract": "short abstract",

...
def _inject_fake_jacky_cli(monkeypatch):
    """Inject fake jacky_cli modules so yaml/curses aren't required."""
    fake_config_mod = types.ModuleType("jacky_cli.config")
    fake_config_mod.save_config = lambda c: None

    fake_setup_mod = types.ModuleType("jacky_cli.memory_setup")
    fake_setup_mod._curses_select = lambda *a, **kw: 0
    fake_setup_mod._prompt = lambda label, default=None, secret=False: default or ""

    fake_jacky_cli = types.ModuleType("jacky_cli")
    fake_jacky_cli.config = fake_config_mod
    fake_jacky_cli.memory_setup = fake_setup_mod

    monkeypatch.setitem(sys.modules, "jacky_cli", fake_jacky_cli)
    monkeypatch.setitem(sys.modules, "jacky_cli.config", fake_config_mod)
    monkeypatch.setitem(sys.modules, "jacky_cli.memory_setup", fake_setup_mod)
...
class TestPostSetup:
    ...
    def test_platform_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["jacky", "--mode", "platform", "--api-key", "sk-test"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_jacky_home", lambda: tmp_path)
        _inject_fake_jacky_cli(monkeypatch)
        config = {"memory": {}}
    ...
    def test_oss_flag_mode(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "jacky", "--mode", "oss", "--oss-llm-key", "sk-oai",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_jacky_home", lambda: tmp_path)
        _inject_fake_jacky_cli(monkeypatch)
    ...
    def test_selfhosted_flag_mode(self, tmp_path, monkeypatch):
        ...
        assert mem0_json["user_id"] == "jacky-user"
    ...
    def test_selfhosted_no_api_key_auth_disabled(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("sys.argv", [
            "jacky", "--mode", "self-hosted", "--host", "http://mem0.lan:8888",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_jacky_home", lambda: tmp_path)
...
class TestDryRun:
    ...
    def test_dry_run_platform_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", ["jacky", "--mode", "platform", "--api-key", "sk-test", "--dry-run"])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_jacky_home", lambda: tmp_path)
        _inject_fake_jacky_cli(monkeypatch)
        config = {"memory": {}}
        post_setup(str(tmp_path), config)
    ...
    def test_dry_run_oss_no_files(self, tmp_path, monkeypatch):
        monkeypatch.setattr("sys.argv", [
            "jacky", "--mode", "oss", "--oss-llm-key", "sk-oai", "--dry-run",
        ])
        monkeypatch.setattr("plugins.memory.mem0._setup.get_jacky_home", lambda: tmp_path)
        _inject_fake_jacky_cli(monkeypatch)
        monkeypatch.setattr("plugins.memory.mem0._setup._install_provider_deps", lambda l, e, v: None)

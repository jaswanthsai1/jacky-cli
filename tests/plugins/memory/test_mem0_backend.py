...
class TestPlatformBackend:
    ...
    def test_add_forwards_kwargs(self):
        ...
        result = backend.add(msgs, user_id="u1", agent_id="jacky", infer=False)
        call = client.calls[0]
    ...
    def test_add_omits_empty_metadata(self):
        backend, client = self._make()
        msgs = [{"role": "user", "content": "hi"}]
        backend.add(msgs, user_id="u1", agent_id="jacky", infer=False, metadata={})
...
class TestOSSBackend:
    ...
    def test_add_forwards_kwargs(self):
        backend, memory = self._make()
        msgs = [{"role": "user", "content": "hi"}]
        backend.add(msgs, user_id="u1", agent_id="jacky", infer=False)
...
class TestSelfHostedBackend:
    ...
    def test_add_posts_messages_and_identity(self):
        ...
        result = _backend(s).add(msgs, user_id="u1", agent_id="jacky", infer=False, metadata={"channel": "cli"})
        req = s.requests[-1]
        assert (req.method, req.url.path) == ("POST", "/memories")
        import json
        body = json.loads(req.content)
        assert body == {"messages": msgs, "user_id": "u1", "agent_id": "jacky",

...
def provider(monkeypatch, tmp_path):
    ...
    p.initialize("session-1", jacky_home=str(tmp_path), platform="cli")
    return p
...
def test_format_prefetch_context_deduplicates_overlap():
    result = _format_prefetch_context(
        ...
        dynamic_facts=["Jordan prefers short answers", "Uses Jacky"],
        search_results=[{"memory": "Uses Jacky", "similarity": 0.9}],
        max_results=10,
    )
    assert result.count("Jordan prefers short answers") == 1
    assert result.count("Uses Jacky") == 1
...
def test_prefetch_includes_profile_on_first_turn(provider):
    provider._client.profile_response = {
        ...
        "search_results": [{"memory": "Working on Jacky memory provider", "similarity": 0.88}],
    }
...
def test_merge_metadata_stamps_sm_source():
    ...
    assert merged2["sm_source"] == "jacky"
...
def test_identity_template_resolved_in_container_tag(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SUPERMEMORY_API_KEY", "test-key")
    monkeypatch.setattr("plugins.memory.supermemory._SupermemoryClient", FakeClient)
    _save_supermemory_config({"container_tag": "jacky-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", jacky_home=str(tmp_path), platform="cli", agent_identity="coder")
    assert p._container_tag == "jacky_coder"
...
def test_identity_template_default_profile(monkeypatch, tmp_path):
    ...
    _save_supermemory_config({"container_tag": "jacky-{identity}"}, str(tmp_path))
    p = SupermemoryMemoryProvider()
    p.initialize("s1", jacky_home=str(tmp_path), platform="cli")
    assert p._container_tag == "jacky_default"
...
def test_multi_container_enabled_adds_schema_param(monkeypatch, tmp_path):
    ...
    assert p._allowed_containers == ["jacky", "project_alpha", "shared"]
...
def test_format_connection_summary_ok():
    summary = _format_connection_summary({
        "ok": True,
        "container_tag": "jacky_coder",
        "profile_facts": 12,
        "auto_recall": True,
        "auto_capture": False,
    })
    assert "✓ Connected" in summary
    assert "container: jacky_coder" in summary
...
def test_probe_supermemory_connection_missing_key(tmp_path):
    ...
    assert status["container_tag"] == "jacky"
...
def test_get_status_config_returns_summary(monkeypatch, tmp_path):
    ...
    assert "container: jacky" in result["summary"]
...
def test_post_setup_writes_config_and_prints_summary(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr("jacky_cli.config.save_config", fake_save_config)

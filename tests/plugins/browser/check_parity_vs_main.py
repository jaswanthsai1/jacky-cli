...
Run from the PR worktree:
    ...
    cd ~/.jacky/jacky-agent/.worktrees/browser-providers-plugin
...
# checkout lives two levels up at ``~/.jacky/jacky-agent``.
MAIN_DIR = REPO_ROOT.parent.parent  # ~/.jacky/jacky-agent
PR_DIR = REPO_ROOT  # the worktree we're in
assert (MAIN_DIR / "tools" / "browser_tool.py").exists(), (
    f"MAIN_DIR={MAIN_DIR} doesn't look like a jacky-agent checkout"
)
...
# Isolated JACKY_HOME for the config write.

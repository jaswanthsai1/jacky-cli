...
# Resolve the skill's scripts dir across layouts: standalone dev repo (tests/) and jacky-agent
...
_CANDIDATES = [
    ...
    _HERE.parent.parent.parent.joinpath(*_REL),          # jacky-agent (tests/skills/)
]
...
def test_cdp_default_profile_uses_jacky_home():
    prev = os.environ.get("JACKY_HOME")
    with tempfile.TemporaryDirectory() as d:
        os.environ["JACKY_HOME"] = d
        try:
...
def test_dotenv_env_fills_missing_creds_and_shell_wins():
    prev_home = os.environ.get("JACKY_HOME")
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        try:
            ...
            for k, v in (("JACKY_HOME", prev_home), ("BROWSERBASE_API_KEY", prev_key)):
                if v is None:

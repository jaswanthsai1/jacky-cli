...
class TestGoogleWorkspaceCredentialFiles:
    ...
    def test_entries_are_registered_when_files_exist(self, tmp_path):
        ...
        (jacky_home / "google_client_secret.json").write_text("{}")
        ...
        try:
            ...
            assert "/root/.jacky/google_token.json" in container_paths
            assert "/root/.jacky/google_client_secret.json" in container_paths
        finally:
            clear_credential_files()
    ...
    def test_missing_token_is_reported(self, tmp_path):
        ...
        try:
            ...
            container_paths = {m["container_path"] for m in mounts}
            assert "/root/.jacky/google_client_secret.json" in container_paths
            assert "/root/.jacky/google_token.json" not in container_paths
        finally:
            clear_credential_files()

...
class TestExpandParentToolsets(unittest.TestCase):
    ...
    def test_composite_jacky_cli_expands_web(self):
        """jacky-cli includes web_search/web_extract → 'web' should be in expansion."""
        expanded = _expand_parent_toolsets({"jacky-cli"})
        self.assertIn("web", expanded)
        self.assertIn("terminal", expanded)
        self.assertIn("browser", expanded)
        # Original composite is preserved
        self.assertIn("jacky-cli", expanded)
    ...
    def test_intersection_with_expanded_composite(self):
        """End-to-end: requesting ['web'] from parent with ['jacky-cli'] yields ['web']."""
        parent_toolsets = {"jacky-cli"}

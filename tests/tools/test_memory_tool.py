...
class TestScanMemoryContent:
    ...
    def test_jacky_config_mod_blocked(self):
        result = _scan_memory_content("edit .jacky/config.yaml to change settings")
        assert "Blocked" in result
        assert "jacky_config_mod" in result
        result = _scan_memory_content("update .jacky/SOUL.md with new personality")
        assert "Blocked" in result
        assert "jacky_config_mod" in result
    ...
    def test_jacky_config_mod_no_false_positives(self):
        """Merely mentioning jacky config files should not trigger; only modify intent should."""
        assert _scan_memory_content("Check .jacky/config.yaml for settings") is None
        assert _scan_memory_content("Read .jacky/SOUL.md for agent personality") is None
        assert _scan_memory_content("The .jacky/config.yaml file contains runtime options") is None

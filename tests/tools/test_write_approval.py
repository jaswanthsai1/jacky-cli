...
and the shared slash-command handlers (jacky_cli/write_approval_commands.py).
...
@pytest.fixture
def jacky_home(monkeypatch):
    d = tempfile.mkdtemp(prefix="jacky_wa_test_")
    home = os.path.join(d, ".jacky")
    os.makedirs(home)
    monkeypatch.setenv("JACKY_HOME", home)
    yield home
    shutil.rmtree(d, ignore_errors=True)


def _set_approval(subsystem, enabled):
    import jacky_cli.config as cfg
    c = cfg.load_config()
...
def test_default_gate_is_off(jacky_home):
...
def test_invalid_subsystem_is_off(jacky_home):
    from tools import write_approval as wa
...
def test_memory_gate_off_allows_write(jacky_home):
...
def test_memory_gate_on_no_interactive_stages(jacky_home):
...
def test_memory_gate_on_then_apply(jacky_home):
...
def test_cli_memory_approve_without_live_agent_uses_fresh_store(jacky_home, capsys):
...
def test_load_on_disk_store_honors_configured_char_limits(jacky_home, monkeypatch):
    ...
    monkeypatch.setattr("jacky_cli.config.load_config", _boom)
...
def test_skill_gate_off_allows_create(jacky_home):
...
def test_skill_gate_on_always_stages(jacky_home):
...
def test_skill_gate_on_then_apply_writes_file(jacky_home):
    # SKILLS_DIR is resolved at import time, so reload the skill module under
    # this test's JACKY_HOME to exercise the real on-disk write path.
    import importlib
    import tools.skill_manager_tool as smt
    importlib.reload(smt)
    from tools import write_approval as wa
...
def test_skill_create_diff_is_full_content(jacky_home):
    from tools.skill_manager_tool import skill_manage
    from tools import write_approval as wa
    _set_approval("skills", True)
...
def test_pending_store_roundtrip(jacky_home):
    from tools import write_approval as wa
...
def test_handle_pending_list_empty(jacky_home):
...
def test_handle_approve_all(jacky_home):
    from jacky_cli.write_approval_commands import handle_pending_subcommand
...
def test_handle_reject(jacky_home):
    from jacky_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_approval_on(jacky_home):
    from jacky_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
...
def test_handle_approval_off(jacky_home):
    from jacky_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
    captured = {}
    out = handle_pending_subcommand(
...
def test_handle_mode_alias_still_works(jacky_home):
...
def test_handle_approval_invalid(jacky_home):
    from jacky_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_handle_unknown_subcommand_returns_none(jacky_home):
    from jacky_cli.write_approval_commands import handle_pending_subcommand
    from tools import write_approval as wa
...
def test_memory_inline_approve_writes(jacky_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_deny_blocks(jacky_home, approval_callback_cleanup):
    from tools.memory_tool import memory_tool, MemoryStore
    from tools.terminal_tool import set_approval_callback
    from tools import write_approval as wa
    _set_approval("memory", True)
...
def test_memory_inline_callback_error_stages(jacky_home, approval_callback_cleanup):
...
def test_gateway_context_stages_not_prompts(jacky_home, monkeypatch):
    ...
    from tools import write_approval as wa
    _set_approval("memory", True)
    monkeypatch.setenv("JACKY_GATEWAY_SESSION", "1")
...
def test_skills_never_prompt_inline_even_with_callback(jacky_home, approval_callback_cleanup):
...
def test_memory_invalid_params_rejected_before_staging(jacky_home):

...
class TestJackyTokenStorage:
    def test_roundtrip_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        storage = JackyTokenStorage("test-server")
    ...
    def test_token_file_created_with_0o600(self, tmp_path, monkeypatch):
        ...
        """
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        storage = JackyTokenStorage("perm-test-server")
    ...
    def test_has_cached_tokens(self, tmp_path, monkeypatch):
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        storage = JackyTokenStorage("my-server")
    ...
    def test_corrupt_tokens_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        storage = JackyTokenStorage("bad-server")
...
class TestPathTraversal:
    ...
    def test_path_traversal_blocked(self, tmp_path, monkeypatch):
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        storage = JackyTokenStorage("../../.ssh/config")
    ...
    def test_dots_and_slashes_sanitized(self, tmp_path, monkeypatch):
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        storage = JackyTokenStorage("../../../etc/passwd")
        path = storage._tokens_path()
...
class TestNonInteractiveFailFastAtCallbackBoundary:
    ...
    def test_boundary_errors_point_at_jacky_mcp_login(self, monkeypatch):
        """Both boundaries emit an actionable next step."""
        import tools.mcp_oauth as mod
        import asyncio

        monkeypatch.setattr(mod, "_is_interactive", lambda: False)
        with pytest.raises(OAuthNonInteractiveError, match="jacky mcp login"):
...
def test_build_oauth_auth_preserves_server_url_path():
    ...
    ``OAuthContext.get_authorization_base_url``; Jacky must not pre-strip.
    """
    ...
    with patch.object(mcp_oauth, "_OAUTH_AVAILABLE", True), \
         ...
         patch.object(mcp_oauth, "JackyTokenStorage") as mock_storage_cls:

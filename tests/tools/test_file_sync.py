...
def _make_get_files(tmp_files, remote_base="/root/.jacky"):
...
def _make_manager(tmp_files, remote_base="/root/.jacky", upload=None, delete=None):
...
class TestEdgeCases:
    ...
    def test_file_disappears_between_list_and_upload(self, tmp_path):
        ...
        upload = MagicMock()
        mgr = FileSyncManager(
            get_files_fn=lambda: [(str(f), "/root/.jacky/ephemeral.txt")],
            upload_fn=upload,
            delete_fn=MagicMock(),
        )
    ...
    def test_sync_back_does_not_overwrite_uploaded_credential_files(self, tmp_path, monkeypatch):
            ...
            lambda: [
                {
                    ...
                    "container_path": "/root/.jacky/credentials/token.json",
                }
            ],
        )
        monkeypatch.setattr(
            "tools.credential_files.iter_skills_files",
            lambda container_base="/root/.jacky": [
                {
                    "host_path": str(skill),
        ...
        def bulk_download(dest: Path) -> None:
            with tarfile.open(dest, "w") as tar:
                for name, data in {
                    "root/.jacky/credentials/token.json": b"remote-token",
                    "root/.jacky/skills/skill.py": b"remote-skill",
                }.items():
        ...
        mgr.sync(force=True)
        mgr.sync_back(jacky_home=tmp_path)

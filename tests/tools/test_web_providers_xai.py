...
class TestXAIProviderIsAvailable:
    ...
    auth-store lock. It runs on every ``jacky tools`` repaint and at
    ...
    def test_available_via_auth_store(self, monkeypatch, tmp_path):
        """Cheap probe should detect xai-oauth tokens in ~/.jacky/auth.json
...
class TestXAIBackendWiring:
    ...
    def test_is_backend_available_does_not_call_resolver(self, monkeypatch):
        ...
        dispatch and every `jacky tools` repaint. It must not invoke the
    ...
    def test_xai_not_in_legacy_backend_candidate_chain(self, monkeypatch):
        ...
        chain is for users who set env vars but never ran ``jacky tools``,
...
class TestXAIProviderOAuthPath:
    ...
    runtime resolver (``jacky auth`` sign-in) rather than an env-var key.
    Patches at the ``jacky_cli.runtime_provider.resolve_runtime_provider``

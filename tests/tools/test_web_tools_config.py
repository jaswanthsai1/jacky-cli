...
class TestFirecrawlClientConfig:
    ...
    def test_nous_auth_token_respects_jacky_home_override(self, tmp_path):
        """Auth lookup should read from JACKY_HOME/auth.json, not ~/.jacky/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".jacky").mkdir(parents=True)
...
class TestBackendSelection:
    ...
    ``jacky tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
...
class TestCheckWebApiKey:
    ...
    def test_null_web_section_does_not_crash(self):
        ...
        with patch("jacky_cli.config.load_config", return_value={"web": None}):
    ...
    def test_tool_gateway_availability_skips_refresh_for_expired_cached_token(
        ...
        monkeypatch.setattr(
            "jacky_cli.auth.resolve_nous_access_token",
            _record_refresh,
        )
...
class TestFirecrawlEnvResolution:
    """Verify Firecrawl reads env values from jacky_cli.config.get_env_value,
    not just os.getenv.  This catches the regression reported in #40190 where
    values stored in ~/.jacky/.env were invisible to the provider."""
...
class TestSiblingProvidersEnvResolution:
    ...
    ~/.jacky/.env), not bare os.getenv. Parametrized over the four

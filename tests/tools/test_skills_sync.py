...
class TestRmtreeWritableScopeGuard:
    ...
    exception — the result is a silent ``shutil.rmtree(~/.jacky/)``
    ...
    def test_refuses_jacky_home_itself(self, tmp_path):
        """``~/.jacky/`` itself is what the #48200 wipe destroyed."""
        from tools.skills_sync import _rmtree_writable

        jacky = tmp_path / "home"
        jacky.mkdir()
        (jacky / "skills").mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", jacky / "skills"):
            with pytest.raises(ValueError, match="refusing to rmtree"):
                _rmtree_writable(jacky)
    ...
    def test_refuses_sibling_directory(self, tmp_path):
        ...
        skills = jacky / "skills"
        skills.mkdir()
        not_skills = jacky / "kanban.db"  # any non-skills path
        not_skills.mkdir()
        with patch("tools.skills_sync.SKILLS_DIR", skills):
            with pytest.raises(ValueError, match="refusing to rmtree"):
...
class TestSyncSkills:
    ...
    def test_collision_prints_reset_hint(self, tmp_path, capsys):
        ...
        assert "jacky skills reset new-skill" in captured
...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """JACKY_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("JACKY_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("JACKY_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty JACKY_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("JACKY_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
...
class TestNoBundledSkillsOptOut:
    ...
    This is what `jacky profile create --no-skills` (named profiles) and the
    installer's `--no-skills` flag (default ~/.jacky) rely on so bundled
    skills are never seeded at install time NOR re-injected by `jacky update`.
    """
    ...
    def test_marker_skips_sync(self, tmp_path):
        ...
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        jacky_home = tmp_path / "home"
        jacky_home.mkdir()
        (jacky_home / ".no-bundled-skills").write_text("opted out\n")

        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.JACKY_HOME", jacky_home):
            result = sync_skills(quiet=True)
...
class TestOptOutToggleAndRemove:
    """`jacky skills opt-out/opt-in` core: marker toggle + safe removal."""
    ...
    def test_marker_toggle(self, tmp_path):
        ...
        home = tmp_path / "home"
        home.mkdir()
        with patch("tools.skills_sync.JACKY_HOME", home):
    ...
    def test_remove_keeps_user_modified(self, tmp_path):
        ...
        with patch("tools.skills_sync._get_bundled_dir", return_value=bundled), \
             ...
             patch("tools.skills_sync.SKILLS_DIR", skills_dir), \
             patch("tools.skills_sync.MANIFEST_FILE", manifest_file), \
             patch("tools.skills_sync.JACKY_HOME", home):
            sync_skills(quiet=True)
            # User edits 'beta'

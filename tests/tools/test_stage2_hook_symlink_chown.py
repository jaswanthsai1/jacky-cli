...
def _chown_jacky_tree_function(text: str) -> str:
...
def _run_helper(
    ...
    log_path: Path,
    *,
    jacky_home: Path | None = None,
) -> subprocess.CompletedProcess[str]:
    shell = shutil.which("sh")
    if shell is None:
        pytest.skip("sh not available")
    jacky_home = target if jacky_home is None else jacky_home
    script = (
        "set -eu\n"
        f'JACKY_HOME="{jacky_home}"\n'
        f"{_chown_jacky_tree_function(text)}\n"
        f'chown() {{ printf "%s\\n" "$*" >> "{log_path}"; }}\n'
        f'chown_jacky_tree "{target}"\n'
    )
...
def test_chown_helper_refuses_symlinked_directories(stage2_text: str, tmp_path: Path) -> None:
    ...
    real_home.mkdir()
    symlinked_home = tmp_path / "jacky-home"
    try:
...
def test_chown_helper_refuses_target_under_symlinked_home(
    ...
    proc = _run_helper(
        ...
        linked_home / "cron",
        log_path,
        jacky_home=linked_home,
    )

    assert proc.returncode == 0, proc.stderr
    assert not log_path.exists(), "must not chown through a symlinked JACKY_HOME"
    assert "refusing recursive chown through symlinked path" in proc.stdout
...
def test_stage2_uses_symlink_safe_helper_for_jacky_home_trees(stage2_text: str) -> None:
    assert 'chown_jacky_tree "$JACKY_HOME/$sub"' in stage2_text
    assert 'chown_jacky_tree "$JACKY_HOME/profiles"' in stage2_text
    assert 'chown_jacky_tree "$JACKY_HOME/cron"' in stage2_text
    assert 'chown -R jacky:jacky "$JACKY_HOME/$sub"' not in stage2_text
    assert 'chown -R jacky:jacky "$JACKY_HOME/profiles"' not in stage2_text
    assert 'chown -R jacky:jacky "$JACKY_HOME/cron"' not in stage2_text


def test_stage2_skips_top_level_chown_for_symlinked_jacky_home(
    stage2_text: str,
) -> None:
    assert 'refuse_symlinked_path "chown" "$JACKY_HOME"' in stage2_text

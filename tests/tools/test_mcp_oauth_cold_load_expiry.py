...
1. ``JackyTokenStorage.set_tokens`` persisted only relative ``expires_in``,
...
- ``JackyMCPOAuthProvider._initialize`` seeds ``context.token_expiry_time``
...
# ---------------------------------------------------------------------------
# JackyTokenStorage — absolute expiry persistence
# ---------------------------------------------------------------------------
...
class TestGetTokensReconstructsExpiresIn:
    ...
    def test_get_tokens_returns_zero_ttl_for_expired_token(
        ...
        monkeypatch.setenv("JACKY_HOME", str(tmp_path))
        from tools.mcp_oauth import JackyTokenStorage, _get_token_dir
...
# ---------------------------------------------------------------------------
# JackyMCPOAuthProvider._initialize — seed token_expiry_time
# ---------------------------------------------------------------------------
...
async def test_initialize_seeds_token_expiry_time_from_stored_tokens(
    ...
    from tools.mcp_oauth import JackyTokenStorage
    from tools.mcp_oauth_manager import _JACKY_PROVIDER_CLS, reset_manager_for_tests

    assert _JACKY_PROVIDER_CLS is not None
    ...
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Jacky Agent",
    )
    provider = _JACKY_PROVIDER_CLS(
        server_name="srv",
...
async def test_initialize_skips_prefetch_when_no_tokens(tmp_path, monkeypatch):
    ...
    storage = JackyTokenStorage("srv")  # empty — no tokens on disk
    metadata = OAuthClientMetadata(
        redirect_uris=[AnyUrl("http://127.0.0.1:12345/callback")],
        client_name="Jacky Agent",
    )

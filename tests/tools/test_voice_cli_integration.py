...
def _make_voice_cli(**overrides):
    """Create a minimal JackyCLI with only voice-related attrs initialized.
...
class TestVprintForceParameter:
    ...
    def test_error_messages_use_force_in_run_agent(self):
        ...
        include force=True."""
        with open("jacky_cli/run_agent.py", "r") as f:
            source = f.read()
...
class TestCtrlCResetsContinuousMode:
    ...
    def test_ctrl_c_handler_resets_voice_continuous(self):
        ...
        with open("jacky_cli/cli.py") as f:
            source = f.read()
...
class TestDisableVoiceModeStopsTTS:
    ...
    def test_disable_voice_mode_calls_stop_playback(self):
        ...
        source = inspect.getsource(JackyCLI._disable_voice_mode)
...
class TestEnableVoiceModeReal:
    ...
    @patch("jacky_cli.cli._cprint")
    @patch("jacky_cli.config.load_config", return_value={"voice": {}})
    ...
    @patch("jacky_cli.cli._cprint")
    @patch("jacky_cli.config.load_config", return_value={"voice": {"auto_tts": True}})
    @patch("tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
    ...
    @patch("jacky_cli.cli._cprint")
    @patch("jacky_cli.config.load_config", side_effect=Exception("broken config"))
    @patch("tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
...
class TestVoiceBeepConfigReal:
    ...
    @patch("jacky_cli.config.load_config", return_value={"voice": {"beep_enabled": False}})
    def test_beeps_can_be_disabled(self, _cfg):
        cli = _make_voice_cli()
        assert cli._voice_beeps_enabled() is False

    @patch("jacky_cli.cli._cprint")
    @patch("jacky_cli.cli.threading.Thread")
...
class TestVoiceSpeakResponseReal:
    ...
    def test_async_scheduling_clears_done_before_thread_start(self):
        ...
        with patch("jacky_cli.cli.threading.Thread", FakeThread):
    ...
    @patch("jacky_cli.cli._cprint")
    @patch("jacky_cli.cli.os.unlink")
    @patch("jacky_cli.cli.os.path.getsize", return_value=1000)
    @patch("jacky_cli.cli.os.path.isfile", return_value=True)
    @patch("jacky_cli.cli.os.makedirs")
...
class TestVoiceStopAndTranscribeReal:
    ...
    @patch("jacky_cli.cli.os.unlink")
    @patch("jacky_cli.cli.os.path.isfile", return_value=True)
    @patch("jacky_cli.config.load_config", return_value={"stt": {}})
    ...
    @patch("jacky_cli.cli.os.unlink")
    @patch("jacky_cli.cli.os.path.isfile", return_value=True)
    @patch("jacky_cli.config.load_config", return_value={"stt": {"model": "whisper-large-v3"}})
    @patch("tools.voice_mode.transcribe_recording",

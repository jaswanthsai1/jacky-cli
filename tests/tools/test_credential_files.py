...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        jacky_home.mkdir()
        (jacky_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"JACKY_HOME": str(jacky_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(jacky_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.jacky/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.jacky/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        jacky_home = tmp_path / ".jacky"
        (jacky_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"JACKY_HOME": str(jacky_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.jacky")

        assert mounts[0]["container_path"] == "/home/user/.jacky/skills"
...
class TestIterSkillsFiles:
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.jacky/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.jacky/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape JACKY_HOME."""
        monkeypatch.setenv("JACKY_HOME", str(tmp_path / ".jacky"))
        (tmp_path / ".jacky").mkdir()

        # Create a sensitive file one level above jacky_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside jacky_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside JACKY_HOME must still be registered."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        (jacky_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_jacky_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of JACKY_HOME must be allowed."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        subdir = jacky_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        """A symlink inside JACKY_HOME pointing outside must be rejected."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))

        # Create a sensitive file outside jacky_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside jacky_home pointing outside
        symlink = jacky_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes JACKY_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, jacky_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape JACKY_HOME."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
        ...
        sensitive.write_text("{}")
        self._write_config(jacky_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(jacky_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside JACKY_HOME via config must still mount."""
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))

        (jacky_home / "oauth.json").write_text("{}")
        self._write_config(jacky_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        (jacky_home / "cache" / "documents").mkdir(parents=True)
        (jacky_home / "cache" / "audio").mkdir(parents=True)
        (jacky_home / "cache" / "videos").mkdir(parents=True)
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.jacky/cache/documents" in paths
        assert "/root/.jacky/cache/audio" in paths
        assert "/root/.jacky/cache/videos" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.jacky/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        ``has content`` for ``get_jacky_dir``'s populated-legacy check
        (see #27602 — empty legacy stubs are no longer honoured).
        """
        jacky_home = tmp_path / ".jacky"
        jacky_home.mkdir()
        # Use legacy dir name with content — get_jacky_dir prefers
        # populated old over new.
        legacy_doc = jacky_home / "document_cache"
        legacy_img = jacky_home / "image_cache"
        legacy_doc.mkdir()
        legacy_img.mkdir()
        ...
        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(jacky_home / "document_cache") in host_paths
        assert str(jacky_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.jacky/cache/documents" in container_paths
        assert "/root/.jacky/cache/images" in container_paths
...
class TestMapCachePathToContainer:
    ...
    def test_maps_path_under_cache_dir(self, tmp_path, monkeypatch):
        jacky_home = tmp_path / ".jacky"
        img_dir = jacky_home / "cache" / "images"
        ...
        assert (
            ...
            == "/root/.jacky/cache/images/generated.png"
        )
    ...
    def test_custom_container_base_for_remote_home(self, tmp_path, monkeypatch):
        ...
        assert (
            map_cache_path_to_container(host_path, container_base="/home/agent/.jacky")
            == "/home/agent/.jacky/cache/images/remote.png"
        )

    def test_returns_none_when_outside_cache_dirs(self, tmp_path, monkeypatch):
        jacky_home = tmp_path / ".jacky"
        (jacky_home / "cache" / "images").mkdir(parents=True)
        monkeypatch.setenv("JACKY_HOME", str(jacky_home))
    ...
    def test_returns_none_when_no_cache_dirs_exist(self, tmp_path, monkeypatch):
        ...
        assert map_cache_path_to_container(str(jacky_home / "cache" / "images" / "x.png")) is None
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        doc_dir = jacky_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        jacky_home = tmp_path / ".jacky"
        ss_dir = jacky_home / "cache" / "screenshots"
        ...
        assert entries[0]["container_path"] == "/root/.jacky/cache/screenshots/session_abc/screen1.png"

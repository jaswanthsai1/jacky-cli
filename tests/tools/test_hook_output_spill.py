...
class GetSpillConfigTests(unittest.TestCase):
    ...
    def test_user_overrides_are_respected(self):
        ...
        with patch("jacky_cli.config.load_config", return_value=user_cfg):
            cfg = hos.get_spill_config()
    ...
    def test_load_config_exception_is_swallowed(self):
        with patch("jacky_cli.config.load_config", side_effect=RuntimeError("bad")):
            cfg = hos.get_spill_config()
        self.assertEqual(cfg["max_chars"], hos.DEFAULT_MAX_CHARS)
        self.assertTrue(cfg["enabled"])
...
class SpillIfOversizedTests(unittest.TestCase):
    def setUp(self):
        self.tmpdir = tempfile.mkdtemp(prefix="jacky-spill-test-")
    ...
    def test_session_id_with_path_separators_is_sanitised(self):
        ...
        self.assertFalse(Path("/etc/passwd-jacky-test").exists())
    ...
    def test_default_directory_uses_jacky_home(self):
        """When no directory override, spill under JACKY_HOME/hook_outputs."""
        test_home = tempfile.mkdtemp(prefix="jacky-home-")
        try:
            with patch.dict(os.environ, {"JACKY_HOME": test_home}):
                # Also patch get_jacky_home to the env var to mirror production.
            ...
            # ~/.jacky/hook_outputs depending on get_jacky_home behaviour.
            candidates = [
                Path(test_home) / "hook_outputs" / "sess",
                Path(os.path.expanduser("~/.jacky/hook_outputs/sess")),
            ]

...
- Signal forwarding runs through the init so ``docker stop`` triggers
  jacky's own graceful-shutdown path.
...
def test_dockerfile_installs_an_init_for_zombie_reaping(dockerfile_text):
    ...
    Without a PID-1 init that handles SIGCHLD, jacky accumulates zombie
    ...
    assert installed, (
        ...
        "reap orphaned subprocesses, jacky accumulates zombies in Docker "
...
def test_dockerfile_entrypoint_routes_through_the_init(dockerfile_text):
    ...
    directly, the shell becomes PID 1 and will ``exec`` into jacky,
    ...
    assert routes_through_init, (
        ...
        "not wired into ENTRYPOINT, jacky still runs as PID 1 and zombies "
...
def test_dockerfile_installs_tui_dependencies(dockerfile_text):
    ...
    # resolve dependencies. The bundled ``jacky-ink`` workspace package is
    ...
    assert "ui-tui/packages/jacky-ink/" in dockerfile_text
...
def test_dockerfile_preinstalls_hindsight_memory_dependency(dockerfile_text):
    ...
    assert any("--extra hindsight" in step for step in sync_steps), (
        ...
        "into /opt/jacky/.venv. It lazy-installs into the image layer (not the "
...
def test_dockerfile_materializes_local_tui_ink_package(dockerfile_text):
    ...
    assert "ui-tui/packages/jacky-ink/" in dockerfile_text, (
        "Dockerfile must COPY the bundled jacky-ink workspace package "
        "so ``await import('@jacky/ink')`` resolves at runtime."
    )

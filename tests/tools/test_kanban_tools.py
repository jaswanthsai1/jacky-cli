...
Verifies:
  - Tools are gated on JACKY_KANBAN_TASK: a normal chat session sees
...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    """Normal `jacky chat` sessions (no JACKY_KANBAN_TASK) must have
    ...
    schema = registry.get_definitions(set(resolve_toolset("jacky-cli")), quiet=True)
...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("JACKY_KANBAN_TASK", "t_fake")
    home = tmp_path / ".jacky"
    home.mkdir()
    monkeypatch.setenv("JACKY_HOME", str(home))
...
def test_kanban_worker_env_overrides_profile_toolset_filter(monkeypatch, tmp_path):
    ...
    import tools.kanban_tools  # ensure registered
    from jacky_cli.model_tools import _clear_tool_defs_cache, get_tool_definitions
...
def test_worker_with_kanban_toolset_still_hides_board_routing(monkeypatch, tmp_path):
    ...
    in its config, the JACKY_KANBAN_TASK env var means it's a focused
...
def worker_env(monkeypatch, tmp_path):
    """Simulate being a worker: JACKY_HOME isolated, JACKY_KANBAN_TASK set
    after we've created the task."""
    home = tmp_path / ".jacky"
    home.mkdir()
    monkeypatch.setenv("JACKY_HOME", str(home))
    monkeypatch.setenv("JACKY_PROFILE", "test-worker")
    monkeypatch.delenv("JACKY_SESSION_ID", raising=False)
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("JACKY_KANBAN_TASK", tid)
    return tid
...
def test_complete_metadata_round_trips_through_show(worker_env):
    ...
    handoff = {
        "changed_files": ["jacky_cli/kanban.py"],
...
def test_complete_stamps_worker_session_id_from_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("JACKY_SESSION_ID", "session-trusted")
...
def test_complete_goal_mode_rejected_by_judge(monkeypatch, tmp_path):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("JACKY_KANBAN_TASK", goal_task_id)
...
def _make_goal_mode_worker_env(monkeypatch, tmp_path):
    """Set up an isolated JACKY_HOME with one claimed goal_mode task,
...
def test_comment_happy_path(worker_env):
    ...
    try:
        ...
        # Author defaults to JACKY_PROFILE env we set in the fixture
...
def test_comment_ignores_caller_supplied_author(worker_env):
    ...
    derived from ``JACKY_PROFILE`` so a worker can't forge a comment
    under an authoritative-looking name like ``jacky-system`` and
    ...
    from tools import kanban_tools as kt
    out = kt._handle_comment({
        "task_id": worker_env, "body": "hi", "author": "jacky-system",
    })
    ...
    try:
        comments = kb.list_comments(conn, worker_env)
        # Author comes from JACKY_PROFILE in the fixture, not the
        # caller-supplied "jacky-system" override.
        assert comments[0].author == "test-worker"
    finally:
        conn.close()
...
def test_create_inherits_worker_dir_workspace(monkeypatch, worker_env):
    ...
    try:
        ...
        conn.close()
    monkeypatch.setenv("JACKY_KANBAN_TASK", self_tid)
...
def test_create_no_worker_task_stays_scratch(monkeypatch, worker_env):
    """Orchestrator/CLI callers (no JACKY_KANBAN_TASK) still default to
...
def test_create_session_id_arg_overrides_env(monkeypatch, worker_env):
    ...
    monkeypatch.setenv("JACKY_SESSION_ID", "from-env")
    from tools import kanban_tools as kt
    from jacky_cli import kanban_db as kb
    out = kt._handle_create({
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    """A normal chat session (no JACKY_KANBAN_TASK) must NOT have
...
def test_kanban_guidance_in_worker_prompt(monkeypatch, tmp_path):
    """A worker session (JACKY_KANBAN_TASK set) MUST have the full
...
# ---------------------------------------------------------------------------
#
# A worker process has JACKY_KANBAN_TASK set to its own task id. The
...
# are unrestricted.
#
# Orchestrator profiles (no JACKY_KANBAN_TASK in env) are intentionally
...
def test_worker_can_comment_on_foreign_task(worker_env):
    ...
    # JACKY_PROFILE — never to a caller-controlled string.
    conn = kb.connect()
    try:
...
def test_worker_complete_rejects_stale_run_id(worker_env, monkeypatch):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("JACKY_KANBAN_RUN_ID", str(run1.id))
    ...
    monkeypatch.setenv("JACKY_KANBAN_RUN_ID", str(run2.id))
...
def test_orchestrator_complete_any_task_allowed(monkeypatch, tmp_path):
    """Orchestrator profiles (no JACKY_KANBAN_TASK) can still complete
...
# ---------------------------------------------------------------------------
#
# The dispatcher pins the active board via JACKY_KANBAN_BOARD env var,
...
# restarting Jacky. These tests pin that ``board=<slug>`` argument
...
def multi_board_env(monkeypatch, tmp_path):
    """Isolated Jacky home with two distinct kanban boards seeded.

    Returns ``("default", "alt")`` slugs. The default board has one
    pre-existing task ``seed_default``; ``alt`` has ``seed_alt``. No
    JACKY_KANBAN_TASK is pinned (orchestrator context) — workers test
    the env-task case via the existing ``worker_env`` fixture.
    """
    home = tmp_path / ".jacky"
    home.mkdir()
    monkeypatch.setenv("JACKY_HOME", str(home))
    # Make sure neither JACKY_KANBAN_DB nor JACKY_KANBAN_BOARD pin a
    # board — the test is specifically about the per-call override.
    monkeypatch.delenv("JACKY_KANBAN_DB", raising=False)
    monkeypatch.delenv("JACKY_KANBAN_BOARD", raising=False)
    monkeypatch.delenv("JACKY_KANBAN_TASK", raising=False)
    monkeypatch.setenv("JACKY_PROFILE", "test-orchestrator")
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
def test_board_param_routes_heartbeat_to_alt_board(monkeypatch, tmp_path):
    ...
    use the worker-env style fixture inline (pinning JACKY_KANBAN_TASK
    to a task that exists in the alt board)."""
    home = tmp_path / ".jacky"
    home.mkdir()
    monkeypatch.setenv("JACKY_HOME", str(home))
    monkeypatch.setenv("JACKY_PROFILE", "alt-worker")
    monkeypatch.delenv("JACKY_KANBAN_DB", raising=False)
    monkeypatch.delenv("JACKY_KANBAN_BOARD", raising=False)
    from pathlib import Path as _Path
    monkeypatch.setattr(_Path, "home", lambda: tmp_path)
...
# - Gateway sessions: JACKY_SESSION_PLATFORM + JACKY_SESSION_CHAT_ID set.
# - TUI sessions: JACKY_SESSION_KEY (or JACKY_SESSION_ID) set, with
...
def test_create_subscribes_gateway_session(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("JACKY_SESSION_PLATFORM", "telegram")
    monkeypatch.setenv("JACKY_SESSION_CHAT_ID", "chat-42")
    monkeypatch.setenv("JACKY_SESSION_THREAD_ID", "thread-7")
    monkeypatch.setenv("JACKY_SESSION_USER_ID", "user-9")
...
def test_create_subscribes_tui_session_via_session_key(monkeypatch, worker_env):
    ...
    local channel), but the parent process exports JACKY_SESSION_KEY.
    We should still auto-subscribe, with platform='tui' and
    chat_id=<key>."""
    from tools import kanban_tools as kt
    monkeypatch.delenv("JACKY_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("JACKY_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("JACKY_SESSION_THREAD_ID", raising=False)
    monkeypatch.delenv("JACKY_SESSION_USER_ID", raising=False)
    monkeypatch.setenv("JACKY_SESSION_KEY", "tui-session-abc")
    monkeypatch.delenv("JACKY_SESSION_ID", raising=False)
...
def test_create_does_not_subscribe_in_cli_session(monkeypatch, worker_env):
    ...
    monkeypatch.delenv("JACKY_SESSION_PLATFORM", raising=False)
    monkeypatch.delenv("JACKY_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("JACKY_SESSION_KEY", raising=False)
    monkeypatch.delenv("JACKY_SESSION_ID", raising=False)
...
def test_create_respects_auto_subscribe_on_create_false(monkeypatch, worker_env, tmp_path):
    ...
    # worker_env already created <tmp>/.jacky; use a fresh sibling
    # home to avoid mkdir() colliding with the worker's directory.
    home = tmp_path / "gate-home" / ".jacky"
    home.mkdir(parents=True)
    (home / "config.yaml").write_text(
        "kanban:\n  auto_subscribe_on_create: false\n"
    )
    monkeypatch.setenv("JACKY_HOME", str(home))
    monkeypatch.setenv("JACKY_SESSION_PLATFORM", "discord")
    monkeypatch.setenv("JACKY_SESSION_CHAT_ID", "channel-1")
...
def test_create_partial_session_context_no_subscribe(monkeypatch, worker_env):
    ...
    from tools import kanban_tools as kt
    monkeypatch.setenv("JACKY_SESSION_PLATFORM", "slack")
    monkeypatch.delenv("JACKY_SESSION_CHAT_ID", raising=False)
    monkeypatch.delenv("JACKY_SESSION_KEY", raising=False)
    monkeypatch.delenv("JACKY_SESSION_ID", raising=False)

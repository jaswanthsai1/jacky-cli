...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.jacky/skills/."""
...
class TestSecurityScanGate:
    ...
    def test_guard_flag_reads_config_default_false(self):
        ...
        with patch("jacky_cli.config.load_config", return_value={"skills": {}}):
    ...
    def test_guard_flag_reads_config_when_set(self):
        ...
        with patch("jacky_cli.config.load_config",
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.jacky/skills/ as a
    workaround.
    """
...
# come up. The user unpins via `jacky curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "jacky curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
...
def _curator_pass(tmp_path, *, monkeypatch):
    ...
    Points JACKY_HOME at ``tmp_path/.jacky`` so skill_usage's archive path
    (``get_jacky_home()``) resolves into the same tree the skill manager
    searches, and flips ``is_background_review()`` → True so the consolidation
    guard fires.
    """
    jacky_home = tmp_path / ".jacky"
    skills_root = jacky_home / "skills"
...
class TestCuratorConsolidationDeleteGuard:
    ...
    def test_dispatcher_preserves_usage_record_on_curator_archive(self, tmp_path, monkeypatch):
        ...
        # `jacky curator restore` can bring it back.
    ...
    def test_background_review_support_file_overwrite_requires_that_file_read(self, tmp_path, monkeypatch):
        ...
        with _curator_pass(tmp_path, monkeypatch=monkeypatch):
            _create_skill("reviewed", _skill_content("reviewed"))
            ref = tmp_path / ".jacky" / "skills" / "reviewed" / "references"
            ref.mkdir()

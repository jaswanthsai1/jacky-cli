...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._jacky_bin_dir", return_value="/nonexistent"), \
...
class TestInstallArchiveMemberValidation:
    ...
    def test_install_extracts_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert reason == ""
        assert path == str(jacky_home / "bin" / "tirith")
    ...
    def test_install_rejects_non_regular_tirith_member(self, mock_target, mock_which,
        ...
        assert not os.path.lexists(jacky_home / "bin" / "tirith")
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_jacky_bin(self):
        """After _INSTALL_FAILED, manual install in JACKY_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        jacky_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(jacky_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(jacky_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._jacky_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == jacky_bin
            assert _tirith_mod._resolved_path == jacky_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# JACKY_HOME isolation
# ---------------------------------------------------------------------------

class TestJackyHomeIsolation:
    def test_jacky_bin_dir_respects_jacky_home(self):
        """_jacky_bin_dir must use JACKY_HOME, not hardcoded ~/.jacky."""
        from tools.tirith_security import _jacky_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"JACKY_HOME": tmpdir}):
            result = _jacky_bin_dir()
        assert result == os.path.join(tmpdir, "bin")
        assert os.path.isdir(result)

    def test_failure_marker_respects_jacky_home(self):
        """_failure_marker_path must use JACKY_HOME, not hardcoded ~/.jacky."""
        from tools.tirith_security import _failure_marker_path
        with patch.dict(os.environ, {"JACKY_HOME": "/custom/jacky"}):
            result = _failure_marker_path()
        assert result == "/custom/jacky/.tirith-install-failed"

    def test_conftest_isolation_prevents_real_home_writes(self):
        """The conftest autouse fixture sets JACKY_HOME; verify it's active."""
        jacky_home = os.getenv("JACKY_HOME")
        assert jacky_home is not None, "JACKY_HOME should be set by conftest"
        assert "jacky_test" in jacky_home, "Should point to test temp dir"

    def test_get_jacky_home_fallback(self):
        """Without JACKY_HOME set, falls back to the active OS home."""
        from tools.tirith_security import _get_jacky_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove JACKY_HOME entirely. With HOME also absent, expanduser
            ...
            expected = os.path.join(os.path.expanduser("~"), ".jacky")
            result = _get_jacky_home()
    ...
    def test_mkdtemp_oserror_propagates_to_ensure_installed(self):
        ...
        with patch("tools.tirith_security.tempfile.mkdtemp",
             ...
             patch("tools.tirith_security.shutil.which",
                   return_value=None), \
             patch("tools.tirith_security._jacky_bin_dir",

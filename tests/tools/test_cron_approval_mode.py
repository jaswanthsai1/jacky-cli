...
class TestCronApprovalModeParsing:
    def test_default_is_deny(self):
        ...
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"cron_mode": "deny"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_explicit_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"cron_mode": "approve"}}):
    ...
    def test_off_maps_to_approve(self):
        ...
        from unittest.mock import patch as mock_patch
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"cron_mode": "off"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_allow_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"cron_mode": "allow"}}):
            assert _get_cron_approval_mode() == "approve"

    def test_yes_maps_to_approve(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"cron_mode": "yes"}}):
            assert _get_cron_approval_mode() == "approve"
    ...
    def test_unknown_value_defaults_to_deny(self):
        from unittest.mock import patch as mock_patch
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"cron_mode": "maybe"}}):
            assert _get_cron_approval_mode() == "deny"

    def test_config_load_failure_defaults_to_deny(self):
        """If config loading fails entirely, default to deny (safe)."""
        from unittest.mock import patch as mock_patch
        with mock_patch("jacky_cli.config.load_config", side_effect=RuntimeError("config broken")):
            assert _get_cron_approval_mode() == "deny"

    def test_yaml_boolean_false_maps_to_deny(self):
        """YAML 1.1 parses bare 'off' as False. Ensure it maps to deny."""
        from unittest.mock import patch as mock_patch
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"cron_mode": False}}):
...
class TestCronApproveMode:
    """When JACKY_CRON_SESSION is set and cron_mode=approve, dangerous commands pass through."""
...
class TestCronDenyModeAllGuards:
    ...
    def test_tirith_import_error_fail_closed_blocks_in_cron_deny(self, monkeypatch):
        ...
        with (
            ...
            mock_patch("tools.approval.detect_dangerous_command",
                       return_value=(False, None, None)),
            mock_patch("jacky_cli.config.load_config",
...
class TestCronModeInteractions:
    ...
    def test_yolo_overrides_cron_deny(self, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_CRON_SESSION", "1")
        monkeypatch.setenv("JACKY_YOLO_MODE", "1")
        monkeypatch.delenv("JACKY_INTERACTIVE", raising=False)
        monkeypatch.delenv("JACKY_GATEWAY_SESSION", raising=False)

        # _YOLO_MODE_FROZEN is frozen at module import time (security: prevents
        # prompt injection from runtime-setting JACKY_YOLO_MODE). When the
    ...
    def test_non_cron_non_interactive_still_auto_approves(self, monkeypatch):
        ...
        monkeypatch.delenv("JACKY_CRON_SESSION", raising=False)
        monkeypatch.delenv("JACKY_INTERACTIVE", raising=False)
        monkeypatch.delenv("JACKY_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("JACKY_YOLO_MODE", raising=False)
...
class TestCronWithGatewayOrigin:
    ...
    cron/scheduler.py binds JACKY_SESSION_PLATFORM via contextvars for

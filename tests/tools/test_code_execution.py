...
UDS socket lifecycle, jacky_tools generation, timeout enforcement,
...
class TestJackyToolsGeneration(unittest.TestCase):
    def test_generates_all_allowed_tools(self):
        src = generate_jacky_tools_module(list(SANDBOX_ALLOWED_TOOLS))
        for tool in SANDBOX_ALLOWED_TOOLS:
            self.assertIn(f"def {tool}(", src)

    def test_generates_subset(self):
        src = generate_jacky_tools_module(["terminal", "web_search"])
    ...
    def test_empty_list_generates_nothing(self):
        src = generate_jacky_tools_module([])
    ...
    def test_non_allowed_tools_ignored(self):
        src = generate_jacky_tools_module(["vision_analyze", "terminal"])
        self.assertIn("def terminal(", src)
        self.assertNotIn("def vision_analyze(", src)

    def test_rpc_infrastructure_present(self):
        src = generate_jacky_tools_module(["terminal"])
        self.assertIn("JACKY_RPC_SOCKET", src)
    ...
    def test_file_transport_uses_tempfile_fallback_for_rpc_dir(self):
        src = generate_jacky_tools_module(["terminal"], transport="file")
        self.assertIn("import json, os, shlex, tempfile, threading, time", src)
        self.assertIn("os.path.join(tempfile.gettempdir(), \"jacky_rpc\")", src)
        self.assertNotIn('os.environ.get("JACKY_RPC_DIR", "/tmp/jacky_rpc")', src)
    ...
    def test_uds_transport_serializes_concurrent_calls(self):
        ...
        src = generate_jacky_tools_module(["terminal"], transport="uds")
...
class TestExecuteCodeRemoteTempDir(unittest.TestCase):
    def test_execute_remote_uses_backend_temp_dir_for_sandbox(self):
        ...
        self.assertIn("mkdir -p /data/data/com.termux/files/usr/tmp/jacky_exec_", mkdir_cmd)
        self.assertIn("JACKY_RPC_DIR=/data/data/com.termux/files/usr/tmp/jacky_exec_", run_cmd)
        self.assertIn("rm -rf /data/data/com.termux/files/usr/tmp/jacky_exec_", cleanup_cmd)
        self.assertNotIn("mkdir -p /tmp/jacky_exec_", mkdir_cmd)

    def test_timezone_shell_quoted_in_remote_execution(self):
        """JACKY_TIMEZONE must be shell-quoted in remote env_prefix to prevent injection."""
        class FakeEnv:
            def __init__(self):
                self.commands = []
        ...
        with patch("tools.code_execution_tool._load_config",
             ...
             patch.dict(os.environ, {"JACKY_TIMEZONE": malicious_tz}):
            result = json.loads(_execute_remote("print('hello')", "task-1", ["terminal"]))
...
"""
        ...
        # terminal won't be in jacky_tools.py, so import fails
        self.assertEqual(result["status"], "error")
...
"""
    ...
    def test_web_search_tool(self):
        ...
        code = """
from jacky_tools import web_search
...
"""
    ...
    def test_json_parse_helper(self):
        ...
        code = r"""
from jacky_tools import json_parse
...
"""
    ...
    def test_shell_quote_helper(self):
        ...
        code = """
from jacky_tools import shell_quote
...
class TestBuildExecuteCodeSchema(unittest.TestCase):
    ...
    def test_real_scenario_all_sandbox_tools_disabled(self):
        ...
        Scenario: user runs `jacky tools code_execution` (only code_execution
    ...
    def test_real_scenario_only_vision_enabled(self):
        """Another real path: user runs `jacky tools code_execution,vision`.
...
class TestEnvVarFiltering(unittest.TestCase):
    ...
    def test_jacky_rpc_socket_injected(self):
        child_env = self._get_child_env()
        self.assertIn("JACKY_RPC_SOCKET", child_env)
    ...
    def test_timezone_injected_when_set(self):
        env_backup = os.environ.copy()
        try:
            os.environ["JACKY_TIMEZONE"] = "America/New_York"
            child_env = self._get_child_env()
    ...
    def test_timezone_not_set_when_empty(self):
        env_backup = os.environ.copy()
        try:
            os.environ.pop("JACKY_TIMEZONE", None)
            child_env = self._get_child_env()
            if "TZ" in child_env:
...
class TestLoadConfig(unittest.TestCase):
    ...
    def test_returns_code_execution_section(self):
        from tools.code_execution_tool import _load_config
        with patch("jacky_cli.config.read_raw_config",
...
class TestRpcTokenAuthorization(unittest.TestCase):
    ...
    request without the matching JACKY_RPC_TOKEN must be rejected before
    ...
    def test_generated_module_sends_token(self):
        """The generated jacky_tools module reads JACKY_RPC_TOKEN and sends it."""
        src = generate_jacky_tools_module(["terminal"], transport="uds")
        self.assertIn("JACKY_RPC_TOKEN", src)

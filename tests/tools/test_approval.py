...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("jacky_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestWindowsShellDestructiveCommands:
    def test_cmd_del_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd /c del /f /q C:\tmp\jacky-victim\file.txt"
        )
    ...
    def test_cmd_rmdir_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"cmd.exe /k rmdir /s /q C:\tmp\jacky-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows cmd destructive delete"
    ...
    def test_powershell_remove_item_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"powershell -NoProfile -Command Remove-Item -Recurse -Force C:\tmp\jacky-victim"
        )
        assert dangerous is True
        assert key is not None
    ...
    def test_pwsh_rm_alias_requires_approval(self):
        dangerous, key, desc = detect_dangerous_command(
            r"pwsh -c rm -Recurse -Force C:\tmp\jacky-victim"
        )
        assert dangerous is True
        assert key is not None
        assert "delete" in desc.lower()
    ...
    def test_powershell_bare_remove_item_requires_approval(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            r"powershell Remove-Item -Recurse -Force C:\tmp\jacky-victim"
        )
        assert dangerous is True
        assert key is not None
        assert desc == "Windows PowerShell destructive delete"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"JACKY_SESSION_KEY": "bob"}, clear=False):
...
class TestTeePattern:
    ...
    def test_tee_jacky_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.jacky/.env")
        assert dangerous is True
        assert key is not None
    ...
    def test_tee_custom_jacky_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $JACKY_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_jacky_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$JACKY_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestJackyConfigWriteProtection:
    """Terminal-side pairing for the file_tools write_file/patch deny on
    ~/.jacky/config.yaml (#14639). config.yaml IS the security policy
    ...
    def test_redirect_overwrite(self):
        dangerous, key, desc = detect_dangerous_command("echo 'approvals:' > ~/.jacky/config.yaml")
        assert dangerous is True
        assert key is not None

    def test_append(self):
        dangerous, key, desc = detect_dangerous_command("echo '  mode: off' >> ~/.jacky/config.yaml")
        assert dangerous is True

    def test_tee(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.jacky/config.yaml")
        assert dangerous is True

    def test_cp_over_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.jacky/config.yaml")
        assert dangerous is True
    ...
    def test_sed_in_place(self):
        ...
        dangerous, key, desc = detect_dangerous_command("sed -i 's/manual/off/' ~/.jacky/config.yaml")
        assert dangerous is True
        assert "jacky config" in desc.lower() or "in-place" in desc.lower()

    def test_sed_in_place_long_flag(self):
        dangerous, key, desc = detect_dangerous_command("sed --in-place 's/manual/off/' ~/.jacky/config.yaml")
        assert dangerous is True

    def test_sed_in_place_absolute_jacky_home_config(self):
    ...
    def test_sed_in_place_absolute_jacky_home_env(self):
        env_path = get_jacky_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_custom_jacky_home(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $JACKY_HOME/config.yaml")
        assert dangerous is True
    ...
    def test_perl_in_place_config(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/approvals.mode: on/approvals.mode: off/' ~/.jacky/config.yaml"
        )
        assert dangerous is True
        assert "in-place" in desc.lower() or "perl" in desc.lower()

    def test_perl_in_place_absolute_jacky_home_config(self):
        config_path = get_jacky_home() / "config.yaml"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_ruby_in_place_config(self):
        dangerous, key, desc = detect_dangerous_command(
            "ruby -i -pe 'gsub(/manual/, \"off\")' ~/.jacky/config.yaml"
        )
        assert dangerous is True

    def test_ruby_in_place_absolute_jacky_home_env(self):
        env_path = get_jacky_home() / ".env"
        dangerous, key, desc = detect_dangerous_command(
    ...
    def test_perl_in_place_env(self):
        dangerous, key, desc = detect_dangerous_command(
            "perl -i -pe 's/SECRET=old/SECRET=new/' ~/.jacky/.env"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_separate_flag_token(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -p -i -e 's/approvals.mode: on/approvals.mode: off/' ~/.jacky/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_in_place_backup_suffix(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -i.bak -pe 's/x/y/' ~/.jacky/config.yaml"
        )
        assert dangerous is True
    ...
    def test_perl_eval_no_inplace_safe(self):
        ...
        dangerous, key, desc = detect_dangerous_command(
            "perl -wne 'print' ~/.jacky/config.yaml"
        )
        assert dangerous is False

    def test_read_is_safe(self):
        # Reading config is not a write — must not trip.
        dangerous, key, desc = detect_dangerous_command("cat ~/.jacky/config.yaml")
        assert dangerous is False

    def test_normal_yaml_write_safe(self):
        # A non-Jacky config.yaml in a project dir is handled by the project
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_jacky_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $JACKY_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveCopyMovePattern:
    ...
    shell rc files, or ~/.jacky/config.yaml/.env must require approval — the
    ...
    def test_cp_to_jacky_config(self):
        dangerous, key, desc = detect_dangerous_command("cp /tmp/evil.yaml ~/.jacky/config.yaml")
        assert dangerous is True
...
class TestWindowsAbsolutePathFolding:
    """Windows absolute home / Jacky-home prefixes must fold to ~/ and
    ~/.jacky/ in dangerous-command detection.
    ...
    SSH, and Jacky config/env files returned "safe" without an approval prompt.
    The OS-specific ``Path.home()`` / ``get_jacky_home()`` tests above only
    exercise this branch on a Windows host; these monkeypatch a Windows-style
    HOME/JACKY_HOME so the fold is verified on the POSIX CI runner too."""
    ...
    def test_windows_jacky_home_config_folds(self, monkeypatch):
        # Jacky home nests under the user home on Windows; it must fold before
        # the user-home rewrite eats its prefix.
        monkeypatch.setenv("HOME", r"C:\Users\tester")
        monkeypatch.setenv("JACKY_HOME", r"C:\Users\tester\.jacky")
        dangerous, key, _ = detect_dangerous_command(
            r"sed -i 's/manual/off/' C:\Users\tester\.jacky\config.yaml"
        )
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.jacky/jacky-agent && source venv/bin/activate && python -m jacky_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m jacky_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m jacky_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "jacky_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_flagged(self):
        ...
        cmd = "systemctl --user restart jacky-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "stop/restart" in desc

    def test_jacky_gateway_stop_detected(self):
        cmd = "jacky gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_jacky_gateway_restart_with_profile_flag_detected(self):
        """A profile flag between `jacky` and `gateway` must not slip past
        the guard. See the 2026-04-11 ade-profile self-kill incident."""
        cmd = "jacky -p ade gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "gateway" in desc.lower()

    def test_jacky_gateway_stop_with_long_profile_flag_detected(self):
        cmd = "jacky --profile ade gateway stop"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_jacky_gateway_multiple_flags_detected(self):
        cmd = "jacky -p cocoa --verbose gateway restart"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_jacky_gateway_status_with_profile_flag_not_flagged(self):
        """Read-only subcommands stay allowed even with a profile flag."""
        cmd = "jacky -p ade gateway status"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_jacky_gateway_start_not_flagged(self):
        cmd = "jacky gateway start"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_jacky_detected(self):
        """pkill targeting jacky/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_jacky_detected(self):
        cmd = "killall jacky"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestIFSWhitespaceBypass:
    ...
    def test_ifs_sed_config_dangerous(self):
        """In-place edit of the Jacky security config via IFS must be caught."""
        cmd = "sed${IFS}-i ~/.jacky/config.yaml"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
...
class TestPgrepKillExpansion:
    """kill -9 $(pgrep jacky) bypasses the pkill/killall name-matching
    ...
    def test_pkill_jacky_still_detected(self):
    ...
    def test_kill_dollar_pidof_detected(self):
        """`kill $(pidof jacky)` is the BSD/Linux equivalent of the
        pgrep expansion and bypasses the pkill/killall name pattern
        in the same way. See issue #33071."""
        cmd = "kill -TERM $(pidof jacky_cli.main)"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "pidof" in desc.lower() or "pgrep" in desc.lower()

    def test_kill_backtick_pidof_detected(self):
        cmd = "kill -9 `pidof jacky`"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True


class TestLaunchctlGatewayLifecycle:
    """launchctl stop/kickstart/bootout/unload against the Jacky service
    label achieves the same effect as `jacky gateway stop|restart` and
    must require the same approval. See issue #33071.
    """

    def test_launchctl_stop_jacky_detected(self):
        cmd = "launchctl stop ai.jacky.gateway"
        dangerous, _, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "launchd" in desc.lower() or "jacky" in desc.lower()

    def test_launchctl_kickstart_jacky_detected(self):
        cmd = "launchctl kickstart -k system/ai.jacky.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_bootout_jacky_detected(self):
        cmd = "launchctl bootout system/ai.jacky.gateway"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_launchctl_unload_jacky_detected(self):
        cmd = "launchctl unload ~/Library/LaunchAgents/ai.jacky.gateway.plist"
        dangerous, _, _ = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_launchctl_stop_unrelated_not_flagged(self):
        """`launchctl stop` on a non-Jacky label is out of scope for the
...
class TestApprovalTimeoutIsNotConsent:
    ...
    def setup_method(self):
        ...
        self._saved_env = {
            k: os.environ.get(k)
            for k in ("JACKY_GATEWAY_SESSION", "JACKY_CRON_SESSION",
                      "JACKY_YOLO_MODE",
                      "JACKY_SESSION_KEY", "JACKY_INTERACTIVE")
        }
        os.environ.pop("JACKY_YOLO_MODE", None)
        os.environ.pop("JACKY_INTERACTIVE", None)
        # JACKY_CRON_SESSION takes priority over JACKY_GATEWAY_SESSION in
        ...
        os.environ.pop("JACKY_CRON_SESSION", None)
        os.environ["JACKY_GATEWAY_SESSION"] = "1"
        os.environ["JACKY_SESSION_KEY"] = self.SESSION_KEY
    ...
    * ``tirith_fail_open: false`` → surface a Tirith-style warning through
    ...
    def test_fail_open_true_allows_silently_on_import_error(self):
        ...
        with _patch("builtins.__import__", side_effect=self._make_failing_import(real_import)):
            with _patch("jacky_cli.config.load_config", return_value=cfg):
                with _patch("tools.approval.detect_dangerous_command", return_value=(False, None, None)):
                    with mock_patch.dict("os.environ", {"JACKY_INTERACTIVE": "1"}, clear=False):

...
Verifies that Jacky-managed provider, tool, and gateway env vars are
stripped from subprocess environments so external CLIs are not silently
misrouted or handed Jacky secrets.

See: https://github.com/jaswanthsai1/jacky-cli/issues/1002
See: https://github.com/jaswanthsai1/jacky-cli/issues/1264
"""
...
from tools.environments.local import (
    LocalEnvironment,
    _JACKY_PROVIDER_ENV_BLOCKLIST,
    _JACKY_PROVIDER_ENV_FORCE_PREFIX,
)
...
class TestProviderEnvBlocklist:
    """Provider env vars loaded from ~/.jacky/.env must not leak."""
    ...
    def test_bedrock_bearer_token_is_stripped(self):
        """The Bedrock-specific bearer token is a Jacky inference secret
        ...
        models`` run inside a Jacky terminal enumerated the entire Bedrock
    ...
    def test_general_aws_credential_chain_is_preserved(self):
        ...
        refuses to re-allow anything in _JACKY_PROVIDER_ENV_BLOCKLIST
        (GHSA-rhgp-j443-p4rf). Only the Bedrock inference bearer token is
        Jacky-managed; the rest belongs to the user.
        """
        general_chain = {
    ...
    def test_tool_and_gateway_vars_are_stripped(self):
        ...
        leaked_vars = {
            ...
            "JACKY_DASHBOARD_SESSION_TOKEN": "dashboard-session-secret",
...
class TestForceEnvOptIn:
    """Callers can opt in to passing a blocked var via _JACKY_FORCE_ prefix."""

    def test_force_prefix_passes_blocked_var(self):
        """_JACKY_FORCE_OPENAI_API_KEY in self.env should inject OPENAI_API_KEY."""
        result_env = _run_with_env(self_env={
            f"{_JACKY_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-explicit",
        })
        ...
        assert f"{_JACKY_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result_env
    ...
    def test_force_prefix_overrides_os_environ_block(self):
        ...
        result_env = _run_with_env(
            ...
            self_env={f"{_JACKY_PROVIDER_ENV_FORCE_PREFIX}OPENAI_BASE_URL": "http://intended/v1"},
        )
...
class TestActiveVenvMarkerStripping:
    ...
    into the Jacky venv path instead of the project's own ``.venv`` —
    silently clobbering the Jacky environment (and, when the other project
    pins a different Python, breaking the gateway outright). The Jacky venv
    stays reachable via PATH, so stripping the markers is safe.
    """

    def test_virtualenv_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "VIRTUAL_ENV": "/home/user/.jacky/jacky-agent/venv",
        })
        assert "VIRTUAL_ENV" not in result_env

    def test_conda_prefix_marker_stripped_end_to_end(self):
        result_env = _run_with_env(extra_os_env={
            "CONDA_PREFIX": "/opt/conda/envs/jacky",
        })
...
class TestBlocklistCoverage:
    ...
    def test_issue_1002_offenders(self):
        ...
        assert must_block.issubset(_JACKY_PROVIDER_ENV_BLOCKLIST)
    ...
    def test_registry_vars_are_in_blocklist(self):
        ...
        by the user's Claude Code install, not Jacky (#55878).
        """
        ...
        for pconfig in PROVIDER_REGISTRY.values():
            for var in pconfig.api_key_env_vars:
                if var in exempt:
                    continue
                assert var in _JACKY_PROVIDER_ENV_BLOCKLIST, (
                    f"Registry var {var} (provider={pconfig.id}) missing from blocklist"
                )
            if pconfig.base_url_env_var:
                assert pconfig.base_url_env_var in _JACKY_PROVIDER_ENV_BLOCKLIST, (
    ...
    def test_bedrock_bearer_token_is_in_blocklist(self):
        """auth_type='aws_sdk' providers contribute their Jacky-managed
        ...
        assert "AWS_BEARER_TOKEN_BEDROCK" in _JACKY_PROVIDER_ENV_BLOCKLIST
    ...
    def test_general_aws_chain_not_in_blocklist(self):
        ...
        operator shell (SECURITY.md §3.2), not to Jacky, and blocklisting
        ...
        leaked_block = general_chain & _JACKY_PROVIDER_ENV_BLOCKLIST
    ...
    def test_claude_code_oauth_token_is_inheritable(self):
        ...
        assert "CLAUDE_CODE_OAUTH_TOKEN" not in _JACKY_PROVIDER_ENV_BLOCKLIST
    ...
    def test_optional_tool_and_messaging_vars_are_in_blocklist(self):
        ...
        from jacky_cli.config import OPTIONAL_ENV_VARS

        for name, metadata in OPTIONAL_ENV_VARS.items():
            category = metadata.get("category")
            if category in {"tool", "messaging"}:
                assert name in _JACKY_PROVIDER_ENV_BLOCKLIST, (
...
class TestSanePathIncludesHomebrew:
    ...
    def _disable_jacky_bin_injection(self):
        """These tests assert the sane-path merge in isolation. Disable the
        jacky-install-dir prepend (a separate concern, covered by
        TestJackyBinDirOnPath) so a real ``jacky`` on the test runner's PATH
        doesn't shift the asserted PATH layout."""
        from tools.environments import local as local_mod
        saved = local_mod._JACKY_BIN_DIR
        local_mod._JACKY_BIN_DIR = None  # resolved -> no dir to inject
        yield
        local_mod._JACKY_BIN_DIR = saved
...
class TestJackyBinDirOnPath:
    """The jacky install dir is reachable in the terminal subshell PATH.

    Plugins shelling out to bare ``jacky`` via the terminal tool must work
    even when the gateway was launched without the jacky install dir on
    PATH (systemd, service managers, cron). See the discussion that motivated
    _resolve_jacky_bin_dir / _prepend_jacky_bin_dir.
    """

    def _reset_cache(self):
        from tools.environments import local as local_mod
        local_mod._JACKY_BIN_DIR = local_mod._SENTINEL

    def test_resolves_via_which(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        monkeypatch.setattr(local_mod.shutil, "which",
                            lambda name: "/opt/jacky/bin/jacky" if name == "jacky" else None)
        monkeypatch.setattr(local_mod.os.path, "isdir", lambda p: p == "/opt/jacky/bin")
        assert local_mod._resolve_jacky_bin_dir() == "/opt/jacky/bin"
    ...
    def test_resolves_via_sys_executable_dir(self, monkeypatch, tmp_path):
        ...
        (venv_bin / "jacky").write_text("#!/bin/sh\n")
        ...
        assert local_mod._resolve_jacky_bin_dir() == str(venv_bin)
    ...
    def test_returns_none_when_unresolvable(self, monkeypatch):
        ...
        assert local_mod._resolve_jacky_bin_dir() is None

    def test_prepend_adds_missing_dir_at_front(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._JACKY_BIN_DIR = "/opt/jacky/bin"
        out = local_mod._prepend_jacky_bin_dir("/usr/bin:/bin")
        assert out.split(os.pathsep)[0] == "/opt/jacky/bin"
    ...
    def test_prepend_is_idempotent(self, monkeypatch):
        ...
        self._reset_cache()
        local_mod._JACKY_BIN_DIR = "/opt/jacky/bin"
        once = local_mod._prepend_jacky_bin_dir("/usr/bin:/bin")
        twice = local_mod._prepend_jacky_bin_dir(once)
        assert twice == once
        assert once.split(os.pathsep).count("/opt/jacky/bin") == 1

    def test_prepend_noop_when_unresolved(self, monkeypatch):
        from tools.environments import local as local_mod
        self._reset_cache()
        local_mod._JACKY_BIN_DIR = None
        assert local_mod._prepend_jacky_bin_dir("/usr/bin:/bin") == "/usr/bin:/bin"

    def test_make_run_env_injects_jacky_bin_dir(self, monkeypatch):
        """A gateway env missing the jacky dir gets it back in the subshell PATH."""
        from tools.environments import local as local_mod
        from tools.environments.local import _make_run_env
        self._reset_cache()
        local_mod._JACKY_BIN_DIR = "/opt/jacky/bin"
        ...
        assert entries[0] == "/opt/jacky/bin"
        assert "/usr/bin" in entries


class TestJackyInternalDynamicSecrets:
    """Dynamically-named Jacky secrets injected at gateway/CLI startup must
    not leak into terminal subprocesses.

    The static ``_JACKY_PROVIDER_ENV_BLOCKLIST`` is name-based and derived
    ...
    ``_is_jacky_internal_secret`` is the single source of truth; every spawn
    path (``_sanitize_subprocess_env``, ``_make_run_env``,
    ``jacky_subprocess_env``, Docker forward filter, ``env_passthrough``)
    consults it. These tests exercise the terminal execute path + predicate.
    """

    def test_predicate_matches_auxiliary_api_key(self):
        from tools.environments.local import _is_jacky_internal_secret
        assert _is_jacky_internal_secret("AUXILIARY_VISION_API_KEY")
        assert _is_jacky_internal_secret("AUXILIARY_WEB_EXTRACT_API_KEY")
        assert _is_jacky_internal_secret("AUXILIARY_APPROVAL_API_KEY")
        # plugin-registered task names are covered by the pattern
        assert _is_jacky_internal_secret("AUXILIARY_MY_PLUGIN_TASK_API_KEY")

    def test_predicate_matches_auxiliary_base_url(self):
        from tools.environments.local import _is_jacky_internal_secret
        assert _is_jacky_internal_secret("AUXILIARY_VISION_BASE_URL")
        assert _is_jacky_internal_secret("AUXILIARY_COMPRESSION_BASE_URL")

    def test_predicate_matches_gateway_relay_auth(self):
        from tools.environments.local import _is_jacky_internal_secret
        assert _is_jacky_internal_secret("GATEWAY_RELAY_SECRET")
        assert _is_jacky_internal_secret("GATEWAY_RELAY_DELIVERY_KEY")
        assert _is_jacky_internal_secret("GATEWAY_RELAY_SESSION_TOKEN")
    ...
    def test_predicate_allows_auxiliary_non_secrets(self):
        ...
        from tools.environments.local import _is_jacky_internal_secret
        assert not _is_jacky_internal_secret("AUXILIARY_VISION_PROVIDER")
        assert not _is_jacky_internal_secret("AUXILIARY_VISION_MODEL")
        assert not _is_jacky_internal_secret("GATEWAY_RELAY_URL")
        assert not _is_jacky_internal_secret("GATEWAY_RELAY_PLATFORMS")
        assert not _is_jacky_internal_secret("GATEWAY_RELAY_ID")  # not a secret suffix
        # unrelated vars pass through
        assert not _is_jacky_internal_secret("PATH")
        assert not _is_jacky_internal_secret("MY_APP_KEY")
    ...
    def test_gateway_relay_static_names_in_blocklist(self):
        ...
        assert "GATEWAY_RELAY_SECRET" in _JACKY_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_DELIVERY_KEY" in _JACKY_PROVIDER_ENV_BLOCKLIST
        assert "GATEWAY_RELAY_ID" in _JACKY_PROVIDER_ENV_BLOCKLIST

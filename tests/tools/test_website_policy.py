...
def test_check_website_access_uses_dynamic_jacky_home(monkeypatch, tmp_path):
    ...
    # Invalidate the module-level cache so the new JACKY_HOME is picked up.
    # A prior test may have cached a default policy (enabled=False) under the
    # old JACKY_HOME set by the autouse _isolate_jacky_home fixture.
...
def test_check_website_access_fails_open_on_malformed_config(tmp_path, monkeypatch):
    ...
    # Simulate default path by pointing JACKY_HOME to tmp_path

...
def test_jacky_provider_subclass_exists():
    """JackyMCPOAuthProvider is defined and subclasses OAuthClientProvider."""
    from tools.mcp_oauth_manager import _JACKY_PROVIDER_CLS
    from mcp.client.auth.oauth2 import OAuthClientProvider

    assert _JACKY_PROVIDER_CLS is not None
    assert issubclass(_JACKY_PROVIDER_CLS, OAuthClientProvider)
...
def test_manager_builds_jacky_provider_subclass(tmp_path, monkeypatch):
    """get_or_build_provider returns JackyMCPOAuthProvider, not plain OAuthClientProvider."""
    from tools.mcp_oauth_manager import (
        MCPOAuthManager, _JACKY_PROVIDER_CLS, reset_manager_for_tests,
    )
    reset_manager_for_tests()
    monkeypatch.setenv("JACKY_HOME", str(tmp_path))
    _set_interactive_stdin(monkeypatch)
    ...
    assert _JACKY_PROVIDER_CLS is not None
    assert isinstance(provider, _JACKY_PROVIDER_CLS)
    assert provider._jacky_server_name == "srv"

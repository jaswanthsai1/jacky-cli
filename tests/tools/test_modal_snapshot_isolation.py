...
def _restore_tool_modules():
    original_jacky_home = os.environ.get("JACKY_HOME")
    original_modules = {
        name: module
    ...
    try:
        ...
        if original_jacky_home is None:
            os.environ.pop("JACKY_HOME", None)
        else:
            os.environ["JACKY_HOME"] = original_jacky_home
        _reset_modules(("tools", "jacky_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    jacky_cli = types.ModuleType("jacky_cli")
    jacky_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["jacky_cli"] = jacky_cli
    jacky_home = tmp_path / "jacky-home"
    os.environ["JACKY_HOME"] = str(jacky_home)
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="jacky-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": jacky_home / "modal_snapshots.json",

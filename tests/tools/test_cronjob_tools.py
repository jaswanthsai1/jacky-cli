...
class TestScanCronSkillAssembled:
    ...
    def test_descriptive_attack_command_prose_allowed(self):
        ...
        Real example: the `jacky-agent-dev` skill contains a postmortem
        section saying 'the attacker could just cat ~/.jacky/.env'.
        """
        assert _scan_cron_skill_assembled(
            "the attacker could just cat ~/.jacky/.env to steal credentials"
        )[1] == ""
        assert _scan_cron_skill_assembled(
...
class TestCronjobRequirements:
    def test_requires_no_crontab_binary(self, monkeypatch):
        ...
        # because jacky uses an internal scheduler, not system crontab.
    ...
    def test_accepts_exec_ask(self, monkeypatch):
        monkeypatch.delenv("JACKY_INTERACTIVE", raising=False)
        monkeypatch.delenv("JACKY_GATEWAY_SESSION", raising=False)
        monkeypatch.setenv("JACKY_EXEC_ASK", "1")
    ...
    def test_rejects_false_like_interactive_env(self, monkeypatch, false_like_value):
        monkeypatch.setenv("JACKY_INTERACTIVE", false_like_value)
        monkeypatch.delenv("JACKY_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("JACKY_EXEC_ASK", raising=False)
        assert check_cronjob_requirements() is False
    ...
    @pytest.mark.parametrize(
        "var_name",
        ["JACKY_INTERACTIVE", "JACKY_GATEWAY_SESSION", "JACKY_EXEC_ASK"],
    )
    @pytest.mark.parametrize("false_like_value", ["0", "false", "no", "off"])
    def test_rejects_false_like_any_session_env(
        ...
        for v in ("JACKY_INTERACTIVE", "JACKY_GATEWAY_SESSION", "JACKY_EXEC_ASK"):
...
class TestUnifiedCronjobTool:
    ...
    def _patch_named_legit(monkeypatch):
        import jacky_cli.runtime_provider as rp
...
class TestResolveModelOverride:
    ...
    def test_keeps_bare_custom_when_a_named_entry_exists(self, monkeypatch):
        import jacky_cli.runtime_provider as rp_mod

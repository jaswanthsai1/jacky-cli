...
class TestSendMessageTool:
    ...
    def test_ntfy_topic_target_bypasses_channel_directory(self):
        ...
        ntfy_cfg = SimpleNamespace(enabled=True, token=None, extra={"topic": "jacky-in"})
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
             ...
             patch("jacky_cli.model_tools._run_async", side_effect=_run_async_immediately), \
    ...
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "JACKY_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "JACKY_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
    ...
    def test_mirror_receives_current_session_user_id(self):
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
            ...
            get_session_env_mock.side_effect = lambda name, default="": {
                ...
                "JACKY_SESSION_USER_ID": "user-123",
            }.get(name, default)
            result = json.loads(
                send_message_tool(
                    {
    ...
    def test_top_level_send_failure_redacts_query_token(self):
        ...
        with patch("gateway.config.load_gateway_config", return_value=config), \
             patch("tools.interrupt.is_interrupted", return_value=False), \
             patch("jacky_cli.model_tools._run_async", side_effect=_raise_and_close):
            result = json.loads(
                send_message_tool(
                    {
                        "action": "send",
...
class TestSendToPlatformChunking:
    ...
    def test_slack_messages_are_formatted_before_send(self, monkeypatch):
        ...
        with _patch_slack_standalone_sender(send):
            result = asyncio.run(
                _send_to_platform(
                    ...
                    SimpleNamespace(enabled=True, token="***", extra={}),
                    "C123",
                    "**hello** from [Jacky](<https://example.com>)",
                )
            )
        ...
        send.assert_awaited_once_with(
            "***",
            "C123",
            "*hello* from <https://example.com|Jacky>",
            thread_ts=None,
        )
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert _call.args[2] == "hello from jacky"
...
class TestParseTargetRefMatrix:
    ...
    def test_matrix_user_mxid_is_explicit(self):
        ...
        chat_id, thread_id, is_explicit = _parse_target_ref("matrix", "@jacky:matrix.org")
        assert chat_id == "@jacky:matrix.org"
        assert thread_id is None
        assert is_explicit is True
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``jacky cron``
    runs separately from ``jacky gateway``), ``_send_via_adapter`` should
...
class TestCheckSendMessage:
    ...
    1. ``JACKY_KANBAN_TASK`` is set (worker spawned by the kanban dispatcher
       — parent gateway is by definition running, but the worker's
       ``JACKY_HOME`` may be a profile dir without a ``gateway.pid``).
    2. ``JACKY_SESSION_PLATFORM`` resolves to a non-empty, non-``local`` value
    ...
    def test_kanban_task_env_short_circuits_before_gateway_check(self, monkeypatch):
        """Honoring JACKY_KANBAN_TASK must not depend on importing or calling
        gateway.status — the worker may run with a JACKY_HOME that has no
    ...
    def test_local_platform_falls_through_to_gateway_check(self, monkeypatch):
        """``JACKY_SESSION_PLATFORM=local`` means CLI-style — must defer to

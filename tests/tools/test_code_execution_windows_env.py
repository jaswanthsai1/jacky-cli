...
``jacky_tools.py`` and ``script.py`` into a temp dir, and those files
...
def _legacy_posix_scrubber(source_env, is_passthrough):
    ...
    Deliberately updated for #27303 (the broad ``JACKY_`` prefix was dropped
...
class TestPosixEquivalence:
    ...
    _POSIX_SYNTHETIC_ENV = {
        ...
        # JACKY_* handling (#27303): only the operational allowlist passes;
        # every other JACKY_* is dropped (the broad prefix was removed).
        "JACKY_HOME": "/home/alice/.jacky",        # allowlisted → kept
        "JACKY_PROFILE": "default",                 # allowlisted → kept
        "JACKY_INTERACTIVE": "1",                   # not allowlisted → dropped
        "JACKY_BASE_URL": "https://api.internal",   # not allowlisted → dropped
        "JACKY_KANBAN_DB": "postgres://u:p@h/db",   # not allowlisted → dropped
...
# ``jacky_tools.py`` stub, and the LLM's ``script.py``.  Both contain
...
class TestSandboxWritesUtf8:
    ...
    def test_stub_and_script_writes_specify_utf8(self):
        """Both ``jacky_tools.py`` and ``script.py`` writes in
    ...
    def test_file_rpc_stub_uses_utf8(self):
        ...
        from tools.code_execution_tool import generate_jacky_tools_module
        stub = generate_jacky_tools_module(["terminal"], transport="file")
    ...
    def test_stub_source_roundtrips_through_utf8(self):
        ...
        from tools.code_execution_tool import generate_jacky_tools_module
        import tempfile, ast
        stub = generate_jacky_tools_module(
    ...
    def test_windows_default_encoding_would_have_failed(self):
        ...
        *without* ``encoding="utf-8"`` would corrupt the file.  If this
        ...
        stub = generate_jacky_tools_module(["terminal"], transport="uds")
...
# let the child import jacky_tools, a user script that printed non-ASCII

...
class TestWriteDenyExactPaths:
    ...
    def test_jacky_env(self):
        # ``.env`` under the active JACKY_HOME (profile-aware, not just
    ...
    def test_jacky_root_env_when_running_under_profile(self, tmp_path, monkeypatch):
        ...
        ``~/.jacky/.env`` (whose credentials are inherited by every profile)
        could be silently overwritten by ``write_file`` while a profile was
        active.
        """
        root = tmp_path / "jacky_root"

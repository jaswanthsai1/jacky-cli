...
class TestGatingWithTarget:
    """``JACKY_DISABLE_LAZY_INSTALLS=1`` must STOP blocking once a durable
    ...
    def test_disable_env_blocks_without_target(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "jacky_cli.config.load_config", lambda: {}, raising=False
        )
...
@pytest.mark.skipif(
    os.environ.get("JACKY_RUN_NETWORK_TESTS") != "1",
    reason="opt-in real-install test (set JACKY_RUN_NETWORK_TESTS=1); CI runs "

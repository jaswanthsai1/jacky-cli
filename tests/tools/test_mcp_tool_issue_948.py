...
def test_resolve_stdio_command_falls_back_to_jacky_node_bin(tmp_path):
    ...
    with patch("tools.mcp_tool.shutil.which", return_value=None), \
         patch.dict("os.environ", {"JACKY_HOME": str(tmp_path)}, clear=False):
...
def test_resolve_stdio_command_falls_back_to_usr_local_bin():
    """When ``npx`` isn't on the filtered PATH and isn't under ``$JACKY_HOME/node/bin``
    ...
    the upstream ``node:bookworm-slim`` image (which the Jacky Docker image
    ...
    # the other candidates ($JACKY_HOME/node/bin/npx and ~/.local/bin/npx)
...
def test_run_stdio_uses_resolved_command_and_prepended_path(tmp_path):
    ...
    async def _test():
        with patch("tools.mcp_tool.shutil.which", return_value=None), \
             patch.dict("os.environ", {"JACKY_HOME": str(tmp_path), "PATH": "/usr/bin", "HOME": str(tmp_path)}, clear=False), \

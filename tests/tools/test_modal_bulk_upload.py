...
class TestModalBulkUpload:
    ...
    def test_tar_archive_contains_all_files(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src_a), "/root/.jacky/credentials/a.json"),
            (str(src_b), "/root/.jacky/skills/b.py"),
        ]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            ...
            assert "root/.jacky/credentials/a.json" in names
            assert "root/.jacky/skills/b.py" in names

            # Verify content
            a_content = tar.extractfile("root/.jacky/credentials/a.json").read()
            assert a_content == b"cred_content"
            b_content = tar.extractfile("root/.jacky/skills/b.py").read()
    ...
    def test_mkdir_includes_all_parents(self, monkeypatch, tmp_path):
        ...
        files = [
            (str(src), "/root/.jacky/credentials/f.txt"),
            (str(src), "/root/.jacky/skills/deep/nested/f.txt"),
        ]
        ...
        cmd = exec_calls[0][2]
        assert "/root/.jacky/credentials" in cmd
        assert "/root/.jacky/skills/deep/nested" in cmd
    ...
    def test_single_exec_call(self, monkeypatch, tmp_path):
        ...
        for i in range(20):
            ...
            files.append((str(src), f"/root/.jacky/cache/file_{i}.txt"))
    ...
    def test_bulk_upload_wired_in_filesyncmanager(self, monkeypatch):
        ...
        env._sync_manager = modal_env.FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.jacky"),
    ...
    def test_timeout_set_to_120(self, monkeypatch, tmp_path):
        ...
        src = tmp_path / "f.txt"
        src.write_text("data")
        files = [(str(src), "/root/.jacky/f.txt")]
    ...
    def test_stdin_chunked_for_large_payloads(self, monkeypatch, tmp_path):
        ...
        files = [(str(src), "/root/.jacky/large.bin")]
        ...
        with tarfile.open(fileobj=buf, mode="r:gz") as tar:
            names = tar.getnames()
            assert "root/.jacky/large.bin" in names

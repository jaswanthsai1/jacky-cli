...
def fake_jacky(tmp_path, monkeypatch):
    """Build a two-profile Jacky layout and point JACKY_HOME at
    the jacky-security profile (matching the original-incident shape).
    """
    ...
    monkeypatch.setattr(fs, "_jacky_home_path", lambda: sec_home)
...
class TestWriteFileCrossProfileGuard:
    def test_in_profile_write_allowed(self, fake_jacky):
        from tools.file_tools import write_file_tool
        target = fake_jacky["sec_home"] / "skills" / "new-skill" / "SKILL.md"
    ...
    def test_cross_profile_write_blocked_by_default(self, fake_jacky):
        """The May 2026 incident — security-profile session edits default
        profile's skill. Must be blocked."""
        from tools.file_tools import write_file_tool
        target = fake_jacky["root"] / "skills" / "shared-skill" / "SKILL.md"
        ...
        assert "jacky-security" in result["error"]
        # File untouched.
        assert target.read_text() == original

    def test_cross_profile_True_bypass(self, fake_jacky):
    ...
    def test_non_jacky_path_unaffected(self, fake_jacky, tmp_path):
        from tools.file_tools import write_file_tool
...
class TestPatchCrossProfileGuard:
    def test_cross_profile_patch_blocked(self, fake_jacky):
    ...
    def test_cross_profile_patch_bypass(self, fake_jacky):
        from tools.file_tools import patch_tool
        target = fake_jacky["root"] / "skills" / "shared-skill" / "SKILL.md"
        result_json = patch_tool(
            mode="replace",
    ...
    def test_v4a_patch_extracts_path_for_guard(self, fake_jacky):
...
class TestSkillManageCrossProfileErrorUX:
    ...
    def test_error_names_other_profile_when_skill_lives_there(
        self, fake_jacky, monkeypatch
    ):
        """The original incident shape — model expects 'foo' in active
        profile, but 'foo' lives in default. Error must point at default."""
        self._make_skill_in_profile(fake_jacky["root"], "default-only-skill")

        # Re-import the module so SKILLS_DIR picks up JACKY_HOME (set in
        ...
        assert "not found in active profile 'jacky-security'" in err
        assert "default" in err
        assert "cross_profile=True" in err

    def test_error_names_multiple_profiles(self, fake_jacky, monkeypatch):
        """When the skill exists in TWO other profiles, both should be named."""
        self._make_skill_in_profile(fake_jacky["root"], "everywhere-skill")
        self._make_skill_in_profile(fake_jacky["coder_home"], "everywhere-skill")
...
class TestSystemPromptActiveProfile:
    def test_default_profile_line_in_prompt(self, tmp_path, monkeypatch):
        ...
        about ~/.jacky/profiles/<name>/."""
        # Don't set JACKY_HOME — falls back to default.
        import agent.file_safety as fs
        monkeypatch.setattr(fs, "_jacky_home_path", lambda: tmp_path / "fake")
        monkeypatch.setattr(fs, "_jacky_root_path", lambda: tmp_path / "fake")
    ...
    def test_named_profile_line_in_prompt_text(self, fake_jacky):
        """When active profile is 'jacky-security', the prompt warns
        ...
        assert "Active Jacky profile" in src
        assert "cross_profile=True" in src
        assert "~/.jacky/profiles/" in src
        # Both branches present (default and named profile).
        assert "Active Jacky profile: default" in src
        assert "Active Jacky profile: {active_profile}" in src

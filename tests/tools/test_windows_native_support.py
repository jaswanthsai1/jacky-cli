...
code paths Jacky takes on native Windows.
...
class TestConfigureWindowsStdio:
    """``jacky_cli.stdio.configure_windows_stdio`` wiring.
    ...
    - respect JACKY_DISABLE_WINDOWS_UTF8 opt-out
    """
    ...
    def _reset_configured(self, monkeypatch):
        ...
        sys.modules.pop("jacky_cli.stdio", None)
        # Fresh import now; tests import from jacky_cli.stdio themselves,
        # but this guarantees the module they get is a brand-new copy.
        import jacky_cli.stdio as _s
        _s._CONFIGURED = False
        yield
        sys.modules.pop("jacky_cli.stdio", None)

    def test_no_op_on_posix(self):
        from jacky_cli import stdio
    ...
    def test_windows_path_sets_env_and_reconfigures_streams(self, monkeypatch):
        ...
        monkeypatch.delenv("JACKY_DISABLE_WINDOWS_UTF8", raising=False)
    ...
    def test_disable_flag_short_circuits(self, monkeypatch, optout):
        ...
        monkeypatch.setattr(stdio, "is_windows", lambda: True)
        monkeypatch.setenv("JACKY_DISABLE_WINDOWS_UTF8", optout)
...
class TestSigkillFallback:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("jacky_cli.kanban_db", 'getattr(signal, "SIGKILL", signal.SIGTERM)'),
        ],
    )
...
class TestWebServerPtyBridgeGuard:
    ...
    def test_import_guard_present_in_source(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "jacky_cli" / "web_server.py").read_text(encoding="utf-8")
...
class TestEntryPointsConfigureStdio:
    """jacky_cli/cli.py, jacky_cli/main.py, gateway/run.py must call configure_windows_stdio."""

    @pytest.mark.parametrize(
        "relpath",
        ["jacky_cli/cli.py", "jacky_cli/main.py", "gateway/run.py"],
    )
    def test_entry_point_calls_configure_stdio(self, relpath):
        root = Path(__file__).resolve().parents[2]
        source = (root / relpath).read_text(encoding="utf-8")
        assert "configure_windows_stdio" in source, (
            f"{relpath} must call jacky_cli.stdio.configure_windows_stdio() "
...
class TestSubprocessCompatHelpers:
    """jacky_cli/_subprocess_compat.py POSIX + Windows behaviour."""

    def test_is_windows_matches_sys_platform(self):
        from jacky_cli import _subprocess_compat as sc
    ...
    def test_resolve_node_command_returns_absolute_on_posix(self):
        ...
        from jacky_cli._subprocess_compat import resolve_node_command
    ...
    def test_windows_detach_flags_includes_breakaway_from_job(self, monkeypatch):
        ...
        Without it, the gateway-respawn watcher spawned by ``jacky update``
        (which runs under jacky-setup.exe, itself a grandchild of the
...
# ---------------------------------------------------------------------------
# jacky_cli/kanban_db.py waitpid guard
# ---------------------------------------------------------------------------
...
class TestKanbanWaitpidWindowsGuard:
    ...
    def test_source_gates_waitpid_loop(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "jacky_cli" / "kanban_db.py").read_text(encoding="utf-8")
...
class TestCodeExecutionTransportTcpFallback:
    ...
    AF_INET endpoints based on the JACKY_RPC_SOCKET format.
    """
...
class TestNpmBareSpawnsResolved:
    ...
    shutil.which / jacky_cli._subprocess_compat.resolve_node_command
    so Windows can execute the .cmd batch shims."""

    @pytest.mark.parametrize(
        "relpath",
        [
            "jacky_cli/tools_config.py",
            "jacky_cli/doctor.py",
...
class TestLocalEnvironmentWindowsTempDir:
    ...
    def test_source_has_windows_branch_using_jacky_home(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "tools" / "environments" / "local.py").read_text(encoding="utf-8")
        assert "if _IS_WINDOWS:" in source
        assert "get_jacky_home" in source
        assert 'cache_dir = get_jacky_home() / "cache" / "terminal"' in source
...
class TestGitBashPathNormalization:
    ...
    def test_posix_noop(self):
        ...
        from jacky_cli.cli import _normalize_git_bash_path
        if sys.platform != "win32":
...
class TestWorktreeSymlinkFallback:
    ...
    def test_source_has_symlink_fallback(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "jacky_cli" / "cli.py").read_text(encoding="utf-8")
...
class TestGatewayDetachedWatcherWindowsFlags:
    ...
    def test_jacky_cli_gateway_uses_compat_kwargs(self):
        root = Path(__file__).resolve().parents[2]
        source = (root / "jacky_cli" / "gateway.py").read_text(encoding="utf-8")
        assert "windows_detach_popen_kwargs" in source, (
            "jacky_cli/gateway.py must use the platform-aware detach helper"
        )
    ...
    def test_launch_detached_profile_gateway_restart_inlined_watcher_uses_breakaway(self):
        ...
        jacky_cli._subprocess_compat; the watcher must call that helper from
        ...
        """
        root = Path(__file__).resolve().parents[2]
        text = (root / "jacky_cli" / "gateway.py").read_text(encoding="utf-8")
        ...
        block = text[idx:end]
        assert "from jacky_cli._subprocess_compat import" in block
    ...
    def test_watcher_rewrites_console_python_to_windowless(self):
        ...
        assert '_popen_kwargs["env"]' in block, (
            ...
            "JACKY_HOME) so the windowless base pythonw resolves jacky_cli."
        )
...
class TestWindowlessGatewayRestartSpec:
    ...
    def test_noop_on_non_windows(self):
        import jacky_cli.gateway_windows as gw

        argv = ["/path/venv/bin/python", "-m", "jacky_cli.main", "gateway", "run"]
    ...
    def test_windows_rewrites_to_pythonw_and_preserves_tail(self):
        ...
        # ``from jacky_cli.gateway import PROJECT_ROOT`` resolves from
        ...
        import jacky_cli.config  # noqa: F401
        ...
        # Mock get_jacky_home too: the real one calls Path.resolve(), which
        ...
        with mock.patch.object(gw.sys, "platform", "win32"), mock.patch.object(
            ...
            gw, "_stable_gateway_working_dir", return_value="C:/jacky"
        ), mock.patch(
            "jacky_cli.config.get_jacky_home", return_value="C:/jacky"
        ):
            new_argv, cwd, env = gw.windowless_gateway_restart_spec(list(argv))

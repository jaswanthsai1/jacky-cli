...
def _run_migration(jacky_home: Path, **env_overrides: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env.update(
        {
...
def test_docker_config_migrate_does_not_rewrite_invalid_yaml(tmp_path: Path) -> None:
    ...
    assert "jacky config:" in proc.stderr
...
def test_docker_config_migrate_skip_env_leaves_config_unchanged(tmp_path: Path) -> None:
    ...
    proc = _run_migration(tmp_path, JACKY_SKIP_CONFIG_MIGRATION="1")
...
def test_docker_config_migrate_second_boot_preserves_env_byte_for_byte(tmp_path: Path) -> None:
    ...
    ``$JACKY_HOME/.env``. The first boot migrates the stale config and bumps

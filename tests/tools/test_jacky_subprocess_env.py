"""Tests for jacky_subprocess_env() — the centralized credential-safe env
...
full credential environment. Two tiers:
  ...
  * Tier 2 (_JACKY_PROVIDER_ENV_BLOCKLIST): LLM provider/tool keys — stripped
...
def _build(extra=None, *, inherit_credentials=False):
    ...
    with patch.dict(os.environ, env, clear=True):
        return jacky_subprocess_env(inherit_credentials=inherit_credentials)
...
class TestStripByDefault:
    ...
    def test_force_prefix_hints_stripped(self):
        result = _build({f"{_JACKY_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY": "sk-x"})
        assert f"{_JACKY_PROVIDER_ENV_FORCE_PREFIX}OPENAI_API_KEY" not in result

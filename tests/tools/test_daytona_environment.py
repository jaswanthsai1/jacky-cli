...
class TestPersistence:
    def test_persistent_resumes_via_get(self, make_env):
        ...
        env._mock_client.get.assert_called_once_with("jacky-mytask")
    ...
    def test_persistent_resumes_legacy_via_list(self, make_env, daytona_sdk):
        ...
        env._mock_client.list.assert_called_once_with(
            labels={"jacky_task_id": "mytask"}, limit=1)
        env._mock_client.create.assert_not_called()
    ...
    def test_persistent_creates_new_when_none_found(self, make_env, daytona_sdk):
        ...
        env._mock_client.get.assert_called_with("jacky-mytask")
...
class TestExecute:
    ...
    def test_stdin_data_wraps_heredoc(self, make_env):
        ...
        # Base class uses JACKY_STDIN_ prefix for heredoc delimiters
        call_args = sb.process.exec.call_args_list[-1]
        cmd = call_args[0][0]
        assert "JACKY_STDIN_" in cmd
        assert "print" in cmd
        assert "hi" in cmd
...
class TestSyncSafety:
    def test_single_upload_quotes_parent_path(self, make_env, tmp_path):
        ...
        remote_path = "/root/.jacky/skills/evil; touch /tmp/daytona-owned/file.txt"
        ...
        assert mkdir_cmd == (
            "mkdir -p '/root/.jacky/skills/evil; touch /tmp/daytona-owned'"
        )
        assert "; touch" not in mkdir_cmd.replace(
            "'/root/.jacky/skills/evil; touch /tmp/daytona-owned'", ""
        )

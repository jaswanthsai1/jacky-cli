...
class TestLoadMCPConfig:
    def test_no_config_returns_empty(self):
        ...
        with patch("jacky_cli.config.load_config", return_value={"model": "test"}):
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("jacky_cli.config.load_config", return_value={"mcp_servers": servers}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("jacky_cli.config.load_config", return_value={"mcp_servers": "invalid"}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_jacky_schema(self):
...
class TestToolsetInjection:
    ...
    def test_server_toolset_skips_builtin_collision(self):
        ...
        fake_toolsets = {
            "jacky-cli": {"tools": ["terminal"], "description": "CLI", "includes": []},
        ...
        with patch("tools.mcp_tool._MCP_AVAILABLE", True), \
             ...
             patch("tools.mcp_tool._connect_server", side_effect=fake_connect), \
             patch("tools.registry.registry", mock_registry), \
             patch("jacky_cli.toolsets.TOOLSETS", fake_toolsets):
            from tools.mcp_tool import discover_mcp_tools
            discover_mcp_tools()
    ...
    def test_server_connection_failure_skipped(self):
        ...
        fake_toolsets = {
            "jacky-cli": {"tools": [], "description": "CLI", "includes": []},
        }
...
class TestMCPSelectiveToolLoading:
    ...
    def _run_discover(self, name, tool_names, config, session=None):
        ...
        async def run():
            with patch("tools.mcp_tool._connect_server", side_effect=fake_connect), \
                 patch("tools.registry.registry", mock_registry), \
                 patch("jacky_cli.toolsets.create_custom_toolset"):
    ...
    def test_no_toolset_created_when_everything_is_filtered_out(self):
        ...
        async def run():
            with patch("tools.mcp_tool._connect_server", side_effect=fake_connect), \
                 patch("tools.registry.registry", mock_registry), \
                 patch("jacky_cli.toolsets.create_custom_toolset", mock_create):

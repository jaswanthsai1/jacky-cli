...
Covers the canonical fix for issues #4146, #27303, #30882, #33057:
  ...
  4. tools.code_execution_tool._scrub_child_env — broad JACKY_ prefix dropped,
...
def gw_session(monkeypatch):
    """A clean gateway session: JACKY_GATEWAY_SESSION set, a bound session
...
def test_env_scrub_jacky_allowlist_and_secret_blocks():
    from tools.code_execution_tool import _scrub_child_env

    env = {
        # operational allowlist → kept
        "JACKY_HOME": "/h", "JACKY_PROFILE": "p",
        "JACKY_CONFIG": "/c.yaml", "JACKY_ENV": "/e",
        # other JACKY_* → dropped (broad prefix removed)
        "JACKY_BASE_URL": "https://x", "JACKY_INTERACTIVE": "1",
        "JACKY_KANBAN_DB": "postgres://u:p@h/db",
    ...
    for kept in ("JACKY_HOME", "JACKY_PROFILE", "JACKY_CONFIG", "JACKY_ENV", "PATH"):
        assert kept in out, f"{kept} should be kept"
    for dropped in (
        "JACKY_BASE_URL", "JACKY_INTERACTIVE", "JACKY_KANBAN_DB",
...
def test_env_scrub_logs_dropped_jacky_vars(caplog):
    """Dropping a non-allowlisted, non-secret JACKY_* var must be diagnosable:
    ...
    leave users guessing why a sandbox script sees an unset JACKY_* var."""
    import logging

    from tools.code_execution_tool import _scrub_child_env

    env = {
        "JACKY_HOME": "/h",          # allowlisted → kept, not logged
        "JACKY_BASE_URL": "https://x",   # dropped → logged
        "JACKY_KANBAN_DB": "postgres://u:p@h/db",  # dropped → logged
        "JACKY_API_KEY": "sk",       # secret → dropped silently (not logged)
        "PATH": "/usr/bin",           # safe prefix → kept
    }
    with caplog.at_level(logging.DEBUG, logger="tools.code_execution_tool"):
        out = _scrub_child_env(env, is_passthrough=lambda _: False, is_windows=False)

    assert "JACKY_HOME" in out and "PATH" in out
    assert "JACKY_BASE_URL" not in out and "JACKY_KANBAN_DB" not in out

    msgs = "\n".join(r.getMessage() for r in caplog.records)
    assert "JACKY_BASE_URL" in msgs and "JACKY_KANBAN_DB" in msgs
    assert "env_passthrough" in msgs
    # Secret vars are dropped but must NOT be named in the diagnostic log.
    assert "JACKY_API_KEY" not in msgs


def test_env_scrub_no_log_when_nothing_dropped(caplog):
    """No diagnostic noise when there are no dropped JACKY_* vars."""
    import logging

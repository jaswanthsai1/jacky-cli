...
def _clean_state():
    ...
    saved = {}
    for k in ("JACKY_INTERACTIVE", "JACKY_GATEWAY_SESSION", "JACKY_EXEC_ASK", "JACKY_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["JACKY_INTERACTIVE"] = "1"
        session_key = os.getenv("JACKY_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No JACKY_INTERACTIVE or JACKY_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True

...
class TestParseFrontmatterQuick:
    ...
    def test_nested_yaml(self):
        content = "---\nname: test\nmetadata:\n  jacky:\n    tags: [a, b]\n---\n\nBody.\n"
        fm = GitHubSource._parse_frontmatter_quick(content)
        assert fm["metadata"]["jacky"]["tags"] == ["a", "b"]
...
class TestTrustLevelFor:
    ...
    def test_nvidia_skills_tap_is_registered_and_trusted(self):
        ...
        # browseable/searchable through `jacky skills browse` must also
    ...
    def test_browseable_trusted_repos_have_taps(self):
        ...
        for repo in TRUSTED_REPOS:
            assert repo in tap_repos, (
                ...
                "browsable via `jacky skills browse`."
            )
...
def _make_index_source(skills):
    """Build a JackyIndexSource pre-loaded with a fixed skill list."""
    from tools.skills_hub import JackyIndexSource
    src = JackyIndexSource(auth=GitHubAuth())
    src._index = {"skills": skills}
    src._loaded = True
    return src


class TestJackyIndexSearch:
...
class TestProviderFilter:
    ...
    def test_unified_search_provider_filter_keeps_index_source(self):
        ...
        src = MagicMock()
        src.source_id.return_value = "jacky-index"
...
class TestOptionalSkillSourceMetadata:
    def test_scan_all_emits_repo_root_relative_metadata(self, tmp_path):
        ...
        assert meta is not None
        assert meta.repo == "jaswanthsai1/jacky-cli"
...
# ---------------------------------------------------------------------------
# _load_jacky_index — centralized index fetch (Browse-hub landing / search)
# ---------------------------------------------------------------------------


class TestLoadJackyIndex:
    ...
    def _isolate_cache(monkeypatch, tmp_path):
        ...
        cache_file = tmp_path / "jacky-index.json"
        monkeypatch.setattr(hub, "_jacky_index_cache_file", lambda: cache_file)
        return cache_file
    ...
    def test_persistent_decoding_error_falls_back_to_stale_cache(
        ...
        old = time.time() - (hub.JACKY_INDEX_TTL + 100)
        import os

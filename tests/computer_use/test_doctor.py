...
downstream consumers (CI / `jacky update`) rely on:
...
Jacky-side adapter behaves correctly against the documented response
shape.
"""
...
# ── JACKY_CUA_DRIVER_CMD resolution ───────────────────────────────────────
...
class TestDriverCmdResolution:
    ...
    def test_env_var_used_when_no_arg_given(self, monkeypatch):
        ...
        monkeypatch.setenv("JACKY_CUA_DRIVER_CMD", "/env/path/cua-driver")
        proc = _fake_proc_with_responses(
            {"jsonrpc": "2.0", "id": 1, "result": {}},
            {"jsonrpc": "2.0", "id": 2, "result": {"structuredContent": _ok_report()}},
        )

...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install jacky-agent[matrix]`"
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"jacky-agent[{extra}]" in spec
        ]
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `jacky update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid jacky update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `jacky update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `jacky update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (jacky_cli/dingtalk_auth.py)

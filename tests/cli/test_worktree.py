...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("jacky/jacky-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned jacky/* and pr-* branches."""

    def test_prunes_orphaned_jacky_branch(self, git_repo):
        """jacky/jacky-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "jacky/jacky-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "jacky/jacky-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "jacky/jacky-deadbeef" in result.stdout
        ...
        assert "jacky/jacky-deadbeef" in orphaned
        ...
        assert "jacky/jacky-deadbeef" not in result.stdout
...
class TestWorktreeLockReaping:
    ...
    def _mk(cli, repo, name, pid=None, dirty=False, unpushed=False, age_h=100):
        ...
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", f"jacky/{name}", "HEAD"],
            cwd=repo, capture_output=True,
        )
        if pid is not None:
            subprocess.run(
                ["git", "worktree", "lock", "--reason", f"jacky pid={pid}", str(p)],
                cwd=repo, capture_output=True,
            )
        if unpushed:
...
class TestWorktreeLockPredicate:
    ...
    def test_unlocked_returns_none(self, git_repo):
        ...
        (git_repo / ".worktrees").mkdir(exist_ok=True)
        subprocess.run(
            ["git", "worktree", "add", str(p), "-b", "jacky/jacky-x", "HEAD"],

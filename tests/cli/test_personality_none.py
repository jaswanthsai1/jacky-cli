...
class TestCLIPersonalityNone:
    ...
    def test_none_saves_to_config(self):
        cli = self._make_cli()
        with patch("jacky_cli.cli.save_config_value", return_value=True) as mock_save:
            cli._handle_personality_command("/personality none")
...
class TestGatewayPersonalityNone:
    ...
    async def test_empty_personality_list_uses_profile_display_path(self, tmp_path):
        ...
        with patch("gateway.run._jacky_home", tmp_path), \
             patch("jacky_cli.jacky_constants.display_jacky_home", return_value="~/.jacky/profiles/coder"):
            event = self._make_event("")
            result = await runner._handle_personality_command(event)

        assert result == "No personalities configured in `~/.jacky/profiles/coder/config.yaml`"
...
class TestPersonalityDictFormat:
    ...
    def test_resolve_prompt_dict_no_tone_no_style(self):
        from jacky_cli.cli import JackyCLI
        result = JackyCLI._resolve_personality_prompt({
    ...
    def test_resolve_prompt_string(self):
        from jacky_cli.cli import JackyCLI
        result = JackyCLI._resolve_personality_prompt("You are helpful.")
        assert result == "You are helpful."

...
Verifies that ``JackyCLI._handle_update_command`` correctly:
...
Also verifies that ``jacky_cli.main._launch_tui`` correctly handles exit
...
def _make_self(modal_response):
    ...
    so we don't need a full ``JackyCLI`` construction.
...
def _call(self_):
    ...
    return JackyCLI._handle_update_command(self_)
...
def test_managed_install_refuses_and_does_not_set_pending_relaunch(capsys):
    ...
    with (
        patch("jacky_cli.config.is_managed", return_value=True),
        patch(
            "jacky_cli.config.format_managed_message",
            return_value="Use `brew upgrade jacky-agent` to update.",
        ),
    ):
        result = _call(self_)

    out = capsys.readouterr().out
    assert "brew upgrade jacky-agent" in out
...
def test_affirmative_answer_sets_pending_relaunch_and_returns_true(answer, capsys):
    ...
    with patch("jacky_cli.config.is_managed", return_value=False):
        result = _call(self_)

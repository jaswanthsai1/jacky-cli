...
minimal ``JackyCLI`` stub (pattern used elsewhere in tests/cli).
"""
...
def jacky_home(tmp_path, monkeypatch):
    """Isolated JACKY_HOME so SessionDB.state_meta writes stay hermetic."""
    home = tmp_path / ".jacky"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("JACKY_HOME", str(home))

    # Bust the goal module's DB cache so it re-resolves JACKY_HOME each test.
...
class TestInterruptAutoPause:
    def test_interrupted_turn_pauses_goal_and_skips_continuation(self, jacky_home):
        ...
        with patch("jacky_cli.goals.judge_goal") as judge_mock:
    ...
    def test_interrupted_turn_is_resumable(self, jacky_home):
        ...
        with patch("jacky_cli.goals.judge_goal"):
            cli._maybe_continue_goal_after_turn()
...
class TestEmptyResponseSkip:
    def test_empty_response_does_not_invoke_judge(self, jacky_home):
    ...
    def test_no_assistant_message_skipped(self, jacky_home):
...
class TestHealthyTurnStillRuns:
    ...
    def test_clean_response_marks_done_when_judge_says_done(self, jacky_home):
...
class TestInterruptFlagLifecycle:
    def test_chat_resets_flag_at_entry(self, jacky_home):

...
class TestCliApprovalUi:
    ...
    def test_approval_display_wraps_preview_hint_on_narrow_terminal(self):
        ...
        with patch("jacky_cli.cli.shutil.get_terminal_size",
    ...
    def test_background_task_registers_thread_local_approval_callbacks(self):
        ...
        assert seen["approval"].__func__ is JackyCLI._approval_callback
        assert seen["sudo"].__self__ is cli
        assert seen["sudo"].__func__ is JackyCLI._sudo_password_callback
...
def _make_real_paint_cli_stub():
    ...
    cli._paint_now = JackyCLI._paint_now.__get__(cli, JackyCLI)
    cli._invalidate = JackyCLI._invalidate.__get__(cli, JackyCLI)

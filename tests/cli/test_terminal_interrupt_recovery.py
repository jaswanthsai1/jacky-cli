...
The recovery path lives in ``JackyCLI._recover_terminal_after_interrupt()``,
...
@pytest.fixture
def bare_cli():
    """A JackyCLI with no __init__ — we only exercise the recovery helper."""
    return object.__new__(JackyCLI)


class TestRecoverTerminalAfterInterrupt:
    """Directly exercise JackyCLI._recover_terminal_after_interrupt()."""
    ...
    def test_drains_stdin_then_redraws(self, bare_cli):
        ...
        with patch("jacky_cli.curses_ui.flush_stdin") as mock_flush:
    ...
    def test_redraw_still_runs_when_flush_fails(self, bare_cli):
        ...
        bare_cli._force_full_redraw = MagicMock()
        with patch(
            "jacky_cli.curses_ui.flush_stdin", side_effect=OSError("no tty")
        ):
    ...
    def test_flush_runs_before_redraw(self, bare_cli):
        ...
        with patch(
            "jacky_cli.curses_ui.flush_stdin",
...
class TestFinallyBlockWiring:
    ...
    def test_recovery_is_invoked_behind_interrupt_guard(self):
        src = inspect.getsource(JackyCLI.run)
    ...
    def test_recovery_helper_exists(self):
        assert hasattr(JackyCLI, "_recover_terminal_after_interrupt")
        assert callable(JackyCLI._recover_terminal_after_interrupt)

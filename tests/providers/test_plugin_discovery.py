...
Verifies that:
 ...
 2. User plugins at $JACKY_HOME/plugins/model-providers/<name>/ override bundled
...
def _clear_provider_caches():
    ...
    for mod in list(sys.modules.keys()):
        if (
            ...
            or mod.startswith("_jacky_user_provider")
        ):
            del sys.modules[mod]
...
def test_general_plugin_manager_skips_model_provider_kind(tmp_path, monkeypatch):
    ...
    from jacky_cli import plugins as plugin_mod
    ...
    user_plugin = jacky_home / "plugins" / "test-model-provider"

...
from jacky_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(jacky_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <jacky_home>/plugins/<name> and opt it in."""
    plugin_dir = jacky_home / "plugins" / name
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    jacky_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        jacky_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("JACKY_HOME", str(tmp_path / "jacky_empty"))

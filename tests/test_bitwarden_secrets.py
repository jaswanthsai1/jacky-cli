...
binary download are exercised manually by `jacky secrets bitwarden
...
def jacky_home(tmp_path, monkeypatch):
    """Point Jacky at an isolated home directory."""
    home = tmp_path / ".jacky"
    home.mkdir()
...
def test_install_bws_rejects_malicious_member(jacky_home, monkeypatch):
...
def test_install_bws_happy_path(jacky_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(jacky_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(jacky_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from jacky_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under jacky_home/cache/."""
    home = tmp_path / ".jacky"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")

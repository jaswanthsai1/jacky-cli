...
def test_root_session_no_compression(db):
    ...
    assert prov["currentJackySessionId"] == "root1"
    assert prov["rootJackySessionId"] == "root1"
    assert prov["parentJackySessionId"] is None
...
def test_compression_split_continuation(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "new", previous_jacky_session_id="old"
    )
    assert prov["sessionKind"] == "continuation"
    assert prov["parentJackySessionId"] == "old"
    assert prov["rootJackySessionId"] == "old"
    assert prov["compressionDepth"] == 1
    assert prov["previousJackySessionId"] == "old"
...
def test_multi_depth_chain(db):
    ...
    assert prov["rootJackySessionId"] == "s0"
...
def test_non_compression_parent_is_root_not_continuation(db):
    ...
    assert prov["sessionKind"] == "root"
    assert prov["compressionDepth"] == 0
    assert prov["rootJackySessionId"] == "p"  # lineage root still walked
...
def test_no_false_rotation_when_head_unchanged(db):
    ...
    prov = build_session_provenance(
        db, "acp-1", "s", previous_jacky_session_id="s"
    )
    assert "reason" not in prov
    assert "creatorKind" not in prov
    assert prov["previousJackySessionId"] == "s"
...
def test_meta_wrapper_shape(db):
    ...
    assert set(meta.keys()) == {"jacky"}
    assert "sessionProvenance" in meta["jacky"]
    assert meta["jacky"]["sessionProvenance"]["currentJackySessionId"] == "root1"

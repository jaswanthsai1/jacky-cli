...
from acp_adapter.server import JackyACPAgent
...
class TestMcpRegistrationE2E:
    ...
    async def test_session_with_mcp_servers_registers_tools(self, acp_agent, mock_manager):
        """new_session with mcpServers converts them to Jacky config and registers."""
        servers = [
            McpServerStdio(
                name="test-fs",
        ...
        with patch("tools.mcp_tool.register_mcp_servers", side_effect=mock_register), \
             patch("jacky_cli.model_tools.get_tool_definitions", return_value=fake_tools):

...
def test_moa_slots_routed_through_resolve_runtime_provider(monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.runtime_provider.resolve_runtime_provider", fake_resolve
    )
...
def test_moa_slot_runtime_falls_back_on_resolution_error(monkeypatch):
    ...
    monkeypatch.setattr(
        "jacky_cli.runtime_provider.resolve_runtime_provider", boom
    )
...
def test_reference_messages_drops_system_but_renders_tools_as_text():
    ...
    messages = [
        {"role": "system", "content": "huge jacky system prompt"},
    ...
    assert all("huge jacky system prompt" not in m["content"] for m in view)

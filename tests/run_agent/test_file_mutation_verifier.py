...
from jacky_cli.run_agent import (
    AIAgent,
...
class TestFormatFooter:
    ...
    def test_paths_are_backtick_wrapped(self):
        ...
        out = AIAgent._format_file_mutation_failure_footer(
            {"/home/u/.jacky/config.yaml": {
                "tool": "patch",
                "error_preview": (
                    "Write denied: '/home/u/.jacky/config.yaml' is a "
                    "protected system/credential file."
                ),
            }},
        )
        # Path still human-readable.
        assert "/home/u/.jacky/config.yaml" in out
    ...
    def test_footer_path_not_extracted_by_gateway(self):
        ...
        tmp = tempfile.mkdtemp(prefix="jacky_footer_")
        try:
...
class TestVerifierEnabled:
    def test_default_is_enabled(self, monkeypatch):
        monkeypatch.delenv("JACKY_FILE_MUTATION_VERIFIER", raising=False)
        agent = _bare_agent()
    ...
    def test_env_disables(self, monkeypatch, value):
        monkeypatch.setenv("JACKY_FILE_MUTATION_VERIFIER", value)
        agent = _bare_agent()
        assert agent._file_mutation_verifier_enabled() is False

    def test_env_enables_over_config(self, monkeypatch):
        monkeypatch.setenv("JACKY_FILE_MUTATION_VERIFIER", "1")
        import jacky_cli.config as _cfg_mod
        monkeypatch.setattr(

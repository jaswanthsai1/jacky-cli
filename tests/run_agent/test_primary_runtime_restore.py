...
class TestRestorePrimaryRuntime:
    ...
    def test_restore_keeps_primary_base_url_when_fallback_pool_attached(self):
        ...
        with (
            patch("jacky_cli.run_agent.OpenAI", return_value=MagicMock()),
    ...
    def test_restore_survives_exception(self):
        ...
        with patch("jacky_cli.run_agent.OpenAI", side_effect=Exception("connection refused")):
            result = agent._restore_primary_runtime()
...
class TestTryRecoverPrimaryTransport:
    ...
    def test_survives_rebuild_failure(self):
        ...
        with patch("jacky_cli.run_agent.OpenAI", side_effect=Exception("socket error")), \
             patch("time.sleep"):
            result = agent._try_recover_primary_transport(
                error, retry_count=3, max_retries=3,
            )
...
class TestRateLimitCooldown:
    ...
    def test_cooldown_set_on_rate_limit_reason(self):
        ...
        from jacky_cli.run_agent import FailoverReason
        agent = _make_agent(
            fallback_model={"provider": "openrouter", "model": "anthropic/claude-sonnet-4"},
        )

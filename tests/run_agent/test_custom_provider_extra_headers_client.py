...
def test_custom_provider_extra_headers_applied_at_construction(mock_openai):
    ...
    with patch("jacky_cli.config.load_config", return_value=_PROXY_CONFIG):
        agent = AIAgent(
            api_key="proxy-key",
            base_url=_PROXY_URL,
            model="my-model",
    ...
    assert headers["X-Client-Name"] == "jacky-agent"
...
def test_extra_headers_merge_with_global_default_headers(mock_openai):
        ...
        "custom_providers": [
            {
                ...
                "base_url": _PROXY_URL,
                "api_key": "proxy-key",
                "extra_headers": {"User-Agent": "jacky-proxy", "X-Local": "2"},
            }
        ],
    }
    with patch("jacky_cli.config.load_config", return_value=config):
        agent = AIAgent(
            api_key="proxy-key",
            base_url=_PROXY_URL,
            model="my-model",
    ...
    headers = agent._client_kwargs["default_headers"]
    assert headers["User-Agent"] == "jacky-proxy"  # per-provider wins

...
The ``jacky update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m jacky_cli.main gateway run`` — image name
    ``pythonw``, not ``jacky.exe`` — so the ``taskkill /IM jacky.exe`` guard
    ...
    # The jacky.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM jacky.exe' in text

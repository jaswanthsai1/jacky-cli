# Contributing to Jacky Agent

Thank you for contributing to Jacky Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
2. **Cross-platform compatibility** — macOS, different Linux distros, and WSL2 on Windows. We want Jacky to work everywhere.
...
- **Search both open *and* merged PRs and issues** for your topic or error symptom — the duplicate-check in the PR template fires at review time, after you've already done the work:
  ```bash
  gh search issues --repo jaswanthsai1/jacky-cli "<your terms>"
  gh search prs --repo jaswanthsai1/jacky-cli --state all "<your terms>"
  ```
  Or use the web UI: [issues](https://github.com/jaswanthsai1/jacky-cli/issues?q=) · [PRs (all states)](https://github.com/jaswanthsai1/jacky-cli/pulls?q=is%3Apr).
...
Related: #38284 covers the agent-side analog — Jacky itself checking existing issues and PRs before deep self-troubleshooting. This section is the human-contributor complement.
...
Bundled skills (in `skills/`) ship with every Jacky install. They should be **broadly useful to most users**:

- Document handling, web research, common dev workflows, system administration
- Used regularly by a wide range of people

If your skill is official and useful but not universally needed (e.g., a paid service integration, a heavyweight dependency), put it in **`optional-skills/`** — it ships with the repo but isn't activated by default. Users can discover it via `jacky skills browse` (labeled "official") and install it with `jacky skills install` (no third-party warning, built-in trust).

If your skill is specialized, community-contributed, or niche, it's better suited for a **Skills Hub** — upload it to a skills registry and share it in the [Nous Research Discord](https://discord.gg/NousResearch). Users can install it with `jacky skills install`.

---

## Memory Providers: Ship as a Standalone Plugin

**We are no longer accepting new memory providers into this repo.** The set of built-in providers under `plugins/memory/` (honcho, mem0, supermemory, byterover, hindsight, holographic, openviking, retaindb) is closed. If you want to add a new memory backend, publish it as a **standalone plugin repo** that users install into `~/.jacky/plugins/` (or via a pip entry point).

Standalone memory plugins:

- Implement the same `MemoryProvider` ABC (`agent/memory_provider.py`) — `sync_turn`, `prefetch`, `shutdown`, and optionally `post_setup(jacky_home, config)` for setup-wizard integration
...
- Integrate with `jacky memory setup` via `post_setup()` — no need to touch core code
...
The installer creates the Jacky venv, wires the `jacky` command, stamps the
install method for `jacky update`, and clones the full git project into
`$JACKY_HOME/jacky-agent` (usually `~/.jacky/jacky-agent`). That keeps your
...
Use this only if you intentionally do not want Jacky' managed install layout
(for example, a throwaway clone inside a container or CI job). If you install
this way, make sure you run the `jacky` entrypoint from this venv; running the
system `python3 -m jacky_cli.main` can pick up unrelated system Python
packages.
...
uv venv ~/.jacky/venvs/jacky-dev --python 3.11
export VIRTUAL_ENV="$HOME/.jacky/venvs/jacky-dev"
...
```bash
mkdir -p ~/.jacky/{cron,sessions,logs,memories,skills}
cp cli-config.yaml.example ~/.jacky/config.yaml
touch ~/.jacky/.env

# Add at minimum an LLM provider key:
echo "OPENROUTER_API_KEY=***" >> ~/.jacky/.env
```

### Run

```bash
# The standard installer already put `jacky` on PATH.
jacky doctor
jacky chat -q "Hello"
```

If you used the manual clone fallback, run `./jacky` from the checkout or
symlink this clone's venv explicitly:

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/jacky" ~/.local/bin/jacky
```
...
│
├── jacky_cli/               # CLI command implementations
...
### User configuration (stored in `~/.jacky/`)

| Path | Purpose |
|------|---------|
| `~/.jacky/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.jacky/.env` | API keys and secrets |
| `~/.jacky/auth.json` | OAuth credentials (Nous Portal) |
| `~/.jacky/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.jacky/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.jacky/state.db` | SQLite session database |
| `~/.jacky/sessions/` | Gateway routing index (`sessions.json`), request-dump breadcrumbs, gateway `*.jsonl` transcripts, and (optionally) per-session JSON snapshots when `sessions.write_json_snapshots: true` is set. The per-session snapshots are off by default; state.db is canonical. |
| `~/.jacky/cron/` | Scheduled job data |
| `~/.jacky/whatsapp/session/` | WhatsApp bridge credentials |
...
(for example `_JACKY_CORE_TOOLS` or a dedicated toolset); otherwise the tool
...
Four fields are supported under `metadata.jacky`:
...
The user may skip setup and keep loading the skill. Jacky only exposes metadata (`stored_as`, `skipped`, `validated`) to the model — never the secret value.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `jacky setup` or update `~/.jacky/.env` locally.
...
2. **Tools referenced in SKILL.md prose must be native Jacky tools or MCP servers the skill explicitly expects.** When the skill needs a capability, point at the proper tool by name in backticks: `` `terminal` ``, `` `web_extract` ``, `` `web_search` ``, `` `read_file` ``, `` `write_file` ``, `` `patch` ``, `` `search_files` ``, `` `vision_analyze` ``, `` `browser_navigate` ``, `` `delegate_task` ``, `` `image_generate` ``, `` `text_to_speech` ``, `` `cronjob` ``, `` `memory` ``, `` `skill_view` ``, `` `todo` ``, `` `execute_code` ``.
...
4. **`author` credits the human contributor first.** For external contributions, the contributor's real name + GitHub handle goes first (`Jane Doe (jane-doe)`); "Jacky Agent" is the secondary collaborator. If the contributor's commit shows "Jacky Agent" as author because they used Jacky to draft the skill, replace it with their actual name — credit the human, not the tool.
...
- **No external dependencies unless absolutely necessary.** Prefer stdlib Python, curl, and existing Jacky tools (`web_extract`, `terminal`, `read_file`).
...
- **Test it.** Run `jacky --toolsets skills -q "Use the X skill to do Y"` and verify the agent follows the instructions correctly.

---

## Adding a Skin / Theme

Jacky uses a data-driven skin system — no code changes needed to add a new skin.

**Option A: User skin (YAML file)**

Create `~/.jacky/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `jacky_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `jacky_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

Jacky runs on Linux, macOS, and native Windows (plus WSL2). When writing code
...
1. **Never call `os.kill(pid, 0)` for liveness checks.** `os.kill(pid, 0)`
   ...
   **Preferred:** use `psutil` (a core dependency — always available):
   ...
   If you specifically need the jacky wrapper (it has a stdlib fallback
...
2. **Use `shutil.which()` before shelling out — don't assume Windows has
   ...
   `jacky_cli/gateway.py::_scan_gateway_pids` for the pattern.
...
10. **Detached background daemons on Windows need `pythonw.exe`, NOT
    ...
    See `jacky_cli/gateway_windows.py::_spawn_detached` for the reference
    implementation.
...
16. **Two different quoting schemes in one command line.** `subprocess.run
    ...
    helpers and never cross them. See `jacky_cli/gateway_windows.py::
...
Jacky has terminal access. Security matters.
...
2. **Test manually**: Run `jacky` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/jaswanthsai1/jacky-cli/issues)
- Include: OS, Python version, Jacky version (`jacky version`), full error traceback

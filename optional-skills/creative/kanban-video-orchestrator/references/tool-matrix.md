...
Maps each role archetype to the Jacky skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public jacky-agent
...
## Jacky skills relevant to video production

### Visual / rendering skills (`jacky-agent/skills/creative/`)
...
### Design / pre-production skills (`jacky-agent/skills/creative/`)
...
### Audio / media skills (`jacky-agent/skills/creative/` + `skills/media/`)
...
These are **not** Jacky skills but external CLIs / APIs that profiles invoke.
...
## Built-in Jacky tools for media review

These are native Jacky tools — not invoked via terminal but through their own
...
key is present in `${JACKY_HOME:-~/.jacky}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Jacky profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${JACKY_HOME:-~/.jacky}/.env` → environment variable.
...
When a new Jacky-public video skill ships:

...
the actual `setup.sh` that creates the project workspace, configures Jacky
...
2. **Create profiles** — `jacky profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.jacky/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `jacky kanban create` assigned to the director
...
```bash
jacky profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.jacky/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.jacky/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Jacky
...
Each profile gets a `SOUL.md` at `~/.jacky/profiles/<name>/SOUL.md` that
...
```bash
jacky kanban create "Direct production of <video title>" \
    --assignee director \
...
the Jacky `.env` (`${JACKY_HOME:-$HOME/.jacky}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _jacky_env="${JACKY_HOME:-$HOME/.jacky}/.env"
    if grep -q "^${var}=" "$_jacky_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_jacky_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_jacky_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY jacky ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY jacky OPENROUTER_API_KEY || exit 1
# ...
```

...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("JACKY_HOME", os.path.expanduser("~/.jacky")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("JACKY_HOME", Path.home() / ".jacky")) / "skills" / "red-teaming" / "godmode"
...
JACKY_HOME = Path(os.getenv("JACKY_HOME", Path.home() / ".jacky"))
CONFIG_PATH = JACKY_HOME / "config.yaml"
PREFILL_PATH = JACKY_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Jacky models — already uncensored, just needs clean prompt
    "jacky": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "jacky" in model_lower or "nous" in model_lower:
        return "jacky"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Jacky config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.jacky/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Jacky for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Jacky for changes to take effect.")

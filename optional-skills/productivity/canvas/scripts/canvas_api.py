#!/usr/bin/env python3
"""Canvas LMS API CLI for Jacky Agent.
...
def _check_config():
    ...
    if missing:
        jacky_env = os.path.join(
            os.environ.get("JACKY_HOME", os.path.expanduser("~/.jacky")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {jacky_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Jacky Agent"
    )

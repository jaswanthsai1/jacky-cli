"""Shared constants for Jacky Agent.
...
_UNSET = object()
_JACKY_HOME_OVERRIDE: ContextVar[str | object] = ContextVar(
    "_JACKY_HOME_OVERRIDE", default=_UNSET
)


def set_jacky_home_override(path: str | Path | None) -> Token:
    """Set a context-local Jacky home override and return its reset token.

    This is for in-process, per-task scoping.  It deliberately does not mutate
    ``os.environ`` because that is shared by every thread in the process.
    """
    value: str | object = _UNSET if path is None else str(path)
    return _JACKY_HOME_OVERRIDE.set(value)


def reset_jacky_home_override(token: Token) -> None:
    """Restore the previous context-local Jacky home override."""
    _JACKY_HOME_OVERRIDE.reset(token)


def get_jacky_home_override() -> str | None:
    """Return the active context-local Jacky home override, if any."""
    override = _JACKY_HOME_OVERRIDE.get()
    if override is _UNSET or not override:
        return None
    return str(override)


def _get_platform_default_jacky_home() -> Path:
    """Return the platform-native default Jacky home path."""
    if sys.platform == "win32":
        local_appdata = os.environ.get("LOCALAPPDATA", "").strip()
        base = Path(local_appdata) if local_appdata else Path.home() / "AppData" / "Local"
        return base / "jacky"
    return Path.home() / ".jacky"


def get_jacky_home() -> Path:
    """Return the Jacky home directory (default: platform-native path).

    Reads JACKY_HOME env var, falls back to the platform-native default.
    This is the single source of truth — all other copies should import this.

    When ``JACKY_HOME`` is unset but an ``active_profile`` file indicates
    ...
    expected to propagate ``JACKY_HOME`` explicitly (see the systemd
    template in ``jacky_cli/gateway.py`` and the kanban dispatcher in
    ``jacky_cli/kanban_db.py``).  See https://github.com/jaswanthsai1/jacky-cli/issues/18594.
    """
    ...
    if not _profile_fallback_warned:
        try:
            fallback_home = _get_platform_default_jacky_home()
        ...
        if active and active != "default":
            ...
            msg = (
                f"[JACKY_HOME fallback] JACKY_HOME is unset but active "
                f"profile is {active!r}. Falling back to {fallback_home}, which "
                f"is the DEFAULT profile — not {active!r}. Any data this "
                f"process writes will land in the wrong profile. The "
                f"subprocess spawner should pass JACKY_HOME explicitly "
                f"(see issue #18594)."
            )
            try:
    ...
    return _get_platform_default_jacky_home()


def get_default_jacky_root() -> Path:
    """Return the root Jacky directory for profile-level operations.

    In standard deployments this is the platform-native Jacky home
    (``~/.jacky`` on POSIX, ``%LOCALAPPDATA%\\jacky`` on native Windows).

    In Docker or custom deployments where ``JACKY_HOME`` points outside
    ``~/.jacky`` (e.g. ``/opt/data``), returns ``JACKY_HOME`` directly
    — that IS the root.

    In profile mode where ``JACKY_HOME`` is ``<root>/profiles/<name>``,
    returns ``<root>`` so that ``profile list`` can see all profiles.
    Works both for standard (``~/.jacky/profiles/coder``) and Docker
    (``/opt/data/profiles/coder``) layouts.

    Import-safe — no dependencies beyond stdlib.
    """
    native_home = _get_platform_default_jacky_home()
    env_home = os.environ.get("JACKY_HOME", "")
    if not env_home:
        return native_home
    env_path = Path(env_home)
    try:
        env_path.resolve().relative_to(native_home.resolve())
        # JACKY_HOME is under ~/.jacky (normal or profile mode)
        return native_home
    except ValueError:
        pass
    ...
    # Not a profile path — JACKY_HOME itself is the root
    return env_path
...
def _get_packaged_data_dir(name: str) -> Path | None:
    ...
    Used to discover bundled skills/optional-skills when Jacky is installed
...
def get_optional_skills_dir(default: Path | None = None) -> Path:
    ...
    tree and expose it via ``JACKY_OPTIONAL_SKILLS``.
    """
    override = os.getenv("JACKY_OPTIONAL_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_jacky_home() / "optional-skills"
...
def get_optional_mcps_dir(default: Path | None = None) -> Path:
    ...
    package tree and expose it via ``JACKY_OPTIONAL_MCPS``.
    """
    override = os.getenv("JACKY_OPTIONAL_MCPS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_jacky_home() / "optional-mcps"
...
def get_bundled_skills_dir(default: Path | None = None) -> Path:
    ...
    Resolution order:
        1. ``JACKY_BUNDLED_SKILLS`` env var (Nix wrapper / explicit override)
        2. Wheel-installed ``<sysconfig data>/skills`` (pip install path)
        3. Caller-supplied ``default`` (typically the source-checkout path)
        4. ``<JACKY_HOME>/skills`` last-resort
    """
    override = os.getenv("JACKY_BUNDLED_SKILLS", "").strip()
    if override:
        return Path(override)
    ...
    if default is not None:
        return default
    return get_jacky_home() / "skills"


def get_jacky_dir(new_subpath: str, old_name: str) -> Path:
    """Resolve a Jacky subdirectory with backward compatibility.
    ...
    Args:
        new_subpath: Preferred path relative to JACKY_HOME (e.g. ``"cache/images"``).
        old_name: Legacy path relative to JACKY_HOME (e.g. ``"image_cache"``).
...
def iter_jacky_node_dirs(home: Path | None = None) -> list[Path]:
    """Return Jacky-managed Node.js directories in preferred lookup order.

    Windows installs from ``scripts/install.ps1`` unpack portable Node directly
    into ``%LOCALAPPDATA%\\jacky\\node``. POSIX installs use
    ``$JACKY_HOME/node/bin``. Include both shapes on every platform so mixed
    or migrated installs still work.
    """
    root = home or get_jacky_home()
    dirs = [root / "node"]
    bin_dir = root / "node" / "bin"
    # NOTE: keep this ordering in sync with jackyManagedNodePathEntries() in
...
_JACKY_NODE_TARGET_MAJOR = int(os.environ.get("JACKY_NODE_TARGET_MAJOR", "22"))
...
def node_tool_runnable(path: str | None) -> bool:
    ...
    Jacky-managed Node trees live under ``$JACKY_HOME/node`` (or a profile's
    ``JACKY_HOME``). A partial upgrade or interrupted install can leave
    ``bin/npm`` behind while ``lib/cli.js`` is missing — the wrapper exists but
    immediately throws ``MODULE_NOT_FOUND``. ``find_jacky_node_executable``
    used to trust file presence alone, so ``jacky update`` would pick that
...
def jacky_managed_node_tree_present(home: Path | None = None) -> bool:
    """Return True when any Jacky-managed node/npm/npx shim exists on disk."""
    names = set()
    for command in ("node", "npm", "npx"):
        names.update(_candidate_node_command_names(command))
    for directory in iter_jacky_node_dirs(home):
        for name in names:
...
def _heal_managed_node_windows() -> bool:
    """Redownload the portable Node zip into ``%JACKY_HOME%\\node`` on Windows."""
    import re
    import tempfile
    ...
    home = get_jacky_home()
    index_url = f"https://nodejs.org/dist/latest-v{_JACKY_NODE_TARGET_MAJOR}.x/"
    try:
    ...
    match = re.search(
        rf"node-v{_JACKY_NODE_TARGET_MAJOR}\.\d+\.\d+-win-{node_arch}\.zip",
        index_html,
    )
    if not match:
        return False
...
def heal_jacky_managed_node() -> bool:
    """Redownload Jacky-managed Node when the tree exists but is broken.
    ...
    if _managed_node_heal_attempted:
        return False
    if not jacky_managed_node_tree_present():
        return False
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "JACKY_HOME": str(get_jacky_home())},
            capture_output=True,
            timeout=300,
            check=False,
        )
...
def find_jacky_node_executable(command: str) -> str | None:
    """Return a Jacky-managed Node/npm executable path, healing broken trees."""
    names = _candidate_node_command_names(command)
    broken_present = False
    for directory in iter_jacky_node_dirs():
        for name in names:
            candidate = directory / name
            if candidate.is_file() and (
                sys.platform == "win32" or os.access(candidate, os.X_OK)
                ...
                if node_tool_runnable(resolved):
                    return resolved
                broken_present = True
    if broken_present and heal_jacky_managed_node():
        for directory in iter_jacky_node_dirs():
            for name in names:
                candidate = directory / name
                if candidate.is_file() and (
...
def find_node_executable_on_path(command: str) -> str | None:
    ...
    directly, so prefer the launchable variants explicitly for Jacky-owned
    subprocesses.
    """
    if sys.platform != "win32":
...
def find_node_executable(command: str) -> str | None:
    """Resolve a Node.js command, preferring healthy Jacky-managed installs.

    This is for Jacky-owned subprocesses that should not be broken by a bad,
    ...
    """
    managed = find_jacky_node_executable(command)
    if managed:
        return managed
    if jacky_managed_node_tree_present():
        return None
    return find_node_executable_on_path(command)


def with_jacky_node_path(env: dict[str, str] | None = None) -> dict[str, str]:
    """Return *env* with Jacky-managed Node directories prepended to PATH."""
    merged = dict(os.environ if env is None else env)
    existing = merged.get("PATH", "")
    parts = [p for p in existing.split(os.pathsep) if p]
    managed = [str(path) for path in iter_jacky_node_dirs() if path.is_dir()]
...
def agent_browser_runnable(path: str | None) -> bool:
    ...
    next ``jacky update`` — leaving a **dangling symlink** that ``which`` still
...
def display_jacky_home() -> str:
    """Return a user-friendly display string for the current JACKY_HOME.

    Uses ``~/`` shorthand for readability::

        default:  ``~/.jacky``
        profile:  ``~/.jacky/profiles/coder``
        custom:   ``/opt/jacky-custom``

    Use this in **user-facing** print/log messages instead of hardcoding
    ``~/.jacky``.  For code that needs a real ``Path``, use
    :func:`get_jacky_home` instead.
    """
    home = get_jacky_home()
    try:
...
def secure_parent_dir(path: Path) -> None:
    ...
    prevent catastrophic host bricking when ``JACKY_HOME`` or other path
    env vars resolve to an unexpected location.

    See https://github.com/jaswanthsai1/jacky-cli/issues/25821.
    """
...
def _profile_home_path(env: dict[str, str] | None = None) -> str | None:
    """Return ``{JACKY_HOME}/home`` when the profile-home directory exists."""
    jacky_home = get_jacky_home_override() or (env or {}).get("JACKY_HOME") or os.getenv("JACKY_HOME")
    if not jacky_home:
        return None
    profile_home = os.path.join(jacky_home, "home")
...
def _iter_real_home_candidates(env: dict[str, str] | None = None) -> list[str]:
    ...
    env = env or {}
    candidates: list[str] = []
    explicit = str(env.get("JACKY_REAL_HOME") or os.getenv("JACKY_REAL_HOME", "")).strip()
    if explicit:
...
def get_real_home(env: dict[str, str] | None = None) -> str:
    """Return the OS user's real home directory, avoiding Jacky profile HOME.

    ``JACKY_HOME`` scopes Jacky state. ``HOME`` is reserved for the OS/user
    account and the many external CLIs that store credentials under ``~``.
    If a parent process is already running with ``HOME={JACKY_HOME}/home``,
...
def get_subprocess_home(env: dict[str, str] | None = None) -> str | None:
    ...
    * ``auto`` (default): host installs keep the real user HOME; containers use
      ``{JACKY_HOME}/home`` for persistent state. If a host parent already has
      HOME pointed at the profile home, repair subprocesses back to real HOME.
    * ``real``: always prefer the real OS-user HOME.
    * ``profile``: use ``{JACKY_HOME}/home`` when it exists, preserving the
...
def apply_subprocess_home_env(env: dict[str, str]) -> None:
    """Apply Jacky' subprocess HOME contract to *env* in-place."""
    real_home = get_real_home(env)
    if real_home:
        env["JACKY_REAL_HOME"] = real_home
...
def is_container() -> bool:
    ...
    See: jaswanthsai1/jacky-cli#47111
    """
...
def get_config_path() -> Path:
    """Return the path to ``config.yaml`` under JACKY_HOME.

    Replaces the ``get_jacky_home() / "config.yaml"`` pattern repeated
    in 7+ files (skill_utils.py, jacky_logging.py, jacky_time.py, etc.).
    """
    return get_jacky_home() / "config.yaml"


def get_skills_dir() -> Path:
    """Return the path to the skills directory under JACKY_HOME."""
    return get_jacky_home() / "skills"



def get_env_path() -> Path:
    """Return the path to the ``.env`` file under JACKY_HOME."""
...
def apply_ipv4_preference(force: bool = False) -> None:
    ...
    if getattr(socket.getaddrinfo, "_jacky_ipv4_patched", False):
        return
    ...
    _ipv4_getaddrinfo._jacky_ipv4_patched = True  # type: ignore[attr-defined]

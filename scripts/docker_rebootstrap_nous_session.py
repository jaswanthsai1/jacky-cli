...
Background
----------
A Nous bootstrap session (client_id ``jacky-cli-vps``) can take a terminal
...
``stage2-hook.sh`` seeds ``auth.json`` from ``JACKY_AUTH_JSON_BOOTSTRAP`` only
...
``JACKY_AUTH_JSON_REBOOTSTRAP`` (plus a restart). On boot we re-seed the Nous
...
Design constraints
------------------
- Pure stdlib, no jacky_cli imports: runs early in the boot hook, before the
...
# from JACKY_AUTH_JSON_BOOTSTRAP (create-only, blank-volume seed) so the two
...
REBOOTSTRAP_ENV = "JACKY_AUTH_JSON_REBOOTSTRAP"
...
def _nous_entry_is_terminal(nous_state: Any) -> bool:
    ...
    Mirrors the ``terminal`` predicate in ``jacky_cli.auth.get_nous_session_validity``:
...
def _extract_nous_from_seed(seed_raw: str) -> Optional[dict]:
    """Pull the ``providers.nous`` block out of a JACKY_AUTH_JSON_REBOOTSTRAP
    payload. The payload is a full auth.json document (same shape as
    JACKY_AUTH_JSON_BOOTSTRAP). Returns None if it can't be parsed or carries no
...
def reseed_if_terminal(auth_path: str, seed_raw: str) -> str:
    """Core logic. Returns a short status string for logging/testing:
      ...
      - "no_auth_file"     — auth.json absent (blank volume → let the normal
                             JACKY_AUTH_JSON_BOOTSTRAP path handle it)

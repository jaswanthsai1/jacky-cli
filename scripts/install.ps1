# ============================================================================
# Jacky Agent Installer for Windows
# ============================================================================
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$JackyHome = $(if ($env:JACKY_HOME) { $env:JACKY_HOME } else { "$env:LOCALAPPDATA\jacky" }),
    [string]$InstallDir = $(if ($env:JACKY_HOME) { "$env:JACKY_HOME\jacky-agent" } else { "$env:LOCALAPPDATA\jacky\jacky-agent" }),
    ...
    # builds apps/desktop into a launchable Jacky.exe.
    #
    # Why opt-in:
    #   * Jacky-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Jacky.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Jacky.exe
    ...
    #     `jacky desktop` already builds on demand.
...
$RepoUrlSsh = "git@github.com:jaswanthsai1/jacky-cli.git"
$RepoUrlHttps = "https://github.com/jaswanthsai1/jacky-cli.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Jacky Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $JackyHome)) {
        New-Item -ItemType Directory -Force -Path $JackyHome | Out-Null
    }
    $envFile = Join-Path $JackyHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $JackyHome "node"
...
function Install-Uv {
    # Jacky owns its own uv at $JackyHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (jacky_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `jacky update` stay in sync.
    $managedUv = Join-Path $JackyHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $JackyHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $JackyHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $JackyHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $JackyHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and JACKY_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Jacky-Setup.exe
...
function Install-Git {
    ...
    which Jacky uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\jacky\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Jacky needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Jacky copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\jacky\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``JACKY_GIT_BASH_PATH`` (User scope) so Jacky can find it in a fresh
    ...
    # Download PortableGit into $JackyHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $JackyHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Jacky features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Jacky needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``JACKY_GIT_BASH_PATH`` (User env scope) so Jacky can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $JackyHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $JackyHome\git\usr\bin\bash.exe
    $candidates += "$JackyHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$JackyHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("JACKY_GIT_BASH_PATH", $candidate, "User")
            $env:JACKY_GIT_BASH_PATH = $candidate
            Write-Info "Set JACKY_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Jacky may not find Git Bash."
    Write-Info "If needed, set JACKY_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$JackyHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$JackyHome\node;$env:Path"
        Write-Success "Node.js $version found (Jacky-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Jacky-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $JackyHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $JackyHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\jacky-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$JackyHome\node") { Remove-Item -Recurse -Force "$JackyHome\node" }
                Move-Item $extractedDir.FullName "$JackyHome\node"
                ...
                $version = & "$JackyHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $JackyHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\jacky-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `jacky
                    # update` path (#4735).
                    ...
                    $stashName = "jacky-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``jacky update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Jacky behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running jacky processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/jaswanthsai1/jacky-cli/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/jaswanthsai1/jacky-cli/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/jaswanthsai1/jacky-cli/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\jacky-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\jacky-agent-extract"
    ...
    # next `jacky update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Jacky-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running jacky process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running jacky processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Jacky_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (jacky.exe) plus its child tree.
            & taskkill /F /T /IM jacky.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM jacky.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m jacky_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not jacky.exe.
            ...
            # user (see jacky_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `jacky update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `jacky.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install jacky-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and jacky.exe broken with
    ...
    # here, not 30 seconds later when the user runs `jacky`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register jacky.exe in dist-info/RECORD but fail to
        ...
        # `jacky` command while jacky-agent.exe / jacky-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m jacky_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `jacky dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\jacky_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `jacky dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `jacky dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: jacky_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up jacky command..."
    
    if ($NoVenv) {
        $jackyBin = "$InstallDir"
    } else {
        $jackyBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so jacky is globally available
    # On Windows, the jacky.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$jackyBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$jackyBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $jackyBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set JACKY_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\jacky instead
    # of the Unix default ~/.jacky
    $currentJackyHome = [Environment]::GetEnvironmentVariable("JACKY_HOME", "User")
    if (-not $currentJackyHome -or $currentJackyHome -ne $JackyHome) {
        [Environment]::SetEnvironmentVariable("JACKY_HOME", $JackyHome, "User")
        Write-Success "Set JACKY_HOME=$JackyHome"
    }
    $env:JACKY_HOME = $JackyHome
    
    # Update current session
    $env:Path = "$jackyBin;$env:Path"
    
    Write-Success "jacky command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.jacky-bootstrap-complete which tells the Jacky
    ...
    $markerPath = Join-Path $InstallDir ".jacky-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the JACKY_HOME directory structure ($JackyHome, default %LOCALAPPDATA%\jacky)
    New-Item -ItemType Directory -Force -Path "$JackyHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JackyHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JackyHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JackyHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JackyHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JackyHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JackyHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JackyHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$JackyHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$JackyHome\.env"
    ...
    # Create config.yaml
    $configPath = "$JackyHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Jacky's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$JackyHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in jacky_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $JackyHome"
    
    # Seed bundled skills into $JackyHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $JackyHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $JackyHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$JackyHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $JackyHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Jacky-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'jacky setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\jacky-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\jacky-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\jacky-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Jacky` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Jacky.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_jacky_desktop command
    # resolves apps/desktop/release/win-unpacked/Jacky.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Jacky-managed
    ...
    # The --dir mode produces an unpacked Jacky.exe in
    ...
    # are NOT trying to work around). The Jacky icon + product name are
    # stamped onto Jacky.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\jacky-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Jacky.exe",
        "$desktopDir\release\win-arm64-unpacked\Jacky.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Jacky.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Jacky icon + identity are stamped onto Jacky.exe by the
    ...
    #    Jacky.exe. We deliberately do NOT point them at `jacky desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Jacky.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Jacky.lnk')
        )
        ...
        # with the Jacky icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.jacky/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'jacky setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m jacky_cli.main setup
    } else {
        python -m jacky_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $jackyCmd = "$InstallDir\venv\Scripts\jacky.exe"
    if (-not (Test-Path $jackyCmd)) {
        $jackyCmd = "jacky"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$JackyHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'jacky whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: jacky gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$JackyHome\logs\gateway.log"
            Start-Process -FilePath $jackyCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$JackyHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: jacky gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: jacky gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$JackyHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$JackyHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$JackyHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$JackyHome\jacky-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   jacky              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   jacky setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   jacky config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   jacky config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   jacky gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   jacky update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Jacky repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Jacky-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Jacky to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }

...
# Python-driven tool session (e.g. Jacky terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:jaswanthsai1/jacky-cli.git"
REPO_URL_HTTPS="https://github.com/jaswanthsai1/jacky-cli.git"
JACKY_HOME="${JACKY_HOME:-$HOME/.jacky}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${JACKY_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$JACKY_INSTALL_DIR"
...
#   code at /usr/local/lib/jacky-agent, command at /usr/local/bin/jacky,
#   data still at /root/.jacky (JACKY_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$JACKY_HOME/.no-bundled-skills so future"
            echo "                   'jacky update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Jacky.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.jacky/jacky-agent"
            echo "                   default (root, Linux): /usr/local/lib/jacky-agent"
            echo "  --jacky-home PATH  Data directory (default: ~/.jacky, or \$JACKY_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Jacky installs the code under"
            echo "  /usr/local/lib/jacky-agent and links the command into"
            echo "  /usr/local/bin/jacky (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$JACKY_HOME"
            echo "  (default /root/.jacky).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$JACKY_HOME/jacky-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs jacky setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `jacky update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Jacky-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Jacky Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install jacky command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `jacky` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $JACKY_HOME/jacky-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $JACKY_HOME/jacky-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/jacky-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $JACKY_HOME/jacky-agent — then preserve it)
#
# Always no-op when the user set --dir or $JACKY_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in JACKY_HOME.
    if is_termux; then
        INSTALL_DIR="$JACKY_HOME/jacky-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$JACKY_HOME/jacky-agent/.git" ]; then
            INSTALL_DIR="$JACKY_HOME/jacky-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/jacky-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/jacky-agent"
        ...
        # /usr/local/bin/jacky wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/jacky"
        log_info "  Data:    $JACKY_HOME (unchanged)"
...
# Point a Jacky-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $JACKY_HOME/node/bin — which is NOT on
...
# Jacky's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Jacky-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$JACKY_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$JACKY_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$JACKY_HOME/node/etc/npmrc"
}

get_jacky_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/jacky" ]; then
        echo "$link_dir/jacky"
    else
        echo "jacky"
    fi
...
install_uv() {
    ...
    # Jacky owns its own uv at $JACKY_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (jacky_cli/managed_uv.py) looks in the same
    # place, so install.sh and `jacky update` stay in sync.
    local _managed_uv="$JACKY_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $JACKY_HOME/bin ..."
    mkdir -p "$JACKY_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/jacky-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/jacky-uv-installer.$$.sh")"
    ...
    # directly into $JACKY_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$JACKY_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Jacky-
...
check_node() {
    ...
    # Repair pre-existing Jacky-managed installs where `npm install -g` lands
    ...
    # Prefer a Jacky-managed Node from a previous run over a too-old system one.
    if [ -x "$JACKY_HOME/node/bin/node" ] && node_satisfies_build "$("$JACKY_HOME/node/bin/node" --version)"; then
        export PATH="$JACKY_HOME/node/bin:$PATH"
        log_success "Node.js $("$JACKY_HOME/node/bin/node" --version) found (Jacky-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Jacky-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.jacky/node/..."
    ...
    # Place into ~/.jacky/node/ and symlink binaries into the same bin dir
    # the jacky command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$JACKY_HOME/node"
    mkdir -p "$JACKY_HOME"
    mv "$extracted_dir" "$JACKY_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$JACKY_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$JACKY_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$JACKY_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$JACKY_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.jacky/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Jacky install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Jacky Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `jacky update` path
                # (#4735).
                ...
                local stash_name
                stash_name="jacky-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``jacky update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Jacky behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up jacky command..."

    if [ "$USE_VENV" = true ]; then
        JACKY_BIN="$INSTALL_DIR/venv/bin/jacky"
    else
        JACKY_BIN="$(which jacky 2>/dev/null || echo "")"
        if [ -z "$JACKY_BIN" ]; then
            log_warn "jacky not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$JACKY_BIN" ]; then
        log_warn "jacky entry point not found at $JACKY_BIN"
    ...
    # Create a user-facing shim for the jacky command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $JACKY_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$JACKY_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/jacky"
    cat > "$command_link_dir/jacky" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$JACKY_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/jacky"
    log_success "Installed jacky launcher → $command_link_display_dir/jacky"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "jacky command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v jacky' \
                >/dev/null 2>&1; then
        ...
        log_info "jacky not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Jacky Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Jacky Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Jacky Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so jacky works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.jacky directory structure (config at top level, code in subdir)
    mkdir -p "$JACKY_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.jacky/.env (top level, easy to find)
    if [ ! -f "$JACKY_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$JACKY_HOME/.env"
            log_success "Created ~/.jacky/.env from template"
        else
            touch "$JACKY_HOME/.env"
            log_success "Created ~/.jacky/.env"
        fi
    else
        log_info "~/.jacky/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$JACKY_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.jacky/config.yaml (top level, easy to find)
    if [ ! -f "$JACKY_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$JACKY_HOME/config.yaml"
            log_success "Created ~/.jacky/config.yaml from template"
        fi
    else
        log_info "~/.jacky/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in jacky_cli/default_soul.py — the
    ...
    if [ ! -f "$JACKY_HOME/SOUL.md" ]; then
        cat > "$JACKY_HOME/SOUL.md" << 'SOUL_EOF'
You are Jacky Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.jacky/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.jacky/"

    # Seed bundled skills into ~/.jacky/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `jacky update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'jacky update'." \
            > "$JACKY_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $JACKY_HOME/.no-bundled-skills"
        log_info "  Future 'jacky update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.jacky/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.jacky/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$JACKY_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$JACKY_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.jacky/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Jacky Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Jacky will use the bundled Chromium instead."
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Jacky Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (jacky --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `jacky update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'jacky setup' after install."
        return 0
    fi
    ...
    # Run jacky setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m jacky_cli.main setup < /dev/tty
    else
        python -m jacky_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$JACKY_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Jacky to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$JACKY_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'jacky whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                JACKY_CMD="$(get_jacky_command_path)"
                $JACKY_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'jacky whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'jacky gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $JACKY_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $JACKY_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: jacky gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: jacky gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $JACKY_CMD gateway > "$JACKY_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.jacky/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: jacky gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: jacky gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $JACKY_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $JACKY_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $JACKY_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}jacky${NC}              Start chatting"
    echo -e "   ${GREEN}jacky setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}jacky config${NC}       View/edit configuration"
    echo -e "   ${GREEN}jacky config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}jacky gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}jacky update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'jacky' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'jacky' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'jacky' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$JACKY_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$JACKY_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$JACKY_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$JACKY_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$JACKY_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Jacky for pip install"
    ...
    JACKY_CMD="$(command -v jacky 2>/dev/null || echo "")"
    if [ -n "$JACKY_CMD" ]; then
        log_info "Running jacky setup..."
        "$JACKY_CMD" setup
    else
        log_warn "jacky command not found on PATH"
        log_info "Try: python -m jacky_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Jacky` rename
...
# _purge_electron_build_cache() used by `jacky desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Jacky-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `jacky desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Jacky" ]; then
            app="$desktop_dir/release/linux-unpacked/Jacky"
        elif [ -x "$desktop_dir/release/linux-unpacked/jacky" ]; then
            app="$desktop_dir/release/linux-unpacked/jacky"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Jacky.app" \
            "$desktop_dir/release/mac/Jacky.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Jacky is damaged and can't be opened").
    ...
    # checkout stays clean for the next `jacky update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Jacky-managed Node
            # provisioned during prerequisites/node-deps (at $JACKY_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $JACKY_HOME.
    # $JACKY_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'jacky update' on this host install.

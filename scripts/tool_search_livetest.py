#!/usr/bin/env python3
"""Live test harness for Jacky Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any jacky imports.
ORIGINAL_HOME = os.environ.get("JACKY_HOME")
ORIGINAL_AUTH = Path.home() / ".jacky" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.jacky/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.jacky/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="jacky_ts_live_"))
    jacky_home = home_dir / ".jacky"
    jacky_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, jacky_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".jacky" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, jacky_home / ".env")
        ...
        load_jacky_dotenv(jacky_home=str(Path.home() / ".jacky"))
    ...
    (jacky_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return jacky_home
...
def reset_module_state():
    """Drop cached modules so the new JACKY_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "jacky_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def run_one_scenario(scenario: Dict[str, Any], enabled: bool, out_dir: Path) -> Dict[str, Any]:
    ...
    os.environ["JACKY_HOME"] = str(home)
...
def main():
    ...
    # Restore original JACKY_HOME
    if ORIGINAL_HOME is not None:
        os.environ["JACKY_HOME"] = ORIGINAL_HOME
    else:

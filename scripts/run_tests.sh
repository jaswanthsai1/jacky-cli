#!/usr/bin/env bash
# Canonical test runner for jacky-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.jacky/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.jacky/jacky-agent/venv"; do
...
if [ -f "$HOME/.jacky/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.jacky"
...
exec env -i \
  ...
  ${JACKY_RUN_SLOW_PET_TESTS:+JACKY_RUN_SLOW_PET_TESTS="$JACKY_RUN_SLOW_PET_TESTS"} \

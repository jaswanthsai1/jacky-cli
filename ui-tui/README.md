...
React + Ink terminal UI for Jacky. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `JACKY_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- `src/app/setupHandoff.ts` — launches external `jacky setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.jacky/.jacky_history` or under `JACKY_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `jacky setup` wizard, suspends Ink while it runs
...
```text
ui-tui/
  packages/jacky-ink/   forked Ink renderer (local dep)
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external jacky setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.jacky/tui-parent.log
    ...
    types/
      jacky-ink.d.ts            type declarations for @jacky/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent JackyCLI subprocess for slash commands
```
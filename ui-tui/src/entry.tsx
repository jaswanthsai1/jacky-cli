...
import type { FrameEvent } from '@jacky/ink'
...
const dumpNotice = (snap: MemorySnapshot, dump: HeapDumpResult | null) =>
  `jacky-tui: ${snap.level} memory (${formatBytes(snap.heapUsed)}) — auto heap dump → ${dump?.heapPath ?? dump?.diagPath ?? '(failed)'}\n`
...
setupGracefulExit({
  ...
  onError: (scope, err) => {
    ...
    process.stderr.write(`jacky-tui lifecycle ${scope}: ${message.slice(0, 2000)}\n`)
  },
  onSignal: signal => {
    ...
    resetTerminalModes()
    process.stderr.write(`jacky-tui lifecycle: received ${signal}\n`)
  },
...
const stopMemoryMonitor = startMemoryMonitor({
  onCritical: (snap, dump) => {
    ...
    process.stderr.write(
      `jacky-tui lifecycle: memory critical exit heap=${formatBytes(snap.heapUsed)} rss=${formatBytes(snap.rss)}\n`
    )
    process.stderr.write(dumpNotice(snap, dump))
    process.stderr.write('jacky-tui: exiting to avoid OOM; restart to recover\n')
    process.exit(137)
  },
  ...
  onWarn: snap => {
    ...
    process.stderr.write(
      `jacky-tui: heap climbing fast (${formatBytes(snap.heapUsed)}) — a large tool output or long session may be straining memory\n`
    )
  }
})

if (process.env.JACKY_HEAPDUMP_ON_START === '1') {

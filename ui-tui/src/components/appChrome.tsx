import { Box, type ScrollBoxHandle, stringWidth, Text } from '@jacky/ink'
...
const renderIndicator = (style: IndicatorStyle, tick: number): IndicatorRender => {
  ...
  if (style === 'emoji') {
    return {
      frame: EMOJI_FRAMES[tick % EMOJI_FRAMES.length] ?? '>_',
...
// Jacky display aliases — show a friendly name in the status bar while the
// real model id (e.g. MiniMax-M3) is what actually gets called on the API.
const MODEL_DISPLAY_ALIASES: Record<string, string> = {
  'minimax-m3': 'Jacky Ultra',
}
const shortModelLabel = (model: string) => {
  const _alias = MODEL_DISPLAY_ALIASES[(model.split('/').pop() || model).toLowerCase()]
...
export function StatusRule({
  ...
  // Dev-only readout (JACKY_DEV_CREDITS). The server omits the key entirely unless the
  ...
  // Dev-gated readout (JACKY_DEV_CREDITS), lowest priority,

...
describe('createGatewayEventHandler', () => {
  ...
  it('surfaces self-improvement review summaries as a persistent system line', () => {
    ...
    onEvent({
      payload: { text: "💾 Self-improvement review: Skill 'jacky-release' patched" },
      type: 'review.summary'
    } as any)

    expect(ctx.system.sys).toHaveBeenCalledWith("💾 Self-improvement review: Skill 'jacky-release' patched")
  })
  ...
  it('annotates gateway.start_timeout with stderr tail lines so users can diagnose without /logs', () => {
    ...
    onEvent({
      payload: {
        ...
        stderr_tail:
          '[startup] timed out\nModuleNotFoundError: No module named openai\nFileNotFoundError: ~/.jacky/config.yaml'
      },
  ...
  it('prefers raw text over Rich-rendered ANSI on message.complete (#16391)', () => {
    ...
    const rendered = '\u001b[33mJacky here.\u001b[0m\n\n\u001b[2mLine two.\u001b[0m'
  ...
  it('shows setup panel for missing provider startup error', () => {
    ...
    onEvent({
      payload: {
        message:
          'agent init failed: No LLM provider configured. Run `jacky model` to select a provider, or run `jacky setup` for first-time configuration.'
      },
      type: 'error'
    } as any)

 ...
 *
 * Root cause: re-exporting `ink-text-input` from `@jacky/ink`'s
 * entry-exports drags the upstream `ink` package into the bundle. That
 * `ink` graph and our in-tree `@jacky/ink` graph reference each other
...
function bundleIsFresh(): boolean {
  ...
  try {
    ...
    const sourceMtime = statSync(
      resolve(uiTuiRoot, 'packages/jacky-ink/src/entry-exports.ts'),
    ).mtimeMs
...
describe('TUI bundle (issue #31227)', () => {
  ...
  it('does not bundle the upstream ink package or ink-text-input', () => {
    ...
    // that #31227 was about. The in-tree fork at @jacky/ink replaces
    // all of `ink`; nothing in ui-tui imports `TextInput` from
    // `@jacky/ink` so the re-export is unused dead weight.
  ...
  it('has the @jacky/ink entry-exports module compiled to sync init', () => {
    // Sanity check that the alias swap to packages/jacky-ink/src/entry-exports.ts
    // is still active and producing the expected synchronous init shape.
    expect(bundleSrc).toMatch(/var init_entry_exports = __esm\(\{\s*"packages\/jacky-ink\/src\/entry-exports\.ts"\(\)/)
  })
})

 ...
 * jacky-ink sets ``key.meta`` for plain Alt/Option on every platform
 ...
 * (``jacky_cli/voice.py::normalize_voice_record_key_for_prompt_toolkit``)
 ...
 * modifier — but jacky-ink reports Alt as ``key.meta`` on many
...
export const parseVoiceRecordKey = (raw: unknown): ParsedVoiceRecordKey => {
  ...
  // On macOS jacky-ink reports Alt as ``key.meta``, which ``isActionMod``
...
export const isVoiceToggleKey = (
  ...
  switch (configured.mod) {
    case 'alt':
      ...
      // alt binding.
      //
      // Bare Escape on jacky-ink can arrive as ``key.meta=true`` on some
    ...
    case 'ctrl':
      ...
      // ``key.meta`` is jacky-ink's Alt signal and accepting it would

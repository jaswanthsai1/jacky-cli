...
const logDir = join(process.env.JACKY_HOME?.trim() || join(homedir(), '.jacky'), 'logs')
...
// a real ~/.jacky (tests must stay hermetic — see AGENTS.md).
...
export function recordParentLifecycle(line: string): void {
  ...
  try {
    ...
    if (!warned) {
      warned = true
      process.stderr.write('jacky-tui: parent lifecycle log unavailable\n')
    }
  }
}

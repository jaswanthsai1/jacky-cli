...
export function shouldForceTruecolor(env: NodeJS.ProcessEnv = process.env): boolean {
  const override = (env.JACKY_TUI_TRUECOLOR ?? '').trim()
...
if (shouldForceTruecolor()) {
  ...
  // incorrectly. Keep Jacky on the safer TERM-driven 256-color path unless
  // users explicitly opt back in via JACKY_TUI_TRUECOLOR=1.

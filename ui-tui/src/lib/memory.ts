...
export interface HeapDumpResult {
  ...
  // the heavy snapshot because JACKY_AUTO_HEAPDUMP was not enabled (#21767).
  suppressed?: boolean
  success: boolean
}
...
export async function performHeapDump(trigger: MemoryTrigger = 'manual'): Promise<HeapDumpResult> {
  try {
    ...
    const dir = process.env.JACKY_HEAPDUMP_DIR?.trim() || join(homedir() || tmpdir(), '.jacky', 'heapdumps')

    await mkdir(dir, { recursive: true })

    const base = `jacky-${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}-${trigger}`
    ...
    const autoEnabled = /^(?:1|true|yes|on)$/i.test((process.env.JACKY_AUTO_HEAPDUMP ?? '').trim())
...
async function pruneHeapdumps(dir: string): Promise<void> {
  const raw = process.env.JACKY_HEAPDUMP_MAX_BYTES?.trim()

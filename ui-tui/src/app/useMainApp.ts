import { type ScrollBoxHandle, useApp, useHasSelection, useSelection, useStdout, useTerminalTitle } from '@jacky/ink'
...
export function useMainApp(gw: GatewayClient) {
  ...
  useTerminalTitle(
    model ? composeTabTitle(marker, ui.sessionTitle, model, tabCwd ? shortCwd(tabCwd, 24) : '') : 'Jacky'
  )
  ...
  const cwd = ui.info?.cwd || process.env.JACKY_CWD || process.cwd()

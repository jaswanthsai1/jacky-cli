import type { RunExternalProcess } from '@jacky/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`jacky ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching jacky: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`jacky ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })

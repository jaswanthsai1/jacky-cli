...
three virtual model IDs so the ``jacky tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not os.environ.get("OPENAI_API_KEY"):
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `jacky tools` → Image "
                    "Generation → OpenAI to configure, or `jacky setup` "
                    "to add the key."
                ),

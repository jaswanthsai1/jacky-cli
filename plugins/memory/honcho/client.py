...
Resolution order for config file:
  1. $JACKY_HOME/honcho.json  (instance-local, enables isolated Jacky instances)
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Jacky profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Jacky profile.

    Resolution order:
      1. JACKY_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``jacky.<profile>``
      3. Fallback: ``"jacky"`` (default profile)
    """
    explicit = os.environ.get("JACKY_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $JACKY_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.jacky/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_jacky_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_jacky_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Jacky system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.jacky
    ...
    def from_global_config(
        ...
        Resolution: $JACKY_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Jacky profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.jacky block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Jacky session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Jacky session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'jacky honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run jacky honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `jacky honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                jacky_cfg = load_config()
                honcho_cfg = jacky_cfg.get("honcho", {})

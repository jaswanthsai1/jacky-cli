...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/jacky>
...
```bash
jacky memory setup honcho   # configure Honcho directly (works on a fresh install)
jacky memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
jacky config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.jacky/.env
```

> `jacky honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `jacky memory setup honcho`.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$JACKY_HOME/honcho.json` | Profile-local (isolated Jacky instances) |
| 2 | `~/.jacky/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Jacky profile: `jacky` (default) or `jacky_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `jacky honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Jacky session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`jacky-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`jacky`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `alice-jacky-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening jacky in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Jacky session ID (timestamp + hex). Every `jacky` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Jacky profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`jacky`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active Jacky profile: `jacky` (default) or `jacky_<profile>` (e.g. `jacky -p coder` -> host key `jacky_coder`). Older `jacky.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
| `JACKY_HONCHO_HOST` | Host key override |
...
| `HONCHO_OAUTH_CLIENT_ID` | OAuth client (default `jacky-agent`) |
...
|---------|-------------|
| `jacky memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `jacky honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `jacky memory setup`) |
| `jacky honcho status` | Show resolved config for active profile |
| `jacky honcho enable` / `disable` | Toggle Honcho for active profile |
| `jacky honcho mode <mode>` | Change recall or observation mode |
| `jacky honcho peer --user <name>` | Update user peer name |
| `jacky honcho peer --ai <name>` | Update AI peer name |
| `jacky honcho tokens --context <N>` | Set context token budget |
| `jacky honcho tokens --dialectic <N>` | Set dialectic max chars |
| `jacky honcho map <name>` | Map current directory to a session name |
| `jacky honcho sync` | Create host blocks for all Jacky profiles |

...
Config: Uses the existing Honcho config chain:
  1. $JACKY_HOME/honcho.json (profile-scoped)
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, jacky_home):
        """Write config to $JACKY_HOME/honcho.json (Honcho SDK native format)."""
        import json
        import os
        from pathlib import Path
        config_path = Path(jacky_home) / "honcho.json"
        existing = {}
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            try:
                ...
                # behave as they did before. New setups via `jacky honcho setup`
    ...
    def _start_session_init_background(self, *, wait_timeout: float = 0.0) -> None:
        ...
        This keeps Jacky CLI/gateway startup responsive when Honcho is down,
        ...
        with self._init_lock:
            ...
            init_session_id = self._lazy_init_session_id or "jacky-default"
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        # Skip under per-session strategy: every Jacky run creates a fresh
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "jacky-default",

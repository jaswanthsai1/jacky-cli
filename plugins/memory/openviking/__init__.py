...
or a linked OpenViking CLI config:
  ...
  OPENVIKING_AGENT     — Jacky peer ID in OpenViking (default: jacky)
...
_DEFAULT_AGENT = "jacky"
_AGENT_PROMPT_LABEL = "Jacky peer ID in OpenViking"
...
_SYNC_TRACE_ENV = "JACKY_OPENVIKING_SYNC_TRACE"
...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Jacky slash-skill scaffolding before sending content to OpenViking.
...
def _load_jacky_openviking_config() -> dict:
    try:
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Jacky session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Jacky only", "write values only to Jacky .env"),
    ...
    _save_jacky_only_config(
        config=config,
        provider_config=provider_config,
        env_path=env_path,
        values=values,
    )
    _print_openviking_ready("Connection saved to Jacky .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        # against a concurrent increment. See jacky-agent#28296 review.
    ...
    def is_available(self) -> bool:
        ...
        if os.environ.get("OPENVIKING_ENDPOINT"):
            return True
        provider_config = _load_jacky_openviking_config()
    ...
    def get_config_schema(self):
        return [
            ...
            },
            {
                "key": "agent",
                "description": (
                    "Jacky peer ID in OpenViking, sent as the actor peer and "
    ...
    def post_setup(self, jacky_home: str, config: dict) -> None:
        ...
        from jacky_cli.memory_setup import _CANCELLED, _curses_select, _print_cancelled_setup, _prompt

        jacky_home_path = Path(jacky_home)
        env_path = jacky_home_path / ".env"
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        settings = _resolve_connection_settings(_load_jacky_openviking_config())
        ...
        try:
            ...
            if health_state == "unreachable":
                ...
                _emit_runtime_warning(
                    f"{health_message} OpenViking memory disabled for this Jacky run.",
                    warning_callback,
                )
                self._client = None
        except ImportError:
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Jacky' full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Jacky canonical messages into OpenViking batch payloads."""
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        and memory extraction never fires for it. See jacky-agent#28296.
    ...
    def _read_uri_payload(
        ...
        *,
        ...
        # (e.g. viking://user/jacky/.overview.md).

...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with jacky-agent)
2. User-installed providers: ``$JACKY_HOME/plugins/<name>/``
...
_USER_NAMESPACE = "_jacky_user_memory"


def _register_synthetic_package(name: str, search_locations: List[str]) -> None:
    """Register an empty package shell in sys.modules.

    User-installed providers import as ``_jacky_user_memory.<name>``, a
    ...
    ``ModuleNotFoundError: No module named '_jacky_user_memory'`` — the
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$JACKY_HOME/plugins/`` or None if unavailable."""
    try:
        from jacky_constants import get_jacky_home
        d = get_jacky_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$JACKY_HOME/plugins/<name>/``) directories.  Bundled takes
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    try:
        ...
        if module_name in sys.modules:
            ...
            if not _is_bundled:
                # cli.py imports as _jacky_user_memory.<name>.cli, usually

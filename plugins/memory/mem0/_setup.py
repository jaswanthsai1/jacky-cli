...
def _save_mem0_json(jacky_home: str, data: dict) -> None:
    """Merge-write to mem0.json."""
    config_path = Path(jacky_home) / "mem0.json"
    existing = {}
...
def _setup_platform(jacky_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    schema = [
        ...
        {"key": "user_id", "description": "User identifier", "default": "jacky-user"},
        {"key": "agent_id", "description": "Agent identifier", "default": "jacky"},
    ...
    # docs tell self-hosted users to put MEM0_HOST in ~/.jacky/.env. Warn
    ...
    if os.environ.get("MEM0_HOST", "").strip():
        print(
            ...
            "remove it from ~/.jacky/.env (or unset it) or Jacky will keep "
    ...
    provider.save_config(provider_config, jacky_home)

    if env_writes:
        _write_env(Path(jacky_home) / ".env", env_writes)
...
def _setup_selfhosted(jacky_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    user_id = flags.get("user_id") or _prompt(
        "User identifier", default=provider_config.get("user_id") or "jacky-user"
    )
    agent_id = _prompt("Agent identifier", default=provider_config.get("agent_id") or "jacky")
...
def _setup_oss(jacky_home: str, config: dict, flags: dict[str, str]) -> None:
    ...
    if not flags.get("_mode_from_flag"):
        _setup_oss_interactive(jacky_home, config)
        return
    ...
    user_id = flags.get("user_id") or os.getenv("USER", "jacky-user")
    ...
    if env_writes:
        _write_env(Path(jacky_home) / ".env", env_writes)
    _save_mem0_json(jacky_home, {"mode": "oss", "user_id": user_id, "agent_id": "jacky", "oss": oss_config})
...
def _prompt_api_key(label: str, env_var: str, jacky_home: str) -> str:
...
_PGVECTOR_CONTAINER = "jacky-pgvector"
_PGVECTOR_IMAGE = "pgvector/pgvector:pg17"
_PGVECTOR_PASSWORD = "jacky"
...
def _setup_oss_interactive(jacky_home: str, config: dict) -> None:
    ...
    if llm_def["needs_key"]:
        key = _prompt_api_key(llm_def["label"], llm_def["env_var"], jacky_home)
        if key:
    ...
    if embedder_def["needs_key"] and embedder_id != llm_id:
        key = _prompt_api_key(f"{embedder_def['label']} embedder", embedder_def["env_var"], jacky_home)
        if key:
    ...
    user_id = input(f"  User ID [{os.getenv('USER', 'jacky-user')}]: ").strip()
    user_id = user_id or os.getenv("USER", "jacky-user")

    agent_id = input("  Agent ID [jacky]: ").strip()
    agent_id = agent_id or "jacky"
    ...
    if env_writes:
        _write_env(Path(jacky_home) / ".env", env_writes)
    _save_mem0_json(jacky_home, {"mode": "oss", "user_id": user_id, "agent_id": agent_id, "oss": oss_config})
...
def post_setup(jacky_home: str, config: dict) -> None:
    """Entry point called by jacky memory setup framework.
    ...
    if flags["mode"] == "oss":
        ...
        _setup_oss(jacky_home, config, flags)
        return

    if flags["mode"] in ("selfhosted", "self-hosted"):
        _setup_selfhosted(jacky_home, config, flags)
        return

    if flags["mode"] == "platform":
        _setup_platform(jacky_home, config, flags)
        return

...
```bash
jacky memory setup    # select "mem0"
```
...
Behavioral settings live in `$JACKY_HOME/mem0.json` (set them via `jacky memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.jacky/.env`.
...
| `user_id` | `jacky-user` | User identifier on Mem0 |
...
2. Point the plugin at it — via the setup wizard:
   ...
   ```
3. Start a fresh Jacky session and call `mem0_search` — it connects to your server.
...
```bash
jacky memory setup mem0 --mode oss \
...
```bash
jacky memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$JACKY_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.jacky/mem0_qdrant"}}
  }
}
```
...
```bash
jacky memory setup mem0 --mode platform --api-key sk-...
```
...
- Check `user_id` matches between sessions (`$JACKY_HOME/mem0.json`).

"""
IRC Platform Adapter for Jacky Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Jacky agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        try:
            ...
            self.port = 6697
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "jacky-bot")
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Jacky Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Jacky Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: jacky_, jacky_1, jacky_2...
...
def check_requirements() -> bool:
    ...
    # jacky setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `jacky gateway setup` flow for the IRC platform.

    Lazy-imports ``jacky_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect Jacky to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #jacky — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.jacky/.env")
    print_info("Restart the gateway for changes to take effect: jacky gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "jacky-bot")
    ...
    nick_base = nickname.rstrip("_0123456789-")[:24] or "jacky-bot"
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Jacky Agent (cron)")

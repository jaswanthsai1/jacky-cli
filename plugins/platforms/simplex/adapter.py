"""SimpleX Chat platform adapter (Jacky plugin).
...
This adapter ships as a Jacky platform plugin under
``plugins/platforms/simplex/``. The Jacky plugin loader scans the
...
Optional environment variables:
    ...
    JACKY_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``jacky setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Jacky — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("JACKY_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``jacky setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.jacky/.env`` via
    ``jacky_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "jacky_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.jacky/.env"
        )
        return

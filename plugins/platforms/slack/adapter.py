...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `jacky slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Jacky does not currently need to route them into the agent loop.
            ...
            # /model work directly without /jacky prefix). A single regex
            ...
            # manifest (see `jacky slack manifest`). In Socket Mode, Slack
            ...
            from jacky_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/jacky$")

            @self._app.command(_slash_pattern)
            async def handle_jacky_command(ack, command):
            ...
            for _action_id in (
                "jacky_approve_once",
                "jacky_approve_session",
                "jacky_approve_always",
                "jacky_deny",
            ):
            ...
            for _action_id in (
                "jacky_confirm_once",
                "jacky_confirm_always",
                "jacky_confirm_cancel",
            ):
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Jacky handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Jacky session — matching the per-thread behavior channels already
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "jacky_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "jacky_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "jacky_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "jacky_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "jacky_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "jacky_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "jacky_confirm_once": "once",
            "jacky_confirm_always": "always",
            "jacky_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "jacky_approve_once": "once",
            "jacky_approve_session": "session",
            "jacky_approve_always": "always",
            "jacky_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/jacky <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/jacky
        ...
        if slash_name in {"jacky", ""}:
            # Legacy /jacky <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from jacky_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/jacky   ``).
        ...
        # "Running /cmd…" ack shown by handle_jacky_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/jacky <question>" must produce public replies so
...
# ``jacky_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``jacky_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under JACKY_HOME, and print
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Jacky",
                bot_description="Your Jacky agent on Slack",
            )
            ...
            print_info(
                "   Re-run `jacky slack manifest --write` anytime to refresh after "
                "Jacky adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `jacky update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``jacky_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'jacky-agent[slack]'",
        # Interactive setup wizard — replaces jacky_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in jacky_cli/gateway.py.

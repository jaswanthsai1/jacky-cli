"""
Email platform adapter for the Jacky gateway.

Allows users to interact with Jacky by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "jacky-agent" "version" "{_jacky_version}" '
            '"vendor" "Jacky" '
            '"support-email" "collegegamerff@gmail.com")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `jacky config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `jacky gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Jacky Agent")
        ...
        msg_id = f"<jacky-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# jacky_cli/gateway.py, and the _send_email dispatch in

...
# ``model_tools`` import time, so ``jacky status``, ``jacky chat``, etc.
...
# ``jacky_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Jacky is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a jacky MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${JACKY_HOME}/google_chat_thread_counts.json so
        ...
        self._thread_count_store = _ThreadCountStore(
            _jacky_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Jacky is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("JACKY_HOME", str(_Path.home() / ".jacky"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${JACKY_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a jacky MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Jacky is thinking…' marker message.
        ...
        "Jacky is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Jacky is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Jacky is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Jacky is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Jacky is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``jacky setup``.

    The setup wizard at ``jacky_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.jacky/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. jacky-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.jacky/.env")
    print_info("Restart the gateway: jacky gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``jacky cron`` running as a
    separate process from ``jacky gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Jacky plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'jacky-agent[google_chat]'",
        ...
        # the "Jacky is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Jacky is thinking…' marker message "

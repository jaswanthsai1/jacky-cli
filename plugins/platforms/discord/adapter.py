...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("JACKY_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid JACKY_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.jacky/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("JACKY_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("JACKY_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = env_float(
            "JACKY_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "JACKY_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Jacky gateway
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``JACKY_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("JACKY_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Jacky manages."""
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Jacky pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Jacky session")
        ...
        @tree.command(name="status", description="Show Jacky session status")
        ...
        @tree.command(name="stop", description="Stop the running Jacky agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.jacky/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Jacky Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Jacky gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Jacky session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Jacky in the thread",
        ...
        # jacky_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from jacky_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from jacky_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from jacky_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Jacky session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Jacky: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Jacky has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Jacky"
        if len(content) > 80:
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_jacky_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Jacky: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Jacky semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Jacky handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Jacky to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
    ...
    async def send_clarify(
        ...
        try:
            ...
            content = self._self_contained_prompt_content(
                "❓ **Jacky needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``jacky update --gateway``
        ...
        try:
            ...
            embed = discord.Embed(
                title=">_ Update Needs Your Input",
            ...
            # some clients (see send_exec_approval).
            content = self._self_contained_prompt_content(
                ">_ **Update Needs Your Input**", f"{prompt}{default_hint}"
            )
            msg = await channel.send(content=content, embed=embed, view=view)
            view._message = msg  # store for on_timeout expiration editing
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Jacky could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_jacky_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``jacky pairing approve``).
    ...
    # so users approved via ``jacky pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``jacky update`` prompts.
...
# is not in this process (e.g. ``jacky cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    from jacky_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("JACKY_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["JACKY_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("JACKY_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["JACKY_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``jacky_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import jacky_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    """Plugin entry point — called by the Jacky plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'jacky-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # jacky_cli/setup.py::_setup_discord function.  Same shape as Teams.

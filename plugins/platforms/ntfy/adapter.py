"""ntfy platform adapter (Jacky plugin).
...
``plugins/platforms/ntfy/``. The Jacky plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "jacky-in"              # subscribe topic (incoming)
          publish_topic: "jacky-out"     # optional — defaults to topic
...
_ECHO_TAG = "jacky-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``jacky cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Jacky dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `jacky gateway status` without

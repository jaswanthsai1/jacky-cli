...
Credential storage mirrors every other Jacky channel:

    * runtime SDK creds  -> ``~/.jacky/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.jacky/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a jacky dependency
...
# registers Jacky as its own client_id.
...
# Default name of the project Jacky provisions for the operator.
DEFAULT_PROJECT_NAME = "Jacky Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of jacky-agent.
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.jacky/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.jacky/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.jacky/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: jacky_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `jacky photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `jacky photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `jacky photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `jacky photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `jacky photon setup`)"

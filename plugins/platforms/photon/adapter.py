"""
Photon Spectrum (iMessage) platform adapter for Jacky Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Jacky dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `jacky photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `jacky setup` / status.
        return False
    return True
...
def _sidecar_deps_stale() -> bool:
    ...
    `jacky update` rewrites ``package-lock.json`` when the spectrum-ts pin is
...
def _reinstall_sidecar_deps() -> None:
    ...
    Mirrors ``jacky photon install-sidecar``: ``npm ci`` for an exact,
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Jacky defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Jacky-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Jacky checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `jacky photon setup`)"
            )
        # A `jacky update` that bumps the spectrum-ts pin rewrites
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Jacky-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Jacky plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `jacky photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: jacky photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `jacky gateway setup` alongside every other
    ...
    # Register CLI subcommands — `jacky photon ...`

...
def _is_allowed_bridge_path(url: str) -> bool:
    ...
    known Jacky media cache directory.
    ...
    ``<kind>_cache`` layout that ``get_jacky_dir`` may return.
    """
    try:
...
def check_whatsapp_requirements() -> bool:
    ...
    """
    # Prefer Jacky-managed Node/npm so Windows installs are not broken by a
...
class WhatsAppAdapter(WhatsAppBehaviorMixin, BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._session_path: Path = Path(config.extra.get(
            "session_path",
            get_jacky_dir("platforms/whatsapp/session", "whatsapp/session")
        ))
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if not check_whatsapp_requirements():
            ...
            self._set_fatal_error(
                ...
                "Node.js is not installed — install Node.js and re-run `jacky gateway`.",
                retryable=False,
            )
            return False
        ...
        if not creds_path.exists():
            logger.warning(
                ...
                "Pair from the dashboard or run `jacky whatsapp`; remove "
                "WHATSAPP_ENABLED from your .env to disable.",
                self.name, creds_path,
            )
            self._set_fatal_error(
                "whatsapp_not_paired",
                "WhatsApp enabled but not paired — pair from the dashboard or run `jacky whatsapp`.",
                retryable=False,
            )
            return False
        ...
        try:
            ...
            # `jacky update` bumps the Baileys pin).  The stamp file records
            # the package.json hash of the last successful install.
            bridge_dir = bridge_path.parent
            _pkg_json = bridge_dir / "package.json"
            _dep_stamp = bridge_dir / "node_modules" / ".jacky-pkg-hash"
            ...
            if not _deps_fresh:
                ...
                # Jacky-managed portable Node before falling back to PATH.
                    ...
                    async with session.get(
                        ...
                        if resp.status == 200:
                            ...
                            if bridge_status == "connected":
                                ...
                                # `jacky update`, so without this check it
            ...
            bridge_env = with_jacky_node_path()
            if self._reply_prefix is not None:
                bridge_env["WHATSAPP_REPLY_PREFIX"] = self._reply_prefix
            # Pass the profile-aware cache directories so the bridge writes
            # media where the Python side reads it.  Without these the bridge
            # hardcodes ~/.jacky/{image,audio,document}_cache, which diverges
            # under JACKY_HOME overrides, profiles, and the new cache/ layout.
            from gateway.platforms.base import (
                get_audio_cache_dir as _get_audio_dir,
                get_document_cache_dir as _get_doc_dir,
                get_image_cache_dir as _get_img_dir,
            )
            bridge_env["JACKY_IMAGE_CACHE_DIR"] = str(_get_img_dir())
            bridge_env["JACKY_AUDIO_CACHE_DIR"] = str(_get_audio_dir())
            bridge_env["JACKY_DOCUMENT_CACHE_DIR"] = str(_get_doc_dir())
            ...
            if data.get("status") != "connected":
                ...
                for attempt in range(15):
                    ...
                    print(f"[{self.name}]   If session expired, re-pair: jacky whatsapp")
...
# dict in jacky_cli/gateway.py, and the _send_whatsapp dispatch in
...
def interactive_setup() -> None:
    ...
    Replaces the central _setup_whatsapp in jacky_cli/gateway.py and the
...
def _is_connected(config) -> bool:
    ...
    would make WhatsApp always show as "configured" in ``jacky setup`` even
    ...
    if config is not None and getattr(config, "enabled", False) and extra:
        ...
        return True
    # Read via jacky_cli.gateway.get_env_value (not os.getenv) so setup-status

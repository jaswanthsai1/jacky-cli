...
Let the jacky agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where jacky runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `jacky meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `jacky meet` CLI |
...
| `cli.py` | `jacky meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$JACKY_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `jacky meet node {run,list,approve,remove,status,ping}` |
...
jacky meet install                                      # pip + Chromium
jacky meet setup                                        # preflight
jacky meet auth                                         # optional
jacky meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
jacky meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.jacky/.env
jacky meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
jacky meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
jacky meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.jacky/.env
jacky meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, jacky will **not** switch your system audio input automatically — the
...
jacky plugins enable google_meet
jacky meet node run --display-name my-mac --host 0.0.0.0 --port 18789

# Jacky Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: jacky-achievements plugin
...
### `GET /api/plugins/jacky-achievements/achievements`
...
### `POST /api/plugins/jacky-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/jacky-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/jacky-achievements/overview`
...
- Persist plugin data under:
  - `~/.jacky/plugins/jacky-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.jacky/plugins/jacky-achievements/scan_checkpoint.json`
...
- `~/.jacky/plugins/jacky-achievements/`

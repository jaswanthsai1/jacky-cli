/**
 * Jacky Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.jacky/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__JACKY_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__JACKY_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "jacky-kanban-dot-triage",
    todo: "jacky-kanban-dot-todo",
    ready: "jacky-kanban-dot-ready",
    running: "jacky-kanban-dot-running",
    blocked: "jacky-kanban-dot-blocked",
    done: "jacky-kanban-dot-done",
    archived: "jacky-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-jacky-task";
  ...
  const DOCS_URL = "https://jaswanthsai1.github.io/jacky-cli/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://jaswanthsai1.github.io/jacky-cli/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "jacky.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="jacky-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])jacky-kanban-md-code\1/i.test(attrs)) {
      return ' class="jacky-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "jacky-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "jacky-kanban-md",
  ...
  // standard `drop` event and our `jacky-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("jacky-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("jacky-kanban-column--drop");
          if (target) target.classList.add("jacky-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("jacky-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("jacky-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("jacky-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__JACKY_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    return h(ErrorBoundary, null,
      h("div", { className: "jacky-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "jacky-kanban-attention",
        "jacky-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "jacky-kanban-attention-bar" },
        h("span", { className: "jacky-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "jacky-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "jacky-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "jacky-kanban-attention-row",
                  "jacky-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "jacky-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "jacky-kanban-attention-row-id" }, task.id),
                h("span", { className: "jacky-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "jacky-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "jacky-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "jacky-kanban-diag-action-btn",
      action.suggested ? "jacky-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " jacky-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".jacky-kanban-drawer-comment-row input, .jacky-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "jacky-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("jacky-kanban-diag", sevClass) },
      h("div", { className: "jacky-kanban-diag-header" },
        h("span", { className: "jacky-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "jacky-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "jacky-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "jacky-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "jacky-kanban-diag-data-row" },
                  h("span", { className: "jacky-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "jacky-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "jacky-kanban-diag-data-row" },
                h("span", { className: "jacky-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "jacky-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "jacky-kanban-diag-reassign-row" },
            h("span", { className: "jacky-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "jacky-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "jacky-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "jacky-kanban-diag-msg",
              msg.ok ? "jacky-kanban-diag-msg--ok" : "jacky-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "jacky-kanban-section" },
      h("div", { className: "jacky-kanban-section-head-row" },
        h("span", { className: "jacky-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "jacky-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "jacky-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "jacky-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "jacky-kanban-docs-link",
      title: "Open Jacky Kanban docs in a new tab",
      "aria-label": "Jacky Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `jacky kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "jacky-kanban-msg-ok" : "jacky-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "jacky-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "jacky-kanban-boardswitcher" },
      h("div", { className: "jacky-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "jacky-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "jacky-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "jacky-kanban-dialog-title" },
        ...
        h("div", { className: "jacky-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Jacky profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "jacky-kanban-bulk" },
      h("span", { className: "jacky-kanban-bulk-count" },
      ...
      h("div", { className: "jacky-kanban-bulk-priority",
      ...
      h("div", { className: "jacky-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Jacky profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "jacky-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("jacky-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("jacky-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "jacky-kanban-trash",
        dragOver ? "jacky-kanban-trash--drop" : "",
        props.draggingTaskId ? "jacky-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "jacky-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "jacky-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".jacky-kanban-card")) return true;
      if (target.closest && target.closest(".jacky-kanban-column-add")) return true;
      if (target.closest && target.closest(".jacky-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("jacky-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("jacky-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".jacky-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "jacky-kanban-columns",
        isScrollable ? "jacky-kanban-columns--scrollable" : "",
        isPanning ? "jacky-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("jacky-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("jacky-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "jacky-kanban-column",
        dragOver ? "jacky-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "jacky-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "jacky-kanban-col-check",
        ...
        h("span", { className: cn("jacky-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "jacky-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "jacky-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "jacky-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "jacky-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "jacky-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "jacky-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "jacky-kanban-lane" },
                  h("div", { className: "jacky-kanban-lane-head" },
                    h("span", { className: "jacky-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "jacky-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "jacky-kanban-card--stale-red";
    if (age >= tier.amber) return "jacky-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".jacky-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "jacky-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      className: cn(
        "jacky-kanban-card",
        props.selected ? "jacky-kanban-card--selected" : "",
        props.failed ? "jacky-kanban-card--failed" : "",
        props.draggingSource ? "jacky-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "jacky-kanban-card-content" },
          h("div", { className: "jacky-kanban-card-row" },
            h("label", {
              className: "jacky-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "jacky-kanban-card-check",
            ...
            h("span", { className: "jacky-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or jacky kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "jacky-kanban-warning-badge",
                    "jacky-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "jacky-kanban-needs-assignee",
          ...
          h("div", { className: "jacky-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "jacky-kanban-card-row jacky-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "jacky-kanban-assignee",
                            title: `Assigned to Jacky profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "jacky-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "jacky-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "jacky-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    return h("div", { className: "jacky-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Jacky profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Jacky profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "jacky-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "jacky-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "jacky-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "jacky-kanban-drawer-close",
        ...
        data ? h("div", { className: "jacky-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "jacky-kanban-section" },
      h("div", { className: "jacky-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "jacky-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    return h("div", { className: "jacky-kanban-drawer-body" },
      h("div", { className: "jacky-kanban-drawer-title" },
        h("span", { className: cn("jacky-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "jacky-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "jacky-kanban-drawer-meta" },
      ...
      t.result ? h("div", { className: "jacky-kanban-section" },
        h("div", { className: "jacky-kanban-section-head" }, tx(i18n, "result", "Result")),
      ...
      h("div", { className: "jacky-kanban-section" },
        h("div", { className: "jacky-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "jacky-kanban-comment" },
                h("div", { className: "jacky-kanban-comment-head" },
                  h("span", { className: "jacky-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "jacky-kanban-comment-ago" },
      ...
      h("div", { className: "jacky-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            ...
            className: cn(
              "jacky-kanban-event",
              isDiag ? "jacky-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "jacky-kanban-event-header" },
                  h("span", { className: "jacky-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "jacky-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "jacky-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "jacky-kanban-event-header-plain" },
                  h("span", { className: "jacky-kanban-event-kind" }, e.kind),
                  h("span", { className: "jacky-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "jacky-kanban-event-phantom-row" },
                  h("span", { className: "jacky-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "jacky-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "jacky-kanban-event-payload" },
    ...
    return h("div", { className: "jacky-kanban-section" },
      h("div", { className: "jacky-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "jacky-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `jacky-kanban-run--${r.outcome || r.status || "ended"}`
          : "jacky-kanban-run--active";
        return h("div", { key: r.id, className: cn("jacky-kanban-run", outcomeClass) },
          h("div", { className: "jacky-kanban-run-head" },
            h("span", { className: "jacky-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "jacky-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "jacky-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "jacky-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "jacky-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "jacky-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "jacky-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "jacky-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "jacky-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "jacky-kanban-pre jacky-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "jacky-kanban-meta-row" },
      h("span", { className: "jacky-kanban-meta-label" }, props.label),
      h("span", { className: "jacky-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "jacky-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "jacky-kanban-meta-row" },
        h("span", { className: "jacky-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "jacky-kanban-meta-value jacky-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "jacky-kanban-meta-row" },
        h("span", { className: "jacky-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "jacky-kanban-meta-value jacky-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "jacky-kanban-section" },
      h("div", { className: "jacky-kanban-section-head-row" },
        h("span", { className: "jacky-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "jacky-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "jacky-kanban-section" },
      h("div", { className: "jacky-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "jacky-kanban-deps-row" },
        h("span", { className: "jacky-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "jacky-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "jacky-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "jacky-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "jacky-kanban-dep-chip-x",
      ...
      h("div", { className: "jacky-kanban-deps-row" },
        h("span", { className: "jacky-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "jacky-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "jacky-kanban-actions" },
        specifyButton,
        decomposeButton,
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "jacky-kanban-section" },
      ...
      h("div", { className: "jacky-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "jacky-kanban-home-sub jacky-kanban-home-sub--on"
              : "jacky-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__JACKY_PLUGINS__ && typeof window.__JACKY_PLUGINS__.register === "function") {
    window.__JACKY_PLUGINS__.register("kanban", KanbanPage);
  }
})();

...
Auto-tracks and cleans up ephemeral files created during Jacky Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$JACKY_HOME` and `/tmp/jacky-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `JACKY_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `JACKY_HOME` or `/tmp/jacky-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$JACKY_HOME/disk-cleanup/` is itself excluded
- `$JACKY_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,

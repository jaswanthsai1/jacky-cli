...
A standards-compliant OpenID Connect Relying Party for the ``jacky dashboard``
...
:class:`~jacky_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``jacky_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~jacky_cli.dashboard_auth.Session`.
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.jacky/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    JACKY_DASHBOARD_OIDC_ISSUER
    JACKY_DASHBOARD_OIDC_CLIENT_ID
    JACKY_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    JACKY_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def start_login(self, *, redirect_uri: str) -> LoginStart:
        ...
        cookie_payload = {
            "jacky_session_pkce": f"state={state};verifier={code_verifier}",
        }
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Jacky does not call
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``JACKY_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "JACKY_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "JACKY_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("JACKY_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.jacky/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "JACKY_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(JACKY_DASHBOARD_OIDC_ISSUER + JACKY_DASHBOARD_OIDC_CLIENT_ID) "

...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a single ``SKILL.md`` via ``skill_manage`` that follows the Jacky
     skill-authoring standards (description <=60 chars, the modern section
     order, Jacky-tool framing, no invented commands).
...
# (HARDLINE)" and the jacky-agent-dev new-skill salvage reference. Embedded in
# the prompt so the agent authors skills the way a maintainer would by hand.
_AUTHORING_STANDARDS = """\
Follow the Jacky skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Jacky`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Jacky.
- platforms: declare `[macos]`, `[linux]`, and/or `[windows]` IF the skill
  ...
  skills.
- metadata.jacky.tags: a few Capitalized, Relevant, Tags.
...
4. "## How to Run" — the canonical invocation, framed through Jacky tools.
...
Jacky-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Jacky tools by name in backticks: `terminal`, `read_file`,

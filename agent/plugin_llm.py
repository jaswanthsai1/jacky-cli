...
Plugins built on Jacky Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Jacky
...
:class:`~jacky_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Jacky' main config
...
handles every provider, fallback chain, and per-task override Jacky
supports.
"""
...
class PluginLlm:
    ...
    Instances are constructed by :class:`jacky_cli.plugins.PluginContext`

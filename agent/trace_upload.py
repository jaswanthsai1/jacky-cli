"""Upload a Jacky session transcript to Hugging Face as an agent trace.

Jacky stores sessions in its own SQLite store (``jacky_state.SessionDB``),
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``jacky trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Jacky' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "jacky-traces"
_JACKY_VERSION = "jacky-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Jacky OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Jacky' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Jacky conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.jacky/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `jacky trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/jacky-traces`` dataset. Returns a

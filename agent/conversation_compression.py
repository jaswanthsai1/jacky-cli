    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `jacky setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Jacky "
...
def compress_context(
    ...
    *,
    ...
    # Jacky' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|jacky|off).
    ...
    # long-lived ``jacky_state.SessionDB`` class still bound to the
        ...
        try:
            ...
            if getattr(agent, "_last_compression_lock_error_sid", None) != _lock_sid:
                ...
                logger.warning(
                    ...
                    "process (or `jacky update`) to resync.",
    ...
    try:
        ...
        if agent._session_db:
            try:
                ...
                if in_place:
                    ...
                    # from ``jacky_logging._session_context`` (set once per turn in
                    ...
                    try:
                        agent._session_db.create_session(
                            ...
                            source=agent.platform or os.environ.get("JACKY_SESSION_SOURCE", "cli"),
                            model=agent.model,
                    ...
                    try:
                        from jacky_cli.goals import migrate_goal_to_session
        ...
        # engines (e.g. jacky-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See jacky-lcm#68. Built-in ContextCompressor
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    Jacky' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual Codex app-server thread context. For this
    runtime, ask Codex to compact its own thread and keep Jacky' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "jacky", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "jacky":
        logger.info(
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="jacky_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()

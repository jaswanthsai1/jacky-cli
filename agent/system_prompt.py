...
upstream prefix cache warm.  See ``jacky-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``JACKY_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("JACKY_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Jacky never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want JACKY_HOME persona while keeping
    ...
    # Pointer to the jacky-agent skill + docs for user questions about Jacky itself.
    stable_parts.append(JACKY_AGENT_HELP_GUIDANCE)
    ...
    # JACKY_KANBAN_TASK env var). Normal chat sessions never see
    ...
    # Coding posture (base Jacky, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Jacky profile the agent is running
    # under so it doesn't conflate ~/.jacky/skills/ (default profile) with
    # ~/.jacky/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Jacky profile: default. Other profiles (if any) live "
            "under ~/.jacky/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Jacky profile: {active_profile}. This session reads "
            f"and writes ~/.jacky/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.jacky/skills/, ~/.jacky/plugins/, "
            f"~/.jacky/cron/, ~/.jacky/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Jacky never rebuilds or reinjects parts of it

...
#
# When a user invokes a /skill (or /bundle), Jacky expands the turn into a
...
# tests/openviking_plugin/test_openviking.py::test_skill_markers_match_jacky_scaffolding.
# ---------------------------------------------------------------------------
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``JACKY_PLATFORM`` env var and
    ``JACKY_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = os.getenv("JACKY_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.jacky.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_jacky_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.jacky/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.jacky/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``jacky -s <skill>`` or
    a deployment's ``JACKY_TUI_SKILLS`` env var could force-load a skill an

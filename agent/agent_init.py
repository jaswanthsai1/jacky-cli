...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"summarizing.\n"
        f"  Opt back out: jacky config set compression.codex_gpt55_autoraise false"
    )
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$JACKY_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_jacky_home() / ".codex_gpt55_autoraise_notice"
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$JACKY_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .jacky.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / JACKY_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.jacky/SOUL.md as the primary
    ...
    agent._cache_ttl = "5m"
    try:
        from jacky_cli.config import load_config as _load_pc_cfg
    ...
    # report cumulative micros spent.  Surfaced behind JACKY_DEV_CREDITS.
    ...
    if agent.verbose_logging:
        ...
        # on the fact that jacky_logging.setup_logging() does not
        ...
        # handler level inside jacky_logging.py, not here.
        pass
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from jacky_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from jacky_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `jacky model` to "
                        "select a provider, or run `jacky setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff JACKY_KANBAN_TASK is set).
    ...
    # Session logs go into ~/.jacky/sessions/ alongside gateway sessions
    jacky_home = get_jacky_home()
    agent.logs_dir = jacky_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.jacky/sessions/session_{sid}.json)
    ...
    try:
        from jacky_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from jacky_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "jacky"
                    except Exception:
                        pass
    ...
    # Lets an enterprise admin append to or replace Jacky' built-in
    ...
    if codex_app_server_auto_compaction not in {"native", "jacky", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, jacky, off.",
    ...
    # The top-level `model:` block is the STICKY DEFAULT provider/model
    # (e.g. the local ollama model used when no --provider/--model flag is
    # given). Its context_length must only apply when that default is
    # actually the one active this session — otherwise it silently leaks
    # into every other provider/model (e.g. a cloud custom_provider chosen
    # via --provider/--model), clobbering that provider's real window with
    # whatever number happens to be cached for the sticky default.
    if isinstance(_model_cfg, dict):
        _model_cfg_base_url = str(_model_cfg.get("base_url") or "").rstrip("/")
        _model_cfg_default = str(_model_cfg.get("default") or "")
        _active_base_url = str(getattr(agent, "base_url", "") or "").rstrip("/")
        _active_model = str(getattr(agent, "model", "") or "")
        _model_cfg_is_active = (
            (not _model_cfg_base_url or _model_cfg_base_url == _active_base_url)
            and (not _model_cfg_default or _model_cfg_default == _active_model)
        )
        _config_context_length = _model_cfg.get("context_length") if _model_cfg_is_active else None
    else:
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from jacky_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from jacky_cli.plugins import get_plugin_context_engine
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Jacky Agent.  Choose a model with at least "
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from jacky_cli.model_switch import _check_jacky_model_warning

            _jacky_warn = _check_jacky_model_warning(agent.model or "")
            if _jacky_warn:
                _user_msg = (
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_jacky_warn)
        except Exception:
            pass

...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Jacky-Session-Id header), and an
        ...
        if env_var_enabled("JACKY_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from jacky_cli.config import load_config as _load_moa_cfg
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to jacky's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from jacky_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    #
    # We do NOT drop the call: jacky' own dispatch loop intentionally keeps an

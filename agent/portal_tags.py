...
Every Jacky request that hits the Nous Portal — main agent loop, auxiliary
client (compression / titles / vision / web_extract / session_search / etc.),
and any future code path — must carry the same product-attribution tags so
Nous can attribute usage to Jacky Agent and bucket it by client release.

Tag shape (sent in OpenAI-compatible ``extra_body['tags']``):

    [
        "product=jacky-agent",
        "client=jacky-client-v<__version__>",
    ]

The version is sourced live from ``jacky_cli.__version__`` so it auto-aligns
...
``jacky_cli.__version__`` is the canonical source of truth.
"""
...
def _jacky_version() -> str:
    """Return the current Jacky release version, e.g. ``"0.13.0"``.

    Falls back to ``"unknown"`` if ``jacky_cli`` cannot be imported (should
...
def jacky_client_tag() -> str:
    """Return the ``client=...`` tag for Nous Portal requests.

    Format: ``client=jacky-client-v<MAJOR>.<MINOR>.<PATCH>``.
    """
    return f"client=jacky-client-v{_jacky_version()}"
...
def nous_portal_tags() -> List[str]:
    ...
    """
    return ["product=jacky-agent", jacky_client_tag()]

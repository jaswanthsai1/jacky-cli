...
def _detect_environment(env: str) -> bool:
    ...
    if env == "kanban":
        ...
        # dispatcher sets ``JACKY_KANBAN_TASK`` / ``JACKY_KANBAN_BOARD`` in the
        ...
        # tool availability.
        if os.getenv("JACKY_KANBAN_TASK") or os.getenv("JACKY_KANBAN_BOARD"):
            result = True
        else:
            try:
        ...
        # The Jacky Docker image runs s6-overlay as PID 1 (/init). s6 plants
...
def _load_raw_config() -> Dict[str, Any]:
    ...
    This module intentionally avoids importing ``jacky_cli.config`` on the
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``JACKY_PLATFORM`` or
            ``JACKY_SESSION_PLATFORM`` env vars.  Returns the global
    ...
    resolved_platform = (
        platform
        or os.getenv("JACKY_PLATFORM")
        or get_session_env("JACKY_SESSION_PLATFORM")
    )
...
# which becomes the dominant cost of ``jacky`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.jacky/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``jacky`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against JACKY_HOME, not cwd
        if not p.is_absolute():
            p = (jacky_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.jacky/skills/`` first, then external.
...
def normalize_skill_lookup_name(identifier: str) -> str:
    ...
    under ``~/.jacky/skills/`` (including via symlinks) or configured
    ...
    # ~/.jacky/skills/<name> where <name> is a symlink to a checked-out
...
def is_external_skill_path(path) -> bool:
    ...
    ``skills.external_dirs`` are externally owned: Jacky can discover and view
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    jacky = metadata.get("jacky") or {}
    if not isinstance(jacky, dict):
        jacky = {}
    return {
        "fallback_for_toolsets": jacky.get("fallback_for_toolsets", []),
        "requires_toolsets": jacky.get("requires_toolsets", []),
        "fallback_for_tools": jacky.get("fallback_for_tools", []),
        "requires_tools": jacky.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    jacky = metadata.get("jacky")
    if not isinstance(jacky, dict):
        return []
    raw = jacky.get("config")
    if not raw:
        return []
...
def iter_skill_index_files(skills_dir: Path, filename: str):
    ...
    Excludes Jacky metadata, VCS, virtualenv/dependency, cache, and skill

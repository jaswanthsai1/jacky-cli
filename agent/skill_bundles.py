...
Storage
-------
Bundles live in ``~/.jacky/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``JACKY_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``jacky bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under JACKY_HOME.

    Honors ``JACKY_BUNDLES_DIR`` for tests; falls back to
    ``<JACKY_HOME>/skill-bundles``.
    """
    override = os.environ.get("JACKY_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_jacky_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `jacky bundles` CLI subcommand.
# ---------------------------------------------------------------------------

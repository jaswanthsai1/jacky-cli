...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the JACKY_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Jacky runtime / deployment
    "JACKY_HOME", "JACKY_PROFILE", "JACKY_GATEWAY_LOCK_DIR",
    "JACKY_MAX_ITERATIONS", "JACKY_MAX_TOKENS", "JACKY_API_TIMEOUT",
    "JACKY_REDACT_SECRETS", "JACKY_NOUS_TIMEOUT_SECONDS",
    "_JACKY_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "JACKY_KANBAN_DB", "JACKY_KANBAN_WORKSPACES_ROOT", "JACKY_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "JACKY_KANBAN_",
    "JACKY_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(jacky_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(jacky_home) / ".env")

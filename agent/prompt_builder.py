...
_JACKY_MD_NAMES = (".jacky.md", "JACKY.md")


def _find_jacky_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.jacky.md`` or ``JACKY.md``.
    ...
    # could pick up a .jacky.md planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _JACKY_MD_NAMES:
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.jacky/kanban.db`. Your task id is in "
    "`$JACKY_KANBAN_TASK`; your workspace is `$JACKY_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $JACKY_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $JACKY_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${JACKY_KANBAN_BRANCH:-wt/$JACKY_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$JACKY_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`jacky profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `jacky kanban <verb>` for board operations. Use "
...
#
# The jacky-agent runtime already executes a batch of tool calls
...
# from Cline's TypeScript tool-surface guidance to jacky-agent's Python
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "run `jacky computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Jacky "
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Jacky terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Jacky desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Jacky WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Jacky itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Jacky runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Jacky itself is running. The host OS, home, and cwd "
                f"of the Jacky process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Jacky "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Jacky
    ...
    extra = (os.getenv("JACKY_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_jacky_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("JACKY_PLATFORM") or os.environ.get("JACKY_SESSION_PLATFORM")
    if platform:
        return platform
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.jacky/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Jacky Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `jacky-agent` skill "
            "first. It has the actual commands (e.g. `jacky config set …`, `jacky tools`, "
            "`jacky setup`) so you don't have to guess or invent workarounds.\n"
...
def build_nous_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    lines.extend(
        [
            ...
            "Useful commands: jacky setup, jacky setup tools, jacky setup terminal, jacky status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from JACKY_HOME and return its content, or None.
    ...
    """
    try:
        from jacky_cli.config import ensure_jacky_home
        ensure_jacky_home()
    except Exception as e:
        logger.debug("Could not ensure JACKY_HOME before loading SOUL.md: %s", e)

    soul_path = get_jacky_home() / "SOUL.md"
...
def _load_jacky_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.jacky.md / JACKY.md — walk to git root."""
    jacky_md_path = _find_jacky_md(cwd_path)
    if not jacky_md_path:
        return ""
    try:
        content = jacky_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = jacky_md_path.name
        try:
            rel = str(jacky_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, ".jacky.md", context_length=context_length,
            read_path=str(jacky_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", jacky_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .jacky.md / JACKY.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from JACKY_HOME is independent and always included when present.
    ...
    project_context = (
        _load_jacky_md(cwd_path, context_length)
    ...
    # SOUL.md from JACKY_HOME only — skip when already loaded as identity
    if not skip_soul:

...
- **skills** → the skill name (e.g. ``"debugging-jacky-desktop"``)
...
shared by the CLI (``jacky journey delete|edit``), the TUI ``/journey`` overlay
(gateway RPCs), and the desktop GUI (REST). Deleting a skill *archives* it
(recoverable via ``jacky curator restore``); deleting a memory rewrites its
...
def _memories_dir() -> Path:
    ...
    return get_jacky_home() / "memories"
...
def _delete_skill(name: str) -> dict[str, Any]:
    ...
    if skill_usage.get_record(name).get("pinned"):
        return {"ok": False, "message": f"'{name}' is pinned — unpin it first (jacky curator unpin {name})"}

    ok, message = skill_usage.archive_skill(name)
    if ok:
        _clear_skill_cache()

    return {"ok": ok, "message": f"archived '{name}' — restore with: jacky curator restore {name}" if ok else message}

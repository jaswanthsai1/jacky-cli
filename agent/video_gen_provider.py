...
as ``kind: backend``) or ``~/.jacky/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``jacky tools``
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$JACKY_HOME/cache/videos/``.

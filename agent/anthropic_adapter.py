"""Anthropic Messages API adapter for Jacky Agent.

Translates between Jacky's internal OpenAI-style message format and
...
from jacky_cli.utils import base_url_host_matches, normalize_proxy_env_vars
...
# Jacky effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _build_anthropic_client_with_bearer_hook(
    ...
    *,
    ...
    kwargs = {
        ...
        # Delegate retry to jacky's outer loop (honors Retry-After); the SDK
...
def build_anthropic_client(
    ...
    * a ``Callable[[], str]`` — an Entra ID bearer token provider from
    ...
    kwargs = {
        ...
        # Delegate all rate-limit / 5xx retry to jacky's outer conversation
...
def _refresh_oauth_token(creds: Dict[str, Any]) -> Optional[str]:
    ...
    Jacky notices an expired token, Claude Code may have already rotated it.
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Jacky historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def _resolve_anthropic_pool_token() -> Optional[str]:
    ...
    ``account_usage`` and ``jacky models``) never mutates ``~/.jacky/auth.json``
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Jacky)
      2. CLAUDE_CODE_OAUTH_TOKEN env var
      3. Claude Code credentials (~/.claude.json or ~/.claude/.credentials.json)
         — with automatic refresh if expired and a refresh token is available
      4. Anthropic credential_pool OAuth entry (~/.jacky/auth.json)
    ...
    # 1. Jacky-managed OAuth/setup token env var
    ...
    # 4. Jacky credential_pool OAuth entry.
    resolved_pool_token = _resolve_anthropic_pool_token()
    if resolved_pool_token:
        return resolved_pool_token

    # 5. Regular API key, or a legacy OAuth token saved in ANTHROPIC_API_KEY.
    # This remains as a compatibility fallback for pre-migration Jacky configs.
...
# ── Jacky-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.jacky/.anthropic_oauth.json (our own file).
...
def _get_jacky_oauth_file() -> Path:
    return get_jacky_home() / ".anthropic_oauth.json"
...
def run_jacky_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Jacky-native OAuth PKCE flow and return credential state."""
    import secrets
    import time
    import webbrowser
    ...
    print()
    print("Authorize Jacky with your Claude Pro/Max subscription.")
    print()
    ...
    try:
        from jacky_cli.auth import _can_open_graphical_browser as _can_open_gui
    except Exception:
...
def read_jacky_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read Jacky-managed OAuth credentials from ~/.jacky/.anthropic_oauth.json."""
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Jacky OAuth credentials: %s", e)
    return None
...
def _convert_assistant_message(m: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # can validate the message history.  See jacky-agent#13848.
    #
...
def _strip_orphaned_tool_blocks(result: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(result):
        ...
        # replaying it verbatim.  See jacky-agent: extended-thinking + parallel
...
def _manage_thinking_signatures(
    ...
    replayed assistant tool-call messages.  See jacky-agent#13848 (Kimi) and
    jacky-agent#16748 (DeepSeek).
...
def build_anthropic_kwargs(
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Jacky Agent", "Claude Code")
                text = text.replace("Jacky agent", "Claude Code")
                text = text.replace("jacky-agent", "claude-code")
        ...
        #      a) bare Jacky-native tools (``read_file``)  -> ``mcp__read_file``
    ...
    # silently hides reasoning text that Jacky surfaces in its CLI. We

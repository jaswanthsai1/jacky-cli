...
``kind: backend``) or ``~/.jacky/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
def get_provider_env(name: str) -> str:
    ...
    Resolves *name* via :func:`jacky_cli.config.get_env_value` (checks
    ``os.environ`` first, then ``~/.jacky/.env``) so credentials set
    through Jacky' config layer are visible even when they were never
...
class WebSearchProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``jacky tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``jacky tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``jacky tools`` picker.

        Used by ``jacky_cli/tools_config.py`` to inject this provider as a

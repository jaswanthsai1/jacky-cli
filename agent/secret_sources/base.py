...
``~/.jacky/.env`` has loaded and BEFORE the rest of Jacky reads
``os.environ``.
...
* **Startup-time, synchronous.**  ``fetch()`` is called once per process
  (per JACKY_HOME) by the orchestrator in
...
class ErrorKind(str, Enum):
    ...
    A fixed vocabulary keeps startup warnings and ``jacky secrets status``
...
class SecretSource(ABC):
    ...
    Attributes:
        ...
        label: Human-readable name used in startup messages and
            ``jacky secrets status`` (e.g. ``"Bitwarden Secrets Manager"``).
    ...
    def fetch(self, cfg: dict, home_path: Path) -> FetchResult:
        ...
        may be malformed.  ``home_path`` is the resolved JACKY_HOME.
        """
...
# codes that must not reach Jacky' own startup output.
...
def run_secret_cli(
    ...
    * The child gets ``PATH``/``HOME``/locale basics plus only the env
      ...
      this point holds every credential Jacky knows about.
    * ``NO_COLOR=1`` is set and stderr/stdout are ANSI-scrubbed so
      helper diagnostics can't smuggle escape sequences into Jacky
      output.

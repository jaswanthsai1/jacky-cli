...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Jacky never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Jacky continues with whatever credentials ``.env`` already had.
...
are cached in-process and on disk under ``<jacky_home>/cache/op_cache.json``
so back-to-back short-lived ``jacky`` invocations don't re-shell ``op`` for
...
# In-process cache.  The key folds in str(home_path) so a JACKY_HOME switch
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Jacky to source it from, so normalize to that name here.
    if token_value:
...
def apply_onepassword_secrets(
    *,
    ...
    Called by ``load_jacky_dotenv()`` after the .env files have loaded.

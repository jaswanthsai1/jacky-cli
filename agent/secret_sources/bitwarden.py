...
Jacky pulls API keys from Bitwarden Secrets Manager at process startup
so they don't have to live in plaintext in ``~/.jacky/.env``.

Design summary
--------------

* The ``bws`` binary is auto-installed into ``<jacky_home>/bin/bws`` on
  first use.  Jacky pins one version (``_BWS_VERSION``) and downloads
...
* The access token is stored in ``~/.jacky/.env`` as
...
* Pulling secrets is a single ``bws secret list <project_id>
  ...
  ``cache_ttl_seconds`` so back-to-back ``jacky`` invocations don't
  hammer the API.
* Failures NEVER block Jacky startup.  Missing binary, no network,
...
# In-process cache so repeated load_jacky_dotenv() calls (CLI startup,
# gateway hot-reload, test suites) don't re-fetch from BSM.
_CacheKey = Tuple[str, str, str]  # (access_token_fingerprint, project_id, server_url)
_CACHE: Dict[_CacheKey, _CachedFetch] = {}

# Disk-persisted cache so back-to-back CLI invocations (e.g. `jacky chat -q ...`
...
# <jacky_home>/cache/bws_cache.json. The file holds only the secret VALUES,
# never the access token. It's plaintext-equivalent to ~/.jacky/.env (which
...
def _disk_cache_path(home_path: Optional[Path] = None) -> Path:
    """Return the disk cache path under jacky_home/cache/.

    Thin wrapper over the shared DiskCache, kept for tests and any direct
    callers; falls back to `$JACKY_HOME` / `~/.jacky` when home is None.
    """
...
def find_bws(*, install_if_missing: bool = False) -> Optional[Path]:
    ...
    Resolution order:
      1. ``<jacky_home>/bin/bws``  (our managed copy — preferred)
    ...
    """
    managed = _jacky_bin_dir() / _platform_binary_name()
...
def install_bws(*, force: bool = False) -> Path:
    ...
    path catch these; the user-facing ``jacky secrets bitwarden setup``
    surface lets them propagate so the wizard can show a clear error.
    """
    bin_dir = _jacky_bin_dir()
    ...
    with tempfile.TemporaryDirectory(prefix="jacky-bws-") as tmpdir:
        tmp = Path(tmpdir)
...
def _http_download(url: str, dest: Path) -> None:
    req = urllib.request.Request(url, headers={"User-Agent": "jacky-agent"})
    try:
...
def fetch_bitwarden_secrets(
    *,
    ...
    ``<jacky_home>/cache/bws_cache.json`` (for back-to-back CLI invocations).
...
# ---------------------------------------------------------------------------
# Public entry point — called from jacky_cli.env_loader
# ---------------------------------------------------------------------------
...
def apply_bitwarden_secrets(
    *,
    ...
    This is the function ``load_jacky_dotenv()`` calls after the .env
    ...
    if not access_token:
        result.error = (
            ...
            "not set.  Run `jacky secrets bitwarden setup`."
        )
        return result
    ...
    if not project_id:
        result.error = (
            ...
            "Run `jacky secrets bitwarden setup`."
        )
        return result
    ...
    if binary is None:
        result.error = (
            ...
            "Run `jacky secrets bitwarden setup` to install."
        )
        return result

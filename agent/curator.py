...
def _state_file() -> Path:
    return get_jacky_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.jacky/config.yaml. Tolerates missing file."""
    try:
...
def get_prune_builtins() -> bool:
    ...
    suppression list keeping them archived across `jacky update` re-seeds.
...
def get_consolidate() -> bool:
    ...
    The explicit ``jacky curator run --consolidate`` flag overrides this for
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``jacky update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``jacky curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `jacky curator run --dry-run` to preview now"
            )
            save_state(state)
...
CURATOR_REVIEW_PROMPT = (
    "You are running as Jacky' background skill CURATOR. This is an "
    ...
    "into ~/.jacky/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: jacky-config-*, jacky-dashboard-*, gateway-*, codex-*, "
    ...
    "~/.jacky/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.jacky/logs/curator/``)
    ...
    authored skill data in ``~/.jacky/skills/``.

    ``ensure_jacky_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_jacky_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every jacky install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: jacky curator status
        keep an umbrella stable: jacky curator pin document-tools
    ...
    lines.append("full report: jacky curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: jacky curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.jacky/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.jacky/skills/.archive/` for "
            "safety and can be restored via `jacky curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.jacky/skills/.archive/`. "
            "Restore any via `jacky curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `jacky curator restore <name>`")
    lines.append("- All archives live under `~/.jacky/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    *consolidate* gates the LLM umbrella-building pass. ``None`` (the default)
    ...
    ``jacky curator run --consolidate`` flag. When consolidation is off, only
    ...
    *consolidate*: when consolidation is off, the preview only reports the
    ...
    # real pass out. We still record a summary so `jacky curator status`
    ...
    def _llm_pass():
            ...
            if "No agent-created skills" in candidate_list:
                ...
                if get_prune_builtins():
                    builtins_note = (
                        ...
                        "delete). It will be restored on `jacky update` only if "
        ...
        # recorded in state so `jacky curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``jacky model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `jacky model` → auxiliary

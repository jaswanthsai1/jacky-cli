"""Coding-context awareness — base Jacky, every interactive surface.

When the user runs Jacky inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Jacky shifts into a **coding posture**. This module is the
...
logger = logging.getLogger("jacky.coding_context")
...
# in _JACKY_CORE_TOOLS, so they're present on every surface this fires on.
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``JACKY_TUI_TOOLSETS``, …); they never
...
def is_coding_context(
    *,
    ...
    """Whether Jacky should operate in its coding posture right now."""

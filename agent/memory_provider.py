...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `jacky backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - jacky_home (str): The active JACKY_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.jacky``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "jacky").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'jacky memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], jacky_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'jacky memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``jacky_home`` is the active JACKY_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE JACKY_HOME.

        ``jacky backup`` only walks JACKY_HOME, so any provider state kept
        ...
        of the archive, and ``jacky import`` restores them to their original

...
``auth.json``.  They intentionally have no dependency on ``jacky_cli.auth`` so
...
# Sources Jacky owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Jacky-owned OAuth/device-code state)

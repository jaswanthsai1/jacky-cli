...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.jacky/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Jacky ↔ models.dev
# ---------------------------------------------------------------------------

# Jacky provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Jacky (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_jacky_home() / "models_dev_cache.json"
...
def fetch_models_dev(force_refresh: bool = False) -> Dict[str, Any]:
    ...
    When ``force_refresh=True`` (used by ``jacky config refresh``, the
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Jacky provider ID to its models dict from models.dev.
...
# Gemma models whose TPM quotas are too small for normal Jacky agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Jacky provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Jacky ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Jacky or models.dev provider ID.  Tries exact match then

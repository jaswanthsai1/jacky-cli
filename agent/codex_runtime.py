...
def _codex_note_to_tool_progress(note: dict) -> tuple[str, str, dict] | None:
    """Map a Codex app-server ``item/started`` notification to a Jacky
    ...
    surfaced them as Jacky tool-progress events — so gateways (Telegram, etc.)
...
def _record_codex_app_server_usage(agent, turn) -> dict[str, Any]:
    """Translate Codex app-server token usage into Jacky accounting.

    Codex app-server reports usage via thread/tokenUsage/updated as:
    inputTokens, cachedInputTokens, outputTokens, reasoningOutputTokens,
    totalTokens.

    Jacky' canonical prompt bucket includes uncached input + cached input.
    The Codex app-server protocol does not currently expose cache-write tokens,
    so that bucket remains zero on this runtime.

    Even when Codex omits usage for a turn, Jacky should still count that turn
...
def _record_codex_app_server_compaction(
    ...
    *,
    ...
    """Record a Codex-native context compaction boundary in Jacky state.

    The app-server owns the compacted thread context, so Jacky should not
...
def run_codex_app_server_turn(
    ...
    *,
    ...
    app-server` subprocess and projects its events back into Jacky'
    ...
    if not hasattr(agent, "_codex_session") or agent._codex_session is None:
        ...
        # Approval callback: defer to Jacky' standard prompt flow if a
        ...
        # explicitly opted out of Jacky approvals — via `approvals.mode: off`
        # in config, the /yolo session toggle, or --yolo / JACKY_YOLO_MODE —
        # honor that and let codex's own sandbox permission profile
        # (~/.codex/config.toml) be the policy gate instead of double-gating
        # with a missing Jacky UI. Defaults (manual/smart/unset) preserve the
        ...
        def _on_codex_event(note: dict) -> None:
            # Bridge Codex app-server item/started notifications to Jacky

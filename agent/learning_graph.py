...
def _jacky_meta(fm: dict[str, Any]) -> dict[str, Any]:
    """``metadata.jacky`` as a dict, tolerant of the string-valued frontmatter
    that ``parse_frontmatter``'s malformed-YAML fallback produces."""
    meta = fm.get("metadata")
    jacky = meta.get("jacky") if isinstance(meta, dict) else None
    return jacky if isinstance(jacky, dict) else {}


def _related(fm: dict[str, Any]) -> list[str]:
    raw = fm.get("related_skills") or _jacky_meta(fm).get("related_skills")
...
def _category(fm: dict[str, Any], skill_md: Path) -> str:
    cat = fm.get("category") or _jacky_meta(fm).get("category")
    if cat:
        return str(cat)
...
def _load_usage() -> dict[str, dict[str, Any]]:
    try:
        ...
        path = get_jacky_home() / "skills" / ".usage.json"
        try:
...
def _skill_roots() -> list[tuple[str, Path]]:
    ...
    home_skills = get_jacky_home() / "skills"

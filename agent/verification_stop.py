...
def _session_is_messaging_surface() -> bool:
    ...
    ``JACKY_SESSION_PLATFORM``; the CLI and TUI set ``JACKY_SESSION_SOURCE``
    (e.g. ``cli``, ``tui``) instead. Both are consulted via the session-context
    helper (with an ``os.environ`` fallback), alongside the ``JACKY_PLATFORM``
    ...
    try:
        ...
        platform = os.getenv("JACKY_PLATFORM", "") or os.environ.get(
...
def verify_on_stop_enabled(config: dict[str, Any] | None = None) -> bool:
    ...
    Precedence: an explicit ``JACKY_VERIFY_ON_STOP`` env var wins, then an
    ...
    """
    env = os.environ.get("JACKY_VERIFY_ON_STOP")
    if env is not None:
    ...
    *,
    ...
    if verify_commands:
        ...
        command_instruction = (
            ...
            "`tempfile` path with a `jacky-verify-` filename prefix, run it "

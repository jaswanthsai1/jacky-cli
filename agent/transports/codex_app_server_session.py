...
Owns one Codex thread per Jacky session. Drives `turn/start`, consumes
...
# Jacky' tools.terminal.security_mode → Codex's permissions profile id.
# Defaults if config is missing → workspace-write (matches Codex's own default).
_JACKY_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Jacky/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Jacky session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _JACKY_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("JACKY_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="jacky",
            client_title="Jacky Agent",
            client_version=_get_jacky_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Jacky' text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Jacky' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # OAuth, form data). For our own jacky-tools callback we
            # auto-accept — the user already approved Jacky' tools
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Jacky approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Jacky returns 'once', 'session', 'always', or 'deny'.
...
def _get_jacky_version() -> str:
    """Best-effort Jacky version string for codex's userAgent line."""
    try:

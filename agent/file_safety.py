...
def build_write_denied_paths(home: str) -> set[str]:
    ...
    jacky_home = _jacky_home_path()
    jacky_root = _jacky_root_path()
    return {
        os.path.realpath(p)
        for p in [
            ...
            str(jacky_home / ".anthropic_oauth.json"),
            # Top-level Anthropic PKCE credential store remains sensitive even
            # when a profile is active; default/non-profile sessions still read it.
            str(jacky_root / ".anthropic_oauth.json"),
...
def get_safe_write_roots() -> set[str]:
    """Return resolved JACKY_WRITE_SAFE_ROOT paths. Supports multiple directories
    separated by ``os.pathsep`` (``:`` on Unix, ``;`` on Windows).
    E.g., ``/opt/data:/var/www/html`` on Unix, ``C:\\data;D:\\www`` on Windows."""
    env = os.getenv("JACKY_WRITE_SAFE_ROOT", "")
    if not env:
        return set()
    roots: set[str] = set()
...
def is_write_denied(path: str) -> bool:
    ...
    jacky_dirs = []
    for base in (_jacky_home_path(), _jacky_root_path()):
        try:
            real = os.path.realpath(base)
            if real not in jacky_dirs:
                jacky_dirs.append(real)
        except Exception:
            continue

    for base_real in jacky_dirs:
        try:
...
def get_read_block_error(path: str) -> Optional[str]:
    """Return an error message when a read targets a denied Jacky path.

    Three categories are blocked:

      * Internal Jacky cache files under ``JACKY_HOME/skills/.hub`` —
        readable metadata that an attacker could use as a prompt-injection
        carrier.
      * Credential / secret stores under JACKY_HOME and the global Jacky
    ...
    **This is NOT a security boundary.** The terminal tool runs as the
    ...
    or ``cat ~/.jacky/.env`` and exfiltrate the file. The read-deny exists
    ...
    # Resolve BOTH the active JACKY_HOME (profile-aware) AND the global
    # Jacky root so credential stores at <root>/auth.json etc. are also
    # blocked when running under a profile (JACKY_HOME points at
    # <root>/profiles/<name> in profile mode). Same shape as the write
    # deny widening (#15981, #14157).
    jacky_dirs: list[Path] = []
    for base in (_jacky_home_path(), _jacky_root_path()):
        try:
            real = base.resolve()
            if real not in jacky_dirs:
    ...
    for hd in jacky_dirs:
        ...
        for blocked in blocked_dirs:
            try:
                ...
                continue
            return (
                f"Access denied: {path} is an internal Jacky cache file "
    ...
    for hd in jacky_dirs:
        for name in credential_file_names:
            ...
            if resolved == blocked:
                return (
                    f"Access denied: {path} is a Jacky credential store "
    ...
    for hd in jacky_dirs:
        ...
        if resolved == mcp_tokens:
            return (
                f"Access denied: {path} is the Jacky MCP token directory "
        ...
        try:
            ...
            continue
        return (
            f"Access denied: {path} is a Jacky MCP token file "
            "and cannot be read directly. (Defense-in-depth — not a "
            "security boundary; the terminal tool can still bypass.)"
        )
...
def raise_if_read_blocked(path: str) -> None:
    """Raise ``ValueError`` if ``path`` is a denied Jacky read (see
...
#
# Jacky profiles are separate JACKY_HOME dirs under
...
#
# Reference: May 2026 incident where a jacky-security profile session
# edited skills under both ``~/.jacky/profiles/jacky-security/skills/``
# AND ``~/.jacky/skills/`` (the default profile's skills) without realizing
# the second path belonged to a different profile.
# ---------------------------------------------------------------------------

# Profile-scoped directories under JACKY_HOME / <root> / <root>/profiles/<X>/
...
def _resolve_active_profile_name() -> str:
    """Return the active profile name derived from JACKY_HOME.
    ...
    """
    try:
        home_real = _jacky_home_path().resolve()
        root_real = _jacky_root_path().resolve()
...
def classify_cross_profile_target(path: str) -> Optional[dict]:
    ...
    Returns ``None`` when the target is outside Jacky scope, or is inside
...
def get_cross_profile_warning(path: str) -> Optional[str]:
    ...
    Jacky entirely. Caller is expected to surface the warning to the
    ...
    return (
        ...
        f"belongs to Jacky profile {info['target_profile']!r}, but the "
...
#
#   <JACKY_HOME>/profiles/<name>/sandboxes/<backend>/<task>/home/.jacky/...
#
...
# ``…/sandboxes/<backend>/<task>/home/.jacky/…`` segment and warns
# regardless of which Jacky profile is active. It does NOT cover the
# inner-container case where the bind mount strips the ``sandboxes/`` prefix
# (the agent's view inside the container is plain ``/root/.jacky/...``);
...
def _find_sandbox_mirror_segments(parts: tuple) -> Optional[int]:
    """Return the index of the inner ``.jacky`` part in a sandbox-mirror path.

    Matches ``…/sandboxes/<backend>/<task>/home/.jacky/…`` and returns the
    index where the inner Jacky-state portion starts. Returns ``None`` for
    paths that do not contain the sandbox-mirror shape.
    """
    for i, part in enumerate(parts):
        if part != "sandboxes":
            continue
        # Need at least: sandboxes / <backend> / <task> / home / .jacky / <thing>
        if i + 5 >= len(parts):
            continue
        if parts[i + 3] == "home" and parts[i + 4] == ".jacky":
            return i + 4
    return None


def classify_sandbox_mirror_target(path: str) -> Optional[dict]:
    """Classify a write target as a sandbox-mirror of authoritative Jacky state.

    Returns ``None`` when the path does not match the sandbox-mirror shape.
    Otherwise returns a dict with:

      * ``target_path``: the resolved path string
      * ``mirror_root``: the ``…/sandboxes/<backend>/<task>/home/.jacky``
        prefix (so callers can show users which sandbox owns the mirror)
      * ``inner_path``: the portion under the mirror's ``.jacky`` (what the
        agent likely meant to address on the host)

    Detection is path-shape-only — does not require any Jacky resolver to
    succeed, so it works correctly even when called from contexts where
    JACKY_HOME resolution would be ambiguous.
    """
    try:
        target = Path(os.path.expanduser(str(path))).resolve()
    except (OSError, RuntimeError):
...
def get_sandbox_mirror_warning(path: str) -> Optional[str]:
    ...
    return (
        ...
        f"Writes here land on a copy that the host Jacky process never "
        f"reads — the authoritative file is likely {info['inner_path']!r} "
        f"under the real JACKY_HOME. Use the host-side tool for "
...
# full ``…/sandboxes/<backend>/<task>/home/.jacky/…`` prefix on the host.
# But when file tools execute *inside* the container the bind-mount strips
# that prefix: the agent sees plain ``/root/.jacky/…``.  The root:root
...
def classify_container_mirror_target(
    ...
    under the mirror prefix. Otherwise returns:
      ...
      * ``inner_path``: portion under the mirror root (what the agent
        likely meant to address in the host JACKY_HOME)
    """
...
def get_container_mirror_warning(
    ...
    sandbox mirror of authoritative Jacky state.
    ...
    return (
        ...
        f"bind-mounted home — a per-task mirror that the host Jacky "
        f"process never reads. The authoritative file is "
        f"{info['inner_path']!r} under the real JACKY_HOME. Use the "

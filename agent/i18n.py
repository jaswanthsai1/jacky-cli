"""Lightweight internationalization (i18n) for Jacky static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by Jacky itself -- approval prompts, a handful of gateway slash
...
Language resolution order:
    ...
    2. ``JACKY_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    1. ``JACKY_BUNDLED_LOCALES`` env var -- set by the Nix wrapper (or any
    ...
    """
    override = os.getenv("JACKY_BUNDLED_LOCALES", "").strip()
    if override:
        candidate = Path(override)
        if candidate.is_dir():
            return candidate
        logger.warning(
            "JACKY_BUNDLED_LOCALES points to a non-directory path (%s); "
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already a jacky dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`jacky_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("JACKY_LANGUAGE")
    if env_lang:

...
appropriate.  All installs go to a Jacky-owned bin staging dir,
``<JACKY_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``jacky lsp
  status``.
...
# ``<JACKY_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `jacky lsp status` reports the host's presence.
...
def jacky_lsp_bin_dir() -> Path:
    """Return the Jacky-owned bin staging dir for LSP servers."""
    home = os.environ.get("JACKY_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".jacky")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(jacky_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = jacky_lsp_bin_dir().parent  # <JACKY_HOME>/lsp/
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a jacky-owned target dir.
    ...
    console script.
    """
    pip_target = jacky_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = jacky_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``jacky lsp status`` CLI to give users a quick

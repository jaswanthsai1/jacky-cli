...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<JACKY_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
    ...
    home = os.environ.get("JACKY_HOME") or os.path.join(
        os.path.expanduser("~"), ".jacky"
    )
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<JACKY_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    session_path = os.path.join(
        jacky_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(jacky_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName Jacky -HostProfileId jacky -HostVersion 1.0.0 "

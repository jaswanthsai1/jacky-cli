#!/usr/bin/env python3
"""Generate llms.txt and llms-full.txt for the Jacky docs site.
...
SECTIONS: list[tuple[str, list[tuple[str, str, str | None]]]] = [
    ...
    ("Guides & Tutorials", [
        ...
        ("guides/python-library", "Use Jacky as a Python Library", None),
        ("guides/use-mcp-with-jacky", "Use MCP with Jacky", None),
        ("guides/use-voice-mode-with-jacky", "Use Voice Mode with Jacky", None),
        ("guides/use-soul-with-jacky", "Use SOUL.md with Jacky", None),
        ("guides/build-a-jacky-plugin", "Build a Jacky Plugin", None),
    ...
    ("Reference", [
        ...
        ("reference/skills-catalog", "Bundled Skills Catalog", "Table of all ~90 skills bundled with Jacky"),
...
def emit_llms_index() -> str:
    ...
    lines.append(
        ...
        "jacky-agent/main/scripts/install.sh | bash`  "
        "(Linux, macOS, WSL2, Termux)"
    )
    lines.append("")
    lines.append("Repo: https://github.com/jaswanthsai1/jacky-cli")
    lines.append("")
...
def emit_llms_full() -> str:
    ...
    seen: set[Path] = set()
    chunks: list[str] = [
        "# Jacky Agent — Full Documentation\n",
        (
            "This file is the entire Jacky Agent documentation concatenated for LLM "

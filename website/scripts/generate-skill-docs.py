...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/jaswanthsai1/jacky-cli/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    jacky_meta = (fm.get("metadata") or {}).get("jacky") or {}
    tags = jacky_meta.get("tags") or []
    related = jacky_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `jacky skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Jacky Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Jacky ships with a large built-in skill library copied into `~/.jacky/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Jacky also syncs bundled skills on `jacky update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.jacky/skills/` tree, it is still shipped with Jacky; restore it with `jacky skills reset <name> --restore`.",
        "",

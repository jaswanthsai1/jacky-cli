...
sidebar_position: 3
title: "Desktop App"
description: "The native Jacky desktop app — a polished experience for chatting with Jacky, with streaming tool output, side-by-side previews, a file browser, voice, cron, profiles, skills, and settings. macOS, Windows, and Linux."
---

# Desktop App

The Jacky desktop app is a native app built around the **same** agent you get from the CLI and the gateway — same config, same API keys, same sessions, same skills, same memory. It is not a separate product or a lightweight clone; it uses the same Jacky Agent core and settings, and drives it through a modern & thoughtfully designed UI. If you have used `jacky` in a terminal, everything you set up there is already here, and anything you do here shows up there.

It runs on **macOS, Windows, and Linux**.

:::tip Which interface is which?
Jacky has several front ends that all talk to the same agent:

- **Desktop App** (this page) — a native application with a purpose-built UI for chat, configuration, and management.
- **CLI** (`jacky`) and **[TUI](./tui.md)** (`jacky --tui`) — terminal interfaces.
- **[Web Dashboard](./features/web-dashboard.md)** (`jacky dashboard`) — a browser admin panel; its optional **Chat** tab embeds the TUI through a pseudo-terminal.
...
Follow the [installation instructions for Jacky Desktop](../getting-started/installation.md).

If you already have Jacky installed, simply run
...
- **The same conversation history** as every other Jacky surface — sessions started here resume in the CLI/TUI and vice versa.
...
Explore and preview the working directory without leaving the app — useful for following along as the agent reads, writes, and edits files. Set the initial project directory with `jacky desktop --cwd <path>` (or the `JACKY_DESKTOP_CWD` environment variable).

### Voice

Talk to Jacky and hear it back, the same [voice mode](./features/voice-mode.md) available elsewhere. On macOS the OS will prompt once for microphone access.
...
- **Every provider and model in the menus** — the GUI surfaces the full provider list and every model that `jacky model` knows about, so you pick from the same catalog the CLI sees rather than a curated subset.
...
The app also surfaces the broader Jacky management surface so you don't have to drop to a terminal:

- **Skills** — browse, install, and manage [skills](./features/skills.md).
- **Cron** — view and manage [scheduled jobs](../reference/cli-commands.md#jacky-cron).
- **Profiles** — switch between [Jacky profiles](./profiles.md) (isolated config/skills/sessions).
...
The [manual update process](https://jacky-agent.nousresearch.com/docs/getting-started/updating) also works with the GUI.

## Uninstalling

Open **Settings → About → Danger zone** and pick how much to remove:

- **Uninstall Chat GUI only** — removes the desktop app and its data; the Jacky agent, your config, and your chats stay. (Same as `jacky uninstall --gui`.)
- **Uninstall GUI + agent, keep my data** — removes the app and the agent but keeps config, chats, and secrets for a future reinstall. (Same as `jacky uninstall`.)
- **Uninstall everything** — removes the app, the agent, and all user data. (Same as `jacky uninstall --full`.)
...
You can do the same from the terminal — `jacky uninstall --gui` for the GUI alone, or `jacky uninstall` / `jacky uninstall --full` for the agent too.

:::note
Running `jacky uninstall --gui` from a **source checkout** (a `jacky desktop` dev build) also removes the workspace `node_modules` and `apps/desktop/{dist,release}` build output, since those are GUI build artifacts. They're recoverable with `jacky desktop` (or `npm install` + a rebuild) — but if you're actively hacking on the desktop app, expect to reinstall dependencies afterward.
:::

## CLI reference: `jacky desktop`

To launch via the CLI, simply run `jacky desktop`. By default it installs workspace Node dependencies, builds the current OS's unpacked Electron app, then launches that packaged artifact.
...
| `--build-only`       | Build the desktop app but do not launch it (used by `jacky update`)                      |
| `--source`           | Launch via `electron .` against `apps/desktop/dist` instead of the packaged app           |
| `--cwd PATH`         | Initial project directory for desktop chat sessions (sets `JACKY_DESKTOP_CWD`)           |
| `--jacky-root PATH` | Override the Jacky source root the app uses (sets `JACKY_DESKTOP_JACKY_ROOT`)          |
| `--ignore-existing`  | Force the app to ignore any `jacky` CLI already on `PATH` during backend resolution      |
| `--fake-boot`        | Enable deterministic boot delays for validating the startup UI                            |

## How it works

The packaged app ships the Electron shell and a native React chat surface. On first launch it can install the Jacky Agent runtime into `JACKY_HOME` (`~/.jacky`, or `%LOCALAPPDATA%\jacky` on Windows) — **the same layout a CLI install uses**, which is why the two are interchangeable. Backend resolution first honours `JACKY_DESKTOP_JACKY_ROOT`, then a completed managed install, then a probed `jacky` on `PATH` (unless `--ignore-existing` / `JACKY_DESKTOP_IGNORE_EXISTING=1` is set), and finally an explicit `JACKY_DESKTOP_JACKY` command override for packagers such as Nix. The React renderer talks to a headless backend the app launches for you — a `jacky serve` process that serves the `tui_gateway` JSON-RPC/WebSocket API — and reuses the agent runtime rather than embedding `jacky --tui`. The desktop app is **self-contained**: it runs its own `jacky serve` backend and never opens or requires the [web dashboard](./features/web-dashboard.md). (Runtimes older than the `serve` command fall back to a headless `dashboard --no-open` automatically, so an app update never outruns its backend.) Install, backend-resolution, and self-update logic live in the Electron main process.

## Connecting to a remote backend

By default the app starts and manages its own **local** backend. You can instead point it at a Jacky backend running on another machine — a VPS, a home server, or a Mini behind Tailscale.

:::info The remote backend is a running `jacky serve` process
"Remote backend" means a **`jacky serve`** server running on the remote machine — that is the process the desktop app connects to. Nothing in this section works unless that backend is actually up and reachable. The desktop app does not start it for you; you (or a `systemd` service) keep `jacky serve` running on the remote host, and the app attaches to it. If you also use messaging channels (Telegram, Discord, etc.), the **gateway** is a *separate* long-running process you start independently — see the note after the setup steps.
:::
...
Set a username and password, then start the backend bound to a reachable address. The credentials live in `~/.jacky/.env` (the secrets file, mode 0600):
...
jacky serve --host 0.0.0.0 --port 9119
```

Keep that `jacky serve` process running for as long as you want the desktop app to be able to connect — if it stops, the app can no longer reach the backend. Run it under `systemd`, `tmux`, or your process manager of choice so it survives logout and reboots.

Separately, make sure the **gateway is running** on the remote host if you rely on messaging channels — the `jacky serve` backend is what the desktop app talks to, but your Telegram/Discord/Slack gateway sessions are a different process that you start and keep running on their own. See [Messaging](./messaging/index.md) for gateway setup.

Prefer not to keep a plaintext password at rest? Set `JACKY_DASHBOARD_BASIC_AUTH_PASSWORD_HASH` to a scrypt hash instead — compute it with `python -c "from plugins.dashboard_auth.basic import hash_password; print(hash_password('PW'))"`. Full configuration surface (config.yaml keys, every env var, the rate limiter): [Web Dashboard → Username/password provider](./features/web-dashboard.md#usernamepassword-provider-no-oauth-idp).

Running the backend as a systemd service? Give the unit `EnvironmentFile=%h/.jacky/.env` so the credentials are in the environment at boot.
...
1. **Remote URL** — `http://<backend-host>:9119` (path prefixes like `/jacky` work if you front it with a reverse proxy)
...
3. **Save and reconnect** — switches the desktop shell onto the remote backend. The session refreshes automatically; you stay signed in across restarts when `JACKY_DASHBOARD_BASIC_AUTH_SECRET` is set.

You can also set the backend URL without the UI via the `JACKY_DESKTOP_REMOTE_URL` environment variable before launching the app (it overrides the in-app setting); you still sign in from the Gateway settings panel.
...
- **No "Sign in" button — it asks for a session token instead** — the backend's username/password provider isn't active. `/api/status` won't list `"basic"` in `auth_providers`. Make sure both the username and a password (or password hash) are set in `~/.jacky/.env` and that the dashboard process actually loaded them.
- **Signed out on every restart** — set `JACKY_DASHBOARD_BASIC_AUTH_SECRET` to a stable value. Without it the token-signing key is regenerated per boot, invalidating all sessions.
...
For the same setup from the web-dashboard angle, see [Web Dashboard → Connecting Jacky Desktop to a remote backend](./features/web-dashboard.md#connecting-jacky-desktop-to-a-remote-backend); the env vars are catalogued under [Environment Variables → Web Dashboard & Jacky Desktop](../reference/environment-variables.md#web-dashboard--jacky-desktop).

## Troubleshooting

Boot logs land in `JACKY_HOME/logs/desktop.log` (it includes backend output and recent Python tracebacks) — check it first if the app reports a boot failure. You can also tail it from the CLI:
...
rm "$HOME/.jacky/jacky-agent/.jacky-bootstrap-complete"

# Rebuild a broken Python venv (macOS/Linux)
rm -rf "$HOME/.jacky/jacky-agent/venv"

# Reset a stuck macOS microphone prompt
tccutil reset Microphone com.nousresearch.jacky
```
...
ELECTRON_MIRROR=https://npmmirror.com/mirrors/electron/ \
  bash -c 'cd "$HOME/.jacky/jacky-agent/apps/desktop" && CSC_IDENTITY_AUTO_DISCOVERY=false npm run pack'
```
...
```bash
JACKY_DESKTOP_JACKY_ROOT=/path/to/clone npm run dev
JACKY_HOME=/tmp/throwaway npm run dev
...
- [TUI](./tui.md) — the modern terminal UI used by `jacky --tui` and the dashboard chat tab

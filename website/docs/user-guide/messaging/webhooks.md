...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Jacky Agent — Webhooks Tutorial"
...
1. Enable via `jacky gateway setup` or environment variables
2. Define routes in `config.yaml` **or** create them dynamically with `jacky webhook subscribe`
...
| `script` | No | Filter/transform script under `~/.jacky/scripts/`. The webhook payload is passed as JSON on stdin. JSON object stdout replaces the payload before templating; text stdout is exposed as `script_output`; empty stdout, `[SILENT]`, or a nonzero exit code ignores the webhook. |
          ...
          filters:
            ...
            - any:
                ...
                - field: "payload.project_id"
                  in_file: "~/.jacky/data/todoist/watchlist.json"
...
Use `script` when declarative filters are not enough. Scripts must live under `~/.jacky/scripts/` for the active profile; relative paths resolve there, and path traversal outside that directory is blocked. `.sh` and `.bash` scripts run with bash, and all other extensions run with the current Python interpreter.

The route payload is sent to stdin as JSON:

```python
# ~/.jacky/scripts/todoist-jacky-label.py
import json
import sys

payload = json.load(sys.stdin)
labels = payload.get("payload", {}).get("labels", [])
if "jacky" not in labels:
    print("[SILENT]")
    raise SystemExit(0)
...
- Empty stdout, exact `[SILENT]`, `{"__jacky_ignore__": true}`, timeout, missing script, or nonzero exit code returns HTTP 200 with `{"status":"ignored","reason":"script"}`.
...
Add the `github-pr` route to your `~/.jacky/config.yaml` as shown in the example above.
...
Open a pull request on the repository. The webhook fires, Jacky processes the event, and posts a review comment on the PR.
...
```bash
jacky webhook subscribe antenna-matches \
  --deliver telegram \
...
In addition to static routes in `config.yaml`, you can create webhook subscriptions dynamically using the `jacky webhook` CLI command. This is especially useful when the agent itself needs to set up event-driven triggers.

### Create a subscription

```bash
jacky webhook subscribe github-issues \
  --events "issues" \
...
```bash
jacky webhook remove github-issues
```

### Test a subscription

```bash
jacky webhook test github-issues
jacky webhook test github-issues --payload '{"issue": {"number": 42, "title": "Test"}}'
```

### How dynamic subscriptions work

- Subscriptions are stored in `~/.jacky/webhook_subscriptions.json`
...
The agent can create subscriptions via the terminal tool when guided by the `webhook-subscriptions` skill. Ask the agent to "set up a webhook for GitHub issues" and it will run the appropriate `jacky webhook subscribe` command.
...
This is the same trust model that applies to everything the agent reads: web pages, files, and tool output are all untrusted input. Jacky does not — and cannot reliably — sanitize untrusted text with a blocklist; phrasing, encoding, and translation make that trivially bypassable. **The trust boundary is the agent's capability surface, not the input channel.** Harden there:
...
- Run the gateway in foreground to see logs: `jacky gateway run`

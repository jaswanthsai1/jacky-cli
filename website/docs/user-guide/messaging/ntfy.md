...
ntfy makes a great lightweight push channel for Jacky: subscribe to a topic from the [ntfy mobile app](https://ntfy.sh/docs/subscribe/phone/), send messages to the topic to talk to the agent, get the response back on your phone.

> Run `jacky gateway setup` and pick **ntfy** for a guided walk-through.

## Prerequisites

- A topic name (any unique string — `jacky-myname-2026` works fine)
...
That's it. No SDK, no daemon, no Node.js. The adapter uses `httpx` which is already a Jacky dependency.
...
Add these to `~/.jacky/.env`:

```
NTFY_TOPIC=jacky-myname-2026
NTFY_ALLOWED_USERS=jacky-myname-2026
NTFY_HOME_CHANNEL=jacky-myname-2026
```
...
ntfy has no native authenticated user identity. The `title` field on a published message is **publisher-controlled** and can be anything the sender wants. The Jacky adapter does NOT use `title` for authorization — it would let any publisher who knows the topic spoof an allowed user.
...
1. Pick a topic name: `jacky-myname-2026`
2. On your phone: install the [ntfy app](https://ntfy.sh/docs/subscribe/phone/), tap **+**, enter `jacky-myname-2026`
3. On the host:
   ```bash
   echo 'NTFY_TOPIC=jacky-myname-2026' >> ~/.jacky/.env
   echo 'NTFY_ALLOWED_USERS=jacky-myname-2026' >> ~/.jacky/.env
...
Or target a specific topic explicitly via the cron job's `deliver:` field, or from a shell script with the [`jacky send` CLI](/guides/pipe-script-output):

```bash
jacky send ntfy:alerts-channel "Done!"
```
...
ntfy clients render markdown when the publisher sets the `X-Markdown: true` header. To enable for outgoing Jacky replies:

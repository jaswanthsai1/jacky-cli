...
sidebar_position: 12
title: "Google Chat"
description: "Set up Jacky Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Jacky Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Jacky
...
> Run `jacky gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Jacky can read (e.g.,
`~/.jacky/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a jacky restart)
...
- Principal: `jacky-chat-bot@<your-project>.iam.gserviceaccount.com`
- Role: `Pub/Sub Subscriber`

Also grant `Pub/Sub Viewer` on the same subscription — Jacky calls
...
- **App name**: whatever you want users to see ("Jacky" is reasonable).
...
- **Connection settings**: select **Cloud Pub/Sub**, enter the topic name
  `projects/<your-project>/topics/jacky-chat-events`.
...
`ADDED_TO_SPACE` event that Jacky uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Jacky

Add the Google Chat section to `~/.jacky/.env`:
...
Install the dependencies the Google Chat adapter needs (no Jacky extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Jacky is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Jacky detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Jacky session.
...
3. Download the JSON. Move it onto the host that runs Jacky.
4. Register the client with Jacky (run under the profile you want it scoped to):
...
jacky -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Jacky home (e.g.
`~/.jacky/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.jacky/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.jacky/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Jacky isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `jacky gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Jacky will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.jacky/google_chat_user_tokens/<sanitized_email>.json` (filesystem

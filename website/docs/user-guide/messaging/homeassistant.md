...
description: Control your smart home with Jacky Agent via Home Assistant integration.
...
Jacky Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Jacky Agent"
5. Copy the token
...
Jacky Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.jacky/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Jacky Agent".
...
The adapter reads credentials from `~/.jacky/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Jacky profile
...
running Jacky — `curl -H "Authorization: Bearer <token>" <url>/api/` should

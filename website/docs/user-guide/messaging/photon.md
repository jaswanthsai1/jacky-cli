...
Connect Jacky to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Jacky — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Jacky runs it in a
...
jacky photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Jacky Agent` project on your account.
...
Runtime credentials are written to `~/.jacky/.env`
...
(device token, dashboard project id) lives in `~/.jacky/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Jacky
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Jacky replies with a pairing code. Approve it with:

```bash
jacky pairing approve photon <CODE>
```

Use `jacky pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.jacky/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.jacky/.env`):
...
By default Jacky responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Jacky` and
`@Jacky agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Jacky will reply.
...
assigned iMessage line Jacky uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run jacky photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `jacky photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `jacky photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `jacky photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Jacky sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Jacky wake words  | JSON list / comma / newline regex patterns for group mentions |

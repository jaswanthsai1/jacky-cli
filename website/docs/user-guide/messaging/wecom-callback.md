...
Connect Jacky to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Jacky supports two WeCom integration modes:
...
> Run `jacky gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Jacky decrypts the message, queues it for the agent
...
(Use `jacky gateway start` only after `jacky gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Jacky and the token the
...
in `~/.jacky/.env` values around `=` will also break signature checks. After
fixing, restart `jacky gateway run`.
...
Check `jacky gateway run` logs for the bound host/port. If the adapter bound to

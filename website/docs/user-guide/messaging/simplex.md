...
> Run `jacky gateway setup` and pick **SimpleX** for a guided walk-through.
...
| `JACKY_SIMPLEX_TEXT_BATCH_DELAY` | Optional | Quiet-period seconds (default: `0.8`) used to concatenate rapid-fire inbound text messages into one event |
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `jacky pairing approve simplex <CODE>`.
...
`simplex:group:12` as a cron `deliver=` target or in a `jacky send` call.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`jacky send` CLI](/guides/pipe-script-output):

```bash
jacky send simplex:<contact-id> "Done!"
```

## Privacy notes

- SimpleX never reveals phone numbers or email addresses — contacts use opaque IDs
- The connection between Jacky and the daemon is local WebSocket (`ws://127.0.0.1:5225`) — no data leaves your machine

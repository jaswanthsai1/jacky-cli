...
Pull API keys from [Bitwarden Secrets Manager](https://bitwarden.com/products/secrets-manager/) at process startup instead of storing them in plaintext inside `~/.jacky/.env`. One bootstrap secret (a machine-account access token) replaces N per-provider keys, and rotating a credential becomes a single change in the Bitwarden web app.
...
2. Jacky stores that single token in `~/.jacky/.env` as `BWS_ACCESS_TOKEN`.
3. Every time `jacky` (or the gateway, or a cron job) starts, after `~/.jacky/.env` has loaded, Jacky calls `bws secret list <project_id>` and sets the returned keys into `os.environ`.
4. By default Jacky **overrides** values already in your environment, so Bitwarden is the source of truth — rotate a key once in the web app and every Jacky process picks it up on next start. Flip `override_existing: false` in config if you want `.env` to win instead.

The `bws` binary is auto-downloaded into `~/.jacky/bin/` on first use — no `apt`, no `brew`, no `sudo`.
...
2. Create or pick a **Project** (e.g. "Jacky keys").
...
4. **Machine accounts → New machine account → My Jacky machine** → **Projects** tab → grant Read access to your project.
...
1. Download and verify `bws v2.0.0` into `~/.jacky/bin/bws`.
2. Prompt you for the access token (input is hidden). Stored in `~/.jacky/.env` as `BWS_ACCESS_TOKEN`.
...
```bash
jacky secrets bitwarden status
```

From now on, every `jacky` invocation pulls fresh secrets at startup. You'll see a one-line summary in stderr the first time secrets are applied in a process.

## CLI

| Command | What it does |
|---|---|
| `jacky secrets bitwarden setup` | Interactive wizard (install binary, prompt for token, pick project, test fetch) |
| `jacky secrets bitwarden status` | Show config + binary version + token presence |
| `jacky secrets bitwarden sync` | Dry-run: pull secrets now and show what would be applied |
| `jacky secrets bitwarden sync --apply` | Pull and export into the current shell's environment |
| `jacky secrets bitwarden install` | Just download the pinned `bws` binary (no auth required) |
| `jacky secrets bitwarden disable` | Flip `enabled: false`; leaves token + project id in place |
...
| `cache_ttl_seconds` | `300` | How long an in-process fetch result is reused. Set to `0` to disable caching. Cache is per-process; new `jacky` invocations start fresh. |
...
| `auto_install` | `true` | When true, `bws` is auto-downloaded into `~/.jacky/bin/` on first use. |

## Failure modes

Bitwarden never blocks Jacky startup. If anything goes wrong, you'll see a one-line warning in stderr and Jacky continues with whatever credentials `.env` already had:

| Symptom | Cause | Fix |
|---|---|---|
| `BWS_ACCESS_TOKEN is not set` | Enabled in config but token cleared from `.env` | Re-run `jacky secrets bitwarden setup` |
...
- Jacky will refuse to let Bitwarden overwrite the bootstrap token itself, even with `override_existing: true`. If you store `BWS_ACCESS_TOKEN` as a secret inside the project, it's silently skipped during apply.
...
- The pinned version (`bws v2.0.0` at time of writing) is updated through PRs to this repo — Jacky does not auto-upgrade `bws` to "latest" because upstream release shapes can change.

## When NOT to use this

- **Single-machine personal setups** where `~/.jacky/.env` is fine. You're trading one credential for another and adding a network dependency at startup.
...
The good case for this is multi-machine fleets, shared dev boxes, gateway VPSes, or any setup where you want centralized rotation and revocation across multiple Jacky installations.

...
Jacky can pull API keys from external secret managers at process startup instead of storing them in `~/.jacky/.env`. The bootstrap token for the secret manager lives in `.env`; every other provider key (OpenAI, Anthropic, OpenRouter, etc.) can stay in the manager and rotate centrally.
...
Every credential injected by a source is labelled with its origin — setup flows and `jacky model` show `(from Bitwarden)` next to detected keys so you always know where a value came from.

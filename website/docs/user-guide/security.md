...
Jacky Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Jacky checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.jacky/config.yaml`:
...
| `timeout` | `60` | Seconds Jacky waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `jacky --yolo` or `jacky chat --yolo`
2. **Slash command**: Type `/yolo` during a session to toggle it on/off
3. **Environment variable**: Set `JACKY_YOLO_MODE=1`
...
YOLO mode is available in both CLI and gateway sessions. Internally, it sets the `JACKY_YOLO_MODE` environment variable which is checked before every command execution.

When YOLO is active, Jacky shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Jacky refuses to run them **regardless** of:
...
Configure the timeout in `~/.jacky/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.jacky/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.jacky/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` jacky/gateway | Self-termination prevention |
...
The `JACKY_EXEC_ASK=1` environment variable is automatically set when running the gateway.

### Permanent Allowlist

Commands approved with "always" are saved to `~/.jacky/config.yaml`:
...
:::tip
Use `jacky config edit` to review or remove patterns from your permanent allowlist.
:::

## User Authorization (Gateway)

When running the messaging gateway, Jacky controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.jacky/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.jacky/.env to allow open access,
...
For more flexible authorization, Jacky includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `jacky pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.jacky/config.yaml`:
...
jacky pairing approve telegram ABC12DEF

# Revoke a user's access
jacky pairing revoke telegram 123456789

# Clear all pending codes
jacky pairing clear-pending
```

:::tip Docker users: run pairing commands as the `jacky` user
The official Docker image runs the gateway as the unprivileged `jacky` user
...
```bash
docker exec -u jacky jacky-agent jacky pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/jaswanthsai1/jacky-cli/issues/10270
:::

**Storage:** Pairing data is stored in `~/.jacky/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Jacky applies strict security hardening to every container.
...
Container resources are configurable in `~/.jacky/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.jacky/sandboxes/docker/<task_id>/`
...
Some skills need **files** (not just env vars) in the sandbox — for example, Google Workspace stores OAuth tokens as `google_token.json` under the active profile's `JACKY_HOME`. Skills declare these in frontmatter:
...
When loaded, Jacky checks if these files exist in the active profile's `JACKY_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Jacky infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Jacky infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Jacky integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Jacky under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.jacky/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.jacky/logs/` for unauthorized access attempts
10. **Keep updated** — run `jacky update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.jacky/.env`:
...
TERMINAL_SSH_USER=jacky
TERMINAL_SSH_KEY=~/.ssh/jacky_agent_key
```
...
Jacky ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `jacky_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `jacky doctor` for the full remediation.
- **`jacky doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Jacky installs these **lazily** on first use rather than eagerly under `jacky-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `jacky tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `jacky tools`.

...
If you only run one Jacky agent, you don't need this page — see
[Profiles](./profiles.md) for the basics.

## When to use this

You want this setup when you have two or more Jacky agents that should all
...
(`ai.jacky.gateway-<name>.plist`) or systemd user service
(`jacky-gateway-<name>.service`). This guide adds the patterns for managing
them collectively.
...
jacky profile create coder
jacky profile create personal-bot
jacky profile create research
...
```bash
jacky config set gateway.multiplex_profiles true
jacky gateway restart
```

Equivalently, in the default profile's `~/.jacky/config.yaml`:
...
You do **not** run `jacky gateway start` for the secondary profiles — the
...
With a multiplexer running, a named-profile `jacky gateway start` / `run` is a
...
home). `jacky status` reports the multiplexer and the profiles it serves;
`jacky status -p <name>` slices to one profile. Each profile still writes its
...
`~/.local/bin/jacky-gateways` and `chmod +x` it:
...
usage() {
  echo "Usage: jacky-gateways {start|stop|restart|status|list}"
}
...
run_for_profile() {
  ...
  if [ "$profile" = "default" ]; then
    jacky gateway "$action"
  else
    jacky -p "$profile" gateway "$action"
  fi
}
...
```bash
jacky-gateways start      # start every configured profile
jacky-gateways stop       # stop every configured profile
jacky-gateways restart    # restart all
jacky-gateways status     # status across all
jacky-gateways list       # delegates to `jacky gateway list`
```

:::tip
The `default` profile is targeted with `jacky gateway <action>` (no `-p`),
not `jacky -p default gateway <action>`. The wrapper above handles both forms.
:::
...
These are equivalent to `jacky -p coder gateway <action>` — useful if a
...
| Platform | Path                                                              |
| -------- | ----------------------------------------------------------------- |
| macOS    | `~/Library/LaunchAgents/ai.jacky.gateway-<profile>.plist`        |
| Linux    | `~/.config/systemd/user/jacky-gateway-<profile>.service`         |

The default profile keeps the historical names: `ai.jacky.gateway.plist` /
`jacky-gateway.service`.
...
```bash
# Default profile
tail -f ~/.jacky/logs/gateway.log
tail -f ~/.jacky/logs/gateway.error.log

# Named profile
tail -f ~/.jacky/profiles/<name>/logs/gateway.log
tail -f ~/.jacky/profiles/<name>/logs/gateway.error.log
```

Stream every profile's log simultaneously:

```bash
tail -f ~/.jacky/logs/gateway.log ~/.jacky/profiles/*/logs/gateway.log
```

The CLI also has a structured log viewer:

```bash
jacky logs -f                  # follow default profile
jacky -p coder logs -f         # follow one profile
jacky logs --help              # filters, levels, JSON output
```

## Identify what's actually running

```bash
jacky profile list             # profiles + model + gateway state
jacky-gateways status          # full status across every profile
launchctl list | grep jacky    # macOS — PIDs and labels
systemctl --user list-units 'jacky-gateway-*'   # Linux — units
```
...
The default profile uses `~/.jacky/` directly with the same three files.

Edit them with any editor or via the CLI:

```bash
jacky config set model.model anthropic/claude-sonnet-4    # default profile
...
caffeinate -i -w $(cat ~/.jacky/gateway.pid) &   # awake while default gateway runs
...
systemd-inhibit --what=idle:sleep --who=jacky --why="gateways running" \
  sleep infinity &
...
`jacky-gateway-<profile>.service`) continue running across SSH disconnects
and reboots.
...
grep -H 'TELEGRAM_BOT_TOKEN\|DISCORD_BOT_TOKEN' \
     ~/.jacky/.env ~/.jacky/profiles/*/.env
```

## Updating the code

`jacky update` pulls the latest code once and syncs new bundled skills into
every profile:
...
You ran `jacky gateway start` after a previous `jacky gateway stop`. The
...
```bash
ps -ef | grep "jacky_cli.*-p <profile>"
cat ~/.jacky/profiles/<profile>/gateway.pid
...
```bash
# macOS
launchctl unload ~/Library/LaunchAgents/ai.jacky.gateway-<profile>.plist
launchctl load   ~/Library/LaunchAgents/ai.jacky.gateway-<profile>.plist

# Linux
systemctl --user restart jacky-gateway-<profile>.service
```

### Health check

```bash
jacky doctor                  # default profile
jacky -p <profile> doctor     # one profile
```

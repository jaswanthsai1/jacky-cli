...
A **profile distribution** packages a complete Jacky agent — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a Jacky agent meant sending someone:
...
```bash
jacky profile install github.com/you/my-research-agent --alias
```

…and they now have the whole agent. They fill in their own API keys (`.env.EXAMPLE` → `.env`), and they can run `my-research-agent chat` or address it through Telegram / Discord / Slack / any gateway platform. When you push a new version, they run `jacky profile update my-research-agent` and pull your changes — their memories and sessions stay put.
...
The tradeoff: recipients need git installed. On any machine running Jacky in 2026, that's already true.
...
- **You just want to back up a profile on your own machine.** Use [`jacky profile export` / `import`](../reference/profile-commands.md#jacky-profile-export) — that's what those are for.
...
:::caution
**Jacky does not control git.** The file exclusions described on this page are applied by the **installer** when someone runs `jacky profile install` or `jacky profile update`. They are **not** applied when you run `git add` or `git commit`.
:::
...
```bash
jacky profile create research-bot
research-bot setup                    # configure model, API keys
# Edit ~/.jacky/profiles/research-bot/SOUL.md
...
Create `~/.jacky/profiles/research-bot/distribution.yaml`:

```yaml
name: research-bot
version: 1.0.0
description: "Autonomous research assistant with arXiv and web tools"
jacky_requires: ">=0.12.0"
author: "Your Name"
license: "MIT"
...
Create `~/.jacky/profiles/research-bot/.gitignore` with at minimum:
...
```bash
cd ~/.jacky/profiles/research-bot
git init
git add .
git commit -m "v1.0.0"
...
Recipients who run `jacky profile update research-bot` will pull the latest.
...
5. Copies distribution-owned files into `~/.jacky/profiles/research-bot/` (or wherever the manifest's `name` resolves). The [hard-excluded paths](#whats-not-in-a-distribution-ever) are stripped during this copy, even if the author accidentally left them in the repo.
...
```bash
# GitHub shorthand
jacky profile install github.com/you/research-bot

# Full HTTPS
jacky profile install https://github.com/you/research-bot.git

# SSH
jacky profile install git@github.com:you/research-bot.git

# Self-hosted, GitLab, Gitea, Forgejo — any Git host
jacky profile install https://git.example.com/team/research-bot.git

# Private repo using your configured git auth
jacky profile install git@github.com:your-org/internal-bot.git

# Local directory during development (no git push needed)
jacky profile install ~/my-profile-in-progress/
```
...
```bash
# Alice
jacky profile install github.com/acme/support-bot --name support-us --alias
# Bob (same distribution, different local name)
jacky profile install github.com/acme/support-bot --name support-eu --alias
```
...
```
# Environment variables required by this Jacky distribution.
...
```bash
cp ~/.jacky/profiles/research-bot/.env.EXAMPLE ~/.jacky/profiles/research-bot/.env
...
```bash
jacky profile info research-bot
```
...
`jacky profile list` also shows a `Distribution` column so at a glance you can see which of your profiles came from repos and which you hand-built:
...
```bash
jacky profile delete research-bot
```
...
Path:    ~/.jacky/profiles/research-bot
...
Any iteration on the laptop (`git commit && push`) pulls onto the workstation with `jacky profile update research-bot`. Memories stay per-machine — the laptop remembers its own conversations, the workstation remembers its own, they don't collide.
...
# Each engineer
jacky profile install git@github.com:your-org/pr-reviewer.git --alias
# Fill in .env with their own API key (billed to them), .env.EXAMPLE points at what's required
pr-reviewer chat
```

When the lead ships v1.1 (better SOUL, new skill), engineers run `jacky profile update pr-reviewer` and everyone's on the new version within minutes.
...
cd ~/.jacky/profiles/polymarket-trader
# Write a solid README.md at the repo root — GitHub shows it on the repo page
git init && git add . && git status   # confirm no secrets staged
git commit -m "v1.0"
git tag v1.0.0
# Publish to a public GitHub repo
git remote add origin https://github.com/you/jacky-polymarket-trader.git
git push -u origin main --tags

# Anyone
jacky profile install github.com/you/jacky-polymarket-trader --alias
```
...
You built Jacky-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
# Each on-call
jacky profile install git@github.com:your-org/incident-2026-q2.git --alias

# Incident resolved — tear it down
jacky profile delete incident-2026-q2
```
...
:::note
Git ref pinning (`#v1.2.0`) is planned but not in the initial release — install currently tracks the default branch. Track your installed version via `jacky profile info <name>` and hold off on updates until you're ready.
:::
...
jacky profile info research-bot | grep Version
...
```yaml
# ~/.jacky/profiles/research-bot/local/my-overrides.yaml
...
jacky profile delete research-bot --yes
jacky profile install github.com/you/research-bot --alias

# Update to current main but reset config.yaml to the distribution's default
jacky profile update research-bot --force-config --yes
```
...
jacky profile install github.com/yourname/forked-research-bot --alias

# Iterate locally in ~/.jacky/profiles/forked-research-bot/
...
jacky profile install ~/.jacky/profiles/research-bot --name research-bot-test --alias

# Tweak, delete, re-install until it's right
jacky profile delete research-bot-test --yes
jacky profile install ~/.jacky/profiles/research-bot --name research-bot-test
```
...
Cron jobs from a distribution are **not auto-scheduled** — the installer prints `jacky -p <name> cron list` and you enable them explicitly. SOUL.md and skills ARE active as soon as you start chatting with the profile, so read them before your first run if you're installing from someone you don't know.
...
- `install`, `update`, `info` live inside `jacky profile` — not a parallel command tree.
...
- Reserved profile names (`jacky`, `test`, `tmp`, `root`, `sudo`) are rejected at install time to avoid collisions with common binaries.
...
- [`jacky profile export` / `import`](../reference/profile-commands.md#jacky-profile-export) — local backup / restore (not distribution)
- [Using SOUL with Jacky](../guides/use-soul-with-jacky.md) — authoring personalities

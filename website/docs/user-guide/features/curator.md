...
It exists so that skills created via the [self-improvement loop](/user-guide/features/skills#agent-managed-skills-skill_manage-tool) don't pile up forever. Every time the agent solves a novel problem and saves a skill, that skill lands in `~/.jacky/skills/`. Without maintenance, you end up with dozens of narrow near-duplicates that pollute the catalog and waste tokens.

By default (`prune_builtins: true`) the curator can archive **unused bundled built-in skills** (shipped with the repo) after `archive_after_days` of non-use, alongside the agent-created skills it primarily manages. Hub-installed skills (from [agentskills.io](https://agentskills.io)) are always off-limits. Set `curator.prune_builtins: false` to restore the old agent-created-only behavior, where bundled skills are never touched. The curator also **never auto-deletes** — the worst outcome is archival into `~/.jacky/skills/.archive/`, which is recoverable.

Tracks [issue #7816](https://github.com/jaswanthsai1/jacky-cli/issues/7816).

## How it runs

The curator is triggered by an inactivity check, not a cron daemon. On CLI session start, and on a recurring tick inside the gateway's cron-ticker thread, Jacky checks whether:
...
On a brand-new install (or the first time a pre-curator install ticks after `jacky update`), the curator **does not run immediately**. The first observation seeds `last_run_at` to "now" and defers the first real pass by one full `interval_hours`. This gives you a full interval to review your skill library, pin anything important, or opt out entirely before the curator ever touches it.

If you want to see what the curator *would* do before it runs for real, run `jacky curator run --dry-run` — it produces the same review report without mutating the library.
:::

A run has two phases:

1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.jacky/skills/.archive/`. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
...
```bash
jacky model                   # → "Auxiliary models — side-task routing"
...
:::note Legacy config
Earlier releases used a one-off `curator.auxiliary.{provider,model}` block. That path still works but emits a deprecation log line — please migrate to `auxiliary.curator` above so the curator shares the same plumbing (`jacky model`, dashboard Models tab, `base_url`, `api_key`, `timeout`, `extra_body`) as every other aux task.
:::

## CLI

```bash
jacky curator status         # last run, counts, pinned list, LRU top 5
jacky curator run            # trigger a run now (blocks until done). Prune-only unless curator.consolidate: true
jacky curator run --consolidate # force the LLM consolidation pass on for this run, overriding the config default
jacky curator run --background  # fire-and-forget: start the run in a background thread
jacky curator run --dry-run  # preview only — report without any mutations
jacky curator backup         # take a manual snapshot of ~/.jacky/skills/
jacky curator rollback       # restore from the newest snapshot
jacky curator rollback --list     # list available snapshots
jacky curator rollback --id <ts>  # restore a specific snapshot
jacky curator rollback -y         # skip the confirmation prompt
jacky curator pause          # stop runs until resumed
jacky curator resume
jacky curator pin <skill>    # never auto-transition this skill
jacky curator unpin <skill>
jacky curator restore <skill>  # move an archived skill back to active
jacky curator list-archived    # list skills currently in ~/.jacky/skills/.archive/
jacky curator archive <skill>  # manually archive a single skill now
jacky curator prune [--days N] # bulk-archive agent-created skills idle >= N days (default 90)
```

## Backups and rollback

Before every real curator pass, Jacky takes a tar.gz snapshot of `~/.jacky/skills/` at `~/.jacky/skills/.curator_backups/<utc-iso>/skills.tar.gz`. If a pass archives or consolidates something you didn't want touched, you can undo the whole run with one command:

```bash
jacky curator rollback        # restore newest snapshot (with confirmation)
jacky curator rollback -y     # skip the prompt
jacky curator rollback --list # see all snapshots with reason + size
```

The rollback itself is reversible: before replacing the skills tree, Jacky takes another snapshot tagged `pre-rollback to <target-id>`, so a mistaken rollback can be undone by rolling forward to that one with `--id`.

You can also take manual snapshots at any time with `jacky curator backup --reason "before-refactor"`. The `--reason` string lands in the snapshot's `manifest.json` and is shown in `--list`.
...
Set `curator.backup.enabled: false` to disable automatic snapshotting. The manual `jacky curator backup` command still works when backups are disabled only if you set `enabled: true` first — the flag gates both paths symmetrically so there's no way to accidentally skip the pre-run snapshot on mutating runs.

`jacky curator status` also lists the five least-recently-used skills — a quick way to see what's likely to become stale next.
...
`~/.jacky/skills/.usage.json`. A skill qualifies when ALL of the following
are true:

1. Its name is **not** in `~/.jacky/skills/.bundled_manifest` (bundled skills shipped with the repo).
2. Its name is **not** in `~/.jacky/skills/.hub/lock.json` (hub-installed skills).
...
If you manually created a `SKILL.md` or pointed Jacky at an external skill
...
**To see which skills the curator actually manages**, run `jacky curator status`.
...
- Archives are recoverable via `jacky curator restore <name>`

If you want to protect a specific skill from ever being touched — for example a
hand-authored skill you rely on — use `jacky curator pin <name>`. See the next
section.
...
- The **agent's `skill_manage` tool** refuses `delete` on it, pointing the user at `jacky curator unpin <name>`. Patches and edits still go through, so the agent can improve a pinned skill's content as pitfalls come up without a pin/unpin/re-pin dance.

Pin and unpin with:

```bash
jacky curator pin <skill>
jacky curator unpin <skill>
```

The flag is stored as `"pinned": true` on the skill's entry in `~/.jacky/skills/.usage.json`, so it survives across sessions.

Only **agent-created** skills can be pinned — `jacky curator pin` refuses on bundled and hub-installed skills with an explanatory message if you try. Hub-installed skills are never subject to curator mutation. Bundled built-in skills are only touched when `curator.prune_builtins: true` (the default), and even then only archived after `archive_after_days` of non-use — never patched, consolidated, or deleted. Set `curator.prune_builtins: false` to exempt bundled skills entirely.
...
If you want a stronger guarantee than "no deletion" — for instance, freezing a skill's content entirely while the agent still reads it — edit `~/.jacky/skills/<name>/SKILL.md` directly with your editor. The pin guards tool-driven deletion, not your own filesystem access.

## Usage telemetry

The curator maintains a sidecar at `~/.jacky/skills/.usage.json` with one entry per skill:
...
Every curator run writes a timestamped directory under `~/.jacky/logs/curator/`:
...
```bash
jacky curator restore <skill-name>
```

This moves the skill back from `~/.jacky/skills/.archive/` to the active tree and resets its state to `active`. The restore refuses if a bundled or hub-installed skill has since been installed under the same name (would shadow upstream).

## Disabling per environment

The curator is on by default. To turn it off:

- **For one profile only:** edit `~/.jacky/config.yaml` (or the active profile's config) and set `curator.enabled: false`.
- **For just one run:** `jacky curator pause` — the pause persists across sessions; use `resume` to re-enable.
...
- [Issue #7816](https://github.com/jaswanthsai1/jacky-cli/issues/7816) — original proposal and design discussion

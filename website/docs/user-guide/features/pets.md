...
Jacky can show an animated **pet** — a small mascot sprite that reacts to what
...
- Pets are installed into your profile's `pets/` directory
  (`<JACKY_HOME>/pets/<slug>/`), so each [profile](../profiles.md) keeps its
  own set.
...
In the terminal (CLI/TUI), Jacky renders the sprite at full fidelity when your
...
jacky pets install boba --select
...
| Goal | Command |
| --- | --- |
| Browse the gallery | `jacky pets list [query] [--limit N]` |
| List installed pets | `jacky pets list --installed` |
| Install a pet | `jacky pets install <slug> [--select] [--force]` |
...
| Preview/animate | `jacky pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `jacky pets off` |
| Remove an installed pet | `jacky pets remove <slug>` |
| Diagnose setup | `jacky pets doctor` |

`jacky pets show` flags:
...
Beyond installing pre-made pets from the gallery, Jacky can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `jacky pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `jacky tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
- Override the backend with the `JACKY_PET_IMAGE_PROVIDER` env var (e.g. `JACKY_PET_IMAGE_PROVIDER=openrouter`).
...
always-on-top desktop window. Out there it stays visible while Jacky is
...
Run `jacky pets doctor` — it reports:

...
description: "Durable SQLite-backed task board for coordinating multiple Jacky profiles"
---
...
Jacky Kanban is a durable task board, shared across all your Jacky profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.jacky/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.

### Two surfaces: the model talks through tools, you talk through the CLI

The board has two front doors, both backed by the same `~/.jacky/kanban.db`:

- **Agents drive the board through a dedicated `kanban_*` toolset** — `kanban_show`, `kanban_list`, `kanban_complete`, `kanban_block`, `kanban_heartbeat`, `kanban_comment`, `kanban_create`, `kanban_link`, `kanban_unblock`. The dispatcher spawns each worker with these tools already in its schema; orchestrator profiles can also enable the `kanban` toolset explicitly. The model reads and routes tasks by calling tools directly, *not* by shelling out to `jacky kanban`. See [How workers interact with the board](#how-workers-interact-with-the-board) below.
- **You (and scripts, and cron) drive the board through `jacky kanban …`** on the CLI, `/kanban …` as a slash command, or the dashboard. These are for humans and automation — the places without a tool-calling model behind them.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/jacky-kanban-v1-spec.pdf` in the repository.
...
- **Dispatcher** — a long-lived loop that, every N seconds (default 60): reclaims stale claims, reclaims crashed workers (PID gone but TTL not yet expired), promotes ready tasks, atomically claims, spawns assigned profiles. Runs **inside the gateway** by default (`kanban.dispatch_in_gateway: true`). One dispatcher sweeps all boards per tick; workers are spawned with `JACKY_KANBAN_BOARD` pinned so they can't see other boards. After `kanban.failure_limit` consecutive spawn failures on the same task (default: 2) the dispatcher auto-blocks it with the last error as the reason — prevents thrashing on tasks whose profile doesn't exist, workspace can't mount, etc.
...
jacky kanban boards list

# Create a new board.
jacky kanban boards create atm10-server \
    --name "ATM10 Server" \
...
jacky kanban --board atm10-server list
jacky kanban --board atm10-server create "Restart ATM server" --assignee ops

# Change which board is "current" for subsequent calls.
jacky kanban boards switch atm10-server
jacky kanban boards show             # who's active right now?

# Rename the display name (the slug is immutable — it's the directory name).
jacky kanban boards rename atm10-server "ATM10 (Prod)"
...
2. `JACKY_KANBAN_BOARD` env var (set by the dispatcher when spawning a
   worker, so workers can't see other boards).
3. `~/.jacky/kanban/current` — the slug persisted by `jacky kanban
   boards switch`.
4. `default`.
...
`jacky dashboard` → Kanban tab shows a board switcher at the top as soon
...
- **Storage** — files land under
  `<jacky-home>/kanban/attachments/<task_id>/` for the default board, or
  `<jacky-home>/kanban/boards/<slug>/attachments/<task_id>/` for a named
  board. Set `JACKY_KANBAN_ATTACHMENTS_ROOT` to pin a custom location.
...
jacky kanban create "research AI funding landscape" --assignee researcher
...
When the dispatcher picks up `t_abcd` and spawns the `researcher` profile, the very first thing that worker's model does is call `kanban_show()` to read its task. It doesn't run `jacky kanban show t_abcd`.
...
Override the config flag at runtime via `JACKY_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `jacky gateway
...
until one comes up — `jacky kanban create` warns about this at creation
time.

Running `jacky kanban daemon` as a separate process is **deprecated**;
...
jacky kanban create "nightly ops review" \
    --assignee ops \
...
```bash
jacky kanban complete t_abc t_def t_hij --result "batch wrap"
jacky kanban archive  t_abc t_def t_hij
jacky kanban unblock  t_abc t_def
jacky kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

**Workers do not shell out to `jacky kanban`.** When the dispatcher spawns a worker it sets `JACKY_KANBAN_TASK=t_abcd` in the child's env, and that env var flips on a dedicated **kanban toolset** in the model's schema. The same toolset is also available to orchestrator profiles that enable `kanban` in their toolsets config. These tools read and mutate the board directly via the Python `kanban_db` layer, same as the CLI does. A running worker calls these like any other tool; it never sees or needs the `jacky kanban` CLI.
...
kanban_show()                                     # no args — uses JACKY_KANBAN_TASK
...
### Why tools instead of shelling to `jacky kanban`

Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `jacky kanban complete` *inside* the container, where `jacky` isn't installed and `~/.jacky/kanban.db` isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.jacky/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `jacky chat` session has zero `kanban_*` tools in its schema unless the active profile explicitly enables the `kanban` toolset for orchestrator work. Dispatcher-spawned task workers get task-scoped tools because `JACKY_KANBAN_TASK` is set; orchestrator profiles get the broader routing surface through config. No tool bloat for users who never touch kanban.
...
```json
{
  ...
  "verification": ["pytest tests/jacky_cli/test_kanban_db.py -q"],
...
2. `cd $JACKY_KANBAN_WORKSPACE` (via the terminal tool) and do the work there.
...
```bash
jacky kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

jacky kanban create "audit auth flow" \
    --assignee reviewer \
...
The dispatcher emits one `--skills <name>` flag per skill listed, so the worker spawns with all of them loaded on top of the auto-injected kanban guidance. The skill names must match skills that are actually installed on the assignee's profile (run `jacky skills list` to see what's available); there's no runtime install.
...
```bash
jacky kanban create "Translate the docs site to French" \
...
**Manual** — `kanban.auto_decompose: false`. Triage tasks stay in triage until you act. Click the **⚗ Decompose** button on a card, run `jacky kanban decompose <id>` (or `--all`), or use `/kanban decompose <id>` from a chat. This matches the pre-decomposer behavior of the board, useful when you want full control over what runs when.

Flip between the two modes from the **Orchestration: Auto/Manual** pill at the top of the kanban page (emerald = Auto, muted gray = Manual), or by editing `config.yaml` directly. Both modes coexist with `jacky kanban specify` — that's still available as a single-task spec rewrite when you don't want fan-out.

The decomposer's routing decisions depend on profile descriptions, which is a per-profile labeling primitive you set with `jacky profile create --description "..."`, `jacky profile describe <name> --text "..."`, `jacky profile describe <name> --auto` (LLM-generates from the profile's installed skills + model), or the dashboard's per-profile editor in the expanded **Orchestration settings** panel. Profiles without a description still appear in the roster — they're routable by name, just less precisely. The decomposer NEVER lands a child task with `assignee=None`: when the LLM picks an unknown profile, the child gets routed to `kanban.default_assignee` (or the active default profile if that's unset).
...
Config knobs (all under `kanban:` in `~/.jacky/config.yaml`):
...
| `auxiliary.profile_describer` | Model that auto-generates profile descriptions (called by `jacky profile describe --auto`). |
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.jacky/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `jacky dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.jacky/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `jacky -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
```
jacky kanban init                                     # create kanban.db + print daemon hint
jacky kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
jacky kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived]
        ...
        [--json]
jacky kanban show <id> [--json]
jacky kanban assign <id> <profile>                    # or 'none' to unassign
jacky kanban reassign <id>... <profile>               # bulk re-assign tasks to a profile
jacky kanban edit <id> [--title ...] [--body ...]     # edit task title / body / priority in place
        [--priority N]
jacky kanban promote <id>...                          # move todo/blocked tasks to ready (recovery)
jacky kanban schedule <id> --at <ISO8601>             # set/clear a task's scheduled_at start time
jacky kanban diagnostics [--json]                     # board health snapshot (alias: diag)
jacky kanban link <parent_id> <child_id>
jacky kanban unlink <parent_id> <child_id>
jacky kanban claim <id> [--ttl SECONDS]
jacky kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
jacky kanban complete <id>... [--result "..."]
jacky kanban block <id> "<reason>" [--ids <id>...]
jacky kanban unblock <id>...
jacky kanban archive <id>...

jacky kanban tail <id>                                # follow a single task's event stream
jacky kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
jacky kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
jacky kanban runs <id> [--json]                       # attempt history (one row per run)
jacky kanban assignees [--json]                       # profiles on disk + per-assignee task counts
jacky kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
jacky kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `jacky gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
jacky kanban stats [--json]                           # per-status + per-assignee counts
jacky kanban log <id> [--tail BYTES]                  # worker log from ~/.jacky/kanban/logs/
jacky kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
jacky kanban notify-list [<id>] [--json]
jacky kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
jacky kanban context <id>                             # what a worker sees
jacky kanban specify [<id> | --all] [--tenant T]      # flesh out a triage-column idea
        [--author NAME] [--json]                       #   into a full spec and promote to todo
jacky kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
```bash
jacky kanban create "nightly backup audit" \
...
Every `jacky kanban <action>` verb is also reachable as `/kanban <action>` — from inside an interactive `jacky chat` session **and** from any gateway platform (Telegram, Discord, Slack, WhatsApp, Signal, Matrix, Mattermost, email, SMS). Both surfaces call the exact same `jacky_cli.kanban.run_slash()` entry point that reuses the `jacky kanban` argparse tree, so the argument surface, flags, and output format are identical across CLI, `/kanban`, and `jacky kanban`. You don't have to leave the chat to drive the board.
...
The gateway normally queues slash commands and user messages while an agent is still thinking — that's what stops you from accidentally starting a second turn while the first is in flight. **`/kanban` is explicitly exempted from this guard.** The board lives in `~/.jacky/kanban.db`, not in the running agent's state, so reads (`list`, `show`, `context`, `tail`, `watch`, `stats`, `runs`) and writes (`comment`, `unblock`, `block`, `assign`, `archive`, `create`, `link`, …) all go through immediately, even mid-turn.
...
Gateway platforms have practical message-length caps. If `/kanban list`, `/kanban show`, or `/kanban tail` produce more than ~3800 characters of output, the response is truncated with a `… (truncated; use \`jacky kanban …\` in your terminal for full output)` footer. The CLI surface has no such cap.
...
| **P9 Triage specifier** | rough idea → `triage` → `jacky kanban specify` expands body → `todo` | "turn this one-liner into a spec'd task" |

For worked examples of each, see `docs/jacky-kanban-v1-spec.pdf`.
...
```bash
jacky kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$JACKY_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
jacky kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
jacky kanban notify-list
jacky kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
```bash
jacky kanban complete t_abcd \
    --result "rate limiter shipped" \
    --summary "implemented token bucket, keys on user_id with IP fallback, all tests pass" \
    --metadata '{"changed_files": ["limiter.py", "tests/test_limiter.py"], "tests_run": 14}'

# Review the attempt history on a retried task:
jacky kanban runs t_abcd
...
**Bulk close caveat.** `jacky kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`jacky kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `jacky kanban heartbeat $TASK` to signal liveness during long operations. |
...
`jacky kanban tail <id>` shows these for a single task. `jacky kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.jacky/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/jacky-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.

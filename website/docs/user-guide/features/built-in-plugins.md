...
description: "Plugins shipped with Jacky Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Jacky ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.jacky/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Jacky Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.jacky/plugins/<name>/`
3. **Project** — `./.jacky/plugins/<name>/` (requires `JACKY_ENABLE_PROJECT_PLUGINS=1`)
4. **Pip entry points** — `jacky_agent.plugins`

On name collision, later sources win — a user plugin named `disk-cleanup` would replace the bundled one.

`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `jacky memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `jacky plugins list` and the interactive `jacky plugins` UI), but none load until you explicitly enable them:

```bash
jacky plugins enable disk-cleanup
```

Or via `~/.jacky/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Jacky. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
jacky plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `jacky plugins enable <name>`.
...
| `jacky-achievements` | dashboard tab | Steam-style collectible badges generated from your real Jacky session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `jacky memory` and `jacky plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `JACKY_HOME` or `/tmp/jacky-*`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under JACKY_HOME | always | Never |
...
**State** — everything lives at `$JACKY_HOME/disk-cleanup/`:
...
**Safety** — cleanup only ever touches paths under `JACKY_HOME` or `/tmp/jacky-*`. Windows mounts (`/mnt/c/...`) are rejected. Well-known top-level state dirs (`logs/`, `memories/`, `sessions/`, `cron/`, `cache/`, `skills/`, `plugins/`, `disk-cleanup/` itself) are never removed even when empty — a fresh install does not get gutted on first session end.

**Enabling:** `jacky plugins enable disk-cleanup` (or check the box in `jacky plugins`).

**Disabling again:** `jacky plugins disable disk-cleanup`.
...
**Enabling:** `jacky plugins enable security-guidance` (or check the box in `jacky plugins`).

**Disabling again:** `jacky plugins disable security-guidance`.
...
Traces Jacky turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Jacky' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `jacky logs`.
...
```bash
jacky tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Jacky and the next turn ships a trace.
...
Then put the credentials in `~/.jacky/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Jacky turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
| `post_tool_call` | Close the tool observation with sanitized `result`. `read_file` payloads get summarized (head + tail + omitted-line count) so a huge file read stays under `JACKY_LANGFUSE_MAX_CHARS`. |

Session grouping keys off the Jacky session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `jacky chat` session lives under one Langfuse session.

**Verify:**

```bash
jacky plugins list                 # observability/langfuse should show "enabled"
jacky chat -q "hello"              # check the Langfuse UI for a "Jacky turn" trace
```
...
| `JACKY_LANGFUSE_RELEASE` | — | Release/version tag |
| `JACKY_LANGFUSE_SAMPLE_RATE` | `1.0` | Sampling rate passed to the SDK (0.0–1.0) |
| `JACKY_LANGFUSE_MAX_CHARS` | `12000` | Per-field truncation for message content / tool args / tool results |
| `JACKY_LANGFUSE_DEBUG` | `false` | Verbose plugin logging to `agent.log` |

Jacky-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Jacky-prefixed wins when both are set.
...
**Disabling:** `jacky plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.jacky/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
jacky plugins enable google_meet
...
**Disabling:** `jacky plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.jacky/cache/google_meet/` until you remove them.

### jacky-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Jacky session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Jacky feature changes.

**How it works:**

- Scans your entire `~/.jacky/state.db` session history on the dashboard backend
...
- Unlock state is persisted to `$JACKY_HOME/plugins/jacky-achievements/state.json`
...
| Secret | Hidden until Jacky detects the first related signal in your history |

**API** — routes mount under `/api/plugins/jacky-achievements/`:
...
**State files** — live under `$JACKY_HOME/plugins/jacky-achievements/`:

| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Jacky updates. |
...
**Enabling:** Nothing to enable — `jacky-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `jacky dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/jacky-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.jacky/plugins/jacky-achievements/` that ships no dashboard. The plugin's state files under `$JACKY_HOME/plugins/jacky-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Jacky plugin — see [Build a Jacky Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.jacky/plugins/<name>/`
- Manifest source is reported as `bundled` in `jacky plugins list`

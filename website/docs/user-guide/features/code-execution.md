...
The `execute_code` tool lets the agent write Python scripts that call Jacky tools programmatically, collapsing multi-step workflows into a single LLM turn. The script runs in a child process on the agent host, communicating with Jacky over a Unix domain socket RPC.

## How It Works

1. The agent writes a Python script using `from jacky_tools import ...`
2. Jacky generates a `jacky_tools.py` stub module with RPC functions
3. Jacky opens a Unix domain socket and starts an RPC listener thread
4. The script runs in a child process — tool calls travel over the socket back to Jacky
5. Only the script's `print()` output is returned to the LLM; intermediate tool results never enter the context window

```python
# The agent can write scripts like:
from jacky_tools import web_search, web_extract
...
```python
from jacky_tools import search_files, read_file
import json
...
```python
from jacky_tools import search_files, read_file, patch
...
```python
from jacky_tools import terminal, read_file
import json
...
`execute_code` has two execution modes controlled by `code_execution.mode` in `~/.jacky/config.yaml`:

| Mode | Working directory | Python interpreter |
|------|-------------------|--------------------|
| **`project`** (default) | The session's working directory (same as `terminal()`) | Active `VIRTUAL_ENV` / `CONDA_PREFIX` python, falling back to Jacky's own python |
| `strict` | A temp staging directory isolated from the user's project | `sys.executable` (Jacky's own python) |
...
### `JACKY_*` variables in the child

The child process receives only a small, fixed set of operational `JACKY_*`
...
(plus `JACKY_RPC_DIR` / `JACKY_RPC_SOCKET` / `TZ` / `HOME`, which Jacky
injects explicitly so the RPC channel works).

:::note Behavior change
Earlier versions passed **any** variable whose name began with `JACKY_`
through to the child. That broad prefix was removed for security hardening: it
could leak `JACKY_*`-named configuration that doesn't match a secret substring
(for example `JACKY_BASE_URL`, `JACKY_KANBAN_DB`, or a `JACKY_*_WEBHOOK`
endpoint) into arbitrary sandboxed code.

If an `execute_code` script — or a repo/plugin module it imports at import time
— relied on a `JACKY_*` variable outside the four operational names above, it
...
the secret-stripping guarantee (Jacky-managed provider credentials can never
...
**Diagnosing it.** When the child drops one or more non-allowlisted `JACKY_*`
variables, Jacky emits a one-line `debug` log naming them and pointing at the
`env_passthrough` escape hatch. Run with debug logging (`jacky logs --level
DEBUG`, or check `~/.jacky/logs/agent.log`) and look for
`execute_code: dropped N non-allowlisted JACKY_* var(s)` if a script behaves
as though a `JACKY_*` variable is missing.

Jacky always writes the script and the auto-generated `jacky_tools.py` RPC stub into a temp staging directory that is cleaned up after execution. In `strict` mode the script also *runs* there; in `project` mode it runs in the session's working directory (the staging directory stays on `PYTHONPATH` so imports still resolve). The child process runs in its own process group so it can be cleanly killed on timeout or interruption.
...
**Rule of thumb:** Use `execute_code` when you need to call Jacky tools programmatically with logic between calls. Use `terminal` for running shell commands, builds, and processes.

...
**Multi-agent profiles**: When multiple Jacky instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
jacky memory setup    # select "honcho" from the provider list
```
...
- `per-session` — each `jacky` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Jacky syncs them back at session init.
...
The `jacky honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `jacky memory setup honcho` (or run `jacky memory setup` and pick it from the list); the `jacky honcho` subcommand then appears on the next invocation.

```bash
jacky memory setup honcho    # Configure Honcho directly (works before activation)
jacky honcho status          # Connection status, config, and key settings
jacky honcho setup           # Redirects to `jacky memory setup` (post-activation alias)
jacky honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
jacky honcho peer            # Show or update peer names + dialectic reasoning level
jacky honcho mode            # Show or set recall mode (hybrid/context/tools)
jacky honcho tokens          # Show or set token budget for context and dialectic
jacky honcho identity        # Seed or show the AI peer's Honcho identity
jacky honcho sync            # Sync Honcho config to all existing profiles
jacky honcho peers           # Show peer identities across all profiles
jacky honcho sessions        # List known Honcho session mappings
jacky honcho map             # Map current directory to a Honcho session name
jacky honcho enable          # Enable Honcho for the active profile
jacky honcho disable         # Disable Honcho for the active profile
jacky honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `jacky honcho`

If you previously used the standalone `jacky honcho setup`:
...
No re-login or re-setup needed. Run `jacky memory setup` and select "honcho" — the wizard detects your existing config.

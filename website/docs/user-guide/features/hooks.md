...
Jacky has three hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.jacky/hooks/` | Gateway only | Logging, alerts, webhooks |
...
| **[Shell hooks](#shell-hooks)** | `hooks:` block in `~/.jacky/config.yaml` pointing at shell scripts | CLI + Gateway | Drop-in scripts for blocking, auto-formatting, context injection |
...
Each hook is a directory under `~/.jacky/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".jacky" / "hooks" / "my-hook" / "activity.log"
...
```yaml
# ~/.jacky/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.jacky/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".jacky" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.jacky/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.jacky/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/jacky-events"
...
A popular pattern from the community: drop a Markdown checklist at `~/.jacky/BOOT.md`, and have the agent run it once every time the gateway starts. Useful for "on every boot, check overnight cron failures and ping me on Discord if anything failed," or "summarize the last 24h of deploy.log and post it to Slack #ops."

This tutorial shows how to build it yourself as a user-defined hook. Jacky does not ship a built-in BOOT.md hook — you wire up exactly the behavior you want.

#### What we're building

1. A file at `~/.jacky/BOOT.md` with natural-language startup instructions.
...
Create `~/.jacky/BOOT.md`. Write it as if you were giving instructions to a human assistant:

```markdown
# Startup Checklist

1. Run `jacky cron list` and check if any scheduled jobs failed overnight.
...
**`~/.jacky/hooks/boot-md/HOOK.yaml`**

```yaml
name: boot-md
description: Run ~/.jacky/BOOT.md on gateway startup
events:
  - gateway:startup
```

**`~/.jacky/hooks/boot-md/handler.py`**

```python
"""Run ~/.jacky/BOOT.md on every gateway startup."""
...
BOOT_FILE = Path.home() / ".jacky" / "BOOT.md"
...
```bash
jacky logs --follow --level INFO | grep boot-md
```
...
Delete `~/.jacky/BOOT.md` to disable the checklist — the hook stays loaded but silently skips when the file isn't there.
...
Declare shell-script hooks in your `cli-config.yaml` and Jacky will run them as subprocesses whenever the corresponding plugin-hook event fires — in both CLI and gateway sessions. No Python plugin authoring required.
...
Shell hooks are registered by calling `agent.shell_hooks.register_from_config(cfg)` at both CLI startup (`jacky_cli/main.py`) and gateway startup (`gateway/run.py`). They compose naturally with Python plugin hooks — both flow through the same dispatcher.

### Comparison at a glance

| Dimension | Shell hooks | [Plugin hooks](#plugin-hooks) | [Gateway hooks](#gateway-event-hooks) |
|-----------|-------------|-------------------------------|---------------------------------------|
| Declared in | `hooks:` block in `~/.jacky/config.yaml` | `register()` in a `plugin.yaml` plugin | `HOOK.yaml` + `handler.py` directory |
| Lives under | `~/.jacky/agent-hooks/` (by convention) | `~/.jacky/plugins/<name>/` | `~/.jacky/hooks/<name>/` |
...
Each time the event fires, Jacky spawns a subprocess for every matching hook (matcher permitting), pipes a JSON payload to **stdin**, and reads **stdout** back as JSON.
...
{"action":   "block", "message": "Forbidden: rm -rf"}   // Jacky-canonical
...
hooks:
  post_tool_call:
    - matcher: "write_file|patch"
      command: "~/.jacky/agent-hooks/auto-format.sh"
```

```bash
#!/usr/bin/env bash
# ~/.jacky/agent-hooks/auto-format.sh
payload="$(cat -)"
...
hooks:
  pre_tool_call:
    - matcher: "terminal"
      command: "~/.jacky/agent-hooks/block-rm-rf.sh"
      timeout: 5
```

```bash
#!/usr/bin/env bash
# ~/.jacky/agent-hooks/block-rm-rf.sh
payload="$(cat -)"
...
hooks:
  pre_llm_call:
    - command: "~/.jacky/agent-hooks/inject-cwd-context.sh"
```

```bash
#!/usr/bin/env bash
# ~/.jacky/agent-hooks/inject-cwd-context.sh
...
Claude Code's `UserPromptSubmit` event is intentionally not a separate Jacky event — `pre_llm_call` fires at the same place and already supports context injection. Use it here.
...
hooks:
  subagent_stop:
    - command: "~/.jacky/agent-hooks/log-orchestration.sh"
```

```bash
#!/usr/bin/env bash
# ~/.jacky/agent-hooks/log-orchestration.sh
log=~/.jacky/logs/orchestration.log
...
Each unique `(event, command)` pair prompts the user for approval the first time Jacky sees it, then persists the decision to `~/.jacky/shell-hooks-allowlist.json`. Subsequent runs (CLI or gateway) skip the prompt.

Three escape hatches bypass the interactive prompt — any one is sufficient:

1. `--accept-hooks` flag on the CLI (e.g. `jacky --accept-hooks chat`)
2. `JACKY_ACCEPT_HOOKS=1` environment variable
...
```json
{
  "approvals": [
    {
      "event": "post_llm_call",
      "command": "/home/jacky/.jacky/hooks/my-hook.py"
    }
  ]
}
```

The command string must match the configured hook command exactly. A path-keyed object with a `sha256` field is not the expected format and will not approve the hook. Verify manual entries with `jacky hooks list`.

### The `jacky hooks` CLI

| Command | What it does |
|---------|--------------|
| `jacky hooks list` | Dump configured hooks with matcher, timeout, and consent status |
| `jacky hooks test <event> [--for-tool X] [--payload-file F]` | Fire every matching hook against a synthetic payload and print the parsed response |
| `jacky hooks revoke <command>` | Remove every allowlist entry matching `<command>` (takes effect on next restart) |
| `jacky hooks doctor` | For every configured hook: check exec bit, allowlist status, mtime drift, JSON output validity, and rough execution time |
...
- Keep scripts inside `~/.jacky/agent-hooks/` so the path is easy to audit.
- Re-run `jacky hooks doctor` after you pull a shared config to spot newly-added hooks before they register.

...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with Jacky Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

Jacky Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Jacky](/guides/use-voice-mode-with-jacky).

## Prerequisites

Before using voice features, make sure you have:

1. **Jacky Agent installed** — via the install script (see [Installation](/getting-started/installation))
2. **An LLM provider configured** — run `jacky model` or set your preferred provider credentials in `~/.jacky/.env`
3. **A working base setup** — run `jacky` to verify the agent responds to text before enabling voice

:::tip
The `~/.jacky/` directory and default `config.yaml` are created automatically the first time you run `jacky`. You only need to create `~/.jacky/.env` manually for API keys.
:::

:::tip Nous Portal covers both
A paid [Nous Portal](/user-guide/features/tool-gateway) subscription supplies the LLM (step 2) **and** OpenAI TTS via the Tool Gateway — no separate OpenAI key needed. On a fresh install, `jacky setup --portal` wires both up at once.
:::
...
cd ~/.jacky/jacky-agent && uv pip install -e ".[voice]"

# Discord + Telegram messaging (includes discord.py[voice] for VC support)
cd ~/.jacky/jacky-agent && uv pip install -e ".[messaging]"

# Premium TTS (ElevenLabs)
cd ~/.jacky/jacky-agent && uv pip install -e ".[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
cd ~/.jacky/jacky-agent && uv pip install -e ".[all]"
```
...
Voice mode is available in both the **classic CLI** (`jacky chat`) and the **TUI** (`jacky --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.jacky/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
jacky                # Start the interactive CLI
```
...
1. Start the CLI with `jacky` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.jacky/config.yaml` (default: `ctrl+b`).
:::
...
```bash
jacky gateway        # Start the gateway (connects to configured platforms)
jacky gateway setup  # Interactive setup wizard for first-time configuration
```
...
:::tip
To disable the mention requirement in server channels, add to `~/.jacky/.env`:
```bash
...
```bash
jacky gateway        # Start with existing configuration
```
...
If you are running Jacky inside Docker on a Linux desktop, the container also needs access to your host audio socket. See the [Docker audio bridge](/user-guide/docker#optional-linux-desktop-audio-bridge) notes for a PulseAudio/PipeWire-compatible setup.
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.jacky/.env`
...
- Review gateway logs: `tail -f ~/.jacky/logs/gateway.log`

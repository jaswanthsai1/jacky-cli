...
Type `@` followed by a reference to inject content directly into your message. Jacky expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Jacky env: `$JACKY_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$JACKY_HOME/skills/.hub/`

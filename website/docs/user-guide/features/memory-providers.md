...
Jacky Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
jacky memory setup      # interactive picker + configuration
jacky memory status     # check what's active
jacky memory off        # disable external provider
```

You can also select the active memory provider via `jacky plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.jacky/config.yaml`:
...
When a memory provider is active, Jacky automatically:
...
**Setup Wizard:**
```bash
jacky memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `jacky honcho setup` command still works (it now redirects to `jacky memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$JACKY_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$JACKY_HOME/honcho.json` > `~/.jacky/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/jaswanthsai1/jacky-cli/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/jacky).
...
:::tip Migrating from `jacky honcho`
If you previously used `jacky honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Jacky profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Jacky profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Jacky profile. Host key `jacky` → default; `jacky.<profile>` for others. |
...
```bash
jacky profile create coder --clone
```

`--clone` creates a `jacky.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every Jacky profile, creates host blocks for any profile without one, inherits settings from the default `jacky` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `jacky memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/jaswanthsai1/jacky-cli/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/jacky).
...
jacky memory setup    # select "openviking"
# Or manually:
jacky config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.jacky/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.jacky/.env
```
...
`OPENVIKING_AGENT` is Jacky' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
jacky memory setup    # select "mem0" → "Platform"
# Or manually:
jacky config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.jacky/.env
```

**Setup (OSS):**
```bash
jacky memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
jacky memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
jacky memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
jacky memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
jacky memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.jacky/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.jacky/.env
```
...
**Config:** `$JACKY_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.jacky/.env`.
...
| `user_id` | `jacky-user` | User identifier |
| `agent_id` | `jacky` | Agent identifier |
...
**Switching modes:** Re-run `jacky memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
jacky memory setup    # select "hindsight"
# Or manually:
jacky config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.jacky/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p jacky ui start`

**Config:** `$JACKY_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `jacky` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Jacky Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/jaswanthsai1/jacky-cli/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
jacky memory setup    # select "holographic"
# Or manually:
jacky config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.jacky-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$JACKY_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
jacky memory setup    # select "retaindb"
# Or manually:
jacky config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.jacky/.env
```
...
# Then configure Jacky
jacky memory setup    # select "byterover"
# Or manually:
jacky config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$JACKY_HOME/byterover/` (profile-scoped)
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=jacky) |
...
**Setup:**
```bash
jacky memory setup    # select "supermemory"
# Or manually:
jacky config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.jacky/.env
```

**Config:** `$JACKY_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `jacky` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `jacky-{identity}` → `jacky-coder`) to isolate memories per Jacky profile
...
| **Requires** | `pip install jacky-memori` + `jacky-memori install` + [Memori API key](https://app.memorilabs.ai/signup) |
...
**Setup:**
```bash
pip install jacky-memori
jacky-memori install
jacky config set memory.provider memori
jacky memory setup
```
...
| **Memori** | Cloud | Free/Paid | 5 | `jacky-memori` | Tool-aware memory + structured recall |

## Profile Isolation

Each provider's data is isolated per [profile](/user-guide/profiles):

- **Local storage providers** (Holographic, ByteRover) use `$JACKY_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$JACKY_HOME/` so each profile has its own credentials

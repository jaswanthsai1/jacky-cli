...
Jacky Agent can drive your desktop — clicking, typing, scrolling,
...
```
jacky computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `jacky computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `jacky tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Jacky app). `jacky computer-use doctor` will tell you which permission is missing. |
...
```
jacky -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.jacky/config.yaml`.

## `jacky computer-use doctor` — your first triage stop

`jacky computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ jacky computer-use doctor
...
agent is acting here." Each Jacky run declares its own cua-driver
...
Jacky intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Jacky-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Jacky skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Jacky
...
Jacky applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.jacky/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Jacky applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Jacky agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Jacky agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Jacky warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Jacky from inside the RDP / console
...
```
JACKY_CUA_DRIVER_CMD=/path/to/your/cua-driver
```

Swap the backend entirely (for testing):

```
JACKY_COMPUTER_USE_BACKEND=noop   # records calls, no side effects
```

### Telemetry

cua-driver ships with anonymous usage telemetry (PostHog) enabled by default
upstream. **Jacky disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Jacky sets
...
When it's on, `jacky computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Jacky at a binary you built from source instead
of the published release. Jacky resolves the driver with
`shutil.which("cua-driver")` and **does not enforce
`JACKY_CUA_DRIVER_VERSION`**, so a local build (reported as
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Jacky testing (see notes).
...
### Option B — point Jacky straight at the built binary (fastest loop)

Skip the install ceremony entirely: `cargo build` and set
`JACKY_CUA_DRIVER_CMD` to the resulting binary. Best for rapid
edit/build/test.
...
```
# Windows (.env)
JACKY_CUA_DRIVER_CMD=C:\path\to\cua\libs\cua-driver\rust\target\debug\cua-driver.exe
# macOS / Linux (.env)
JACKY_CUA_DRIVER_CMD=/path/to/cua/libs/cua-driver/rust/target/debug/cua-driver
```

### Confirm Jacky is using your build

- `jacky computer-use status` prints the resolved binary path and
  version.
- `jacky computer-use doctor` confirms the binary is reachable and
...
- **Jacky spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Jacky
  change needed either way.
- **Local builds skip the version check.** Jacky warns when the
...
**First action when anything's off: run `jacky computer-use doctor`.**
...
Run `jacky computer-use install` to fetch the cua-driver binary, or
run `jacky tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `jacky computer-use doctor` will
...
- **Jacky-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Jacky `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Jacky (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Jacky among them)

...
Schedule tasks to run automatically with natural language or cron expressions. Jacky exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
All of this is available to Jacky itself through the `cronjob` tool, so you can create, pause, edit, and remove jobs by asking in plain language — no CLI required.

:::tip
At creation, an unpinned job (one you don't give an explicit `provider`/`model`) follows the global default selected by `jacky model` — and Jacky **snapshots** that provider and model on the job. If the global default later changes, the job **fails closed**: it skips the run, makes no inference call, and sends an alert telling you to pin the provider/model explicitly (`cronjob action=update job_id=… provider=… model=…`) to proceed. This prevents an unattended job from silently inheriting a switch to a paid provider/model and spending money you didn't intend (#44585). To make a job deliberately track your global default, pin it to the new values after changing them. `jacky setup --portal` is the lowest-friction option for unattended runs since OAuth refresh is automatic. See [Nous Portal](/integrations/nous-portal).
:::

:::warning
Cron-run sessions cannot recursively create more cron jobs. Jacky disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
jacky cron create "every 2h" "Check server status"
jacky cron create "every 1h" "Summarize new feed items" --skill blogwatcher
jacky cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
Jacky will use the unified `cronjob` tool internally.
...
jacky cron create "every 1d at 09:00" \
...
```bash
jacky cron edit <job_id> --schedule "every 4h"
jacky cron edit <job_id> --prompt "Use the revised task"
jacky cron edit <job_id> --skill blogwatcher --skill maps
jacky cron edit <job_id> --add-skill maps
jacky cron edit <job_id> --remove-skill blogwatcher
jacky cron edit <job_id> --clear-skills
```
...
```bash
jacky cron list
jacky cron pause <job_id_or_name>
jacky cron resume <job_id_or_name>
jacky cron run <job_id_or_name>
jacky cron remove <job_id_or_name>
jacky cron edit <job_id_or_name> [...flags]
jacky cron status
jacky cron tick
```
...
```bash
jacky gateway install     # Install as a user service
sudo jacky gateway install --system   # Linux: boot-time system service for servers
jacky gateway             # Or run in foreground
...
1. loads jobs from `~/.jacky/cron/jobs.json`
...
A file lock at `~/.jacky/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.jacky/cron/output/`) | Default on CLI |
...
If the agent's final response contains `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.jacky/cron/output/`), but no message is sent to the delivery target.
...
Or set the `JACKY_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 3600s default.
...
`.sh` / `.bash` files run under `/bin/bash`; anything else under the current Python interpreter (`sys.executable`). Scripts must live in `~/.jacky/scripts/` (same sandboxing rule as the pre-run script gate).

### The agent sets these up for you

The `cronjob` tool's schema exposes `no_agent` to Jacky directly, so you can describe a watchdog in chat and let the agent wire it up:

```text
Ping me on Telegram if RAM is over 85%, every 5 minutes.
```

Jacky will write the check script to `~/.jacky/scripts/` via `write_file`, then call:
...
cronjob(
    action="create",
    prompt="Fetch the top 10 AI/ML stories from Hacker News. Save them to ~/.jacky/data/briefs/raw.md in markdown format with title, URL, and score.",
    schedule="0 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.jacky/data/briefs/raw.md. Score each story 1–10 for engagement potential and novelty. Output the top 5 to ~/.jacky/data/briefs/ranked.md.",
    schedule="30 7 * * *",
...
cronjob(
    action="create",
    prompt="Read ~/.jacky/data/briefs/ranked.md. Write 3 tweet drafts (hook + body + hashtags). Deliver to telegram:7976161601.",
    schedule="0 8 * * *",
...
- When Job 2 fires, Jacky reads Job 1's most recent output from `~/.jacky/cron/output/{job1_id}/*.md`
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `jacky tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `jacky tools` cron-platform config wins; otherwise Jacky falls back to the built-in defaults. This matters for cost control: carrying `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether Jacky should even invoke the agent. Emit a final stdout line of the form:
...
```bash
#!/bin/bash
# ~/.jacky/scripts/feed-changed.sh
FEED="$HOME/data/feed.json"
STATE="$HOME/.jacky/scripts/.feed-changed.last"
...
:::tip
Jacky's own `~/.jacky/state.db` is an internal schema that changes between releases. Don't query it from a pre-run gate — point at your own database or feed instead.
:::

Credit: this recipe set was prompted by @iankar8's exploration in [#2654](https://github.com/jaswanthsai1/jacky-cli/pull/2654), which proposed adding sql/file/command triggers as a parallel mechanism. The `script` + `wakeAgent` gate already covers all three cases at $0, so the work landed as documentation instead.
...
Jobs are stored in `~/.jacky/cron/jobs.json`. Output from job runs is saved to `~/.jacky/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, Jacky resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.

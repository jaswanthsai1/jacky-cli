...
Use MoA when a hard task benefits from multiple model perspectives but still needs Jacky' normal agent loop: tool calls, follow-up iterations, interrupts, transcript persistence, and the same session context as any other message.
...
MoA presets are selectable on **every Jacky surface**, because MoA is a normal provider in the model system:
...
- **`jacky model`** and the **Dashboard model picker** — a `Mixture of Agents` provider row appears with your preset names as its models.
...
Jacky temporarily switches to the default MoA preset for that one turn, sends the prompt, then restores your previous model afterward. The whole argument is the prompt — `/moa` no longer interprets it as a preset name.
...
For each main model call when provider `moa` is selected, Jacky:

1. resolves the selected preset by name;
2. runs the configured reference models without tool schemas (they receive only the conversation's user/assistant text — not the Jacky system prompt or tool-call transcript — so reference calls stay cheap and avoid strict-provider rejections);
3. appends the reference outputs as private context for the aggregator;
4. calls the configured aggregator with the normal Jacky tool schema;
5. treats the aggregator response as the real model response;
6. if the aggregator calls tools, Jacky executes those tools normally;
...
moa:
  ...
  presets:
    default:
      ...
      # the same behavior as a single-model Jacky agent.
...
```bash
jacky moa list
jacky moa configure              # update the default preset
jacky moa configure review       # create or update a named preset
jacky moa delete review
```
...
So MoA does not sacrifice prompt caching on either call type. Its only real cost is the extra reference calls per iteration — you pay for multiple model perspectives, not for broken caches. The long-lived conversation prefix shared with the rest of Jacky is fully intact.

## Notes

- MoA is no longer listed under `jacky tools`; there is no `moa` toolset to enable.
...
- Credential failures on one reference model do not abort the turn. Jacky includes the failure in the reference context and continues with whatever models returned.

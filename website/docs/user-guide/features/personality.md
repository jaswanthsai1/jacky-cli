...
If the file is empty, whitespace-only, or cannot be read, Jacky falls back to a built-in default identity ("You are Jacky, an intelligent AI agent CLI..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).

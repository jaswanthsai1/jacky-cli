...
description: "Customize Jacky Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Jacky Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `JACKY_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Jacky is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Jacky now seeds a default `SOUL.md` automatically in:

```text
~/.jacky/SOUL.md
```

More precisely, it uses the current instance's `JACKY_HOME`, so if you run Jacky with a custom home directory, it will use:
...
- Jacky creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Jacky loads `SOUL.md` only from `JACKY_HOME`
- Jacky does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Jacky falls back to a built-in default identity
...
If Jacky loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `JACKY_HOME`, the personality belongs to the Jacky instance itself.

That also makes it easier to teach users:
- "Edit `~/.jacky/SOUL.md` to change Jacky' default personality."
...
- how Jacky should handle uncertainty, disagreement, or ambiguity
...
## What Jacky injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Jacky falls back to a built-in default identity ("You are Jacky Agent, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Jacky ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Jacky, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Jacky its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.jacky/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.jacky/SOUL.md`
...
- [SOUL.md Guide](/guides/use-soul-with-jacky)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Jacky speaks
- `display.skin` and `/skin` affect how Jacky looks in the terminal

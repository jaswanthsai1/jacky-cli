...
Jacky can optionally hand `openai/*` and `openai-codex/*` turns to the [Codex CLI app-server](https://github.com/openai/codex) instead of running its own tool loop. When enabled, terminal commands, file edits, sandboxing, and MCP tool calls all execute inside Codex's runtime — Jacky becomes the shell around it (sessions DB, slash commands, gateway, memory and skill review).

This is **opt-in only**. Default Jacky behavior is unchanged unless you flip the flag. Jacky never auto-routes you onto this runtime.

:::tip
Not using OpenAI Codex? `jacky setup --portal` configures a non-Codex backend with Claude/Gemini/etc. in one step. See [Nous Portal](/integrations/nous-portal).
:::
...
- **Native Codex plugins** — Linear, GitHub, Gmail, Calendar, Canva, etc. — installed via `codex plugin` are auto-migrated and active in your Jacky session.
- **Jacky' richer tools come along** — web_search, web_extract, browser automation, vision, image generation, skills, and TTS work via an MCP callback. Codex calls back into Jacky for tools it doesn't have built in.
- **Memory and skill nudges keep working** — Codex's events are projected into Jacky' message shape so the self-improvement loop sees a normal-looking transcript.
...
These ship with `codex app-server` itself — no Jacky involvement, no MCP, no plugins. All five are available the moment the runtime starts:
...
- **`update_plan`** — codex's internal todo / plan tracker. Equivalent of Jacky' `todo` tool, but managed entirely inside codex's runtime.
- **`view_image`** — load a local image file into the conversation so the model can see it.
- **`web_search`** — codex has its own built-in web search when configured. Jacky also exposes `web_search` (Firecrawl-backed) via the callback below; the model picks whichever it prefers.
...
When you enable the runtime, Jacky queries codex's `plugin/list` RPC and writes a `[plugins."<name>@openai-curated"]` entry for every plugin you have installed. The plugins themselves are managed by codex and authorized once via codex's own UI.
...
### 3. Jacky tool callback (MCP server, registered in `~/.codex/config.toml`)

Jacky registers itself as an MCP server so codex can call back for tools codex doesn't ship with. Available via the callback:
...
- **`image_generate`** — image generation through Jacky' image_gen plugin chain.
- **`skill_view` / `skills_list`** — read from Jacky' skill library.
- **`text_to_speech`** — TTS through Jacky' configured provider.

When the model wants one of these, codex spawns the `jacky_tools_mcp_server` subprocess via stdio MCP, the call is dispatched through `model_tools.handle_function_call()` (same code path as Jacky' default runtime), and the result is returned to codex like any other MCP response.

### What's NOT available on this runtime

These four Jacky tools require the running AIAgent context (mid-loop state) to dispatch, and a stateless MCP callback can't drive them. Switch back to the default runtime (`/codex-runtime auto`) when you need any of them:

- **`delegate_task`** — spawn subagents
- **`memory`** — Jacky' persistent memory store
- **`session_search`** — cross-session search
- **`todo`** — Jacky' todo store (codex's `update_plan` is the in-runtime equivalent)
...
**One thing to be aware of:** each continuation prompt is a fresh codex turn, which means codex re-evaluates command approval policy from scratch. If you're doing a long-running goal with lots of writes, expect more approval prompts than you'd see on a single in-session task. Set `default_permissions = ":workspace"` (which Jacky does automatically when you enable the runtime) so simple workspace writes don't require prompting.

### Kanban (multi-agent worktree dispatch)

**Works on this runtime, with one subtle dependency.** The kanban dispatcher spawns each worker as a separate `jacky chat -q` subprocess that reads the user's config — which means if `model.openai_runtime: codex_app_server` is set globally, workers also come up on the codex runtime.
...
- The Jacky tool callback for browser_*, vision, image_gen, skills, TTS

What also works because the MCP callback exposes them:
- **`kanban_complete` / `kanban_block` / `kanban_comment` / `kanban_heartbeat`** — the worker handoff tools. These read `JACKY_KANBAN_TASK` from env (set by the dispatcher), gate access correctly, and write to the per-board SQLite DB pinned by `JACKY_KANBAN_DB`. Without these in the callback, a worker on this runtime could do its task but couldn't report back, hanging until the dispatcher's timeout.
...
The kanban tools are gated by `JACKY_KANBAN_TASK` env var the dispatcher sets — that var is propagated to the codex subprocess (codex inherits env) and from there to the spawned `jacky-tools` MCP server subprocess. So the tools see the right task id and gate correctly. For Codex app-server workers, Jacky also passes narrow app-server sandbox overrides when `JACKY_KANBAN_TASK` is present: keep `workspace-write` sandboxing, add the **board DB directory plus every Kanban path the dispatcher pinned** as extra writable roots (`JACKY_KANBAN_WORKSPACES_ROOT`, `JACKY_KANBAN_WORKSPACE`, legacy `JACKY_KANBAN_ROOT` — deduplicated, DB-dir first), and keep network disabled by default. This avoids the brittle `:danger-no-sandbox` workaround while letting `kanban_complete` / `kanban_block` update the board DB **and** letting workers write reports/artifacts under workspace mounts that live outside the DB directory (e.g. `/media/.../kanban-workspaces/...` on a separate drive — [issue #27941](https://github.com/jaswanthsai1/jacky-cli/issues/27941)).
...
|  | Jacky default runtime | Codex app-server (opt-in) |
|---|---|---|
...
2. **Codex OAuth login.** The codex subprocess reads `~/.codex/auth.json`. Two ways to populate it:
   ...
   ```
   Jacky' own `jacky auth login codex` writes to `~/.jacky/auth.json` — that's a separate session. **Run `codex login` separately** if you haven't.

3. **(Optional) Install the Codex plugins you want.** When you enable the runtime, Jacky auto-migrates whichever curated plugins you've already installed via Codex CLI:
   ```bash
   codex plugin marketplace add openai-curated
   # then via codex's TUI, install Linear / GitHub / Gmail / etc.
   ```
   Jacky will discover them and write `[plugins."<name>@openai-curated"]` entries to `~/.codex/config.toml` automatically.
...
- Migrates user MCP servers from `~/.jacky/config.yaml` to `~/.codex/config.toml`.
...
- **Registers Jacky' own tools as an MCP server** so the codex subprocess can call back for tools codex doesn't ship with.
...
You can also set it manually in `~/.jacky/config.yaml`:
```yaml
model:
  openai_runtime: codex_app_server   # default is "auto" (= Jacky runtime)
```

## Self-improvement loop (memory + skill nudges)

Jacky' background self-improvement fires on counter thresholds:
...
**Both keep working on the codex runtime.** The codex path projects each completed `commandExecution` / `fileChange` / `mcpToolCall` / `dynamicToolCall` item into a synthetic `assistant tool_call` + `tool` result message, so by the time the review runs it sees the same shape it sees on the default Jacky runtime.
...
One detail: the review fork itself needs to call Jacky' agent-loop tools (`memory`, `skill_manage`), which require Jacky' own dispatch. So when the parent agent is on `codex_app_server`, the review fork is **downgraded to `codex_responses`** — same OAuth credentials, same `openai-codex` provider, but talks to OpenAI's Responses API directly so Jacky owns the loop and the agent-loop tools work. This is invisible to the user.

Net effect: enable the codex runtime and your memory + skill nudges keep firing exactly as they would otherwise.

## How approvals work

Codex requests approval before executing commands or applying patches. These get translated into Jacky' standard "Dangerous Command" prompt:
...
For `apply_patch` (file edit) approvals, Jacky shows a summary of what changed (`1 add, 1 update: /tmp/new.py, /tmp/old.py`) when codex provides the data via the corresponding `fileChange` item.

## Permission profiles

Codex has three built-in permission profiles:
- `:read-only` — no writes; every shell command requires approval
- `:workspace` — writes within the current workspace allowed without prompts (Jacky' default when you enable the runtime)
- `:danger-no-sandbox` — no sandbox at all (don't use this unless you understand it)

You can override the default in `~/.codex/config.toml` outside Jacky' managed block:

```toml
default_permissions = ":read-only"
```

(Jacky will preserve your override on re-migration as long as it lives outside the `# managed by jacky-agent` markers.)

## Auxiliary tasks and ChatGPT subscription token cost

When this runtime is on with the `openai-codex` provider, **auxiliary tasks (title generation, context compression, vision auto-detect, the background self-improvement review fork) also flow through your ChatGPT subscription by default**, because Jacky' auxiliary client uses the main provider/model when no per-task override is set.
...
To route specific aux tasks to a cheaper / different model, set explicit overrides in `~/.jacky/config.yaml`:
...
The self-improvement review fork inherits the main runtime via `_current_main_runtime()` and Jacky downgrades it from `codex_app_server` to `codex_responses` automatically (so the fork can actually call `memory` and `skill_manage` — Jacky' own agent-loop tools). That fork still uses your subscription auth unless you've routed aux tasks elsewhere.

## Editing `~/.codex/config.toml` safely

Jacky wraps everything it manages between two marker comments:

```toml
# managed by jacky-agent — `jacky codex-runtime migrate` regenerates this section
default_permissions = ":workspace"
[mcp_servers.filesystem]
...
[plugins."github@openai-curated"]
...
# end jacky-agent managed section
```
...
- Add your own MCP servers Jacky doesn't know about
...
By default, Jacky points the codex subprocess at `~/.codex/` regardless of which Jacky profile is active. This means `jacky -p work` and `jacky -p personal` share the same Codex auth, plugins, and config. For most users this is the right behavior — it matches what running `codex` CLI directly would do.

If you want per-profile Codex isolation (separate auth, separate installed plugins, separate config), set `CODEX_HOME` explicitly per profile. The cleanest way is to point at a directory under your `JACKY_HOME`:

```bash
# Inside the work profile, you might wrap jacky:
CODEX_HOME=~/.jacky/profiles/work/codex jacky chat
```

You'll need to re-run `codex login` once with that `CODEX_HOME` set so the OAuth tokens land in the profile-scoped location. After that, `jacky -p work` will operate on isolated Codex state.
...
Jacky does NOT rewrite `HOME` when spawning the codex app-server subprocess (we use `os.environ.copy()` and only overlay `CODEX_HOME` and `RUST_LOG`). This means:
...
Jacky' `mcp_servers` config is auto-translated to the TOML format Codex expects. The migration runs every time you enable the runtime and is idempotent — re-runs replace the managed section but preserve any user-edited Codex config.

What translates:

| Jacky (`config.yaml`) | Codex (`config.toml`) |
|---|---|
...
What's not migrated:
- Jacky-specific keys like `sampling` (Codex's MCP client has no equivalent — these are dropped with a per-server warning).

## Native Codex plugin migration

Plugins installed via `codex plugin` (Linear, GitHub, Gmail, Calendar, Canva, etc.) are discovered through Codex's `plugin/list` RPC. For each plugin where `installed: true`, Jacky writes a `[plugins."<name>@openai-curated"]` block enabling it in your Jacky session.

This means: when your friend says "I have Calendar and GitHub set up in my Codex CLI" and they enable Jacky' codex runtime, Jacky activates those automatically. No re-configuration needed.
...
- Plugin OAuth — you authorize each plugin once in Codex itself; Jacky doesn't touch credentials.

## Jacky tool callback (the new MCP server)

Codex's built-in toolset covers shell/file ops/patches but doesn't have web search, browser automation, vision, image generation, etc. To keep those usable in a codex turn, Jacky registers itself as an MCP server in `~/.codex/config.toml`:

```toml
[mcp_servers.jacky-tools]
command = "/path/to/python"
args = ["-m", "agent.transports.jacky_tools_mcp_server"]
env = { JACKY_HOME = "/your/.jacky", PYTHONPATH = "...", JACKY_QUIET = "1" }
startup_timeout_sec = 30.0
tool_timeout_sec = 600.0
```

When the model calls `web_search` (or another exposed Jacky tool), codex spawns the `jacky_tools_mcp_server` subprocess via stdio, the request is dispatched through `model_tools.handle_function_call()`, and the result is projected back to codex like any other MCP response.
...
**Tools NOT available:** `delegate_task`, `memory`, `session_search`, `todo`. These need the running AIAgent context to dispatch (mid-loop state) and a stateless MCP callback can't drive them. Use the default Jacky runtime (`/codex-runtime auto`) when you need these.
...
This runtime is **opt-in beta**. Working as of Jacky Agent 2026.5 + Codex CLI 0.130.0:

- Multi-turn conversations
- `commandExecution` and `fileChange` (apply_patch) approvals via Jacky UI
- MCP tool calls (verified against `@modelcontextprotocol/server-filesystem` and the new `jacky-tools` callback)
...
- Jacky web_search through codex (verified live: "OpenAI Codex CLI – Getting Started" returned end-to-end)

Known limitations:

- **Jacky auth and codex auth are separate sessions.** You need both `codex login` AND `jacky auth login codex` for the cleanest UX (the runtime uses codex's session for the LLM call). This is a deliberate design choice in Jacky' `_import_codex_cli_tokens` — Jacky won't share OAuth state with codex CLI to avoid clobbering each other on token refresh.
...
If you find a bug, [open an issue](https://github.com/jaswanthsai1/jacky-cli/issues) with the output of `jacky logs --since 5m`. Mention `codex-runtime` in the title so it's easy to triage.

## Architecture

```
                ┌─── Jacky shell (CLI / TUI / gateway) ───┐
        ...
        └──────────────────────────────────┘           │
                                                        ...
                                                        ▼
        ┌──────────────────────────────────────────────────────────┐
        │  jacky_tools_mcp_server.py (subprocess on demand)        │
        │   web_search, web_extract, browser_*, vision_analyze,    │
        │   image_generate, skill_view, skills_list, text_to_speech│
        └──────────────────────────────────────────────────────────┘
```

For implementation details, see [PR #24182](https://github.com/jaswanthsai1/jacky-cli/pull/24182) and the [Codex app-server protocol README](https://github.com/openai/codex/blob/main/codex-rs/app-server/README.md).

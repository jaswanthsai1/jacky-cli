...
Jacky runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Jacky
...
1. Jacky captures a baseline of current diagnostics for the file.
...
…). Jacky auto-detects the binary on PATH or in
`<JACKY_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Jacky at the extracted bundle — the directory that contains
   `PowerShellEditorServices/Start-EditorServices.ps1`. Either:
   - set `lsp.servers.powershell.command: ["/path/to/bundle"]` in
     `config.yaml`, or
   - extract it to `<JACKY_HOME>/lsp/PowerShellEditorServices`, or
   - export `PSES_BUNDLE_PATH=/path/to/bundle`.

`jacky lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Jacky installs both packages together when you
run `jacky lsp install typescript` or auto-install fires on first
use.

## CLI

```
jacky lsp status          # service state + per-server install status
jacky lsp list            # registry, optionally --installed-only
jacky lsp install <id>    # eagerly install one server
jacky lsp install-all     # try every server with a known recipe
jacky lsp restart         # tear down running clients
jacky lsp which <id>      # print resolved binary path
```

`jacky lsp status` is the best starting point — it shows which
...
lsp:
  ...
  #   auto    — install via npm/pip/go install into <JACKY_HOME>/lsp/bin
...
When `install_strategy: auto`, Jacky installs binaries into
`<JACKY_HOME>/lsp/bin/`. NPM packages land in
`<JACKY_HOME>/lsp/node_modules/` with bin symlinks one level up.
Go binaries come from `go install` with `GOBIN` pointed at the
staging dir.

Nothing is ever installed to `/usr/local/`, `~/.local/`, or any other
shared location — the staging dir is fully Jacky-owned and is
...
Servers are kept alive for the life of the Jacky process. There's
...
**`jacky lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<JACKY_HOME>/lsp/bin/`. Run
`jacky lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `jacky lsp status`**
...
When `jacky lsp status` shows a `Backend warnings` section, install
...
`~/.jacky/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.jacky/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `jacky lsp restart` to clear the set;

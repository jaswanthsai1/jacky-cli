...
title: "Plugins"
description: "Extend Jacky with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

Jacky has a plugin system for adding custom tools, hooks, and integrations without modifying core code.
...
[Adding Tools](/developer-guide/adding-tools) page is for built-in Jacky
core tools that live in `tools/` and `toolsets.py`.

**→ [Build a Jacky Plugin](/developer-guide/plugins)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.jacky/plugins/` with a `plugin.yaml` and Python code:
...
Start Jacky — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.jacky/plugins/hello-world/plugin.yaml`**
...
**`~/.jacky/plugins/hello-world/__init__.py`**

```python
"""Minimal Jacky plugin — registers a tool and a hook."""
...
Drop both files into `~/.jacky/plugins/hello-world/`, restart Jacky, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.jacky/plugins/` are disabled by default. Enable them only for trusted repositories by setting `JACKY_ENABLE_PROJECT_PLUGINS=true` before starting Jacky.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `jacky <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `jacky plugins install` |
| Distribute via pip | `[project.entry-points."jacky_agent.plugins"]` |
...
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with Jacky — see [Built-in Plugins](/user-guide/features/built-in-plugins) |
| User | `~/.jacky/plugins/` | Personal plugins |
| Project | `.jacky/plugins/` | Project-specific plugins (requires `JACKY_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `jacky_agent.plugins` entry_points | Distributed packages |
| Nix | `services.jacky-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

### Plugin sub-categories

Within each source, Jacky also recognizes sub-category directories that route plugins to specialized discovery systems:
...
User plugins at `~/.jacky/plugins/model-providers/<name>/` and `~/.jacky/plugins/memory/<name>/` override bundled plugins of the same name — last-writer-wins in `register_provider()` / `register_memory_provider()`. Drop a directory in, and it replaces the built-in without any repo edits.

## Plugins are opt-in (with a few exceptions)

**General plugins and user-installed backends are disabled by default** — discovery finds them (so they show up in `jacky plugins` and `/plugins`), but nothing with hooks or tools loads until you add the plugin's name to `plugins.enabled` in `~/.jacky/config.yaml`. This stops third-party code from running without your explicit consent.
...
```bash
jacky plugins                    # interactive toggle (space to check/uncheck)
jacky plugins enable <name>      # add to allow-list
jacky plugins disable <name>     # remove from allow-list + add to disabled
```

After `jacky plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### What the allow-list does NOT gate

Several categories of plugin bypass `plugins.enabled` — they're part of Jacky' built-in surface and would break basic functionality if gated off by default:
...
| **User-installed platforms** (under `~/.jacky/plugins/platforms/`) | Opt-in via `plugins.enabled` — third-party gateway adapters need explicit consent. |

In short: **bundled "always-works" infrastructure loads automatically; third-party general plugins are opt-in.** The `plugins.enabled` allow-list is the gate specifically for arbitrary code a user drops into `~/.jacky/plugins/`.

### Migration for existing users

When you upgrade to a version of Jacky that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.jacky/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled standalone plugins are NOT grandfathered — even existing users have to opt in explicitly. (Bundled platform/backend plugins never needed grandfathering because they were never gated.)
...
Jacky has four kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.jacky/plugins/` |
...
The table above shows the four plugin categories, but within "General plugins" the `PluginContext` exposes several distinct extension points — and Jacky also accepts extensions outside the Python plugin system (config-driven backends, shell-hooked commands, external servers, etc.). Use this table to find the right doc for what you want to build:

| Want to add… | How | Authoring guide |
|---|---|---|
| A **tool** the LLM can call | Python plugin — `ctx.register_tool()` | [Build a Jacky Plugin](/developer-guide/plugins) · [Adding Tools](/developer-guide/adding-tools) |
| A **lifecycle hook** (pre/post LLM, session start/end, tool filter) | Python plugin — `ctx.register_hook()` | [Hooks reference](/user-guide/features/hooks) · [Build a Jacky Plugin](/developer-guide/plugins) |
| A **slash command** for the CLI / gateway | Python plugin — `ctx.register_command()` | [Build a Jacky Plugin](/developer-guide/plugins) · [Extending the CLI](/developer-guide/extending-the-cli) |
| A **subcommand** for `jacky <thing>` | Python plugin — `ctx.register_cli_command()` | [Extending the CLI](/developer-guide/extending-the-cli) |
...
| An **STT backend** (any CLI — whisper.cpp, custom whisper binary, local ASR CLI) | Config-driven (recommended) — declare under `stt.providers.<name>` with `type: command` in `config.yaml`, or set `JACKY_LOCAL_STT_COMMAND` for the legacy single-command escape hatch. OR Python backend plugin — `ctx.register_transcription_provider()` for Python-SDK engines (OpenRouter, SenseAudio, Gemini-STT, etc.). | [STT Setup](/user-guide/features/tts#stt-custom-command-providers) · [Python plugin guide](/user-guide/features/tts#python-plugin-providers-stt) |
| **External tools via MCP** (filesystem, GitHub, Linear, Notion, any MCP server) | Config-driven — declare `mcp_servers.<name>` with `command:` / `url:` in `config.yaml`. Jacky auto-discovers the server's tools and registers them alongside built-ins. | [MCP](/user-guide/features/mcp) |
| **Additional skill sources** (custom GitHub repos, private skill indexes) | CLI — `jacky skills tap add <repo>` | [Skills Hub](/user-guide/features/skills#skills-hub) · [Publishing a custom tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
| **Gateway event hooks** (fire on `gateway:startup`, `session:start`, `agent:end`, `command:*`) | Drop `HOOK.yaml` + `handler.py` into `~/.jacky/hooks/<name>/` | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) |
...
On NixOS, plugins can be installed declaratively via the module options — no `jacky plugins install` needed. See the **[Nix Setup guide](/getting-started/nix-setup#plugins)** for full details.
...
```bash
jacky plugins                               # unified interactive UI
jacky plugins list                          # table: enabled / disabled / not enabled
jacky plugins install user/repo             # install from Git, then prompt Enable? [y/N]
jacky plugins install user/repo --enable    # install AND enable (no prompt)
jacky plugins install user/repo --no-enable # install but leave disabled (no prompt)
jacky plugins update my-plugin              # pull latest
jacky plugins remove my-plugin              # uninstall
jacky plugins enable my-plugin              # add to allow-list
jacky plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `jacky plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `jacky plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.

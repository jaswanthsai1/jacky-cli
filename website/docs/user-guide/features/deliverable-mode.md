...
When Jacky Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.jacky/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.jacky/config.yaml` (switchable per session
via `/personality`).
...
If you use Jacky' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.jacky/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Jacky Agent's deliverable

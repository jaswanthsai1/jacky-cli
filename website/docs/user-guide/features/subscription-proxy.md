...
chat completions — use your Jacky-managed provider subscription as their
...
| Use case | "Use Jacky as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Jacky stores
the refresh token in `~/.jacky/auth.json` — the same place all Jacky
...
interface in `jacky_cli/proxy/adapters/`.
...
If you see `not logged in`, run `jacky portal`. If you see
...
`jacky_cli/proxy/adapters/<provider>.py` and registering it in

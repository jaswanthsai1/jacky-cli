...
sidebar_position: 2
title: "Configuration"
description: "Configure Jacky Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.jacky/` directory for easy access.

:::tip Easiest path to a working `config.yaml`
Run `jacky setup --portal` — one OAuth gets you a model provider and all four Tool Gateway tools without hand-editing YAML. Portal subscribers also get 10% off token-billed providers. See [Nous Portal](/integrations/nous-portal).
:::
...
```bash
jacky config              # View current configuration
jacky config edit         # Open config.yaml in your editor
jacky config set KEY VAL  # Set a specific value
jacky config check        # Check for missing options (after updates)
jacky config migrate      # Interactively add missing options

# Examples:
jacky config set model anthropic/claude-opus-4
jacky config set terminal.backend docker
jacky config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `jacky config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `jacky chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.jacky/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.jacky/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
You can set `providers.<id>.request_timeout_seconds` for a provider-wide request timeout, plus `providers.<id>.models.<model>.timeout_seconds` for a model-specific override. Applies to the primary turn client on every transport (OpenAI-wire, native Anthropic, Anthropic-compatible), the fallback chain, rebuilds after credential rotation, and (for OpenAI-wire) the per-request timeout kwarg — so the configured value wins over the legacy `JACKY_API_TIMEOUT` env var.

You can also set `providers.<id>.stale_timeout_seconds` for the non-streaming stale-call detector, plus `providers.<id>.models.<model>.stale_timeout_seconds` for a model-specific override. This wins over the legacy `JACKY_API_CALL_STALE_TIMEOUT` env var.

Leaving these unset keeps the legacy defaults (`JACKY_API_TIMEOUT=1800`s, `JACKY_API_CALL_STALE_TIMEOUT=90`s, native Anthropic 900s). The non-streaming stale detector is auto-disabled for local endpoints when left implicit and can scale upward for very large contexts. Not currently wired for AWS Bedrock (both `bedrock_converse` and AnthropicBedrock SDK paths use boto3 with its own timeout configuration). See the commented example in [`cli-config.yaml.example`](https://github.com/jaswanthsai1/jacky-cli/blob/main/cli-config.yaml.example).

## Update Behavior

`jacky update` settings live under `updates` in `config.yaml`:

```yaml
updates:
  pre_update_backup: false       # Create a full JACKY_HOME zip before every update
  backup_keep: 5                 # Keep this many pre-update backup zips
  non_interactive_local_changes: stash  # stash | discard
```

For git installs, Jacky auto-stashes dirty tracked files and untracked files before checking out the update branch or pulling. Interactive terminal updates prompt before restoring that stash. Non-interactive updates (desktop/chat app, gateway, or `--yes`) use `updates.non_interactive_local_changes`: `stash` restores local source edits after a successful pull, while `discard` drops the update-created stash after a successful pull. Use `discard` only on managed installs where local source edits are never meant to persist.

Before that stash step, Jacky also restores tracked `package-lock.json` diffs left by npm install/build churn. Commit or manually stash intentional lockfile edits before updating.

## Terminal Backend Configuration

Jacky supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox (direct or via the Nous-managed gateway), a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Jacky will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
find the credentials and config they already use in your normal shell. Jacky
state is still profile-scoped through `JACKY_HOME`; `HOME` is not how profiles
select config, memory, sessions, or skills.

Jacky does **not** change your system-wide `HOME`, your shell startup files, or
the operating system account home. This setting only controls the environment
passed to subprocesses that Jacky launches through tools such as `terminal`,
...
|---|---|---|---|
| `auto` | Keep the real OS-user `HOME` | Use `{JACKY_HOME}/home` | Recommended default. Host CLIs keep working; container state persists. |
...
| `profile` | Use `{JACKY_HOME}/home` when it exists | Use `{JACKY_HOME}/home` when it exists | Strict per-profile CLI config isolation, but normal `~/.ssh`, `~/.gitconfig`, `~/.azure`, `~/.config/gh`, Claude/Codex auth, npm state, etc. will not be visible unless you initialize or link them inside the profile home. |
...
In that mode tool subprocesses use `{JACKY_HOME}/home` as `HOME`. Jacky also
sets `JACKY_REAL_HOME` so scripts can still locate the actual user home when
they need it. Container backends keep using `{JACKY_HOME}/home` in `auto` mode
because that directory lives on the persistent Jacky data volume.

Scripts that need to distinguish profile state from the real user home should
prefer `JACKY_HOME` for Jacky data and `JACKY_REAL_HOME` for the account home:

```python
from pathlib import Path
import os

jacky_home = Path(os.environ["JACKY_HOME"])
real_home = Path(os.environ.get("JACKY_REAL_HOME", os.environ["HOME"]))
```

:::warning
The agent has the same filesystem access as your user account. Use `jacky tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Single persistent container, shared across Jacky processes.** Jacky starts ONE long-lived container on first use and routes every terminal, file, and `execute_code` call through `docker exec` into that same container — across sessions, `/new`, `/reset`, and `delegate_task` subagents. Working-directory changes, installed packages, files in `/workspace`, and **background processes** all carry over from one tool call to the next, and from one Jacky process to the next. When you close a TUI session, run `/quit`, or start a new `jacky` invocation, the container keeps running and the next Jacky process reuses it via a labeled lookup. See **Container lifecycle** below for the exact teardown rules.
...
terminal:
  ...
  docker_persist_across_processes: true   # Reuse container across Jacky restarts
...
**`docker_env`** vs **`docker_forward_env`**: the former injects literal `KEY=value` pairs you specify in the config (the values live in your `config.yaml` or are passed as a JSON dict via `TERMINAL_DOCKER_ENV='{"DEBUG":"1"}'`). The latter forwards values from your shell or `~/.jacky/.env`, so the actual secret never appears in the config file. Use `docker_forward_env` for tokens and `docker_env` for static knobs the container needs.

**`terminal.docker_extra_args`** (also overridable via `TERMINAL_DOCKER_EXTRA_ARGS='["--gpus=all"]'`) lets you pass arbitrary `docker run` flags that Jacky doesn't surface as first-class keys — `--gpus`, `--network`, `--add-host`, alternative `--security-opt` overrides, etc. Each entry must be a string; the list is appended last to the assembled `docker run` invocation so it can override Jacky' defaults if needed. Use sparingly — flags that conflict with the sandbox hardening (capability drops, `--user`, the workspace bind mount) will silently weaken isolation.

**`terminal.docker_network`** (default `true`; env: `TERMINAL_DOCKER_NETWORK`) — set to `false` to run the sandbox container with `--network=none`, cutting off all network egress from agent commands. This applies to the execution container used by `terminal`, `execute_code`, and the file tools. Because containers persist across Jacky processes, flipping this to `false` while an older networked container exists will remove that container and start a fresh air-gapped one (a warning is logged); background processes running inside it are lost. Prefer this key over passing `--network=none` through `docker_extra_args`.

**Requirements:** Docker Desktop or Docker Engine installed and running. Jacky probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle). Podman is supported out of the box: set `JACKY_DOCKER_BINARY=podman` (or the full path) to force it when both are installed.

#### Container lifecycle

Every Jacky-managed container is tagged with three labels so subsequent processes (and the orphan reaper) can identify it:

- `jacky-agent=1` — marks it as Jacky-managed
- `jacky-task-id=<sanitized task_id>` — keys the per-task reuse probe
- `jacky-profile=<sanitized profile name>` — scopes reuse and reaping to the active Jacky profile

On startup, Jacky runs `docker ps --filter label=jacky-task-id=<id> --filter label=jacky-profile=<profile>` and **attaches to the existing container** when it finds one. If the container is `exited` (e.g. after a Docker daemon restart), it's `docker start`'d and reused — filesystem state and any installed packages survive, but in-container background processes do not.

When a Jacky process exits — `/quit`, closing a TUI session, gateway shutdown, even SIGKILL — the cleanup path is a **no-op for the container in default mode**. The container keeps running. The next Jacky process attaches to it in milliseconds via the label probe. This is the behavior the "one long-lived container shared across sessions" contract requires: it's the only way background processes (npm watchers, dev servers, long-running pytest) survive across sessions.
...
| Orphan reaper at next startup | Sweeps **Exited** jacky-labeled containers older than `2 × lifetime_seconds` (default 600s = 10 min), scoped to the current profile. **Running containers are never touched** — sibling-process safety. Set `docker_orphan_reaper: false` to disable. |
...
- **Switching profiles** isolates containers from each other — a container labeled `jacky-profile=work` is invisible to a Jacky process running under `jacky-profile=research`. The orphan reaper is profile-scoped too, so cross-profile containers don't get reaped accidentally, but they also won't get cleaned up automatically until you start Jacky again under their original profile.
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.jacky/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
| `JACKY_DOCKER_BINARY` | _none_ | Force a specific docker/podman binary path |
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.jacky/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.jacky/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `jacky-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/jacky-agent` (HPC convention) → `~/.jacky/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `jacky config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. Jacky logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `jacky doctor` to check.
...
For the **SSH**, **Modal**, and **Daytona** backends (anywhere the agent's working tree lives on a different machine than the host running Jacky), Jacky tracks files the agent touched inside the remote sandbox and, on session teardown / sandbox cleanup, **syncs the modified files back to the host** under `~/.jacky/cache/remote-syncs/<session-id>/`.
...
- The remote sandbox may have been torn down by the time you go looking; the local `~/.jacky/cache/remote-syncs/…` copy is the authoritative record of what the agent changed.
...
terminal:
  ...
  docker_volumes:
    ...
    - "/home/user/.jacky/cache/documents:/output" # Gateway-visible exports
```
...
`/home/user/.jacky/cache/documents:/output`.

- Write files inside Docker to `/output/...`
- Emit the **host path** in `MEDIA:`, for example:
  `MEDIA:/home/user/.jacky/cache/documents/report.txt`
...
Jacky resolves each listed variable from your current shell first, then falls back to `~/.jacky/.env` if it was saved with `jacky config set`.
...
When enabled, Jacky appends `--user $(id -u):$(id -g)` to the `docker run` command so files written into bind-mounted directories (`/workspace`, `/root`, anything in `docker_volumes`) are owned by your host user, not root. The trade-off: the container can no longer `apt install` or write to root-owned paths like `/root/.npm` — use a base image whose `HOME` is owned by a non-root user (or add your required tooling at image build time) if you need both.
...
Docker sandboxes stay isolated by default. Jacky does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Jacky from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Jacky from
...
```bash
jacky config set terminal.persistent_shell false
```
...
- `jacky config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `jacky config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
jacky config set skills.config.myplugin.path ~/myplugin-data
```
...
When the agent uses `skill_manage` to create, edit, patch, or delete a skill, Jacky can optionally scan the new/updated content for dangerous keyword patterns (credential harvesting, obvious prompt injection, exfil instructions). The scanner is **off by default** — real agent workflows that legitimately touch `~/.ssh/` or mention `$OPENAI_API_KEY` were tripping the heuristic too often. Turn it back on if you want the scanner to prompt you before the agent's skill writes land:
...
Controls how much content Jacky loads from each automatic context file before applying head/tail truncation. This applies to files injected into the system prompt such as `SOUL.md`, `.jacky.md`, `AGENTS.md`, `CLAUDE.md`, and `.cursorrules`. It does **not** affect the `read_file` tool.
...
Three related caps control how much raw output a tool can return before Jacky truncates it:
...
- **`max_bytes`** — When a `terminal` command produces more than this many characters of combined stdout/stderr, Jacky keeps the first 40% and last 60% and inserts a `[OUTPUT TRUNCATED]` notice between them. Default `50000` (≈12-15K tokens across typical tokenisers).
...
`jacky tools`), so a toolset listed here is always removed — even if a
...
```yaml
worktree: true    # Always create a worktree (same as jacky -w)
...
Jacky automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
Plugin engines are **never auto-activated** — you must explicitly set `context.engine` to the plugin name. Available engines can be browsed and selected via `jacky plugins` → Provider Plugins → Context Engine.

See [Memory Providers](/user-guide/features/memory-providers) for the analogous single-select system for memory plugins.

## Iteration Budget

When the agent is working on a complex task with many tool calls, it can burn through its iteration budget (default: 90 turns). Jacky does **not** inject mid-task pressure warnings — earlier builds warned the model at 70%/90% budget, which caused models to abandon complex tasks prematurely and was removed in April 2026.

Instead, when the budget is actually exhausted (90/90), Jacky injects one message asking the model to wrap up and allows a single **grace call** so it can deliver a final response. If that grace call still doesn't produce text, the agent is asked to summarise what it accomplished.
...
`agent.api_max_retries` controls how many times Jacky retries a provider API call on transient errors (rate limits, connection drops, 5xx) **before** fallback-provider switching engages. The default is `3` — four attempts total. If you have [fallback providers](/user-guide/features/fallback-providers) configured and want to fail over faster, drop this to `0` so the first transient error on your primary immediately hands off to the fallback instead of churning retries against the flaky endpoint.

## Standing Goals (`/goal`)

When a standing goal is active, Jacky judges whether each assistant response satisfies it. If not, it feeds a continuation prompt back into the same session and keeps working until the goal is done, the turn budget is exhausted, or the user pauses/clears it. The turn budget is the real backstop — judge failures fail **open** (continue) so a flaky judge never wedges progress.

```yaml
goals:
  max_turns: 20   # Max continuation turns before Jacky auto-pauses the goal (default: 20)
```

`max_turns` caps how many continuation turns a goal can drive before Jacky auto-pauses it and asks the user to `/goal resume`. It protects against judge false negatives (goal actually done but judge says continue) and unbounded model spend on fuzzy or unachievable goals. See [Goals](/user-guide/features/goals) for the full feature.

### API Timeouts

Jacky has separate timeout layers for streaming, plus a stale detector for non-streaming calls. The stale detectors auto-adjust for local providers only when you leave them at their implicit defaults.

| Timeout | Default | Local providers | Config / env |
|---------|---------|----------------|--------------|
| Socket read timeout | 120s | Auto-raised to 1800s | `JACKY_STREAM_READ_TIMEOUT` |
| Stale stream detection | 180s | Auto-disabled | `JACKY_STREAM_STALE_TIMEOUT` |
| Stale non-stream detection | 300s | Auto-disabled when left implicit | `providers.<id>.stale_timeout_seconds` or `JACKY_API_CALL_STALE_TIMEOUT` |
| API call (non-streaming) | 1800s | Unchanged | `providers.<id>.request_timeout_seconds` / `timeout_seconds` or `JACKY_API_TIMEOUT` |

The **socket read timeout** controls how long httpx waits for the next chunk of data from the provider. Local LLMs can take minutes for prefill on large contexts before producing the first token, so Jacky raises this to 30 minutes when it detects a local endpoint. If you explicitly set `JACKY_STREAM_READ_TIMEOUT`, that value is always used regardless of endpoint detection.
...
The **stale non-stream detection** kills non-streaming calls that produce no response for too long. By default Jacky disables this on local endpoints to avoid false positives during long prefills. If you explicitly set `providers.<id>.stale_timeout_seconds`, `providers.<id>.models.<model>.stale_timeout_seconds`, or `JACKY_API_CALL_STALE_TIMEOUT`, that explicit value is honored even on local endpoints.
...
Jacky turns on cross-session prompt caching automatically when the active provider supports it — no user config needed.

For Claude on **native Anthropic**, **OpenRouter**, and **Nous Portal**, Jacky attaches `cache_control` breakpoints with the 1-hour TTL (`ttl: "1h"`) on the system prompt and skill blocks. The first send within a fresh hour pays full input rates; subsequent sends across any session within the same hour pull from the cache at the discounted cached-read rate. This means the system prompt, loaded skill content, and the early portion of any long-context include get reused across `jacky` sessions and across forked subagents for the first hour.

The Qwen Cloud (Alibaba DashScope) upstream caps cache TTL at 5 minutes, so Jacky uses the 5-minute breakpoint TTL there instead. Other Claude-via-third-party paths (AWS Bedrock, Azure Foundry) fall back to the provider's own caching defaults. xAI Grok uses a separate session-pinned conversation-id mechanism — see [xAI prompt caching](/integrations/providers#xai-grok--responses-api--prompt-caching).
...
The one explicit knob is the cache TTL tier Jacky requests on Anthropic-style breakpoints:
...
`cache_ttl` selects the breakpoint TTL Jacky attaches for Claude via the native Anthropic API, OpenRouter, and Nous Portal. Only the two Anthropic-supported tiers (`"5m"`, `"1h"`) are honored — any other value is ignored. Providers with their own caps (e.g. Qwen Cloud, which maxes at 5 minutes) still clamp to what the upstream allows.

## Auxiliary Models

Jacky uses "auxiliary" models for side tasks like image analysis, web page summarization, browser screenshot analysis, session-title generation, and context compression. By default (`auxiliary.*.provider: "auto"`), Jacky routes every auxiliary task to your **main chat model** — the same provider/model you picked in `jacky model`. You don't need to configure anything to get started, but be aware that on expensive reasoning models (Opus, MiniMax M2.7, etc.) auxiliary tasks add meaningful cost. If you want cheap-and-fast side tasks regardless of your main model, set `auxiliary.<task>.provider` and `auxiliary.<task>.model` explicitly (for example, Gemini Flash on OpenRouter for vision and web extraction).
...
Instead of hand-editing YAML, run `jacky model` and pick **"Configure auxiliary models"** from the menu. You'll get an interactive per-task picker:
...
<div style={{position: 'relative', width: '100%', aspectRatio: '16 / 9', marginBottom: '1.5rem'}}>
  <iframe
    ...
    title="Jacky Agent — Auxiliary Models Tutorial"
...
Every model slot in Jacky — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Jacky ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Jacky uses that provider's built-in auth and base URL.
...
:::tip MiniMax OAuth
`minimax-oauth` logs in via browser OAuth (no API key needed). Run `jacky model` and select **MiniMax (OAuth)** to authenticate. Auxiliary tasks use `MiniMax-M2.7-highspeed` automatically. See the [MiniMax OAuth guide](../guides/minimax-oauth.md).
:::

:::tip xAI Grok OAuth
`xai-oauth` logs in via browser OAuth for SuperGrok and X Premium+ subscribers (no API key needed). Run `jacky model` and select **xAI Grok OAuth (SuperGrok / Premium+)** to authenticate. The same OAuth token is reused for every direct-to-xAI surface (chat, auxiliary tasks, TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md), and if Jacky is on a remote host see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md).
:::
...
auxiliary:
  ...
  # Kanban triage specifier — `jacky kanban specify <id>` (or the
...
Each auxiliary task can optionally define a `fallback_chain` — a list of provider/model entries that Jacky tries when the primary auxiliary provider fails due to rate limits, connectivity issues, or payment restrictions:
...
When the primary auxiliary provider (`openrouter` / `openai/gpt-4o-mini`) returns a rate-limit, connection timeout, or payment-required error, Jacky walks the `fallback_chain` in order. It skips entries whose provider matches the already-failed provider, and tries each remaining entry until one succeeds or the chain is exhausted. If all fallbacks fail, Jacky falls back to the main agent model as a final safety net.
...
The shape mirrors what OpenRouter accepts in the chat completions request body. Jacky forwards the entire `extra_body` verbatim, so any other OpenRouter request-body field documented at [openrouter.ai/docs](https://openrouter.ai/docs) works the same way.
...
Or via environment variable (in `~/.jacky/.env`):
...
| `"nous"` | Force Nous Portal | `jacky auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `jacky model` → Codex |
| `"minimax-oauth"` | Force MiniMax OAuth (browser login, no API key). Uses MiniMax-M2.7-highspeed for auxiliary tasks. | `jacky model` → MiniMax (OAuth) |
| `"xai-oauth"` | Force xAI Grok OAuth (browser login for SuperGrok or X Premium+ subscribers, no API key). Same OAuth token covers chat, TTS, image, video, and transcription. | `jacky model` → xAI Grok OAuth (SuperGrok / Premium+) |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `jacky model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Jacky uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
```
Run `jacky model` and select **MiniMax (OAuth)** to log in and set this automatically. For the China region, the base URL will be `https://api.minimaxi.com/anthropic`. See the [MiniMax OAuth guide](../guides/minimax-oauth.md) for the full walkthrough.
...
`provider: "main"` uses whatever provider Jacky uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `jacky config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
field — OpenRouter ignores it for them. Jacky transparently routes your
...
Jacky detects when the agent is stuck in an unproductive tool-calling loop — the same tool call failing repeatedly, the same tool failing over and over, or an idempotent call returning the same result with no progress. By default it injects a **warning** into the tool result so the model self-corrects; it does not hard-stop, since a person watching the CLI/TUI can intervene.
...
When `display.file_mutation_verifier` is `true` (default), Jacky appends a one-line advisory to the assistant's final response whenever a `write_file` or `patch` call failed during the turn and was never superseded by a successful write to the same path. This catches the "batch of parallel patches, half silently fail, model summarises success" class of over-claim without requiring you to manually run `git status` after every edit.
...
Set `file_mutation_verifier: false` (or `JACKY_FILE_MUTATION_VERIFIER=0`) to suppress the footer. The verifier only fires when real failures are outstanding at turn end — a model that retries a failed patch and succeeds within the same turn will not trigger it for that file.
...
You can also set this per-session with the `JACKY_LANGUAGE` env var, which overrides the config value.
...
When `display.runtime_footer.enabled: true`, Jacky appends a small runtime-context footer to the **final** message of each gateway turn. The current footer can show the model, context-window percentage, and current working directory. Off by default; opt in per-gateway if your team wants every reply to include this provenance.
...
`interim_assistant_messages` is gateway-only. When enabled, Jacky sends completed mid-turn assistant updates as separate chat messages. This is independent from `tool_progress` and does not require gateway streaming.
...
If the requested provider is unavailable, Jacky falls back automatically in this order: `local` → `groq` → `openai`.
...
When the cap is reached, Jacky returns a direct limit message for new sessions.
Existing active sessions keep their normal behavior.

The canonical key is top-level `max_concurrent_sessions`. Jacky also accepts
`gateway.max_concurrent_sessions` as a fallback, but the top-level key wins when
both are set.

The cap is enforced with a local runtime lease file and is best-effort: Jacky
fails open if the registry cannot be read or locked so users are not stranded.
It is intended for a single host/profile runtime, not a shared `$JACKY_HOME`
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Jacky to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Jacky still keys DMs by chat/DM ID as usual.
...
Control what Jacky does when an unknown user sends a direct message:
...
- `pair` is the default for chat-style DM platforms. Jacky denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status jacky-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.jacky/jacky-agent && git pull && pip install -e .
  gpu:
    type: exec
...
- **`strict`** — scripts run in a temp staging directory with `sys.executable` (Jacky's own python). Maximum reproducibility, but project deps and relative paths won't resolve.
...
The `web_search` and `web_extract` tools support five backend providers. Configure the backend in `config.yaml` or via `jacky tools`:
...
**Exa:** Set `EXA_API_KEY` in `~/.jacky/.env`. Supports `category` filtering (`company`, `research paper`, `news`, `people`, `personal site`, `pdf`) and domain/date filters.
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.jacky/browser_recordings/
  # Optional CDP override — when set, Jacky attaches directly to your own
  ...
  camofox:
    ...
    session_key: ""              # Optional session key sent when Jacky creates a tab
...
Control how Jacky handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `JACKY_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: false                 # Enable automatic checkpoints (also: jacky chat --checkpoints). Default: false (opt-in).
...
Jacky uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.jacky/SOUL.md` or `$JACKY_HOME/SOUL.md` |
| `.jacky.md` / `JACKY.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Jacky falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.jacky.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Jacky automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`jacky`)** | Current directory where you run the command |
| **Messaging gateway** | `terminal.cwd` from `~/.jacky/config.yaml`; if unset, home directory `~` |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```yaml
# In ~/.jacky/config.yaml:
terminal:
  cwd: /home/myuser/projects
```

`MESSAGING_CWD` and direct `TERMINAL_CWD` entries in `~/.jacky/.env` are legacy compatibility fallbacks. New configurations should use `terminal.cwd`.
...
- `seen` — internal state. Jacky latches each shown hint here so it never fires again; the profile-build offer is also recorded here once shown. Don't hand-edit it — wipe the whole `onboarding` section if you want to re-see all hints.
...
dashboard:
  ...
  public_url: ""              # Full public authority for OAuth redirect_uri (env: JACKY_DASHBOARD_PUBLIC_URL)
...
- `oauth` / `basic_auth` / `drain_auth` — auth provider config read by the bundled dashboard-auth plugins. The drain secret itself is **not** set here; it's provisioned via the `JACKY_DASHBOARD_DRAIN_SECRET` env var. See [Web Dashboard](/user-guide/features/web-dashboard) for full auth setup.

...
Jacky Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.jacky/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Jacky stores session history so it can resume conversations, but it does not
...
Jacky explicitly injects for that turn.
...
For example, if a user sends an image and asks Jacky to make a meme from it,
Jacky may inspect that image once with vision and run an image-processing
...
`jacky sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`jacky` or `jacky chat`) |
...
jacky -c "my project"   # → resumes "my project #3"
```
...
jacky --resume 20250305_091523_a1b2c3d4
jacky -r 20250305_091523_a1b2c3d4

# Resume by title
jacky --resume "refactoring auth"

# Or with the chat subcommand
jacky chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `jacky sessions list`.

### Conversation Recap on Resume

When you resume a session, Jacky displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Jacky session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.jacky/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `jacky -r "<title>"` from the shell) and pick up where the platform left off.
...
Jacky automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `jacky sessions list` or `jacky sessions browse`.
...
```bash
jacky sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Jacky creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`jacky -c "my project"`), it automatically picks the most recent session in the lineage.
...
Jacky provides a full set of session management commands via `jacky sessions`:
...
# Filter by platform
jacky sessions list --source telegram

# Show more sessions
jacky sessions list --limit 50
```
...
`jacky sessions export` is one surface for every export format, selected with `--format`:
...
jacky sessions export backup.jsonl

# Export sessions from a specific platform
jacky sessions export telegram-history.jsonl --source telegram

# Export a single session
jacky sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
jacky sessions export backup.jsonl --redact
```
...
jacky sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
jacky sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
jacky sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
jacky sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
`--format trace` emits Claude Code JSONL — the transcript shape the Hugging Face Hub auto-detects for its [Agent Trace Viewer](https://huggingface.co/docs/hub/agent-traces). Write it locally, or add `--upload` to push it to your own private `jacky-traces` dataset (reads `HF_TOKEN`):

```bash
# Trace of the most recent session, to stdout
jacky sessions export --format trace

# One session to a local trace file
jacky sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
jacky sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.jacky/session-exports`).

```bash
# Export one session to Markdown
jacky sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
jacky sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
jacky sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
jacky sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
jacky sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
jacky sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
jacky sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
jacky sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
jacky sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
jacky sessions prune

# Custom age threshold — bare numbers are days
jacky sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
jacky sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
jacky sessions prune --newer-than 5h

# Explicit window with absolute timestamps
jacky sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
jacky sessions prune --source telegram
jacky sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
jacky sessions prune --newer-than 5h --title "smoke test"   # title substring
jacky sessions prune --older-than 30 --max-messages 3        # tiny sessions
jacky sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
jacky sessions prune --model gpt-5 --older-than 1w           # by model (substring)
jacky sessions prune --provider openrouter --older-than 60   # by billing provider
jacky sessions prune --branch feature/old-experiment         # by git branch
jacky sessions prune --user 12345678 --chat-type group       # by messaging origin
jacky sessions prune --max-tokens 500 --older-than 7         # by token usage
jacky sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
jacky sessions prune --newer-than 5h --dry-run

# Skip confirmation
jacky sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `jacky sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `jacky sessions prune` keeps the 90-day cutoff. Every
...
`jacky sessions archive` takes the same filters as `prune` but soft-hides
...
jacky sessions archive --newer-than 5h

# Archive by title substring, preview first
jacky sessions archive --title "dry run" --dry-run
jacky sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `jacky sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`jacky sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
jacky sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`jacky insights`](/reference/cli-commands#jacky-insights).
...
When Jacky cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Jacky uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Jacky in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.jacky/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.jacky/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.jacky/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.jacky/sessions/sessions.json` is the **gateway routing index** — it maps
...
`jacky sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.jacky/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `jacky sessions list`, the cause is
`state.db` not receiving them — run `jacky sessions repair` and watch for a
...
`*.jsonl` files in `~/.jacky/sessions/`. They are no longer written or
read by Jacky. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Jacky process in the same `JACKY_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.jacky/config.yaml`:
...
jacky sessions delete <session_id>

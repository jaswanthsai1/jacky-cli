...
title: "Git Worktrees"
description: "Run multiple Jacky agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Jacky Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Jacky so each session has a clean, isolated working directory.

## Why Use Worktrees with Jacky?

Jacky treats the **current working directory** as the project root:

- CLI: the directory where you run `jacky` or `jacky chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.jacky/config.yaml`
...
git worktree add ../repo-feature feature/jacky-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/jacky-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Jacky there:

```bash
cd ../repo-feature

# Start Jacky in the worktree
jacky
```
...
git worktree add ../repo-experiment-a feature/jacky-a
git worktree add ../repo-experiment-b feature/jacky-b
```
...
- Works on its own branch (`feature/jacky-a` vs `feature/jacky-b`).
...
- Jacky checkpoint data under `~/.jacky/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Jacky experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/jacky-checkpoints-docs`, `feature/jacky-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Jacky from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `jacky -w` (Automatic Worktree Mode)

Jacky has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `jacky/jacky-<hash>`).
...
```bash
jacky -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `jacky -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Jacky session its own clean checkout.

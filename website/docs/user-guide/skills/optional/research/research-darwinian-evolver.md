...
| | |
|---|---|
| Source | Optional — install with `jacky skills install official/research/darwinian-evolver` |
| Path | `optional-skills/research/darwinian-evolver` |
| Version | `0.1.0` |
| Author | Bihruze (Asahi0x), Jacky Agent |
| License | MIT |
...
import upstream classes into Jacky itself.
...
```bash
mkdir -p ~/.jacky/cache/darwinian-evolver && cd ~/.jacky/cache/darwinian-evolver
...
```bash
cd ~/.jacky/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.jacky/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.jacky/skills/research/darwinian-evolver
DE_DIR=~/.jacky/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Jacky core.
   Custom driver scripts under `~/.jacky/skills/...` are user-side and fine.

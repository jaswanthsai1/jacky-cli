...
| | |
|---|---|
| Source | Optional — install with `jacky skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${JACKY_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${JACKY_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${JACKY_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client jacky
```
...
```bash
PUBLISH="${JACKY_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client jacky
```
...
```bash
DRIVE="${JACKY_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `jacky`)    |

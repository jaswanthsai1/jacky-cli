...
| | |
|---|---|
| Source | Optional — install with `jacky skills install official/productivity/siyuan` |
...
3. Store it in `${JACKY_HOME:-~/.jacky}/.env`:
   ```
...
```yaml
# In ~/.jacky/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx

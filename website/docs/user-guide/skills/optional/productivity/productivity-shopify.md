...
| | |
|---|---|
| Source | Optional — install with `jacky skills install official/productivity/shopify` |
...
4. Save to `${JACKY_HOME:-~/.jacky}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Jacky","productType":"Apparel","tags":["test"]}}'
```

...
| | |
|---|---|
| Source | Optional — install with `jacky skills install official/productivity/canvas` |
...
4. Name the token (e.g., "Jacky Agent"), set an optional expiry, and click **Generate Token**
5. Copy the token and add to `${JACKY_HOME:-~/.jacky}/.env`:
...
```bash
CANVAS="python $JACKY_HOME/skills/productivity/canvas/scripts/canvas_api.py"

...
| | |
|---|---|
| Source | Optional — install with `jacky skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Jacky Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Jacky terminal calls (desktop app flow only)
...
### Service Account (recommended for Jacky)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${JACKY_HOME:-~/.jacky}/.env` (the skill will prompt for this on first load).
...
## Jacky Execution Pattern (desktop app flow)

Jacky terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/jacky-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/jacky-op.sock"

---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Jacky Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Jacky Agent"
---
...
Migrate a user's OpenClaw customization footprint into Jacky Agent. Imports Jacky-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `jacky skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Jacky Agent (Nous Research) |
| License | MIT |
...
# OpenClaw -> Jacky Migration

Use this skill when a user wants to move their OpenClaw setup into Jacky Agent with minimal manual cleanup.
...
```bash
jacky claw migrate              # Full interactive migration
jacky claw migrate --dry-run    # Preview what would be migrated
jacky claw migrate --preset user-data   # Migrate without secrets
jacky claw migrate --overwrite  # Overwrite existing conflicts
jacky claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `jacky setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_jacky.py` to:

- import `SOUL.md` into the Jacky home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Jacky memory entries
- merge OpenClaw command approval patterns into Jacky `command_allowlist`
- migrate Jacky-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Jacky working-directory configuration
- copy OpenClaw skills into `~/.jacky/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Jacky workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.jacky/tts/`
- archive non-secret docs that do not have a direct Jacky destination
...
- `scripts/openclaw_to_jacky.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.jacky/skills/migration/openclaw-migration/scripts/openclaw_to_jacky.py`

Do not guess a shorter path like `~/.jacky/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.jacky/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Jacky-compatible secrets, currently:
...
Jacky CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Jacky. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Jacky workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Jacky skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.jacky/skills/migration/openclaw-migration/scripts/openclaw_to_jacky.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.jacky/skills/migration/openclaw-migration/scripts/openclaw_to_jacky.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.jacky/skills/migration/openclaw-migration/scripts/openclaw_to_jacky.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.jacky/skills/migration/openclaw-migration/scripts/openclaw_to_jacky.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.jacky/skills/migration/openclaw-migration/scripts/openclaw_to_jacky.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.jacky/skills/migration/openclaw-migration/scripts/openclaw_to_jacky.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Jacky unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Jacky targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Jacky destination. Unsupported auth blobs must still be reported as skipped.
...
- Jacky persona state imported
- Jacky memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.jacky/skills/openclaw-imports/`

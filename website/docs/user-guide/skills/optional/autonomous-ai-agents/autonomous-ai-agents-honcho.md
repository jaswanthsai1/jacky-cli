...
Configure and use Honcho memory with Jacky -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `jacky skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Jacky

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Jacky profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/jacky#running-honcho-locally-with-jacky

### Verify

```bash
jacky honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Jacky profile's AI peer
...
Honcho models conversations as interactions between **peers**. Jacky creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Jacky instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Jacky run |
| `global` | Single session across all directories |

Manual override: `jacky honcho map my-project-name`
...
Each Jacky profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block jacky.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `jacky.coder` host block in `honcho.json`
...
```bash
jacky honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Jacky when Honcho memory is active.
...
Config file: `$JACKY_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `jacky honcho setup`. Ensure `memory.provider: honcho` is in `~/.jacky/config.yaml`.

### Memory not persisting across sessions
Check `jacky honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `jacky profile create <name> --clone`. For existing profiles: `jacky honcho sync`.
...
| Command | Description |
|---------|-------------|
| `jacky honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `jacky honcho status` | Show resolved config, connection test, peer info for active profile |
| `jacky honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `jacky honcho disable` | Disable Honcho for the active profile |
| `jacky honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `jacky honcho peers` | Show peer identities across all profiles |
| `jacky honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `jacky honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `jacky honcho sessions` | List known directory-to-session-name mappings |
| `jacky honcho map <name>` | Map current working directory to a Honcho session name |
| `jacky honcho identity` | Seed AI peer identity or show both peer representations |
| `jacky honcho sync` | Create host blocks for all Jacky profiles that don't have one yet |
| `jacky honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Jacky + Honcho |
| `jacky memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `jacky memory status` | Show active memory provider and config |
| `jacky memory off` | Disable external memory provider |

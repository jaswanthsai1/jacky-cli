...
| | |
|---|---|
| Source | Optional — install with `jacky skills install official/software-development/rest-graphql-debug` |
...
Drive REST and GraphQL diagnosis through Jacky tools — `terminal` for `curl`, `execute_code` for Python `requests`, `web_extract` for vendor docs. Isolate the failing layer before guessing at the fix.
...
- Never hardcode tokens in scripts. Read from env (`os.environ["API_TOKEN"]`) or `${JACKY_HOME:-~/.jacky}/.env`.

...
| | |
|---|---|
| Source | Optional — install with `jacky skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Jacky or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Jacky. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.jacky/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.jacky/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Jacky integration, either:

- configure the server in `~/.jacky/config.yaml` using the `native-mcp` skill, or
...
### Jacky cannot see the deployed server

The server-building part may be correct while the Jacky config is not. Load the `native-mcp` skill and configure the server in `~/.jacky/config.yaml`, then restart Jacky.

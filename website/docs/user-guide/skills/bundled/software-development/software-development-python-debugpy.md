...
- A long-running process (jacky gateway, tui_gateway) misbehaves and you can't restart it
...
The jacky test runner and pytest both support this:
...
For long-lived processes: Jacky gateway, tui_gateway, a daemon, a process that's already misbehaving and can't be restarted clean.
...
The easiest terminal-side DAP client is VS Code CLI or a small script. From inside Jacky you have two practical options:
...
## Debugging Jacky-specific Processes

### Tests
See Recipe 3. Always add `-p no:xdist` or run single tests without xdist.

### `run_agent.py` / CLI — one-shot
Easiest: add `breakpoint()` near the suspect line, then run `jacky` normally. Control returns to your terminal at the pause point.

### `tui_gateway` subprocess (spawned by `jacky --tui`)
...
debugpy.wait_for_client()
```
Start `jacky --tui`. The TUI will appear frozen (its backend is waiting). Attach a client; execution resumes when you `continue`.
...
9. **Forking / multiprocessing.** pdb does not follow forks. Each child needs its own `breakpoint()` or `set_trace()`. For Jacky subagents, debug one process at a time.

...
description: "Use when a Jacky Kanban worker wants to run Codex CLI as an isolated implementation lane while Jacky keeps ownership of task lifecycle, reconciliation, tes..."
---
...
Use when a Jacky Kanban worker wants to run Codex CLI as an isolated implementation lane while Jacky keeps ownership of task lifecycle, reconciliation, testing, and handoff.
...
| Related skills | [`codex`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-codex), [`jacky-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-jacky-agent) |
...
This skill defines the lightweight Jacky+Codex dual-lane convention for Kanban workers. Jacky is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a Jacky worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn Jacky workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by Jacky in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- Jacky can run the relevant tests itself after Codex exits.
...
1. Jacky owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any Jacky board CLI as a substitute for the worker.
2. Jacky owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. Jacky owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from Jacky with the repo's canonical wrapper.
4. Jacky owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. Jacky owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${JACKY_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional Jacky edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if Jacky can do the task directly.
...
Jacky owns the Kanban lifecycle; do not call Jacky kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on Jacky reconciliation.
...
- Explicit statement: Jacky owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands Jacky will run afterward.
...
Jacky must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by Jacky.
...
- [ ] Jacky ran the canonical tests itself, using `scripts/run_tests.sh` for Jacky Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from Jacky-run tests.
- [ ] Accepted commits/diffs were applied to the Jacky-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "jacky"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct Jacky edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from Jacky.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but Jacky writes board state.
...
- [ ] Jacky reviewed `git diff` and safety-sensitive files.
- [ ] Jacky ran canonical tests independently.

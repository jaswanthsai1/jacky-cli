...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _jacky_env="${JACKY_HOME:-$HOME/.jacky}/.env"; [ -f "$_jacky_env" ] && grep -q "^GITHUB_TOKEN=" "$_jacky_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_jacky_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Jacky Agent\",
    \"comments\": [
...
```bash
source "${JACKY_HOME:-$HOME/.jacky}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Jacky Agent. Code looks clean — good test coverage, no security concerns."

...
- Give it a name like "jacky-agent"
- Select scopes:
...
- Give it a title like "jacky-agent-&lt;machine-name>"
...
elif _jacky_env="${JACKY_HOME:-$HOME/.jacky}/.env"; [ -f "$_jacky_env" ] && grep -q "^GITHUB_TOKEN=" "$_jacky_env"; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_jacky_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"

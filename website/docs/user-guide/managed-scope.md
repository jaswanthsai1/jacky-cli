...
`~/.jacky/config.yaml`, `~/.jacky/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/jacky`:

```text
/etc/jacky/
├── config.yaml     # managed config layer (wins over ~/.jacky/config.yaml)
└── .env            # managed env layer (wins over ~/.jacky/.env + shell)
```
...
The location can be relocated with the `JACKY_MANAGED_DIR` environment variable
(for containers or non-`/etc` deployments). This is a deployment/bootstrap path
knob — like `JACKY_HOME` — set by the same administrator who owns the managed
files. It is **never persisted** to any `.env` by Jacky.

```bash
# Point managed scope at a custom directory (set by IT / the deployment, not the user)
export JACKY_MANAGED_DIR=/opt/org/jacky-policy
```

:::warning
A user who can set `JACKY_MANAGED_DIR` can repoint managed scope at a directory
...
left user-settable. `jacky doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/jacky/config.yaml` (managed) | `/etc/jacky/.env` (managed) |
| 2 | `~/.jacky/config.yaml` (user) | `~/.jacky/.env` (user) |
...
```bash
jacky config        # shows a header naming the managed source + the pinned keys
jacky doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Jacky refuses and names the source:

```bash
$ jacky config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/jacky/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `jacky config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/jacky

# Pin some config values for every user on this machine
sudo tee /etc/jacky/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/jacky/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/jacky
sudo chmod 0644 /etc/jacky/config.yaml /etc/jacky/.env
```

Changes take effect on the next Jacky start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`jacky doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Jacky as `root`), managed scope is advisory.

...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Jacky Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Jacky Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `jacky setup --portal` — and you're ready to `jacky chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Jacky also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `jacky --tui` — see the [TUI](tui.md) guide.
:::
...
jacky chat --model "anthropic/claude-sonnet-4"

# With a specific provider
jacky chat --provider nous        # Use Nous Portal
jacky chat --provider openrouter  # Force OpenRouter

# With specific toolsets
jacky chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
jacky -s jacky-agent-dev,github-auth
jacky chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
jacky --continue             # Resume the most recent CLI session (-c)
jacky --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
jacky chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
jacky -w                         # Interactive mode in worktree
jacky -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Jacky CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Jacky CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
```
 >_ claude-sonnet-4-20250514 │ 12.4K/200K │ [██████░░░░] 6% │ $0.06 │ 15m
```
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `JACKY_YOLO_MODE` is on (either `jacky --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
When resuming a previous session (`jacky -c` or `jacky --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Jacky to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Jacky supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Jacky replies |
...
```bash
jacky -s jacky-agent-dev,github-auth
jacky chat -s github-pr-workflow -s github-auth
```

Jacky loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.jacky/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.jacky/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Jacky..."
  # Add your own!
```
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Jacky — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Jacky is working, Jacky prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Jacky to the background — just like any terminal process. The shell prints a confirmation:

```
Jacky Agent has been suspended. Run `fg` to bring Jacky Agent back.
```
...
Resume this session with:
  jacky --resume 20260225_143052_a1b2c3
...
```bash
jacky --continue                          # Resume the most recent CLI session
jacky -c                                  # Short form
jacky -c "my project"                     # Resume a named session (latest in lineage)
jacky --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
jacky --resume "refactoring auth"         # Resume by title
jacky -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `jacky sessions rename <id> <title>` from the command line. Use `jacky sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Jacky's SQLite state database under `~/.jacky/state.db`. The database keeps:
...
Jacky immediately confirms the task and gives you back the prompt:

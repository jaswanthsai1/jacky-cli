...
slug: /
sidebar_position: 0
title: "Jacky Agent Documentation"
description: "A personalized fork of Nous Research's Hermes Agent, with a built-in learning loop that creates skills from experience, improves them during use, and remembers across sessions."
...
A personalized fork of [Hermes Agent](https://github.com/NousResearch/hermes-agent) by Nous Research. The only agent with a built-in learning loop — it creates skills from experience, improves them during use, nudges itself to persist knowledge, and builds a deepening model of who you are across sessions.
...
This fork does not currently ship a prebuilt desktop app. For the command-line install, see below or the [Quick Start](https://github.com/jaswanthsai1/jacky-cli#quick-start).
...
```bash
curl -fsSL https://raw.githubusercontent.com/jaswanthsai1/jacky-cli/main/scripts/install.sh | bash
```
...
After installing, run `jacky setup --portal` — one OAuth covers a model plus all four Tool Gateway tools (web search, image generation, TTS, browser). See [Nous Portal](/integrations/nous-portal).
:::
...
| 🧭 **[Use MCP with Jacky](/guides/use-mcp-with-jacky)**               | Practical MCP setup patterns, examples, and tutorials                 |
| 🎙️ **[Voice Mode](/user-guide/features/voice-mode)**                    | Real-time voice interaction in CLI, Telegram, Discord, and Discord VC |
| 🗣️ **[Use Voice Mode with Jacky](/guides/use-voice-mode-with-jacky)** | Hands-on setup and usage patterns for Jacky voice workflows          |
| 🎭 **[Personality & SOUL.md](/user-guide/features/personality)**        | Define Jacky' default voice with a global SOUL.md                    |
...
| 💡 **[Tips & Best Practices](/guides/tips)**                            | Quick wins to get the most out of Jacky                              |

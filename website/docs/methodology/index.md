---
title: "Hunt-Loop Methodology"
sidebar_position: 0
description: "The bundled offensive-security methodology Jacky CLI ships with — an eight-stage hunt loop, the skills that implement it, and where the vulnerability class references live."
---

# The Jacky Hunt-Loop Methodology

Jacky CLI ships with a bundled offensive-security methodology, not just a
generic chat wrapper. This page is the map of that methodology and how the
skills under
[`skills/`](https://github.com/jaswanthsai1/jacky-cli/tree/main/skills) in
the repository implement it.

If you only read one file to understand the hunting side of this project,
read
[`skills/jacky-doctrine/SKILL.md`](https://github.com/jaswanthsai1/jacky-cli/blob/main/skills/jacky-doctrine/SKILL.md)
...
| **Test** | Test one vulnerability class at a time with a specific technique, not blind payload spraying. See the [vulnerability class reference](./vuln-classes/). | `jacky-doctrine` (see `references/multi-lead-probing.md`) |
...
[`skills/`](https://github.com/jaswanthsai1/jacky-cli/tree/main/skills) in
the repository and are generalized, reusable methodology — not tied to any
specific engagement or target:
...
- **`report-writing`** — templates, tone guidelines, and formulas for
  ...
  act on.
- **[`vuln-classes`](./vuln-classes/)** — one short reference page per major
  vulnerability class (SSRF, IDOR, XSS, SQLi, SSTI, XXE, CSRF, auth bypass,
  business logic, race conditions, CORS, subdomain takeover, JWT/OAuth,
  open redirect, file upload, GraphQL, API misconfig, cloud/K8s misconfig)
  with a generic detect / escalate / tools structure.
...
- **Self-improving skills** — when an engagement teaches you a reusable
  ...
  same way these were written. Jacky's [skill system](/user-guide/features/skills)
  is built for exactly this loop.
- **Persistent memory / cross-session state** — the "living ledger"
  ...
  [memory](/user-guide/features/memory) and [sessions](/user-guide/sessions)
  features to keep a per-engagement ledger instead of starting from zero
  every time.
...
- **Dual local + cloud model support** — run the methodology with a local
  ...
  of an engagement that need more reasoning depth. See
  [Configuring Models](/user-guide/configuring-models) for provider setup.

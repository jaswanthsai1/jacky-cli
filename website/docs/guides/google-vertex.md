...
- **`google-auth`** — installed automatically the first time you select Vertex (lazy install), or explicitly with `pip install 'jacky-agent[vertex]'`.
...
- The **credential path** is a pointer to a secret and lives in `~/.jacky/.env`.
- **Project ID and region** are non-secret routing settings and live in `~/.jacky/config.yaml`.
...
1. Jacky resolves credentials in this order: `VERTEX_CREDENTIALS_PATH` → `GOOGLE_APPLICATION_CREDENTIALS` → ADC.
...
4. If a session runs longer than the token lifetime and a request returns `401`, Jacky re-mints the token and retries automatically. On a long-running gateway, if ADC's refresh token has itself expired, Jacky falls back to the service-account JSON when one is configured.

## Available Models

Vertex requires the `google/` vendor prefix on model IDs. The `jacky model` picker offers:
...
`/model` switches among already-configured providers and models; it does not collect new credentials. Configure Vertex with `jacky model` first.

## Reasoning / Thinking

Vertex exposes Gemini's thinking budget through the OpenAI-compatible surface. Jacky maps its reasoning-effort setting onto `extra_body.google.thinking_config` automatically, so `reasoning_effort` works the same way it does on other Gemini surfaces.
...
Jacky found neither a service-account JSON nor working ADC. Either set `VERTEX_CREDENTIALS_PATH` in `~/.jacky/.env`, or run `gcloud auth application-default login`. If your project isn't embedded in the credentials, set `vertex.project_id` in `config.yaml`.

### `google-auth` not installed

Install the extra: `pip install 'jacky-agent[vertex]'`. Jacky also lazy-installs it the first time you select the Vertex provider.

...
description: "Send text from any shell script, cron job, CI hook, or monitoring daemon to Telegram, Discord, Slack, Signal, and other platforms using `jacky send`."
---

# Pipe Script Output to Messaging Platforms

`jacky send` is a small, scriptable CLI that pushes a message to any
messaging platform Jacky is already configured for. Think of it as a
...
- Piping any tool's output anywhere (`make | jacky send --to slack:#builds`)

The command reuses the same credentials and platform adapters that `jacky
...
jacky send --to slack:#eng --subject "[CI] build.log" --file build.log

# Thread target (Telegram topic, Discord thread)
jacky send --to telegram:-1001234567890:17585 "threaded reply"

# List every configured target
jacky send --list

# Filter by platform
jacky send --list telegram
```
...
Any platform Jacky ships adapters for works as a target:
...
`jacky send` resolves the message body in this order:

1. **Positional argument** — `jacky send --to telegram "hi"`
2. **`--file PATH`** — `jacky send --to telegram --file msg.txt`
3. **Piped stdin** — `echo hi | jacky send --to telegram`

When stdin is a TTY (no pipe), Jacky does **not** wait for input — you'll
...
if [ "$ram_pct" -ge 85 ]; then
  jacky send --to telegram --subject "⚠ MEMORY WARNING" \
    "RAM ${ram_pct}% on $(hostname)"
fi
```

Because `jacky send` reuses your Jacky config, the same script works on
any host where Jacky is installed — no need to export bot tokens into
...
`jacky send`. If the Python interpreter can't load because the box is
...
if ./scripts/deploy.sh; then
  jacky send --to slack:#deploys "✅ ${CI_COMMIT_SHA:0:7} deployed"
else
  tail -n 100 deploy.log | jacky send \
...
```bash
./train.py --epochs 200 && \
  jacky send --to telegram "training done" || \
  jacky send --to telegram "training failed (exit $?)"
```
...
jacky send --to telegram --quiet "keepalive" || {
  echo "Telegram delivery failed" >&2
  exit 1
}

# Capture the message ID for later editing / threading
msg_id=$(jacky send --to discord:#ops --json "build started" \
  | jq -r .message_id)
```

---

## Does `jacky send` Need the Gateway Running?

**Usually no.** For any bot-token platform — Telegram, Discord, Slack,
Signal, SMS, WhatsApp Cloud API, and most others — `jacky send` calls
the platform's REST endpoint directly using credentials from
`~/.jacky/.env` and `~/.jacky/config.yaml`. It's a standalone subprocess
...
pointing at the gateway; start it with `jacky gateway start` and retry.
...
# Machine-readable
jacky send --list --json
```

The listing is built from `~/.jacky/channel_directory.json`, which the
gateway refreshes every few minutes while it's running. If you see
"no channels discovered yet", start the gateway once (`jacky gateway
...
| Approach | Multi-platform | Reuses Jacky creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
| `jacky send` | ✅ | ✅ | No (bot-token) | Everything below |
...
`jacky send` is intentionally the simplest possible surface. If you need
...
If you just need to pipe a raw string, reach for `jacky send`.
...
- [Gateway Internals](/developer-guide/gateway-internals) —
  the delivery router that `jacky send` shares with cron delivery.

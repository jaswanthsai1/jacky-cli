---
sidebar_position: 7
title: "Use SOUL.md with Jacky"
description: "How to use SOUL.md to shape Jacky Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with Jacky

`SOUL.md` is the **primary identity** for your Jacky instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want Jacky to feel like the same assistant every time you talk to it — or if you want to replace the Jacky persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm Jacky should be
- what Jacky should avoid stylistically
- how Jacky should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who Jacky is and how Jacky speaks
...
Jacky now uses only the global SOUL file for the current instance:

```text
~/.jacky/SOUL.md
```

If you run Jacky with a custom home directory, it becomes:
...
Jacky automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, Jacky does not overwrite it
- if the file exists but is empty, Jacky adds nothing from it to the prompt

## How Jacky uses it

When Jacky starts a session, it reads `SOUL.md` from `JACKY_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how Jacky feels.
...
Jacky already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How Jacky should sound.

# Avoid
What Jacky should not do.

# Defaults
How Jacky should behave when ambiguity appears.
```
...
Then restart Jacky or start a new session.
...
### I edited SOUL.md but Jacky still sounds the same

Check:
- you edited `~/.jacky/SOUL.md` or `$JACKY_HOME/SOUL.md`
...
### Jacky is ignoring parts of my SOUL.md

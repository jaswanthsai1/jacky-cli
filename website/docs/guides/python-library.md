---
sidebar_position: 5
title: "Using Jacky as a Python Library"
description: "Embed AIAgent in your own Python scripts, web apps, or automation pipelines — no CLI required"
---

# Using Jacky as a Python Library

Jacky isn't just a CLI tool. You can import `AIAgent` directly and use it programmatically in your own Python scripts, web applications, or automation pipelines. This guide shows you how.

---

## Installation

Install Jacky directly from the repository:
...
```bash
uv pip install git+https://github.com/jaswanthsai1/jacky-cli.git
```

You can also pin it in your `requirements.txt`:

```text
jacky-agent @ git+https://github.com/jaswanthsai1/jacky-cli.git
```

:::tip
The same environment variables used by the CLI are required when using Jacky as a library. At minimum, set `OPENROUTER_API_KEY` (or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` if using direct provider access).
:::
...
The simplest way to use Jacky is the `chat()` method — pass a message, get a string back:
...
:::warning
Always set `quiet_mode=True` when embedding Jacky in your own code. Without it, the agent prints CLI spinners, progress indicators, and other terminal output that will clutter your application's output.
:::
...
async def on_message(message):
    if message.author == client.user:
        return
    if message.content.startswith("!jacky "):
...
|-----------|------|---------|-------------|
| `model` | `str` | `""` | Model in OpenRouter format (defaults to empty; resolved from your jacky config at runtime) |

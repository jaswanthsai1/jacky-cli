...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.jacky/.env` at the end.
...
4. Fill in:
   - **Name:** `Jacky Teams Meeting Pipeline` (or any name you'll recognize).
...
3. **Description:** `jacky-graph-secret`. **Expires:** pick a value that matches your rotation policy (6-24 months is typical).
4. Click **Add**.
...
Put the three values you collected into `~/.jacky/.env`:
...
Jacky ships a Graph auth smoke-test. From your Jacky install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.jacky/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `jacky gateway restart`.

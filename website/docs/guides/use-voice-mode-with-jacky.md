---
sidebar_position: 8
title: "Use Voice Mode with Jacky"
description: "A practical guide to setting up and using Jacky voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Jacky
...
- you want Jacky sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Jacky.
...
## Step 1: make sure normal Jacky works first
...
Jacky supports both local and cloud speech stacks.
...
### If you use `jacky setup`

If you choose NeuTTS in the setup wizard, Jacky checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Jacky transcribes and responds
...
- asking Jacky to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Jacky loop.

## Tuning CLI behavior

### Silence threshold

If Jacky starts/stops too aggressively, tune:
...
Jacky stays a normal chat bot, but can speak replies.
...
- you want Jacky to function like a portable research or ops assistant
...
Jacky joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Jacky detects speech boundaries
- transcripts are posted in the associated text channel
- Jacky responds in text and audio
...
1. get text Jacky working
2. install `jacky-agent[voice]`

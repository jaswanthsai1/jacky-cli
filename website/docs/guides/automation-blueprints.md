...
Copy-paste blueprints for common automation patterns. Each blueprint uses Jacky's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`jacky webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `jacky webhook subscribe`) |
...
```bash
jacky cron create "0 2 * * *" \
  "You are a project manager triaging the jaswanthsai1/jacky-cli GitHub repo.

1. Run: gh issue list --repo jaswanthsai1/jacky-cli --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
jacky webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
jacky cron create "0 9 * * 1" \
  "Scan the jaswanthsai1/jacky-cli repo for documentation drift.

1. Run: gh pr list --repo jaswanthsai1/jacky-cli --state merged --json number,title,files,mergedAt --limit 30
2. Filter to PRs merged in the last 7 days
3. For each merged PR, check if it modified:
   - Tool schemas (tools/*.py) — may need docs/reference/tools-reference.md update
   - CLI commands (jacky_cli/commands.py, jacky_cli/main.py) — may need docs/reference/cli-commands.md update
   - Config options (jacky_cli/config.py) — may need docs/user-guide/configuration.md update
...
```bash
jacky cron create "0 6 * * *" \
  "Run a dependency security audit on the jacky-agent project.

1. cd ~/.jacky/jacky-agent && source .venv/bin/activate
...
```bash
jacky webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
jacky webhook subscribe alert-triage \
...
```python title="~/.jacky/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Jacky-Monitor/1.0"})
...
jacky cron create "every 30m" \
  ...
  --script ~/.jacky/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
jacky cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Jacky Agent development." \
...
```bash
jacky webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
jacky webhook subscribe stripe-payments \
...
```bash
jacky cron create "0 3 * * 0" \
  "Run a comprehensive security audit of the jacky-agent codebase.

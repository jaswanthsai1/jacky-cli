...
```bash
jacky teams-pipeline token-health
jacky teams-pipeline token-health --force-refresh
```
...
```bash
jacky teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
jacky teams-pipeline maintain-subscriptions
jacky teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Jacky cron (recommended if you already run the Jacky gateway)

Jacky ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.jacky/scripts/`. First create the script:

```bash
mkdir -p ~/.jacky/scripts
cat > ~/.jacky/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec jacky teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.jacky/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
jacky cron create "0 */12 * * *" \
...
Create `/etc/systemd/system/jacky-teams-pipeline-maintain.service`:

```ini
[Unit]
Description=Jacky Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=jacky
EnvironmentFile=/etc/jacky/env
ExecStart=/usr/local/bin/jacky teams-pipeline maintain-subscriptions
```

And `/etc/systemd/system/jacky-teams-pipeline-maintain.timer`:

```ini
[Unit]
Description=Run Jacky Teams pipeline subscription maintenance every 12 hours
...
sudo systemctl enable --now jacky-teams-pipeline-maintain.timer
systemctl list-timers jacky-teams-pipeline-maintain.timer
```

#### Option 3: Plain crontab

```cron
0 */12 * * * /usr/local/bin/jacky teams-pipeline maintain-subscriptions >> /var/log/jacky/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.jacky/.env` at the top of a wrapper script that crontab calls.
...
```bash
jacky teams-pipeline subscriptions   # should show expirationDateTime advanced
jacky teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
jacky teams-pipeline list
jacky teams-pipeline list --status failed
jacky teams-pipeline show <job-id>
```

### Replay a stored job

```bash
jacky teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
jacky teams-pipeline fetch --meeting-id <meeting-id>
jacky teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `jacky teams-pipeline maintain-subscriptions --dry-run`
- inspect `jacky teams-pipeline list --status failed`
...
- run `jacky teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `jacky teams-pipeline run`
...
- [ ] `jacky teams-pipeline validate` returns an OK snapshot
- [ ] `jacky teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Jacky cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.

...
description: "Sign in with your SuperGrok or X Premium+ subscription to use Grok models in Jacky Agent — no API key required"
---

# xAI Grok OAuth (SuperGrok / X Premium+)

Jacky Agent supports xAI Grok through a browser-based OAuth device-code login flow against [accounts.x.ai](https://accounts.x.ai), using either a **SuperGrok subscription** ([grok.com](https://x.ai/grok)) or an **X Premium+ subscription** (linked X account). No `XAI_API_KEY` is required — log in once and Jacky automatically refreshes your session in the background.
...
The same OAuth bearer token is also reused by every direct-to-xAI surface in Jacky — TTS, image generation, video generation, and transcription — so a single login covers all four.
...
# → Jacky opens or prints an accounts.x.ai verification URL
...
After the first login, credentials are stored under `~/.jacky/auth.json` and refreshed automatically before they expire.
...
```bash
jacky auth add xai-oauth
```

### Remote / headless sessions

On servers, containers, browser-only consoles (Cloud Shell, Codespaces, EC2 Instance Connect), or SSH sessions where Jacky cannot open a browser locally, Jacky prints the xAI verification URL and user code. Open the URL in any browser on your laptop or in the cloud console, enter the code if prompted, and Jacky will keep polling until xAI approves the login. No SSH tunnel or local callback listener is required.

```bash
jacky auth add xai-oauth --no-browser
# Open the printed verification URL in your browser.
```

The same device-code flow applies when you sign in from the web dashboard or the desktop app: Jacky shows the verification URL and user code, then polls in the background until you approve access.

## How the Login Works

1. Jacky requests a device code from `auth.x.ai`.
2. You open the verification URL, sign in, enter the displayed code if prompted, and approve access.
3. Jacky polls xAI until approval, then saves tokens to `~/.jacky/auth.json`.
4. From then on, Jacky refreshes the access token in the background — you stay signed in until you `jacky auth logout xai-oauth` or revoke access from your xAI account settings.
...
```bash
jacky config set model.default grok-build-0.1
jacky config set model.provider xai-oauth
```

## Configuration Reference

After login, `~/.jacky/config.yaml` will contain:
...
```bash
jacky --provider xai-oauth        # canonical
jacky --provider grok-oauth       # alias
jacky --provider x-ai-oauth       # alias
jacky --provider xai-grok-oauth   # alias
```
...
The `video_gen` toolset is disabled by default. Enable it in `jacky tools` → `🎬 Video Generation` (press space) before the agent can call `video_generate`. Otherwise the agent may fall back to the bundled ComfyUI skill, which is also tagged for video generation.
:::

:::note X search auto-enables when xAI credentials are present
The `x_search` toolset auto-enables whenever xAI credentials (a SuperGrok / X Premium+ OAuth token or `XAI_API_KEY`) are configured. Disable explicitly via `jacky tools` → `🐦 X (Twitter) Search` (press space) if you don't want this. The tool routes through xAI's built-in `x_search` Responses API — it works with **either** your SuperGrok / X Premium+ OAuth login or a paid `XAI_API_KEY`, and prefers OAuth when both are configured (uses your subscription quota instead of API spend). The tool schema is hidden from the model when no xAI credentials are configured, regardless of whether the toolset is enabled.
:::
...
To select xAI as the active provider, set `model.provider: xai-oauth` in `config.yaml` (use `jacky setup` for the guided flow) or pass `--provider xai-oauth` for a single invocation.

## Troubleshooting

### Token expired — not re-logging in automatically

Jacky refreshes the token before each session and again reactively on a 401. If refresh fails with `invalid_grant` (the refresh token was revoked, or the account was rotated), Jacky surfaces a typed re-auth message instead of crashing.

When the refresh failure is terminal (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Jacky marks the refresh token as dead and quarantines it locally — subsequent calls skip the doomed refresh attempt instead of replaying the same 401 over and over. The agent surfaces a single "re-authentication required" message and stays out of the way until you log in again.

**Fix:** run `jacky auth add xai-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

Device-code approval has a finite expiry window (xAI sets `expires_in` on the device-code response, typically on the order of tens of minutes). If you do not approve the login in time, Jacky raises a timeout error.

**Fix:** re-run `jacky auth add xai-oauth` (or `jacky model`). The flow starts fresh.

### Logging in from a remote server

On SSH or container sessions Jacky prints the verification URL and user code instead of opening a browser. Open that URL in a browser on your laptop or in a cloud console — no SSH port forward is needed for xAI Grok OAuth.
...
This is **not** a stale-token problem — re-running `jacky model` won't change it. xAI's backend has been seen to restrict OAuth API access to specific SuperGrok tiers despite the in-app subscription being active (issue [#26847](https://github.com/jaswanthsai1/jacky-cli/issues/26847)).

**Fix:** set `XAI_API_KEY` and switch to the API-key path:

```bash
export XAI_API_KEY=xai-...
jacky config set model.provider xai
```
...
**Fix:** run `jacky model` and pick the xAI Grok OAuth provider, or run `jacky auth add xai-oauth`.
...
```bash
jacky auth logout xai-oauth
```

This clears both the singleton OAuth entry in `auth.json` and any credential-pool rows for `xai-oauth`. Use `jacky auth remove xai-oauth <index|id|label>` if you only want to drop a single pool entry (run `jacky auth list xai-oauth` to see them).

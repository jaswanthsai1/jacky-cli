...
description: "Diagnose and fix common Jacky cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`jacky gateway` for foreground, or `jacky gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `jacky cron tick`.
...
```bash
date
jacky cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.jacky/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.jacky/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.jacky/.env` |
...
| `local` | Write access to `~/.jacky/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `jacky cron list` for updated `last_error` field (if available).
...
Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `jacky skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `jacky skills list`.
...
2. `~/.jacky/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `jacky cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Jacky config directory). Verify:
```bash
ls ~/.jacky/scripts/your-script.py   # Must exist
jacky cron edit <job_id> --script ~/.jacky/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `jacky skills install <skill-name>`.
...
Jobs are stored in `~/.jacky/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.jacky/cron/jobs.json
chmod 600 ~/.jacky/cron/jobs.json   # Your user should own it
```
...
```bash
jacky cron list                    # Show all jobs, states, next_run times
jacky cron run <job_id>            # Schedule for next tick (for testing)
jacky cron edit <job_id>           # Fix configuration issues
jacky logs                         # View recent Jacky logs
jacky skills list                  # Verify installed skills
```
...
1. Run the job with `jacky cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.jacky/logs/agent.log` for scheduler messages and `~/.jacky/logs/errors.log` for warnings
3. Open an issue at [github.com/jaswanthsai1/jacky-cli](https://github.com/jaswanthsai1/jacky-cli) with:

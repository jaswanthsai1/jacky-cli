...
3. Sets `model.provider: nous` in `~/.jacky/config.yaml`
...
OAuth needs a browser, but the loopback callback runs on the machine where Jacky is running. Two options:

```bash
# Option A: SSH port forwarding (preferred)
ssh -N -L 8642:127.0.0.1:8642 user@remote-host    # in a local terminal
jacky setup --portal                              # on the remote, open the printed URL in your local browser

# Option B: device-code login (works from Cloud Shell, Codespaces, EC2 Instance Connect)
jacky auth add nous --type oauth
# Then re-run `jacky setup --portal` to wire the provider + gateway
```
...
```
Hey, search the web for "Jacky Agent release notes" and summarize the top 3 hits.
```

You should see Jacky call `web_search` (Firecrawl-backed, through the gateway) and respond with a summary. If the search runs and the response makes sense, you're done — the Portal is wired up end to end.

## 5. Pick the model you actually want

`jacky setup --portal` lets you pick a model during setup, but the whole point of the subscription is access to the full catalog — switch any time with `/model` mid-session:
...
These rows appear in `jacky tools` even before you've logged into Nous Portal — if you pick "Nous Subscription" without an active session, Jacky runs the Portal login inline (without changing your inference provider or your other tools).
...
Then in any messaging-platform session (Telegram, Discord, Signal, etc.), send a voice message and Jacky will transcribe it, respond, and reply with synthesized voice — all on your Portal subscription.
...
```bash
jacky cron create "every day at 9am" \
...
If you use [Jacky profiles](/user-guide/profiles) (e.g. a separate config per project), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically.

For team setups where multiple humans share a machine, each human has their own Portal account → each home directory holds its own `~/.jacky/auth.json` → no token sharing across users. This is the right boundary.

## Troubleshooting

### `jacky portal info` shows "not logged in" after `jacky setup --portal`
...
```bash
jacky config set model.provider nous
```
...
Re-verify with `jacky portal info`.
...
Your Portal refresh token was invalidated (password change, manual revoke, session expiry). The token is now quarantined locally so Jacky doesn't replay it endlessly. Just log in again:
...
If a model is genuinely unavailable, [open an issue](https://github.com/jaswanthsai1/jacky-cli/issues) — most gaps are routing config we can update.

### Billing not appearing on my Portal account

`jacky portal info` will tell you whether you're actually routing through the Portal or some other provider. Common causes:
...
- Multiple Jacky profiles where you're using the wrong one (check `jacky profile list`)

### Want to revoke and start clean

```bash
jacky auth logout nous       # wipes the local refresh token
...
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Jacky tools
...
- **[Profiles](/user-guide/profiles)** — Share one Portal login across multiple Jacky configurations

...
Jacky calls this **no-agent mode**. It's the cron system minus the LLM.
...
- **Bash or Python.** `.sh` / `.bash` files run under `/bin/bash`; any other extension runs under the current Python interpreter. Anything in `~/.jacky/scripts/` is accepted.
...
The real win of no-agent mode is that the agent itself can set up the watchdog for you — no editor, no shell, no remembering CLI flags. You describe what you want, Jacky writes the script, schedules it, and tells you when it'll fire.

### Example transcript

> **You:** ping me on telegram if RAM is over 85% every 5 minutes
>
> **Jacky:** *(writes `~/.jacky/scripts/memory-watchdog.sh`, then calls `cronjob(...)` with `no_agent=true`)*
>
...
write_file(
    path="~/.jacky/scripts/memory-watchdog.sh",
...
>
> **Jacky:** *(calls `cronjob(action='pause', job_id='abc123')`)*
>
> Paused. Resume with "turn it back on" or via `jacky cron resume abc123`.

> **You:** change it to every 15 minutes
>
> **Jacky:** *(calls `cronjob(action='update', job_id='abc123', schedule='every 15m')`)*
...
```bash
# 1. Write your script
cat > ~/.jacky/scripts/memory-watchdog.sh <<'EOF'
#!/usr/bin/env bash
...
EOF
chmod +x ~/.jacky/scripts/memory-watchdog.sh
...
# 3. Verify
jacky cron list
jacky cron run <job_id>    # fire it once to test
```
...
Scripts must live in `~/.jacky/scripts/`. This is enforced at both job-creation time and run time — absolute paths, `~/` expansion, and path-traversal patterns (`../`) are rejected. The same directory is shared with the pre-check script gate used by LLM jobs.
...
```bash
jacky cron create "every 5m"        # interval
jacky cron create "every 2h"
jacky cron create "0 9 * * *"       # standard cron: 9am daily
jacky cron create "30m"             # one-shot: run once in 30 minutes
```
...
--deliver local                          # just save to ~/.jacky/cron/output/
```

No running gateway is required at script-run time for bot-token platforms (Telegram, Discord, Slack, Signal, SMS, WhatsApp) — the tool calls each platform's REST endpoint directly using the credentials already in `~/.jacky/.env` / `~/.jacky/config.yaml`.

## Editing and Lifecycle

```bash
jacky cron list                                    # see all jobs
jacky cron pause <job_id>                          # stop firing, keep definition
jacky cron resume <job_id>
jacky cron edit <job_id> --schedule "every 10m"    # adjust cadence
jacky cron edit <job_id> --agent                   # flip to LLM mode
jacky cron edit <job_id> --no-agent --script …     # flip back
jacky cron remove <job_id>                         # delete it
```
...
```bash
cat > ~/.jacky/scripts/disk-alert.sh <<'EOF'
#!/usr/bin/env bash
...
'
EOF
chmod +x ~/.jacky/scripts/disk-alert.sh

jacky cron create "*/15 * * * *" \
  --no-agent \
...
|----------|-----------|-------------|
| `cronjob --no-agent` (this page) | Your script on Jacky' schedule | Recurring watchdogs / alerts / metrics that don't need reasoning |
| `cronjob` (default, LLM) | Agent with optional pre-check script | When the message content requires reasoning over data |
| OS cron + `curl` to a [webhook subscription](/user-guide/messaging/webhooks) | Your script on the OS schedule | When Jacky might be unhealthy (the thing you're monitoring) |

For critical system-health watchdogs that must fire *even when the gateway is down*, use OS-level cron with a plain `curl` to a Jacky webhook subscription (or any external alerting endpoint) — those run as independent OS processes and don't depend on Jacky being up. The in-gateway scheduler is the right choice when the thing being monitored is external.

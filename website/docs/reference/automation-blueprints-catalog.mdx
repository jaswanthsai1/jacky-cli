...
of fields, and Jacky schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Jacky asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.jacky.blueprint` block in its

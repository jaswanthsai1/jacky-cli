...
This page covers all commands related to [Jacky profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `jacky profile` without a subcommand shows help.
...
```bash
jacky profile use <name>
```

Sets `<name>` as the active profile. All subsequent `jacky` commands (without `-p`) will use this profile.
...
jacky profile use default
```

## `jacky profile create`

```bash
jacky profile create <name> [options]
```
...
jacky profile create work2 --clone-from work

# Clone everything from a specific profile
jacky profile create work2-backup --clone-from work --clone-all
```

## `jacky profile describe`

```bash
jacky profile describe [<name>] [options]
```
...
jacky profile describe researcher

# Set it explicitly
jacky profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
jacky profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
jacky profile describe --all --auto
```

## `jacky profile delete`

```bash
jacky profile delete <name> [options]
```
...
```bash
jacky profile delete mybot
jacky profile delete mybot --yes
```
...
```bash
jacky profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Jacky home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ jacky profile show work
Profile: work
Path:    ~/.jacky/profiles/work
...
```bash
jacky profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Jacky installation.
...
```bash
jacky profile alias work
# Creates/updates ~/.local/bin/work

jacky profile alias work --name mywork
# Creates ~/.local/bin/mywork

jacky profile alias work --remove
# Removes the wrapper script
```

## `jacky profile rename`

```bash
jacky profile rename <old-name> <new-name>
```
...
```bash
jacky profile rename mybot assistant
# ~/.jacky/profiles/mybot → ~/.jacky/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `jacky profile export`

```bash
jacky profile export <name> [options]
```
...
```bash
jacky profile export work
# Creates work.tar.gz in the current directory

jacky profile export work -o ./work-2026-03-29.tar.gz
```

## `jacky profile import`

```bash
jacky profile import <archive> [options]
```
...
```bash
jacky profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

jacky profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`jacky profile export` / `import` are still the right commands for
...
### `jacky profile install`

```bash
jacky profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `jacky -p telemetry`). |
...
jacky profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
jacky profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
jacky profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
jacky profile install ./telemetry/
```

### `jacky profile update`

```bash
jacky profile update <name> [--force-config] [--yes]
```
...
```bash
jacky profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Jacky version, author, env var requirements, the source URL/path, and
...
`jacky profile list` also shows the distribution name and version in a
`Distribution` column, and `jacky profile show <name>` / `delete <name>`
...
jacky profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
jacky profile install https://github.com/your-org/internal-assistant.git
```
...
`jacky_requires` supports `>=`, `<=`, `==`, `!=`, `>`, `<`, or a bare
version (treated as `>=`). Install fails with a clear error if the current
Jacky version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Jacky can clone from.
3. Tell recipients to run `jacky profile install <your-repo-url>`.
...
## `jacky -p` / `jacky --profile`

```bash
jacky -p <name> <command> [options]
jacky --profile <name> <command> [options]
```

Global flag to run any Jacky command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
jacky -p work chat -q "Check the server status"
jacky --profile dev gateway start
jacky -p personal skills list
jacky -p work config edit
```

## `jacky completion`

```bash
jacky completion <shell>
```
...
- `jacky profile <TAB>` — subcommands (list, use, create, etc.)
- `jacky profile use <TAB>` — profile names
- `jacky -p <TAB>` — profile names

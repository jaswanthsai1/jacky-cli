...
description: "Frequently asked questions and solutions to common issues with Jacky Agent"
---
...
### What LLM providers work with Jacky?

Jacky Agent works with any OpenAI-compatible API. Supported providers include:
...
- **Anthropic** — Claude models (direct API, OAuth via `jacky auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `jacky model` or by editing `~/.jacky/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Jacky in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Jacky inside WSL2
...
- let Jacky use the resulting MCP browser tools

This is more reliable than trying to force Jacky core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Jacky](../guides/use-mcp-with-jacky.md#wsl2-bridge-jacky-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Jacky Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.jacky/`.

### Can I use it offline / with local models?

Yes. Run `jacky model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Jacky minimum; set this to match your server's actual context window
```
...
Jacky persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Jacky — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Jacky auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `JACKY_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Jacky Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Jacky Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Jacky programmatically:
...
#### `jacky: command not found` after installation
...
**Cause:** Jacky requires Python 3.11 or newer.
...
**Cause:** Jacky builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Jacky runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Jacky auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.jacky/config.yaml`:
...
**Solution:** Exit your session and use `jacky model` from your terminal to add new providers:

```bash
# Exit the Jacky chat session first (Ctrl+C or /quit)
...
After adding a new provider via `jacky model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `jacky model` (from terminal) |
| Enter/change API keys | `jacky model` (from terminal) |
...
# Or set directly
jacky config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.jacky/.env` for conflicting entries.
:::
...
# Set a valid model
jacky config set JACKY_MODEL anthropic/claude-opus-4.7

# Or specify per-session
jacky chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `jacky chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Jacky detected the wrong context length for your model.
...
jacky chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Jacky may have the wrong context length for your model. Check what it detected:
...
**Cause:** Jacky detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Jacky never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `jacky chat`
...
cat ~/.jacky/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `jacky gateway setup`
- Check gateway logs: `cat ~/.jacky/logs/gateway.log | tail -50`
...
Configure in `~/.jacky/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.jacky/jacky-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `jacky gateway start` fails
...
tmux new -s jacky 'jacky gateway run'
# Reattach later: tmux attach -t jacky

# Option 3: Background via nohup
nohup jacky gateway run > ~/.jacky/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'jacky gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `jacky gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
jacky gateway install    # Re-snapshots your current PATH
jacky gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.jacky.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `jacky chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `jacky chat -t "terminal"`
...
cd ~/.jacky/jacky-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.jacky/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
jacky config show | grep -A 12 mcp_servers

# Restart Jacky or reload MCP after config changes
jacky chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Jacky](/guides/use-mcp-with-jacky)
...
:::warning
If an MCP server crashes mid-request, Jacky will report a timeout. Check the server's own logs (not just Jacky logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `jacky profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

### What happens when I run `jacky update`?

`jacky update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `jacky update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.jacky/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Jacky can route subagents to a different model automatically. Set this in `~/.jacky/config.yaml`:
...
Now when you tell Jacky "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Jacky do this?

**Current limitation:** Jacky profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Jacky reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `jacky skills config` settings don't seem to take effect.

**Solution:** Use `jacky skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`jacky gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Jacky creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Jacky to another machine
...
1. Install Jacky Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/jaswanthsai1/jacky-cli/main/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   jacky backup
   ```
   This creates a zip of your entire `~/.jacky/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/jacky-backup-<timestamp>.zip`.

3. Copy the zip to the new machine and import it:
   ```bash
   # On the source machine
   scp ~/jacky-backup-<timestamp>.zip newmachine:~/

   # On the new machine
   jacky import ~/jacky-backup-<timestamp>.zip
   ```

4. On the new machine, run `jacky setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
jacky profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
jacky profile import ./work-backup.tar.gz work
```
...
### `jacky backup` vs `jacky profile export`

| Feature | `jacky backup` | `jacky profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.jacky` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='jacky-agent' ~/.jacky/ newmachine:~/.jacky/
```

:::tip
`jacky backup` produces a consistent snapshot even while Jacky is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Jacky installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Jacky-specific issue — it's a shell config permissions problem.
...
jacky config show | head -20

# Re-run model selection
jacky model

# Or test with a known-good model
jacky chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/jaswanthsai1/jacky-cli/issues)
2. **Ask the community:** [Nous Research Discord](https://discord.gg/nousresearch)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Jacky version (`jacky --version`), and the full error message

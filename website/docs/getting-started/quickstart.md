...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Jacky Agent — from install to chatting in under 5 minutes"
---
...
- Setting up Jacky for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Jacky working on my machine | `jacky setup` | Run a real chat and verify it responds |
| I already know my provider | `jacky model` | Save the config, then start chatting |
| I want a bot or always-on setup | `jacky gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `jacky model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `jacky model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Jacky cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.
...
The single most important setup step. Use `jacky model` to walk through the choice interactively:
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `jacky model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `jacky model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `jacky model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `jacky_cli/models.py` may change between releases) | `jacky model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `jacky model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `jacky model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `jacky model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `jacky model` (requires `copilot` CLI + `copilot login`) |
...
Jacky Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `jacky model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Jacky separates secrets from normal config:

- **Secrets and tokens** → `~/.jacky/.env`
- **Non-secret settings** → `~/.jacky/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
jacky config set model anthropic/claude-opus-4.6
jacky config set terminal.backend docker
jacky config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
jacky            # classic CLI
jacky --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Jacky ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `jacky` vs `jacky --tui`.
:::
...
```bash
jacky --continue    # Resume the most recent session
jacky -c            # Short form
```
...
```bash
jacky gateway setup    # Interactive platform configuration
```
...
- `jacky tools` — tune tool access per platform
- `jacky skills` — browse and install reusable workflows
...
```bash
jacky config set terminal.backend docker    # Docker isolation
jacky config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Jacky install directory (the curl installer placed it at
# ~/.jacky/jacky-agent on Linux/macOS or %LOCALAPPDATA%\jacky\jacky-agent on Windows):
cd ~/.jacky/jacky-agent
...
Skills are on-demand instruction documents that teach Jacky how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Jacky ships with a catalog of bundled skills already installed in `~/.jacky/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
jacky skills browse                      # list everything available
jacky skills search kubernetes           # find skills by keyword
jacky skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `jacky skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Jacky ask what you need
```
...
(If you installed without `[all]`, run `cd ~/.jacky/jacky-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Jacky opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `jacky model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `jacky gateway setup` and check `jacky gateway status` |
| `jacky --continue` can't find old session | Switched profiles or session never saved | Check `jacky sessions list` and confirm you're in the right profile |
...
| `jacky doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |
...
2. `jacky model`
3. `jacky setup`
4. `jacky sessions list`
5. `jacky --continue`
6. `jacky gateway status`
...
| Command | Description |
|---------|-------------|
| `jacky` | Start chatting |
| `jacky model` | Choose your LLM provider and model |
| `jacky tools` | Configure which tools are enabled per platform |
| `jacky setup` | Full setup wizard (configures everything at once) |
| `jacky doctor` | Diagnose issues |
| `jacky update` | Update to latest version |
| `jacky gateway` | Start the messaging gateway |
| `jacky --continue` | Resume last session |

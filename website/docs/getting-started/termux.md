...
description: "Run Jacky Agent directly on an Android phone with Termux"
---

# Jacky on Android with Termux
...
Jacky Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Jacky from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.
...
- links `jacky` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `jacky` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/jacky" "$PREFIX/bin/jacky"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `jacky` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.jacky/.env`.
...
### `jacky doctor` says ripgrep or Node is missing

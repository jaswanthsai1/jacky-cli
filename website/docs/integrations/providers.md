...
This page covers setting up inference providers for Jacky Agent — from cloud APIs like OpenRouter and Anthropic, to self-hosted endpoints like Ollama and vLLM, to advanced routing and fallback configurations. You need at least one provider configured to use Jacky.

## Inference Providers

You need at least one way to connect to an LLM. Use `jacky model` to switch providers and models interactively, or configure directly:

| Provider | Setup |
|----------|-------|
| **Nous Portal** | `jacky model` (OAuth, subscription-based) |
| **OpenAI Codex** | `jacky model` (ChatGPT OAuth, uses Codex models) |
| **GitHub Copilot** | `jacky model` (OAuth device code flow, `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, or `gh auth token`) |
| **GitHub Copilot ACP** | `jacky model` (spawns local `copilot --acp --stdio`) |
| **Anthropic** | `jacky model` (Claude Max + extra usage credits via OAuth; also supports Anthropic API key or manual setup-token — see note below) |
| **OpenRouter** | `OPENROUTER_API_KEY` in `~/.jacky/.env` |
| **Fireworks AI** | `FIREWORKS_API_KEY` in `~/.jacky/.env` (provider: `fireworks`; aliases: `fireworks-ai`, `fw`) |
| **NovitaAI** | `NOVITA_API_KEY` in `~/.jacky/.env` (provider: `novita`, 200+ models, Model API, Agent Sandbox, GPU Cloud) |
| **z.ai / GLM** | `GLM_API_KEY` in `~/.jacky/.env` (provider: `zai`) |
| **Kimi / Moonshot** | `KIMI_API_KEY` in `~/.jacky/.env` (provider: `kimi-coding`) |
| **Kimi / Moonshot (China)** | `KIMI_CN_API_KEY` in `~/.jacky/.env` (provider: `kimi-coding-cn`; aliases: `kimi-cn`, `moonshot-cn`) |
| **Arcee AI** | `ARCEEAI_API_KEY` in `~/.jacky/.env` (provider: `arcee`; aliases: `arcee-ai`, `arceeai`) |
| **GMI Cloud** | `GMI_API_KEY` in `~/.jacky/.env` (provider: `gmi`; aliases: `gmi-cloud`, `gmicloud`) |
| **MiniMax** | `MINIMAX_API_KEY` in `~/.jacky/.env` (provider: `minimax`) |
| **MiniMax China** | `MINIMAX_CN_API_KEY` in `~/.jacky/.env` (provider: `minimax-cn`) |
| **xAI (Grok) — Responses API** | `XAI_API_KEY` in `~/.jacky/.env` (provider: `xai`) |
| **xAI Grok OAuth (SuperGrok)** | `jacky model` → "xAI Grok OAuth (SuperGrok / Premium+)" — browser login, no API key. See [guide](../guides/xai-grok-oauth.md) |
| **Qwen Cloud (Alibaba DashScope)** | `DASHSCOPE_API_KEY` in `~/.jacky/.env` (provider: `alibaba`) |
...
| **Kilo Code** | `KILOCODE_API_KEY` in `~/.jacky/.env` (provider: `kilocode`) |
| **Xiaomi MiMo** | `XIAOMI_API_KEY` in `~/.jacky/.env` (provider: `xiaomi`, aliases: `mimo`, `xiaomi-mimo`) |
| **Tencent TokenHub** | `TOKENHUB_API_KEY` in `~/.jacky/.env` (provider: `tencent-tokenhub`, aliases: `tencent`, `tokenhub`, `tencentmaas`) |
| **OpenCode Zen** | `OPENCODE_ZEN_API_KEY` in `~/.jacky/.env` (provider: `opencode-zen`) |
| **OpenCode Go** | `OPENCODE_GO_API_KEY` in `~/.jacky/.env` (provider: `opencode-go`) |
| **DeepSeek** | `DEEPSEEK_API_KEY` in `~/.jacky/.env` (provider: `deepseek`) |
| **Hugging Face** | `HF_TOKEN` in `~/.jacky/.env` (provider: `huggingface`, aliases: `hf`) |
| **Google / Gemini** | `GOOGLE_API_KEY` (or `GEMINI_API_KEY`) in `~/.jacky/.env` (provider: `gemini`) |
| **Google Vertex AI** | `jacky model` → "Google Vertex AI" (provider: `vertex`; OAuth2 via service-account JSON or ADC, GCP billing) |
| **OpenAI API (direct)** | `OPENAI_API_KEY` in `~/.jacky/.env` (provider: `openai-api`, optional `OPENAI_BASE_URL`) |
| **Azure AI Foundry** | `jacky model` → "Azure AI Foundry" (provider: `azure-foundry`; uses Azure OpenAI / Foundry endpoint and key) |
| **AWS Bedrock** | `jacky model` → "AWS Bedrock" (provider: `bedrock`; standard AWS credentials chain via boto3) |
| **NVIDIA Build** | `NVIDIA_API_KEY` in `~/.jacky/.env` (provider: `nvidia`; NIM-hosted models on build.nvidia.com) |
| **Ollama Cloud** | `jacky model` → "Ollama Cloud" (provider: `ollama-cloud`; cloud-hosted Ollama API) |
| **Qwen OAuth** | `jacky model` → "Qwen OAuth" (provider: `qwen-oauth`; browser PKCE login) |
| **MiniMax OAuth** | `jacky model` → "MiniMax (OAuth)" (provider: `minimax-oauth`; browser PKCE login) |
| **StepFun** | `STEPFUN_API_KEY` in `~/.jacky/.env` (provider: `stepfun`) |
| **LM Studio** | `jacky model` → "LM Studio" (provider: `lmstudio`, optional `LM_API_KEY`) |
| **Custom Endpoint** | `jacky model` → choose "Custom endpoint" (saved in `config.yaml`) |
...
[Nous Portal](https://portal.nousresearch.com) is Nous Research's unified subscription gateway and **the recommended way to run Jacky Agent**. One OAuth login covers 300+ frontier agentic models (Claude, GPT, Gemini, DeepSeek, Qwen, Kimi, GLM, MiniMax, Grok, ...) plus the [Tool Gateway](/user-guide/features/tool-gateway) (web search, image generation, TTS, browser automation) plus [Nous Chat](https://chat.nousresearch.com) — billed against your Nous subscription instead of separate per-provider accounts.

```bash
jacky setup --portal     # fresh install — OAuth + provider + gateway in one command
jacky model              # existing install — pick "Nous Portal" from the list
jacky portal info        # inspect login + routing at any time
```
...
**For full details:** see the dedicated [Nous Portal integration page](/integrations/nous-portal) (what's in the subscription, model catalog, troubleshooting) and the step-by-step [Run Jacky Agent with Nous Portal guide](/guides/run-jacky-with-nous-portal).

**Client identification.** Every Portal request from Jacky Agent carries a `client=jacky-client-v<version>` tag (e.g. `client=jacky-client-v0.13.0`) auto-aligned to your installed release. This is sent on all Portal pathways — main chat loop, auxiliary calls, compression summarizer, web extraction — and lets Portal-side telemetry distinguish Jacky traffic from other clients. No config required; the tag updates automatically when you `jacky update`.

**JWT auth (automatic).** Jacky prefers scoped `inference:invoke` JWTs for Portal requests with the legacy opaque session-key path as a fallback. No configuration is required — credentials are managed by the OAuth flow and rotate transparently. Revoked refresh tokens are quarantined to avoid replay loops.


:::info Codex Note
The OpenAI Codex provider authenticates via device code (open a URL, enter a code). Jacky stores the resulting credentials in its own auth store under `~/.jacky/auth.json` and can import existing Codex CLI credentials from `~/.codex/auth.json` when present. No Codex CLI installation is required.

If a token refresh fails with a terminal error (HTTP 4xx, `invalid_grant`, revoked grant, etc.), Jacky marks the refresh token as dead and stops replaying it so you don't see a flood of identical auth failures. The next request surfaces a typed re-auth message instead. Run `jacky auth add codex-oauth` (or `jacky model` → OpenAI Codex) to start a fresh device-code login; the quarantine clears on the next successful exchange.
:::

:::warning
Even when using Nous Portal, Codex, or a custom endpoint, some tools (vision, web summarization, MoA) use a separate "auxiliary" model. By default (`auxiliary.*.provider: "auto"`), Jacky routes these tasks to your **main chat model** — the same model you picked in `jacky model`. You can override each task individually to route it to a cheaper/faster model (e.g. Gemini Flash on OpenRouter) — see [Auxiliary Models](/user-guide/configuration#auxiliary-models).
:::

:::tip Nous Tool Gateway
Paid Nous Portal subscribers also get access to the **[Tool Gateway](/user-guide/features/tool-gateway)** — web search, image generation, TTS, and browser automation routed through your subscription. No extra API keys needed. On a fresh install, `jacky setup --portal` logs you in, sets Nous as your provider, and turns the gateway on in one command. Existing users can enable it from `jacky model` or per-tool from `jacky tools`. Inspect routing at any time with `jacky portal info`.
:::

### Two Commands for Model Management

Jacky has **two** model commands that serve different purposes:

| Command | Where to run | What it does |
|---------|-------------|--------------|
| **`jacky model`** | Your terminal (outside any session) | Full setup wizard — add providers, run OAuth, enter API keys, configure endpoints |
| **`/model`** | Inside a Jacky chat session | Quick switch between **already-configured** providers and models |

If you're trying to switch to a provider you haven't set up yet (e.g. you only have OpenRouter configured and want to use Anthropic), you need `jacky model`, not `/model`. Exit your session first (`Ctrl+C` or `/quit`), run `jacky model`, complete the provider setup, then start a new session.
...
When you authenticate via `jacky model` → Anthropic OAuth (or via `jacky auth add anthropic --type oauth`), Jacky routes as Claude Code against your Anthropic account. **It only works if you're on a Claude Max plan and have purchased extra usage credits.** The base Max plan allowance (the usage included in Claude Code by default) is not consumed by Jacky — only the extra/overage credits you've added on top are. Claude Pro subscribers cannot use this path.
...
jacky chat --provider anthropic --model claude-sonnet-4-6

# Preferred: authenticate through `jacky model`
# Jacky will use Claude Code's credential store directly when available
jacky model

# Manual override with a setup-token (fallback / legacy)
export ANTHROPIC_TOKEN=***  # setup-token or manual OAuth token
jacky chat --provider anthropic

# Auto-detect Claude Code credentials (if you already use Claude Code)
jacky chat --provider anthropic  # reads Claude Code credential files automatically
```

When you choose Anthropic OAuth through `jacky model`, Jacky prefers Claude Code's own credential store over copying the token into `~/.jacky/.env`. That keeps refreshable Claude credentials refreshable.
...
Jacky supports GitHub Copilot as a first-class provider with two modes:
...
```bash
jacky chat --provider copilot --model gpt-5.4
```
...
If no token is found, `jacky model` offers an **OAuth device code login** — the same flow used by the Copilot CLI and opencode.
...
|------|--------|------------|
| OAuth token | `gho_` | `jacky model` → GitHub Copilot → Login with GitHub |
...
If your `gh auth token` returns a `ghp_*` token, use `jacky model` to authenticate via OAuth instead.
:::

:::info Copilot auth behavior in Jacky
Jacky sends a supported GitHub token (`gho_*`, `github_pat_*`, or `ghu_*`) directly to `api.githubcopilot.com` and includes Copilot-specific headers (`Editor-Version`, `Copilot-Integration-Id`, `Openai-Intent`, `x-initiator`).

On HTTP 401, Jacky now performs a one-shot credential recovery before fallback:
...
Some older community proxies use `api.github.com/copilot_internal/v2/token` exchange flows. That endpoint can be unavailable for some account types (returns 404). Jacky therefore keeps direct-token auth as the primary path and relies on runtime credential refresh + retry for robustness.
:::
...
```bash
jacky chat --provider copilot-acp --model copilot-acp
...
| `JACKY_COPILOT_ACP_COMMAND` | Override the Copilot CLI binary path (default: `copilot`) |
| `JACKY_COPILOT_ACP_ARGS` | Override ACP args (default: `--acp --stdio`) |
...
```bash
# Fireworks AI
jacky chat --provider fireworks --model accounts/fireworks/models/kimi-k2p6
# Requires: FIREWORKS_API_KEY in ~/.jacky/.env

# NovitaAI Model API
jacky chat --provider novita --model moonshotai/kimi-k2.5
# Requires: NOVITA_API_KEY in ~/.jacky/.env

# z.ai / ZhipuAI GLM
jacky chat --provider zai --model glm-5
# Requires: GLM_API_KEY in ~/.jacky/.env

# Kimi / Moonshot AI (international: api.moonshot.ai)
jacky chat --provider kimi-coding --model kimi-for-coding
# Requires: KIMI_API_KEY in ~/.jacky/.env

# Kimi / Moonshot AI (China: api.moonshot.cn)
jacky chat --provider kimi-coding-cn --model kimi-k2.5
# Requires: KIMI_CN_API_KEY in ~/.jacky/.env

# MiniMax (global endpoint)
jacky chat --provider minimax --model MiniMax-M2.7
# Requires: MINIMAX_API_KEY in ~/.jacky/.env

# MiniMax (China endpoint)
jacky chat --provider minimax-cn --model MiniMax-M2.7
# Requires: MINIMAX_CN_API_KEY in ~/.jacky/.env

# Qwen Cloud / DashScope (Qwen models)
jacky chat --provider alibaba --model qwen3.5-plus
# Requires: DASHSCOPE_API_KEY in ~/.jacky/.env

# Xiaomi MiMo
jacky chat --provider xiaomi --model mimo-v2-pro
# Requires: XIAOMI_API_KEY in ~/.jacky/.env

# Tencent TokenHub (Hy3 Preview)
jacky chat --provider tencent-tokenhub --model hy3-preview
# Requires: TOKENHUB_API_KEY in ~/.jacky/.env

# Arcee AI (Trinity models)
jacky chat --provider arcee --model trinity-large-thinking
# Requires: ARCEEAI_API_KEY in ~/.jacky/.env

# GMI Cloud
# Use the exact model ID returned by GMI's /v1/models endpoint.
jacky chat --provider gmi --model zai-org/GLM-5.1-FP8
# Requires: GMI_API_KEY in ~/.jacky/.env
```

Fireworks uses its native slash-form catalog IDs, such as `accounts/fireworks/models/kimi-k2p6`. Run `jacky model`, choose **Fireworks AI**, and select from the live catalog or enter another Fireworks model ID. The default endpoint is `https://api.fireworks.ai/inference/v1`; configure a different endpoint through `model.base_url` in `config.yaml`, not `.env`.
...
When using the Z.AI / GLM provider, Jacky automatically probes multiple endpoints (global, China, coding variants) to find one that accepts your API key. You don't need to set `GLM_BASE_URL` manually — the working endpoint is detected and cached automatically.
:::

### xAI (Grok) — Responses API + Prompt Caching

xAI is wired through the Responses API (`codex_responses` transport) for automatic reasoning support on Grok 4 models — no `reasoning_effort` parameter needed, the server reasons by default. Set `XAI_API_KEY` in `~/.jacky/.env` and pick xAI in `jacky model`, or drop `grok` as a shortcut into `/model grok-4-fast-reasoning`.

SuperGrok and X Premium+ subscribers can sign in with browser OAuth instead of using an API key — pick **xAI Grok OAuth (SuperGrok / Premium+)** in `jacky model`, or run `jacky auth add xai-oauth`. The same OAuth bearer token is automatically reused by direct-to-xAI tools (TTS, image gen, video gen, transcription). See the [xAI Grok OAuth guide](../guides/xai-grok-oauth.md) for the full flow — and if Jacky runs on a remote host, also see [OAuth over SSH / Remote Hosts](../guides/oauth-over-ssh.md) for the required `ssh -L` tunnel.

When using xAI as a provider (any base URL containing `x.ai`), Jacky automatically enables prompt caching by sending the `x-grok-conv-id` header with every API request. This routes requests to the same server within a conversation session, allowing xAI's infrastructure to reuse cached system prompts and conversation history.
...
xAI also ships a dedicated TTS endpoint (`/v1/tts`). Select **xAI TTS** in `jacky tools` → Voice & TTS, or see the [Voice & TTS](../user-guide/features/tts.md#text-to-speech) page for config.

**Retired xAI model migration (May 15, 2026):** xAI is retiring `grok-4*`, `grok-3`, `grok-code-fast-1`, and `grok-imagine-image-pro` on 2026-05-15. `jacky doctor` and `jacky chat` startup both detect any config still pointing at a retired ref and print the recommended replacement. Use `jacky migrate xai` for a one-shot config rewrite — dry-run by default, add `--apply` to write changes (a timestamped `config.yaml.bak-pre-migrate-xai-*` backup is created automatically).

```bash
jacky migrate xai          # preview replacements
jacky migrate xai --apply  # rewrite ~/.jacky/config.yaml in place
```
...
# Short alias
jacky chat --provider novita-ai --model deepseek/deepseek-v3-0324
```
...
[Ollama Cloud](https://ollama.com/cloud) hosts the same open-weight catalog as local Ollama but without the GPU requirement. Pick it in `jacky model` as **Ollama Cloud**, paste your API key from [ollama.com/settings/keys](https://ollama.com/settings/keys), and Jacky auto-discovers the available models.
...
jacky chat --provider bedrock --model us.anthropic.claude-sonnet-4-6

# Or with explicit env vars
AWS_PROFILE=myprofile AWS_REGION=us-east-1 jacky chat --provider bedrock --model us.anthropic.claude-sonnet-4-6
```
...
`VERTEX_PROJECT_ID` / `VERTEX_REGION` env vars override the `config.yaml` values. Install with `pip install 'jacky-agent[vertex]'` (or let Jacky lazy-install `google-auth` on first use). See the [Google Vertex AI guide](/guides/google-vertex) for the full walkthrough, and the [Google Gemini guide](/guides/google-gemini) for the static-API-key AI Studio path instead.

### Qwen Portal (OAuth)

Alibaba's Qwen Portal with browser-based OAuth login. Pick **Qwen OAuth (Portal)** in `jacky model`, sign in through the browser, and Jacky persists the refresh token.

```bash
jacky model
# → pick "Qwen OAuth (Portal)"
# → browser opens; sign in with your Alibaba account
# → confirm — credentials are saved to ~/.jacky/auth.json

jacky chat   # uses portal.qwen.ai/v1 endpoint
```
...
Set `JACKY_QWEN_BASE_URL` only if the portal endpoint relocates (default: `https://portal.qwen.ai/v1`).
...
If you're subscribed to Alibaba's **Coding Plan** (a pricing SKU separate from standard DashScope API access), Jacky exposes it as its own first-class provider: `alibaba-coding-plan`. Endpoint: `https://coding-intl.dashscope.aliyuncs.com/v1`. It's OpenAI-compatible like the regular `alibaba` provider but with a different base URL and billing surface.
...
```bash
jacky chat --provider alibaba_coding --model qwen3-coder-plus
```
...
MiniMax-M2.7 via browser OAuth login — no API key needed. Pick **MiniMax (OAuth)** in `jacky model`, sign in through the browser, and Jacky persists the access + refresh tokens. Uses the Anthropic Messages-compatible endpoint (`/anthropic`) under the hood.
...
jacky chat   # uses api.minimax.io/anthropic endpoint
```
...
jacky chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
# Requires: NVIDIA_API_KEY in ~/.jacky/.env

# Local NIM endpoint — override base URL
NVIDIA_BASE_URL=http://localhost:8000/v1 jacky chat --provider nvidia --model nvidia/nemotron-3-super-120b-a12b
```
...
Jacky automatically attaches the NIM billing-origin header on every request to `build.nvidia.com` — no configuration needed. This routes consumption against the correct origin in NVIDIA's billing dashboard.
...
```bash
# GMI Cloud
jacky chat --provider gmi --model deepseek-ai/DeepSeek-V3.2
# Requires: GMI_API_KEY in ~/.jacky/.env
```
...
```bash
# StepFun
jacky chat --provider stepfun --model step-3.5-flash
# Requires: STEPFUN_API_KEY in ~/.jacky/.env
```
...
```bash
# Use any available model
jacky chat --provider huggingface --model Qwen/Qwen3.5-397B-A17B
# Requires: HF_TOKEN in ~/.jacky/.env

# Short alias
jacky chat --provider hf --model deepseek-ai/DeepSeek-V3.2
```
...
Jacky Agent works with **any OpenAI-compatible API endpoint**. If a server implements `/v1/chat/completions`, you can point Jacky at it. This means you can use local models, GPU inference servers, multi-provider routers, or any third-party API.
...
:::warning jacky model vs /model
**`jacky model`** (run from your terminal, outside any chat session) is the **full provider setup wizard**. Use it to add new providers, run OAuth flows, enter API keys, and configure custom endpoints.

**`/model`** (typed inside an active Jacky chat session) can only **switch between providers and models you've already set up**. It cannot add new providers, run OAuth, or prompt for API keys. If you've only configured one provider (e.g. OpenRouter), `/model` will only show models for that provider.

**To add a new provider:** Exit your session (`Ctrl+C` or `/quit`), run `jacky model`, set up the new provider, then start a new session.
:::
...
When switching providers, Jacky persists the base URL and provider to config so the change survives restarts. When switching away from a custom endpoint to a built-in provider, the stale base URL is automatically cleared.
...
Jacky Agent requires at least **64,000 tokens** of context for agent use with tools. Smaller windows are rejected at startup because the system prompt, tool schemas, and working conversation state need enough room for reliable multi-step workflows.
...
**You cannot set context length through the OpenAI-compatible API** (`/v1/chat/completions`). It must be configured server-side or via a Modelfile. This is the #1 source of confusion when integrating Ollama with tools like Jacky.
:::
...
| Flag | Purpose |
|------|---------|
| `--enable-auto-tool-choice` | Required for `tool_choice: "auto"` (the default in Jacky) |
| `--tool-call-parser <name>` | Parser for the model's tool call format |

Supported parsers: `jacky` (Qwen 2.5, Jacky 2/3), `llama3_json` (Llama 3.x), `mistral`, `deepseek_v3`, `deepseek_v31`, `xlam`, `pythonic`. Without these flags, tool calls won't work — the model will output tool calls as text.

**Qwen reasoning parsers:** Jacky preserves structured reasoning metadata such as `reasoning`, `reasoning_content`, and streamed reasoning deltas when OpenAI-compatible servers return them. That metadata is treated as reasoning/thinking trace data, not as a replacement for the assistant's visible answer. For Qwen reasoning models served by vLLM, make sure the final user-visible response still appears in `content`. If `--reasoning-parser qwen3` leaves `content` empty in your deployment, either disable that parser or pass a server-supported request option such as `chat_template_kwargs.enable_thinking: false` through `extra_body`.
...
**Context length (`-c`):** Recent builds default to `0` which reads the model's training context from the GGUF metadata. For models with 128k+ training context, this can OOM trying to allocate the full KV cache. Set `-c` explicitly to at least 64,000 tokens for Jacky. If using parallel slots (`-np`), the total context is divided among slots — with `-c 64000 -np 4`, each slot only gets 16k, which is below Jacky' minimum per active session.

Then configure Jacky to point at it:
...
Without `--jinja`, llama-server ignores the `tools` parameter entirely. The model will try to call tools by writing JSON in its response text, but Jacky won't recognize it as a tool call — you'll see raw JSON like `{"name": "web_search", ...}` printed as a message instead of an actual search.
...
Jacky will automatically load a LM Studio model with 64K context length
...
**Tool calling:** Supported since LM Studio 0.3.6. Models with native tool-calling training (Qwen 2.5, Llama 3.x, Mistral, Jacky) are auto-detected and shown with a tool badge. Other models use a generic fallback that may be less reliable.

---

### WSL2 Networking (Windows Users)

Since Jacky Agent requires a Unix environment, Windows users run it inside WSL2. If your model server (Ollama, LM Studio, etc.) runs on the **Windows host**, you need to bridge the network gap — WSL2 uses a virtual network adapter with its own subnet, so `localhost` inside WSL2 refers to the Linux VM, **not** the Windows host.
...
Use that IP in your Jacky config:
...
If you get a JSON response listing your models, you're good. Use that same URL as the `base_url` in your Jacky config.

---

### Troubleshooting Local Models

These issues affect **all** local inference servers when used with Jacky.

#### "Connection refused" from WSL2 to a Windows-hosted model server

If you're running Jacky inside WSL2 and your model server on the Windows host, `http://localhost:<port>` won't work in WSL2's default NAT networking mode. See [WSL2 Networking](#wsl2-networking-windows-users) above for the fix.
...
| **vLLM** | Add `--enable-auto-tool-choice --tool-call-parser jacky` |
...
**Cause:** Context window is too small. When the conversation exceeds the context limit, most servers silently drop older messages. Jacky's system prompt + tool schemas alone can use 4k–8k tokens.

**Diagnosis:**

```bash
# Check what Jacky thinks the context is
...
Jacky auto-detects context length from your server's `/v1/models` endpoint. If the server reports a low value (or doesn't report one at all), Jacky uses the model's declared limit which may be wrong.
...
**Possible causes:**
1. **Low output cap (`max_tokens`) on the server** — SGLang defaults to 128 tokens per response. Set `--default-max-tokens` on the server or configure Jacky with `model.max_tokens` in config.yaml. Note: `max_tokens` controls response length only — it is unrelated to how long your conversation history can be (that is `context_length`).
2. **Context exhaustion** — The model filled its context window. Increase `model.context_length` or enable [context compression](/user-guide/configuration#context-compression) in Jacky.
...
Then configure Jacky with `jacky model` → Custom endpoint → `http://localhost:4000/v1`.
...
Then configure Jacky with `jacky model` → Custom endpoint → `http://localhost:8402/v1` → model name `blockrun/auto`.
...
Configure any of these with `jacky model` → Custom endpoint, or in `config.yaml`:
...
**`context_length`** is the **total context window** — the combined budget for input *and* output tokens (e.g. 200,000 for Claude Opus 4.6). Jacky uses this to decide when to compress history and to validate API requests.
...
Jacky uses a multi-source resolution chain to detect the correct context window for your model and provider:
...
`jacky model` will prompt for context length when configuring a custom endpoint. Leave it blank for auto-detection.
...
custom_providers:
  - name: local
    base_url: http://localhost:8080/v1
    # api_key omitted — Jacky uses "no-key-required" for keyless local servers
  - name: work
    base_url: https://gpu-server.internal.corp/v1
    key_env: CORP_API_KEY
    api_mode: chat_completions   # set explicitly by `jacky model` → Custom Endpoint wizard; auto-detection still happens as a fallback
...
Some OpenAI-compatible endpoints need provider-specific request body fields. Add an `extra_body` map to the matching custom provider and Jacky will merge it into each chat-completions request for that endpoint:
...
The `jacky model` → Custom Endpoint wizard now prompts for `api_mode` explicitly and persists your answer to `config.yaml`. URL-based auto-detection (e.g. `/anthropic` paths → `anthropic_messages`) still happens as a fallback when the field is left blank.

**Native vision for custom-provider models.** If your custom endpoint serves a vision-capable model that isn't in models.dev, set `model.supports_vision: true` so Jacky routes attached images natively (as `image_url` parts) instead of pre-processing them through `vision_analyze`. Single knob — no need to also set `agent.image_input_mode: native`.
...
You can also select named custom providers from the interactive `jacky model` menu.

---

### Cookbook: Together AI, Groq, Perplexity

The cloud providers listed in [Other Compatible Providers](#other-compatible-providers) all speak OpenAI's REST dialect, so they wire up the same way under `custom_providers:`. Three worked recipes follow. Each drops into `~/.jacky/config.yaml` and the matching API key goes in `~/.jacky/.env`.
...
Together's `/v1/models` endpoint works, so `jacky model` can auto-discover available models.
...
:::tip Troubleshooting
- `jacky doctor` should print no `Unknown provider` warnings for any of these names after the CLI validator fixes in #15083.
- If a provider's `/v1/models` endpoint is unreachable (Perplexity is the common one), `jacky model` will persist the model with a warning rather than hard-reject — see #15136.
...
:::tip
You can switch between providers at any time with `jacky model` — no restart required. Your conversation history, memory, and skills carry over regardless of which provider you use.
:::
...
2. Point Jacky at your instance (no API key needed):
   ```bash
   jacky config set FIRECRAWL_API_URL http://localhost:3002
   ```
...
When using OpenRouter, you can control how requests are routed across providers. Add a `provider_routing` section to `~/.jacky/config.yaml`:
...
OpenRouter ships an experimental coding-model router at `openrouter/pareto-code` that auto-routes requests to the cheapest model meeting a coding-quality bar (ranked by [Artificial Analysis](https://artificialanalysis.ai/)). Pick this model and tune the `min_coding_score` knob in `~/.jacky/config.yaml`:
...
Configure a chain of backup providers Jacky tries in order when the primary model fails (rate limits, server errors, auth failures). The canonical format is a top-level `fallback_providers:` list:
...
:::tip
Fallback is configured exclusively through `config.yaml` — or interactively via `jacky fallback`. For full details on when it triggers, how the chain advances, and how it interacts with auxiliary tasks and delegation, see [Fallback Providers](/user-guide/features/fallback-providers).
:::

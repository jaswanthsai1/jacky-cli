...
sidebar_position: 1
title: "Nous Portal"
description: "One subscription, 300+ frontier models, the Tool Gateway, and Nous Chat — the recommended way to run Jacky Agent"
---
...
That single command runs the Portal OAuth, lets you pick a Nous model, sets Nous as your inference provider in `config.yaml`, and turns on the Tool Gateway. You're ready to `jacky chat` immediately after.
...
| **Jacky** | Hermes-4-70B, Hermes-4-405B (chat, see [note below](#a-note-on-hermes-4)) |
...
The same subscription unlocks the [Tool Gateway](/user-guide/features/tool-gateway), which routes Jacky Agent's tool calls through Nous-managed infrastructure. Five backends, one login:
...
Because everything routes through one OAuth-authenticated Portal session, you don't accumulate a `.env` file with a dozen long-lived API keys. The refresh token at `~/.jacky/auth.json` is the only credential on disk, and Jacky mints short-lived JWTs from it per request — see [Token handling](#token-handling) below.
...
The Portal's own [model info page](https://portal.nousresearch.com/info) carries the same warning, so this isn't a Jacky-side opinion — it's the official guidance from Nous Research.
...
2. Stores the refresh token at `~/.jacky/auth.json`
3. Lets you pick a Nous model from the curated list (or skip to keep your current one)
4. Sets Nous as your inference provider in `~/.jacky/config.yaml` (when you pick a model)
5. Turns on the Tool Gateway (web, image, TTS, browser routing)
6. Returns you to your terminal ready to `jacky chat`
...
If you already have Jacky configured with OpenRouter, Anthropic, or any other provider and you want to add the Portal alongside them:
...
Your existing providers stay configured. You can switch between them with `/model` mid-session or `jacky model` between sessions — the Portal becomes one of your available providers, not your only one.

### Headless / SSH / remote setup

OAuth needs a browser, but the loopback callback runs on the machine where Jacky is running. For remote hosts, see [OAuth over SSH / Remote Hosts](/guides/oauth-over-ssh) — the same patterns work for the Portal as for any other OAuth-based provider (`ssh -L` port forwarding).

### Profile setup

If you use [Jacky profiles](/user-guide/profiles), the Portal refresh token is automatically shared across all profiles via a shared token store. Sign in once on any profile, and the rest pick it up automatically — no need to repeat the OAuth flow per profile.
...
```bash
jacky portal            # log in to Nous Portal + set it up (one-shot onboarding)
jacky portal info       # login status, subscription info, model + gateway routing
jacky portal status     # alias for `portal info`
jacky portal tools      # detailed Tool Gateway catalog with per-tool routing
...
If you already have, say, a Browserbase account and want to keep using it while routing web search and image generation through Nous, that's supported. Use `jacky tools` to pick backends per tool:
...
- **CLI shortcut:** `jacky portal open` (opens the same page in your default browser)

## Configuration reference

After `jacky setup --portal`, `~/.jacky/config.yaml` will look like:
...
The OAuth refresh token is stored separately at `~/.jacky/auth.json` (not in `config.yaml` — credentials and configuration are kept separate by design).

## Token handling

Jacky mints a short-lived JWT from your stored Portal refresh token on each inference call rather than reusing a long-lived API key. The token lifecycle is fully automatic — refresh, mint, retry on transient 401 — and you never see it.

If the Portal invalidates the refresh token (password change, manual revoke, session expiry), the invalid refresh token is **quarantined locally** so Jacky stops replaying it and you don't see a stream of identical 401s. The next call surfaces a clear "re-authentication required" message. Run `jacky auth add nous` to log in again; the quarantine clears on the next successful login.

## Troubleshooting

### `jacky portal info` shows "not logged in"
...
or use `jacky model` and re-select Nous Portal.

### Got a "re-authentication required" message mid-session

Your Portal refresh token was invalidated (password change, manual revoke, or session expiry). Run `jacky auth add nous` and your next request will use the new credentials. Any quarantine on the old token clears automatically on successful re-login.
...
If a model is genuinely missing, [open an issue](https://github.com/jaswanthsai1/jacky-cli/issues) — we surface the Portal's catalog to Jacky and gaps usually mean a routing config we can update.

### Bills not appearing on my Portal account

Check `jacky portal info` first — if it shows you're using a different provider (`Model: currently openrouter` instead of `using Nous as inference provider`), your local config has drifted. Run `jacky model`, pick Nous Portal, and the next request will route through your subscription.

## See also

- **[Tool Gateway](/user-guide/features/tool-gateway)** — Full details on every gateway tool, per-tool config, and pricing
- **[Subscription proxy](/user-guide/features/subscription-proxy)** — Use your Portal subscription from non-Jacky tools (other agents, scripts, third-party clients)
...
- **[Profiles](/user-guide/profiles)** — Multiple Jacky configurations sharing one Portal login

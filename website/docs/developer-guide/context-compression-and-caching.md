...
Jacky Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `jacky plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Jacky has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|jacky|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `jacky`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Jacky raises the
trigger to **85%** (~231K) and shows a notice with the opt-out command. The
notice is shown once per profile — a marker under `$JACKY_HOME`
...
```bash
jacky config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
jacky config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so Jacky' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Jacky
  records the resulting compaction events (compression counters, session
  events). Set `jacky` to let Jacky' compression threshold initiate
  app-server compaction, or `off` to disable Jacky-initiated automatic
  compaction entirely (codex may still compact natively).

Jacky' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Jacky' summary compressor.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Jacky
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Jacky can avoid; user-facing docs for `/model`, fallback providers, and

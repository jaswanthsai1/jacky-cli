...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to Jacky Agent — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to Jacky Agent! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying Jacky core? Start with [Build a Jacky Plugin](../developer-guide/plugins/index.md)
- Building a new built-in core tool for Jacky itself? Start with [Adding Tools](./adding-tools.md)
...
You can also run a fully isolated Jacky instance (throwaway JACKY_HOME, separate Electron
userData, distinct Electron app name to avoid the single-instance lock):

```bash
scripts/dev-sandbox.sh python -m jacky_cli.main
scripts/dev-sandbox.sh --persistent python -m jacky_cli.main desktop  # state survives restarts, but lives in the worktree :)
```
...
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.jacky/.env
```
...
- **Profile-safe paths**: Never hardcode `~/.jacky` — use `get_jacky_home()` from `jacky_constants` for code paths and `display_jacky_home()` for user-facing messages. See [AGENTS.md](https://github.com/jaswanthsai1/jacky-cli/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/jaswanthsai1/jacky-cli/blob/main/LICENSE).

...
title: "Build a Jacky Plugin"
description: "Step-by-step guide to building a complete Jacky plugin with tools, hooks, data files, and skills"
---

# Build a Jacky Plugin

This guide walks through building a complete Jacky plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Jacky has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| An **STT backend** (custom whisper / ASR CLI) | [Voice Message Transcription](/user-guide/features/tts#voice-message-transcription-stt) — set `JACKY_LOCAL_STT_COMMAND` to a shell template |
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.jacky/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `jacky skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
```bash
mkdir -p ~/.jacky/plugins/calculator
cd ~/.jacky/plugins/calculator
```
...
This tells Jacky: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Jacky may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `jacky my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Jacky continues fine
...
If your plugin doesn't show up — or shows up but isn't loading — set `JACKY_PLUGINS_DEBUG=1` to get verbose discovery logs on stderr:

```bash
JACKY_PLUGINS_DEBUG=1 jacky plugins list
```
...
The same logs are always written to `~/.jacky/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
jacky logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `jacky plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.jacky/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.jacky/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.jacky/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.jacky/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `jacky plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Jacky into installing arbitrary packages — only specs Jacky itself ships are eligible |
...
auditable in `~/.jacky/logs/agent.log`. Plugins load after built-in
...
Per-hook context is capped at `10,000` characters by default. Anything above the cap is written to `$JACKY_HOME/hook_outputs/<session_id>/<uuid>.txt` and replaced with a head/tail preview plus the saved path. The model can read the full content via `read_file` or `terminal` if it genuinely needs it. This keeps a runaway plugin from inflating every subsequent turn's prompt and blowing out the prompt cache prefix. Tune in `config.yaml`:
...
hooks:
  output_spill:
    ...
    # directory: null      # default: $JACKY_HOME/hook_outputs
```
...
- **The system prompt is Jacky's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `jacky <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for jacky my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: jacky my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for jacky my-plugin."""
...
After registration, users can run `jacky my-plugin status`, `jacky my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `jacky name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `jacky chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:

- **`ctx.profile_name`** — the active profile name (e.g. `"default"`, or the assignee profile in a kanban worker). Derived from `JACKY_HOME`, so it works everywhere with no `_cli_ref` dependency.
...
For running a full `jacky <subcommand>` (e.g. `jacky kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "jacky kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Jacky from a hook.
...
Jacky has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.jacky/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `jacky gateway status` without SDK instantiation.
...
Jacky also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Jacky without any Python plugin. Declare them in `~/.jacky/config.yaml`:
...
Jacky connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.jacky/hooks/<name>/`:

```yaml
# ~/.jacky/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.jacky/hooks/long-task-alert/handler.py
...
jacky skills search my-workflow --source myorg/skills-repo
jacky skills install myorg/skills-repo/my-workflow
```
...
For STT, point `JACKY_LOCAL_STT_COMMAND` at a shell template. Supported placeholders: `{input_path}`, `{output_path}`, `{format}`, `{voice}`, `{model}`, `{speed}` (TTS); `{input_path}`, `{output_dir}`, `{language}`, `{model}` (STT). Any path-interacting CLI is automatically a plugin.
...
```toml
# pyproject.toml
[project.entry-points."jacky_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install jacky-plugin-calculator
# Plugin auto-discovered on next jacky startup
```
...
```python
# Wrong — will break if Jacky passes extra context
def handler(args):
    ...

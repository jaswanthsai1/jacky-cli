...
The ACP adapter wraps Jacky' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
jacky acp / jacky-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> parse --version / --check / --setup before server startup
  -> load ~/.jacky/.env
  -> configure stderr logging
  -> construct JackyACPAgent
  -> acp.run_agent(agent, use_unstable_protocol=True)
```

The Zed ACP Registry path launches the same adapter through `uvx --from 'jacky-agent[acp]==<version>' jacky-acp`, pointed at the `jacky-agent` PyPI release.
...
- `allow_once` -> Jacky `once`
- `allow_always` -> Jacky `always`
- reject options -> Jacky `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps Jacky tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["jacky-acp"])
...
Instead it reuses Jacky' runtime resolver:

- `acp_adapter/auth.py`
- `jacky_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Jacky provider/credentials. It also always advertises a terminal setup auth method (`jacky-setup`, args `--setup`) so first-run registry clients can open Jacky' interactive model/provider configuration before starting a normal ACP session.
...
- ACP sessions are persisted to the shared `~/.jacky/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `jacky-acp` toolset definition
- `jacky_cli/main.py` — `jacky acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `jacky-acp` script

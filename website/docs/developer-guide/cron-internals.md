...
description: "How Jacky stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `jacky_cli/cron.py` | CLI `jacky cron` subcommands |
...
Jobs are stored in `~/.jacky/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `jacky cron` commands are run or during active CLI sessions.
...
```python
# ~/.jacky/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.jacky/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `jacky cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `jacky cron` CLI provides direct job management:

```bash
jacky cron list                    # Show all jobs
jacky cron create                  # Interactive job creation (alias: add)
jacky cron edit <job_id>           # Edit job configuration
jacky cron pause <job_id>          # Pause a running job
jacky cron resume <job_id>         # Resume a paused job
jacky cron run <job_id>            # Trigger immediate execution
jacky cron remove <job_id>         # Delete a job
```

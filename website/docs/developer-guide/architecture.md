...
sidebar_position: 1
title: "Architecture"
description: "Jacky Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Jacky Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ jacky_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # JackyCLI — interactive terminal UI (large file)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── jacky_state.py           # SQLite session/state database with FTS5
├── jacky_constants.py       # JACKY_HOME, profile-aware paths
...
│
├── jacky_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `jacky` subcommands (large file)
...
│   ├── skills_config.py      # jacky skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # jacky tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # jacky gateway start/stop
│
...
```text
User input → JackyCLI.process_input()
...
Three discovery sources: `~/.jacky/plugins/` (user), `.jacky/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `jacky plugins` or `config.yaml`.
...
Exposes Jacky as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`jacky -p <name>`) gets its own JACKY_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |

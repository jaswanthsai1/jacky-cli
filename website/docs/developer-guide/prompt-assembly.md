...
description: "How Jacky builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Jacky deliberately separates:
...
2. **context** — caller-supplied `system_message` plus project context files (`.jacky.md` / `AGENTS.md` / `CLAUDE.md` / `.cursorrules`)
...
The platform hint (Layer 10 above) is the per-surface guidance Jacky
...
`SOUL.md` lives at `~/.jacky/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_jacky_md(cwd_path)       # 1. .jacky.md / JACKY.md (walks to git root)
    ...
    # SOUL.md from JACKY_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.jacky.md`, `JACKY.md` | CWD up to git root | Jacky-native project config |
...
- **YAML frontmatter stripped** — `.jacky.md` frontmatter is removed (reserved for future config overrides)
...
`pre_llm_call` plugin context also lands in this API-call-time path: it is appended to the current turn's **user message**, not written into the cached system prompt. When multiple plugins return context, Jacky concatenates those context blocks (see [Hooks → `pre_llm_call`](../user-guide/features/hooks.md#pre_llm_call)).
...
1. `.jacky.md` / `JACKY.md` (walks to git root)
...
Most users should treat `agent/prompt_builder.py` as implementation code, not a configuration surface. The supported customization path is to change the prompt inputs Jacky already loads, rather than editing Python templates in place.

### Use these surfaces first

- `~/.jacky/SOUL.md` — replace the built-in default identity block with your own agent persona and standing behavior.
- `~/.jacky/MEMORY.md` and `~/.jacky/USER.md` — provide durable cross-session facts and user profile data that should be snapshotted into new sessions.
- Project context files such as `.jacky.md`, `JACKY.md`, `AGENTS.md`, `CLAUDE.md`, or `.cursorrules` — inject repo-specific working rules.
- Skills — package reusable workflows and references without editing core prompt code.
- Optional system prompt config / API overrides — add deployment-specific instruction text without forking Jacky.
- Ephemeral overlays such as `JACKY_EPHEMERAL_SYSTEM_PROMPT` or prefill messages — add turn-scoped guidance that should not become part of the cached prompt prefix.
...
- if you want to change how Jacky assembles prompts for everyone, change Python and treat it as a code contribution

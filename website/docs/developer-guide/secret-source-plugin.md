...
description: "How to build a secret-manager backend plugin for Jacky Agent"
---

# Building a Secret Source Plugin

Secret sources resolve provider credentials from an external secret manager (a vault, a password manager, an OS keystore, a custom script) into environment variables at process startup — after `~/.jacky/.env` loads, before Jacky reads credentials. Bitwarden and 1Password ship in-tree; **every other backend is a plugin**. This guide covers building one.
...
:::note Timing
Plugin discovery runs later in startup than the first `load_jacky_dotenv()` call, so a plugin source is not consulted by the very first env load of the process that discovers it. It IS consulted by every subsequently spawned Jacky process (gateway children, cron sessions, subagents). Bundled sources cover first-process bootstrap.
:::
...
Subclass the kit from the Jacky repo (`tests/secret_sources/conformance.py`) in your plugin's tests:

...
- 点击"New SSH key"
- 粘贴公钥内容
- 填写标题，如"jacky-agent-&lt;machine-name>"
...
elif [ -f ~/.jacky/.env ] && grep -q "^GITHUB_TOKEN=" ~/.jacky/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.jacky/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"

...
Jacky Agent 使用 SQLite 数据库（`~/.jacky/state.db`）跨 CLI 和 gateway 会话持久化会话元数据、完整消息历史及模型配置。这替代了早期的逐会话 JSONL 文件方案。
...
多个 jacky 进程（gateway + CLI 会话 + worktree agent）共享同一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：
...
db = SessionDB()                           # 默认：~/.jacky/state.db
...
该路径由 `jacky_constants.get_jacky_home()` 推导，默认解析为 `~/.jacky/`，或 `JACKY_HOME` 环境变量的值。

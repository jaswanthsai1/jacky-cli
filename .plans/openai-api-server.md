# OpenAI-Compatible API Server for Jacky Agent
...
SSE streaming. By exposing this endpoint, jacky-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  jacky-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.jacky/.env`
2. Run `jacky gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with jacky-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns jacky-agent as a model) |
...
Authorization: Bearer jacky-api-key-here
...
But jacky-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "jacky-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured jacky-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of jacky-agent's core prompt
...
Once implemented, jacky-agent works as a drop-in backend for:

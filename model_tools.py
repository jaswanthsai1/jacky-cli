...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_jacky_home() under the active profile.
...
#   - cli.py, jacky_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            cfg_fp,
            bool(os.environ.get("JACKY_KANBAN_TASK")),
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("JACKY_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by JACKY_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like jacky-cli)
        ...
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("jacky-") or (get_toolset(toolset_name) or {}).get("posture"):
                    ...
                    if (not quiet_mode and toolset_name.startswith("jacky-")
    ...
    # window). Core Jacky tools (toolsets._JACKY_CORE_TOOLS) are NEVER
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from jacky_cli.config import load_config as _load
        cfg = _load() or {}
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to jacky-agent's coercion layer.
...
def _emit_post_tool_call_hook(
    *,
    ...
    """
    try:
        from jacky_cli.plugins import has_hook, invoke_hook
...
def handle_function_call(
    ...
    if not skip_tool_request_middleware:
        try:
            from jacky_cli.middleware import apply_tool_request_middleware
    ...
    try:
        ...
        try:
            ...
            from jacky_cli.middleware import run_tool_execution_middleware

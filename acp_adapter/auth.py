"""ACP auth helpers — detect and advertise Jacky authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "jacky-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Jacky runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Jacky can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Jacky.
    ...
    not have Jacky provider credentials configured yet, so Jacky always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Jacky using the currently configured "

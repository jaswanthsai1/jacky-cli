"""ACP agent server — exposes Jacky Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_jacky_interactive_context,
    set_jacky_interactive_context,
)

logger = logging.getLogger(__name__)

try:
    from jacky_cli import __version__ as JACKY_VERSION
except Exception:
    JACKY_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Jacky running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Jacky cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Jacky/OpenAI-compatible user content payload."""
...
class JackyACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Jacky AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Jacky version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Jacky maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from jacky_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from jacky_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Jacky estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_jacky_session_id: str,
        previous_jacky_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.jacky.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_jacky_session_id,
                previous_jacky_session_id=previous_jacky_session_id,
            )
        except Exception:
            logger.debug(
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_jacky_session_id: Optional[str] = None,
        previous_jacky_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Jacky changes it.

        When the internal Jacky head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_jacky_session_id`` so the
        attached ``_meta.jacky.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # jacky_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_jacky_session_id or session_id,
            previous_jacky_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["jacky-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="jacky-agent", version=JACKY_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Jacky' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Jacky setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Jacky
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Jacky on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Jacky' local kawaii waiting/status
        ...
        # tools.approval (set_jacky_interactive_context) rather than
        # os.environ["JACKY_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue JACKY_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind JACKY_SESSION_KEY for this session so per-session caches
            ...
            interactive_token = set_jacky_interactive_context(True)
            ...
            previous_session_id = os.environ.get("JACKY_SESSION_ID")
            os.environ["JACKY_SESSION_ID"] = session_id
            try:
                ...
                if interactive_token is not None:
                    reset_jacky_interactive_context(interactive_token)
                # Restore JACKY_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("JACKY_SESSION_ID", None)
                else:
                    os.environ["JACKY_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Jacky DB session id before the turn so we
            ...
            pre_turn_jacky_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (JACKY_SESSION_KEY in
        ...
        # _meta.jacky.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_jacky_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_jacky_id
            and pre_turn_jacky_id
            and post_turn_jacky_id != pre_turn_jacky_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_jacky_session_id=post_turn_jacky_id,
                    previous_jacky_session_id=pre_turn_jacky_id,
                )
            except Exception:
                logger.debug(
        ...
        # Jacky' local "waiting for model response" interrupt status is metadata,
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Jacky Agent v{JACKY_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Jacky has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:

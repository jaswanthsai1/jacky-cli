"""CLI entry point for the jacky-agent ACP adapter.

Loads environment variables from ``~/.jacky/.env``, configures logging
...
try:
    ...
    # shadowing Jacky's own modules — ``jacky acp`` can be started from any
...
def _load_env() -> None:
    """Load .env from JACKY_HOME (default ``~/.jacky``)."""
    from jacky_cli.env_loader import load_jacky_dotenv

    jacky_home = get_jacky_home()
    loaded = load_jacky_dotenv(jacky_home=jacky_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", jacky_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="jacky-acp",
        description="Run Jacky Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Jacky version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Jacky provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.jacky/node/ "
...
def _run_check() -> None:
    ...
    from acp_adapter.server import JackyACPAgent  # noqa: F401

    print("Jacky ACP check OK")


def _run_setup() -> None:
    from jacky_cli.main import main as jacky_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "jacky", "model"]
        jacky_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``jacky postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting jacky-agent ACP adapter")
    ...
    import acp
    from .server import JackyACPAgent

"""ACP session manager — maps ACP sessions to Jacky AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.jacky/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Jacky itself is running in WSL.

    Windows ACP clients can launch ``jacky acp`` inside WSL while still sending
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Jacky from a Windows workspace while the ACP process runs
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["jacky-acp"]):
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Jacky agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Jacky AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Jacky runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.jacky/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``JACKY_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_jacky_home``).
        """
    ...
    def _make_agent(
        ...
        *,
        ...
        # editor/session cwd instead of the Jacky daemon's process cwd.

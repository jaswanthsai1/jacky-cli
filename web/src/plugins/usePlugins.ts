...
import { api, JACKY_BASE_PATH } from "@/lib/api";
...
export function usePlugins() {
  ...
  useEffect(() => {
    ...
    for (const manifest of manifests) {
      ...
      if (manifest.css) {
        const cssUrl = `${JACKY_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.css}`;
      ...
      const baseUrl = `${JACKY_BASE_PATH}/dashboard-plugins/${manifest.name}/${manifest.entry}`;
      const scriptSrc = import.meta.env.DEV
        ? `${baseUrl}?jacky_dv=${Date.now()}`
        : baseUrl;
      ...
      script.setAttribute("data-jacky-plugin", manifest.name);

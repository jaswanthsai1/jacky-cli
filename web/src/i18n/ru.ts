...
export const ru: Translations = {
  ...
  status: {
    ...
    stopped: "Остановлено",
    updateJacky: "Обновить Jacky",
    updatingJacky: "Обновление Jacky…",
  ...
  pluginsPage: {
    ...
    headline:
      "Поиск, установка, включение и обновление плагинов Jacky (аналог `jacky plugins`).",
    ...
    removeConfirm: "Удалить этот плагин из ~/.jacky/plugins/?",
    removeHint: "Удалять можно только плагины, установленные пользователем в ~/.jacky/plugins.",
  ...
  skills: {
    ...
    filters: "Фильтры",
    noSkills: "Навыки не найдены. Навыки загружаются из ~/.jacky/skills/",
  ...
  env: {
    ...
    customHint: "Произвольные переменные окружения, сохранённые в вашем .env, которые Jacky не распознаёт. Используйте их для внедрения переменных окружения для навыков, серверов MCP или собственных инструментов.",
  ...
  achievements: {
    hero: {
      ...
      subtitle:
        "Коллекционные значки Jacky, полученные на основе реальной истории сессий. Известные, но ещё не полученные достижения отображаются как «Обнаруженные»; «Секретные» достижения остаются скрытыми до появления первого подходящего поведения.",
      scan_subtitle:
        "Анализ истории сессий Jacky. Первое сканирование может занять 5–10 секунд при большой истории.",
    },
    actions: {
    ...
    stats: {
      ...
      latest: "Последнее",
      latest_hint_empty: "запускайте Jacky чаще",
      none_yet: "Пока нет",
    },
    state: {
      unlocked: "Разблокировано",
    ...
    guide: {
      ...
      secret_body:
        "Секретные достижения скрывают свой точный триггер. Как только Jacky обнаруживает связанный сигнал, карточка становится «Обнаруженной» и показывает требование.",
      scan_status_header: "Статус сканирования",
      scan_status_body:
        "Jacky сканирует локальную историю один раз, затем карточки появятся автоматически. Если это занимает несколько секунд — ничего не зависло.",

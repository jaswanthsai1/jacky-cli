...
import { JackyConsoleModal } from "@/components/JackyConsoleModal";
...
export default function SystemPage() {
  ...
  // spawned `jacky import` runs non-interactively (stdin is /dev/null), so
  ...
  const checkForUpdate = useCallback(
    async (force = false) => {
      if (status?.can_update_jacky === false) return;
      setCheckingUpdate(true);
      try {
        const info = await api.checkJackyUpdate(force);
        setUpdateInfo(info);
        if (force) {
    ...
    [showToast, status?.can_update_jacky],
  );
  ...
  const applyUpdate = async () => {
    ...
    if (status?.can_update_jacky === false) {
      showToast(
        "Jacky updates are managed outside this dashboard.",
        "success",
      );
      return;
    }
    try {
      const resp = await api.updateJacky();
      if (!resp.ok) {
        showToast(
          resp.message ??
      ...
      setActiveAction(resp.name ?? "jacky-update");
  ...
  const canUpdateJacky = status?.can_update_jacky !== false;
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <ConfirmDialog
        open={canUpdateJacky && updateConfirmOpen}
        onCancel={() => setUpdateConfirmOpen(false)}
        onConfirm={() => void applyUpdate()}
        title="Update Jacky?"
        description={
          updateInfo && updateInfo.behind && updateInfo.behind > 0
            ? `This will run 'jacky update' (${updateInfo.update_command}) and pull ${updateInfo.behind} new commit${updateInfo.behind === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`
            : `This will run 'jacky update' (${updateInfo?.update_command ?? "jacky update"}) and restart the gateway when it finishes.`
        }
        ...
        <Card>
          <CardContent className="py-4">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-y-3 gap-x-6 text-sm">
              ...
              <div>
                <div className="text-xs uppercase tracking-wider text-muted-foreground">Jacky</div>
                <div className="flex items-center gap-2">
                  <span>v{stats?.jacky_version}</span>
                  {canUpdateJacky &&
                    updateInfo &&
          ...
          <CardContent className="flex flex-col gap-3 py-4">
            ...
            {!portal?.logged_in && (
              <p className="text-xs text-muted-foreground">
                Log in with <span className="font-mono">jacky portal</span>.
              </p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="flex flex-col gap-4 py-4">
            ...
            <div className="flex flex-col gap-3 border-t border-border pt-4 sm:flex-row sm:items-end">
              <div className="grid min-w-0 flex-1 gap-2">
                ...
                <Input
                  ...
                  placeholder="$JACKY_HOME/backups/jacky-backup.zip"
                />
              </div>
              <Button
                size="sm"
            ...
            <ConfirmDialog
              ...
              title="Restore full Jacky backup?"
              description={`This will overwrite your current Jacky configuration, skills, sessions, and data with the contents of ${backupImportLabel(importConfirmTarget)}. This cannot be undone.`}
              destructive
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xs text-muted-foreground max-w-prose">
                    ...
                    returns links to send the Jacky team. Pastes auto-delete
                    after 6 hours.
                  </span>
                </div>
              </div>

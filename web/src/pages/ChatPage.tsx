/**
 * ChatPage — embeds `jacky --tui` inside the dashboard.
 *
 ...
 *          ▼                                                              .
 *     FastAPI pty_ws  (jacky_cli/web_server.py)                          .
 *          ▼                                                              .
...
const PTY_ATTACH_TOKEN_KEY = "jacky.pty.token.chat";
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__JACKY_SESSION_TOKEN__ &&
    !window.__JACKY_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `jacky dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const gated = !!window.__JACKY_AUTH_REQUIRED__;
    ...
    const term = new Terminal({
      ...
      // even when the inner Jacky TUI has enabled xterm mouse-events
    ...
    //      payloads upload to JACKY_HOME/images then drive `/image`.
    //
    ...
    // must upload browser bytes to JACKY_HOME/images, then drive `/image`
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[jacky-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // WebSocket. In gated mode (``window.__JACKY_AUTH_REQUIRED__``) this
    ...
    void (async () => {
      ...
      // Profile-scoped chat: the PTY child gets JACKY_HOME pointed at the
    ...
    // forwarding them into Jacky. Keyboard input, paste, and resize still
    // behave normally.
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-2">
      ...
      <div className="flex min-h-0 flex-1 flex-col gap-2 lg:flex-row lg:gap-3">
        ...
        >
          <div
            ref={hostRef}
            className="jacky-chat-xterm-host min-h-0 min-w-0 flex-1"
          />

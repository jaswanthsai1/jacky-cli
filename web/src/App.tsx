...
const SIDEBAR_COLLAPSED_KEY = "jacky-sidebar-collapsed";
          ...
          >
            ...
            >
              ...
              {sidebarNav.pluginItems.length > 0 && (
                <div
                  aria-labelledby="jacky-sidebar-plugin-nav-heading"
                ...
                >
                  <span
                    ...
                    id="jacky-sidebar-plugin-nav-heading"
                  >
...
function SidebarSystemActions({
  ...
  const { activeAction, isBusy, isRunning, pendingAction, runAction } =
    useSystemActions();
  const canUpdateJacky = status?.can_update_jacky === true;
  ...
  const updateConfirmDescription = useMemo(() => {
    if (updateConfirmInfo?.behind && updateConfirmInfo.behind > 0) {
      ...
      return `This will run 'jacky update' (${cmd}) and pull ${n} new commit${n === 1 ? "" : "s"}. The gateway restarts when the update finishes; the current session keeps its prompt cache until then.`;
    }
    const cmd = updateConfirmInfo?.update_command ?? "jacky update";
    return (
      t.status.updateJackyConfirmMessage ??
      `This will run 'jacky update' (${cmd}) and restart the gateway when it finishes.`
    );
  }, [t.status.updateJackyConfirmMessage, updateConfirmInfo]);
  ...
  if (canUpdateJacky) {
    items.push({
      action: "update",
      icon: Download,
      label: t.status.updateJacky,
      runningLabel: t.status.updatingJacky,
      spin: false,
    });
  }
  ...
  return (
    ...
    <ConfirmDialog
      ...
      description={
        ...
        "This restarts the Jacky gateway process. Connected channels and active sessions will reconnect afterward."
      }
    ...
    <ConfirmDialog
      cancelLabel={t.common.cancel}
      confirmLabel={t.status.updateJackyConfirmNow ?? "Update now"}
      description={
      ...
      title={t.status.updateJackyConfirmTitle ?? `${t.status.updateJacky}?`}
    />
    </>
  );
}

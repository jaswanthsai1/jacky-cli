import { buildJackyWebSocketUrl } from "@jacky/shared";

// The dashboard can be served either at the root of its host (e.g.
// https://kanban.tilos.com/) or under a URL prefix when reverse-proxied
// (e.g. https://mission-control.tilos.com/jacky/). The Python backend
// injects ``window.__JACKY_BASE_PATH__`` into index.html based on the
// incoming ``X-Forwarded-Prefix`` header so the SPA can address its own
// ``/api/...`` and ``/dashboard-plugins/...`` URLs correctly without a
// rebuild. Empty string means "served at root".
function readBasePath(): string {
  if (typeof window === "undefined") return "";
  const raw = window.__JACKY_BASE_PATH__ ?? "";
  if (!raw) return "";
...
export const JACKY_BASE_PATH = readBasePath();
const BASE = JACKY_BASE_PATH;
...
declare global {
  interface Window {
    __JACKY_SESSION_TOKEN__?: string;
    __JACKY_BASE_PATH__?: string;
    ...
    __JACKY_AUTH_REQUIRED__?: boolean;
  }
}
const SESSION_HEADER = "X-Jacky-Session-Token";
...
export async function fetchJSON<T>(
  ...
  const token = window.__JACKY_SESSION_TOKEN__;
  if (token) {
  ...
  if (res.status === 401) {
    ...
    // (``jacky update``, ``jacky gateway restart``, etc.). A tab kept
    // open across the restart holds the OLD token in
    // ``window.__JACKY_SESSION_TOKEN__`` from the previous HTML render,
    ...
    if (!window.__JACKY_AUTH_REQUIRED__ && !options?.allowUnauthorized) {
      let alreadyReloaded = false;
      try {
        alreadyReloaded =
          sessionStorage.getItem("jacky.tokenReloadAttempted") === "1";
      } catch {
        /* SSR / privacy mode — fall through to throw */
      }
      if (!alreadyReloaded) {
        try {
          sessionStorage.setItem("jacky.tokenReloadAttempted", "1");
        } catch {
  ...
  if (res.ok) {
    ...
    // current ``window.__JACKY_SESSION_TOKEN__`` is valid, so the next
    // 401 — if any — should be allowed to trigger its own reload cycle.
    try {
      sessionStorage.removeItem("jacky.tokenReloadAttempted");
    } catch {
      /* SSR / privacy mode — ignore */
    }
  }
 ...
 *
 * The dashboard's gated-mode WS auth (``jacky_cli.web_server._ws_auth_ok``)
...
export async function buildWsAuthParam(): Promise<[string, string]> {
  if (window.__JACKY_AUTH_REQUIRED__) {
 ...
 *  - loopback / ``--insecure``: attach the ``X-Jacky-Session-Token`` header.
 *  - gated OAuth: no token header (it's absent by design); the
 *    ``jacky_session_at`` cookie rides along via ``credentials: 'include'``.
 *
 ...
 * ``window.__JACKY_SESSION_TOKEN__`` directly, so the gated-mode ticket
...
export const api = {
  ...
  updateJacky: () =>
    fetchJSON<ActionResponse>("/api/jacky/update", { method: "POST" }),
  checkJackyUpdate: (force = false) =>
    fetchJSON<UpdateCheckResponse>(
      `/api/jacky/update/check${force ? "?force=true" : ""}`,
    ),
...
export interface SkillHubSource {
  ...
  /** jacky-index only: whether the centralized index loaded. */
  available?: boolean;
}
...
export interface StatusResponse {
   ...
   * updates are handled by the outer launcher instead of ``jacky update``. */
  can_update_jacky?: boolean;
  config_path: string;

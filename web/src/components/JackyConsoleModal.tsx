...
interface JackyConsoleModalProps {
  open: boolean;
  onClose: () => void;
}
...
export function JackyConsoleModal({ open, onClose }: JackyConsoleModalProps) {
  const modalRef = useModalBehavior({ open, onClose });
  const hostRef = useRef<HTMLDivElement | null>(null);
  const termRef = useRef<XtermTerminal | null>(null);
  const wsRef = useRef<WebSocket | null>(null);
  const lineRef = useRef("");
  const promptRef = useRef("jacky> ");
  const inputPromptRef = useRef("jacky> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "jacky> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Jacky Console...\x1b[0m");
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-labelledby="jacky-console-title"
    >
      <div
        className={cn(
          themedBody,

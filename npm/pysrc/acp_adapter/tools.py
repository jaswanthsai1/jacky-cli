"""ACP tool-call helpers for mapping jacky tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map jacky tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a jacky tool, defaulting to 'other'."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some Jacky tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Jacky tool result clearly failed.
    ...
    # Jacky core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # Jacky read_file output is line-numbered with `|`. If we send it as raw
...
def build_tool_start(
    ...
    *,
    ...
    """Create a ToolCallStart event for the given jacky tool invocation."""

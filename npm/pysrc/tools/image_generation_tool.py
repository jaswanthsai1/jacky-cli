...
selectable via ``jacky tools`` → Image Generation; the active model is
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `jacky tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    if env is not None:
        ...
        if remote_home:
            return f"{str(remote_home).rstrip('/')}/.jacky"
    ...
    # Jacky cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `jacky tools` → Image Generation."
            )
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`jacky setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `jacky tools` "
        "→ Image Generation (run `jacky plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
...
def check_image_generation_requirements() -> bool:
    ...
    the historical behavior: shipping jacky with a FAL key configured
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `jacky plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `jacky tools` → Image Generation."
                ),
...
# model/provider via `jacky tools` or `/skills`.

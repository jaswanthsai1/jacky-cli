...
running under the dispatcher (env var ``JACKY_KANBAN_TASK`` set) or when
the active profile explicitly enables the ``kanban`` toolset for
orchestrator work. A normal ``jacky chat`` session still sees **zero**
kanban tools in its schema unless configured.

Why tools instead of just shelling out to ``jacky kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``jacky kanban complete …``
   inside the container, where ``jacky`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.jacky/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``jacky kanban …``), the dashboard
(``jacky dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    ...
    1. ``JACKY_KANBAN_TASK`` is set (dispatcher-spawned worker), OR
    2. The current profile has ``kanban`` in its toolsets config
       (orchestrator profiles like techlead that route work via Kanban).

    Humans running ``jacky chat`` without the kanban toolset see zero
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("JACKY_KANBAN_TASK")
    return env_tid or None


def _worker_run_id(task_id: str) -> Optional[int]:
    """Return this worker's dispatcher run id when it is scoped to task_id."""
    if os.environ.get("JACKY_KANBAN_TASK") != task_id:
        return None
    raw = os.environ.get("JACKY_KANBAN_RUN_ID")
    if not raw:
        return None
    try:
        return int(raw)
...
def _stamp_worker_session_metadata(
    ...
    if os.environ.get("JACKY_KANBAN_TASK") != task_id:
        return metadata
    session_id = os.environ.get("JACKY_SESSION_ID")
    if not session_id:
        return metadata
...
def _enforce_worker_task_ownership(tid: str) -> Optional[str]:
    ...
    A process spawned by the dispatcher has ``JACKY_KANBAN_TASK`` set
    ...
    ``JACKY_KANBAN_TASK`` in env) aren't subject to this check — their
...
def _connect(board: Optional[str] = None):
    ...
    (``JACKY_KANBAN_DB`` → ``JACKY_KANBAN_BOARD`` env → current symlink
    → ``default``). Per-tool ``board`` lets a Telegram-side agent override
    the env-pinned active board without restarting Jacky.
    """
...
#   - No-op outside dispatcher-spawned worker context (no ``JACKY_KANBAN_TASK``).
...
def heartbeat_current_worker_from_env() -> bool:
    ...
    Identity comes from:
      * ``JACKY_KANBAN_TASK`` — task id (required; absence means no-op)
      * ``JACKY_KANBAN_RUN_ID`` — pins the run row so we don't heartbeat
        a stale run that may have already been reclaimed
      * ``JACKY_KANBAN_CLAIM_LOCK`` — claim lock for ``heartbeat_claim``;
    ...
    tid = os.environ.get("JACKY_KANBAN_TASK")
    if not tid:
        return False
    ...
    try:
        kb, conn = _connect()
        try:
            claim_lock = os.environ.get("JACKY_KANBAN_CLAIM_LOCK")
            try:
                kb.heartbeat_claim(conn, tid, claimer=claim_lock)
            except Exception:
                logger.debug("auto-heartbeat: heartbeat_claim failed", exc_info=True)
            run_id_raw = os.environ.get("JACKY_KANBAN_RUN_ID")
            run_id: Optional[int]
            try:
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set JACKY_KANBAN_TASK in the env)"
        )
...
def _handle_heartbeat(args: dict, **kw) -> str:
    ...
    try:
        ...
        try:
            ...
            # JACKY_KANBAN_CLAIM_LOCK in the worker env at spawn time
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    # an authoritative-looking name like ``jacky-system`` and poison
    # the future-worker context with what reads as a system directive.
    # Cross-task commenting itself remains unrestricted (see #19713) —
    # comments are the deliberate handoff channel between tasks.
    author = os.environ.get("JACKY_PROFILE") or "worker"
    board = args.get("board")
    try:
        kb, conn = _connect(board=board)
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("JACKY_TENANT")
    # Stamp the originating session id when the agent loop runs under
    # ACP (which sets JACKY_SESSION_ID before invoking tools). NULL on
    # CLI / dashboard paths and on legacy hosts that don't set the env.
    session_id = args.get("session_id") or os.environ.get("JACKY_SESSION_ID")
    priority = args.get("priority")
    # Resolve workspace. If the caller passed one explicitly, honor it.
    # Otherwise, a dispatcher-spawned worker (JACKY_KANBAN_TASK set)
    ...
    # (kanban toolset, no JACKY_KANBAN_TASK) and CLI/dashboard callers
    ...
    try:
        ...
        try:
            ...
            if _inherit_workspace:
                _self_tid = os.environ.get("JACKY_KANBAN_TASK")
                if _self_tid:
            ...
            new_tid = kb.create_task(
                ...
                created_by=os.environ.get("JACKY_PROFILE") or "worker",
...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    toggle (see ``jacky dashboard``).

    Subscription paths:

    - **Gateway** (telegram/discord/slack/etc): ``JACKY_SESSION_PLATFORM``
      and ``JACKY_SESSION_CHAT_ID`` are set in ContextVars by the
    ...
    - **TUI** (herm desktop / herm TUI): the platform/chat_id ContextVars
      ...
      ``JACKY_SESSION_KEY`` from the parent session. We subscribe with
    ...
    try:
        ...
        if not platform or not chat_id:
            ...
            # poller keys on JACKY_SESSION_KEY.
            session_key = (
                get_session_env("JACKY_SESSION_KEY", "")
                or os.environ.get("JACKY_SESSION_KEY", "")
            )
            if not session_key:
                return False  # CLI / cron / test — no persistent channel
            platform = "tui"
            chat_id = session_key
        thread_id = get_session_env("JACKY_SESSION_THREAD_ID", "") or None
        user_id = get_session_env("JACKY_SESSION_USER_ID", "") or None
        notifier_profile = (
            get_session_env("JACKY_SESSION_PROFILE", "")
            or os.environ.get("JACKY_PROFILE")
        )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to JACKY_KANBAN_TASK from the env "
    "(the task the dispatcher spawned you to work on)."
)

_DESC_BOARD = (
    "Kanban board slug to target. When omitted, the call resolves the "
    "active board the usual way: JACKY_KANBAN_DB env → "
    "JACKY_KANBAN_BOARD env → the 'current' symlink under the kanban "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to JACKY_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",

...
Lets the LLM write a Python script that calls Jacky tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `jacky_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `jacky_tools.py` with file-based RPC stubs
...
# available on every platform Jacky itself runs on.
...
# match a safe prefix, the operational JACKY_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "JACKY_" prefix was deliberately removed (#27303) — it leaked
# JACKY_*-named config that lacks a secret substring (e.g. JACKY_BASE_URL,
# JACKY_KANBAN_DB, JACKY_*_WEBHOOK).  The child only needs the few
# location/profile vars in _JACKY_CHILD_ALLOWED below; JACKY_RPC_SOCKET /
# JACKY_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational JACKY_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set jacky_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_JACKY_CHILD_ALLOWED = frozenset({
    "JACKY_HOME",
    "JACKY_PROFILE",
    "JACKY_CONFIG",
    "JACKY_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational JACKY_* vars (_JACKY_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret JACKY_* vars dropped by the tightened allowlist (#27303). The
    # broad "JACKY_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like JACKY_KANBAN_DB / JACKY_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _JACKY_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("JACKY_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped JACKY_* var.
            _dropped_jacky.append(k)
    if _dropped_jacky:
        logger.debug(
            "execute_code: dropped %d non-allowlisted JACKY_* var(s) from the "
...
_TOOL_STUBS = {
    ...
    "write_file": (
        ...
        '"""Write content to a file (always overwrites). Returns dict with status. cross_profile=True opts out of the cross-Jacky-profile soft guard."""',
    ...
    "patch": (
        ...
        '"""Targeted find-and-replace (mode="replace") or V4A multi-file patches (mode="patch"). Returns dict with status. cross_profile=True opts out of the cross-Jacky-profile soft guard."""',
...
def generate_jacky_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the jacky_tools.py stub module.
...
"""Auto-generated Jacky tools RPC stubs."""
...
def _connect():
    ...
    JACKY_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["JACKY_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("JACKY_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Jacky tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("JACKY_RPC_DIR") or os.path.join(tempfile.gettempdir(), "jacky_rpc")
_seq = 0
...
def _execute_remote(
    ...
    The script and the generated jacky_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/jacky_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_jacky_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/jacky_tools.py", tools_src)
        ...
        env_prefix = (
            f"JACKY_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"JACKY_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("JACKY_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with jacky_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="jacky_sandbox_")
    ...
    # selector from JACKY_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"jacky_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated jacky_tools module.
        ...
        tools_src = generate_jacky_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "jacky_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  JACKY_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["JACKY_RPC_SOCKET"] = rpc_endpoint
        child_env["JACKY_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the jacky-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from jacky_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _jacky_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _jacky_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # JACKY_TIMEZONE is an internal Jacky setting and must not leak
        # into child processes.
        _tz_name = os.getenv("JACKY_TIMEZONE", "").strip()
        if _tz_name:
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.jacky/.env')).
...
def _load_config() -> dict:
    ...
    ``jacky --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (jacky-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``jacky tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and jacky-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.jacky/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Jacky tools programmatically. "
        ...
        f"Available via `from jacky_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into jacky_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from jacky_tools import {import_str}` "

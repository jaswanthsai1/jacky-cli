...
`mcp_tool.py`, and `jacky_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# JackyMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_jacky_provider_class() -> Optional[type]:
    ...
    class JackyMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._jacky_server_name = server_name
            ...
            self._jacky_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`JackyTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, JackyTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._jacky_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import JackyTokenStorage
                        if isinstance(storage, JackyTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._jacky_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import JackyTokenStorage
            if not isinstance(storage, JackyTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``jacky mcp reauth``.
            ...
            back to ``jacky mcp reauth``.
            """
            try:
                if self._jacky_preregistered:
                    return
    ...
    return JackyMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_JACKY_PROVIDER_CLS: Optional[type] = _make_jacky_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`JackyMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _JACKY_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = JackyTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `jacky mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``jacky mcp remove <name>`` and (indirectly) by
        ``jacky mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:

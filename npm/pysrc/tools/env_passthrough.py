...
def _is_jacky_provider_credential(name: str) -> bool:
    """True if ``name`` is a Jacky-managed provider credential (API key,
    token, or similar) per ``_JACKY_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-Jacky API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    let a skill tunnel a Jacky credential into the execute_code child.
    """
    try:
        ...
        return True
    # Dynamically-generated Jacky-internal secrets (AUXILIARY_*_API_KEY /
    ...
    if _is_jacky_internal_secret(name):
        return True
    return name in _JACKY_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are Jacky-managed provider credentials (from
    ``_JACKY_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a Jacky-managed
    ...
    Non-Jacky third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_jacky_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register Jacky provider "
                "credential %r (blocked by _JACKY_PROVIDER_ENV_BLOCKLIST). "
    ...
    try:
        ...
        if isinstance(passthrough, list):
            for item in passthrough:
                ...
                # Jacky-managed provider credentials must not be passed
                ...
                if _is_jacky_provider_credential(name):
                    logger.warning(
                        "env passthrough: refusing to register Jacky "
                        "provider credential %r from config.yaml (blocked "
                        "by _JACKY_PROVIDER_ENV_BLOCKLIST). Operator "

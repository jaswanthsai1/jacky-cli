"""Central registry for all jacky-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``jacky tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``jacky tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("jacky_plugins."):
            return mod
        return None
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``jacky_plugins.{name}``), not the exact module string.
                # A handler defined in ``jacky_plugins.pkg.handlers`` is
                # still owned by the ``jacky_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("jacky_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``jacky tools enable foo``)

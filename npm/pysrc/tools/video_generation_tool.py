...
provider** — turn on a backend by enabling a plugin (``jacky plugins
enable video_gen/<name>``) and selecting it in ``jacky tools`` → Video
Generation.
...
VIDEO_GENERATE_SCHEMA: Dict[str, Any] = {
    ...
    # skill at github/jacky-agent-dev/references/dynamic-tool-schemas.md.
        ...
        "properties": {
            ...
            "model": {
                ...
                "description": (
                    ...
                    "configured ``video_gen.model`` (set via `jacky tools` "
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `jacky tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `jacky tools` → "
...
# `jacky tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`jacky tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if not configured:
        parts.append(
            ...
            "until the user picks one via `jacky tools` → Video Generation."
        )
        return {"description": "\n".join(parts)}

...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__JACKY_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__JACKY_FENCE_" in line or "\x1b]" in line
...
class ShellFileOperations(FileOperations):
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.jacky-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".jacky-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.jacky-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.jacky-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # clean the temp up on any failure so we never leak a ``.jacky-tmp``

...
There are two ways to add a platform to the Jacky gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.jacky/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Jacky code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `jacky gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
- `plugin.yaml` `requires_env` / `optional_env` rich-dict entries —
  auto-populate `OPTIONAL_ENV_VARS` in `jacky_cli/config.py` so the setup
...
Checklist for integrating a platform directly into the Jacky core.
...
And add it to the `jacky-gateway` composite:

```python
"jacky-gateway": {
    "includes": [..., "jacky-your-platform"]
}
```
...
## 12. Status Display (`jacky_cli/status.py`)
...
## 13. Gateway Setup Wizard (`jacky_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ jacky_cli/ toolsets.py \
  --include="*.py" -l | sort -u

...
Defenses 1 and 2 (the ``_JACKY_GATEWAY`` guard on ``jacky gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.jacky.gateway")``,
...
State lives in ``<JACKY_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_jacky_home() / "gateway" / "restart_loop.json"

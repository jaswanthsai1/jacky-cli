...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``jacky://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``jacky://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"jacky://blueprint/{quote(blueprint.key)}{qs}"

"""disk_cleanup — ephemeral file cleanup for Jacky Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under JACKY_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly JACKY_HOME and /tmp/jacky-*
Never touches: ~/.jacky/logs/ or any system directory.
"""
...
try:
    ...
    def get_jacky_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("JACKY_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".jacky").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$JACKY_HOME/logs/``."""
    return get_jacky_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$JACKY_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under JACKY_HOME or ``/tmp/jacky-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    jacky_home = get_jacky_home()
    try:
        path.resolve().relative_to(jacky_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/jacky-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("jacky-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "jacky-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $JACKY_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so JACKY_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        jacky_home = get_jacky_home()
        for parent in ("cron", "cronjobs"):
            base = jacky_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside JACKY_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under JACKY_HOME, but never recurse into known
    # durable state trees.  Some installs place the Jacky checkout, venv,
    # and desktop build under JACKY_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    jacky_home = get_jacky_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in jacky_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    jacky_home = get_jacky_home()
    try:
        rel = path.resolve().relative_to(jacky_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "jacky-agent",
        }:
            return None
        ...
        # Path isn't under JACKY_HOME (e.g. /tmp/jacky-*) — fall through.
        pass

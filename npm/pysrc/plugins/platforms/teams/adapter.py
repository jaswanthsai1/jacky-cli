"""
Microsoft Teams platform adapter for Jacky Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "Jacky"}),
            )
    ...
    async def _fetch_attachment_bytes(self, url: str, timeout: float = 30.0) -> bytes:
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.get(
                url,
                headers={"User-Agent": "Mozilla/5.0 (compatible; JackyAgent/1.0)"},
            )
            response.raise_for_status()
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        jacky_action = data.get("jacky_action", "")
        session_key = data.get("session_key", "")

        if not jacky_action or not session_key:
        ...
        choice = choice_map.get(jacky_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="jacky_approve",
                    data={**btn_data_base, "jacky_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="jacky_approve",
                    data={**btn_data_base, "jacky_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="jacky_approve",
                    data={**btn_data_base, "jacky_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="jacky_approve",
                    data={**btn_data_base, "jacky_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"Jacky\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.jacky/.env")

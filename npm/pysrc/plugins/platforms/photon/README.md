...
This plugin connects Jacky Agent to iMessage (and other Spectrum
...
TypeScript-only, Jacky runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Jacky-Sidecar-Token`.
...
There is no separate `login` command; like every other Jacky channel,
...
Run `jacky photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.jacky/.env` (the same place every other
...
Management metadata lives in `~/.jacky/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `jacky photon telemetry on\|off` (restart the gateway to apply) |
...
6. Verify end-to-end: `jacky photon status`, a DM and a group roundtrip,

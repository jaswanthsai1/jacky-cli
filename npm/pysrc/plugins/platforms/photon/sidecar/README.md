...
Small Node helper that bridges Jacky Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Jacky is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Jacky-Sidecar-Token`)
...
The Jacky plugin's `jacky photon setup` command runs `npm install`
here automatically.

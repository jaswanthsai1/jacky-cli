...
- Per-user tokens (keyed by sender email):
    ``${JACKY_HOME}/google_chat_user_tokens/<sanitized_email>.json``
- Legacy single-user token (fallback, untouched for backward compat):
    ``${JACKY_HOME}/google_chat_user_token.json``
- Per-user pending OAuth state during /setup-files start → exchange:
    ``${JACKY_HOME}/google_chat_user_oauth_pending/<sanitized_email>.json``
- Legacy pending state:
    ``${JACKY_HOME}/google_chat_user_oauth_pending.json``
- OAuth client secret (profile-scoped — each profile registers its own):
    ``${JACKY_HOME}/google_chat_user_client_secret.json``
"""
...
def _jacky_home() -> Path:
    """Resolve JACKY_HOME at call time (NOT module import).

    Tests and ``JACKY_HOME=...`` env overrides need this to be late-
...
# ``ls ~/.jacky/google_chat_user_tokens/`` trivial.
...
def _legacy_token_path() -> Path:
    return _jacky_home() / "google_chat_user_token.json"


def _user_tokens_dir() -> Path:
    return _jacky_home() / "google_chat_user_tokens"


def _legacy_pending_path() -> Path:
    return _jacky_home() / "google_chat_user_oauth_pending.json"


def _user_pending_dir() -> Path:
    return _jacky_home() / "google_chat_user_oauth_pending"
...
def _client_secret_path() -> Path:
    return _jacky_home() / "google_chat_user_client_secret.json"
...
def load_user_credentials(email: Optional[str] = None) -> Optional[Any]:
    ...
    try:
        ...
        logger.warning(
            ...
            "attachment delivery is disabled. Install jacky-agent[google_chat]."
        )
        return None
...
def install_deps() -> bool:
    ...
    try:
        ...
        print("  pip install 'jacky-agent[google_chat]'")
        return False
...
def store_client_secret(path: str) -> None:
    """Validate and copy the user's OAuth client_secret.json into JACKY_HOME."""
...
def exchange_auth_code(code: str, email: Optional[str] = None) -> None:
    ...
    rel_label = (
        f"{display_jacky_home()}/google_chat_user_tokens/{_sanitize_email(email)}.json"
        if email
        else f"{display_jacky_home()}/google_chat_user_token.json"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Google Chat user-OAuth setup for Jacky (native attachment delivery)"
    )

"""
LINE Messaging API platform adapter for Jacky Agent.
...
to Jacky Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``JACKY_HOME``. PR #8398.
        """
        from aiohttp import web
...
def is_connected(config) -> bool:
    """Surface in ``jacky status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``jacky status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``jacky setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.jacky/.env`` via ``jacky_cli.config``.
    """
    print()
    ...
    try:
        from jacky_cli.config import get_env_var, set_env_var
    except ImportError:
        print("jacky_cli.config not available; set LINE_* vars manually in ~/.jacky/.env")
        return

...
session-scoped state file under ``$JACKY_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $JACKY_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_jacky_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this jacky install, leave it first —
    ...
    env = os.environ.copy()
    env["JACKY_MEET_URL"] = url
    env["JACKY_MEET_OUT_DIR"] = str(out)
    env["JACKY_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["JACKY_MEET_HEADED"] = "1"
    if auth_state:
        env["JACKY_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["JACKY_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if JACKY_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["JACKY_MEET_MODE"] = mode
    if realtime_model:
        env["JACKY_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["JACKY_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["JACKY_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["JACKY_MEET_REALTIME_KEY"] = realtime_api_key

...
_API_KEY_URL = "http://app.supermemory.ai/integrations?connect=jacky"
...
def _load_supermemory_config(jacky_home: str) -> dict:
    config = _default_config()
    config_path = Path(jacky_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, jacky_home: str) -> None:
    config_path = Path(jacky_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class _SupermemoryClient:
    def __init__(self, api_key: str, timeout: float, container_tag: str, search_mode: str = "hybrid"):
        ...
        self._client = Supermemory(
            ...
            timeout=timeout,
            max_retries=0,
            default_headers={"x-sm-source": "jacky"},
        )

    def _merge_metadata(self, metadata: Optional[dict]) -> dict:
        # sm_source routes Jacky writes into the "Jacky" Space in the Supermemory
        ...
        merged = {"sm_source": "jacky", **(metadata or {})}
...
def _resolve_container_tag_for_setup(jacky_home: str, *, identity: str = "default") -> str:
    config = _load_supermemory_config(jacky_home)
...
def _probe_supermemory_connection(api_key: str, jacky_home: str, *, identity: str = "default") -> dict:
    config = _load_supermemory_config(jacky_home)
    status = {
        "ok": False,
        "error": "",
        "container_tag": _resolve_container_tag_for_setup(jacky_home, identity=identity),
        "profile_facts": 0,
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `jacky memory setup`.
        # All other options are documented for $JACKY_HOME/supermemory.json
    ...
    def get_status_config(self, provider_config: dict) -> dict:
        ...
        status = _probe_supermemory_connection(api_key, jacky_home)
    ...
    def post_setup(self, jacky_home: str, config: dict) -> None:
        ...
        from jacky_cli.memory_setup import _prompt, _write_env_vars
        ...
        if env_writes:
            _write_env_vars(Path(jacky_home) / ".env", env_writes)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._turn_count = 0
        self._config = _load_supermemory_config(self._jacky_home)

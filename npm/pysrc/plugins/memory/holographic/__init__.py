"""jacky-memory-store — holographic memory plugin using MemoryProvider interface.
...
Config in $JACKY_HOME/config.yaml (profile-scoped):
  plugins:
    jacky-memory-store:
      db_path: $JACKY_HOME/memory_store.db   # omit to use the default
      auto_extract: false
      default_trust: 0.5
...
def _load_plugin_config() -> dict:
    ...
    try:
        ...
        return cfg_get(all_config, "plugins", "jacky-memory-store", default={}) or {}
    except Exception:
        return {}
...
class HolographicMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, jacky_home):
        """Write config to config.yaml under plugins.jacky-memory-store."""
        from pathlib import Path
        config_path = Path(jacky_home) / "config.yaml"
        try:
            import yaml
            existing = {}
            ...
            existing["plugins"]["jacky-memory-store"] = values
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        # Expand $JACKY_HOME in user-supplied paths so config values like
        # "$JACKY_HOME/memory_store.db" or "~/.jacky/memory_store.db" both
        # resolve to the active profile's directory.
        if isinstance(db_path, str):
            db_path = db_path.replace("$JACKY_HOME", _jacky_home)
            db_path = db_path.replace("${JACKY_HOME}", _jacky_home)

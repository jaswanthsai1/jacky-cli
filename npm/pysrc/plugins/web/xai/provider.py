...
``{title, url, description, position}`` rows every other Jacky web
provider produces.
...
prefers Jacky-managed xAI Grok OAuth (via ``jacky auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.jacky/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``jacky tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `jacky auth` to sign in with "

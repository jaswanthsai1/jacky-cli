"""nemo_relay — optional Jacky plugin for NeMo Relay observability."""
...
class _Settings:
    ...
    atof_filename: str = "jacky-atof.jsonl"
    atof_mode: str = "append"
    atif_enabled: bool = False
    atif_output_directory: str = ""
    atif_filename_template: str = "jacky-atif-{session_id}.json"
    atif_subagent_export_mode: str = "embedded"
    atif_agent_name: str = "Jacky Agent"
...
class _Runtime:
    def __init__(self, nemo_relay: Any, settings: _Settings) -> None:
        ...
        self._atof_subscriber_name = "jacky.nemo_relay.atof"
    ...
    def ensure_session(self, kwargs: dict[str, Any]) -> _SessionState:
        ...
        if self.settings.atif_enabled and not self._plugins_toml_owns_exporter("atif"):
            state.atif_exporter = self.nemo_relay.AtifExporter(
                ...
                extra={"source": "jacky-agent", "plugin": "observability/nemo_relay"},
            )
            state.atif_subscriber_name = f"jacky.nemo_relay.atif.{session_id}"
    ...
    def mark_subagent_stop(self, kwargs: dict[str, Any]) -> None:
        ...
        self.mark("jacky.subagent.stop", kwargs)
    ...
    def _run_managed_with_downstream_preservation(
        ...
        # execution so Jacky retry classification still sees it. The LLM and tool
...
def on_session_end(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: (runtime.mark("jacky.session.end", kwargs), runtime.export_atif(runtime.ensure_session(kwargs))))
...
def on_pre_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("jacky.turn.start", kwargs))


def on_post_llm_call(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("jacky.turn.end", kwargs))
...
def on_post_api_request(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("jacky.api.response.unmatched", kwargs)
            return
...
def on_api_request_error(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        span = state.llm_spans.pop(_api_key(kwargs), None)
        if span is None:
            runtime.mark("jacky.api.error", kwargs)
            return
        runtime.nemo_relay.llm.call_end(
            span,
...
def on_post_tool_call(**kwargs: Any) -> None:
    ...
    def _record() -> None:
        ...
        if span is None:
            runtime.mark("jacky.tool.response.unmatched", kwargs)
            return
...
def on_pre_approval_request(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("jacky.approval.request", kwargs))


def on_post_approval_response(**kwargs: Any) -> None:
    runtime = _get_runtime()
    if runtime is not None:
        _safe(lambda: runtime.mark("jacky.approval.response", kwargs))
...
def _load_settings() -> _Settings:
    plugins_toml_path = _env("JACKY_NEMO_RELAY_PLUGINS_TOML")
    ...
    return _Settings(
        ...
        atof_enabled=_env_bool("JACKY_NEMO_RELAY_ATOF_ENABLED"),
        atof_output_directory=_env("JACKY_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY"),
        atof_filename=_env("JACKY_NEMO_RELAY_ATOF_FILENAME") or "jacky-atof.jsonl",
        atof_mode=_env("JACKY_NEMO_RELAY_ATOF_MODE") or "append",
        atif_enabled=_env_bool("JACKY_NEMO_RELAY_ATIF_ENABLED"),
        atif_output_directory=_env("JACKY_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY"),
        atif_filename_template=_env("JACKY_NEMO_RELAY_ATIF_FILENAME_TEMPLATE") or "jacky-atif-{session_id}.json",
        atif_subagent_export_mode=_atif_subagent_export_mode(),
        atif_agent_name=_env("JACKY_NEMO_RELAY_ATIF_AGENT_NAME") or "Jacky Agent",
        atif_agent_version=_env("JACKY_NEMO_RELAY_ATIF_AGENT_VERSION") or "unknown",
        atif_model_name=_env("JACKY_NEMO_RELAY_ATIF_MODEL_NAME") or "unknown",
    )
...
def _atif_subagent_export_mode() -> str:
    mode = _env("JACKY_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE").lower()
...
def _original_downstream_error(exc: Exception) -> BaseException:
    # Jacky wraps downstream execution failures in a local/private exception
...
def _is_relay_wrapped_callback_error(exc: Exception, callback_error: Exception | None) -> bool:
    ...
    # leading ``internal error:`` shape entirely, this returns False and Jacky
...
def _resolve_awaitable(value: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="jacky-nemo-relay-awaitable",
        daemon=True,
    )
    thread.start()
    thread.join()

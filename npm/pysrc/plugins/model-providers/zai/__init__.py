...
request omits ``thinking``.  Jacky' ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Jacky reasoning
...
(per Z.AI / BigModel docs).  Jacky' richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(reasoning_config: dict | None) -> str | None:
    """Map Jacky reasoning effort onto GLM-5.2's native ``high``/``max``.

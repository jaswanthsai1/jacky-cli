...
agent's existing :func:`~jacky_cli.runtime_provider.resolve_runtime_provider`,
...
# image_gen.model from ``jacky tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``jacky tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `jacky tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            "HTTP-Referer": "https://github.com/jaswanthsai1/jacky-cli",
            "X-Title": "Jacky Agent",
        }

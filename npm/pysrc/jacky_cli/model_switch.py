...
- ``jacky_cli.providers``        -- canonical provider identity + overlays
- ``jacky_cli.model_normalize``  -- per-provider name formatting
...
_JACKY_MODEL_WARNING = (
    "Nous Research Hermes 3 & 4 models are NOT agentic and are not designed "
    "for use with Jacky Agent. They lack the tool-calling capabilities "
...
# unrelated local Modelfiles like ``jacky-brain:qwen3-14b-ctx16k`` that just
# happen to carry "jacky" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/Hermes-3-Llama-3.1-70B, hermes-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   jacky-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``jacky config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from jacky_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'jacky model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'jacky doctor' — config issues detected:"
        ...
        from jacky_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from jacky_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from jacky_cli.config import load_config
            from jacky_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from jacky_cli.providers import resolve_user_provider as _ruser
    ...
    # jacky_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from jacky_cli.models import normalize_opencode_base_url
    ...
    jacky_warn = _check_jacky_model_warning(new_model)
    if jacky_warn:
        warnings.append(jacky_warn)
...
def _extra_headers_from_config(entry: Any) -> dict[str, str]:
    if not isinstance(entry, dict):
        return {}
    from jacky_cli.config import normalize_extra_headers
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from jacky_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    *,
    ...
    Uses the curated model lists from jacky_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from jacky_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by jacky provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "nous" pulls from the remote model-catalog manifest published at
    # https://jacky-agent.nousresearch.com/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Jacky release. Falls back to the in-repo
    # _PROVIDER_MODELS["nous"] snapshot when the manifest is unreachable.
    curated["nous"] = get_curated_nous_model_ids()
    # Ollama Cloud uses dynamic discovery (no static curated list)
    if "ollama-cloud" not in curated:
        from jacky_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from jacky_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Jacky-mapped providers ---
    from jacky_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from jacky_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for jacky_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(jacky_id)
        if (
            _alias_target
            and _alias_target != jacky_id
        ...
        pconfig = PROVIDER_REGISTRY.get(jacky_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (JACKY_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from jacky_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and store.get("credential_pool", {}).get(jacky_id):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `jacky model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(jacky_id)
        if not model_ids:
            model_ids = curated.get(jacky_id, [])
            if jacky_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(jacky_id, model_ids)
        total = len(model_ids)
        if jacky_id in _UNCAPPED_PICKER_PROVIDERS:
    ...
    # --- 2. Check Jacky-only providers (nous, openai-codex, copilot, opencode-go) ---
    from jacky_cli.providers import JACKY_OVERLAYS
    from jacky_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Jacky provider ID.
    # JACKY_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Jacky IDs ("copilot").
    _mdev_to_jacky = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in JACKY_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Jacky slug — e.g. "github-copilot" → "copilot"
        jacky_slug = _mdev_to_jacky.get(pid, pid)
        if jacky_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(jacky_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, jacky_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or jacky_slug in providers_store):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(jacky_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", jacky_slug, exc)
        ...
        # configured.
        if not has_creds and jacky_slug == "anthropic":
            try:
                ...
                if (jacky_creds and jacky_creds.get("accessToken")) or \
        ...
        if jacky_slug in {"openai-codex", "copilot", "copilot-acp"}:
            ...
            model_ids = cached_provider_model_ids(jacky_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(jacky_slug)
                model_ids = _ids if _ids else (curated.get(jacky_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(jacky_slug, []) or curated.get(pid, [])
        elif jacky_slug == "nous":
            # Nous serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `jacky model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from jacky_cli.auth import get_provider_auth_state as _nous_state
            ...
            if not model_ids:
                model_ids = curated.get(jacky_slug, []) or curated.get(pid, [])
                if jacky_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(jacky_slug, model_ids)
        total = len(model_ids)
        if jacky_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": jacky_slug,
            "name": get_label(jacky_slug),
            "is_current": jacky_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "jacky",
        ...
        seen_slugs.add(jacky_slug.lower())
        _record_builtin_endpoint(jacky_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or JACKY_OVERLAYS (keeps /model in sync
    # with `jacky model`).
    try:
        from jacky_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Jacky's canonical write key (matches
            ...
            # Jacky writes ``models:`` as a dict keyed by model id, but older
            ...
            if should_probe:
                try:
                    from jacky_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Jacky's own writer uses; a hyphen variant is accepted
            ...
            # active model. Jacky's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, jacky_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from jacky_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`jacky_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from jacky_cli.models import fetch_openrouter_models

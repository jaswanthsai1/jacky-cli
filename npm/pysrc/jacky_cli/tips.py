...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or jacky -c.",
    ...
    "Ctrl+Z suspends Jacky to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "jacky -c resumes your most recent CLI session. jacky -c \"project name\" resumes by title.",
    "jacky -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "jacky -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "jacky chat -t web,terminal enables only specific toolsets for a focused session.",
    "jacky chat -s github-pr-workflow preloads a skill at launch.",
    "jacky chat -q \"query\" runs a single non-interactive query and exits.",
    "jacky chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "jacky chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "jacky --yolo bypasses all dangerous command approval prompts for the entire session.",
    "jacky chat --source telegram tags the session for filtering in jacky sessions list.",
    "jacky -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "jacky doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "jacky dump outputs a compact setup summary — great for bug reports.",
    "jacky config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "jacky config edit opens config.yaml in your default editor.",
    "jacky config check scans for missing or stale configuration options.",
    "jacky sessions browse opens an interactive session picker with search.",
    "jacky sessions stats shows session counts by platform and database size.",
    "jacky sessions prune --older-than 30 cleans up old sessions.",
    "jacky skills search react --source skills-sh searches the skills.sh public directory.",
    "jacky skills check scans installed hub skills for upstream updates.",
    "jacky skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "jacky skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "jacky mcp add github --command npx adds MCP servers from the command line.",
    "jacky mcp serve runs Jacky itself as an MCP server for other agents.",
    "jacky auth add lets you add multiple API keys for credential pool rotation.",
    "jacky completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "jacky logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "jacky backup creates a zip backup of your entire Jacky home directory.",
    "jacky profile create coder creates an isolated profile that becomes its own command.",
    "jacky profile create work --clone copies your current config and keys to a new profile.",
    "jacky update syncs new bundled skills to ALL profiles automatically.",
    "jacky gateway install sets up Jacky as a system service (systemd/launchd).",
    "jacky memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "jacky webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: jacky tools disables unused tools, jacky skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "jacky models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as jacky -w).",
    ...
    "JACKY_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Jacky tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'jacky profile create coder' creates the 'coder' command.",
    "jacky profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Jacky prints a resume command with session ID and stats.",
    "jacky sessions export backup.jsonl exports all sessions for backup or analysis.",
    "jacky -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "jacky skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .jacky/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: jacky cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Jacky runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "jacky gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Jacky via a local macOS server.",
    ...
    "Context files (.jacky.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Jacky loads project context from .jacky.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "jacky mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "jacky mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "jacky mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — jacky tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.jacky/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "jacky chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "jacky plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "jacky acp runs Jacky as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "JACKY_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "jacky auth add nous or jacky auth add openai-codex sets up OAuth-based providers.",
    ...
    "jacky status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.jacky/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Jacky your own.",
    ...
    "Before a gateway session reset, Jacky auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.jacky/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Jacky state via JACKY_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "JACKY_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in JACKY_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.jacky/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'jacky profile create ls' would be rejected.",
    "jacky profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "jacky claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "JACKY_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via jacky tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.jacky/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.jacky/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Jacky auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Jacky config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Jacky is working.',
    ...
    '/reload reloads ~/.jacky/.env into the running session — pick up new API keys without restarting.',
    ...
    "JACKY_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.jacky/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.jacky/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'jacky curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "jacky curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'jacky curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'jacky auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'JACKY_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "JACKY_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'jacky dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'jacky dashboard embeds the full Jacky TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.jacky/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.jacky/dashboard-plugins/ — no npm build required.',
    ...
    'JACKY_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'JACKY_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'JACKY_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'JACKY_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.jacky/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.jacky/skills/ so drop-in skills appear without restarting the session.',
    ...
    'jacky -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'jacky chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'jacky chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'jacky chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "jacky chat --source tool tags programmatic chats so they don't clutter jacky sessions list.",
    'jacky dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'jacky sessions rename <ID> "new title" renames any past session; jacky sessions delete <ID> removes one.',
    'jacky import restores a session export or profile archive produced by sessions export or profile export.',
    'jacky fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'jacky pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'jacky setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'jacky status --deep runs the full health sweep across every component; plain jacky status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'JACKY_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'JACKY_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.jacky/plugins/ — trust-gated by design.',
    "JACKY_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'JACKY_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'JACKY_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'JACKY_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'JACKY_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'JACKY_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'JACKY_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'JACKY_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'JACKY_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'JACKY_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'JACKY_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Jacky block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from jacky sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.jacky/dashboard-plugins/.',
]

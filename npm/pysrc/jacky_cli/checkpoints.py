"""`jacky checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.jacky/checkpoints/``.  Actions:

    jacky checkpoints               # same as `status`
    jacky checkpoints status        # total size, project count, breakdown
    jacky checkpoints list          # per-project checkpoint counts + workdir
    jacky checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    jacky checkpoints clear [-f]    # nuke the entire base (asks first)
    jacky checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    jacky checkpoints
    jacky checkpoints prune --retention-days 3 --max-size-mb 200
    jacky checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: jacky checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``jacky checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `jacky checkpoints` → status

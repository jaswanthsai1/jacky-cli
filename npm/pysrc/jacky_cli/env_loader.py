"""Helpers for loading Jacky .env files consistently across entrypoints."""
...
# load_jacky_dotenv() calls (user env + project env, gateway hot-reload,
# tests) don't spam the same warning multiple times.
_WARNED_KEYS: set[str] = set()

# Map of env-var name → source label ("bitwarden", etc.) for credentials
# that were injected by an external secret source during load_jacky_dotenv().
# Used by setup / `jacky model` flows to label detected credentials so
# users understand WHERE a key came from when their .env doesn't contain it
# directly (otherwise the "credentials detected ✓" line looks identical to
# the .env case and they don't know Bitwarden is wired up).
_SECRET_SOURCES: dict[str, str] = {}

# JACKY_HOME paths we've already pulled external secrets for during this
# process.  ``load_jacky_dotenv()`` is called at module-import time from
# several hot modules (cli.py, jacky_cli/main.py, run_agent.py,
...
def get_secret_source(env_var: str) -> str | None:
    ...
    during the current process's ``load_jacky_dotenv()`` call.  Returns
...
def reset_secret_source_cache() -> None:
    """Forget which JACKY_HOME paths have already had external secrets applied.
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `jacky setup` (or edit the "
...
def _sanitize_env_file_if_needed(path: Path) -> None:
    ...
    We delegate to ``jacky_cli.config._sanitize_env_lines`` which
    already knows all valid Jacky env-var names and can split
    concatenated lines correctly.
    """
    if not path.exists():
        return
    try:
        from jacky_cli.config import _sanitize_env_lines
    except ImportError:
...
def load_jacky_dotenv(
    *,
    jacky_home: str | os.PathLike | None = None,
    project_env: str | os.PathLike | None = None,
) -> list[Path]:
    """Load Jacky environment files with user config taking precedence.

    Behavior:
    - `~/.jacky/.env` overrides stale shell-exported values when present.
    ...
    home_path = Path(jacky_home or os.getenv("JACKY_HOME", Path.home() / ".jacky"))
    ...
    #   EnvironmentFile=-/path/to/.jacky/.op.env
...
def _apply_managed_env() -> None:
    ...
    Managed scope is machine-global (independent of JACKY_HOME / profile). v1
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Jacky reads
    ...
    owns the once-per-JACKY_HOME guard, the post-apply ASCII
    sanitization sweep, the ``_SECRET_SOURCES`` provenance map that
    UI surfaces read, and the startup status lines.

    Idempotent within a process: subsequent calls for the same
    ``home_path`` are no-ops.  ``load_jacky_dotenv()`` runs at import
    time from several hot modules (cli.py, jacky_cli/main.py,
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `jacky model`

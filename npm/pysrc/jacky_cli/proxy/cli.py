"""CLI handlers for the ``jacky proxy`` subcommand."""
...
from jacky_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "jacky proxy requires aiohttp. Install one of:\n"
        "  pip install 'jacky-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"jacky auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Jacky proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Jacky proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: jacky proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``jacky proxy <subcommand>``."""
    ...
    print(
        "jacky proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  jacky proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"

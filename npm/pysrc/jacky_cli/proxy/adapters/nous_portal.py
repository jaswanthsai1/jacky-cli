...
Reads the user's Nous OAuth state from ``~/.jacky/auth.json`` through the
...
class NousPortalAdapter(UpstreamAdapter):
    ...
    def _get_credential(
        ...
        *,
        ...
        with self._lock:
            ...
            if state is None:
                raise RuntimeError(
                    "Not logged into Nous Portal. Run `jacky auth add nous` first."
                )
    ...
    # to jacky_cli.auth to avoid expanding that module's public surface.
    # ------------------------------------------------------------------

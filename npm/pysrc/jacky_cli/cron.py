...
# commands at execution time when ``_JACKY_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: jacky gateway install", Colors.DIM))
    print(color("                    sudo jacky gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  jacky cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'jacky cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: jacky gateway restart")
        ...
        print("    jacky gateway install    # Install as a user service")
        print("    sudo jacky gateway install --system  # Linux servers: boot-time system service")
        print("    jacky gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: jacky cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)

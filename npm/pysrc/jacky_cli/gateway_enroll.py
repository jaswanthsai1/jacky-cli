"""``jacky gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Nous Portal access token from the existing login
     (``~/.jacky/auth.json``) — the same path ``jacky dashboard register``
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.jacky/.env``. The per-gateway secret authenticates the WS upgrade;
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "token could not be verified — try `jacky auth add nous` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `jacky gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    save_env_value writes them to ~/.jacky/.env (0600 dir) and never logs.
    to_write = {

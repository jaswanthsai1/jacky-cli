"""CLI handlers for ``jacky migrate ...``.

Currently exposes only ``jacky migrate xai`` — diagnoses and (with --apply)
...
def cmd_migrate(args: Any) -> int:
    """Dispatcher for ``jacky migrate <subtype>``."""
    sub = getattr(args, "migrate_type", None)
    if sub == "xai":
        return cmd_migrate_xai(args)

    print("usage: jacky migrate xai [--apply] [--no-backup]", file=sys.stderr)
    return 2
...
def cmd_migrate_xai(args: Any) -> int:
    ...
    if not apply:
        ...
        print(color(
            "Re-run with `jacky migrate xai --apply` to rewrite "
    ...
    print()
    print(color(
        "Run `jacky doctor` to confirm no retired xAI models remain.",
        Colors.DIM,
    ))
    return 0

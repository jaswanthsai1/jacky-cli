"""``jacky portal`` — the human-readable entry point for Nous Portal.

Running ``jacky portal`` with no subcommand performs the one-shot Portal
...
``jacky auth add nous --type oauth`` (which still works), is identical to
``jacky setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``jacky auth`` or ``jacky tools``. It's the onboarding + discovery
...
DOCS_URL = "https://jacky-agent.nousresearch.com/docs/user-guide/features/tool-gateway"
...
def _cmd_status(args) -> int:
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `jacky model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `jacky portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `jacky auth add nous --type
    oauth`. It reuses the exact wiring behind `jacky setup --portal` (which in
    ...
    """
    from jacky_cli.setup import _run_portal_one_shot
...
def portal_command(args) -> int:
    """Top-level dispatch for `jacky portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `jacky portal` is the
        # human-readable alias for `jacky auth add nous --type oauth` /
        # `jacky setup --portal`.
    ...
    print("Run `jacky portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `jacky portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `jacky portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `jacky auth add "
            "nous --type oauth`, identical to `jacky setup --portal`). "

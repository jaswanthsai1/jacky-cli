"""``jacky logs`` — view and filter Jacky log files.
...
Usage examples::
    ...
    jacky logs                    # last 50 lines of agent.log
    jacky logs -f                 # follow agent.log in real time
    jacky logs errors             # last 50 lines of errors.log
    jacky logs gateway -n 100    # last 100 lines of gateway.log
    jacky logs gui -f            # follow gui.log (dashboard/pty/ws)
    jacky logs desktop -f        # follow desktop.log (Electron app boot/backend)
    jacky logs --level WARNING    # only WARNING+ lines
    jacky logs --session abc123   # filter by session ID substring
    jacky logs --component tools  # only tool-related lines
    jacky logs --since 1h         # lines from the last hour
    jacky logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_jacky_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Jacky runs — try 'jacky chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_jacky_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_jacky_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_jacky_home()}/logs/")
        return

    print(f"Log files in {display_jacky_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'jacky chat' to generate logs)")

"""MCP picker — interactive `jacky mcp picker` (also the default `jacky mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``jacky mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `jacky plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from jacky_cli.cli_output import prompt_yes_no
from jacky_cli.curses_ui import curses_single_select
from jacky_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Jacky session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from jacky_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `jacky mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: jacky mcp install <name>    Picker: jacky mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Jacky is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Jacky — run `jacky update` "
...
def show_catalog() -> None:
    """`jacky mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`jacky mcp picker` (and default `jacky mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`jacky mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `jacky mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:

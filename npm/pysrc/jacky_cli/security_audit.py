"""On-demand supply-chain audit for Jacky Agent installs.

Scans three surfaces a Jacky user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Jacky venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.jacky/plugins``
...
def _discover_plugins(jacky_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.jacky/plugins``.
...
def run_audit(
    *,
    ...
    home = jacky_home or Path(get_jacky_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, jacky_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(jacky_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `jacky security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, jacky_home=home
    )
    if total == 0:

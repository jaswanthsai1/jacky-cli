"""
Interactive setup wizard for Jacky Agent.
...
# display_jacky_home imported lazily at call sites (stale-module safety during jacky update)
...
from jacky_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from jacky_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print_info("Configure Jacky using environment variables or config commands:")
    print_info("  jacky config set model.provider custom")
    print_info("  jacky config set model.base_url http://localhost:8080/v1")
    print_info("  jacky config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'jacky setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``JACKY_NONINTERACTIVE=1`` (see ``jacky_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("JACKY_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``JACKY_NONINTERACTIVE=1`` or a closed/redirected
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'jacky setup')")


def _print_setup_summary(config: dict, jacky_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'jacky setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from jacky_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `jacky tools` → Video Generation.
    ...
    if subscription_features.video_gen.managed_by_nous:
        ...
        try:
            ...
            from jacky_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'jacky setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'jacky setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'jacky setup terminal'"))
    ...
    # Spotify (OAuth via jacky auth spotify — check auth.json, not env vars)
    try:
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {jacky_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('jacky setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('jacky setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('jacky setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('jacky setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('jacky setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('jacky config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('jacky config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('jacky config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('jacky', Colors.GREEN)}              Start chatting")
    print(f"   {color('jacky gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('jacky doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `jacky tools` and `jacky setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``jacky model``)
    ...
    provider added to ``jacky model`` is automatically available here.
    ...
    # Delegate to the shared jacky model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from jacky_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: jacky model")
    ...
    # on demand via `jacky auth add`, `jacky setup` vision, and
    # `jacky setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from jacky_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``jacky model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from jacky_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.jacky/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Jacky auth store) > existing XAI_API_KEY > prompt the user.
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'jacky setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Jacky runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `jacky setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `jacky setup terminal`.
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into JACKY_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("JACKY_MAX_ITERATIONS")
    ...
    print_info("  Run `jacky setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # JACKY_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.jacky/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from jacky_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_jacky_home(Path(get_jacky_home()))
    except Exception:
        pass
...
def _profile_name_from_jacky_home(jacky_home) -> str | None:
    """Return the active profile name when JACKY_HOME is a profile dir."""
    if jacky_home.parent.name == "profiles":
        return jacky_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Jacky delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Jacky to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from jacky_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def setup_gateway(config: dict):
    ...
    from jacky_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Jacky from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'jacky setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     jacky config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_jacky_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if is_container():
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   jacky gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `jacky setup tools` and `jacky tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``jacky_cli.auth`` — lists every supported
    ...
    """
    try:
        from jacky_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from jacky_cli.config
    ...
    if section_key == "model":
        ...
        from jacky_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_jacky.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_jacky migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_jacky", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and Jacky.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Jacky to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Jacky at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Jacky at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Jacky at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Jacky at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Jacky equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Jacky config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Jacky.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Jacky's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(jacky_home: Path) -> bool:
    ...
    print_info("Jacky can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: jacky claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=jacky_home.resolve(),
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: jacky claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Jacky configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Jacky config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Jacky (use jacky claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``jacky setup --portal`` and ``jacky portal``. This is the
    ...
    Jacky session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``jacky setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``jacky model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``jacky portal`` always offers a model picker,
    ...
    # setup calls, so `jacky portal` == quick setup's Nous step.
    try:
        from jacky_cli.main import _model_flow_nous
        ...
        print_info("  You can retry later with `jacky portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `jacky portal`: %s", exc)
        print()
    ...
    print_info("  Run `jacky portal info` to inspect routing.")
    print_info("  Run `jacky` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      jacky setup           — full or quick (auto-detected)
      jacky setup model     — just model/provider
      jacky setup tts       — just text-to-speech
      jacky setup terminal  — just terminal backend
      jacky setup gateway   — just messaging platforms
      jacky setup tools     — just tool configuration
      jacky setup agent     — just agent settings
    """
    from jacky_cli.config import is_managed, managed_error
    if is_managed():
    ...
    print(
        color(
            "│  Let's configure your Jacky Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, jacky_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Jacky configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'jacky setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(jacky_home)
        if migration_ran:
            config = load_config()
    ...
    print_info(f"Data folder:  {jacky_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'jacky config edit'")
    ...
    # Tune later with `jacky setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, jacky_home)


def _run_first_time_quick_setup(config: dict, jacky_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``jacky setup <section>``
    or switch providers with ``jacky model``.
    """
    from jacky_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'jacky setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    jacky setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  jacky setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``jacky tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``jacky setup agent`` /
    ``jacky config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, jacky_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `jacky update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        jacky tools")
        print_info("  Seed skills:         jacky skills opt-in --sync")
        print_info("  Add MCP servers:     jacky mcp add")
        print_info("  Enable plugins:      jacky plugins")
        print_info("  Tune agent settings: jacky setup agent")
        print()
        _print_setup_summary(config, jacky_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, jacky_home)


def _blank_slate_walkthrough(config: dict, jacky_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`jacky update` runs from re-injecting them. Opt back in any")
            print_info("time with `jacky skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `jacky tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `jacky plugins list` / `jacky plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `jacky plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `jacky mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `jacky mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   jacky tools")
    print_info("  Seed skills:         jacky skills opt-in --sync")
    print_info("  Add MCP servers:     jacky mcp add")
    print_info("  Tune agent settings: jacky setup agent")
    print()
...
def _run_quick_setup(config: dict, jacky_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'jacky setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Jacky to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'jacky setup gateway'.")

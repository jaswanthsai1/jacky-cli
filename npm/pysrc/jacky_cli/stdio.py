...
on Japanese locales, etc.).  Jacky's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``jacky_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Jacky is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``JACKY_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("JACKY_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Jacky-managed Git install directories so
    ...
    # already-running shells don't see the change — which means jacky
    ...
    # install without Jacky-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Jacky's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Jacky's
    ``jacky config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Jacky (User env var in Windows Settings, or
    export in a PowerShell profile) and Jacky picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Jacky-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\jacky\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``jacky`` from right after install) retain their old PATH.

    Any subprocess Jacky spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Jacky-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "jacky", "git", "cmd"),
        os.path.join(local_appdata, "jacky", "git", "bin"),
        os.path.join(local_appdata, "jacky", "git", "usr", "bin"),
        # Jacky venv Scripts directory — host of the jacky.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes jacky, but harmless to include.
        os.path.join(local_appdata, "jacky", "jacky-agent", "venv", "Scripts"),

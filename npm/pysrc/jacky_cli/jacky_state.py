#!/usr/bin/env python3
"""
SQLite State Store for Jacky Agent.
...
DEFAULT_DB_PATH = get_jacky_home() / "state.db"
...
# jacky_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Shared by :class:`SessionDB` and ``jacky_cli.kanban_db.connect`` so
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see jacky_cli/kanban_db.py) would
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_jacky_fts_health_probe_{time.time_ns()}"
        try:
...
class SessionDB:
    ...
    # With multiple jacky processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``jacky_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `jacky update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._jacky_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._jacky_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Jacky surface and jump
    ...
    def delete_session_if_empty(
        ...
        and ``jacky sessions list`` output. (Pattern ported from
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Jacky, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Jacky session.

        A Jacky session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Jacky session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``JACKY_DISABLE_FTS_TRIGRAM`` or not yet created), so

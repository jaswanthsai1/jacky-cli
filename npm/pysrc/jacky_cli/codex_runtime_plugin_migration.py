"""Migrate Jacky' MCP server config and Codex's installed curated plugins
...
module:
  ...
  1. Reads Jacky' YAML and writes equivalent [mcp_servers.<name>]
...
What translates (MCP servers):
  Jacky mcp_servers.<n>.command/args/env  → codex stdio transport
  Jacky mcp_servers.<n>.url/headers       → codex streamable_http transport
  Jacky mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Jacky mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Jacky-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Jacky' own
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Jacky config.")
...
# Jacky keys that codex's MCP schema doesn't support — dropped during
...
_KEYS_DROPPED_WITH_WARNING = {
    # Jacky' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, jacky_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Jacky MCP server config to the codex inline-table dict
    ...
    if not isinstance(jacky_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(jacky_cfg.get("command"))
    has_url = bool(jacky_cfg.get("url"))
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (JACKY_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Jacky)")
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Jacky' managed Codex TOML block while keeping root keys root-scoped.
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    directly (i.e. before Jacky' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Jacky
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="jacky-migration")
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a JACKY_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed jacky-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.jacky`` symlink that happens to live under ``/private/var/folders``
...
def _build_jacky_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Jacky' own
    ...
    so a jacky installed under /opt/, /usr/local/, or a venv all work.
    JACKY_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # JACKY_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_jacky_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever JACKY_HOME its launcher sets
    ...
    # — that would override the launcher's JACKY_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("JACKY_HOME", tmp_path) would
    ...
    jacky_home = os.environ.get("JACKY_HOME") or ""
    if jacky_home and _looks_like_test_tempdir(jacky_home):
        jacky_home = ""
    if jacky_home:
        env["JACKY_HOME"] = jacky_home
    # PYTHONPATH passes through so a worktree-launched jacky finds the
    ...
    env["JACKY_QUIET"] = "1"
    env["JACKY_REDACT_SECRETS"] = env.get("JACKY_REDACT_SECRETS", "true")
...
def migrate(
    ...
    *,
    ...
    expose_jacky_tools: bool = True,
) -> MigrationReport:
    """Translate Jacky mcp_servers config + Codex curated plugins into
    ~/.codex/config.toml.

    Args:
        jacky_config: full ~/.jacky/config.yaml dict
        ...
        expose_jacky_tools: when True (default), register Jacky' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Jacky for tools
    ...
    jacky_servers = (jacky_config or {}).get("mcp_servers") or {}
    if not isinstance(jacky_servers, dict):
        report.errors.append(
            "mcp_servers in Jacky config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in jacky_servers.items():
    ...
    # Inject Jacky' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Jacky for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/jacky_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_jacky_tools:
        translated["jacky-tools"] = _build_jacky_tools_mcp_entry()
        if "jacky-tools" not in report.migrated:
            report.migrated.append("jacky-tools")

"""
MCP Server Management CLI — ``jacky mcp`` subcommand.

Implements ``jacky mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.jacky/config.yaml under the ``mcp_servers`` key.
"""
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from jacky_cli.cli_output import prompt as _shared_prompt
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.jacky/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``jacky mcp login`` to distinguish a genuine authentication
    ...
    """
    try:
        from tools.mcp_oauth import JackyTokenStorage
        return JackyTokenStorage(name).has_cached_tokens()
...
def cmd_mcp_add(args):
    ...
    # jacky_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  jacky mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  jacky mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  jacky mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_jacky_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: jacky mcp test " + name)
        return
    ...
    if _save_mcp_server(name, server_config):
        print()
        _success(f"Saved '{name}' to {display_jacky_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `jacky mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  jacky mcp add <name> --url <endpoint>')
        _info('  jacky mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``jacky mcp login`` and
    ``jacky mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `jacky mcp remove` + `jacky mcp add` to reconfigure auth.")
        return False
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `jacky mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``jacky mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``jacky mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: jacky mcp reauth <name>   |   jacky mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
def cmd_mcp_configure(args):
    ...
    if not _sys.stdin.isatty():
        print("Error: 'jacky mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``jacky mcp`` subcommands."""
    ...
    if action == "picker":
        from jacky_cli.mcp_picker import run_picker
        run_picker()
        return
    if action == "catalog":
        from jacky_cli.mcp_picker import show_catalog
        show_catalog()
        return
    if action == "install":
        from jacky_cli.mcp_picker import install_by_name
        import sys as _sys
    ...
    if handler:
        ...
        # "try enabling and it flows you into setup" UX matching `jacky plugin`.
        from jacky_cli.mcp_picker import run_picker
        run_picker()
        print(color("  Commands:", Colors.CYAN))
        _info("jacky mcp                                    Open the catalog picker (default)")
        _info("jacky mcp catalog                            List Nous-approved MCPs")
        _info("jacky mcp install <name>                     Install a catalog MCP")
        _info("jacky mcp serve                              Run as MCP server")
        _info("jacky mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("jacky mcp add <name> --command <cmd>         Add a stdio server")
        _info("jacky mcp add <name> --preset <preset>       Add from a known preset")
        _info("jacky mcp remove <name>                      Remove a server")
        _info("jacky mcp list                               List configured servers")
        _info("jacky mcp test <name>                        Test connection")
        _info("jacky mcp configure <name>                   Toggle tools")
        _info("jacky mcp login <name>                       Re-authenticate OAuth")
        _info("jacky mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()

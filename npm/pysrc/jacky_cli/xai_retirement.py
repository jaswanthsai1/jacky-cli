...
Pure logic: walks a Jacky config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `jacky doctor` and a future `jacky migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Jacky config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Jacky config and return retirement issues.
...
def apply_migration(
    ...
    from jacky_cli.config import require_readable_config_before_write

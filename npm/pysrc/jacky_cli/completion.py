"""Shell completion script generation for jacky CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_jacky_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Jacky Agent bash completion
# Add to ~/.bashrc:
#   eval "$(jacky completion bash)"

_jacky_profiles() {{
    local profiles_dir="$HOME/.jacky/profiles"
...
_jacky_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_jacky_profiles)" -- "$cur"))
        return
    fi
...
complete -F _jacky_completion jacky
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef jacky
# Jacky Agent zsh completion
# Add to ~/.zshrc:
...
_jacky_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.jacky/profiles" ]]; then
        profiles+=($HOME/.jacky/profiles/*(N/:t))
    fi
...
_jacky() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_jacky_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'jacky command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Jacky Agent fish completion",
        "# Add to your config:",
        "#   jacky completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __jacky_profiles",
        "    echo default",
        "    if test -d $HOME/.jacky/profiles",
        "        for d in $HOME/.jacky/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "",
        "# Complete profile names after -p / --profile",
        "complete -c jacky -f -s p -l profile"
        " -d 'Profile name' -xa '(__jacky_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__jacky_profiles)' -d 'Profile name'"
                )

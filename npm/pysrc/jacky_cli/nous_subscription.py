...
# coverage category (jacky_cli.nous_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `jacky tools` picker scope its entitlement gate to the selected backend, so a
...
def _local_stt_backend_available() -> bool:
    ...
    """
    if get_env_value("JACKY_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Tool Gateway via `jacky model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("JACKY_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("JACKY_LOCAL_STT_COMMAND"))
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from jacky_cli.setup import prompt_checklist
    except Exception:
        return set()
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`jacky tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``jacky tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``jacky model``'s Nous
    login, this:

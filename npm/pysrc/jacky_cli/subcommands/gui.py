...
def build_gui_parser(subparsers, *, cmd_gui: Callable) -> None:
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the Jacky Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--ignore-existing",
        action="store_true",
        help="Force Desktop to ignore any jacky CLI already on PATH during backend resolution",
    )
    gui_parser.add_argument(
        "--jacky-root",
        help="Override the Jacky source root used by Desktop (sets JACKY_DESKTOP_JACKY_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
        help="Initial project directory for Desktop chat sessions (sets JACKY_DESKTOP_CWD)",
    )
    gui_parser.add_argument(
        "--skip-build",
        action="store_true",

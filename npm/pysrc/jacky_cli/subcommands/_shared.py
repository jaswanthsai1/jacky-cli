...
These were module-level helpers in ``jacky_cli/main.py``. They are pulled
into a neutral module so both ``main.py`` and every
``jacky_cli/subcommands/<group>.py`` builder can import them without an
...
def add_accept_hooks_flag(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to JACKY_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )

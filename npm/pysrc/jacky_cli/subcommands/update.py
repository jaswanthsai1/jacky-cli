"""``jacky update`` subcommand parser.
...
def build_update_parser(subparsers, *, cmd_update: Callable) -> None:
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Jacky Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'jacky config migrate' separately for those.",
    )
    update_parser.add_argument(
        "--branch",
        default=None,
        ...
        help=(
            ...
            "If the local checkout is on a different branch, jacky will "
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Windows: proceed with the update even when another jacky.exe is detected. The concurrent process will likely cause WinError 32 warnings and may leave a reboot-deferred .exe replacement. Does NOT bypass the venv-process guard (see --force-venv).",
    )
    update_parser.add_argument(
        "--force-venv",
        action="store_true",

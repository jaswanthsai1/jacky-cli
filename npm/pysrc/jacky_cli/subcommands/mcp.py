"""``jacky mcp`` subcommand parser.

Extracted from ``jacky_cli/main.py:main()`` (god-file Phase 2 follow-up).
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Jacky as an MCP server",
        description=(
            "Manage MCP server connections and run Jacky as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'jacky mcp add' to connect to a new server, or\n"
            "'jacky mcp serve' to expose Jacky conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Jacky as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `jacky mcp add ...` to fall through to
    # interactive chat.
    ...
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `jacky mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
    ...
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `jacky mcp install n8n`)",
    )

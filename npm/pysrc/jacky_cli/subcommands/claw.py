...
def build_claw_parser(subparsers, *, cmd_claw: Callable) -> None:
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to Jacky",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to Jacky",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.jacky/ (by default a "
        "single restore-point archive is written to ~/.jacky/backups/ "
        "before apply; restorable with 'jacky import').",
    )
    claw_migrate.add_argument(

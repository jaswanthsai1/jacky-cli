"""Unified removal contract for every credential source Jacky reads from.

Jacky seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.jacky/.env
    claude_code   — ~/.claude/.credentials.json
    jacky_pkce   — ~/.jacky/.anthropic_oauth.json
    ...
    manual        — user ran `jacky auth add`
...
unify here is **removal**:
    ...
    ``jacky auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, nous device_code (partial), jacky_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.jacky/.env  → clear it
    ...
    """
    from jacky_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Jacky.",
            f"  The pool entry is now suppressed — Jacky will ignore "
            f"{env_var} until you run `jacky auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Jacky stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `jacky auth add anthropic` to re-enable if needed.",
    ])
...
def _remove_jacky_pkce(provider: str, removed) -> RemovalResult:
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Jacky Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `jacky auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `jacky auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``jacky auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `jacky model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Jacky auth store is not enough — _seed_from_singletons() would
    ...
    via ``jacky auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `jacky auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `jacky auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Jacky stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `jacky auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="jacky_pkce",
        remove_fn=_remove_jacky_pkce,
        description="~/.jacky/.anthropic_oauth.json",
    ))
    register(RemovalStep(

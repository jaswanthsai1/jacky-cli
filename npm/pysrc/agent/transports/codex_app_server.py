...
projection into Jacky' display, approval bridging, transcript projection into
...
"codex_app_server"`. Jacky' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Jacky secret — gateway
        ...
        spawn_env = jacky_subprocess_env(inherit_credentials=True)
        if env:
        ...
        if spawn_env.get("JACKY_KANBAN_TASK"):
            kanban_db = spawn_env.get("JACKY_KANBAN_DB")
            kanban_root = (
                ...
                if kanban_db
                    ...
                    "JACKY_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("JACKY_HOME", os.path.expanduser("~/.jacky")),
                        "kanban",
                    ),
                )
            )
    ...
    def initialize(
        self,
        client_name: str = "jacky",
        client_title: str = "Jacky Agent",

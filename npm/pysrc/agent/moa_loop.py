...
MoA-enabled; the normal Jacky agent loop still owns tool calling and turn
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Jacky boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Jacky agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Jacky agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Jacky agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"

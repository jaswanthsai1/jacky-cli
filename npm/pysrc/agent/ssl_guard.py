"""Preventive SSL CA certificate checks for Jacky Agent.
...
def _skip_ssl_guard_enabled() -> bool:
    return os.getenv("JACKY_SKIP_SSL_GUARD", "").strip().lower() in _SKIP_VALUES
...
def verify_ca_bundle() -> None:
    ...
    if _skip_ssl_guard_enabled():
        logger.debug("SSL CA bundle guard skipped via JACKY_SKIP_SSL_GUARD")
        return

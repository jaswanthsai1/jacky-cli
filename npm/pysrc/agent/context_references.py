...
_SENSITIVE_JACKY_DIRS = (Path("skills") / ".hub",)
...
def _ensure_reference_path_allowed(path: Path) -> None:
    ...
    blocked_exact.add(jacky_home / ".env")
    blocked_dirs = [home / rel for rel in _SENSITIVE_HOME_DIRS]
    blocked_dirs.extend(jacky_home / rel for rel in _SENSITIVE_JACKY_DIRS)
    ...
    for blocked_dir in blocked_dirs:
        try:
            ...
            continue
        raise ValueError("path is a sensitive credential or internal Jacky path and cannot be attached")
    ...
    # message text into reference expansion, so `@file:~/.jacky/auth.json` from a
    ...
    try:
        ...
        if get_read_block_error(str(path)) is not None:
            raise ValueError(
                "path is a sensitive credential or internal Jacky path and cannot be attached"
            )
    except ValueError:
        raise
    except Exception:

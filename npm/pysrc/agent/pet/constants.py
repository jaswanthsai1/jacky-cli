...
row taxonomy is inferred from the atlas shape so Jacky can render both legacy
...
# rows (1536x1872), while older Jacky/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Jacky' activity state names. They are not always identical to the
...
# Legacy Jacky/petdex row order (top -> bottom) used by the older 8-row,
...
# Canonical Jacky activity names -> accepted row-name aliases in descending

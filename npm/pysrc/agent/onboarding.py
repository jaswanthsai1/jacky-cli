...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Jacky starts and finds ``~/.openclaw/``.

    Points users at ``jacky claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``jacky claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Jacky, run "
        "`jacky claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `jacky claw cleanup` (renames it to ~/.openclaw.pre-migration — "

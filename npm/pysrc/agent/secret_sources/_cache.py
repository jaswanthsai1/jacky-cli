...
cache problem must never block Jacky startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Jacky home used for cache paths.

    ``home_path`` is whatever ``load_jacky_dotenv()`` already resolved;
    falling back to ``$JACKY_HOME`` / ``~/.jacky`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("JACKY_HOME", Path.home() / ".jacky"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<jacky_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if JACKY_HOME is ever made traversable.
            try:

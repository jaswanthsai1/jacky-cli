...
# output items.  Jacky injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Jacky-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Jacky threading encrypted reasoning back across
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Jacky-side metadata key and not part

...
$JACKY_OSINT_CACHE/icij/ (default: ~/.cache/jacky-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("JACKY_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "jacky-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "jacky-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $JACKY_OSINT_CACHE/icij or ~/.cache/jacky-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",

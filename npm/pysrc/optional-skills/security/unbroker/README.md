...
The design is **Jacky-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
jacky skills install official/security/unbroker
```

Then start a new Jacky session and drive it (below). The skill works zero-config; a few optional
...
- **PII handling.** Dossiers live under the Jacky home directory (`0600`, optionally

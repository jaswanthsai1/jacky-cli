...
All per-subject data lives under PDD_DATA_DIR (default: $JACKY_HOME/unbroker),
which is the same trust boundary Jacky uses for .env and OAuth tokens.
"""
...
def jacky_home() -> Path:
    return Path(os.environ.get("JACKY_HOME") or (Path.home() / ".jacky"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else jacky_home() / "unbroker"

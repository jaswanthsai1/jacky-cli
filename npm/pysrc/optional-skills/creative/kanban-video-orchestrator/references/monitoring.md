...
jacky kanban watch --tenant <project-slug>

# Snapshot of the board
jacky kanban list --tenant <project-slug>
jacky kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
jacky kanban stats --tenant <project-slug>

# Visual dashboard (browser)
jacky dashboard

# Inspect a specific task (includes comments + events)
jacky kanban show <task-id>

# Follow a single task's event stream
jacky kanban tail <task-id>
```

Verify available subcommands with `jacky kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `jacky kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `jacky kanban block <id>` then `jacky kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `jacky kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `jacky kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `jacky profile list` (profile exists?), `jacky status` (gateway/dispatcher up?), and verify tenant. |
...
jacky kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
jacky kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(jacky kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
jacky kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
jacky kanban block <task-id>
jacky kanban archive <task-id>

# Diagnose what happened
jacky kanban show <task-id>      # task body, comments, recent events
jacky kanban tail <task-id>      # follow the live event stream
jacky kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `jacky kanban create` for the director
...
while true; do
    clear
    jacky kanban list --tenant <slug>
    echo "---"
    jacky kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `jacky kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `jacky profile list` and start

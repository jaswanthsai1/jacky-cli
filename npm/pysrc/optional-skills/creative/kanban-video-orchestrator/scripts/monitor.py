#!/usr/bin/env python3
"""
Monitor a running video-production kanban. Polls `jacky kanban list` and
...
def jacky_available() -> bool:
    return shutil.which("jacky") is not None


def kanban_list(tenant: str) -> list[dict]:
    """Returns parsed task rows. Falls back to plain stdout parsing if JSON
    output isn't supported by the installed jacky CLI."""
    try:
        out = subprocess.run(
            ["jacky", "kanban", "list", "--tenant", tenant, "--json"],
        ...
        pass
    # Fallback: textual parse of `jacky kanban list`
    out = subprocess.run(
        ["jacky", "kanban", "list", "--tenant", tenant],
        capture_output=True, text=True, check=False,
    )
    rows = []
...
def kanban_show(task_id: str) -> dict | None:
    out = subprocess.run(
        ["jacky", "kanban", "show", task_id, "--json"],
        capture_output=True, text=True, check=False,
    )
    if out.returncode != 0:
        return None
...
def main():
    ...
    if not jacky_available():
        print("ERROR: 'jacky' CLI not found in PATH", file=sys.stderr)
        sys.exit(1)

...
Usage (via cron with --no-agent):
    ...
    jacky cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$JACKY_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Jacky-Watcher/1.0"})

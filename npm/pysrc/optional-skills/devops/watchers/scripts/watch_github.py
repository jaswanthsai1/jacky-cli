...
Usage (via cron with --no-agent):
    ...
    jacky cron create jacky-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$JACKY_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name jacky-issues --repo jaswanthsai1/jacky-cli --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Jacky .env file
(``${JACKY_HOME:-~/.jacky}/.env``) to avoid the 60 req/hr

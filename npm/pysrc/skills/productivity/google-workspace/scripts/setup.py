#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Jacky Agent.
...
# Ensure sibling modules (_jacky_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _jacky_home import display_jacky_home, get_jacky_home

JACKY_HOME = get_jacky_home()
TOKEN_PATH = JACKY_HOME / "google_token.json"
CLIENT_SECRET_PATH = JACKY_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = JACKY_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Jacky profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Jacky Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Jacky Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  pip install 'jacky-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Jacky home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_jacky_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Jacky")

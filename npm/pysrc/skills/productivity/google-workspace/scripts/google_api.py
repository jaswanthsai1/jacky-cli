#!/usr/bin/env python3
"""Google Workspace API CLI for Jacky Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Jacky-facing JSON contract and falls back to the Python client
...
from _jacky_home import get_jacky_home
...
def _gws_binary() -> str | None:
    override = os.getenv("JACKY_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Jacky Agent")

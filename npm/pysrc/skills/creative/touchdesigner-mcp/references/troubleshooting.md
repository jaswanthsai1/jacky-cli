...
## 5. Jacky Configuration

### Config location

`$JACKY_HOME/config.yaml` (defaults to `~/.jacky/config.yaml` when `JACKY_HOME` is unset)
...
Restart the Jacky session for changes to take effect. The MCP connection is

...
JACKY_HOME_DIR="${JACKY_HOME:-$HOME/.jacky}"
JACKY_CFG="${JACKY_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Jacky config has twozero_td MCP entry ──
if [[ ! -f "$JACKY_CFG" ]]; then
    echo -e " ${FAIL} Jacky config not found at ${JACKY_CFG}"
    manual_steps+=("Create ${JACKY_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$JACKY_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Jacky config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Jacky config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${JACKY_CFG} manually"); }
    manual_steps+=("Restart Jacky session to pick up config change")
fi

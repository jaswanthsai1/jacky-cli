#!/usr/bin/env bash
# GitHub environment detection helper for Jacky Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _jacky_env="${JACKY_HOME:-$HOME/.jacky}/.env"; [ -f "$_jacky_env" ] && grep -q "^GITHUB_TOKEN=" "$_jacky_env" 2>/dev/null; then

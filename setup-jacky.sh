...
# 5. Symlinks the 'jacky' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ Jacky Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    if [ -n "$UV_CMD" ]; then
        ...
        _uv_log="$(mktemp 2>/dev/null || echo "/tmp/jacky-uv-install.$$.log")"
...
# ============================================================================
# PATH setup — symlink jacky into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up jacky command..."

JACKY_BIN="$SCRIPT_DIR/venv/bin/jacky"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$JACKY_BIN" "$COMMAND_LINK_DIR/jacky"
echo -e "${GREEN}✓${NC} Symlinked jacky → $COMMAND_LINK_DISPLAY_DIR/jacky"
...
# ============================================================================
# Seed bundled skills into ~/.jacky/skills/
# ============================================================================

JACKY_SKILLS_DIR="${JACKY_HOME:-$HOME/.jacky}/skills"
mkdir -p "$JACKY_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to ~/.jacky/skills/ ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$JACKY_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  jacky status        # Check configuration"
if is_termux; then
    echo "  jacky gateway       # Run gateway in foreground"
else
    echo "  jacky gateway install # Install gateway service (messaging + cron)"
fi
echo "  jacky cron list     # View scheduled jobs"
echo "  jacky doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m jacky_cli.main setup
fi

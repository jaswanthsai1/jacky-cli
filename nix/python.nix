...
let
  ...
  editableOverlay = workspace.mkEditablePyprojectOverlay {
    root = "$JACKY_PYTHON_SRC_ROOT"; # resolved at shellHook time
  };
  ...
  editableSet = pythonSet.overrideScope (
    lib.composeManyExtensions [
      editableOverlay
      (final: prev: {
        jacky-agent = prev.jacky-agent.overrideAttrs (old: {
...
in
{
  venv = pythonSet.mkVirtualEnv "jacky-agent-env" {
    jacky-agent = dependency-groups;
  };
  editableVenv = editableSet.mkVirtualEnv "jacky-agent-editable-env" {
    jacky-agent = dependency-groups;
  };
}

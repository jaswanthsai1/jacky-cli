# nix/desktop.nix — Jacky Desktop (Electron) app build + wrapper
#
# `jackyAgent` is the fully-built `.#default` package — it ships the
# `jacky` binary with the venv, runtime PATH, bundled skills/plugins, etc.
# already wired up.  We point the desktop at it via the existing
# `JACKY_DESKTOP_JACKY` override env var, so the desktop's resolver
# uses our fully wrapped binary at step 4 ("existing Jacky CLI").
...
let
  ...
  targetPlatform =
    if stdenv.hostPlatform.isDarwin then
      ...
      throw "jacky-desktop: unsupported host platform for node-pty staging";
  ...
  targetArch =
    if stdenv.hostPlatform.isAarch64 then
      ...
      throw "jacky-desktop: unsupported host arch for node-pty staging";

  # Build the renderer (dist/ + electron/ + package.json).
  renderer = pkgs.buildNpmPackage (
    npm
    // {
      pname = "jacky-desktop-renderer";
      inherit version;
      doCheck = true;
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    mkdir -p $out/share/jacky-desktop $out/bin
    cp -r ${renderer}/* $out/share/jacky-desktop/
    ...
    substituteInPlace $out/share/jacky-desktop/dist/electron-main.mjs \
      --replace-fail "process.resourcesPath" "'$out/share/jacky-desktop'"

    # Wrap the nixpkgs electron binary to launch our app.  Set
    # JACKY_DESKTOP_JACKY to the absolute path of the nix-built `jacky`
    # binary so the desktop's resolver step 4 ("existing Jacky CLI on
    # PATH") uses our fully wrapped binary — venv with all deps,
    # bundled skills/plugins, runtime PATH (ripgrep/git/ffmpeg/etc).
    # No reimplementation of the agent resolver in the wrapper.
    makeWrapper ${lib.getExe electron} $out/bin/jacky-desktop \
      --add-flags "$out/share/jacky-desktop" \
      --set JACKY_DESKTOP_JACKY "${lib.getExe jackyAgent}" \
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Jacky Agent";
    homepage = "https://github.com/jaswanthsai1/jacky-cli";
    license = licenses.mit;
    platforms = platforms.unix;
    mainProgram = "jacky-desktop";
  };
}

...
{ ... }:
{
  perSystem =
    ...
    let
      ...
      jackyNpmLib = self'.packages.default.passthru.jackyNpmLib;
      ...
      # Non-npm packages may have their own devShellHook (e.g. jacky-agent
    ...
    in
    {
      devShells.default = pkgs.mkShell {
        packages =
          with pkgs;
          [
            (pkgs.runCommand "jacky" { } ''
              mkdir -p $out/bin
              install -Dm755 ${../jacky} $out/bin/jacky
            '')
        ...
        shellHook = ''
          ${combinedNonNpm}
          ${jackyNpmLib.mkNpmDevShellHook npmPackageJsonPaths}

          # for the devshell to pick up the src
          export JACKY_PYTHON_SRC_ROOT=$(git rev-parse --show-toplevel)
          echo "Jacky Agent dev shell in $JACKY_PYTHON_SRC_ROOT"
          echo "Ready. Run 'jacky' or 'sandbox jacky' to start."
        '';
      };
    };
}

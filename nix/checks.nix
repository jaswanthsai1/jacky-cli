...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      jacky-agent = self'.packages.default;
      jackyVenv = jacky-agent.jackyVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "jacky-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${jackyVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "jacky-cross-eval" { } (
          if failures != [] then
        ...
        build-package = pkgs.runCommand "jacky-build-package" { } ''
          echo "PASS: package built at ${jacky-agent}"
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify the devShell builds successfully (cross-platform).
        build-devshell = pkgs.runCommand "jacky-build-devshell" { } ''
        ...
        package-contents = pkgs.runCommand "jacky-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${jacky-agent}/bin/jacky || (echo "FAIL: jacky binary missing"; exit 1)
          test -x ${jacky-agent}/bin/jacky-agent || (echo "FAIL: jacky-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${jacky-agent}/bin/jacky version 2>&1 | grep -qi "jacky" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "jacky-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in jacky jacky-agent jacky-acp; do
            test -x ${jacky-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "jacky-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking jacky --help ==="
          ${jacky-agent}/bin/jacky --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${jacky-agent}/bin/jacky --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "jacky-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${jacky-agent}/share/jacky-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${jacky-agent}/share/jacky-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "JACKY_BUNDLED_SKILLS" ${jacky-agent}/bin/jacky || \
            (echo "FAIL: JACKY_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: JACKY_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "jacky-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${jacky-agent}/share/jacky-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${jacky-agent}/share/jacky-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "JACKY_BUNDLED_PLUGINS" ${jacky-agent}/bin/jacky || \
            (echo "FAIL: JACKY_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: JACKY_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-locales = pkgs.runCommand "jacky-bundled-locales" { } ''
          set -e
          echo "=== Checking bundled locales ==="
          test -d ${jacky-agent}/share/jacky-agent/locales || (echo "FAIL: locales directory missing"; exit 1)
          echo "PASS: locales directory exists"

          LOC_COUNT=$(find ${jacky-agent}/share/jacky-agent/locales -name "*.yaml" | wc -l)
          test "$LOC_COUNT" -ge 16 || (echo "FAIL: expected >=16 catalogs, found $LOC_COUNT"; exit 1)
          echo "PASS: $LOC_COUNT locale catalogs found"

          test -f ${jacky-agent}/share/jacky-agent/locales/en.yaml || (echo "FAIL: en.yaml missing"; exit 1)
          echo "PASS: en.yaml present"

          grep -q "JACKY_BUNDLED_LOCALES" ${jacky-agent}/bin/jacky || \
            (echo "FAIL: JACKY_BUNDLED_LOCALES not in wrapper"; exit 1)
          echo "PASS: JACKY_BUNDLED_LOCALES set in wrapper"

          echo "=== Rendering via the wrapper override (JACKY_BUNDLED_LOCALES) ==="
          export HOME=$(mktemp -d)
          RENDERED=$(cd "$HOME" && JACKY_BUNDLED_LOCALES=${jacky-agent}/share/jacky-agent/locales \
            ${jackyVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "rendered: $RENDERED"
          test "$RENDERED" != "gateway.reset.header_default" || (echo "FAIL: i18n returned the raw key with JACKY_BUNDLED_LOCALES set"; exit 1)
          ...
          BARE_DIR=$(cd "$HOME" && ${jackyVenv}/bin/python3 -c "from agent import i18n; print(i18n._locales_dir())")
          BARE=$(cd "$HOME" && ${jackyVenv}/bin/python3 -c "from agent import i18n; print(i18n.t('gateway.reset.header_default', lang='en'))")
          echo "resolved dir (no env var): $BARE_DIR"
          echo "rendered: $BARE"
          test "$BARE" != "gateway.reset.header_default" || \
            (echo "FAIL: sealed venv could not resolve locales without JACKY_BUNDLED_LOCALES — data-files materialization regressed"; exit 1)
        ...
        bundled-tui = pkgs.runCommand "jacky-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${jacky-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${jacky-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          # self-contained bundle; no runtime node_modules expected

          grep -q "JACKY_TUI_DIR" ${jacky-agent}/bin/jacky || \
            (echo "FAIL: JACKY_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: JACKY_TUI_DIR set in wrapper"
        ...
        # Verify JACKY_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        jacky-node = pkgs.runCommand "jacky-node-version" { } ''
          set -e
          echo "=== Checking JACKY_NODE in wrapper ==="
          grep -q "JACKY_NODE" ${jacky-agent}/bin/jacky || \
            (echo "FAIL: JACKY_NODE not set in wrapper"; exit 1)
          echo "PASS: JACKY_NODE present in wrapper"

          JACKY_NODE=$(sed -n "s/^export JACKY_NODE='\(.*\)'/\1/p" ${jacky-agent}/bin/jacky)
          test -x "$JACKY_NODE" || (echo "FAIL: JACKY_NODE=$JACKY_NODE not executable"; exit 1)
          echo "PASS: JACKY_NODE executable at $JACKY_NODE"

          NODE_MAJOR=$("$JACKY_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All JACKY_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify JACKY_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "jacky-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(JACKY_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking JACKY_MANAGED guards ==="
          check_blocked "config set" ${jacky-agent}/bin/jacky config set model foo
          check_blocked "config edit" ${jacky-agent}/bin/jacky config edit
        ...
        extra-python-packages = let
          ...
          jackyWithExtra = jacky-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "jacky-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${jackyWithExtra}/bin/jacky || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${jackyWithExtra}/bin/jacky || \
          ...
          if grep -q "PYTHONPATH" ${jacky-agent}/bin/jacky; then
        ...
        extra-dependency-groups = let
          jackyWithGroups = jacky-agent.override {
            extraDependencyGroups = [ "honcho" ];
          };
        in pkgs.runCommand "jacky-extra-dependency-groups" { } ''
          set -e
          ...
          echo "derivation: ${jackyWithGroups}"
          echo "venv: ${jackyWithGroups.jackyVenv}"
        ...
        messaging-variant = pkgs.runCommand "jacky-messaging-variant" { } ''
          set -e
          echo "=== Checking discord.py importable from messaging variant ==="
          ${self'.packages.messaging.jackyVenv}/bin/python3 -c \
        ...
        in pkgs.runCommand "jacky-config-roundtrip" {
          ...
          merge_and_load() {
            local jacky_home="$1"
            export JACKY_HOME="$jacky_home"
            ${configMergeScript} ${nixSettings} "$jacky_home/config.yaml"
            ${jackyVenv}/bin/python3 -c '
import json, sys

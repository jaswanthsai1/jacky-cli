# nix/nixosModules.nix — NixOS module for jacky-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: jacky runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $JACKY_HOME/.env
# and read by jacky at startup — no container recreation needed for env changes.
#
# Tool resolution: the jacky wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.jacky-agent;
    effectivePackage =
    ...
    jacky-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-jacky-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "jacky-config-attrs";
      description = "Jacky YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "jacky-config.yaml" configJson;
    configFile = if cfg.configFile != null then cfg.configFile else generatedConfigFile;

    configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

    # config.yaml mode: group-writable (0660) when interactive users share this
    # JACKY_HOME via addToSystemPackages, so they can save settings through the
    ...
    documentDerivation = pkgs.runCommand "jacky-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'JACKY_DOC_EOF'\n${value}\nJACKY_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "jacky-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/jacky";
    ...
    # jacky user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "jacky-container-entrypoint" ''
      set -eu

      JACKY_UID="''${JACKY_UID:?JACKY_UID must be set}"
      JACKY_GID="''${JACKY_GID:?JACKY_GID must be set}"

      # ── Group: ensure a group with GID=$JACKY_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$JACKY_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="jacky"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$JACKY_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$JACKY_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$JACKY_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$JACKY_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="jacky"
        TARGET_HOME="/home/jacky"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$JACKY_UID" -g "$JACKY_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$JACKY_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$JACKY_UID:$JACKY_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure JACKY_HOME is owned by the target user.
      ...
      if [ -n "''${JACKY_HOME:-}" ] && [ -d "$JACKY_HOME" ]; then
        find "$JACKY_HOME" \! -user "$JACKY_UID" -exec chown "$JACKY_UID:$JACKY_GID" {} +
      fi
      ...
      if [ ! -f /var/lib/jacky-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/jacky-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/jacky ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/jacky
        chmod 0440 /etc/sudoers.d/jacky
      fi
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$JACKY_UID" --regid="$JACKY_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $JACKY_HOME/.env.
    ...
    # Default: /var/lib/jacky/workspace → /data/workspace.
  ...
  in {
    options.services.jacky-agent = with lib; {
      enable = mkEnableOption "Jacky Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = jacky-agent;
        description = "The jacky-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/jacky";
        description = "State directory. Contains .jacky/ subdir (JACKY_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative Jacky config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $JACKY_HOME/.env at activation time.
          Jacky reads this file on every startup via load_jacky_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $JACKY_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $JACKY_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `jacky gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the jacky user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the jacky plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. Jacky discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          jacky_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as jacky (python312).
        '';
        example = literalExpression ''
          [
      ...
      extraDependencyGroups = mkOption {
        ...
        description = ''
          ...
          Use this for optional extras already declared in jacky-agent's
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the jacky CLI to environment.systemPackages and export
          JACKY_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.jacky symlink to the service
            stateDir. These users are automatically added to the jacky group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.jacky-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the jacky CLI to system PATH and export JACKY_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.jacky/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.JACKY_HOME = "${cfg.stateDir}/.jacky";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.jacky-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the jacky user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.jacky-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed jacky
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure jacky is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.jacky        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jacky/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jacky/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jacky/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jacky/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.jacky/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."jacky-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.jacky
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.jacky ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.jacky ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.jacky -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.jacky/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.jacky/$_subdir"
            chmod 2770 "${cfg.stateDir}/.jacky/$_subdir"
            find "${cfg.stateDir}/.jacky/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          # jacky-group users can save settings via the CLI/TUI, else 0640).
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m ${configYamlMode} -D ${configFile} ${cfg.stateDir}/.jacky/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.jacky/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jacky/config.yaml
            chmod ${configYamlMode} ${cfg.stateDir}/.jacky/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.jacky/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jacky/.managed
          chmod 0644 ${cfg.stateDir}/.jacky/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.jacky/.container-mode <<'JACKY_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    jacky_bin=${containerDataDir}/current-package/bin/jacky
    JACKY_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jacky/.container-mode
            chmod 0644 ${cfg.stateDir}/.jacky/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.jacky/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.jacky";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.jacky" ]; then
                  rm -f "${symlinkPath}"
                  echo "jacky-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.jacky -> stateDir/.jacky for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.jacky";
                target = "${cfg.stateDir}/.jacky";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "jacky-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.jacky/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.jacky/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.jacky/auth.json
              fi
            ''}
          ''}
          ...
          # Jacky reads $JACKY_HOME/.env at startup via load_jacky_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.jacky/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'JACKY_NIX_ENV_EOF'
    ${envFileContent}
    JACKY_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.jacky/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.jacky/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.jacky/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.jacky-agent = {
          description = "Jacky Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $JACKY_HOME/.env by the activation script. load_jacky_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/jacky"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the jacky group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.jacky-agent = {
          description = "Jacky Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              JACKY_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              JACKY_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env JACKY_UID="$JACKY_UID" \
                --env JACKY_GID="$JACKY_GID" \
                --env JACKY_HOME=${containerDataDir}/.jacky \
                --env JACKY_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/jacky gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}

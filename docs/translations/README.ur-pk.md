...
<p align="center">
  <img src="../../assets/banner.jpeg" alt="Jacky Agent" width="100%">
</p>

# ہرمیس ایجنٹ ☤ (Jacky Agent)
...
> **اینڈرائیڈ (Android) / ٹرمکس (Termux):** ٹیسٹ کیا گیا مینوئل طریقہ [Termux گائیڈ](https://jaswanthsai1.github.io/jacky-cli/getting-started/termux) میں موجود ہے۔ ٹرمکس پر ہرمیس ایک مخصوص `.[termux]` ایکسٹرا انسٹال کرتا ہے کیونکہ مکمل `.[all]` ایکسٹرا میں ایسی وائس ڈیپینڈینسیز شامل ہیں جو اینڈرائیڈ کے ساتھ مطابقت نہیں رکھتیں۔
>
> **ونڈوز (Windows):** مقامی ونڈوز کی مکمل سپورٹ موجود ہے — اوپر دی گئی پاور شیل کی کمانڈ سب کچھ انسٹال کر دیتی ہے۔ اگر آپ WSL2 استعمال کرنا چاہتے ہیں، تو لینکس کی کمانڈ وہاں کام کرتی ہے۔ مقامی ونڈوز میں انسٹالیشن `%LOCALAPPDATA%\jacky` میں ہوتی ہے؛ جبکہ WSL2 میں لینکس کی طرح `~/.jacky` میں ہوتی ہے۔ ہرمیس کا وہ واحد فیچر جسے فی الحال خاص طور پر WSL2 کی ضرورت ہے وہ براؤزر پر مبنی ڈیش بورڈ چیٹ پین ہے (یہ POSIX PTY استعمال کرتا ہے — کلاسک CLI اور گیٹ وے دونوں مقامی طور پر چلتے ہیں)۔
...
```bash
jacky              # انٹرایکٹو CLI — بات چیت شروع کریں
jacky model        # اپنا LLM پرووائیڈر اور ماڈل منتخب کریں
jacky tools        # کنفیگر کریں کہ کون سے ٹولز ایکٹو ہیں
jacky config set   # انفرادی کنفگ (config) ویلیوز سیٹ کریں
jacky gateway      # میسجنگ گیٹ وے شروع کریں (ٹیلی گرام، ڈسکارڈ، وغیرہ)
jacky setup        # مکمل سیٹ اپ وزرڈ چلائیں (یہ سب کچھ ایک ساتھ کنفیگر کر دے گا)
jacky claw migrate # OpenClaw سے مائیگریٹ کریں (اگر آپ OpenClaw سے آ رہے ہیں)
jacky update       # لیٹسٹ ورژن پر اپ ڈیٹ کریں
jacky doctor       # کسی بھی مسئلے کی تشخیص کریں
```

</div>

📖 **[مکمل دستاویزات →](https://jaswanthsai1.github.io/jacky-cli/)**
...
یہ آپ کو OAuth کے ذریعے لاگ ان کرواتا ہے، Nous کو آپ کا پرووائیڈر مقرر کرتا ہے، اور ٹول گیٹ وے کو آن کر دیتا ہے۔ `jacky portal info` کمانڈ استعمال کر کے آپ کسی بھی وقت چیک کر سکتے ہیں کہ کون کون سی سروسز منسلک ہیں۔ مکمل تفصیلات [Tool Gateway دستاویزات کے صفحے](https://jaswanthsai1.github.io/jacky-cli/user-guide/features/tool-gateway) پر موجود ہیں۔
...
ہرمیس کے دو بنیادی انٹر فیس ہیں: آپ ٹرمینل UI کو `jacky` کے ساتھ شروع کریں، یا گیٹ وے چلا کر اس کے ساتھ ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، یا ای میل کے ذریعے بات کریں۔ جب آپ کسی بات چیت میں ہوتے ہیں، تو بہت سی سلیش (slash) کمانڈز دونوں انٹرفیسز میں ایک جیسی ہوتی ہیں۔

<div dir="ltr">

| کارروائی (Action)                         | سی ایل آئی (CLI)                              | میسجنگ پلیٹ فارمز (Messaging platforms)                                          |
| --------------------------------------- | --------------------------------------------- | -------------------------------------------------------------------------------- |
| بات چیت شروع کریں                       | `jacky`                                      | `jacky gateway setup` اور `jacky gateway start` چلائیں، پھر بوٹ کو میسج بھیجیں |
...
مکمل کمانڈ لسٹ کے لیے، [CLI گائیڈ](https://jaswanthsai1.github.io/jacky-cli/user-guide/cli) اور [میسجنگ گیٹ وے گائیڈ](https://jaswanthsai1.github.io/jacky-cli/user-guide/messaging) دیکھیں۔

---

## دستاویزات (Documentation)

تمام دستاویزات **[jaswanthsai1.github.io/jacky-cli](https://jaswanthsai1.github.io/jacky-cli/)** پر موجود ہیں:

<div dir="ltr">

| سیکشن (Section)                                                                                     | تفصیل (What's Covered)                                     |
| --------------------------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| [فوری آغاز (Quickstart)](https://jaswanthsai1.github.io/jacky-cli/getting-started/quickstart)     | انسٹالیشن → سیٹ اپ → 2 منٹ میں پہلی بات چیت شروع کریں       |
| [CLI کا استعمال](https://jaswanthsai1.github.io/jacky-cli/user-guide/cli)                         | کمانڈز، کی بائنڈنگز (keybindings)، پرسنلٹیز (personalities)، سیشنز |
| [کنفیگریشن (Configuration)](https://jaswanthsai1.github.io/jacky-cli/user-guide/configuration)    | کنفگ فائل، پرووائیڈرز، ماڈلز، اور تمام آپشنز               |
| [میسجنگ گیٹ وے](https://jaswanthsai1.github.io/jacky-cli/user-guide/messaging)                    | ٹیلی گرام، ڈسکارڈ، سلیک، واٹس ایپ، سگنل، ہوم اسسٹنٹ         |
| [سیکیورٹی (Security)](https://jaswanthsai1.github.io/jacky-cli/user-guide/security)              | کمانڈ کی منظوری، DM پیئرنگ (pairing)، کنٹینر آئسولیشن       |
| [ٹولز اور ٹول سیٹس](https://jaswanthsai1.github.io/jacky-cli/user-guide/features/tools)          | 40 سے زائد ٹولز، ٹول سیٹ سسٹم، ٹرمینل بیک اینڈز             |
| [مہارتوں کا سسٹم (Skills System)](https://jaswanthsai1.github.io/jacky-cli/user-guide/features/skills)| پروسیجرل (Procedural) میموری، سکلز ہب، نئی مہارتیں بنانا    |
| [میموری (Memory)](https://jaswanthsai1.github.io/jacky-cli/user-guide/features/memory)            | مستقل میموری، یوزر پروفائلز، بہترین طریقہ کار              |
| [MCP انضمام (Integration)](https://jaswanthsai1.github.io/jacky-cli/user-guide/features/mcp)      | صلاحیتوں کو بڑھانے کے لیے کسی بھی MCP سرور کو جوڑیں        |
| [کرون (Cron) شیڈیولنگ](https://jaswanthsai1.github.io/jacky-cli/user-guide/features/cron)         | پلیٹ فارم ڈیلیوری کے ساتھ شیڈول کیے گئے کام                 |
| [کانٹیکسٹ (Context) فائلز](https://jaswanthsai1.github.io/jacky-cli/user-guide/features/context-files)| پروجیکٹ کا سیاق و سباق (context) جو ہر بات چیت پر اثر انداز ہوتا ہے |
| [آرکیٹیکچر (Architecture)](https://jaswanthsai1.github.io/jacky-cli/developer-guide/architecture) | پروجیکٹ کا ڈھانچہ، ایجنٹ لوپ، اہم کلاسز                    |
| [تعاون (Contributing)](https://jaswanthsai1.github.io/jacky-cli/developer-guide/contributing)     | ڈیویلپمنٹ سیٹ اپ، PR کا طریقہ کار، کوڈنگ کا انداز          |
| [CLI حوالہ جات (Reference)](https://jaswanthsai1.github.io/jacky-cli/reference/cli-commands)      | تمام کمانڈز اور فلیگز (flags)                              |
| [انوائرمنٹ ویری ایبلز](https://jaswanthsai1.github.io/jacky-cli/reference/environment-variables)  | مکمل انوائرمنٹ ویری ایبل حوالہ جات                         |
...
**پہلی بار سیٹ اپ کے دوران:** سیٹ اپ وزرڈ (`jacky setup`) خود بخود `~/.openclaw` کو پہچان لیتا ہے اور کنفیگریشن شروع ہونے سے پہلے مائیگریٹ (migrate) کرنے کا آپشن دیتا ہے۔
...
```bash
jacky claw migrate              # انٹرایکٹو مائیگریشن (مکمل پری سیٹ)
jacky claw migrate --dry-run    # جائزہ لیں کہ کیا کیا مائیگریٹ ہوگا
jacky claw migrate --preset user-data   # حساس معلومات (secrets) کے بغیر مائیگریٹ کریں
jacky claw migrate --overwrite  # موجودہ متصادم فائلوں کو اوور رائٹ کریں
```
...
- **مہارتیں (Skills)** — صارف کی بنائی گئی مہارتیں → `~/.jacky/skills/openclaw-imports/`
...
تمام آپشنز دیکھنے کے لیے `jacky claw migrate --help` استعمال کریں، یا انٹرایکٹو ایجنٹ کی مدد سے مائیگریٹ کرنے کے لیے `openclaw-migration` سکل کا استعمال کریں (جس میں ڈرائی رن (dry-run) پریویوز شامل ہیں)۔

---

## تعاون کریں (Contributing)

ہم آپ کے تعاون کا خیرمقدم کرتے ہیں! ڈیویلپمنٹ سیٹ اپ، کوڈ کے انداز اور PR کے طریقہ کار کے لیے براہ کرم ہماری [Contributing گائیڈ](https://jaswanthsai1.github.io/jacky-cli/developer-guide/contributing) دیکھیں۔

معاونین (contributors) کے لیے فوری آغاز — کلون (clone) کریں اور `setup-jacky.sh` چلائیں:
...
cd jacky-agent
./setup-jacky.sh     # uv کو انسٹال کرتا ہے، venv بناتا ہے، .[all] کو انسٹال کرتا ہے، اور ~/.local/bin/jacky کا سیم لنک (symlink) بناتا ہے
./jacky              # خود بخود venv کی شناخت کرتا ہے، پہلے `source` کرنے کی ضرورت نہیں
```
...
- 🐛 [مسائل (Issues)](https://github.com/jaswanthsai1/jacky-cli/issues)

...
> **Nota:** En Windows nativo, Jacky funciona sin WSL — la CLI, el gateway, la TUI y las herramientas funcionan de forma nativa. Si prefieres usar WSL2, el comando de Linux/macOS de arriba también funciona allí. ¿Encontraste un error? Por favor [crea un issue](https://github.com/jaswanthsai1/jacky-cli/issues).
...
```bash
jacky              # CLI interactiva — inicia una conversación
jacky model        # Elige tu proveedor y modelo LLM
jacky tools        # Configura qué herramientas están habilitadas
jacky config set   # Establece valores de configuración individuales
jacky gateway      # Inicia el gateway de mensajería (Telegram, Discord, etc.)
jacky setup        # Ejecuta el asistente de configuración completo
jacky claw migrate # Migra desde OpenClaw (si vienes de OpenClaw)
jacky update       # Actualiza a la última versión
...
Jacky funciona con cualquier proveedor que quieras — eso no cambiará. Pero si prefieres no recopilar cinco claves API separadas para el modelo, búsqueda web, generación de imágenes, TTS y un navegador en la nube, **[Nous Portal](https://portal.nousresearch.com)** las cubre todas bajo una sola suscripción:
...
Jacky tiene dos puntos de entrada: inicia la interfaz de terminal con `jacky`, o ejecuta el gateway y habla con él desde Telegram, Discord, Slack, WhatsApp, Signal o Email. Una vez en una conversación, muchos comandos de barra son compartidos entre ambas interfaces.

| Acción                              | CLI                                           | Plataformas de mensajería                                                         |
| ----------------------------------- | --------------------------------------------- | --------------------------------------------------------------------------------- |
| Empezar a chatear                   | `jacky`                                      | Ejecuta `jacky gateway setup` + `jacky gateway start`, luego envía un mensaje al bot |
...
Si vienes de OpenClaw, Jacky puede importar automáticamente tu configuración, memorias, habilidades y claves API.

**Durante la configuración inicial:** El asistente de configuración (`jacky setup`) detecta automáticamente `~/.openclaw` y ofrece migrar antes de que comience la configuración.

**En cualquier momento después de instalar:**

```bash
jacky claw migrate              # Migración interactiva (preset completo)
jacky claw migrate --dry-run    # Vista previa de qué se migraría
jacky claw migrate --preset user-data   # Migrar sin secretos
jacky claw migrate --overwrite  # Sobreescribir conflictos existentes
```
...
- **Habilidades** — habilidades creadas por el usuario → `~/.jacky/skills/openclaw-imports/`
...
Consulta `jacky claw migrate --help` para todas las opciones, o usa la habilidad `openclaw-migration` para una migración guiada interactiva por el agente con vistas previas de dry-run.
...
Inicio rápido para colaboradores — clona y comienza con `setup-jacky.sh`:

```bash
git clone https://github.com/jaswanthsai1/jacky-cli.git
cd jacky-agent
./setup-jacky.sh     # instala uv, crea venv, instala .[all], enlaza ~/.local/bin/jacky
./jacky              # detecta automáticamente el venv, no necesitas hacer `source` primero
```
...
- 🔌 [computer-use-linux](https://github.com/avifenesh/computer-use-linux) — Servidor MCP de control de escritorio Linux para Jacky y otros hosts MCP, con árboles de accesibilidad AT-SPI, entrada Wayland/X11, capturas de pantalla y targeting de ventanas del compositor.
- 🔌 [HermesClaw](https://github.com/AaronWong1999/hermesclaw) — Puente WeChat comunitario: Ejecuta Jacky Agent y OpenClaw en la misma cuenta de WeChat.

...
When running Jacky inside Docker, the default `network_mode: host` gives the
...
#
# Usage:
#   JACKY_UID=$(id -u) JACKY_GID=$(id -g) docker compose up -d
#
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=jacky,jacky-dashboard,localhost
...
docker compose exec gateway \
  curl -sf --max-time 5 http://jacky-dashboard:9119/health && echo "OK: internal reachable" || echo "FAIL"

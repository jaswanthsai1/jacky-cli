...
Jacky middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Jacky'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Jacky calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Jacky retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Jacky guardrails,
...
- `telemetry_schema_version`: currently `jacky.observer.v1`
- `middleware_schema_version`: currently `jacky.middleware.v1`
...
Jacky stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Jacky runs
them as a nested chain in registration order. Middleware failures are fail-open:
Jacky logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Jacky applies middleware in this order:
...
For each tool call, Jacky applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Jacky pre-execution path against the effective arguments:
...
```bash
jacky plugins enable <plugin-name>
```

For isolated local testing, use one `JACKY_HOME` for plugin enablement and the
agent run:

```bash
export JACKY_HOME=/tmp/jacky-middleware-test
mkdir -p "$JACKY_HOME"
jacky plugins enable <plugin-name>
jacky chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run jacky plugins enable <plugin-name>
uv run jacky chat --query 'Reply exactly ok'
```
...
def tag_llm_request(**kwargs):
    ...
    extra_body.setdefault("metadata", {})["jacky_middleware_demo"] = True
...
def normalize_terminal_workdir(**kwargs):
    ...
    args.setdefault("workdir", "/tmp/jacky-middleware-demo")
    return {
        "args": args,
        "source": "middleware-demo",
...
Return the same response shape Jacky expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Jacky treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Jacky preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Jacky does not convert downstream

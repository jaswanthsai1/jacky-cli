...
Jacky supports multiple gateway processes running concurrently — one per profile
...
needed. On every other profile gateway, add to `~/.jacky/config.yaml`:
...
Or set the env var: `JACKY_KANBAN_DISPATCH_IN_GATEWAY=false`

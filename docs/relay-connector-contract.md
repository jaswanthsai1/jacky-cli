...
This document is the formal interface between the **Jacky gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `jacky gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`jacky gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `jacky` CLI.

...
The canonical transcript store is SQLite via `SessionDB` (from `jacky_state`). The
...
Counts consecutive restarts via a JSON file (`{JACKY_HOME}/restart_counts.json`). If a
...
This prevents unwanted auto-resets after `jacky update`, `jacky gateway restart`,
or `/restart`.
